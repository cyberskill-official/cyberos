#!/usr/bin/env python3
"""TASK-DOCS-004: migrate docs/tasks to folder-per-task layout.

<module>/TASK-STEM.md        -> <module>/TASK-STEM/spec.md
<module>/TASK-STEM.audit.md  -> <module>/TASK-STEM/audit.md
TASK-STEM.md (root-level)    -> <module>/TASK-STEM/spec.md   (module = frontmatter `module:`,
                              else the task id segment TASK-<SEG>-..., else `misc`)
...
def _default_root():
    try:
        ...
        return Path.cwd()
# portable (task 1.0.0 kit): --root <dir> targets any repo; default = git toplevel of cwd, falling
...
def _module_for(f):
    """Module for a ROOT-level flat task: frontmatter `module:` > task id segment > misc."""
    try:
        text = f.read_text()
    ...
    mid = re.match(r"TASK-([A-Za-z0-9]+)-", f.name)
    return _sanitize_mod(mid.group(1)) if mid else "misc"

def migrate():
    moved, summary, renames = 0, {}, []
    # 1) root-level flat tasks (docs/tasks/TASK-*.md) - relocate into a module folder
    #    so the folder-per-task rule AND the status hub (which scans <module>/TASK-*/spec.md) see them.
    for f in sorted(task.glob("TASK-*.md")):
        if not f.is_file():
            continue
        is_audit = f.name.endswith(".audit.md")
        stem = f.name[:-len(".audit.md")] if is_audit else f.stem
        mod = _module_for(f)
        target = task / mod / stem / ("audit.md" if is_audit else "spec.md")
        if target.exists():
            continue
    ...
    # 2) module-level flat tasks (<module>/TASK-*.md)
    for mod_dir in sorted(p for p in task.iterdir() if p.is_dir() and not p.name.startswith(("_", "."))):
        n = 0
        for f in sorted(mod_dir.glob("TASK-*.md")):
...
REF_MD = re.compile(r"(docs/tasks/)([a-z][a-z0-9-]*/)(TASK-[A-Za-z0-9-]+?)(\.audit\.md|\.md)")
def new_ref(m):
    kind = "audit.md" if m.group(4) == ".audit.md" else "spec.md"
    return f"{m.group(1)}{m.group(2)}{m.group(3)}/{kind}"

denied = []
SAME_DIR = re.compile(r"(\]\(|\`)(TASK-[A-Za-z0-9-]+?)(\.audit\.md|\.md)(\)|\`)")
def new_same(m):
    kind = "audit.md" if m.group(3) == ".audit.md" else "spec.md"
    return f"{m.group(1)}{m.group(2)}/{kind}{m.group(4)}"
REL_SIB = re.compile(r"(\]\()(\.\./)([a-z][a-z0-9-]*/)(TASK-[A-Za-z0-9-]+?)(\.audit\.md|\.md)(\))")
def new_rel(m):  # inside a moved spec, ../<mod>/TASK-x.md is now two levels up
...
def sweep(renames=()):
    ...
    # filenames only inside files that sit directly in the task root, e.g. BACKLOG.md).
