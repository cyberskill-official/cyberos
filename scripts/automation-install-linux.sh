...
chmod +x "$REPO_ROOT/scripts/automation/cyberos-nightly.sh" \
         "$REPO_ROOT/scripts/automation/cyberos-weekly.sh"
...
ExecStart=/bin/bash ${REPO_ROOT}/scripts/automation/cyberos-nightly.sh ${TARGET}
...
Environment=PATH=${PYTHON_DIR}:/usr/local/bin:/usr/bin:/bin
Environment=CYBEROS_PROJECT=${TARGET}
ExecStart=/bin/bash ${REPO_ROOT}/scripts/automation/cyberos-weekly.sh ${TARGET}
WorkingDirectory=${TARGET}
UNIT
...
9 1 * * *     CYBEROS_PROJECT=${TARGET} ${REPO_ROOT}/scripts/automation/cyberos-nightly.sh ${TARGET}    # cyberos-automation-marker
7 2 * * 0     CYBEROS_PROJECT=${TARGET} ${REPO_ROOT}/scripts/automation/cyberos-weekly.sh ${TARGET}    # cyberos-automation-marker
CRON
    )
    {
