#!/usr/bin/env python3
"""Portable task frontmatter repairer (TASK-DOCS-004 lineage, vendored for target repos).

Iteratively quotes exactly the line strict-YAML trips on (minimal formatting-only edits,
never value semantics). Usage: repair_task_yaml.py [--root <dir>] [--glob '*/TASK-*/spec.md']
Default glob covers BOTH layouts (flat TASK-*.md and folder-per-task spec.md).
"""
import re, sys
...
try:
    ...
    print("repair_task_yaml: needs pyyaml (pip install pyyaml)", file=sys.stderr); sys.exit(2)
...
repaired, failed = [], []
targets = (list(task.glob("TASK-*.md"))                                     # root-level flat (pre-migration)
           + list(task.glob("*/TASK-*.md")) + list(task.glob("*/TASK-*/spec.md")) + list(task.glob("*/TASK-*/audit.md")))
...
print(f"repair_task_yaml: repaired {len(repaired)} file(s) ({sum(c for _, c in repaired)} lines quoted); {len(failed)} need manual attention")
