...
def regen_backlog():
    ...
    for f in sorted(FR.glob("*/FR-*.md")):
        ...
        if not fm:
            continue
        mod = f.parent.name
        mods.setdefault(mod, []).append((f.stem, str(fm.get("status", "")).strip(), str(fm.get("title", ""))))
    totals = {}
    out = ["# CyberOS FR backlog (regenerated 2026-07-08)", "",
           "Source of truth = FR frontmatter. This file lists ONLY remaining work (active statuses);",
           "done / closed / on_hold are counted in each module header. Regenerated by",
           "scripts/migrate_improvement_to_fr.py --backlog after the improvement-backlog migration.", ""]
    body = []
    for mod in sorted(mods):
        ...
        body.append("")
        remaining = [(stem, st, title) for stem, st, title in rows if st in ACTIVE]
        if remaining:
            for stem, st, title in sorted(remaining):
                body.append(f"- [{st}] {stem} - {title}")
        else:
