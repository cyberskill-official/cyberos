...
(memory MEM-*, chat T-*, deep-audit IMP-*) into tasks as FRs with
...
docs/tasks/BACKLOG.md from all FR frontmatter. Does NOT delete the old
...
def regen_backlog():
    ...
    for f in sorted(FR.glob("*/FR-*/spec.md")):
        ...
        fm = read_fm(f)
        if not fm:
            unparseable.append(str(f))   # TASK-DOCS-004 §1 #4: the skip is LOUD, never silent
            continue
...
if __name__ == "__main__":
    import sys
    if "--backlog" in sys.argv:
        # Regenerate docs/tasks/BACKLOG.md from FR frontmatter only.
