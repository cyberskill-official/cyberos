...
def emit_task(task_id: str) -> int:
    ...
    print(f"# {task_id}  module={module}  spec={path}")
    if unresolved:
    ...
    if py:
        ...
        print(f"""  - id: acceptance-{task_id.lower()}
    description: held-out cited test(s) for {task_id}
    ...
    for r in rs:
        ...
        stem = Path(r).stem
        print(f"""  - id: acceptance-{task_id.lower()}-{stem}
    description: held-out cited test for {task_id}
