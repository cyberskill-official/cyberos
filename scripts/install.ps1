...
$cyberos_dst = Join-Path $Target "cyberos"
if ((-not (Test-Path $cyberos_dst)) -or $Force) {
    Copy-Item -Path (Join-Path $Repo "cyberos") -Destination $cyberos_dst -Recurse -Force
...
if ($WithAutomation) {
    ...
    & (Join-Path $Repo "scripts\automation-install.ps1") -Target $Target
    Write-Host ""
}
...
if ($WithPreCommit) {
    ...
    if (Test-Path $hook_dir) {
        Copy-Item -Path (Join-Path $Repo "scripts\hooks\pre-commit") `
