...
def migrate():
    ...
    # 1) root-level flat FRs (docs/feature-requests/FR-*.md) - relocate into a module folder
...
def sweep(renames=()):
    ...
    exact = [(f"docs/feature-requests/{old}", f"docs/feature-requests/{new}") for old, new in renames]
