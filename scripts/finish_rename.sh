...
#     tools/install/install.sh .   ->  "not an assembled payload (no cuo/)"
#     dist/cyberos/install.sh .         ->  correct
# build.sh assembles tools/install/ -> dist/cyberos/; install.sh then lays that
...
run "bash tools/install/build.sh"
run "bash dist/cyberos/install.sh ."           # refreshes .cyberos/ from dist/cyberos
run "bash tools/install/check-chain-coverage.sh dist/cyberos"
...
if command -v node >/dev/null 2>&1; then
  ...
  run "bash tools/install/lib/status-page.sh ."
