...
1. Normalize legacy status tokens to the canonical 10-state enum
   (modules/skill/contracts/task/STATUS-REFERENCE.md). The 2026-05 wave
...
def plan_file(path: Path, lacks_impl: set[str]) -> dict | None:
    ...
    task_id = "-".join(task_id) if task_id else path.stem
    for idx in range(start, end + 1):
        ...
        if canon in PAST_IMPLEMENTING:
            new = "ready_to_implement" if task_id in lacks_impl else RESET_TARGET
        else:
            new = canon
        if new == raw and not unknown:
            return {"path": path, "task_id": task_id, "old": raw, "new": new,
    ...
    return {"path": path, "task_id": task_id, "old": None, "new": None,
...
def main() -> int:
    ...
    for k, v in sorted(transitions.items()):
        print(f"  {v:4d}  {k}")
    if unknown:
        print(f"UNKNOWN status values (review): {[p['task_id'] for p in unknown]}")
    if no_status:
