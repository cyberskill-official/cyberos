...
#   last-gate-failure.json  written by run-gates.sh on failure (IMP-011); not vendored
...
_G5_EXCLUDE='^\.cyberos/(AGENT-ENTRY(\.md)?|status-site(/|$)|status\.html|memory/store|gates\.env|config\.yaml|last-gate-failure\.json|memory/sessions|sessions/|docs/tasks)'
