...
for f in AGENTS.md INTEROP.md memory.schema.json memory.invariants.yaml; do
    src="$SOURCE_REPO/docs/memory/$f"
    dst="$TARGET/docs/memory/$f"
...
if [[ ! -d "$TARGET/cyberos" || "$FORCE" == "1" ]]; then
    cp -r "$SOURCE_REPO/cyberos" "$TARGET/cyberos"
    echo "  ✓ $TARGET/cyberos/"
fi
echo
...
if [[ "$NO_AGENT_SYMLINK" == "0" ]]; then
    ...
    for link_name in AGENTS.md CLAUDE.md; do
        if [[ -e "$link_name" && "$FORCE" != "1" ]]; then
            ...
            echo "  ✓ $link_name → docs/memory/AGENTS.md"
        fi
    done
    echo
else
