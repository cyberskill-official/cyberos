...
def regen_backlog():
    ...
    for f in sorted(task.glob("*/TASK-*/spec.md")):
        ...
        # `type` is the schema field since the 2026-07-14 migration (`class` was folded into
    ...
    out = ["# CyberOS task backlog (regenerated 2026-07-09)", "",
           ...
           "task in every status (TASK-IMP-091); per-status counts sit in each module header. ONE backlog for every type:",
           "`type: improvement` rows are tagged `(improvement)`, untagged rows carry the other types (`feature` / `bug` / `chore`).",
