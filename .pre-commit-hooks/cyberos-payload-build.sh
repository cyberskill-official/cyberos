...
# artifact - nothing is committed. It keeps tools/cyberos-install/init.sh vendoring
...
# tools/cyberos-install/check-version-sync.sh after the rebuild (TASK-IMP-068). The trigger
...
if ! git diff --cached --name-only | grep -Eq \
  '^(modules/cuo/|modules/skill/|modules/memory/memory\.(schema\.json|invariants\.yaml)|AGENTS\.md|tools/cyberos-install/|tools/caf/|scripts/caf_gate\.sh|VERSION)'; then
  exit 0
fi

echo "cyberos: a vendored source changed - rebuilding dist/cyberos ..."
bash "$root/tools/cyberos-install/build.sh" >/dev/null
