...
# Schema for the derived index. Versioned so a schema change becomes a
# clean rebuild rather than a silent migration. Replay from binlog is
# idempotent; bumping ``_INDEX_SCHEMA_VERSION`` triggers a full rebuild
# on next open.
...
def open_index(store_fingerprint: str) -> sqlite3.Connection:
    ...
    # Stash schema version for future rebuild-on-bump logic.
    conn.execute(
        "INSERT INTO sync_state(k, v) VALUES('index_schema_version', ?) "
...
def write_index_manifest(store: Path, schema_version: int, last_seq: int) -> None:
    ...
    export) and records (a) the schema version that the on-disk binlog
    matches and (b) the seq up to which the external SQLite index was
    last sync'd. On open, code can compare this to ``cache_dir()/<fp>/...``
    state to decide whether to rebuild from scratch.
    """
    target = store / "index" / "manifest.json"
    target.parent.mkdir(parents=True, exist_ok=True)
    body = json.dumps(
        {"schema_version": schema_version, "last_applied_seq": last_seq},
        sort_keys=True,
        separators=(",", ":"),
    ).encode("utf-8")
