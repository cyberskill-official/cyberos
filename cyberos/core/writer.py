...
* The six file ops (view / create / str_replace / insert / delete /
  rename) all decompose into :class:`AuditRecord` variants; nothing else
  is permitted to append to the ledger.
"""
...
def normalise_chain(value: str) -> str:
    """Strip legacy ``sha256:`` prefix; validate 64-hex.

    Module-level so both :class:`Writer` and CLI verify code can call it.
    """
    if value.startswith("sha256:"):
        value = value[len("sha256:"):]
    if len(value) != 64:
        raise ValueError(
            f"expected 64-hex chain, got {len(value)} chars: {value!r}"
        )
    bytes.fromhex(value)  # raises on non-hex
    return value


def resolve_initial_chain_from_manifest(store: Path) -> str:
    """Read ``<store>/manifest.json:migration.legacy_last_chain`` if present.

    Precedence:

    1. Manifest's ``legacy_last_chain`` (set by ``cyberos_migrate_v2.py``
       at Phase-4 cut-over) — the bridge from the legacy JSONL ledger to
       the new binlog.
    2. :data:`_GENESIS_CHAIN` (64 zeros).

    Used by both :class:`Writer._resolve_initial_chain` and by the CLI's
    ``verify`` command so the first record's ``prev_chain`` is correctly
    checked against the legacy chain tip rather than GENESIS.

    Returns 64-hex; raises ``ValueError`` if the manifest holds a
    malformed bridge value (silent fallback to GENESIS would corrupt
    the chain).
    """
    manifest_path = store / "manifest.json"
    if not manifest_path.is_file():
        return _GENESIS_CHAIN
    import json  # noqa: WPS433 — local; module is imported by hot paths
    try:
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    except (OSError, ValueError):
        return _GENESIS_CHAIN
    legacy_last = manifest.get("migration", {}).get("legacy_last_chain")
    if not legacy_last:
        return _GENESIS_CHAIN
    return normalise_chain(legacy_last)
    ...
    * If the HEAD update partially completes, the atomic ``tmp + sync +
    ...
    def __init__(
        ...
        *,
        ...
        initial_chain:
            64-hex chain to use as ``prev_chain`` for the first record
            when the binlog is empty. Overrides anything read from
            manifest. Used for tests; production callers should rely on
            the manifest-driven legacy bridge.
        """
        self.store = store
        ...
        self._initial_chain_override = initial_chain
    ...
    def _resolve_initial_chain(self) -> str:
        """Compute the starting prev_chain for an empty binlog."""
        if self._initial_chain_override:
            return normalise_chain(self._initial_chain_override)
        return resolve_initial_chain_from_manifest(self.store)
    ...
    def _recover_tail(self) -> None:
        ...
        Chain continuity across the legacy→v2 boundary: if the binlog is
        empty AND there's an ``initial_chain`` (from ``__init__`` override
        or from ``manifest.migration.legacy_last_chain``), use it as the
        starting ``prev_chain`` so the new ledger extends the legacy one.
        """
        ...
        if not self._binlog_path.exists() or self._binlog_path.stat().st_size == 0:
            self._last_chain = self._resolve_initial_chain()
            self._last_seq = 0
            return
