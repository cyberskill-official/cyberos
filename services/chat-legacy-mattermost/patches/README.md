...
Authentication is no longer done by a build patch. As of TASK-CHAT-013, CHAT
federates to the CyberOS OIDC provider (TASK-AUTH-110) through Mattermost's own
native connector - server configuration, not a patch and not a plugin. See
`../deploy/oidc-sso-config.md`.

`superseded/` holds the two TASK-CHAT-002 patches (`010-disable-builtin-auth`,
