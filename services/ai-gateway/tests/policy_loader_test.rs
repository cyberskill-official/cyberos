...
async fn ac1_valid_yaml_loads_and_matches() {
    ...
    let p = policy::load_for_tenant("org:test-a").await.expect("load_for_tenant");
...
async fn ac2_missing_yaml_returns_policy_missing() {
    ...
    assert!(matches!(err, PolicyError::PolicyMissing { ref tenant_id } if tenant_id == "org:nobody"));
}
    ...
    loop {
        ...
        if p.ai_policy.monthly_cap_usd == dec!(200) {
            assert!(start.elapsed() < Duration::from_millis(2_000),
                "hot reload latency {:?}", start.elapsed());
            return;
        }
...
async fn ac9_concurrent_1000_reads_under_1s() {
    ...
    assert!(start.elapsed() < Duration::from_secs(3),
        "1000×100 reads took {:?}, expected < 3s on CI", start.elapsed());
}
