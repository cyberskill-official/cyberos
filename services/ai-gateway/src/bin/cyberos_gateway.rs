...
//!
//! Boot order (FR-AI-007 / FR-AI-015): initialise the per-tenant policy loader from the config dir before
//! binding the listener, so no request is served before policy is available. The provider call uses the
//! in-repo echo backend until the FR-AI-008 provider adapters land.
