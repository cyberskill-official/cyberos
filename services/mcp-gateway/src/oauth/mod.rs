...
//! introspect, discovery), the JWT minting against the FR-AUTH-004 `auth_signing_keys` / JWKS, and
//! the audience check wired into the `tools/call` hot path are the remaining slice-2 work, planned in
//! `docs/feature-requests/mcp/MCP-004-SLICE2-PLAN.md`. They use runtime-checked `sqlx::query(...)`,
