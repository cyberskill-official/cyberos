...
async fn emit(pool: &PgPool, tenant: Uuid, subject: Uuid, kind: &str, body: serde_json::Value) {
    if let Err(e) =
        cyberos_audit_chain::emit_genesis(pool, tenant, subject, kind, &body.to_string()).await
    {
...
pub async fn token_refreshed(
    ...
    emit(
        ...
        json!({ "sev": 3, "client_id": client_id, "jti": jti }),
    )
    .await;
}
...
pub async fn token_revoked(pool: &PgPool, family_id: Uuid) {
    emit(
        ...
        json!({ "sev": 2, "family_id": family_id }),
    )
    .await;
}
...
pub async fn refresh_reuse_detected(pool: &PgPool, family_id: Uuid) {
    emit(
        ...
        Uuid::nil(),
        Uuid::nil(),
        "mcp.oauth_refresh_reuse_detected",
        json!({ "sev": 1, "family_id": family_id }),
    )
    .await;
}
...
pub async fn code_reuse_detected(pool: &PgPool, client_id: Uuid) {
    emit(
        ...
        Uuid::nil(),
        Uuid::nil(),
        "mcp.oauth_code_reuse_detected",
        json!({ "sev": 2, "client_id": client_id }),
    )
    .await;
}
...
pub async fn client_registered(
    ...
    emit(
        ...
        json!({ "sev": 2, "client_id": client_id, "client_type": client_type }),
    )
    .await;
}
...
pub async fn prm_unknown_module_requested(pool: &PgPool, module: &str) {
    emit(
        ...
        Uuid::nil(),
        Uuid::nil(),
        "mcp.prm_unknown_module_requested",
        json!({ "sev": 3, "module": module }),
    )
    .await;
}
...
pub async fn elicitation_responded(pool: &PgPool, id: Uuid) {
    emit(
        ...
        Uuid::nil(),
        Uuid::nil(),
        "mcp.elicitation_responded",
        json!({ "sev": 3, "elicitation_id": id }),
    )
    .await;
}
...
pub async fn elicitation_validation_failed(pool: &PgPool, id: Uuid) {
    emit(
        ...
        Uuid::nil(),
        Uuid::nil(),
        "mcp.elicitation_validation_failed",
        json!({ "sev": 2, "elicitation_id": id }),
    )
    .await;
}
...
pub async fn task_started(pool: &PgPool, id: Uuid, tool_id: &str) {
    emit(
        ...
        Uuid::nil(),
        "mcp.task_started",
        json!({ "sev": 3, "task_id": id, "tool_id": tool_id }),
    )
    .await;
}
...
pub async fn task_failed(pool: &PgPool, id: Uuid, code: &str) {
    emit(
        ...
        Uuid::nil(),
        "mcp.task_failed",
        json!({ "sev": 2, "task_id": id, "code": code }),
    )
    .await;
}
...
pub async fn skill_name_validated(pool: &PgPool, module: &str, tool_count: usize) {
    emit(
        ...
        json!({ "sev": 3, "module": module, "tools": tool_count }),
    )
    .await;
}
...
pub async fn skill_name_rejected(pool: &PgPool, module: &str, detail: &str) {
    emit(
        ...
        json!({ "sev": 2, "module": module, "detail": detail }),
    )
    .await;
}
