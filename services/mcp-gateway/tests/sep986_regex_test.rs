...
fn valid_skill_ids_pass() {
    ...
    for id in valid {
        assert!(
            validate_sync(id).is_ok(),
            "expected '{id}' to pass, got {:?}",
            validate_sync(id)
        );
    }
}
    ...
    match err {
        NamingError::MalformedSkillId { reason, .. } => {
            assert!(
                ...
                "error should mention the missing prefix: {reason}"
            );
        }
...
fn camelcase_noun_rejected() {
    ...
    assert!(
        matches!(err, NamingError::MalformedSkillId { .. }),
        "got {err:?}"
    );
}
...
fn no_underscore_separator_rejected() {
    ...
    assert!(
        matches!(
            err,
            NamingError::MalformedSkillId { .. } | NamingError::InvalidVerb { .. }
        ),
        "got {err:?}"
    );
}
...
fn error_message_is_specific_for_missing_prefix() {
    let msg = validate_sync("calendar.list_events")
        .expect_err("should fail")
        .to_string();
    assert!(
        msg.contains("cyberos"),
        "error should mention the 'cyberos' prefix: {msg}"
    );
}

#[test]
fn error_message_is_specific_for_invalid_verb() {
    let msg = validate_sync("cyberos.email.retrieve_messages")
        .expect_err("should fail")
        .to_string();
    assert!(
        msg.contains("retrieve") || msg.contains("verb"),
        "error should name the bad verb: {msg}"
    );
}

#[test]
fn error_message_is_specific_for_unknown_module() {
    let msg = validate_sync("cyberos.unknown.list_things")
        .expect_err("should fail")
        .to_string();
    assert!(
        msg.contains("unknown"),
        "error should name the bad module: {msg}"
    );
}
