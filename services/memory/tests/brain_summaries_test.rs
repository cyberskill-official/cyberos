...
async fn rolling_summary_covers_event_range() {
    ...
    for i in 0..6 {
        let ev = env
            .append_interaction_event(env.subject_alice(), "chat.message_created", &format!("standup note {i}"))
            .await;
    ...
    .await
    .unwrap();
    assert!(row.0 <= first && row.1 >= last, "covered range must span the seeded events");
...
async fn new_event_supersedes_prior_summary_version() {
    ...
    let env = BrainTestEnv::new().await;
    for i in 0..5 {
        env.append_interaction_event(env.subject_alice(), "chat.message_created", &format!("note {i}"))
            .await;
    }
    env.run_ingest_once().await;
    env.run_summarize_once("subject", &env.subject_alice().to_string(), Some(env.subject_alice()))
    ...
    env.append_interaction_event(env.subject_alice(), "chat.message_created", "a fresh later note")
        .await;
    env.run_ingest_once().await;
    env.run_summarize_once("subject", &env.subject_alice().to_string(), Some(env.subject_alice()))
        .await;
    ...
    .await
    .unwrap();
    assert_eq!(current_versions.len(), 2, "both versions retained for audit");
...
async fn summaries_first_then_drill() {
    ...
    let env = BrainTestEnv::new().await;
    for i in 0..20 {
        env.append_interaction_event(env.subject_alice(), "chat.message_created", &format!("standup note {i}"))
            .await;
    }
    env.run_ingest_once().await;
    env.run_summarize_once("subject", &env.subject_alice().to_string(), Some(env.subject_alice()))
    ...
    let summary_only = brain::recall::recall(query("standup"), &caller, env.pool(), env.gw())
        ...
        .unwrap();
    assert!(
        summary_only.items.iter().any(|h| matches!(h.source, HitSource::Summary)),
    ...
    q.drill = true;
    let drilled = brain::recall::recall(q, &caller, env.pool(), env.gw()).await.unwrap();
    assert!(
        drilled.items.iter().any(|h| matches!(h.source, HitSource::Event)),
