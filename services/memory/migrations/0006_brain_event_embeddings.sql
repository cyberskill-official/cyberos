-- TASK-MEMORY-123 §3 / §1 #2,#3,#6,#16 — the BRAIN event-embedding table.
--
-- DEC-2721: l1_audit_log (the hash chain) stays the SYSTEM OF RECORD. This table is a DERIVED,
-- rebuildable fast lens over the TASK-MEMORY-121 interaction-event rows on that chain — never authoritative,
...
-- over the chain.
--
-- The cursor key is l1_audit_log.seq (the per-tenant monotonic seq the TASK-MEMORY-121 emitters write).
...
CREATE TABLE IF NOT EXISTS brain_event_embedding (
    tenant_id           UUID   NOT NULL,                 -- TASK-AUTH-003 tenant scope (RLS below)
    ...
    kind                TEXT   NOT NULL,                 -- interaction event kind from TASK-MEMORY-121 payload
...
-- §1 #16 — tenant isolation via RLS (TASK-AUTH-003 pattern). The recall handler sets app.tenant_id per
