...
fn require_checkout(checkout: &str) -> Result<PathBuf, String> {
    ...
    if !root.join("tools/cyberos-init/build.sh").is_file() {
        return Err(format!(
            "not a CyberOS checkout (missing tools/cyberos-init/build.sh): {checkout}"
        ));
    }
    Ok(root)
}
...
/// Build the distributable payload: `bash tools/cyberos-init/build.sh` in the checkout.
#[tauri::command]
pub async fn ops_build(checkout: String) -> Result<OpResult, String> {
    let root = require_checkout(&checkout)?;
    let script = root.join("tools/cyberos-init/build.sh");
