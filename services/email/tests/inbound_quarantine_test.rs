...
fn spam_below_threshold_is_received() {
    ...
    assert_eq!(MessageStatus::from_spam_score(4.999), MessageStatus::Received);
}

#[test]
fn spam_at_or_above_threshold_is_quarantined() {
    assert_eq!(MessageStatus::from_spam_score(5.0), MessageStatus::Quarantined);
    assert_eq!(MessageStatus::from_spam_score(7.3), MessageStatus::Quarantined);
    assert_eq!(MessageStatus::from_spam_score(15.0), MessageStatus::Quarantined);
}
...
async fn memory_blob_store_round_trip_records_body() {
    ...
    assert!(bs.head_object("cyberos-email-vn-1-bodies", "tenant/8472/abc").await);
    let body = bs.body_for("cyberos-email-vn-1-bodies", "tenant/8472/abc").await;
    assert_eq!(body.as_deref(), Some(&b"the body"[..]));

    // The same key in a different bucket DOES NOT exist (proof of bucket isolation).
    assert!(!bs.head_object("cyberos-email-eu-1-bodies", "tenant/8472/abc").await);
}
...
fn body_sha256_hex_is_64_hex_chars() {
    ...
    assert_eq!(h, "d7a8fbb307d7809469ca9abcb0082e4f8d5651e46d3cdb762d02d0bf37c9e592");
}
