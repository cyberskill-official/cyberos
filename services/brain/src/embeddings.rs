...
//! capacity independently of the brain service). This module is the HTTP
...
#[derive(Debug, Error)]
pub enum EmbedError {
    #[error("sidecar URL not configured (set BRAIN_EMBED_URL)")]
    NotConfigured,
    #[error("network: {0}")]
    Network(String),
...
impl EmbeddingClient {
    /// Construct from the `BRAIN_EMBED_URL` env var. Returns `NotConfigured`
    /// if the env var is missing — callers treat this as "skip embeddings".
    pub fn from_env() -> Result<Self, EmbedError> {
        let url = std::env::var("BRAIN_EMBED_URL").map_err(|_| EmbedError::NotConfigured)?;
