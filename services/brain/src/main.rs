...
//!   * **HTTP** — axum app with `/healthz` (+ `/v1/brain/search` once FR-BRAIN-108 lands).
//!   * **Ingest daemon** — background tokio task per tenant calling
//!     `layer2::ingest::run_batch` on `default_poll_interval()`. Tenants
//!     come from the `BRAIN_TENANTS` env var (comma-separated UUIDs) or
...
use cyberos_brain::{layer2, search, state::AppState, VERSION};
...
async fn main() -> ExitCode {
    ...
    let app = Router::new()
        ...
        .route("/v1/brain/search", post(search::search))
...
async fn run_ingest_daemon(
    ...
    let batch_size: i32 = std::env::var("BRAIN_INGEST_BATCH_SIZE")
        .ok()
...
/// Discover tenants to ingest. `BRAIN_TENANTS` env var (comma-separated UUIDs)
...
async fn discover_tenants(pool: &PgPool) -> Result<Vec<TenantId>, sqlx::Error> {
    if let Ok(raw) = std::env::var("BRAIN_TENANTS") {
        let mut out = Vec::new();
        for s in raw.split(',') {
            let s = s.trim();
...
async fn metrics(State(state): State<AppState>) -> Result<String, (StatusCode, String)> {
    ...
    out.push_str("# HELP cyberos_brain_l2_cursor_seq Highest L1 seq materialised per tenant\n");
    out.push_str("# TYPE cyberos_brain_l2_cursor_seq counter\n");
    for (t, seq, _) in &rows {
        out.push_str(&format!("cyberos_brain_l2_cursor_seq{{tenant=\"{}\"}} {}\n", t, seq));
    }
    out.push_str("# HELP cyberos_brain_l2_last_batch_lag_ms Lag observed on the last ingest batch (ms)\n");
    out.push_str("# TYPE cyberos_brain_l2_last_batch_lag_ms gauge\n");
    for (t, _, lag) in &rows {
        out.push_str(&format!("cyberos_brain_l2_last_batch_lag_ms{{tenant=\"{}\"}} {}\n", t, lag));
    }
    Ok(out)
}
