...
/// Default poll interval — overridable via `BRAIN_TAIL_POLL_MS` env var.
pub fn default_poll_interval() -> Duration {
    let ms: u64 = std::env::var("BRAIN_TAIL_POLL_MS")
        .ok()
...
/// Append an L1 row (used by the brain-sync daemon when it lands in the
/// Cloud-BRAIN path, and by tests). Returns the assigned seq.
pub async fn append(
    pool: &PgPool,
    row: &L1Row,
