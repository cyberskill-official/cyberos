//! Spawns the Python brain-sync daemon as a child process and restarts it on
//! exit. Includes a 5-restarts-per-60s circuit breaker; once tripped, the
//! supervisor sleeps a long backoff before retrying.
//!
//! The daemon module (`cyberos.core.brain_sync_daemon`) does not exist yet in
...
const DAEMON_PYMODULE: &str = "cyberos.core.brain_sync_daemon";
...
pub async fn start(_app: AppHandle) {
    tracing::info!(daemon = DAEMON_PYMODULE, "starting brain-sync supervisor");
    ...
    loop {
        ...
        match exit {
            Ok(status) => {
                tracing::warn!(?status, "brain-sync daemon exited");
...
/// Spawns `python3 -m cyberos.core.brain_sync_daemon` and waits for it to
...
async fn run_daemon_once() -> Result<std::process::ExitStatus, SpawnError> {
    ...
    tracing::info!(pid = ?child.id(), "brain-sync daemon spawned");
