...
repo="$(cd "$here/../../.." && pwd)"
RA="$repo/tools/cyberos-init/release-assets.sh"
BOOT="$repo/tools/cyberos-init/bootstrap.sh"
ROLL="$repo/tools/cyberos-init/rollout.sh"
...
t09_docs_real_urls() {                                               # AC 9
  grep -q 'releases/latest/download' "$repo/tools/cyberos-init/README.md" \
    && grep -q 'releases/latest/download' "$repo/docs/deploy/RELEASE.md" \
    && ! grep -q 'available once you host a tarball' "$repo/tools/cyberos-init/README.md" \
