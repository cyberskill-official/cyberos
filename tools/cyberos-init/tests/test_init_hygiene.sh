#!/usr/bin/env bash
# test_init_hygiene.sh - init.sh hygiene contract:
...
GI
  ...
  bash "$TMP/payload/init.sh" "$d" >/dev/null 2>&1     # second run must not duplicate anything
...
t03_root_fr_migration() {
  ...
  out="$(bash "$TMP/payload/init.sh" "$d" 2>&1)"
  local all=1
  ...
  out2="$(bash "$TMP/payload/init.sh" "$d" 2>&1)"
...
t04_payload_self_cleanup() {
  ...
  out="$(bash "$d/.cyberos-init/init.sh" "$d" 2>&1)"
  ...
  out="$(CYBEROS_KEEP_PAYLOAD=1 bash "$k/.cyberos-init/init.sh" "$k" 2>&1)"
  ...
  out="$(bash "$n/vendor-payload/init.sh" "$n" 2>&1)"
