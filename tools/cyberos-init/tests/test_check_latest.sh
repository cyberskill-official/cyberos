...
chk() { CYBEROS_RELEASE_ENDPOINT="${2:-}" ${3:+CYBEROS_OFFLINE=1} bash "$TMP/payload/update.sh" "$1"; }
...
t03_unreachable_degrades() {                                         # AC 3
  ...
  mkrepo "$TMP/r3" "$pv"
  out="$(CYBEROS_RELEASE_ENDPOINT="$TMP/nonexistent-endpoint" bash "$TMP/payload/update.sh" "$TMP/r3")"
...
t04_verdict_matrix() {                                               # AC 4
  ...
  mkrepo "$TMP/rA" "$pv"
  out="$(CYBEROS_RELEASE_ENDPOINT="$TMP/eq" bash "$TMP/payload/update.sh" "$TMP/rA")"
  ...
  out="$(CYBEROS_OFFLINE=1 bash "$PB/update.sh" "$TMP/rB")"
  ...
  mkrepo "$TMP/rC" "$pv"
  out="$(CYBEROS_RELEASE_ENDPOINT="$TMP/newer" bash "$TMP/payload/update.sh" "$TMP/rC")"
  ...
  out="$(CYBEROS_OFFLINE=1 bash "$TMP/payload/update.sh" "$TMP/rD")"
...
t05_numeric_semver() {                                               # AC 5
  ...
  out="$(CYBEROS_RELEASE_ENDPOINT="$TMP/ten" bash "$P5/update.sh" "$TMP/r5")"
...
t07_update_doc_contract() {                                          # AC 7
  local D="$repo/tools/cyberos-init/plugin/commands/update.md" all=1
...
t08_changelog_doc_contract() {                                       # AC 8
  grep -q 'github.com/cyberskill-official/cyberos/releases' "$repo/tools/cyberos-init/plugin/commands/changelog.md" \
    && grep -q 'check-latest.sh' "$repo/tools/cyberos-init/plugin/commands/changelog.md" \
    && ok t08 || fail t08 "release span instruction missing"
}

t01_bare_version_endpoint; t02_github_json_endpoint; t03_unreachable_degrades; t04_verdict_matrix
t05_numeric_semver; t06_offline_env; t07_update_doc_contract; t08_changelog_doc_contract
