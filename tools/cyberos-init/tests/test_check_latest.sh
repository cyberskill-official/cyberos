...
chk() { CYBEROS_RELEASE_ENDPOINT="${2:-}" ${3:+CYBEROS_OFFLINE=1} bash "$TMP/payload/init.sh" --check "$1"; }
...
t03_unreachable_degrades() {                                         # AC 3
  ...
  mkrepo "$TMP/r3" "$pv"
  out="$(CYBEROS_RELEASE_ENDPOINT="$TMP/nonexistent-endpoint" bash "$TMP/payload/init.sh" --check "$TMP/r3")"
...
t04_verdict_matrix() {                                               # AC 4
  ...
  mkrepo "$TMP/rC" "$pv"
  out="$(CYBEROS_RELEASE_ENDPOINT="$TMP/newer" bash "$TMP/payload/init.sh" --check "$TMP/rC")"
  ...
  out="$(CYBEROS_OFFLINE=1 bash "$TMP/payload/init.sh" --check "$TMP/rD")"
...
t05_numeric_semver() {                                               # AC 5
  ...
  out="$(CYBEROS_RELEASE_ENDPOINT="$TMP/ten" bash "$P5/init.sh" --check "$TMP/r5")"
