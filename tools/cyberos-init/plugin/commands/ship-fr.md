...
---
Follow the workflow in the bundled `skills/ship-feature-requests/cuo/ship-feature-requests.md` (with `EXECUTION-DISCIPLINE.md` and `STATUS-REFERENCE.md` beside it). Those are the source of truth.
...
- Run the machine gates: `bash .cyberos/cuo/gates/run-gates.sh` (or the repo's own build/lint/test if the pack is not installed).
...
If `.cyberos/` is missing, run `/fr-init` first.
