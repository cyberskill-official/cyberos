...
2. Initialize the repo:
   ...
   This auto-detects your build/lint/test, writes `.cyberos/gates.env`, scaffolds `docs/feature-requests/`, and copies the workflow machine to `.cyberos/`. (With the Claude plugin, run `/fr-init` instead.) By default it also sets up the BRAIN memory protocol: a local `.cyberos-memory/` store plus the `AGENTS.md` memory rules (skip with `CYBEROS_NO_MEMORY=1`).

3. Check the gates. Open `.cyberos/gates.env` and confirm `BUILD_CMD` / `LINT_CMD` / `TEST_CMD` are right for your repo; edit if the autodetect missed anything. If you have a caf baseline or an awh goldenset, set `CAF_ENABLED` / `AWH_ENABLED` to `true` and point them at your files. Then confirm a clean tree is green:
...
- The agent tried to set `done` itself: that breaks HITL. Point it back at the HITL section of `cuo/ship-feature-requests.md` and the two acceptance gates.
...
When the workflow improves in CyberOS, obtain a newer bundle and re-run `init.sh`. It backs up `gates.env` and never clobbers your `BACKLOG.md`.
