#!/usr/bin/env bash
# test_render_fr_pages.sh - TASK-DOCS-005 §5 suite (t01-t06 -> AC 1-6).
set -uo pipefail
...
t01_corpus_renders() {                                                 # AC 1
  ...
  h="$TMP/a/out/frs/aa/TASK-AA-001-first/index.html"
...
t02_media() {                                                          # AC 2
  ...
  grep -q '<img src="assets/pic.png"' "$h" && grep -q '<video controls src="assets/demo.mp4"' "$h" \
    && [ -f "$TMP/a/out/frs/aa/TASK-AA-001-first/assets/demo.mp4" ] \
...
t04_wired_deterministic_honest() {                                     # AC 4
  ...
  cmp -s "$TMP/a/out/frs/aa/TASK-AA-001-first/index.html" "$TMP/a/out2/frs/aa/TASK-AA-001-first/index.html" || { fail t04 "nondeterministic"; return; }
  mkfix "$TMP/b"
