...
here="$(cd "$(dirname "$0")" && pwd)"                 # tools/cyberos-install
...
[ -f "$repo/VERSION" ] || { echo "cyberos-install: ERROR: $repo/VERSION missing - refusing to build an unstamped payload" >&2; exit 2; }
cyver="$(tr -d ' \n\r' < "$repo/VERSION")"
printf '%s' "$cyver" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+$' || { echo "cyberos-install: ERROR: VERSION is not X.Y.Z semver (got '$cyver')" >&2; exit 2; }

echo "cyberos-install: assembling payload into $out"
rm -rf "$out"
...
cp -R "$here/cli"       "$out/cli"              # npx CLI channel (cyberos-install / -gates / -mcp)
...
if command -v python3 >/dev/null 2>&1; then
  python3 - "$out/plugin/skills" <<'PYCAP' || { echo "cyberos-install: fix the skill error(s) above (structure: rename the dir or its frontmatter name; length: shorten the description in modules/skill/<name>/SKILL.md), then rebuild." >&2; exit 1; }
import os, re, sys
...
for msg in broken:
    print(f"cyberos-install: ERROR {msg}", file=sys.stderr)
if broken:
    sys.exit(1)
for name in sorted(os.listdir(root)):
...
for n, name in sorted(bad, reverse=True):
    print(f"cyberos-install: ERROR skill '{name}' description is {n} chars (plugin limit {LIMIT})", file=sys.stderr)
...
cat > "$out/package.json" <<PKG
{
  ...
  "type": "module",
  "bin": {
    "cyberos-install": "cli/bin/cyberos-install.mjs",
...
# tools/cyberos-install/mcp/package.json is itself stamped at bump time (stamp-release-version.mjs),
...
[ -n "$rules_sha" ] || { echo "cyberos-install: ERROR: rules_sha computation produced nothing" >&2; exit 2; }

cat > "$out/manifest.yaml" <<EOF
# Generated by tools/cyberos-install/build.sh - do not edit by hand.
vendor: cyberos
...
[ "$plugin_bytes" -le 2097152 ] || { echo "cyberos-install: ERROR: cyberos.plugin ${plugin_bytes}B exceeds the 2 MB budget" >&2; exit 2; }

echo "cyberos-install: done. profile=$profile skills=$vendored_skills caf=$caf_vendored payload=${payload_bytes} plugin_zip=${plugin_bytes}"
echo "cyberos-install: payload at $out - install.sh lays it out under a target repo's .cyberos/ (gitignored)"
