...
```bash
bash tools/cyberos-install/build.sh          # assembles dist/cyberos/ (self-contained)
```
...
curl -fsSL https://raw.githubusercontent.com/cyberskill-official/cyberos/main/tools/cyberos-install/bootstrap.sh | bash
...
# Fleet:  bash tools/cyberos-install/rollout.sh --from-release [vX.Y.Z] <repo> [<repo>...]
```
...
```make
cyberos-install:  ; bash /path/to/dist/cyberos/install.sh .
