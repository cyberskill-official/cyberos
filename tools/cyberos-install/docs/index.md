...
- The CyberOS payload: desktop **Build payload**, or `bash tools/cyberos-install/build.sh` → `dist/cyberos/`, or release tarball
...
1. **Get the payload** (once). Desktop: Ops → Build payload. CLI from a CyberOS checkout:
   ...
   ```bash
   bash tools/cyberos-install/build.sh        # → dist/cyberos/
   ```
