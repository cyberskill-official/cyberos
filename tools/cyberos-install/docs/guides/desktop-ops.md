...
The UI path for running CyberOS operations - no terminal required. Everything here is a button over the same canonical scripts the CLI uses (`tools/cyberos-install/build.sh` and `install.sh`), so the UI and the CLI can never disagree.
