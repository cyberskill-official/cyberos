...
# Usage: bash tools/cyberos-install/rollout-fleet.sh <payload-dir> <root-dir> [<root-dir> ...]
set -uo pipefail
...
has_frs() {   # a repo "has tasks" when a spec.md exists, or a flat TASK-*.md still sits in the tree
  ...
  [ -n "$(find "$r/docs/tasks" -name 'TASK-*.md' -not -name '*.audit.md' -not -path '*/_*' -print -quit 2>/dev/null)" ] && return 0
  return 1
}
