...
t04_lifecycle_map_total() {                                           # AC 4
  ...
  # GUIDE source (tools/cyberos-install/docs/index.md, build.sh:175) from a lifecycle map
...
t05_sibling_checks_green() {                                          # AC 5
  bash "$repo/tools/cyberos-install/check-chain-coverage.sh" "$TMP/payload" >/dev/null 2>&1 \
    && ok t05 || fail t05 "chain-coverage red over the expanded set"
  [ -f "$repo/tools/cyberos-install/check-pair-parity.sh" ] \
...
t07_reduced_floor_intact() {                                          # AC 7
  ...
  cp -r "$repo/tools/cyberos-install" "$F/tools/cyberos-install"
  ...
  out7="$(bash "$F/tools/cyberos-install/build.sh" "$F/dist" 2>&1)" || { fail t07 "skill-less build failed: $(echo "$out7" | tail -1)"; return; }
