...
# Standalone bash, no framework. Run: bash tools/cyberos-install/tests/test_check_version_sync.sh
set -uo pipefail

here="$(cd "$(dirname "$0")" && pwd)"
repo="$(cd "$here/../../.." && pwd)"
CHECK="$repo/tools/cyberos-install/check-version-sync.sh"
BUILD="$repo/tools/cyberos-install/build.sh"
...
t04_invalid_version_refused() {                                       # AC 4
  local F="$TMP/fakerepo"; rm -rf "$F"; mkdir -p "$F/tools/cyberos-install"
  cp "$BUILD" "$F/tools/cyberos-install/build.sh"
  echo banana > "$F/VERSION"
  bash "$F/tools/cyberos-install/build.sh" "$F/dist" >/dev/null 2>&1 && fail "t04[banana]" "build passed" || rc=$?
  ...
  bash "$F/tools/cyberos-install/build.sh" "$F/dist" >/dev/null 2>&1 && { fail "t04[rc1]" "build passed"; return; }
  rm "$F/VERSION"
  bash "$F/tools/cyberos-install/build.sh" "$F/dist" >/dev/null 2>&1 && { fail "t04[absent]" "build passed"; return; }
  ok t04
}
...
t06_workflow_shape() {                                                # AC 6
  ...
  for pat in 'name: payload-gate' 'pull_request:' 'push:' 'tools/cyberos-install/\*\*' 'modules/skill/\*\*' 'modules/cuo/\*\*' "'VERSION'" 'build.sh "\$RUNNER_TEMP/payload"' 'check-version-sync.sh "\$RUNNER_TEMP/payload"'; do
...
hook_fixture() { # builds a fixture repo with the real wrapper + recording stubs
  local F="$1"; rm -rf "$F"
  mkdir -p "$F/.githooks" "$F/.pre-commit-hooks" "$F/tools/cyberos-install" "$F/modules/skill" "$F/docs"
  ...
  printf '#!/usr/bin/env bash\necho check >> "%s/calls.log"\nexit 0\n' "$F" > "$F/tools/cyberos-install/check-version-sync.sh"
  ...
  chmod +x "$F/.pre-commit-hooks/cyberos-payload-build.sh" "$F/tools/cyberos-install/check-version-sync.sh" \
...
t10_version_yml_inline_check() {                                      # AC 10
  ...
  local b c p
  b="$(grep -n 'tools/cyberos-install/build.sh' "$W" | head -1 | cut -d: -f1)"
