...
repo="$(cd "$here/../../.." && pwd)"
CL="$repo/tools/cyberos-install/check-latest.sh"
...
t07_version_doc_contract() {                                         # AC 7
  local D="$repo/tools/cyberos-install/plugin/commands/version.md" all=1
...
t08_version_release_pointer() {                                      # AC 8
  grep -q 'github.com/cyberskill-official/cyberos/releases' "$repo/tools/cyberos-install/plugin/commands/version.md" \
    && grep -q 'check-latest.sh' "$repo/tools/cyberos-install/plugin/commands/version.md" \
