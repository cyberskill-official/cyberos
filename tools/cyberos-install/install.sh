...
if [ ! -d "$src/cuo" ]; then
  ...
  echo "  bash tools/cyberos-install/build.sh   # -> dist/cyberos/, then run dist/cyberos/install.sh <repo>" >&2
  exit 1
fi
...
# (cp -R dist/cyberos <repo>/.cyberos-install) is redundant once init succeeds: everything the
