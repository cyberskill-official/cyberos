...
(Fixture copy: CONFIG block only. Every value below is legitimate, filled-in
configuration of the kind real client repos produce — Java generics, shell
redirection, an issue number after a single space, an inline enum comment.
The preflight flagged three of these as placeholders before the F-4 fix.)
