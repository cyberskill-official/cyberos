...
- TARGET HEALTH GATE: if you changed any code this loop, run the target's full RUN_COMMANDS (build / lint / typecheck / test) end-to-end and paste raw output (R1). The loop is NOT complete while RUN_COMMANDS are red — fix the regression, or revert it and mark the task BLOCKED + root cause (R6). Never stop or hand off a loop that left the target failing its own RUN_COMMANDS. Bound each command with a timeout so a hanging/networked check fails loud instead of stalling.
...
7. Target health — the RUN_COMMANDS you ran after your last change and their outcome, on one line: `Target health: PASS` (or `Target health: FAIL —
