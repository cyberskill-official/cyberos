...
1. Vendored and self-contained. `tools/caf/` runs without the standalone repo. Proof:
   `cd <cyberos> && PYTHONPATH=tools/caf/core/evals python3 -m code_audit_validator --all`
...
4. Pinned provenance recorded. The source sha of `code-audit-framework` at vendor time is written
   below, so the vendored copy is traceable.
...
- Source: `CyberSkill/code-audit-framework` (record the exact sha here on the host:
  `git -C ~/Projects/CyberSkill/code-audit-framework rev-parse HEAD`).
