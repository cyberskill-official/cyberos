...
3. Honesty gates you will be held to: every measured metric needs the literal
   ...
   (R8). `python3 core/evals/validate.py --run <target>` checks your artifacts.
...
- Gate: `python3 core/evals/validate.py --all` green BEFORE release. If your change
  is testable, add/extend a fixture in the SAME cycle and register it in
  `core/evals/rules.json`.
...
- `docs/` in THIS repo holds committed documentation (CONTRIBUTING, SECURITY,
  COMPLIANCE). Self-run artifacts are gitignored BY NAME (`docs/BACKLOG.md`,
  `docs/HANDOFF.md`, `docs/AUDIT-WAIVERS.yaml`) — do not fight the gitignore,
  and do not commit self-run output.
- Docs follow changes, in the SAME commit. Anything a change makes stale —
  ...
  index.html / core/evals/README / core/improve/README / this file — gets corrected as
  part of that change. The mechanical subset (version + fixture-count
  surfaces) is enforced by `core/evals/scripts/check-docs-sync.py`, which CI runs;
  the rest is on you.
...
**Root (distribution + tooling shell):** `README.md` front door · `AGENTS.md`
this file (root because agent CLIs auto-read it there) · `action.yml` GitHub
Action (root by resolution rules) · `pyproject.toml`/`package.json` packaging ·
`LICENSE`/`NOTICE`.
**`core/` (the engine — CyberOS-absorbable as a unit):** `AUDIT.md` protocol
(current) · `CHANGELOG.md` history · `improve/` the loop — see
`core/improve/README.md` for the one-screen map (CRITIC, RETROSPECTIVE,
FAILURE_LOG, BLINDSPOTS, versions/, retros/) · `evals/` regression gate
(validate.py shim → code_audit_validator.py, fixtures/, rules.json,
baseline.json, scripts/, TESTING-PROTOCOL.md) · `schemas/` published
contracts (report.v1, feedback.v1).
