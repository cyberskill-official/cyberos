...
Via the npm package the same nine commands are `npx cs <command>`: `install`, `uninstall`, `version`, `status`, `create`, `gates`, `mcp`, `help`, `memory`. `create`, `gates` and `mcp` have no slash command - they are CLI-only. `memory` is local-availability only (cyberos-memory is not bundled with the npm install). The Claude plugin additionally ships `/plan`, `/create-tasks` and `/improve` for the task workflow.
