...
# rules-cone.sh is REQUIRED: every rules_sha comparator recomputes via it (TASK-IMP-122).
cp "$here/lib/rules-cone.sh" "$out/lib/rules-cone.sh"
...
# TASK-IMP-122: rules_sha over the VENDORED cone (shared lib/rules-cone.sh), not the old
# `find cuo plugin mcp cli memory` set. `_rsha` / cone / exclusions live in ONE vendored file.
# shellcheck source=/dev/null
. "$here/lib/rules-cone.sh"
_rules_cone_validate_grammar "$out" || exit 2
rules_sha="$(_rules_sha_of "$out")"
...
rules_sha: $rules_sha     # TASK-IMP-122/074: content fingerprint of the vendored cone (see lib/rules-cone.sh)
...
# TASK-IMP-122 §1.6/§1.7: reconcile the declared cone against a REAL default install, both
# directions. Default ON for production/CI; unit-test scratch builds under /tmp set
# CYBEROS_SKIP_RULES_RECONCILE=1 (or land in a temp path — auto-skipped). Force with
# CYBEROS_RULES_RECONCILE=1. Never parse install.sh.
_do_rules_reconcile=1
case "${CYBEROS_SKIP_RULES_RECONCILE:-}" in 1|true|yes) _do_rules_reconcile=0 ;; esac
case "${CYBEROS_RULES_RECONCILE:-}" in
  1|true|yes|always) _do_rules_reconcile=1 ;;
  0|false|no) _do_rules_reconcile=0 ;;
  "")
    case "$out" in
      /tmp/*|/var/folders/*|/private/var/folders/*) _do_rules_reconcile=0 ;;
    esac
    ;;
esac
if [ "$_do_rules_reconcile" -eq 1 ]; then
  _rr="$(mktemp -d "${TMPDIR:-/tmp}/cyberos-rules-reconcile.XXXXXX")"
  # Isolation: pin env so install cannot write outside the temp root (§1.7).
  # CYBEROS_NO_MEMORY must be unset (default cone). HOME redirected. GLOBAL_SKILLS off.
  unset CYBEROS_NO_MEMORY || true
  if ! (
    export HOME="$_rr/home"
    ...
    mkdir -p "$HOME" "$_rr/repo"
    ( cd "$_rr/repo" && git init -q . >/dev/null 2>&1 || true )
    bash "$out/install.sh" "$_rr/repo"
  ); then
    echo "cyberos: ERROR: rules-cone reconciler install failed (TASK-IMP-122 §1.7)" >&2
    rm -rf "$_rr"
    exit 2
  fi
  _cy="$_rr/repo/.cyberos"
  [ -d "$_cy" ] || { echo "cyberos: ERROR: reconciler: .cyberos missing after install" >&2; rm -rf "$_rr"; exit 2; }
  # Direction 1: every path under $CY must be classified (cone|prune|exempt).
  _unclassified=""
  while IFS= read -r _rel; do
    [ -n "$_rel" ] || continue
    _cls="$(_rules_cone_classify "$_rel")"
    if [ "$_cls" = "unclassified" ]; then
      _unclassified="${_unclassified}${_unclassified:+ }${_rel}"
    fi
  done < <(cd "$_cy" && find . -type f | sed 's|^\./||' | LC_ALL=C sort)
  if [ -n "$_unclassified" ]; then
    echo "cyberos: ERROR: rules-cone reconciler Direction 1 — unclassified path(s) under .cyberos after install:" >&2
    printf '  %s\n' $_unclassified >&2
    rm -rf "$_rr"
    exit 2
  fi
  # Direction 2: every cone path resolved against the payload must exist under $CY.
  _missing=""
  while IFS= read -r _rel; do
    [ -n "$_rel" ] || continue
    if [ ! -f "$_cy/$_rel" ]; then
  ...
  done < <(_rules_cone_list "$out" | LC_ALL=C sort -u)
  if [ -n "$_missing" ]; then
    echo "cyberos: ERROR: rules-cone reconciler Direction 2 — cone path(s) absent from install:" >&2
    printf '  %s\n' $_missing >&2
    rm -rf "$_rr"
    exit 2
  fi
