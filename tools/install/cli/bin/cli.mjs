#!/usr/bin/env node
// npx cyberos <command> [args] — the single CLI entry point.
//
// `cyberos` is the trigger keyword; everything after it is a command. Naming a bin after one
// of its own verbs (`cyberos-install`) made `cyberos-install --help` read as nonsense, and it
...
function usage(stream = process.stdout) {
  const v = existsSync(join(payload, "VERSION"))
    ...
    : "unknown";
  stream.write(`CyberOS ${v} — npx cyberos <command> [args]
...
[repo] defaults to the current directory. A bare \`npx cyberos\` prints this and
changes nothing — installing is always explicit.
Docs: https://cyberos.cyberskill.world/docs
`);
}
...
if (cmd === "mcp") {
  ...
  if (!existsSync(gates)) {
    process.stderr.write(`cyberos gates: no gates at ${gates}. Run \`npx cyberos install\` first.\n`);
    process.exit(2);
  }
  ...
  process.stderr.write(`cyberos: unknown command '${cmd}'\n\n`);
