#!/usr/bin/env bash
# Emit a CycloneDX SBOM for a payload directory.
# Default backend is the hermetic Python walker (stable digests for release).
# Set CYBEROS_SBOM_BACKEND=syft to prefer syft when installed.
set -euo pipefail
payload="${1:-}"; out="${2:-}"
...
backend="${CYBEROS_SBOM_BACKEND:-hermetic}"
if [ "$backend" = "syft" ] && command -v syft >/dev/null 2>&1; then
  mkdir -p "$(dirname "$out")"
  syft "dir:$payload" -o "cyclonedx-json=$out"
  echo "emit-payload-sbom: wrote $out (syft)"
  exit 0
fi
...
if not components:
    print("emit-payload-sbom: payload has no files", file=sys.stderr)
    sys.exit(2)
ver = "unknown"
vp = os.path.join(payload, "VERSION")
if os.path.isfile(vp):
    ver = open(vp, encoding="utf-8").read().strip() or "unknown"
doc = {
    "bomFormat": "CycloneDX",
    "specVersion": "1.5",
    "version": 1,
