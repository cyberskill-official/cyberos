...
# ── npx-cli ──────────────────────────────────────────────────────────────────
# `cs` is the trigger word; verbs come after it (renamed from `cyberos`, TASK-IMP-130 --
# the old name collided on $PATH with the unrelated modules/memory console script of the
# same name). A bin named after a verb (cs-install) makes `npx cs-install --help`
# nonsense. The npm package.json is GENERATED by build.sh, so build.sh is the source of
# truth -- checking a checked-in file would check something npm never sees.
...
if [ -f "$cli" ]; then
  ...
  grep -q '"cs": *"cli/bin/cli.mjs"' "$root/tools/install/build.sh" \
    && ok "npx-cli: build.sh maps bin.cs -> cli/bin/cli.mjs" \
    || bad "npx-cli: build.sh does not map bin.cs to cli/bin/cli.mjs"
  ...
  # A bare `npx cs` must print help and change nothing on disk.
