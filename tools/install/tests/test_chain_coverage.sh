...
repo="$(cd "$here/../../.." && pwd)"
CHECK="$repo/tools/install/check-chain-coverage.sh"
...
t04_allowlist_both_ways() {                                          # AC 4
  ...
  local A="$TMP/allow.txt"; cp "$repo/tools/install/chain-allowlist.txt" "$A"
  ...
  cp "$repo/tools/install/chain-allowlist.txt" "$A"
