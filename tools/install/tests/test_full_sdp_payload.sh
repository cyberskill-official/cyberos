...
t04_lifecycle_map_total() {                                           # AC 4
  ...
  # GUIDE source (tools/install/docs/index.md, build.sh:175) from a lifecycle map
...
t05_sibling_checks_green() {                                          # AC 5
  bash "$repo/tools/install/check-chain-coverage.sh" "$TMP/payload" >/dev/null 2>&1 \
    && ok t05 || fail t05 "chain-coverage red over the expanded set"
  [ -f "$repo/tools/install/check-pair-parity.sh" ] \
...
t07_reduced_floor_intact() {                                          # AC 7
  ...
  cp -r "$repo/tools/install" "$F/tools/install"
