---
name: ship-tasks
description: "Run the CyberOS ship-tasks workflow in any repo: drive a task (product or improvement class) through implement -> review -> test -> done, with HITL required at the two human-acceptance gates and gates from the repo's own build/lint/test. Use when asked to ship, implement, or harden a task (backlog class:improvement), or to drive a docs/tasks backlog. Do NOT use for full-repo inspection remediation — that is /inspect then /harden on an inspection-report@1."
---
# ship-tasks (portable)
...
**Not `/harden`:** "Harden a task" here means shipping a backlog item with `class: improvement`. Full-repo inspection remediation (consume `inspection-report@1`, emit `hardening-record@1`) is the `/inspect` → `/harden` skill chain — distinct, never merged into this conductor.
