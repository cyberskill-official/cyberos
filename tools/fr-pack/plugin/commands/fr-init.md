...
1. If `.cyberos/fr-pack/init.sh` exists, run `bash .cyberos/fr-pack/init.sh` and report what it detected.
2. Otherwise replicate it directly:
   ...
   - Write `.cyberos/fr.gates.env` with `BUILD_CMD` / `LINT_CMD` / `TEST_CMD` / `COVERAGE_CMD`, `COVERAGE_MIN=90`, `CAF_ENABLED=false`, `AWH_ENABLED=false`, `HITL_REQUIRED=true`.
