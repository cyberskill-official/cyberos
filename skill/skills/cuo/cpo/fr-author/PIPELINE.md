# Pipeline — `fr-author` → `fr-audit` (and beyond)
...
graph.add_node("fr-author", fr_author_skill)
graph.add_node("fr-audit",  fr_audit_skill)

# Conditional edge: chain only when fr-author's output envelope says so
graph.add_conditional_edges(
    "fr-author",
    lambda state: "fr-audit" if state.last_output.next_skill_recommendation == "cuo/cpo/fr-audit" else END,
)

# fr-audit edges to either END or back to fr-author on a STALE-001 escalation
graph.add_conditional_edges(
    "fr-audit",
    lambda state: "fr-author" if state.last_output.requires_regen else END,
)
```

**State persistence:** every node transition is checkpointed to `genie.graph_checkpoint` (per SRS §6.1.1). A worker crash mid-audit means the next invocation resumes the audit cleanly without re-running `fr-author`.
...
`fr-audit` is invoked standalone with `fr_paths: [...]`. The FRs may have been authored by `fr-author`, by a human, by an external tool, or reconstructed from BRAIN — the audit doesn't care. This is the original v2.0.0 `audit_only` mode, now its own atomic skill.
...
```
   PRD ──▶ fr-author ──▶ fr-audit ──▶ cuo/cto/fr-to-tech-spec ──▶ tech-spec.md
                                              │
                                              └─ reads cyberos/docs/contracts/feature-request/template.md
                                                 (declared via depends_on_contracts:) to know what an
                                                 FR looks like before deriving spec
```

This is exactly the "persona-grouped, multi-skill, chainable" property the CyberOS skill registry is designed for. `cuo/cpo/fr-author` produces FRs that any downstream skill (own-persona or cross-persona) can consume, because the output envelope schema is documented and stable.

## Failure handling across the chain

If `fr-audit` returns `overall_status: needs_human` for FR-NNN, the supervisor:

1. Writes back to `fr-author`'s manifest: `frs[FR].status = HITL_PAUSE`, `frs[FR].audit_hash = <new hash from fr-audit>`, `frs[FR].blocking_issues += <issues from audit>`.
...
3. If audit now returns `pass`, write `frs[FR].status = PASS` and continue to the next FR in `fr-author`'s WORKER loop.
...
1. `cuo/cpo/fr-author` · `row_kind: question` · plan-approval ask
2. `cuo/cpo/fr-author` · `row_kind: artefact_write` · FR-007.md written
