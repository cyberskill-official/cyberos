# `fr-author` self-audit invariants
...
proposed_amendment_target: cyberos/docs/skills/cuo/cpo/fr-author/SKILL.md
...
proposed_amendment_target: cyberos/docs/skills/cuo/cpo/fr-author/references/MANIFEST_SCHEMA.md
...
**Check.** SQL — `SELECT count(*) FROM genie.action_log WHERE skill_id = 'cuo/cpo/fr-author' AND trace_id = $1 AND row_kind = 'artefact_write' AND (confidence IS NULL OR confidence < 0 OR confidence > 1)` returns 0.
