...
fn default_roots() -> Vec<PathBuf> {
    ...
    // If we're inside skill/ already, look at ./skills/. Otherwise look at skill/skills/.
    let try_local = cwd.join("skills");
    let try_module = cwd.join("skill/skills");
    let mut roots = Vec::new();
    if try_local.is_dir() { roots.push(try_local); }
    if try_module.is_dir() { roots.push(try_module); }
    if roots.is_empty() { roots.push(PathBuf::from("./skills")); }
    roots
}
