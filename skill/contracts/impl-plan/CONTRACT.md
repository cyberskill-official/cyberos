...
> Loaded by `cuo/cto/spec-to-impl-plan` as the generation skeleton. The artefact is the SHADOW RECORD of tickets actually created in PROJ MCP. Future tools (sprint planning, capacity tracking) consume this artefact for offline analysis without round-tripping to Linear/Jira.
...
| `tech_spec_ref` | path to source `tech_spec@1` (or `prd@1` for `lean` profile that skips tech-spec) | yes |
...
| `chain_profile: lean` (no upstream tech-spec) | `## Architecture Note` — brief para explaining the architectural assumptions baked into the ticket breakdown (since lean skips srs-author + fr-to-tech-spec) |
