...
description: Canonical task@1 schema — comprehensive, addressable, assignable unit of work embedded inside a feature_request@1. Each task is self-contained (no character limit), carries a runnable acceptance test, and declares assignability (human / ai-agent / either). Promoted to first-class contract in 2026-05-12 to support the `solo` chain_profile that collapses fr-author + fr-to-tech-spec.
