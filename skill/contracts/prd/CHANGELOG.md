# CHANGELOG — `cyberos/docs/contracts/prd/`
...
- `template.md` — Markdown skeleton `prd-author` adapts when synthesising a PRD. Carries inline `<!-- authority: ... -->` markers per AGENTS.md §5.3.
- This file.

### Driver

Same conversation as `project-brief@1` (registry v0.2.4). The chain needed both contracts: `project-brief` for the structured intake (consumed by prd-author), `prd` for the negotiated product spec (consumed by fr-author + downstream skills).
...
- A round-trip test: `prd-author` produces a PRD from a `project_brief@1`; `prd-audit` (registry v0.2.5) validates frontmatter + section presence + authority markers; `fr-author` consumes the audited PRD; produced FRs reference the PRD by `memory_id` or path.
