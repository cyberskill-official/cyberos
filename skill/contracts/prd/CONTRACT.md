...
contract_id: prd
contract_version: v1
template_literal: prd@1
description: Canonical prd@1 schema body — frontmatter contract + Markdown skeleton for the Product Requirements Document artefact emitted by `cuo/cpo/prd-author`. Consumed downstream by `cuo/cpo/prd-audit` (when registered v0.2.5), `cuo/cpo/fr-author` (existing), and the future `cuo/cto/srs-author`.
...
determinism:
  reproducible: false
  fixity_notes: "PRD authoring is judgement-heavy by nature (per Q4 of registry v0.2.4 design conversation). The CONTRACT body shape is byte-stable; PRD CONTENT is not. prd-audit treats most rules as 'warning' rather than 'error' to acknowledge the judgement load."

# ── Source-tier emitted ──────────────────────────────────────────────
emitted_source_freshness_tier: 18   # high authority — a passed-audit PRD is source-of-truth on product intent
---

# `prd@1` — canonical PRD contract

> A **contract**, not a skill. Holds the single source of truth for the Product Requirements Document artefact shape across CyberOS. Loaded by `cuo/cpo/prd-author` (as the generation skeleton); will be loaded by `cuo/cpo/prd-audit` (registry v0.2.5) as the validation target. Consumed by `cuo/cpo/fr-author` as the canonical PRD shape it decomposes into FRs.
...
PRD ≠ SRS. The SRS (registered as `srs@1` in registry v0.2.6) describes what the SYSTEM does in technical detail; the PRD describes what the PRODUCT should do at a user-outcome level. They have different audiences, lifecycles, and content density.

## Frontmatter contract

The frontmatter that every `prd@1` document MUST carry:

| Field | Type / enum | Required | Audit rule (future, in `prd-audit/RUBRIC.md`) |
| --- | --- | --- | --- |
| `template` | const `prd@1` | yes | FM-004 |
...
| `prd_iteration` | integer ≥ 1 — increments on each amendment-batch round (mirrors fr-author's pattern) | yes | FM-117 |
...
Every `prd@1` body MUST contain these H2 sections in this order. Each section carries an `<!-- authority: ... -->` marker per AGENTS.md §5.3 (because PRDs make strong claims and downstream consumers need to know which claims are human-edited vs. inferred).
...
3. **`## Non-goals`** — what this PRD explicitly does NOT cover. Out-of-scope is a feature (mirrors fr-audit's QA-006 spirit at the PRD level).
...
9. **`## Rough Sizing`** — high-level engineering effort estimate (N × engineer-month). This is a hint for downstream `srs-author` and `fr-author`, not a commitment.
...
`prd-audit` (registry v0.2.5) will REJECT any PRD with `llm-implicit` authority on a `## Goals` claim — goals MUST be at least `llm-explicit`. Lower-authority claims are flagged for human review at PRD-approval time.

## Iteration model

`prd-author` follows fr-author's amendment-batch protocol (per Q5 of registry v0.2.4 design):
...
3. `prd-author` applies the batch, increments `prd_iteration`, rewrites the same `<title>.prd.md` file in place.
...
- `cuo/cpo/fr-author/references/AMENDMENT_PROTOCOL.md` — amendment-batch pattern this contract's author skill mirrors.
- Future consumers: `cuo/cpo/prd-author` v0.1.0 (this version), `cuo/cpo/prd-audit` v0.1.0 (registry v0.2.5), `cuo/cpo/fr-author` v0.3.0+ (when fr-author migrates to consume `prd@1` instead of generic "PRD/spec docs").
