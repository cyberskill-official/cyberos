...
stdlib-only TASK-AI-003-conformant Writer that reads `{path, body, meta}` on stdin, computes
...
ts_ns}` triple on stdout. That is exactly what TASK-AI-003 specifies. But the memory package never shipped
...
- TASK-AI-003 §1 #2 and the build envelope literally specify `WRITER_ARGS = ["-m", "cyberos.writer", "put"]`
...
so TASK-AI-003's audit trail is not actually being written.

Fix options (a design decision, not a one-liner).
1. Add the TASK-AI-003 contract to the memory package: a `cyberos.writer` module (or a
...
TASK-AI-003 contract and the bridge now works (six `cost_hold_expiry` tests green). Two things remain:
...
- Still recommended: wire TASK-AI-003 §1 #10's startup health check (`python3 -m cyberos.writer --version`,
