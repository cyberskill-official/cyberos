...
---
# Ship artefact bundle - FR-IMP-070

## Context map
Patterns per FR-IMP-068/069 lineage. files_outside_immediate_domain: 0 -> no ADR. has_external_dependency: false (GitHub API consumed best-effort at runtime with a hermetic file-path override for tests; never a hard dependency - offline is first-class).
...
2. Legacy `CyberOS: installed=X available=Y` line replaced by the machine-parseable three-value format (spec-mandated). FR-APP-001's Ops tab displays raw output per its own clause 3, so it inherits the richer report.
