# FR-APP-006 — code-review packet (steps 17–18)

Status: `reviewing`. **HALTED at HITL gate 1 (review acceptance).** Diff under review: the `feat(desktop): FR-APP-006 ...` phase commit (6 new files, 3 new dirs; `modified_files: []` honored).
...
| 2 | Package managers ≠ stores; no dependency on FR-APP-003/004 flags | Prep jobs consume the always-on `desktop` job's artifacts only; MAS/MSIX channels never referenced | ✅ |
...
1. **Spec's own §3 skeleton would have violated its own AC #3:** the skeleton's comment text contained submission-command strings verbatim — my workflow phrases every comment to avoid the contiguous strings, and the guard splits its patterns (else the guard flags itself, as FR-APP-005's did).
...
**"FR-APP-006 review: approved"** or **"FR-APP-006 review: rejected — <reason>"**.
