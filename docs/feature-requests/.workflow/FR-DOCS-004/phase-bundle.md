# FR-DOCS-004 phase bundle (batch-mode ship)
...
- FR-PLUGIN-003 (draft) + FR-TEN-002 (ready_to_implement) cited planned files absent from their own
...
- 28 module READMEs are stale hand-written indexes (chat/ lists the RETIRED pre-101 generation) -
  exempted with reasons; the status hub (FR-DOCS-006) becomes the living index.
