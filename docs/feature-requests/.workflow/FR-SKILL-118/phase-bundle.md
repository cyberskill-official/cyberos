# FR-SKILL-118 phase bundle

## repo-context-map (step 1)
Gap inventory via the new checker itself (completeness authority per §6): 86 missing files across
7 pairs + 2 spike acceptance READMEs. The four reference pairs (feature-request, implementation-plan,
architecture-decision-record) pass clean - parity defined by pointing at them, per §2. Conventions
copied from feature-request-author/-audit: PIPELINE = chain-binding tables, AUDIT_LOOP = canonical-loop
...
FR-SKILL-116) and architectural-spike (FR-SKILL-117) - 8 pairs raised, not 6; FR-CUO-209 AC 5's
...
prefixes. BSU rubric versioned backlog_state_update_rubric@2.0 (NOT @1.0): FR-CUO-205 landed first and
...
SKILL.md changes: one additive "Contract files (FR-SKILL-118)" section per skill, appended at EOF -
...
#4 named constants + FR-CUO-207 hook PASS (t03); #5 artefact shapes untouched PASS (t04 additive-only);
...
## ship-manifest dogfood (FR-CUO-206 in production)
docs/feature-requests/.workflow/FR-SKILL-118.ship.json live: initialized at step 1 with fr_sha256 +
