# FR-APP-004 — ship-workflow phase bundle (steps 1–12)

Run: 2026-07-13, ship-feature-requests v2.4.0. `queue: picked FR-APP-004 (priority=SHOULD, slice 2) — 3rd of 5; FR-IMP-073 + FR-APP-003 parked at HITL gate 1`.

## Steps 1–2 — repo context map

- **Patterns:** gated standalone workflow_dispatch channel (release-mas.yml precedent from FR-APP-003, same batch); lint-as-second-gate; `${{ secrets.* }}`-only credentials; toolchain trio (node20 + stamper + dtolnay rust) from the desktop job.
...
- **Blast radius:** 4 new files + 1 lint script. Spec's `modified_files` listed release.yml — **untouched** (standalone workflow, same reasoning as FR-APP-003; declared honestly).
...
- AC #7: no repo secret-scan gate exists yet (FR-IMP-003 draft) — manual assert: all credentials are `${{ secrets.* }}` references; recorded honestly.
