# FR-APP-003 — ship-workflow phase bundle (steps 1–12)

Run: 2026-07-13, ship-feature-requests v2.4.0. Queue echo: `queue: picked FR-APP-003 (priority=SHOULD, created=2026-07-12) — 2nd of 5 in batch; FR-IMP-073 parked at HITL gate 1`.
...
| 5 | security | self-updater inside MAS bundle (sandbox + policy violation) | audit finding recorded; **hard blocker #1** in the answer sheet — MAS_RELEASE must stay off until the `mas` cargo-feature follow-up FR lands. Not rationalized away via unverified config-merge tricks |
...
- AC #8: no secret-scan gate exists yet in the repo (FR-IMP-003 still draft) — asserted manually instead: every credential reference in added files is `${{ secrets.* }}`; recorded honestly rather than citing a nonexistent gate.
