# FR-APP-003 — code-review packet (steps 17–18)
...
2. **Real blocker discovered (AC #1 audit):** `tauri-plugin-updater` is active in the base config → MAS bundle would self-update = sandbox + policy violation. Recorded as **hard blocker #1**; needs a small `mas` cargo-feature follow-up FR (out of FR-APP-003's scope per its own §11). `MAS_RELEASE` must stay off until then.
...
**"FR-APP-003 review: approved"** → ready_to_test, or **"FR-APP-003 review: rejected — <reason>"** → routed back.
