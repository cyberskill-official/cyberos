...
manual work". That standing verdict is the recorded human approval for each FR's two gates below.

Shipped (all 5, single batch leg, per-FR commits):
1. FR-DOCS-004 - folder-per-FR (491 migrated), corpus 100% strict-yaml (42 repaired/63 lines),
   loud regen, walkers + checker updated, 6/6 (+4) AC.
2. FR-TPL-001 - templates module: CDS tokens/glass vendored @ 7231866d with provenance,
   template@1 contract, 3 shells, 4/4 AC.
3. FR-DOCS-005 - 491 self-contained CDS FR pages (media, cross-links, audit blocks), catalog
   links, 6/6 AC. Template fill() html-slot bug found+fixed by the suite.
4. FR-DOCS-006 - status hub (deck + Roadmap|Backlog|Changelog, hash-routed, JS-free fallback),
   roadmap superseded with permanent stub, 6/6 + legacy 7/7 AC.
5. FR-SKILL-120 - authoring wiring (command/author/audit/ship/init grammar), t07-t10.

Field repairs folded in: FR-PLUGIN-003 + FR-TEN-002 new_files omissions; 28 stale module READMEs
