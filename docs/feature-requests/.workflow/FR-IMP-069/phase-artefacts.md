...
---
# Phase artefacts - FR-IMP-069

## Repo context map
Patterns: exit 0/10/2 + `cyberos-init:` prefix (FR-IMP-068 lineage); release.yml = tag-driven job set (desktop/android/ios), top-level permissions contents: write; bootstrap legacy env CYBEROS_PACK_URL + top-level-dir tarballs; rollout takes payload dir as $1.
files_outside_immediate_domain: 2 (.github/workflows/release.yml, docs/deploy/RELEASE.md) -> <= 3, no ADR (channel decision recorded in FR source_decisions + operator plan approval).
