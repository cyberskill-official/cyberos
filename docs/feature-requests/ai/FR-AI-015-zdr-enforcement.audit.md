...
Either:
1. Document that `ProviderKind: Copy` is a load-bearing invariant in §7 (concept dependencies); add a `#[derive(Copy, Clone)]` requirement to FR-AI-008's enum spec; OR
...
This is the same shape as FR-AI-007 ISS-001, FR-AI-012 ISS-001, FR-AI-013 ISS-002 — ACs reference behaviors without matching test bodies. A code-gen agent has no template.
...
- §1 #6 introduces a dedicated `ai.zdr_violation` BRAIN row kind. This is the proof-of-refusal primitive: a regulator asking "did you ever route PDPL data to a non-ZDR provider for tenant X" gets a positive answer (rows showing the refusal) rather than an absence-of-evidence answer.
...
- §3 specifies the BRAIN audit row builder (`canonical::zdr_violation`) inside this FR, not punting to FR-AI-003. The owning-FR-builds-the-builder pattern matches FR-AI-014's `canonical::persona_loaded` fix.
...
- §11 documents the Anthropic "Enterprise plan only" caveat AS A KNOWN GAP with a clear FR-AI-022 follow-up. The honesty about scope is important — ops needs to manually verify Enterprise tier per tenant during onboarding until the runtime check ships.
...
- **ISS-002 RESOLVED**: §5 now has `revocation_warns_and_metricises` with the full pattern (write-revoked-YAML → sleep-for-hot-reload → assert is_zdr false → assert OTel counter increment → restore). Matches the FR-AI-014 tamper-test structure.
...
- **ISS-006 RESOLVED**: §1 #7 explicit on hot-reload-parse-error keeping cache unchanged; §6 `reload_with_diff` shows the WARN-and-skip pattern; §10 row added; §5 (via the watch test framework) covers the path. FR-AI-014's hot-reload-malformed pattern reused.
