...
id: FR-AI-003
title: "BRAIN audit-row bridge — canonical Writer for AI Gateway"
module: AI
priority: MUST
status: accepted
...
source_pages:
  ...
  - website/docs/modules/brain.html#audit-chain
...
new_files:
  - services/ai-gateway/src/brain_writer.rs
  - services/ai-gateway/src/brain_writer/canonical.rs
  - services/ai-gateway/src/brain_writer/subprocess.rs
  - services/ai-gateway/tests/brain_writer_test.rs
modified_files:
  - services/ai-gateway/Cargo.toml   # add serde_json, sha2, base64, tokio process features
  - services/ai-gateway/src/lib.rs   # export brain_writer module
allowed_tools:
  ...
  - bash: cargo test -p cyberos-ai-gateway brain_writer
...
disallowed_tools:
  ...
  - in-process Python (PyO3) — deferred to FR-AI-008 (slice 2 spike)
...
sub_tasks:
  ...
  - "0.5h: integration test (real Writer subprocess, real BRAIN, real chain verification)"
risk_if_skipped: "AI Gateway cannot emit audit rows. FR-AI-001 and FR-AI-002 both call into this bridge; both block. Beyond that: any future AI call would slip through with no provenance, violating the audit-before-action invariant and breaking EU AI Act Art. 12."
---

## §1 — Description (BCP-14 normative)

The AI Gateway service **MUST** provide a `brain_writer` Rust module that emits chained, content-addressed audit rows to the local BRAIN via the canonical Writer subprocess. The module exposes one public emission function and a typed set of row kinds; every other AI Gateway path that needs to write to BRAIN MUST route through this module.

The `brain_writer::emit()` function:

1. **MUST** accept a typed `BrainEmit` struct describing the row (kind, path, extra payload).
...
3. **MUST** treat a non-zero subprocess exit code as a hard failure (`Err(BrainWriterError::WriterFailed)`); callers MUST NOT proceed with the action the row was guarding.
...
8. **MUST** support the following closed set of `ai.*` row kinds (updated 2026-05-16 as additional FRs were specified):
    ...
    - Added by FR-AI-009 (circuit breaker): `ai.failover_triggered`.
    - Added by FR-AI-015 (ZDR enforcement): `ai.zdr_violation`.
    - Added by FR-AI-016 (residency pinning): `ai.residency_violation`.
    - Added by FR-AI-017 (per-tenant cache): `ai.cache_hit`.
    - Added by FR-AI-021 (operator CLI) — operator-action rows in `ai.cli_*` sub-namespace: `ai.cli_policy_updated`, `ai.cli_failover_drill`, `ai.cli_invoice_exported`, `ai.cli_breaker_reset`, `ai.cli_expiry_repaired`, `ai.cli_brain_emitted`.
    - Cross-module rows in adjacent namespaces (NOT owned by this bridge but emitted via the module's own writer; registered here per AUTHORING.md §3.2 rule 8 for the canonical closed-set): `auth.*` per FR-AUTH-001/002/004/006; `brain.sync_row_filtered` per FR-BRAIN-106; `skill.invoked_started` + `skill.invoked_completed` per FR-SKILL-101; `obs.*` per FR-OBS module.
    - **CHAT module rows** (added via the strict-redo 2026-05-16 P.M. expansion of FR-CHAT-002..012; emitted by services/chat/*, services/chat-brain-bridge/*, services/chat-importer/*, services/chat-lumi/*, services/chat-push/*, services/chat-dsar/*):
        - FR-CHAT-002 (authbridge): `chat.session_started`.
        - FR-CHAT-003 (Fargate deployment): `chat.deployment_provisioned`, `chat.deployment_inventory`, `chat.deployment_drift_detected`, `chat.tier_upgraded`, `chat.pitr_test_passed`, `chat.deployment_warning`, `chat.maintenance_started`, `chat.deployment_blocked`, `chat.tf_state_recovered`, `chat.module_deprecated`.
        - FR-CHAT-005 (BRAIN bridge): `chat.message`, `chat.message_edited`, `chat.message_deleted`, `chat.channel_created`, `chat.channel_archived`, `chat.message_reacted`, `chat.message_unreacted`, `chat.user_joined_channel`, `chat.user_left_channel`, `chat.message_attachment`, `chat.bridge_heartbeat`, `chat.bridge_lag_alert`, `chat.bridge_misconfigured`, `chat.bridge_shutdown`, `chat.bridge_redaction_failed`.
        - FR-CHAT-006 (Slack import): `chat.import_started`, `chat.import_step_completed`, `chat.import_finished`, `chat.import_warning`, `chat.import_verification_failed`, `chat.import_aborted`, `chat.import_failed`, `chat.import_rate_limit_exhausted`.
        - FR-CHAT-007 (Zalo import): reuses the FR-CHAT-006 `chat.import_*` family AND adds `chat.import_unsupported_zalo_version`, `chat.import_timestamp_ambiguous`, `chat.import_conversation_completed`.
        - FR-CHAT-008 (Lumi mention): `chat.lumi_invoked`, `chat.lumi_skipped`, `chat.lumi_error`.
        - FR-CHAT-009 (retro capture): `chat.retro_capture_started`, `chat.retro_capture_completed`, `chat.retro_capture_cancelled`, `chat.retro_capture_expired`, `chat.retro_capture_truncated`.
        - FR-CHAT-010 (decommission signal): `chat.decommission_signal`, `chat.decommission_state_changed`, `chat.decommission_snoozed`, `chat.decommission_unsnoozed`.
        - FR-CHAT-011 (mobile push): `chat.push_delivered`, `chat.push_failed`, `chat.push_dnd_dropped`.
        - FR-CHAT-012 (DSAR export): `chat.dsar_requested`, `chat.dsar_exporting`, `chat.dsar_delivered`, `chat.dsar_fully_delivered`, `chat.dsar_acknowledged`, `chat.dsar_failed`, `chat.dsar_expired`, `chat.dsar_url_reused`.
    - **PROJ module rows** (added via the strict-redo 2026-05-16 P.M. expansion of FR-PROJ-002, 005..018; emitted by services/proj-sync/* and web/proj-client/*):
        - FR-PROJ-001 (issue schema baseline): `proj.issue_status_changed` (superseded by FR-PROJ-002's `proj.decision` for status transitions; status_changed retained for non-decision metadata updates).
        - FR-PROJ-002 (BRAIN-anchored decision): `proj.decision`, `proj.decision_retracted`.
        - FR-PROJ-005 (rate-card schema): `proj.rate_card_created`, `proj.rate_card_superseded`, `proj.rate_card_billable_default_changed`, `proj.rate_card_corrected`, `proj.rate_card_currency_mismatch`.
        - FR-PROJ-006 (billable cascade): `proj.billable_resolved`, `proj.member_billable_override_set`, `proj.task_class_billable_set`.
        - FR-PROJ-007 (billing modes): `proj.billing_mode_set`, `proj.billing_mode_changed`, `proj.milestone_invoiced`, `proj.milestone_cancelled`, `proj.milestone_added`, `proj.retainer_overage_emitted`, `proj.retainer_overage_streak`, `proj.retainer_rollover_carry_forward`, `proj.billing_mixed_currency_period`.
        - FR-PROJ-008 (history events): `proj.issue_mutated`, `proj.chain_tampered`.
        - FR-PROJ-009 (BRAIN links): `proj.brain_link_created`, `proj.brain_link_removed`, `proj.brain_link_traversed`.
        - FR-PROJ-010 (citation drift): `proj.citation_drift_detected`, `proj.drift_remediated`.
        - FR-PROJ-011 (blocker detector): `proj.blocker_detected`, `proj.blocker_resolved`, `proj.blocker_resolved_diff`, `proj.blocker_stale`, `proj.blocker_escalated`, `proj.blocker_cycle_detected`.
        - FR-PROJ-012 (cycle review): `proj.cycle_review_drafted`, `proj.cycle_review_accepted`, `proj.cycle_review_iterated`, `proj.cycle_review_skipped`, `proj.cycle_goal_updated`.
        - FR-PROJ-013 (estimate calibration): `proj.estimate_calibration_computed`, `proj.calibration_drift_alert`, `proj.calibration_backfilled`.
        - FR-PROJ-014 (Kanban): `proj.kanban_card_moved`, `proj.kanban_card_move_undone`, `proj.wip_limit_overridden`.
        - FR-PROJ-015 (Timeline): `proj.timeline_bar_moved`.
        - FR-PROJ-016 (Gantt): `proj.dependency_added`, `proj.dependency_removed`, `proj.dependency_near_cycle`, `proj.critical_path_recomputed`.
        - FR-PROJ-017 (Brief Modal): `proj.brief_modal_opened`.
        - FR-PROJ-018 (design tokens): n/a — visual/CI FR; no audit rows.
...
10. **MUST** run a startup health check via `python3 -m cyberos.writer --version` as a synchronous subprocess call during AI Gateway init. Non-zero exit MUST cause the gateway to exit 1 with stderr `brain_writer: Writer subprocess unavailable: <reason>`. This catches misconfigured deploys at deploy time, not first-request time.
11. **MUST NOT** expose `brain_writer::emit_batch()` in slice 1. Callers needing N rows MUST call `emit()` N times (or use `futures::future::join_all`). FR-AI-008 (slice 2 PyO3 spike) re-introduces a real batching API.
12. **MUST** log `ChainHashMismatch` errors at sev-1 via `tracing::error!` with structured fields: `expected_chain` (hex), `actual_chain` (hex), `seq`, `tenant_id`, `payload_canonical_hash`. The OBS dashboard (FR-OBS-007) routes sev-1 chain events to PagerDuty until FR-OBS-007 lands; until then, `tracing::error!` lines surface via journald.
13. **SHOULD** keep chain-hash verification ON by default in both dev and prod. The 5µs per emit cost is negligible compared to the 30ms subprocess fork. A feature flag `brain_writer/skip-verify` exists for ad-hoc debugging but MUST NOT be enabled in prod (CI gate enforces).
14. **SHOULD** emit OTel metrics: `brain_writer_emit_calls_total{kind,outcome}` (counter; outcome ∈ ok/err), `brain_writer_emit_latency_ms` (histogram), `brain_writer_chain_mismatches_total` (counter; sev-1 dashboard alarm at > 0), `brain_writer_writer_unavailable_total` (counter), `brain_writer_path_rejections_total{reason}` (counter).
15. **MUST NOT** expose a `dry_run` mode in slice 1. FR-AI-021 (operator CLI) adds a `cyberos-ai brain emit --dry-run` subcommand that validates payload without emission; the bridge itself stays single-purpose.
16. **MUST** emit history events as pairs per AUTHORING.md §3.8 rule 26: every `ai.precheck_started` row MUST be followed by either `ai.precheck_completed` OR `ai.precheck_failed` within 30s (the precheck deadline). The same pairing rule applies to `ai.reconcile_started` / `ai.reconcile_completed | ai.reconcile_failed`. Standalone `*_started` rows are crash signals — an OBS Grafana lint alerts when `count(*_started) - count(*_completed) - count(*_failed) > 0` over any 5-minute window for any (tenant, module).
17. **MUST** include `extra.trace_id: String` (32-char lower-hex W3C `trace-id` form per AUTHORING.md §3.7 rule 23) in every emitted `BrainRow`. The format MUST be produced via OTel `TraceId` Display (NOT Debug — Debug yields `TraceId(0af7…)` while Display yields the 32-char hex per AUTHORING.md §3.7 rule 24). AC #17 verifies via regex `^[0-9a-f]{32}$` against every emitted row's `extra.trace_id` in the round-trip test.
...
```rust
// services/ai-gateway/src/brain_writer.rs
pub async fn emit(emit_request: BrainEmit) -> Result<EmittedRow, BrainWriterError>;

// Slice-1 startup health check
pub async fn check_writer_available() -> Result<WriterVersion, BrainWriterError>;

pub struct BrainEmit {
    ...
    // NOTE: dedup_key is deferred to FR-AI-008 (slice 2 PyO3 spike). Slice-1 callers must
    // tolerate duplicate audit rows on crash recovery; FR-AI-021 operator CLI provides a
...
pub enum AiInvocationKind {
    Precheck,           // emitted by FR-AI-001
    Invocation,         // emitted by FR-AI-002 (success path)
    InvocationFailed,   // emitted by FR-AI-002 (refund path)
    HoldExpired,        // emitted by FR-AI-004 (cleanup job)
    PersonaLoaded,      // emitted by FR-AI-014 (persona stamping)
}
...
1. **Happy path (precheck row)** — `brain_writer::emit(canonical::precheck(...))` MUST return `Ok(EmittedRow { seq, ts_ns, chain })`. The row MUST appear at `<memory-root>/memories/ai-invocations/<ts_ns>_<tenant>_<key>.md`. The returned `seq` MUST equal the BRAIN's HEAD seq counter immediately after emission. The chain hash MUST equal SHA-256(canonical(row_minus_chain) ‖ prev_chain).
...
3. **Writer non-zero exit → Err** — When the subprocess returns exit code `3` (path traversal), `emit()` MUST return `Err(BrainWriterError::PathRejected)` carrying the stderr's `detail` field; MUST NOT emit a row; MUST NOT mutate `<memory-root>/`.
4. **Writer timeout → kill + Err** — When the subprocess hangs for >5s (test injects a `time.sleep(10)` via a Writer fixture), `emit()` MUST send SIGTERM, wait 1s, send SIGKILL, and return `Err(BrainWriterError::Timeout { waited_ms: 5000 })`. MUST NOT leave a zombie process; MUST NOT leave a partial row on disk.
5. **Chain-hash mismatch → Err** — When the Writer returns a `chain` that does NOT match the AI Gateway's recomputation (test injects a corrupted-Writer fixture), `emit()` MUST return `Err(BrainWriterError::ChainHashMismatch)`; MUST log a sev-1 OBS event; MUST NOT return `Ok`.
...
7. **Path validation** — `BrainEmit { path: "../escape.md", ... }` MUST be rejected at the bridge boundary (BEFORE subprocess spawn) with `Err(BrainWriterError::PathRejected { reason: "traversal" })`. Same for absolute paths and paths under reserved directories (`audit/`, `index/`).
8. **Latency budget** — `emit()` MUST complete within 50ms p95 over a 1000-call integration test against a real BRAIN store on tmpfs. Per-call breakdown logged via `tracing` spans for FR-AI-022.
...
**Unit tests:** `services/ai-gateway/src/brain_writer/canonical.rs` has 5 typed-builder unit tests (one per kind) verifying that the builder produces the documented JSON shape with sorted keys.

**Integration tests:** `services/ai-gateway/tests/brain_writer_test.rs`

```rust
#[tokio::test]
async fn emit_precheck_happy_path() {
    let env = TestBrain::new_tmpfs().await;
    let outcome = brain_writer::emit(
        canonical::precheck(
...
async fn emit_concurrent_serialises() {
    ...
    let handles: Vec<_> = (0..16).map(|i| {
        ...
        tokio::spawn(brain_writer::emit(test_emit_with_path(&path)))
    }).collect();
...
async fn emit_writer_timeout_returns_err() {
    let env = TestBrain::with_hung_writer().await;
    let outcome = brain_writer::emit(test_emit()).await;
    assert!(matches!(outcome, Err(BrainWriterError::Timeout { .. })));
    assert_no_zombie_writer().await;
}

#[tokio::test]
async fn emit_chain_hash_mismatch_rejects() {
    let env = TestBrain::with_corrupted_writer().await;
    let outcome = brain_writer::emit(test_emit()).await;
    assert!(matches!(outcome, Err(BrainWriterError::ChainHashMismatch { .. })));
}

#[tokio::test]
async fn emit_path_traversal_rejects_before_spawn() {
    let outcome = brain_writer::emit(BrainEmit {
        kind: AiInvocationKind::Precheck,
        path: "../../etc/passwd".to_string(),
        extra: json!({}),
    }).await;
    assert!(matches!(outcome, Err(BrainWriterError::PathRejected { .. })));
...
async fn emit_normalises_nfc() {
    ...
    let outcome = brain_writer::emit(test_emit_with_extra_string(payload_with_decomposed)).await.unwrap();
    let on_disk = env.read_row(outcome.seq).await;
...
cargo test -p cyberos-ai-gateway brain_writer
```
...
pub async fn emit(req: BrainEmit) -> Result<EmittedRow, BrainWriterError> {
    // 1. Validate path before spawning
    validate_path(&req.path).map_err(|reason| BrainWriterError::PathRejected {
    ...
    }).map_err(|e| BrainWriterError::CanonicalisationFailed { reason: e })?;
    ...
    let mut child = Command::new(WRITER_BIN)
        ...
        .stderr(Stdio::piped())
        .spawn()
        .map_err(|e| BrainWriterError::WriterUnreachable { reason: e.to_string() })?;
    ...
    let (write_res, exit_res, stdout_buf, stderr_buf) = match timeout(WRITER_TIMEOUT, async {
        ...
        Err(_) => {
            ...
            return Err(BrainWriterError::Timeout { waited_ms: 5000 });
        }
    };

    write_res.map_err(|e| BrainWriterError::WriterUnreachable { reason: e.to_string() })?;
    let exit = exit_res.map_err(|e| BrainWriterError::WriterUnreachable { reason: e.to_string() })?;

    if !exit.success() {
        return Err(BrainWriterError::WriterFailed {
    ...
    let row: WriterStdout = serde_json::from_slice(&stdout_buf?)
        .map_err(|e| BrainWriterError::WriterFailed {
            exit_code: 0, stderr: format!("stdout parse: {}", e),
        })?;

    // 7. Verify chain hash
    let expected = compute_expected_chain(&payload, &row.prev_chain);
    let actual = hex::decode(&row.chain).map_err(|_| BrainWriterError::ChainHashMismatch {
        expected, got: [0u8; 32],
    })?;
    if expected[..] != actual[..] {
        let mut got = [0u8; 32];
        got.copy_from_slice(&actual);
        return Err(BrainWriterError::ChainHashMismatch { expected, got });
    }
...
**Code dependencies:**
- `cyberos.writer` Python module — already shipped as part of BRAIN Layer 1. The AI Gateway invokes it as a subprocess.
...
**Versioning:** the Writer's CLI contract is pinned. If the Writer changes its stdin/stdout shape, this FR must be re-audited. The contract version travels in the BRAIN manifest under `manifest.writer_cli_version`.
...
```rust
Err(BrainWriterError::PathRejected { path: "../escape.md".to_string(), reason: "path_traversal: …".to_string() })
```
...
5. **~~dry_run mode~~** → §1 #15 (NOT in bridge; FR-AI-021 CLI subcommand). Was Q5.
2. **Chain-hash verification cost** — 5µs per emit, but 16 concurrent emits could compound. Profile in §5 integration test; if it pushes p95 over 50ms, gate behind a feature flag (`brain_writer/verify-chain`) that defaults on in dev and off in prod. Decision: leave on by default, profile, revise if needed.
...
4. **OBS event emission on ChainHashMismatch** — Sev-1 ("chain corruption") is the right severity, but FR-OBS-003 isn't shipped yet at this FR's build time. Proposal: log a structured `chain_hash_mismatch` event via `tracing::error!`; OBS slice 1 (FR-OBS-007) will route those to PagerDuty when it lands.
5. **Should the bridge expose a `dry_run` mode?** — Useful for FR-AI-021 (operator CLI) to validate a payload without emitting. Slice-1 proposal: yes, but as an opt-in flag on `emit_batch`, not on `emit`; keeps the hot path simple.
...
| Writer subprocess hangs (> 5s) | tokio timeout fires | `Err(BrainWriterError::Timeout)`; SIGTERM then SIGKILL | Caller retries; if persistent, OBS sev-2 alert |
| Writer subprocess non-zero exit | `child.wait()` returns non-success | `Err(WriterFailed { exit_code, stderr })` | Caller bubbles up; FR-AI-021 `cyberos-ai brain doctor` diagnoses |
...
- This FR is the single biggest latency contributor on the AI Gateway hot path (50ms of the gateway's ~200ms p95 budget). PyO3 (FR-AI-008) could cut that to <10ms; we defer the optimisation until real traffic confirms the win.
...
- This FR is the first non-BRAIN module to depend on the canonical Writer subprocess. Once it lands, the pattern can be lifted into other modules (CHAT, CUO, EMAIL) by duplicating the Rust module into each service crate. A later refactor will extract it into a shared `cyberos-brain-client` crate.
