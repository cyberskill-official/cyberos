...
## §12 — Historical route-back note (2026-06-08, resolved 2026-06-13)

This route-back was resolved on 2026-06-13. The note is retained to preserve the implementation history.

Original reason: the OTel helper module and local propagation tests existed, but the FR's live acceptance path was not implementable or verifiable in that dependency layer. `FR-OBS-001` is now `done`, and the gateway router now emits root spans, child stage spans, provider attempt spans, retry events, W3C TraceContext, baggage, PII-safe attribute lint coverage, span-name documentation, and an overhead guard.

Historical work preserved for the next attempt:
...
Historical required next verification before shipping (superseded by shipped verification below):
...
Shipped verification completed:
- `cargo test -p cyberos-ai-gateway otel -- --nocapture` — passed.
- `cargo test -p cyberos-ai-gateway --test router_test -- --nocapture` — 31 passed.
- `cargo test -p cyberos-ai-gateway -- --test-threads=1` — full AI gateway suite passed.

*End of FR-AI-022. Status: done. Shipped 2026-06-13.*
