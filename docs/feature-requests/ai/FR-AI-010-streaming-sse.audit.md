...
score_post_expansion: 9.0/10      # after expanding to FR-AI-001 depth (~880 lines)
...
Same shape as the FR-AI-007/008/009 ISS-001 patterns: ACs reference behaviors that lack a matching `#[tokio::test]` body or a metric-counter assertion.
...
Result: `cost_ledger::reconcile` is called with `Success { usage }` even though the client never received the Done event. The client is billed for tokens they didn't receive (the column-floor billing per FR-AI-002 §1 #12 SHOULD apply but doesn't because we report Completed, not Cancelled).
...
impl Drop for ReconcileGuard {
    fn drop(&mut self) {
        ...
        if let Ok(handle) = tokio::runtime::Handle::try_current() {
            ...
            // job (FR-AI-001 §1 #14) will sweep it in ≤60s.
            tracing::error!(
...
- §3 introduces `ErrorCode::as_metric_label` and `ReconcileReason::as_metric_label` following the FR-AI-007 ISS-003 pattern; OBS labels are rename-safe across the streaming subsystem.
