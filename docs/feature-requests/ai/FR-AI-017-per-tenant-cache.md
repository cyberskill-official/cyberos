...
disallowed_tools:
  - cache across tenants (cache key MUST start with tenant_id; cross-tenant reads are correctness-load-bearing per §1 #2 and exhaustively tested in FR-AI-018)
  - cache the RAW (un-redacted) prompt anywhere — the redacted prompt is the only legitimate cache key
  - cache streaming responses (FR-AI-010 streaming defeats caching anyway; §1 #5)
  ...
  - bypass `cost_ledger::precheck` even on cache hits (audit-before-action invariant from FR-AI-001 §1 #6)
...
1. **MUST** derive the cache key as `SHA-256(canonical_concat(tenant_id, redacted_prompt, model, persona_handle))` where `redacted_prompt` is the post-redaction text from FR-AI-011 (NEVER the raw prompt — caching raw would create a parallel PII repository defeating redaction). `canonical_concat` joins fields with a unit-separator (`\x1f`) to prevent ambiguity (e.g., `tenant_a + "model" = tenant_amodel` collision attack). The hash output is 32 bytes; the Redis key uses its hex encoding.
2. **MUST** isolate per tenant by construction: every Redis key has the prefix `ai_cache:v1:{tenant_id}:`. A `KEYS ai_cache:v1:tenant_a:*` scan MUST never return a `tenant_b` entry. The tenant prefix is a structural invariant — not a runtime check; the key derivation makes cross-tenant reads impossible without direct Redis surgery. FR-AI-018 will exhaustively test this property.
3. **MUST** include the `persona_handle` (full `<id>@<version>` from FR-AI-014) in the key derivation. A persona change MUST invalidate cache automatically — `cuo-cpo@0.4.1` and `cuo-cpo@0.4.2` are distinct keys. This prevents serving an old-persona response to a request that selected a new persona; the keys diverge naturally.
4. **MUST** apply TTLs per alias-class as defined in `cache/ttl.rs`:
   ...
   New alias classes added to FR-AI-006 MUST extend `ttl.rs`; an alias with no TTL entry is treated as `chat.long` semantics (no cache) with a WARN log so the operator notices.
...
6. **MUST NOT** cache responses on streaming calls (FR-AI-010 streaming returns chunks, not a full response). The cache is bypassed entirely on the streaming code path; the handler decides at request time based on `req.stream == true`.
...
16. **SHOULD** support cache-warming hooks at boot for known-stable prompts (e.g., the persona system prompts themselves trigger common opening-summary calls). Slice 4 ships a no-op stub; FR-AI-022 implements warming once we have hit-rate data showing where it pays off.
...
`canonical::invocation_cache_hit` builder additions to FR-AI-002:
...
- **FR-AI-022 (downstream)** — Cache-warming hooks (slice 4 ships a no-op stub).
...
- Per-tenant prefix `ai_cache:v1:{tenant_id}:` is the structural isolation invariant tested in FR-AI-018.
...
- Per-alias-class hit-rate target (chat.fast might warrant 50% floor; embed.* might warrant 70%) — FR-AI-022.
...
- Cross-region cache replication (replicate hot entries between SG and EU Redis instances for residency-aware tenants) — FR-AI-016 area; current cache is single-region per gateway instance.
...
| Cross-tenant leak (key derivation bug) | proptest in `cache_property_test.rs` (THIS FR) + FR-AI-018 (dedicated cross-tenant test FR) | PR blocked on test failure | Fix `CacheKey::derive` |
...
| Unknown alias added to FR-AI-006 without TTL | `ttl_for_alias` returns None + WARN log | `Skipped(ChatLongOrUnknownAlias)`; no cache | Operator extends `ttl.rs` map |
...
- The cross-tenant property test in this FR (§5) is the BASELINE; FR-AI-018 is the DEDICATED cross-tenant leak FR with adversarial scenarios (1000s of tenants, race conditions, malicious tenant_id strings). FR-AI-018 is downstream and EXTENDS this FR's correctness floor.
- The cache-warming hook (§1 #16) ships as a no-op stub. Slice 4 doesn't have hit-rate data showing where warming would pay off; FR-AI-022 implements warming based on the trend analysis. The stub exists so the wiring is in place for the future implementation.
- Persona-handle inclusion in the cache key (§1 #3) means a single-byte change to a persona prompt (via FR-AI-014's hot-reload) invalidates the entire cache for that persona. The trade-off is correctness > hit rate: a stale-persona response served from cache would violate FR-AI-014's hash-tamper-detect contract anyway. Operators should batch persona edits to avoid hit-rate dips.

---

*End of FR-AI-017. Status: done (shipped 2026-06-08).*
