...
#### Suggested fix
Add §4 AC #16: "**Transition sequence deterministic across runs** — Two test runs feeding `[Failure, Failure, Failure, Failure, Failure, Success]` to the same (Bedrock, model-x) breaker with the same MockClock MUST produce byte-identical sequences of `BrainRow` emissions (sorted by `ts_ns` then `extra.provider` then `extra.model`)." Add §5 test `test_transitions_deterministic_across_runs` that runs the sequence twice and asserts `assert_eq!` on the captured `BrainRow` Vec.
