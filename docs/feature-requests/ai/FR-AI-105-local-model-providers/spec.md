...
depends_on: [FR-AI-005, FR-AI-006, FR-AI-007, FR-AI-008, FR-AI-015, FR-AI-003]
---
...
CyberOS should route real model calls to local, no-key inference servers. Stephen runs models locally (LM Studio today, Ollama earlier), and both must work. This adds two provider adapters to the existing FR-AI-008 router: one for LM Studio and any OpenAI-compatible local server (POST to /v1/chat/completions, default http://localhost:1234), and one for Ollama-native (POST to /api/chat, default http://localhost:11434). Each is selectable per tenant through the existing FR-AI-006 alias map and FR-AI-008 fallback chain, holds no API key, and fails closed when the local server is down. The same change flips the HTTP serving path from the in-repo EchoBackend to the real router, so a tenant whose primary provider is local actually gets a model response rather than an echo.

## Problem

The gateway already has the whole control plane: cost ledger (FR-AI-001/002), tenant policy (FR-AI-005), alias resolution (FR-AI-006), the multi-provider router with retry and failover (FR-AI-008), PII redaction (FR-AI-011/012), ZDR (FR-AI-015), residency (FR-AI-016), and per-tenant cache (FR-AI-017). But none of it drives a real model on the local-dev or self-hosted path. The serving endpoint answers through EchoBackend, and the Provider trait's adapters (Anthropic, OpenAI, Bedrock) are stubs that return an error. So today a chat call cannot reach a model without cloud keys, and even with keys the server does not call the router.
...
8. Both local providers MUST be selectable concurrently: a tenant MAY set one as primary and the other (or a cloud provider) in the fallback chain, and FR-AI-008 failover MUST work across the mix.

9. The adapters MUST emit memory audit kinds `ai.local_provider_invoked` and `ai.local_provider_unreachable` through the FR-AI-003 bridge, carrying tenant_id, provider kind, model, and outcome (prompts already hashed at the FR-AI-006 layer).
...
- Source: memory audit rows (FR-AI-003) and the gateway request log.
...
- Target: zero. Local providers hold no key by construction, and FR-AI-017/018 cache isolation still applies.
- Measurement method: the FR-AI-018 cross-tenant leak property test plus a static check that local adapters read no key material.
- Source: FR-AI-018 test output and code review.
...
- Local embeddings (Ollama /api/embeddings, LM Studio /v1/embeddings). The embedding path stays on FR-AI-019 BGE; local embeddings are a separate FR.
...
- FR-AI-008 multi-provider router - the `Provider` trait, retry, and failover that the adapters plug into.
- FR-AI-006 model-alias resolution - maps the tenant alias to a local model id and yields the `ResolvedModel` the router consumes.
- FR-AI-005 tenant-policy loader - the policy schema gains the two local provider variants.
- FR-AI-007 provider cost-table loader - zero-cost rows for local providers, and the loader must tolerate their absence.
- FR-AI-015 ZDR enforcement - local providers are inherently zero-retention; the ZDR parser learns the new kinds.
- FR-AI-003 memory audit bridge - the two new audit kinds.
...
- Tools used: Claude (Cowork), authoring this FR from Stephen's capability request and the existing FR-AI-008 router code.
...
- Human review: Stephen reviews and approves before status moves past draft; the paired audit (FR-AI-105.audit.md) and the CAF gate validate before merge.
