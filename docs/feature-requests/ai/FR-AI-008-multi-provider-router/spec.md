...
source_decisions:
  - docs/feature-requests/ai/FR-AI-006-model-alias-resolution/spec.md §1 (ResolvedModel.fallback_position)
  - docs/feature-requests/ai/FR-AI-007-provider-cost-table-loader/spec.md (cost basis)
...
17. **Traceparent propagated to outbound (feature-request-audit skill §3.7)** — Header-capture mock provider records the inbound HTTPS `traceparent` value of every call. A round-trip test: inbound `cost_ledger::precheck` opens a span with `traceparent: 00-<inbound-trace-id>-<inbound-span-id>-01`; the mock provider's recorded `traceparent` MUST share the same `<trace-id>` (32-hex) component. The `<span-id>` MUST differ (child span) but `<trace-id>` MUST match.
18. **Router does not mutate policy (feature-request-audit skill §3.10 rule 30)** — `Arc::strong_count(&policy)` before and after `router::call_provider(.., &policy, ..)` MUST be identical (no transient clone retained). Asserts the MUST NOT "mutate policy from inside the router."
