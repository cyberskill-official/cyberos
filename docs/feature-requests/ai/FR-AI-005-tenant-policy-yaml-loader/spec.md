...
related_frs: [FR-AI-001, FR-AI-006, FR-AI-015, FR-AI-016]
depends_on: []
blocks: [FR-AI-001, FR-AI-006, FR-AI-021]
...
source_decisions:
  - docs/feature-requests/ai/FR-AI-001-cost-ledger-precheck/spec.md §3 (TenantPolicy shape)
  - docs/feature-requests/ai/FR-AI-001-cost-ledger-precheck/spec.md §9 Q2 (missing-policy default = refuse)
...
new_files:
  - services/ai-gateway/src/policy.rs                         # already declared in FR-AI-001; this FR populates it
...
disallowed_tools:
  - in-place edit of services/ai-gateway/src/cost_ledger.rs (FR-AI-001 owns it; this FR is consumed by it)
...
risk_if_skipped: "FR-AI-001 has no source of per-tenant caps. Every call would use a hardcoded global default, defeating the entire point of the cost-of-everything gate (each tenant has different caps). Worse, the override/residency/ZDR knobs are all keyed to tenant policy — they all fail without this FR."
---

## §1 — Description (BCP-14 normative)

The AI Gateway service **MUST** provide a `policy::TenantPolicy` value type and a loader that hydrates one per-tenant policy from a YAML file at `config/tenants/<tenant_id>.yaml`. The loader is consumed by `cost_ledger::precheck()` (FR-AI-001) and the router (FR-AI-006/008) to determine cost caps, model selection, residency pinning, and override rules.
...
5. **MUST** refuse to apply a hot-reload of an invalid file — the cached (valid) policy stays in place; a `tracing::error!` event surfaces the invalid update; the operator CLI (FR-AI-021) lists tenants whose live policy is stale due to a rejected reload.
...
13. **MUST** expose a `policy::validate_yaml(yaml: &str) -> Result<TenantPolicy, Vec<String>>` public function. FR-AI-021's `cyberos-ai policy validate <file>` operator subcommand wraps this; the bridge stays a thin loader.
...
15. **MUST** sort the loader's `HashMap<String, Arc<TenantPolicy>>` aggregation by tenant_id when iterated for OBS metric emission, log lines on load-success, and the `ai.policy_reload_completed` audit row's `extra.tenants_loaded: Vec<String>` field per feature-request-audit skill §3.9 rule 27 (determinism). Two consecutive runs on the same set of YAML files MUST produce byte-identical sequences. AC #15 asserts `assert_eq!` on the captured `Vec<String>` across two loads.

This FR is the static configuration layer of the AI Gateway. At slice 1 the YAML files live in-repo (checked in to source); at P2 (FR-TEN-004) the TEN module replaces this with a per-tenant database table. The interface stays the same — only the loader's data source changes.

---

## §2 — Why this design (rationale for humans)

**Why YAML files in the repo and not a database?** Two reasons. (1) Slice-1 deploys are bare metal with one tenant (CyberSkill itself). Standing up a tenant-policy table before the TEN module exists is premature work. (2) YAML in source control gives us a git-tracked history of policy changes for free — every cap adjustment is a PR with a diff and a reviewer. The TEN module's runtime policy editor (FR-TEN-005) replicates this audit trail when it lands. The migration path is a one-time script.

**Why fail-closed on missing policy?** From FR-AI-001 §9 Q2: silent defaults are how budget surprises happen in production. The naive "default cap of $10/month if no policy file" sounds defensive but is actually offensive — a new tenant whose policy file we forgot to ship gets a tiny cap and an angry CEO. Loud failures ("policy file not found for tenant org:foo") are uncomfortable but recoverable; silent caps are not.
...
pub enum Residency {
    ...
    Eu1,    // Frankfurt
    Us1,    // us-east-1
    Vn1,    // Vietnam (slice-1: not yet honoured; FR-AI-016 enforces)
}
...
**Code dependencies:**
- None — this FR is foundational. FR-AI-001 consumes its output; no upstream code dependency.
...
- On NFS or similar filesystems where inotify doesn't fire, `notify` falls back to polling at a default 30s interval. This is acceptable for slice 1; if it becomes a problem, FR-AI-021 surfaces the issue.
...
### Consumer in FR-AI-001's precheck
...
3. **~~Provider model_alias_map overlap with cost table~~** → deferred to slice 4 (FR-AI-007 + alias_profile reference). Was Q3.
4. **~~allowed_personas enforcement~~** → FR-AI-001 §1 #13 (enforced in `precheck()`). Was Q4.
5. **~~policy validate CLI subcommand~~** → §1 #13 of this FR exposes `validate_yaml()`; FR-AI-021 wraps it. Was Q5.
...
| Empty `config/tenants/` directory | Walked but no `.yaml` files | `init_loader()` returns Ok with empty cache; gateway starts | Tenants added later via file-watch — but first request will return `PolicyMissing` (FR-AI-001 #9) |
...
- FR-AI-021 (operator CLI) will surface a `policy list` subcommand that walks the cache and prints active policies; useful for "what is my live cap for tenant X" debugging without grepping YAMLs on disk.
- Long-term migration to FR-TEN-004's per-tenant database table: the YAML loader stays as a fallback for ops scenarios where the DB is unreachable (degraded mode). The Postgres backing store becomes primary; YAML becomes the bootstrap + recovery path.

---

*End of FR-AI-005. Run `feature-request-audit` next: `cargo run -p cyberos-skill-cli -- run feature-request-audit --input '{"fr_path": "docs/feature-requests/ai/FR-AI-005-tenant-policy-yaml-loader/spec.md"}'`*
