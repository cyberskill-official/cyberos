...
The first-pass §1 #5 said: *"MUST emit a BRAIN audit row for any mutating operation (set policy, reset breaker, repair holds)."* But no builder shown. The subcommand catalogue listed audit-row kinds (`policy_updated`, `breaker_reset`, `failover_drill_started`, `expiry_repaired`) without specifying their payloads.
...
- `canonical::cli_brain_emitted(operator_id, emitted_kind, command_sha256, request_id)`
