...
8. **MUST** support the following closed set of `ai.*` row kinds (updated 2026-05-16 as additional FRs were specified):
    ...
    - Added by FR-AI-021 (operator CLI) — operator-action rows in `ai.cli_*` sub-namespace: `ai.cli_policy_updated`, `ai.cli_failover_drill`, `ai.cli_invoice_exported`, `ai.cli_breaker_reset`, `ai.cli_expiry_repaired`, `ai.cli_memory_emitted`.
    - Cross-module rows in adjacent namespaces (NOT owned by this bridge but emitted via the module's own writer; registered here per feature-request-audit skill §3.2 rule 8 for the canonical closed-set): `auth.*` per FR-AUTH-001/002/004/006; `memory.sync_row_filtered` per FR-MEMORY-106; `skill.invoked_started` + `skill.invoked_completed` per FR-SKILL-101; `obs.*` per FR-OBS module.
    - **CHAT module rows** (added via the strict-redo 2026-05-16 P.M. expansion of FR-CHAT-002..012; emitted by services/chat/*, services/chat-memory-bridge/*, services/chat-importer/*, services/chat-lumi/*, services/chat-push/*, services/chat-dsar/*):
    ...
    - **PROJ module rows** (added via the strict-redo 2026-05-16 P.M. expansion of FR-PROJ-002, 005..018; emitted by services/proj-sync/* and web/proj-client/*):
        ...
        - FR-PROJ-002 (memory-anchored decision): `proj.decision`, `proj.decision_retracted`.
        ...
        - FR-PROJ-009 (memory links): `proj.memory_link_created`, `proj.memory_link_removed`, `proj.memory_link_traversed`.
...
11. **MUST NOT** expose `memory_writer::emit_batch()` in slice 1. Callers needing N rows MUST call `emit()` N times (or use `futures::future::join_all`). FR-AI-008 (slice 2 PyO3 spike) re-introduces a real batching API.
...
15. **MUST NOT** expose a `dry_run` mode in slice 1. FR-AI-021 (operator CLI) adds a `cyberos-ai memory emit --dry-run` subcommand that validates payload without emission; the bridge itself stays single-purpose.
16. **MUST** emit history events as pairs per feature-request-audit skill §3.8 rule 26: every `ai.precheck_started` row MUST be followed by either `ai.precheck_completed` OR `ai.precheck_failed` within 30s (the precheck deadline). The same pairing rule applies to `ai.reconcile_started` / `ai.reconcile_completed | ai.reconcile_failed`. Standalone `*_started` rows are crash signals — an OBS Grafana lint alerts when `count(*_started) - count(*_completed) - count(*_failed) > 0` over any 5-minute window for any (tenant, module).
17. **MUST** include `extra.trace_id: String` (32-char lower-hex W3C `trace-id` form per feature-request-audit skill §3.7 rule 23) in every emitted `MemoryRow`. The format MUST be produced via OTel `TraceId` Display (NOT Debug — Debug yields `TraceId(0af7…)` while Display yields the 32-char hex per feature-request-audit skill §3.7 rule 24). AC #17 verifies via regex `^[0-9a-f]{32}$` against every emitted row's `extra.trace_id` in the round-trip test.
...
8. **Latency budget** — `emit()` MUST complete within 50ms p95 over a 1000-call integration test against a real memory store on tmpfs. Per-call breakdown logged via `tracing` spans for FR-AI-022.
...
| Writer subprocess non-zero exit | `child.wait()` returns non-success | `Err(WriterFailed { exit_code, stderr })` | Caller bubbles up; FR-AI-021 `cyberos-ai memory doctor` diagnoses |
...
*End of FR-AI-003. Run `feature-request-audit` next: `cargo run -p cyberos-skill-cli -- run feature-request-audit --input '{"fr_path": "docs/feature-requests/ai/FR-AI-003-memory-audit-bridge/spec.md"}'`*
