...
source_decisions:
  - docs/feature-requests/ai/FR-AI-001-cost-ledger-precheck.md §3 (cost_ledger_hold TTL = 60s)
  - docs/feature-requests/ai/FR-AI-002-cost-ledger-postcall-reconcile.md §3 (state machine: held → expired)
...
15. **MUST** emit `ai.hold_expired_started` BEFORE applying `UPDATE cost_ledger SET state='expired'` for each batched hold per feature-request-audit skill §3.8 rule 25 (audit-before-action). The Postgres transaction wraps both the memory-emit AND the UPDATE in one atomic unit; rollback on either failure. `ai.hold_expired_completed` follows post-commit per feature-request-audit skill §3.8 rule 26 (pair-write). The OBS lint flags any standalone `ai.hold_expired_started` over a 5-minute window. AC #15 added with captured-events ordering test.
16. **MUST** order the per-sweep `SELECT ... FOR UPDATE SKIP LOCKED LIMIT 1000` by `ORDER BY hold_id ASC` per feature-request-audit skill §3.9 rule 27 (determinism), so two runs on the same backlog produce byte-identical expire-order. The per-sweep `ai.cleanup_run_completed` audit row's `extra.expired_hold_ids: Vec<Uuid>` MUST be recorded in the same sorted order; operator diffing across runs is reliable. AC #16 added with deterministic-order test.
...
**Why 30 seconds tick interval?** Holds expire after 60s (FR-AI-001's TTL). A 30s tick guarantees an expired hold gets processed within at most 90s of its original creation (60s TTL + 30s tick lag). Tighter intervals (5–15s) just burn CPU; slacker intervals (60–300s) let the hold table grow under traffic spikes. 30s is the goldilocks zone for slice-1 traffic shapes.
...
4. **Leader election for multiple instances** — One instance is enough for the foreseeable future. If someone deploys two by accident, Postgres's `SKIP LOCKED` keeps them safe (one of them just does no work per tick). Proposal: do nothing; FR-AI-021 surfaces a warning if `ps -ef | grep cost_hold_expiry | wc -l > 1` in the operator-CLI health check.
...
- Added `ai.hold_expired_started`, `ai.hold_expired_completed`, and `ai.cleanup_run_completed` typed memory rows through the canonical Writer bridge.
- Added per-hold pair-write event capture proving `hold_expired_started -> sql_update -> commit -> hold_expired_completed`.
- Added deterministic memory-writer failure injection so one hold rolls back while other holds in the tick succeed.
- Ran the declared 5,000-row bounded-batch verification; one tick processes exactly 500 rows and leaves 4,500 held.
- Added process-level binary tests for SIGTERM exit and initial database outage retry.
- Added crash-after-memory-emission verification: the first process exits before DB commit, the next tick emits the documented duplicate `ai.hold_expired` row and expires the hold.
- Added metric assertions for processed/succeeded counters, tick histogram observations, consecutive-failure gauge, and cleanup pending gauge.
- Added `ai.cleanup_run_completed.extra.expired_hold_ids` in deterministic sorted order.
...
cargo test -p cyberos-ai-gateway --test cost_hold_expiry_test -- --test-threads=1 --nocapture
```
...
- `cargo test -p cyberos-ai-gateway --test cost_reconcile_test -- --test-threads=1`: 13 passed.
