...
score_post_expansion: 9.0/10      # after expanding to FR-AI-019 depth (~1010 lines)
...
FR-AI-020 was expanded from 155 lines to ~1010 lines matching FR-AI-019 depth.
...
### ISS-001 — Inherits all FR-AI-019 issues uncorrected (per-region URL, checksum, max-input, fairness, GPU failover)
...
FR-AI-020 is the sibling sidecar to FR-AI-019. The first-pass missed every operational concern that FR-AI-019's audit had identified:

- Per-region sidecar URLs (FR-AI-019 ISS-001) — first-pass had `localhost:5060` equivalent; would break FR-AI-016 residency by construction.
- Model checksum verification (FR-AI-019 ISS-003) — no startup integrity check; supply-chain attack surface.
- Max-input validation (FR-AI-019 ISS-005 + here as token budget) — silent OOM on long candidates.
- Per-tenant fairness in batch buffer (FR-AI-019 ISS-004) — single tenant could starve others.
- Mid-run GPU failover detection (FR-AI-019 ISS-006) — silent latency degradation.
- Cost-accounting clarity (FR-AI-019 ISS-002) — `cost: 0` claim without amortised-infra rationale.

A reader of FR-AI-020 would build a sidecar replicating all the bugs FR-AI-019's audit fixed.

#### Suggested fix

Mirror FR-AI-019's normative clauses systematically:
...
- §1 #14 per-tenant fairness (mirror FR-AI-019 §1 #5).
- §1 #11 mid-run GPU failover detection (mirror FR-AI-019 §1 #15).
- §1 #4 explicit marginal-vs-amortised cost split (cite FR-AI-019 §1 #6).
...
This contradicts the validation discipline established in FR-AI-019 §1 #12 (over-length input returns 413, doesn't silently truncate). Silent truncation hides upstream bugs (KB module didn't pre-filter properly); 413 makes the caller fix the issue.
...
- §3 mirrors FR-AI-019's adapter pattern point-for-point — `RerankProvider` shape matches `BgeProvider`, errors are analogous, batch buffer follows the same per-tenant fairness pattern. Operational consistency across the two sibling sidecars makes both easier to maintain.
...
- **ISS-001 RESOLVED**: Six new §1 clauses added mirroring FR-AI-019 (per-region URL #8, token budget #7, fairness #14, checksum #9, model identity #10, GPU failover #11); cost-accounting clarity in #4 + §2; cross-FR pattern propagated; corresponding ACs + §5 tests + §10 rows.
...
*End of FR-AI-020 audit (final). Status: PASS at 10/10.*
