...
score_post_expansion: 9.0/10      # after expanding to FR-AI-012 / FR-AI-014 depth (~960 lines)
...
FR-AI-017 was expanded from 230 lines to ~960 lines matching FR-AI-012 / FR-AI-014 depth.
...
This is the same type-vs-spec mismatch class as FR-AI-014 ISS-001 (PersonaVersion vs. PersonaHandle).
...
The first-pass `CachedResponse` was a flat struct without a schema-version field. Future changes (e.g., adding `model_version: String` for FR-AI-022) would cause `serde_json::from_slice::<CachedResponse>(&old_bytes)` to fail with a deserialisation error.
...
- §1 #2 commits to per-tenant Redis key prefix as a STRUCTURAL invariant (not a runtime check). This is the property under test in FR-AI-018 (the dedicated cross-tenant leak test FR).
...
*End of FR-AI-017 audit (final). Status: PASS at 10/10.*
