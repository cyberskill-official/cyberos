...
16. **MUST** emit transitions as audit-row pairs per AUTHORING.md §3.8 rule 26: for every `probe_started` row, a `probe_succeeded` OR `probe_failed` row MUST appear within the probe-call deadline (default 30s, capped by the underlying provider call timeout). The pairing is enforced by an OBS lint — a Grafana alert fires if `count(probe_started) - count(probe_succeeded) - count(probe_failed) > 0` for any `(provider, model)` over a 5-minute window. A standalone `probe_started` is treated as a crash signal and routed to oncall.
