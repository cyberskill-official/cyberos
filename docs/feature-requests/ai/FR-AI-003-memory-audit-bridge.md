...
8. **MUST** support the following closed set of `ai.*` row kinds (updated 2026-05-16 as additional FRs were specified):
    ...
    - Cross-module rows in adjacent namespaces (NOT owned by this bridge but emitted via the module's own writer; registered here per AUTHORING.md §3.2 rule 8 for the canonical closed-set): `auth.*` per FR-AUTH-001/002/004/006; `memory.sync_row_filtered` per FR-MEMORY-106; `skill.invoked_started` + `skill.invoked_completed` per FR-SKILL-101; `obs.*` per FR-OBS module.
