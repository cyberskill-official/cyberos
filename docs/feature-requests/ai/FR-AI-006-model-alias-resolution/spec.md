...
source_decisions:
  - docs/feature-requests/ai/FR-AI-005-tenant-policy-yaml-loader/spec.md §3 (Provider enum + model_alias_map)
  - docs/feature-requests/ai/FR-AI-007-provider-cost-table-loader/spec.md §1 (cost entry must exist)
...
modified_files:
  ...
  - services/ai-gateway/src/policy/schema.rs # add AliasOverrides field if not in FR-AI-005
allowed_tools:
...
risk_if_skipped: "Every consumer module (CUO, KB, CHAT, OBS auto-triage) hard-codes (provider, model) tuples. Switching providers becomes a multi-PR change across 22 modules instead of a one-line YAML edit per tenant. FR-AI-008 router cannot work because there's no abstraction layer to redirect to fallback. The cost-of-everything gate becomes per-(provider+model) instead of per-tenant. ZDR + residency enforcement (FR-AI-015 / FR-AI-016) cannot land cleanly because each consumer would need to re-implement the checks. Vendor lock-in returns by the back door."
---
...
5. **MUST** validate that the resolved `(provider, model)` exists in the cost table (FR-AI-007). If `cost_table::lookup(&provider_kind, &model)` returns `None`, the function MUST return `Err(AliasError::ResolvedModelMissingCostEntry { provider, model })`. The cost-entry check prevents resolution-time success that would fail seconds later at the precheck cost-estimate step.
6. **MUST** validate that the resolved provider satisfies `policy.ai_policy.zdr_required`. If `zdr_required: true` and the resolved provider's `is_zdr` attestation (FR-AI-015 source of truth) is `false`, return `Err(AliasError::ZdrViolation)`. The ZDR check runs after cost-entry validation so that a missing-cost-entry error takes precedence (more actionable).
...
8. **MUST** return `ResolvedModel { provider_kind, region, model, fallback_position, is_zdr, latency_class }` on success. Each field carries information needed by downstream code: `provider_kind` drives the `Provider` trait dispatch in FR-AI-008; `region` is logged in audit rows; `model` is the literal string passed to the provider SDK; `fallback_position` flags "this call used a fallback" in OBS dashboards; `is_zdr` propagates to the audit row's `extra.is_zdr` field; `latency_class` informs FR-AI-008's per-call timeout.
9. **MUST** complete synchronously in under **1ms p95** for any input. All state is in-memory (policy is `Arc<TenantPolicy>` from FR-AI-005's cache; cost table is `ArcSwap` from FR-AI-007's cache; ZDR table is `ArcSwap` from FR-AI-015's cache). No I/O, no Postgres, no network.
...
11. **MUST** be safe under concurrent invocation from many tokio tasks. The `ArcSwap::load()` semantics guarantee each call sees a consistent policy snapshot; if FR-AI-005 hot-reloads policy mid-call, the in-flight call uses the snapshot it loaded, and the next call sees the new policy.
...
13. **MUST** be called from `cost_ledger::precheck` (FR-AI-001 §6) BEFORE the cost-estimate step. FR-AI-001's skeleton is updated to call `alias::resolve` and pass the `ResolvedModel` to the rest of the precheck.
...
15. **SHOULD** expose `alias::supported_aliases() -> &'static [&'static str]` so operator CLI (FR-AI-021 `models list`) and the operator UI can enumerate the closed set without re-reading the const.
16. **MUST** be callable without trace-context parameters — it has no I/O and emits no audit row of its own — AND its return value `ResolvedModel` MUST be carried through to `Provider::complete()` so the inbound `traceparent` (read at the trust boundary in `cost_ledger::precheck`) propagates to the downstream HTTP call per feature-request-audit skill §3.7 rule 22. FR-AI-022 §1 #3 owns the propagation contract; this FR's responsibility is to NOT strip the span context from the caller's flow. A spec violation: passing `ResolvedModel` by value to a function that then opens a new span without inheriting from the caller.

This FR provides the abstraction layer that every consumer module (CUO Phase 2, KB ingest, CHAT, OBS auto-triage, PROJ Genie inline) uses to ask for an LLM call without naming a provider. The provider naming is a tenant-policy concern, not an application concern. Once FR-AI-006 ships, the gateway has a single chokepoint for cross-cutting checks (cost-table, ZDR, residency); FR-AI-008 routes the resolved tuple, FR-AI-015/016 enforce policy at the point of resolution, and adding a new provider (Vertex, on-prem Llama, etc.) is a one-file additive change.
...
pub struct ResolvedModel {
    ...
    pub latency_class: LatencyClass,           // fast | standard | slow (drives FR-AI-008 timeout)
}
...
pub enum AliasError {
    ...
    /// The resolved (provider, model) pair has no cost-table entry — FR-AI-007 source of truth.
...
impl LatencyClass {
    /// Map a class to the timeout budget config field — FR-AI-008 reads this.
...
### TenantPolicy schema additions (extending FR-AI-005's `AiPolicy`)

```yaml
# config/tenants/<tenant_id>.yaml — slice-2 additions
ai_policy:
  # … existing fields from FR-AI-005 …
...
```rust
// In FR-AI-005's policy schema, every Provider variant must implement:
pub trait Provider {
    ...
    // of truth for ZDR attestation is FR-AI-015's zdr::is_zdr(&kind, model)
...
16. **Override without provider field** — A malformed override YAML missing the inner `provider` field MUST be rejected at FR-AI-005 load time (not at `resolve` runtime). `resolve` assumes well-formed input.
...
fn check_and_build_with_model(
    ...
    // Cost-table check (FR-AI-007) — first because most common config error
    ...
    // ZDR check (FR-AI-015 attestation table — single source of truth).
    ...
    // Residency check (FR-AI-016 matcher)
    ...
    Ok(ResolvedModel {
        ...
        model: model.to_string(),
        fallback_position,
        // ISS-001 fix: single source of truth for is_zdr — FR-AI-015's attestation table.
...
- **FR-AI-005** — `TenantPolicy` struct + the `alias_overrides`, `residency_requires_regional_provider` field additions. The schema lives in `services/ai-gateway/src/policy/schema.rs`. This FR adds 2 fields if not already present.
- **FR-AI-007** — `cost_table::lookup` function. Used at step 5 of every resolution.
- **FR-AI-015** — `zdr::is_zdr` function (attestation source of truth). Used at step 6.
- **FR-AI-016** — `residency::matches` function. Used at step 7.
- **FR-AI-022** — W3C trace-propagation contract. This FR's §1 #16 asserts `alias::resolve` does not strip span context from its caller; FR-AI-022 owns the propagation contract for the outbound `Provider::complete()` call that follows.
...
- `fallback_position` semantics: override → 0, primary → 0, fallback[N] → N+1. The "override reports as 0" choice matches FR-AI-008's circuit-breaker logic.

**Operational dependencies:**
- The policy cache (FR-AI-005's `ArcSwap<HashMap<String, Arc<TenantPolicy>>>`) must be initialised before any `resolve()` call. Startup order: cost-table → ZDR table → policy loader → router.
...
1. **~~Should latency_class be exposed in ResolvedModel or computed by caller?~~** — RESOLVED: exposed in struct. Callers shouldn't re-derive; mapping is FR-AI-006's concern.
...
| ZDR violation (primary) | `zdr::is_zdr` returns false + `zdr_required: true` | `Err(ZdrViolation { resolved_provider, resolved_model })` | `403 FORBIDDEN` (`zdr_violation`) | Operator switches provider or relaxes `zdr_required`; FR-AI-015 attestation refresh |
...
| Policy not initialised | `OnceCell::get()` returns None (FR-AI-005 init pending) | This case is impossible at runtime — FR-AI-005 init blocks gateway startup | (gateway exits 1) | Operator fixes config; redeploys |
| Override YAML malformed | Rejected at FR-AI-005 load time | (never reaches resolve) | (gateway exits 1) | Operator fixes YAML; redeploys |
...
- This FR is intentionally small in code (6h effort) because the complexity lives in the data model (FR-AI-005's policy schema) and the cross-cutting attestation tables (FR-AI-007 cost, FR-AI-015 ZDR, FR-AI-016 residency). The function itself is ~150 lines of Rust.
- After this FR ships, every other AI Gateway FR uses `alias::resolve()` instead of inline alias logic. A refactor PR will touch FR-AI-001's §6 skeleton to call `resolve()` rather than the inline `resolve_model_alias` placeholder. The refactor is mechanical.
...
*End of FR-AI-006. Status: draft (10/10 target, expanded from compressed first-pass per workflow correction 2026-05-15). Run `feature-request-audit` next: `cargo run -p cyberos-skill-cli -- run feature-request-audit --input '{"fr_path": "docs/feature-requests/ai/FR-AI-006-model-alias-resolution/spec.md"}'`*
