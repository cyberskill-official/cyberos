...
sub_tasks:
  ...
  - "1.8h: GenieActionsPanel.tsx (live FR-EMAIL-008 stream)"
...
5. **MUST** wake snoozed threads at `snoozed_until` via FR-MCP-007 cron — flip `state` to `open`, notify assignee via FR-CHAT-005.
...
6. **MUST** render frontend at `services/portal-web/src/email/`:
   ...
   - `GenieActionsPanel.tsx`: streams from FR-EMAIL-008 + quick-actions
...
11. **MUST NOT** send email notification on assignment per DEC-1611 — in-app + FR-CHAT-005 mention only.
...
**Why no email notif on assignment (DEC-1611)?** Customer doesn't need to see "Stephen reassigned to Lisa" emails; internal-only via FR-CHAT-005.
...
**Why Genie panel right-side (DEC-1614)?** Active context without disrupting reading flow; consistent with FR-PORTAL-005 chat layout.
...
| Internal comment >10k chars | validate | 400 | use FR-DOC-001 for long |
| Reply quote source includes comment (bug) | test guard | hard CI block | tests catch |
| FR-CHAT-005 unreachable | mention notif retry | inherent | inherent |
