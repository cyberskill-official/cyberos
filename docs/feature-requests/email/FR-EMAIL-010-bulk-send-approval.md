...
source_decisions:
  - DEC-1490 2026-05-17 — Bulk send threshold ≥ 10 distinct recipients triggers approval workflow; below = ordinary 1:1 via FR-EMAIL-009
...
sub_tasks:
  ...
  - "0.5h: dispatch.rs (per-recipient FR-EMAIL-009 spawn)"
  - "0.4h: rate_pacer.rs"
...
3. **MUST** expose draft `POST /v1/email/bulk/draft` body `{ to_addrs[], subject, body }`. Validates `to_addrs.len() >= 10` (below uses FR-EMAIL-009). Status='pending_am_sign'. Emits `email.bulk_drafted` sev-2.
...
| FR-EMAIL-009 send error per recipient | per-recipient try | failed_count++; continues | Inherent |
