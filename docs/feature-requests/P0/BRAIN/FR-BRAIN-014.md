---
title: "Expose `brain.forget(member_id)` admin tool: locate every chunk where…"
author: "@cyberos-bot"
...
expose `brain.forget(member_id)` admin tool: locate every chunk where `dsar_marker = member_id` OR `metadata.author_id = member_id` OR `metadata.subject_id = member_id`; soft-delete (tombstone) immediately; hard-delete after 30-day legal hold; emit `brain.dsar_purged` audit event.

<!-- source: SRS FR-BRAIN-014 · module BRAIN · phase P0 · moscow MUST -->
