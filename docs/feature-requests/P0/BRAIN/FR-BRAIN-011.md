---
title: "Expose `brain.search(query, scope?, k=10)`: hybrid (BM25 + vector + P…"
author: "@cyberos-bot"
...
expose `brain.search(query, scope?, k=10)`: hybrid (BM25 + vector + PGroonga via RRF k=60); returns chunks with provenance link `cyberos://{module}/{path}`.

<!-- source: SRS FR-BRAIN-011 · module BRAIN · phase P0 · moscow MUST -->
