---
title: "Expose per-tenant resources via `cyberos://{module}/{path}` URIs (e"
author: "@cyberos-bot"
...
expose per-tenant resources via `cyberos://{module}/{path}` URIs (e.g., `cyberos://kb/{doc_id}`, `cyberos://chat/{channel_id}`, `cyberos://proj/{task_id}`, `cyberos://crm/contacts/{id}`).

<!-- source: SRS FR-MCP-006 · module MCP · phase P0 · moscow MUST -->
