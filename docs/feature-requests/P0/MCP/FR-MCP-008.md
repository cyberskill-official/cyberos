---
title: "Emit audit log for every write tool with `actor_kind` (user|agent|sys…"
author: "@cyberos-bot"
...
emit audit log for every write tool with `actor_kind` (user|agent|system|genie), `actor_id`, `tool`, `params`, `before`, `after`, `tenant_id`, `occurred_at`.

<!-- source: SRS FR-MCP-008 · module MCP · phase P0 · moscow MUST -->
