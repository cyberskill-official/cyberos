---
title: "Propagate `authorization: Bearer <jwt>` and `x-tenant-id` to every su…"
author: "@cyberos-bot"
...
propagate `authorization: Bearer <jwt>` and `x-tenant-id` to every subgraph + MCP request.

<!-- source: SRS FR-AUTH-007 · module AUTH · phase P0 · moscow MUST -->
