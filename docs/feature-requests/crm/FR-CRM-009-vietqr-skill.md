...
risk_if_skipped: "Without VietQR generation, CFO sends bank details as text — customer mis-types account number (payment lost). Without DEC-1694 memo template, payment unreconcilable (FR-INV-005 can't match). Without DEC-1693 CFO-only gate, anyone can change payout account (fraud risk)."
---
...
4. **MUST** gate bank config writes to CFO role per DEC-1693 via FR-AUTH-101.
...
6. **MUST** generate memo per DEC-1694 — template `{tenant_short}-{deal_id_first_8}`. Must match FR-INV-005 reconciliation pattern.
...
**Why memo template (DEC-1694)?** FR-INV-005 reconciles inbound VietQR payments by memo string; mismatch = orphan payment.

**Why amount cap (1B VND, DEC implicit)?** Napas spec; transfers >1B require separate higher-ceiling rail (FR-TEN-102 covers).
...
| Amount > 1B VND | validate | 400 with rail-switch suggestion | use FR-TEN-102 |
