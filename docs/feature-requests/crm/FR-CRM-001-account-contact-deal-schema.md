...
source_decisions:
  ...
  - DEC-341 (4 default pipeline shapes: sales · partner · inbound · outbound; tenants can create custom pipelines but must derive from one of the 4 shapes for the FR-CRM-005 next-action skill to apply)
...
new_files:
  ...
  - services/crm/migrations/0006_seed_pipelines.sql            # seed 4 default pipelines per tenant on creation (run via FR-TEN-001 hook)
...
disallowed_tools:
  ...
  - ship full VN-specific account_type enum (FR-CRM-003 ships)
...
sub_tasks:
  ...
  - "0.3h: 0006_seed_pipelines.sql — 4 default pipelines seed function for FR-TEN-001 hook"
...
4. **MUST** declare the closed `pipeline_shape` Postgres enum with exactly 4 values (per DEC-341): `'sales'`, `'partner'`, `'inbound'`, `'outbound'`. Adding a 5th shape is an ADR. The shape gates the FR-CRM-005 next-action skill's applicability.
...
**Why language_code on contact (§1 #2)?** Vietnamese-aware salutations + outreach templates need to know the contact's preferred language. Reusing FR-KB-001's `language_code` enum keeps consistency.
...
-- Placeholder enum at slice 1; full VN-specific enum ships in FR-CRM-003.
...
pub struct Account {
    ...
    pub account_type: String,   // closed enum expansion in FR-CRM-003
...
23. **seed_default_pipelines creates 4 pipelines** — called per FR-TEN-001 hook; 4 pipelines × 6 stages each = 24 stages seeded.
...
- **FR-AUTH-101** — RBAC catalogue (`Resource::CrmAccount`, `CrmContact`, `CrmDeal`).

**Downstream (3 placeholders):**
- **FR-CRM-002** — activity feed auto-log from EMAIL/CHAT/Calendar.
- **FR-CRM-003** — VN-specific account_type enum + MST validation.
...
Deferred:
- **VN-specific account_type enum (Sole / LLC / JSC / FDI)** — FR-CRM-003.
- **MST format + GDT registry validation** — FR-CRM-003 + FR-CRM-008.
- **Activity feed auto-log** — FR-CRM-002.
- **Convert-to-Engagement workflow** — FR-CRM-004.
- **Next-action CUO skill** — FR-CRM-005.
- **AI lead scoring** — FR-CRM-006.
- **Win/loss analysis CUO draft** — FR-CRM-007.
- **VietQR invoice collection** — FR-CRM-009.
...
- **Closed pipeline_shape enum drives FR-CRM-005 next-action skill heuristics** — each shape has different optimal moves.
...
- **`account_type` placeholder enum at slice 1** — FR-CRM-003 ships full VN-specific expansion.
- **MST stored but unvalidated at slice 1** — FR-CRM-003 + FR-CRM-008 ship validation against GDT.
