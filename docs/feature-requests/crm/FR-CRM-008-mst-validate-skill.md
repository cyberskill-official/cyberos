...
risk_if_skipped: "Without validation, invalid MSTs persist → FR-INV-007 hóa đơn rejected by GDT at emit time. Without DEC-1683 non-blocking, account save fails when GDT unavailable (frustrating). Without DEC-1684 cache, GDT rate-limits us."
---
...
| MST format invalid | FR-CRM-003 CHECK | format_invalid (no HTTP) | data fix |
