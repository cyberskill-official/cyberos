---
id: FR-CRM-007
title: "CRM win/loss analysis CUO draft — auto-generate analysis at deal close + BRAIN memory persistence for institutional learning"
module: CRM
priority: SHOULD
status: draft
verify: T
...
source_decisions:
  ...
  - DEC-1673 2026-05-17 — Analysis persists as BRAIN memory (kind=lessons) via FR-BRAIN-111 — searchable for similar future deals
  - DEC-1674 2026-05-17 — Draft requires CDO review before BRAIN persistence — never auto-persist (PII + accuracy risk)
  - DEC-1675 2026-05-17 — BRAIN audit kinds: crm.wl_draft_created, crm.wl_draft_approved, crm.wl_brain_memory_written, crm.wl_dismissed
...
build_envelope:
  ...
  new_files:
    ...
    - services/crm/src/win_loss/brain_writer.rs
    ...
    - services/crm/tests/wl_brain_write_on_approve_test.rs
...
sub_tasks:
  ...
  - "0.5h: brain_writer.rs (BRAIN memory persist)"
...
risk_if_skipped: "Without win/loss analysis, lessons evaporate at deal close — no institutional learning. Without DEC-1674 manual review, AI hallucination persists wrong narrative as 'truth'. Without DEC-1673 BRAIN persistence, future similar deals don't benefit."
---

## §1 — Description (BCP-14 normative)

The CRM service **MUST** ship win/loss analysis at `services/crm/src/win_loss/` triggered on deal close, drafted via FR-AI-003, queued for CDO review, persisted to BRAIN on approval, 4 BRAIN audit kinds.
...
4. **MUST** queue draft for CDO review per DEC-1674 — NEVER auto-persist to BRAIN.

5. **MUST** on approval, call `brain_writer.rs::write(draft)` to persist BRAIN memory per DEC-1673 — `kind=lessons`, searchable via FR-BRAIN-108 query.
...
6. **MUST** define table at migration `0007`:
   ...
   GRANT UPDATE (status, reviewed_by, reviewed_at, brain_memory_path) ON crm_win_loss_drafts TO cyberos_app;
   ```

7. **MUST** emit 4 BRAIN audit kinds per DEC-1675. PII per FR-BRAIN-111: analysis_body text hashed.

8. **MUST** thread trace_id from deal close → draft → CDO review → BRAIN write → audit.

9. **MUST NOT** auto-persist BRAIN memory per DEC-1674.
...
**Why BRAIN persistence (DEC-1673)?** Future similar deals query lessons via FR-BRAIN-108; without persist, lessons stay siloed in deal record.
...
POST   /v1/crm/win-loss/drafts/{id}/approve  (writes to BRAIN)
...
1. **Auto-drafted on close**. 2. **6-outcome enum + cardinality test**. 3. **Queued for review, never auto-persist**. 4. **Approve writes to BRAIN memory**. 5. **Dismiss → status=dismissed**. 6. **4 BRAIN audit kinds emitted**. 7. **PII scrubbed (analysis_body SHA256)**. 8. **RLS denies cross-tenant**. 9. **CDO/CRO role only**. 10. **Trace_id preserved**. 11. **Idempotent (UNIQUE on deal_id)**. 12. **Append-only via REVOKE except review/path cols**. 13. **AI failure → status=failed + sev-2 + retry**. 14. **brain_memory_path populated post-approve**. 15. **GET endpoint lists pending drafts**. 16. **Outcome required**. 17. **Lessons text non-empty**. 18. **Customer feedback may be empty if no signals**. 19. **Re-open deal → no auto-redraft (manual trigger)**. 20. **BRAIN memory tagged `kind=lessons` searchable via FR-BRAIN-108**.
...
async fn never_auto_persists() {
    ...
    let brain_memories = ctx.brain_memory_count_for_deal(ctx.deal_id).await;
    assert_eq!(brain_memories, 0);
    let drafts = ctx.fetch_drafts_for_deal(ctx.deal_id).await;
    assert_eq!(drafts[0].status, "pending_review");
}

#[tokio::test]
async fn approve_writes_brain() {
    ...
    assert!(row.brain_memory_path.is_some());
    let mem = ctx.brain_fetch(row.brain_memory_path.unwrap()).await;
...
**Cross-module:** FR-CRM-002 (activity context), FR-AI-003 (draft), FR-BRAIN-111 (PII + memory write), FR-AUTH-101 (CDO role), FR-BRAIN-108 (search).
...
| BRAIN write fail on approve | sev-1; status=approved-pending-retry | retry | inherent |
...
- §11.2 BRAIN memory path: `memories/lessons/<deal_id_prefix>/<draft_id>.md`.
- §11.3 BRAIN memory body: human-readable analysis text + structured tags (outcome, competitors, lessons).
- §11.4 BRAIN audit body: deal_id, outcome enum; analysis SHA256.
- §11.5 Search via FR-BRAIN-108: `kind=lessons AND outcome=lost_competitor` returns similar lost-to-competitor analyses.
