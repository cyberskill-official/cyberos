...
sub_tasks:
  ...
  - "0.7h: ranker.rs (FR-AI-003 prompt)"
...
3. **MUST** build context at `context_builder.rs::build(deal_id)`:
   ...
   - Last 20 activities from FR-CRM-002
   - Contact engagement (last email reply gap, etc.)
   - Account history (won/lost recent deals)

4. **MUST** rank at `ranker.rs::rank(context)` via FR-AI-003 with structured prompt:
...
**Why context from FR-CRM-002 (DEC-1652)?** Activity feed is the truth source; without it AI hallucinates.
