...
sub_tasks:
  ...
  - "0.5h: nightly_refresh.rs (FR-MCP-007 cron)"
...
2. **MUST** run nightly refresh per DEC-1660 at `nightly_refresh.rs::refresh_all(tenant)` triggered by FR-MCP-007 cron at 02:00 tenant_tz — refresh all active contacts.

3. **MUST** build signals at `signal_builder.rs::build(contact_id)` per DEC-1662:
   - Title seniority (Director / VP / C-suite weight higher)
   - Engagement count last 30d (from FR-CRM-002 activity feed)
