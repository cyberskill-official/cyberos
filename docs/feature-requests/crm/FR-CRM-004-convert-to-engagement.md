...
4. **MUST** require user to provide: `rate_card_id` (FR-PROJ-005), `recognition_method` (FR-INV-011 enum), `assigned_am_id`, `engagement_start_date`. Auto-pop from deal: `account_id`, `deal_value`, `currency`, `contact_id`.

5. **MUST** call FR-PROJ-005 engagement create endpoint with assembled payload.
...
**Why FR-INV-011 recognition method enum (DEC-1641)?** Engagement billing depends on this — pre-binding at creation prevents mid-engagement changes.
...
| FR-PROJ-005 create fails | downstream error | conversion=failed; rollback insert | retry |
...
- §11.2 Conversion broadcasts `crm.deal_converted` event for FR-CRM-002 activity log.
...
- §11.5 Future AI mode (FR-CRM-006): suggests rate_card/method/AM based on similar past conversions.

---

*End of FR-CRM-004 spec.*
