...
2. **MUST** `GET /v1/admin/subjects?tenant_id=<id>&cursor=<opaque>&limit=<int>` — tenant-admin (same tenant) OR root-admin (with `X-Switch-Tenant` set); paginated list of subjects in the specified tenant. Cross-tenant requests (tenant-admin of A querying tenant B) return `403`. RLS at the DB layer (FR-AUTH-003) provides defence-in-depth — even if the API check is bypassed, the SELECT returns zero rows. Subject body NEVER includes `password_hash`.
...
8. **MUST** support `Idempotency-Key` header on revoke + unrevoke (mirrors FR-AUTH-001 §1 #5 pattern). Repeat revoke with same key + same subject_id → no-op return; same key + different subject_id → 409.
...
10. **MUST** maintain `sessions` table tracking active jtis: `(jti TEXT PK, subject_id UUID, tenant_id UUID, issued_at, expires_at, source_ip_hash16)`. Inserted on every token issue (FR-AUTH-004); used by revoke to enumerate active jtis to deny-list.
...
12. **MUST** require revoked subjects to re-authenticate via FR-AUTH-004's `/v1/auth/token` with fresh credentials AFTER unrevoke. The deny-list does NOT clear on unrevoke (security default — explicit re-auth proves the operator's intent, not just the unrevoke action).
13. **MUST** include `sessions` table in `TENANT_SCOPED_TABLES` registry (FR-AUTH-003 §1 #1) so RLS applies. Tenant-admin can only list sessions for their own tenant.
...
**Why `sessions` table (§1 #10)?** Without tracking issued jtis, revoke can't enumerate which jtis to deny-list. The sessions table is the registry; revoke joins on `subject_id` to find active jtis. Cleanup is automatic via the JWT exp (expired sessions are pruned by FR-AUTH-006 sweeper).
...
CREATE TABLE sessions (
    jti              TEXT PRIMARY KEY,                 -- ULID from FR-AUTH-004
...
18. `sessions` table populated by FR-AUTH-004 token issue.
...
- **FR-AUTH-001..004** — tenants/subjects/RLS/JWT all required.
- **FR-AUTH-006 (downstream)** — sweeper deletes expired sessions rows.
...
| Sessions table unbounded growth | sweeper deletes expired | Storage OK | FR-AUTH-006 sweeper |
...
*End of FR-AUTH-005. Status: draft (10/10 target).*
