...
18. **MUST** apply the breach check to password resets initiated by the subject (FR-AUTH-102 password-reset flow) as well as admin-initiated resets. The reset-flow handler invokes the same `hibp::check` function as signup/rotation/admin-set — there is one canonical check function with three call sites.
...
8. Password reset flow (FR-AUTH-102) routes through same check.
...
**Depends on**: FR-AUTH-002 (subject_create — wiring point), FR-AUTH-005 (admin REST — wiring point).
...
- **False negatives** (breached password accepted): one user's account is credential-stuffing vulnerable; mitigated by MFA at FR-AUTH-102.
...
**§11.4** Per-tenant policy is cached in the AUTH process with a 60s TTL (matching the FR-AUTH-109 active-tenant pattern). Mutation invalidates the cache via pg_notify.
