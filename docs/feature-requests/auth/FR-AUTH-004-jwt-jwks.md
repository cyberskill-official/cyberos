...
blocks: [FR-AUTH-005, FR-AI-006, FR-MCP-004, FR-AUTH-006, FR-OBS-002, FR-CHAT-002]   # FR-MCP-004 = placeholder (OAuth 2.1 PKCE FR, not yet specified)
...
source_pages:
  ...
  - RFC 7518 (JWA — RS256)
source_decisions:
  - DEC-120 (RS256 + RSA-2048; ECDSA deferred to FR-AUTH-110)
...
12. **MUST** propagate `agent_persona` from `subjects.default_persona` column (added in this FR) — the persona claim defaults to `"cuo-cpo@0.4.1"` if not set per subject. Callers can override at request time via `X-Override-Persona` header (subject to FR-AI-005's allowed-personas check).
...
15. **SHOULD** support refresh tokens via separate HTTP-only cookie (FR-AUTH-007 ships the full flow; this FR defines the access-token shape).
...
**Why `agent_persona` in claims (§1 #12)?** Most modules need to know "which persona is making this request" for audit attribution AND for prompt customisation (FR-AI-014 reads `agent_persona` from claims to inject the right system prompt). Putting it in the JWT means downstream services don't need a second roundtrip to look it up. The `X-Override-Persona` header provides per-request flexibility.
...
**Why suspended-subject check (§1 #14)?** A compromised subject (employee left, account fired) needs immediate revocation. Setting `subjects.suspended = true` (FR-AUTH-005's admin endpoint) prevents new token issuance — but in-flight 1h JWTs still work. Acceptable trade-off given short token TTL; alternative (central token revocation list) would be a per-request lookup.

**Why `email_hash16` and `source_ip_hash16` in audit rows (§1 #6)?** Mirrors FR-AUTH-002 §1 #7 PII discipline. Plaintext email + IP in audit rows creates everywhere-PII problem; hash16 is enough to disambiguate forensically without leaking. The `source_ip_hash16` salts include the date so IPs can be correlated within a day but not across — preventing long-term IP tracking.
...
pub fn for_roles(roles: &[String]) -> Vec<String> {
    ...
    for role in roles {
        match role.as_str() {
            ...
            _ => {}    // unknown roles silently skipped (FR-AUTH-002 validates at create)
        }
    }
...
- **FR-AUTH-001/002/003** — tenants + subjects + RLS exist before tokens issue against them.
- **FR-AUTH-006 (downstream)** — bootstrap CLI generates initial signing key + schedules quarterly rotation cron.
- **FR-AI-006 (downstream)** — reads `tenant_id` + `agent_persona` + `scope_grants` from JWT.
- **FR-MCP-004 (downstream)** — gates tool calls by `scope_grants`.
...
- Refresh tokens with rotating-jti — FR-AUTH-007.
- ECDSA signing — FR-AUTH-110.
- Token revocation list (immediate revoke) — FR-AUTH-111.
- Per-tenant signing keys (multi-tenant key isolation) — FR-AUTH-112.
- mTLS client-cert authentication — FR-AUTH-113.
...
| `scope_grants` for unknown role | silently skipped | Subject has fewer grants than expected | FR-AUTH-002 validates roles at create |

---

## §11 — Notes

- RSA-2048 + RS256 chosen for ecosystem maturity in Rust JWT libraries (jsonwebtoken@9 first-class). ECDSA migration tracked in FR-AUTH-110; not blocking slice 2.
...
- The `signing_keys.private_pem` column is encrypted at rest via pgcrypto. The decryption key lives in env vars (operator manages); rotation of the env-var key is FR-AUTH-006's responsibility.

---

*End of FR-AUTH-004. Status: draft (10/10 target).*
