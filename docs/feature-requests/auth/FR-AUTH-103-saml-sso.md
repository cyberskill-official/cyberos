...
4. **MUST** define `auth_saml_subject_link` table: `(idp_id UUID, nameid TEXT, tenant_id UUID, subject_id UUID NOT NULL REFERENCES auth.subjects(id), linked_at TIMESTAMPTZ, PRIMARY KEY (idp_id, nameid))`. Per-(idp_id, nameid) uniqueness (same shape as FR-AUTH-104 OIDC).
...
16. **MUST** apply per-tenant attribute → role mapping (per DEC-528). Reuses FR-AUTH-104 YAML shape but with SAML AttributeStatement claim names:
   ...
   claim_rules:
     ...
     - attribute: "groups"   # generic
       ...
       grant_role: cfo
   ```
   Validation: every `grant_role` MUST parse to FR-AUTH-101 closed Role enum at config save time.
...
**Upstream:**
- **FR-AUTH-004** — JWT issuer (post-SAML callback issues our token).

**Downstream (1 placeholder):**
- **FR-PORTAL-003** — external IdP SAML 2.0 + OIDC support; consumes both FR-AUTH-103 + FR-AUTH-104.
...
- **Per-tenant attribute_mapping_yaml** — same shape as FR-AUTH-104.
