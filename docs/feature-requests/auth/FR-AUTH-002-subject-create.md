...
blocks: [FR-AUTH-004, FR-AUTH-005, FR-AUTH-006]
...
disallowed_tools:
  ...
  - allow roles outside the slice-1 allow-list (per §1 #5; FR-AUTH-101 expands)
...
**Why does the response not include the password hash (§1 #8)?** No legitimate caller needs the hash. Including it in the response would let a compromised admin client extract every subject's hash by listing subjects — an offline-cracking surface that doesn't need to exist. The hash is for `bcrypt::verify` only, called from the JWT issuance path (FR-AUTH-004).

**Why bcrypt and not Argon2 right now?** Argon2id is technically superior (memory-hard; resistant to GPU cracking). Bcrypt is widely supported, well-understood, and has a large ecosystem. Slice 1 ships bcrypt; slice 4 (FR-AUTH-114) introduces argon2 with a migration path (existing subjects re-hash on next login). Shipping both at slice 1 doubles the verification path complexity for marginal security gain at our threat model.
...
- **FR-AUTH-001** — Tenants table + RLS templates (subjects MUST be added to `TENANT_SCOPED_TABLES`).
- **FR-AUTH-004 (downstream)** — JWT issuance reads `password_hash` for `bcrypt::verify` during login.
- **FR-AUTH-101 (downstream)** — Expands the role allow-list from 2 to 22.
- **FR-AUTH-114 (downstream)** — Argon2id migration; this FR ships bcrypt only.
...
- Argon2id migration (FR-AUTH-114).
- 22-role allow-list expansion (FR-AUTH-101).
- MFA enrolment (FR-AUTH-115).
- Password rotation policy + history (FR-AUTH-116).
...
| Top-10K list out of date | manual refresh process | False negatives possible | Refresh quarterly via FR-AUTH-006 sweeper |
...
- The subjects table is added to `TENANT_SCOPED_TABLES` in `rls/templates.rs` (FR-AUTH-001 §3) — tenant create automatically applies the standard tenant_id RLS policy.
- Argon2id migration (slice 4, FR-AUTH-114) will work as: on next successful login, if `password_hash` starts with `$2b$`, compute argon2 hash and update the row. Both formats coexist during migration; FR-AUTH-004's verify path tries argon2 first then bcrypt.
- The slice-1 role allow-list (`tenant-admin`, `tenant-member`) is intentionally minimal. Most operational roles (`finance-admin`, `pii-officer`, etc.) ship in FR-AUTH-101 with the 22-role expansion.
