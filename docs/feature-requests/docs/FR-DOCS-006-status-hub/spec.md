...
related_frs: [FR-TPL-001, FR-DOCS-003, FR-DOCS-005]
depends_on: [FR-TPL-001]
blocks: []
source_pages:
...
1. A builder `tools/docs-site/render-status-hub.mjs` MUST emit `dist/website/reference/status.html` through the `status-hub@1` template from the SAME three inputs as FR-DOCS-003 (FR frontmatter, CHANGELOG sections, VERSION) - node stdlib only, deterministic stamp (VERSION + commit, no wall clock).
...
3. Three tabs MUST render: **Roadmap** (default; the FR-DOCS-003 board + module rollups, rows linking to FR pages when FR-DOCS-005 pages exist), **Backlog** (filterable table: id, title, module, class, priority, status - module/class/status/priority facets; BACKLOG.md stays the agents' write-path index, this tab is the human view), **Changelog** (the release timeline). Tab routing via URL hash (`#roadmap`, `#backlog`, `#changelog`) with hash-change handling; no JS -> all three panels present in the DOM and the tab bar degrades to in-page anchor links.
4. `roadmap.html` MUST become a redirect stub to `status.html#roadmap` (meta refresh + link, no JS required); the shared nav's entry MUST become "Status" pointing at status.html; FR-DOCS-003's suite is repointed at the hub builder with its assertions preserved (board counts, determinism, honest failures, token-clean) - the superseding is recorded on FR-DOCS-003 as a post-ship amendment.
5. Inline vanilla JS only, CDS tokens only (template rule), same honest failures as FR-DOCS-003 §1 #5 (unparseable FR names file; zero CHANGELOG sections fails).
...
6. **FR-page links** (§1 #3) - board/backlog rows href `frs/<module>/<STEM>/` when FR-DOCS-005 is present in the build (fixture with pages asserts hrefs).
...
FR-TPL-001 (shell). Soft: FR-DOCS-005 (links activate when pages exist; builder feature-detects the frs/ output dir).
...
Keep FR-DOCS-003's greppable prefixes alive in the new summary line; deploy logs already watch them.
