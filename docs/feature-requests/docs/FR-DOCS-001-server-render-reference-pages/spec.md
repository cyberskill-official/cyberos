...
9. **MUST** be Pagefind-indexable — re-running `pagefind --site docs/` after prerender produces an index that returns hits for `NFR-PERF-01`, `RSK-09`, `FR-AI-001` queries.
    ...
    steps:
      ...
      - name: Pagefind index test
        run: |
          ...
          npx pagefind search "FR-AI-001" | jq -e '.results | length > 0'
```
...
2. **Pagefind indexes the data** — `pagefind search "NFR-PERF-01"` returns the nfr-catalog page. Same for `RSK-09` and `FR-AI-001`.
...
npx pagefind search "FR-AI-001" | jq '.results | length'     # expect > 0
...
test('render produces FR cards with all metadata', async () => {
    ...
    assert.ok(html.includes('FR-AI-001'));
