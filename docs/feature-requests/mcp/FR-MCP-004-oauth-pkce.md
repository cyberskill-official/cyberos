...
source_decisions:
  ...
  - DEC-805 2026-05-16 — Token format: signed JWT (RS256 or ES256) per FR-AUTH-004 JWKS; access_token TTL = 1 hour, refresh_token TTL = 30 days
  ...
  - DEC-807 2026-05-16 — Closed `oauth_grant_type` enum (authorization_code, refresh_token) — no client_credentials at this slice (deferred to FR-MCP-007), no implicit, no password
  ...
  - DEC-819 2026-05-16 — Discovery via RFC 8414 `/.well-known/oauth-authorization-server` — separate from FR-MCP-005 Protected Resource Metadata (PRM)
...
1. **MUST** implement exactly two grant types: `authorization_code` and `refresh_token` (DEC-800 + DEC-807). Implicit, resource-owner-password, and client_credentials are NOT in scope for this slice (client_credentials is deferred to FR-MCP-007). Unsupported grant_type returns `400 unsupported_grant_type` per RFC 6749 §5.2.
...
7. **MUST** issue access tokens as signed JWTs using the FR-AUTH-004 JWKS keys (RS256 or ES256). The token claims MUST include:
   - `iss`: the MCP-gateway URL.
   - `aud`: the canonical URL of the MCP resource server (DEC-802 + RFC 8707).
   - `sub`: the authorizing subject's UUID (FR-AUTH-002).
   ...
   - `tenant_id`: the tenant binding (per FR-AUTH-108 Lumi pattern).
...
18. **MUST** implement RFC 7591 Dynamic Client Registration at `POST /register`. Request body:
    ...
    Public client registration is open (no caller auth); confidential client registration requires `tenant_admin` role per FR-AUTH-101.
...
20. **MUST** implement RFC 8414 Discovery at `GET /.well-known/oauth-authorization-server`. Response includes:
    ```json
    {
      ...
      "scopes_supported": [<from FR-MCP-001 registry>]
    }
    ```
...
23. **MUST** verify audience binding at every MCP resource server hot path (DEC-802). The MCP gateway's `tools/call` handler MUST:
    - Decode the bearer JWT via FR-AUTH-004 JWKS.
...
30. **MUST** validate `scope` against the MCP-server's registered scope set (FR-MCP-001 `tools/list` registry — DEC-813). Unknown scopes return `400 invalid_scope`. Scope strings are case-sensitive and conform to RFC 6749 §3.3 syntax (visible printable, no whitespace beyond the space separator).
...
26. Scope outside FR-MCP-001 registry returns `400 invalid_scope`.
...
**Depends on**: FR-AUTH-004 (JWKS for JWT signing), FR-MCP-001 (scope registry from tools/list).

**Blocks**: FR-MCP-005 (Protected Resource Metadata), FR-MCP-006 (resource indicators), FR-MCP-007 (client_credentials grant), FR-MCP-008 (tenant-admin token revocation UI).
...
**§11.3** Access JWT signing uses the FR-AUTH-004 active signing key from JWKS. Key rotation invalidates all in-flight access tokens (acceptable — they re-issue via refresh).
...
**§11.11** Tests use the FR-AUTH-004 testcontainers JWKS fixture for sign + verify.
...
**§11.17** The `consent_screen` is HTML rendered by the gateway with the client name + scope descriptions; the descriptions come from the FR-MCP-001 registry.
