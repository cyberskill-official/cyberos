...
source_decisions:
  ...
  - DEC-1145 2026-05-17 — Elicitation transport: HTTP polling (caller GETs pending elicitations for their session) + NATS push (optional); polling REQUIRED, push OPTIONAL — mirrors FR-MCP-007 progress polling pattern
  ...
  - DEC-1153 2026-05-17 — File-upload elicitation_type returns a presigned S3 URL (FR-DOC-001) for the caller to upload to; max 100 MiB; ttl = elicitation timeout
...
build_envelope:
  ...
  modified_files:
    ...
    - services/mcp/src/gating/elicit.rs                                # de-stub the elicit mode placeholder from FR-MCP-006
...
sub_tasks:
  ...
  - "0.3h: de-stub FR-MCP-006 gating/elicit.rs placeholder"
  - "0.3h: wire-up server_registry + tasks/mod.rs ctx.elicit() API"

risk_if_skipped: "Without Elicitation, tools requiring missing args at runtime must fail with `missing_arg` errors + caller retries with more info — multi-turn UX nightmare. Destructive tools requiring confirmation fall back to the simpler FR-MCP-006 `confirm` mode (returns 403 + caller posts confirm token); `elicit` mode is the richer pattern (server explains WHAT it wants to do + WHY) and is more usable. Without DEC-1142's JSON Schema validation, malformed responses crash tool handlers. Without DEC-1147's sync-tool ban, blocking on elicit in a sync handler exhausts request workers. Without DEC-1150's re-elicit retry cap, a buggy tool could elicit forever. Without DEC-1153's file-upload type, file-aware tools have no clean upload primitive. Without DEC-1151's FR-MCP-006 integration, the `elicit` mode in MCP-006 policy stays as a 503 placeholder forever. The 6h effort lands the interactive primitive that completes the long-running-tool UX story + de-stubs FR-MCP-006's elicit mode."
---
...
10. **MUST** support file_upload elicitation per DEC-1153 via FR-DOC-001 presigned S3 URLs. Handler at elicit creation for file_upload type:
...
11. **MUST** integrate with FR-MCP-006 gating per DEC-1151. When FR-MCP-006 policy says `mode=elicit` for a tool:
    ...
    - De-stubs FR-MCP-006's 503 placeholder (`elicitation_not_yet_supported`); FR-MCP-006 `services/mcp/src/gating/elicit.rs` modified here.
...
**Why polling required + NATS push optional (§1 #6, DEC-1145)?** Mirrors FR-MCP-007 pattern: polling universal, push for high-volume clients. CHAT consumes elicitations via NATS push for sub-second UX; CLI tools poll.
...
**Why FR-MCP-006 confirm-mode satisfied by confirmation elicitation (§1 #11, DEC-1151)?** Both patterns ask "user, please confirm X". Unifying them avoids the gating-layer reinventing prompt UX. The MCP-006 `elicit` mode (previously placeholder per FR-MCP-006 DEC-1055) de-stubs by routing to this FR's confirmation elicitation.
...
**Why audit at sev-3 for routine events (§1 #17)?** Elicitation traffic is high-volume in interactive scenarios. Sev-3 = informational, sampled at the OBS layer per FR-OBS-006. Sev-2 reserved for validation_failed (forensic signal for bad client behaviour or tool bugs).
...
```rust
// available within TaskCtx (FR-MCP-007 task execution context)
...
10. **FR-MCP-006 integration** — FR-MCP-006 policy `mode=elicit` on destructive tool routes through this FR's confirmation elicit; previous 503 placeholder de-stubbed.
...
**Upstream (depends_on):**
- **FR-MCP-001** spec compliance — elicitation defined per MCP 2025-11-25 spec.
- **FR-MCP-004** OAuth 2.1 PKCE — JWT bearer auth for poll/respond/cancel.
...
| Tool process crashes during NOTIFY wait | listener dies | Tool restarted; new instance picks up task; elicitation status still 'pending' or 'responded' | FR-MCP-007 task resume covers; new tool instance polls elicitation status |
...
**§11.7** Rate-limit counter: Redis sliding-window per task_id; FR-MCP-007 ctx exposes the limiter.

**§11.8** Prompt and response_schema stored as JSONB for flexibility; future indexed queries on prompt content (slice 4) trivial.

**§11.9** The 30-day pruning happens via daily `cyberos_pruner` job (same role as FR-TEN-003 + FR-MCP-007 patterns).
...
**§11.11** Confirmation elicitation has a default 5-min timeout (matching FR-MCP-006 confirm-token TTL for consistency); tools can override.
...
**§11.16** Cross-caller audit is sev-1 but uses a kind outside the 5-core list — operators see in security dashboards alongside FR-TEN-103 cross-residency events.
