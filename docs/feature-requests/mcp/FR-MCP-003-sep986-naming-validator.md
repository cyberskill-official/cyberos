...
2. **MUST** parse + validate at `validator.rs::validate(skill_id)` per DEC-2360:
   ...
   - Module ∈ FR-MCP-002 registered modules
   - Verb ∈ closed enum
   - Noun: snake_case identifier

3. **MUST** hook into FR-MCP-001 skill registration per DEC-2362 — reject with sev-2 audit if invalid.
