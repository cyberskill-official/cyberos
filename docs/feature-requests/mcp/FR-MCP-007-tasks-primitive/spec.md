...
source_decisions:
  ...
  - DEC-1120 2026-05-17 — Worker pool isolation: per-module pools prevent one slow module from starving another; FR-MCP-002 registration declares pool size
...
7. **MUST** extend FR-MCP-002 `server_registry` with 3 new annotation fields per DEC-1102 + DEC-1103 + DEC-1108 + DEC-1120:
...
8. **MUST** branch `tools/call` handler per DEC-1102. If invoked tool's `long_running=true`:
    ...
    - Check FR-MCP-006 gating (destructive_hint requires confirmation per DEC-1111); confirmation token consumed at task START only.
...
9. **MUST** expose `GET /v1/mcp/tasks/{task_id}` for status polling per DEC-1105. Handler:
    ...
    - On oversized result: returns `result_url: <s3_url>` (FR-DOC-001 reference); inline result is NULL.
...
17. **MUST** integrate with FR-MCP-006 gating per DEC-1111 + DEC-1122. Confirmation at task START (sync phase of create.rs); subsequent polls do NOT re-confirm; cancellation does NOT require confirmation. The confirmation token consumed in tasks/create.rs per FR-MCP-006 §1 #12 atomic-consume pattern.
...
**Why result size cap 10 MiB (§1 #18, DEC-1112)?** Inline result in JSON-RPC response means it traverses the gateway response path; > 10 MiB = memory pressure + slow client downloads. FR-DOC-001 S3 streaming is the right pattern for larger outputs; result_url indirection keeps the gateway lean.
...
**Why UUIDv7 task IDs (§1 #1 frontmatter, DEC-1104)?** Sortable (chronological list works natively); collision-free across distributed workers (no central counter needed); compatible with FR-TEN-103 residency-nibble encoding.
...
**Upstream (depends_on):**
- **FR-MCP-001** spec compliance — tools/call handler branches sync vs task here.

**Cross-module (related_frs):**
- **FR-MCP-002** Per-module registration — extended with long_running + ttl + max_concurrent fields.
- **FR-MCP-004** OAuth 2.1 PKCE — JWT bearer auth for poll/cancel.
- **FR-MCP-005** PRM — long_running tools advertised via per-module PRM (slice-3 add).
...
- **Deferred:** Task cost attribution (per-tenant compute budget) — slice 4, FR-TEN-004 derivative.
...
| Result oversized (> 10 MiB) | size check pre-INSERT | status='failed' + error.code='-32001' + `result_too_large` | Tool implementer adds FR-DOC-001 streaming |
...
**§11.7** UUIDv7 generation: same `services/ten/src/residency/uuid_gen.rs` from FR-TEN-103 (cross-residency collision-free encoding).
...
**§11.10** Per-tenant rate limit uses Redis sliding-window (matches FR-TEN-101 pattern); fallback to Postgres counter if Redis down (degraded throughput acceptable).
