...
source_decisions:
  - DEC-895 2026-05-17 — RFC 9728 Protected Resource Metadata is the canonical mechanism for an MCP server to advertise which authorization servers issue acceptable tokens; FR-MCP-004 audience-bound tokens require this discovery
  ...
  - DEC-897 2026-05-17 — Per-MCP-server-namespace PRM at `/.well-known/oauth-protected-resource/cyberos.{module}` for the module-namespaced MCP servers (FR-MCP-002 + FR-MCP-003); top-level `/.well-known/oauth-protected-resource` returns the gateway-aggregate PRM
  ...
  - DEC-901 2026-05-17 — `resource_signing_alg_values_supported` = `["RS256", "EdDSA"]` matching FR-AUTH-004's JWT signing algorithms; never include HS256 (symmetric — wrong for federated)
...
17. **MUST** invalidate the cached PRM body + ETag when the FR-MCP-002 registry changes (module register / heartbeat / unregister). Implementation: registry change publishes to NATS subject `cyberos.mcp.registry.changed`; PRM cache subscriber rebuilds + bumps ETag.
...
19. **MUST** emit the `WWW-Authenticate` resource-metadata header on 401 responses from FR-MCP-001 endpoints per MCP spec §2.1, pointing at this FR's PRM URL. Implementation note for FR-MCP-001 integration (this FR provides the endpoint; FR-MCP-001 references it).
...
16. **`mcp.prm_served` row emitted at 1% sample** — 1000 sampled requests produces ~10 audit rows (FR-OBS-006 tail-sampling).
17. **OAuth issuer URL list matches FR-AUTH-004 residency map** — assertion against AUTH-004 `RESIDENCY_ISSUERS` const.
18. **`resource` URI matches FR-MCP-004 audience-binding** — assertion against FR-MCP-004's audience-bound JWT `aud` claim.
19. **Content-Type application/json; charset=utf-8** — header present + correct.
20. **Registry-write fails if registration lacks `exposed_scopes`** — FR-MCP-002 ModuleRegistration validation rejects missing field.
...
pub async fn build_gateway_prm(ctx: &AppCtx) -> PrmDocument {
    PrmDocument {
        ...
        authorization_servers: ctx.auth.residency_issuers(),  // 4 URLs from FR-AUTH-004
...
**Upstream (depends_on):**
- **FR-MCP-004** OAuth 2.1 PKCE — audience-bound tokens require this discovery endpoint to be useful.

**Cross-module (related_frs):**
- **FR-MCP-001** spec compliance — `WWW-Authenticate: Bearer resource_metadata=<this-FR's-URL>` referenced from 401 responses.
- **FR-MCP-002** per-module registration — registry feeds per-module PRM `scopes_supported`.
- **FR-MCP-006** Tool-annotation gating — scopes advertised here reflect the tool-annotation set.
...
### 8.4 401 from FR-MCP-001 endpoint pointing at PRM
...
| Registry returns empty `authorization_servers` | empty list | 503 + sev-1 alert `mcp.prm_no_auth_servers` | Operator verifies FR-AUTH-004 residency config |
...
| Module registers with malformed scope name (invalid chars) | FR-MCP-002 validation | Registration rejected upstream; PRM never sees bad data | FR-MCP-002 owns validation |
...
| FR-AUTH-004 residency issuer list mid-rotation | aggregate PRM advertises stale issuer | Client gets 401 from old issuer; falls back to next | Inherent — multi-issuer list provides resilience |
