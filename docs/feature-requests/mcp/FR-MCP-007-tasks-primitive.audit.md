...
### ISS-003 — Per-tool annotation `long_running` interplay with FR-MCP-005 PRM

§7 cross-FR references FR-MCP-005 "long_running tools advertised via per-module PRM (slice-3 add)". But FR-MCP-005 shipped without this field. The new field is added to ModuleRegistration here; the PRM consumer reads it. Resolved: §7 explicit that FR-MCP-005's `scopes_supported` field is the bridge; future PRM enhancement (slice 4) adds `long_running_tools` array. Slice 3 of THIS FR is independent; PRM-side advertising is a deferred enhancement.
