...
source_decisions:
  ...
  - DEC-269 (tool name format `cyberos.<module>.<verb>_<noun>` per SEP-986 — validator enforced by FR-MCP-003; this FR ships the registration contract that requires it)
  ...
  - DEC-271 (the gateway is stateless per-request; session state (Tasks, sampling continuations) lives in FR-MCP-007's Tasks store; no in-process session)
...
new_files:
  ...
  - services/mcp-gateway/src/federation/registry.rs                    # in-memory federated tool catalog (modules register via FR-MCP-002)
...
disallowed_tools:
  ...
  - hold session state in process (per DEC-271 — Tasks store at FR-MCP-007)
...
5. **MUST** implement the `initialize` method returning capabilities (per DEC-266). The response advertises:
   ```json
   {
     ...
     "instructions": "Federation of 22 CyberOS modules. All calls audit-chained. OAuth 2.1 PKCE auth via FR-MCP-004."
   }
   ```
...
8. **MUST** implement closed JSON-RPC error code mapping (per DEC-272):
   ...
   - `-32005 elicitation_required` — destructive tool requires Elicitation flow (per FR-MCP-006); slice 4 stub returns this when tool annotation `destructiveHint: true` AND `Elicitation-Confirmed: 1` header absent.
...
11. **MUST** verify the OAuth 2.1 access token (FR-AUTH-004 JWT) on every request EXCEPT `initialize`. Verification:
    - JWT signature valid against the AUTH JWKS.
    - `aud` claim matches `https://mcp.cyberos.com` (audience-bound per FR-MCP-004).
...
**Why dispatch to internal module endpoints over HTTP not gRPC (§1 #16)?** HTTP is the spec-mandated transport for MCP; module servers ARE MCP servers (per FR-MCP-002). Using gRPC internally would force every module to implement both. HTTP is consistent and observable (standard logs, traces, metrics).
...
**Why state-less gateway (§1 #4, DEC-271)?** Session resumption uses session id, not in-process session state; Tasks state lives in FR-MCP-007's persistent store. State-less means horizontal scaling is trivial (any instance can serve any request); operational outages don't lose in-flight work.
...
pub fn handle_initialize(params: InitializeParams) -> Result<InitializeResult, JsonRpcError> {
    ...
    Ok(InitializeResult {
        ...
        instructions: "Federation of 22 CyberOS modules. All calls audit-chained. OAuth 2.1 PKCE auth via FR-MCP-004.".into(),
    })
}
```
...
pub async fn handle_tools_call(
    ...
    // Destructive-op gate (FR-MCP-006 ships full Elicitation; slice 4 stub)
...
**Upstream:**
- **FR-AUTH-004** — JWT issuance + JWKS; this FR verifies tokens against the JWKS.

**Downstream (5 placeholders):**
- **FR-MCP-002** — per-module server registration + heartbeat; populates the federated registry.
- **FR-MCP-003** — SEP-986 naming convention validator (`cyberos.<module>.<verb>_<noun>`).
- **FR-MCP-006** — tool-annotation gating (destructive requires confirm or Elicitation).
- **FR-MCP-007** — Tasks primitive (long-running work).
- **FR-MCP-008** — Elicitation server-initiated request/response.
...
```json
{
  ...
  "result": {
    "content": [
      {"type": "text", "text": "Found 3 memories matching 'FR-AUTH-101'..."}
    ],
    "isError": false,
...
Deferred:
- **OAuth 2.1 PKCE flow** — FR-MCP-004 ships the issuance; this FR consumes the JWT.
- **Protected Resource Metadata (RFC 9728)** — FR-MCP-005 publishes at `.well-known/oauth-protected-resource`.
- **Destructive-op Elicitation flow** — FR-MCP-006 ships the full flow; this FR stubs with `-32005 elicitation_required`.
- **Tasks primitive** — FR-MCP-007.
- **Elicitation primitive** — FR-MCP-008.
...
- **Federation registry in-memory** — < 1MB; refreshed on FR-MCP-002 registration events; eventual consistency across gateway instances.
...
- **Audience-bound JWT** — `aud: https://mcp.cyberos.com` per FR-MCP-004; gateway rejects tokens for other audiences.
...
- **`elicitation_required` -32005 is a stub at slice 4** — FR-MCP-006 ships the full flow with the `Elicitation-Confirmed` header check + back-and-forth.
...
- **Module endpoint discovered via FR-MCP-002 registration** — at slice 4 this FR uses a static map for tests; production lookup via FR-MCP-002.
