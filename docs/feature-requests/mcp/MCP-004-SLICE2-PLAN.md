# FR-MCP-004 slice 2 - OAuth 2.1 authorization server build plan
...
1. Token signing reuses FR-AUTH-004 keys, no new keypair. The auth service stores RS256 signing keys
...
The three OAuth migrations were renumbered off 0010-0012 (the backlog reserves those for FR-MCP-007
tasks and FR-MCP-008 elicitation) to 0013-0015. Do the rename as a git move so history follows:
...
FR-MEMORY-111 scrubbing (clause #26). This is the only async/DB-touching part of the otherwise-pure
...
`client_credentials` grant (deferred to FR-MCP-007). ES256 (auth is RS256-only). FR-MCP-005 Protected
