...
related_frs: [FR-OKR-001, FR-OKR-005, FR-CUO-101, FR-BRAIN-111]
depends_on: [FR-OKR-001, FR-CUO-101]
blocks: []
...
source_decisions:
  ...
  - DEC-2024 2026-05-17 — Draft requires CEO review before BRAIN persistence — never auto-persist (FR-CRM-007 pattern)
  - DEC-2025 2026-05-17 — BRAIN audit kinds: okr.retro_drafted, okr.retro_approved, okr.retro_dismissed, okr.retro_persisted
...
build_envelope:
  ...
  new_files:
    ...
    - services/okr/src/retro/brain_persister.rs
...
The OKR service **MUST** ship quarterly retro at `services/okr/src/retro/` auto-drafted Q-end, framing-aware, CEO-reviewed, BRAIN-persisted on approval, 4 BRAIN audit kinds.

1. **MUST** trigger on Q-end per DEC-2020 via FR-MCP-007 cron (Mar 31, Jun 30, Sep 30, Dec 31).
...
4. **MUST** draft at `draft_generator.rs::draft(tenant, quarter, framing)` per DEC-2023:
   ...
   - Pull check-ins from FR-OKR-005
   - FR-AI-003 prompt with framing-aware tone
...
6. **MUST** persist to BRAIN on approval at `brain_persister.rs::persist(retro)` per FR-CRM-007 pattern — searchable kind=lessons.
...
7. **MUST** define table at migration `0007`:
   ...
   GRANT UPDATE (framing, draft_jsonb, status, reviewed_by, reviewed_at, brain_memory_path) ON okr_quarterly_retros TO cyberos_app;
   ```

8. **MUST** expose endpoints:
   ```text
   GET  /v1/okr/retros                          (list)
   POST /v1/okr/retros/{id}/approve             (CEO; persists to BRAIN)
   POST /v1/okr/retros/{id}/dismiss
   POST /v1/okr/retros/{id}/regenerate          body: {framing}  (re-draft with different framing)
   ```

9. **MUST** emit 4 BRAIN audit kinds per DEC-2025. PII per FR-BRAIN-111: draft text SHA-256 hashed.

10. **MUST** thread trace_id from cron → draft → CEO review → BRAIN persist → audit.
...
**Why BRAIN persist (DEC-2025)?** Future retros reference past lessons; without persist, learning evaporates.
...
1. **Auto-trigger Q-end cron**. 2. **retro_framing enum cardinality 3**. 3. **VN tenant defaults to vn_face_saving**. 4. **Other tenant defaults to balanced**. 5. **4 sections drafted**. 6. **CEO can override framing**. 7. **Regenerate endpoint with new framing**. 8. **Never auto-persist to BRAIN**. 9. **Approve writes BRAIN memory**. 10. **4 BRAIN audit kinds emitted**. 11. **PII scrubbed (draft text SHA256)**. 12. **RLS denies cross-tenant**. 13. **CEO-only review**. 14. **Trace_id preserved**. 15. **UNIQUE(tenant_id, quarter) idempotency**. 16. **Append-only via REVOKE except status cols**. 17. **brain_memory_path populated post-approve**. 18. **Dismiss → status=dismissed (audit retained)**. 19. **AI failure → status=failed + sev-2 + retry**. 20. **Status workflow: pending_review → approved → persisted | dismissed**.
...
async fn never_auto_persists() {
    ...
    let mem_count = ctx.brain_memory_count_for_retro(ctx.retro_id).await;
...
async fn approve_writes_brain() {
    ...
    assert!(retro.brain_memory_path.is_some());
}
...
## §7 — Dependencies
**Upstream:** FR-OKR-001, FR-CUO-101.
**Cross-module:** FR-OKR-005 (check-in history), FR-AI-003 (draft + framing), FR-BRAIN-111 (PII + persist), FR-AUTH-101 (CEO role), FR-MCP-007 (cron).
...
| BRAIN write fail | sev-1 | retry | inherent |
...
- §11.1 Cron via FR-MCP-007 `kind: 'okr.quarterly_retro'`, runs Q-end + 1 day.
- §11.2 AI prompt varies by framing — VN face-saving emphasizes "we learned"; western direct emphasizes "what went wrong".
- §11.3 BRAIN memory path: `memories/lessons/okr/<tenant>/<quarter>.md`.
- §11.4 BRAIN audit body: retro_id, quarter, framing; draft text SHA256.
- §11.5 Future retros via FR-BRAIN-108 search return past lessons for context.

---

*End of FR-OKR-007 spec.*
