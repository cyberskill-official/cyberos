...
related_frs: [FR-OKR-001, FR-OKR-005, FR-CUO-101, FR-MEMORY-111]
...
source_decisions:
  ...
  - DEC-2024 2026-05-17 — Draft requires CEO review before memory persistence — never auto-persist (FR-CRM-007 pattern)
...
6. **MUST** persist to memory on approval at `memory_persister.rs::persist(retro)` per FR-CRM-007 pattern — searchable kind=lessons.
...
9. **MUST** emit 4 memory audit kinds per DEC-2025. PII per FR-MEMORY-111: draft text SHA-256 hashed.
...
**Cross-module:** FR-OKR-005 (check-in history), FR-AI-003 (draft + framing), FR-MEMORY-111 (PII + persist), FR-AUTH-101 (CEO role), FR-MCP-007 (cron).
...
- §11.5 Future retros via FR-MEMORY-108 search return past lessons for context.
