...
source_decisions:
  ...
  - DEC-363 (KR type closed enum at 3 values per Doerr/Grove: hit_target · improvement · milestone — full type validation ships in FR-OKR-002)
...
new_files:
  ...
  - services/okr/migrations/0005_progress_log.sql                # append-only KR progress recordings (consumed by FR-OKR-005 check-ins)
...
disallowed_tools:
  ...
  - ship KR progress_source DSL here (FR-OKR-003 ships)
  - ship weekly check-in handler here (FR-OKR-005 ships)
  - ship Monday digest here (FR-OKR-006 ships)
...
risk_if_skipped: "OKR is the quarterly strategy operating loop; without the schema, the cascade is operator-mental rather than data-canonical. Every downstream OKR FR (FR-OKR-002 KR types, FR-OKR-003 progress source DSL, FR-OKR-004 auto-progress batch, FR-OKR-005 weekly check-ins, FR-OKR-006 Monday digest, FR-OKR-007 retros) reads from these tables. Without DEC-365's strict alignment tree, member OKRs drift away from company strategy. Without DEC-364's face-saving status enum, the Vietnamese cultural adaptation is lost — operators see 'failed' and the retro becomes blame-finding. Without DEC-367's append-only progress log, KR progress can be retroactively edited — forecast integrity breaks. Without DEC-369's EU AI Act Art. 14 acknowledgement, OKR-driven employment decisions skip the human-in-loop requirement. The 6h effort lands the foundational primitives + the cultural adaptations baked into the schema."
---
...
**Why teams as a tenant-local primitive (DEC-372, §1 #4)?** HR (FR-HR-001) ships Member records but doesn't ship a Team primitive (deferred to FR-HR-2xx). OKR needs teams now for the Team-scope objectives. Shipping the teams table here (in OKR's schema) is the pragmatic answer; future HR Team primitive can either supersede or join.

**Why append-only kr_progress_log (DEC-367, §1 #13)?** Quarterly retros depend on "what was the KR's progression over time?" — answerable only with a chained history. UPDATE in place loses prior recordings; the log preserves them. The `source` field (manual | auto | check_in) lets FR-OKR-003's auto-progress batch distinguish its own writes from operator manual recordings.
...
**Why milestone KR uses boolean target_value_numeric (DEC-370)?** Milestones are binary (delivered or not). Storing as `target=1, current=0|1` uses the same BIGINT column for all KR types — simplifies the schema. FR-OKR-002 ships the per-type validation that enforces this convention.
...
- **FR-AUTH-101** — RBAC (`Resource::OkrObjective`, `OkrKr`).

**Downstream (3 placeholders):**
- **FR-OKR-002** — full KR type validation (per-type rules for hit_target / improvement / milestone).
- **FR-OKR-003** — progress_source DSL.
...
Deferred:
- **Full KR per-type validation** — FR-OKR-002 (hit_target ≥ baseline; improvement target ≠ start; milestone target_value = 1).
- **Progress source DSL** — FR-OKR-003 (queries against PROJ/INV/HR/LEARN).
- **Auto-progress nightly batch** — FR-OKR-004.
- **Weekly check-in handler** — FR-OKR-005.
- **Monday digest** — FR-OKR-006.
- **Quarterly retro draft** — FR-OKR-007.
...
- **Milestone as boolean (target=1)** — same column for all KR types; per-type rules in FR-OKR-002.
- **Cross-cycle alignment forbidden at trigger** — semantic correctness.
- **`source` enum {manual, auto, check_in}** — distinguishes auto-progress batch writes (FR-OKR-004) from operator + check-in writes.
