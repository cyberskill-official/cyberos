...
4. **MUST** store in FR-DOC-001 per DEC-2202 with `sha256` metadata.
...
5. **MUST** define table at migration `0006`:
   ...
   CREATE TABLE rew_payslip_pdfs (
     ...
     doc_id UUID NOT NULL,  -- FR-DOC-001 ref
...
| FR-DOC-001 store fail | catch | sev-2 | retry |
