...
2. **MUST** generate file at `file_generator.rs::generate(payroll_run)` per DEC-2230:
   - For each member: account_number + amount + memo (template per FR-CRM-009 convention)
...
4. **MUST** reconcile per DEC-2233 — match FR-INV-005 inbound webhook acks to batch entries via memo.
...
**Why FR-INV-005 reconciliation (DEC-2233)?** Inbound VietQR webhook tracks individual transfers; matching by memo enables auto-status.
...
| Memo format mismatch (FR-CRM-009 drift) | reconciliation fail | sev-2 | data fix |
