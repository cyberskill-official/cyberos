...
source_decisions:
  - DEC-2220 2026-05-17 — Quarterly distribution: CEO sets fund VND amount, FR-LEARN-007 emits VP shares, system computes per-member payout, BP ledger debited, P3 added to next payroll
...
2. **MUST** calculate at `calculator.rs::calc(tenant, quarter, fund_vnd)` per DEC-2220:
   - Receive VP shares from FR-LEARN-007 handoff (sums to 1.0)
...
5. **MUST** on execute:
   - Debit BP balance (FR-REW-007 credit_p3_distribution converted to BP-equiv)
   - Add to next payroll (FR-REW-005) as P3 income kind
...
**Why VP share input (DEC-2220)?** Fairness — distribution proportional to contribution per FR-LEARN-007.
...
|---|---|---|---|
| FR-LEARN-007 not emitted | catch | sev-2; await | manual trigger |
