---
id: FR-REW-010
title: "REW BRAIN structural exclusion CI gate — no comp fields appear in BRAIN-ingest paths; static analysis + runtime check"
module: REW
priority: MUST
status: draft
verify: T
...
related_frs: [FR-REW-001, FR-BRAIN-111, FR-BRAIN-101]
depends_on: [FR-REW-001]
blocks: []

source_pages:
  - website/docs/modules/rew.html#brain-exclusion

source_decisions:
  - DEC-2240 2026-05-17 — Structural invariant: comp data (amounts, gross/net VND, deductions) NEVER reaches BRAIN ingest paths — static analysis at CI + runtime check
  ...
  - DEC-2242 2026-05-17 — Static analysis: grep for forbidden patterns (decrypted_amount, gross_vnd, net_vnd, etc.) in BRAIN-ingest code paths; CI blocks PR on match
  - DEC-2243 2026-05-17 — Runtime: BRAIN ingest path validates payload against schema blocklist; reject + sev-1 audit if comp fields detected
  - DEC-2244 2026-05-17 — BRAIN audit kinds: rew.exclusion_ci_passed, rew.exclusion_ci_failed, rew.exclusion_runtime_violation, rew.exclusion_field_blocked
...
build_envelope:
  ...
  new_files:
    ...
    - .github/workflows/rew-brain-exclusion.yml
    - scripts/check_rew_brain_exclusion.sh
...
The REW service **MUST** ship BRAIN exclusion gate at `services/rew/src/exclusion/` with static CI grep + runtime payload check + sev-1 audit on violation, 4 BRAIN audit kinds.

1. **MUST** validate `exclusion_check_kind` against closed enum per DEC-2241.

2. **MUST** run static grep CI at `.github/workflows/rew-brain-exclusion.yml` per DEC-2242:
   - Pattern list: `decrypted_amount`, `gross_vnd`, `net_vnd`, `deductions_total`, `payslip_pdf_bytes`, etc.
   - Search in: any file under `services/rew/src/audit/`, `services/*/src/brain/`, BRAIN ingest paths
   - Match → CI fails
...
5. **MUST** emit 4 BRAIN audit kinds per DEC-2244. Audit body itself MUST NOT carry comp data.
...
```json
{
  "error": "brain_exclusion_violation",
  "field": "gross_vnd",
  "message": "Comp field detected in BRAIN ingest payload; rejected per FR-REW-010."
}
```
...
SCOPES="services/rew/src/audit services/*/src/brain"
matches=$(grep -rE "$FORBIDDEN_PATTERNS" $SCOPES || true)
if [ -n "$matches" ]; then
  echo "BRAIN exclusion violation:"
  echo "$matches"
  exit 1
fi
```
...
async fn runtime_blocks_gross_vnd() {
    ...
    assert!(r.is_err());
    let audits = ctx.fetch_brain_audits("rew.exclusion_runtime_violation").await;
...
async fn ci_script_catches_pattern() {
    ...
    let r = run_shell("scripts/check_rew_brain_exclusion.sh", test_dir);
    assert!(!r.success());
}
...
## §7 — Dependencies
**Upstream:** FR-REW-001.
**Cross-module:** FR-BRAIN-111 (PII enforcement), FR-BRAIN-101 (BRAIN ingest paths to gate).
...
- §11.4 BRAIN audit body: kind, payload_id (no values); structurally safe.
- §11.5 CI integration: required check, cannot bypass.

---

*End of FR-REW-010 spec.*
