...
2. The two paths **MUST** be treated as architecturally distinct, not parallel instances of the same pattern used in FR-APP-003/FR-APP-004, because their submission mechanics genuinely differ: **Snap Store** accepts a CI-built, CI-uploaded `.snap` artifact via `snapcraft upload` (a CI-automatable, credential-gated upload, matching the FR-APP-003/004 pattern); **Flathub** does **NOT** accept a pre-built artifact upload at all — Flathub's own build infrastructure clones a submitted manifest repository and builds the Flatpak itself, so CyberOS's role is limited to maintaining a correct, buildable manifest and (with Stephen's explicit per-instance approval) opening a pull request against the external `flathub/flathub` repository.
...
5. **CI job is inert by default** — with `SNAP_RELEASE` unset or `false`, `release-snap.yml`'s `build-and-upload-snap` job is skipped (verified the same way as FR-APP-003/004 — an unconditional anchor job plus a `gh run view` conclusion check).
...
```yaml
# AC #5 — CI job inert-by-default, mirroring FR-APP-003/004's pattern
...
- **`related_frs` references FR-APP-006, which does not yet exist on disk** — deliberate same-batch forward reference (this FR and FR-APP-006 were approved together in the same PLAN and are being authored sequentially in one session; FR-APP-003 and FR-APP-004 are already landed). `depends_on`/`blocks` are both empty, so no inline placeholder annotation is mechanically required; documented here for the same disclosure reasons FR-APP-003 §9 and FR-APP-004 §9 recorded their own forward references.
