...
source_decisions:
  - DEC-2130 2026-05-17 — Promotion triggered by FR-LEARN-004 council recommendation=promote; CEO+CHRO dual-sign converts to action
...
2. **MUST** initiate per DEC-2130 when FR-LEARN-004 council completes with recommendation=promote.
...
4. **MUST** cascade on executed at `cascade.rs::execute(promotion)` per DEC-2132:
   - FR-HR-001 update member.mastery_level (transactional)
   - FR-REW-001 update comp_band (transactional)
   - FR-CHAT-005 announce in #all (non-blocking)
