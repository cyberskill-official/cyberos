...
2. **MUST** detect trigger at `gap_detector.rs::detect(tenant)` per DEC-2070:
   - SELECT members from FR-RES-001 matrix where `allocation_flag='over_allocated'` for ≥4 consecutive weeks
   - SELECT FR-CRM-001 deals advancing stage in same period
...
3. **MUST** project cost at `cost_projector.rs::project(role, region, contract_type)` per DEC-2072:
   - Base salary band per role (FR-RES-007 future or hardcoded table for v1)
   - FR-REW-004 statutory deductions per region (BHXH + BHYT + BHTN + PIT)
...
4. **MUST** generate memo at `memo_generator.rs::generate(tenant, gap, costs)`:
   ...
   - AI recommendation (FR-AI-003) with reasoning
...
- §11.1 Detector cron via FR-MCP-007 `kind: 'res.hiring_gap_detection'`, weekly.
