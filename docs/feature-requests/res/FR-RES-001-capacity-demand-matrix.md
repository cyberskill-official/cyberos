...
2. **MUST** compute at `computer.rs::compute(member, iso_week)` per DEC-2031:
   - capacity_hours = member.hours_per_week (FR-HR-002) - approved PTO (FR-HR-004) - LEARN training (FR-LEARN-001)
   - demand_hours = sum FR-PROJ-013 estimates for member's assigned issues in week
...
5. **MUST** schedule cron 04:00 tenant_tz via FR-MCP-007.
