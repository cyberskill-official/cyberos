...
related_frs: [FR-EVAL-001, FR-EVAL-003, FR-EVAL-004, FR-MEMORY-123, FR-AUTH-003, FR-PROJ-008]
depends_on: [FR-EVAL-001]
blocks: [FR-EVAL-003]
...
source_decisions:
  ...
  - DEC-2601 (governance first — FR-EVAL-002 is a hard dependent of FR-EVAL-001; no rubric row may be authored before the FR-EVAL-001 consent/access/retention layer exists, and rubric reads are access-gated by it)
  - DEC-2602 (GENIE/Lumi MAY draft rubric items from the documents but a human MUST approve before an item is effective — the draft→approved gate is HITL; an unapproved item never reaches FR-EVAL-003)
...
disallowed_tools:
  ...
  - author any rubric row before the FR-EVAL-001 governance layer is present (per DEC-2601 — governance first)
...
sub_tasks:
  ...
  - "1.0h: GET/POST endpoints (author + read), access-gated by FR-EVAL-001 grants, tenant-scoped by RLS"
...
4. **MUST** carry, per item, a machine-usable check descriptor that FR-EVAL-003 consumes:
...
7. **MUST** resolve, for any date, the single version that was effective then: `resolve_effective(rubric_id, at_date) → rubric_version` returns the published version whose `[effective_from, effective_to)` contains `at_date`. FR-EVAL-003 calls this so an assessment for a period uses the standard that was actually in force then, not whatever is current.
...
8. **MUST** enforce the human-approval (HITL) gate before any item or version becomes effective (DEC-2602):
    ...
    - Only an item that belongs to a `published` version is visible to FR-EVAL-003; a `draft`/`approved`-but-unpublished item **MUST NOT** be evaluable.
...
9. **MUST** support a GENIE DRAFT path that proposes items from the three documents without ever setting the standard (DEC-2602):
    - `draft_genie.rs` calls Lumi (the FR-EVAL-001-governed analysis path) to read the three documents and propose `rubric_item` rows — each pre-filled with a candidate `source_doc`, `clause_ref`, `item_kind`, `check_type`, and bilingual title/description — into `state='draft'`.
...
10. **MUST** be access-gated by the FR-EVAL-001 governance layer, not by an access rule this FR invents (DEC-2601). Authoring (create/edit/approve/publish) **MUST** require the FR-EVAL-001 grant for rubric administration (founder + designated rubric admins); reading the rubric **MUST** require a valid grant. The rubric tables **MUST NOT** ship before FR-EVAL-001 exists.

11. **MUST** emit one hash-chained memory audit row per rubric mutation through `services/eval/src/audit.rs` (DEC-2604): `eval.rubric_drafted`, `eval.rubric_edited`, `eval.rubric_approved`, `eval.rubric_published`, `eval.rubric_superseded`. Each payload carries `{rubric_id, rubric_version_id, version_no, item_id?, actor_subject_id, authored_by?, source_doc?, clause_ref?, trace_id}`. These chain into the same `l1_audit_log` the rest of CyberOS uses (FR-PROJ-008 / FR-MEMORY-123 pattern), so the rubric's full curation history is tamper-evident.

12. **MUST** expose a minimal authoring + read API, tenant-scoped and FR-EVAL-001-gated:
...
**Why separate the standard from the judgement (§1 #14)?** A rubric describes what "good" and "compliant" mean for everyone; an assessment is one person measured against it. Keeping per-person scoring out of this FR (it lives in FR-EVAL-003, referencing `rubric_version_id`) is what makes the rubric fair — the same version measures everyone, and changing the standard for one person is structurally impossible.

**Why governance-first dependency on FR-EVAL-001 (DEC-2601, §1 #10)?** The rubric is the spine of a system that affects pay and progression. Building it before the consent, access, and retention layer exists would mean authoring the most sensitive artifact in the platform with no access control and no disclosed basis. FR-EVAL-001 is a hard `depends_on`, and rubric reads/writes are gated by its grants, so the rubric never exists outside the governance frame.

**Why audit every mutation (DEC-2604, §1 #11)?** The rubric's curation history is itself evidence — who proposed an item, who approved it, when it was published, when it was superseded. Hash-chaining each mutation into `l1_audit_log` (the FR-PROJ-008 / FR-MEMORY-123 pattern) means a reviewer can later reconstruct exactly how the standard came to be and prove it was not quietly altered.
...
CREATE TABLE rubric_item (
    ...
    -- check descriptor consumed by FR-EVAL-003
...
/// The published version effective on `at` (§1 #7). FR-EVAL-003 anchors an
...
pub async fn draft_from_documents(
    tx: &mut sqlx::PgTransaction<'_>,
    version_id: uuid::Uuid,
    docs: &SignedDocSet,                    // the 3 governed documents (FR-EVAL-001 access)
) -> Result<Vec<RubricItem>, RubricError> {
    let proposed = lumi::propose_rubric_items(docs).await?;   // FR-EVAL-001-governed analysis call
    let mut out = Vec::new();
    for p in proposed {
    ...
    Ok(out)   // all in 'draft'; a human approves + publishes before FR-EVAL-003 ever sees them
}
```
...
11. **Draft/unpublished item not evaluable** — FR-EVAL-003's effective-version read returns only items of a `published` version; a `draft` item is absent (AC for §1 #8).
...
14. **Access-gated by FR-EVAL-001** — a caller without the rubric-admin grant gets `403` on authoring; without any grant gets `403` on read (AC for §1 #10).
...
17. **No per-person data stored** — schema inspection confirms no employee/score/evidence column on any rubric table; per-person scoring is FR-EVAL-003's (AC for §1 #14).
...
(Model, versioning, GENIE draft path, and migration above. `authoring.rs` wires the create/edit/approve flow over `model::validate_item`; `lib.rs` mounts the routes behind the FR-EVAL-001 access guard.)

---

## §7 — Dependencies

- **FR-EVAL-001** — governance, consent, access grants, retention. Hard dependency: the rubric tables ship only after it exists, and rubric authoring/reads are gated by its grants (DEC-2601).
- **FR-EVAL-003** — the evaluation engine that consumes the published rubric: it calls `resolve_effective(at)` and reads `rubric_item` rows to score evidence. This FR `blocks` it.
- **FR-MEMORY-123** — brain recall; the same audit-chain + memory substrate the rubric mutations chain into.
- **FR-AUTH-003** — RLS / roles; the tenant isolation and the `cyberos_app` vs `cyberos_ops` role split the immutability REVOKE relies on.
- **FR-PROJ-008 / FR-MEMORY-123** — the established hash-chained audit-row pattern the `eval.rubric_*` rows follow.
...
- A diff view between two rubric versions (which clause changed, which item was added) for the FR-EVAL-004 console — surfacing only, a later additive screen.
...
| Rubric authored before FR-EVAL-001 exists | dependency gate (DEC-2601) | FR not shippable | Ship FR-EVAL-001 first |
| Read by a caller without a grant | FR-EVAL-001 access guard | 403 | Grant access per governance |
| `resolve_effective` finds no in-force version | NoEffectiveVersion | error to FR-EVAL-003 | Publish a version covering the period |
...
- The three tables separate the framework (`rubric`), the immutable published cut (`rubric_version`), and the checkable items (`rubric_item`) so that re-curation is "new version", not "mutate rows" — the same append-only discipline as the FR-PROJ-008 history layer.
- `resolve_effective(at)` is the single seam FR-EVAL-003 uses; everything else about versioning is internal. Anchoring an assessment to the version in force on its period date is what keeps a past assessment interpretable after a contract amendment.
...
- The `cyberos_app` REVOKE makes published rows immutable to the runtime; `cyberos_ops` retains the ability to correct a record (rare, audited via `eval.rubric_edited`). This mirrors the FR-PROJ-008 role split.
- `check_type` is intentionally a small closed enum so FR-EVAL-003's evaluation logic is bounded: five evaluation shapes cover obligations (evidence presence / attestation), working terms (periodic review), KPIs (numeric threshold), and career milestones (milestone reached). A new shape is a deliberate schema change, not a free-form field.
- `weight` is relative within a version; the roll-up math (how item weights compose into a section and an overall draft score) belongs to FR-EVAL-003, not here. This FR stores the weight; it does not compute a score.
- No per-person column exists anywhere in this schema by design (clause 14). If a future need tempts adding one, that is the signal to put it in FR-EVAL-003 against `rubric_version_id`, keeping the standard and the judgement separate.
- The audit rows reuse the `services/eval/src/audit.rs` emitter (the FR-MEMORY-123 chain), so the rubric's curation history lives in the same tamper-evident ledger as every other CyberOS event — a reviewer reconstructs how the standard was set from one place.
