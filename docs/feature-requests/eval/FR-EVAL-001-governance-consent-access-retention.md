...
related_frs: [FR-AUTH-003, FR-MEMORY-101, FR-MEMORY-121, FR-EVAL-002, FR-EVAL-003, FR-CUO-204]
depends_on: [FR-AUTH-003]
blocks: [FR-MEMORY-121, FR-EVAL-002]
...
modified_files:
  ...
  - docs/feature-requests/eval/README.md       # new EVAL module index (created with this FR)
allowed_tools:
...
3. **MUST** expose `is_gated(subject_id) -> bool`: a subject is **gated** (capture and evaluation BLOCKED for that subject) whenever there is no `eval_ack` row matching the tenant's *current* notice version. This is the precondition that makes wide day-1 capture lawful: the moment a person logs into the OS, capture is armed, but it does not record that person until their acknowledgment of the current notice is on file. FR-MEMORY-121/122 capture emitters and FR-EVAL-003 evaluation **MUST** consult `is_gated` and skip a gated subject (emitting a `eval.capture_gated` audit row instead, so the skip is itself recorded).
...
14. **MUST** RLS-enforce every table on `tenant_id = current_setting('app.current_tenant_id')::uuid` (per FR-AUTH-003) so one tenant's notices, acknowledgments, grants, and DSR requests are invisible to another.
...
-- Reuses AUTH's per-tenant RLS GUC (app.current_tenant_id, FR-AUTH-003) and the
...
-- 14. Tenant RLS on every table (FR-AUTH-003 GUC).
...
/// The hot-path predicate FR-MEMORY-121/122 + FR-EVAL-003 call before capturing/evaluating.
...
pub async fn may_read(
    ...
    reader: Uuid,
    target: Uuid,
    reader_is_founder: bool,  // from the JWT role claim (FR-AUTH-101 cyberos_founder)
...
4. **Un-acked subject is gated** — `is_gated(subject)` returns true when the subject has no ack for the current version; FR-MEMORY-121 capture skips them and emits `eval.capture_gated` (AC for §1 #3).
...
- **FR-AUTH-003** — per-tenant RLS via `app.current_tenant_id` GUC + `cyberos_app` runtime role; every EVAL table inherits this tenancy + append-only REVOKE pattern. (depends_on)
- **FR-AUTH-101** — the `cyberos_founder` role claim that `may_read` reads for the founder-can-read-any path.
- **FR-MEMORY-101** — the L1 audit-chain writer protocol; EVAL reuses `services/shared/cyberos-audit-chain::chain_anchor` / `emit_genesis` so its rows verify under memory's reconcile.
- **`l2_memory` / `l2_entity`** (memory `0001_layer2.sql`) — the Layer-2 projections the retention sweeper erases (Layer 1 stays immutable).
- **Consumed by FR-MEMORY-121** (interaction-event schema) and **FR-MEMORY-122** (capture emitters): both call `is_gated` before recording a subject. (blocks)
- **Consumed by FR-EVAL-002** (rubric) and **FR-EVAL-003** (evaluation engine): evaluation runs only for non-gated subjects, reads are `guard_evaluation_read`-gated, and HITL for consequential outcomes is inherited from clause 11. (blocks FR-EVAL-002)
- **FR-CUO-204** — the GENIE/dream loop's denylist already protects PII-recall and auth invariants; EVAL governance tables join that protected set (no autonomous modification of a consent gate).
...
- Whether `manager_of` is sourced from an HR org-chart (FR-HR-001) or set explicitly via grants — Phase 0 supports explicit grants; the HR-derived auto-grant is a Phase-5 wiring (FR-EVAL-004) once the org chart exists.
...
| `app.current_tenant_id` unset on connection | `current_setting(..., true)` is NULL → RLS yields 0 rows | Fail-closed (no data) | Middleware must set the GUC (FR-AUTH-003) |
| Sweeper job fails mid-run | per-category loop; logged | Partial sweep; resumes next cron | Operator inspects logs |
| OTel exporter down | counters buffered then dropped | Metrics gap only | Restore FR-OBS-001 |
| L2 row missing `eval_category` frontmatter | sweep predicate misses it | Row not swept (retained) | Backfill category tag at ingest (FR-MEMORY-122) |
| Founder role claim spoofed | JWT signature verify (FR-AUTH-004) | Forged token rejected upstream | None — auth boundary |
...
- `eval_category` tagging on L2 rows is written by the FR-MEMORY-122 capture emitters (the category each event belongs to). The sweeper reads `frontmatter->>'eval_category'`; if a future schema promotes it to a typed column, the predicate moves with it.
...
- The founder path is role-based (`cyberos_founder` from the JWT, FR-AUTH-101), not a grant row, so the founder never needs a self-issued grant to every subject. Designated managers and HR reviewers are grant rows so they are explicit, revocable, and auditable.
...
- The whole layer is deliberately boring and conservative: it adds gates, registries, audit rows, and retention limits. It does not itself analyse anyone. Analysis is FR-EVAL-003, and it runs only inside the fence this FR builds.
