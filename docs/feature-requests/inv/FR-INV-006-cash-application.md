...
new_files:
  ...
  - services/inv/src/cash_app/step1_exact_ref.rs               # HD/INV prefix match (extends FR-INV-005 memo parser)
...
sub_tasks:
  ...
  - "0.4h: step1_exact_ref.rs — reuse FR-INV-005 memo parser"
...
10. **MUST** implement **step 4 — manual operator match** (per DEC-568). NOT auto-invoked. `POST /v1/inv/cash-app/allocate-manual` handler:
    - Caller MUST have role `cfo` per FR-AUTH-101.
...
13. **MUST** ship `invoice_outstanding_view` SQL view (per DEC-572):
    ...
    SELECT i.id AS invoice_id, i.tenant_id, i.amount_minor, i.currency,
     ...
     GROUP BY i.id;
    ```
    Downstream consumers (FR-INV-009 AR aging, FR-INV-010 dunning) use this view, NOT raw `invoices.status`.
...
23. **MUST** use the `inv_cash_applier` SQL role (defined in FR-INV-005) to INSERT into `payment_allocations` + UPDATE `payment_receipts.invoice_id` (per DEC-574). cyberos_app role gets SELECT only.
...
**Upstream:**
- **FR-INV-003** — Stripe webhook (provides receipts).
- **FR-INV-005** — VietQR webhook (provides receipts + memo parser).
...
Deferred:
- **Multi-currency cross-allocation (e.g. USD receipt against VND invoice via FX)** — FR-INV-002 ships SBV FX snapshot; cross-currency cash app is FR-INV-2xx.
- **ML-suggested matches** — slice 3 augmentation of step 4 manual.
- **Bulk import historical allocations** — FR-INV-2xx migration tool.
- **Customer-side allocation hint via API** — FR-PORTAL-008.
...
- **Reversal as new row** — same correction_to pattern as FR-TIME-001.
...
- **`inv_cash_applier` SQL role** — defense in depth (reuses FR-INV-005 role split).
