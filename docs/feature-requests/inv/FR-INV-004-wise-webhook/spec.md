...
build_envelope:
  ...
  disallowed_tools:
    ...
    - auto-convert currency at receipt time (FX is FR-INV-002 SBV authoritative)
...
10. **MUST** match incoming transfers to invoices by exact-currency comparison (DEC-846 + DEC-852). The processor delegates to FR-INV-006 cash-application cascade ONLY when the transfer currency matches an outstanding invoice in the same currency. Currency mismatch → state `currency_mismatch`, row written to `unmatched_receipts` table, CFO surfaces in dashboard. Never auto-convert.
...
21. **MUST** validate the per-tenant `wise_profile_id` at tenant provisioning (FR-TEN-001 integration). The profile_id is a 12-digit integer per Wise; format-validated at policy save.
...
14. Currency match → delegates to FR-INV-006 cash-app → sev-3 `wise_matched` audit on success.
...
- `currency_match_test` — delegates to FR-INV-006 → matched + sev-3.
...
**Depends on**: FR-AUTH-101 (RBAC for CFO role + tenant_admin gates).

**Blocks**: FR-INV-006 (cash-application — wise receipts feed the cascade as one source).
...
| 11 | FR-INV-006 cash-app match fails | downstream error | 2 | retry_count++; eventual dead-letter |
...
**§11.3** The Wise API endpoint to fetch the public key requires authentication with a tenant-scoped Wise API token, stored encrypted in `tenants.wise_api_token_kms_blob` (analogous to FR-AUTH-103 SP signing key). KMS decryption happens once at cache populate.
...
**§11.19** The processor delegates the cash-application match to FR-INV-006 via a function call (not HTTP) — same Rust binary; no network hop.
