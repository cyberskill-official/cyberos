...
sub_tasks:
  ...
  - "0.8h: AR aging dashboard UI hook (FR-CUO-101)"
...
4. **MUST** support multi-currency per DEC-1544: when `base_currency` differs from invoice currency, convert via FR-INV-002 `fx_snapshot(currency_pair, as_of_date)`. Missing FX → fall back to nearest-prior with sev-2 audit.
...
| FX rate missing for date | snapshot lookup | nearest-prior + sev-2 audit | per FR-INV-002 |
