...
First-pass let CFO manually allocate without trace. Resolved: §1 #16 + DEC-628 + cfo role gate + reason required + sev-2 audit + `matched_manual` BRAIN row; AC #12 + #13.
...
Per AUTHORING.md §0 master rule: spec is now perfect — depth bounded by the genuine architectural surface (4-step closed cascade × append-only at SQL grant × atomic over-allocation defense × partial M:N allocation × reversal reciprocity × per-tenant fuzzy threshold with cap × CFO-only manual + sev-2 audit × 9 BRAIN audit kinds × currency-mismatch hard skip × 5-min job advisory-lock × auto-mark-paid trigger × invoice_outstanding_view × dry-run handler × idempotent re-processing), not by line targets.
