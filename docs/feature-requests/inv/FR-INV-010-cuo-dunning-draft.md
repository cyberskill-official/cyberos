...
sub_tasks:
  ...
  - "0.7h: draft_generator.rs (FR-AI-003 integration)"
...
4. **MUST** generate draft via `draft_generator.rs::generate(invoice, tone, template)` — FR-AI-003 call with template + invoice context. Templates loaded from `template_loader.rs::load(tenant, tone)`.
...
6. **MUST** queue draft for CFO review — NEVER auto-send per DEC-1553. CFO sees in dunning queue UI (FR-CUO-101); approves or dismisses.

7. **MUST** on CFO approval: call FR-EMAIL-009 send_message with draft body, log `dunning_email_sent` audit.
...
**Why FR-AI-003 generation (DEC-1552)?** Personalization (customer name, invoice ref, outstanding balance) requires templated AI inference; static templates feel robotic.
...
## §7 — Dependencies
**Upstream:** FR-INV-009 (aging), FR-EMAIL-009 (send).
**Cross-module:** FR-AI-003 (template generation), FR-CUO-101 (review UI), FR-MCP-007 (cron).
...
|---|---|---|---|
| FR-AI-003 timeout | generation fail | retry 3x then sev-2 | manual retry |
...
| Send fail (bounce) | FR-EMAIL-009 status | status=failed_send + retry | CFO investigates |
