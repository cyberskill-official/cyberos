...
risk_if_skipped: "Without AATL CA, US/global contracts can't get Adobe-trusted signatures → recipients see warnings in Acrobat. Without DEC-1783 CISO gate, AATL creds compromise = mass forgery. Without DEC-1784 PAdES-B-T composability, no LTV path via FR-DOC-011."
---
...
6. **MUST** return PAdES-B-T signature per DEC-1784 — includes timestamp; composes with FR-DOC-011 for LTV.
...
9. **MUST** expose endpoints (mirror DOC-002 shape):
   ...
   POST   /v1/doc/aatl/sign                   (internal — FR-DOC-005 caller)
...
**Why PAdES-B-T not B-LT (DEC-1784)?** AATL CAs typically issue B-T (timestamp only); FR-DOC-011 re-stamps to B-LT at year-9.

---

## §3 — API contract (mirrors DOC-002)

See §1.9 for endpoints. Same payload shape as FR-DOC-002.
