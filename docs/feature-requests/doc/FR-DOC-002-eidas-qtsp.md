...
source_pages:
  ...
  - https://eur-lex.europa.eu/eli/reg/2014/910/oj  # eIDAS Regulation 910/2014
...
7. **MUST** store QTSP creds in KMS per DEC-1773 — `tenant_qtsp_creds.encrypted_creds_arn`; CISO-only writes via FR-AUTH-101.
...
9. **MUST** expose endpoints:
   ...
   POST   /v1/doc/qtsp/sign                   (internal — called by FR-DOC-005)
...
**Why PAdES-B-LT (DEC-1774)?** ETSI standard; enables LTV (long-term validation); FR-DOC-011 re-stamps at year-9 to keep valid.
...
Sample sign request (internal, FR-DOC-005 caller):
```json
{
  "document_id": "uuid",
  "signer_id": "uuid",
