...
source_decisions:
  ...
  - DEC-281 (per-tenant residency pinning — VN tenants store in `vn-1` region (e.g. ap-southeast-1 + VN-residency-mandate label); EU in `eu-1`; SG/US in respective regions per FR-AI-016)
...
new_files:
  ...
  - services/doc/src/residency.rs                                # per-tenant residency lookup → S3 bucket + KMS key (joins FR-AI-016)
...
modified_files:
  - services/auth/src/rbac/permissions.rs                        # +Resource::DocDocument (already in FR-AUTH-101 catalog at slice 1)
...
disallowed_tools:
  ...
  - implement actual eIDAS QTSP / AATL / VNeID signing here (those are FR-DOC-002/003/004 respectively)
...
sub_tasks:
  ...
  - "0.5h: residency.rs — per-tenant residency lookup (consumes FR-AI-016 policy)"
...
8. **MUST** pin each tenant's documents to the residency region per FR-AI-016 (per DEC-281). Lookup function `residency::resolve(tenant_id) -> (region, bucket, kms_key)`. The mapping:
...
11. **MUST** emit the following 8 memory audit row kinds (per DEC-289):
    ...
    - `doc.signed` — signing event from FR-DOC-002/003/004 (placeholder kind at slice 1).
...
12. **MUST** enforce **legal hold dual-signoff** (per DEC-285). `POST /v1/doc/documents/{id}/legal-hold` accepts body `{"apply": true | false, "primary_signer_subject_id": <uuid>, "secondary_signer_subject_id": <uuid>, "reason": "<text>", "case_reference": "<text>"}`. Validation:
    - `primary_signer` MUST have role `clo` per FR-AUTH-101.
    - `secondary_signer` MUST have role `cso` (security CSO, `cseco`) per FR-AUTH-101.
...
19. **MUST** support idempotent creation via `Idempotency-Key` header (same semantics as FR-AUTH-002 §1 #6).

20. **MUST** scope RBAC permissions per bucket_scope (per FR-AUTH-101): caller MUST have `Resource::DocDocument + Action::Write` for the specific scope. Slice 1 ships scope as a single resource; per-scope refinement (HR vs CRM scope override) lands in FR-DOC-2xx via scope_grants.
...
**Why access emits sev-2 audit (DEC-290, §1 #23, §1 #11)?** Document access is a forensically-relevant event — "did Person A read Person B's contract on Date X?". Sev-2 means it goes through FR-OBS-007's operator digest. Routine high-volume access (e.g. operations dashboard auto-fetching every contract) would create noise; the `purpose` requirement filters trivial cases (caller must supply a purpose; routine systems use a stable purpose code).

**Why `parent_object_id` is a soft-typed FK (§1 #1)?** The doc may belong to an HR contract (FR-HR-002), a CRM deal (FR-CRM-004), an ESOP grant (FR-ESOP-001), a vendor agreement (FR-OPS-vendor-2xx). Cross-service FKs are complex; soft-typing on `parent_object_id` + `bucket_scope` together identifies the owner, queried via the appropriate module's API.
...
/// for the given bucket_scope. Consumes FR-AI-016's residency policy.
...
**Upstream:**
- **FR-AUTH-101** — RBAC; `Resource::DocDocument + Action::Write/Read/Admin`; signer-role validation against the closed enum.

**Downstream (6 placeholders):**
- **FR-DOC-002** — eIDAS QTSP partner integration (EU residency).
- **FR-DOC-003** — AATL CA integration (US/non-EU enterprise).
- **FR-DOC-004** — VNeID + VN CA chain (VN tenants).
- **FR-DOC-005** — multi-party signing workflow.
- **FR-DOC-007** — lifecycle metadata (parties, dates, renewal).
- **FR-DOC-010** — DocuSign / Adobe Sign / HelloSign import with LTV preservation.
...
Deferred:
- **eIDAS QTSP / AATL / VNeID signing flows** — FR-DOC-002/003/004.
- **Multi-party signing workflow** — FR-DOC-005.
- **Lifecycle metadata (parties, dates, renewal)** — FR-DOC-007.
- **Expiry alert cascade** — FR-DOC-008.
- **Renewal proposal CUO draft** — FR-DOC-009.
- **DocuSign / Adobe Sign / HelloSign import** — FR-DOC-010.
- **PAdES-B-LT format with re-stamping** — FR-DOC-011.
...
- **`signature_event_log` (FR-DOC-002/003/004)** — those FRs add their own per-signature audit kinds; this FR ships only `doc.signed` as a placeholder.
