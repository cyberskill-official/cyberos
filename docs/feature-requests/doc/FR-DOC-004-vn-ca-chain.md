...
4. **MUST** support VNeID identity linkage per DEC-1793 at `vneid_linker.rs::link(signer, vneid_token)`:
   - Verify VNeID token via gov OAuth (FR-DOC-006 vneid handler shared).
...
7. **MUST** return CMS signature with VN trust chain per DEC-1795; FR-DOC-011 extends to LT.
...
10. **MUST** expose endpoints:
    ...
    POST   /v1/doc/vn-ca/sign                    (internal — FR-DOC-005 caller)
