...
7. **MUST** expose endpoints:
   ...
   POST   /v1/doc/import/{provider}/start             (CLO-only; FR-MCP-007 task)
   GET    /v1/doc/import/jobs/{id}                   (status)
   GET    /v1/doc/imports                            (list completed)
   ```

8. **MUST** run as async FR-MCP-007 task — provider listing + fetching may take hours for large customers.
