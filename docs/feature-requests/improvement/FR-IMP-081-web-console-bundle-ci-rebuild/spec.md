...
risk_if_skipped: "The exact FR-IMP-080 incident recurs on every future version bump that nobody manually rebuilds for: the served badge silently lags the platform VERSION again, and (per FR-IMP-080 §1) only the pre-commit/payload-gate/version.yml/release check-version-sync.sh gate catches it - and only if someone runs one of those paths locally before the next deploy, which is exactly the manual step that was skipped the first time."
---
## §1
...
3. When gated in, the job **MUST** rebuild `apps/web` (`npm ci && npm run build`, identical to the command FR-IMP-080 §1 already prints as the manual rebuild instruction) and, if `apps/console/web` differs from the committed copy, commit it back to `main` as `cyberos-bot <bot@cyberskill.world>` and push using the same `VERSION_BUMP_SSH_KEY` deploy key `version.yml` already uses to bypass the branch ruleset - no new secret, no new bot identity.
4. The commit message **MUST** carry `[skip ci]`: unlike `version.yml`'s bump commit (which must stay visible to tag-triggered workflows per FR-IMP-071), this commit only refreshes a served artifact and has no reason to re-trigger deploy.yml a second time for itself.
...
|---|---|---|
| `apps/web/**` changes but the branch ruleset blocks the bot's push | `GITHUB_STEP_SUMMARY` warning on the run, same as `version.yml`'s existing degrade path | add/rotate the deploy key in the ruleset bypass list (docs/deploy/RELEASE.md), or rebuild manually per FR-IMP-080's printed command |
