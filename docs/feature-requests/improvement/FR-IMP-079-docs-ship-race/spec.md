...
---
## §1
1. Both docs jobs **MUST** ship through one implementation, `tools/docs-site/ship.sh` - the duplicated inline snippet is how the shared-staging race was born, and FR-IMP-074's rules-to-channels principle applies to the shippers themselves.
...
- deploy.yml's docs job and release.yml's docs job still both exist by design (FR-DOCS-003: tags refresh the site too); deduplicating the JOBS is out of scope - this FR dedupes the shipper.
