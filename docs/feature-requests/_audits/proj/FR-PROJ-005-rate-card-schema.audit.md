...
Per feature-request-audit skill §0 master rule: spec is now perfect — depth bounded by the genuine surface (versioning × multi-currency × supersede vs archive × correction workflow × preview/lock/alias/default-pack policy × forensic trail), not by line targets.
