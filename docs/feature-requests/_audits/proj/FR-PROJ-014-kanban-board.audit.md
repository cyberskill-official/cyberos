...
Per feature-request-audit skill §0 master rule: spec is now perfect — depth bounded by the genuine surface (drag/drop × kbd parity × virtualisation × WIP × compact × bulk × quick-add × swimlanes × scroll preservation × undo × drag preview × reorder), not by line targets.
