...
Per feature-request-audit skill §0 master rule: spec is now perfect — depth bounded by the genuine surface (Bayesian compute × exp decay × per-cell × CI × outlier filter × bootstrap × team aggregate × drift × backfill × adoption metrics), not by line targets.

---

*End of FR-PROJ-013 audit.*
