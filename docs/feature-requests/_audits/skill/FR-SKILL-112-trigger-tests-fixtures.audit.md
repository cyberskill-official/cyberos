...
- **Classifier-routing adapter pattern.** FR-SKILL-112 §3 assumed a `classify(phrase) -> ClassificationResult(skill_id, confidence)` function — but the existing CUO router routes at the workflow level, not the skill level (`route(query) -> RoutingDecision(persona_slug, workflow_slug, confidence)`). Implementation bridged via a `SkillRoutingResult` adapter: `classify()` calls the existing `route()`, then resolves the workflow's `skill_chain[0]` as the "entry skill". Tests use monkeypatching to isolate; the live classifier path remains optional until the LLM router (FR-CUO-103) is fully wired.
- **RUBRIC.md location correction.** Same as FR-SKILL-111: rules landed in `modules/skill/SKILL_BUNDLE_RUBRIC.md` as SKB-050..057 (not FM-113 in feature-request-audit's rubric).
...
- **3 exemplar TRIGGER_TESTS.md fixtures shipped live.** feature-request-author (4+4), feature-request-audit (4+4), product-requirements-document-author (4+5) — all parse correctly via `load_fixture()`.
