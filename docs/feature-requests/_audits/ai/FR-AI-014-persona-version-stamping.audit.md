...
score_post_expansion: 9.0/10      # after expanding to FR-AI-012 depth (~860 lines)
...
This is the same identity-collision class as FR-AI-006 ISS-001 (province-code list duplicated across recognizers): the type doesn't carry enough information to unambiguously identify the entity.
...
This is the same pattern as FR-AI-009 ISS-004 and FR-AI-012 ISS-003 — `init` functions need either guard-against-double-call OR an explicit reset for tests.
...
- §3 introduces `PersonaHandle`, `PersonaId`, `Version`-from-`semver` as distinct, parseable types — preventing the identity-collision class of bug from ISS-001 once and for all. The newtype pattern is consistent with FR-AI-005's `TenantId` / `Region` types.
...
- §1 #11 explicitly defers persona-allow enforcement to FR-AI-001 §1 #13; the spec doesn't duplicate the check, eliminating the dual-source-of-truth problem.
...
*End of FR-AI-014 audit (final). Status: PASS at 10/10.*
