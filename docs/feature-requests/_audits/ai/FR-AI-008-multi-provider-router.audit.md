...
score_post_expansion: 9.0/10      # after expanding to FR-AI-001 depth
...
The clause says "MAY return Err" (optional) while the AC says "MUST return Err" (required). A reader following only §1 might write a partial streaming impl in slice 2 and pass §1 but fail §4. The 1500ms first-token SHOULD belongs to FR-AI-010 (the FR that actually wires streaming), not here.

#### Suggested fix
Replace §1 #16 with a slice-2-scoped clause and move the latency SHOULD to FR-AI-010:
...
16. **MUST** expose `call_provider_streaming` with the same signature as `call_provider` but
    ...
    `Err(RouterError::StreamingNotImplemented)`. Slice 3's FR-AI-010 replaces this stub with
    the SSE pipeline; FR-AI-010 owns the 1500ms p95 first-token SLA.
```
...
A code-gen agent reading this can't generate the real call — it has to invent the missing `mocks::resolved_with_primary(anthropic)` / `mocks::resolved_with_primary(openai)` lines. The same shape that was rejected in FR-AI-007 ISS-001 (proptest body promised but not shown).
...
This is the same single-source-of-truth pattern called out in FR-AI-006 ISS-001 (Provider trait having `is_zdr()` while FR-AI-015 also has `zdr::is_zdr`).
...
- §3 closed-set enums (`AttemptStatus` with 11 explicit variants, `FinishReason` with 5, `CacheState` with 3) match the FR-AI-007 pattern of exhaustive enum coverage.
- §6 skeleton's `metrics` module preamble (added per FR-AI-006 ISS-003 pattern) is the right level of detail and demonstrates `as_metric_label()` use throughout.
...
- ISS-001 RESOLVED (2026-05-16): §1 #16 rewritten to scope streaming behavior to slice 2 (MUST return `StreamingNotImplemented`); the 1500ms first-token SHOULD is moved to FR-AI-010's domain. AC #16 unchanged (still passes).
...
*End of FR-AI-008 audit (final). Status: PASS at 10/10.*
