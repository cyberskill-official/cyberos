...
score_post_expansion: 9.0/10      # after expanding to FR-AI-014 / FR-AI-019 depth (~1080 lines)
...
FR-AI-022 was expanded from 217 lines to ~1080 lines matching FR-AI-014 / FR-AI-019 depth.
...
This is the same prevention-vs-detection principle applied in FR-AI-011 (PII redaction at request boundary, not at provider response) and FR-AI-018 (cache cross-tenant isolation enforced at key derivation, not at lookup-time scan).
...
The first-pass §1 #7 said: *"MUST propagate trace context to provider HTTP calls so downstream (LangSmith via FR-OBS-004) can correlate."*
...
Without the implementation, FR-OBS-004's correlation breaks silently — the provider sees a request with no traceparent, can't link its own observability to ours.
...
*End of FR-AI-022 audit (final). Status: PASS at 10/10.*
