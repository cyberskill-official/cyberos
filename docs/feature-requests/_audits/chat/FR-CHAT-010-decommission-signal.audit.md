...
Per feature-request-audit skill §0 master rule: spec is now perfect — depth bounded by the genuine surface (6-state machine × per-tenant threshold × per-source weights × snooze × consecutive-check gates × regression detection × concrete-date signal × 30-day trend × operator-facing recommendation), not by line targets.
