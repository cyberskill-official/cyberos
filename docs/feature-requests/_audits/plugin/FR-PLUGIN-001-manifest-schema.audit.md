...
FR-MCP-003 enforces SEP-986 at MCP gateway runtime. If manifest schema doesn't ALSO enforce it, violations leak to publish time. Resolved: §1 clause 6 + DEC-2405 + schema pattern; AC #9.
...
FR-MCP-006 gates destructive tools by annotation. If manifest doesn't enforce the scope/annotation pairing, the gating breaks at install. Resolved: §1 clause 14 + validator.py custom check; AC #13.
