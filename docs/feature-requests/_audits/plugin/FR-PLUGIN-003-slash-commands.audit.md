...
Canonical slash-commands — 4 markdown files at modules/plugin/commands/, each with YAML frontmatter binding to FR-PLUGIN-002 tools. SCHEMA.md contract; 4 validator tests. 320 lines, 11 §1 clauses, 17 ACs, 4 test files, 14 failure modes, 8 implementation notes. 6 issues resolved (mirror tool input_schema for single-source consistency; 4-trigger discipline matches FR-SKILL-111 routing fingerprint; 60-480-char description range matches FR-SKILL-111 ceiling; destructive flag surfaces FR-MCP-006 gating to host UX; body-section worked-example forces meaningful authorship; explicit "no command renames in v1.x.y" prevents user muscle-memory breakage). **Score = 10/10.**
