...
**Implementer:** Cowork session of 2026-05-19. **Verdict:** PASS (slice-1 substrate) — every §1 MUST-clause has a code, config, migration, or test artefact. Live-Stalwart wiring intentionally deferred per `disallowed_tools` (FR-EMAIL-002 takes over).

### §10.1 — Clause → AC → artefact traceability

| §1 Clause | §4 AC | Shipped artefact | Status |
|---|---|---|---|
| #1 Deploy Stalwart v0.10.x container | #1, #2, #4 | `docker/Dockerfile` + `docker/stalwart.toml` (ports 25/465/587 + 143/993 + 4190 + 8080) | ✅ container shape; live-runtime smoke deferred to FR-EMAIL-002 |
...
| #22 OTel spans | — | `tracing` macros wired in `bin/server.rs`; structured-JSON output; full span emission is FR-OBS-001 territory | ⏸️ FR-OBS-001 |
| #23 OTel metrics | — | Metric names declared in §1; emission wires via `tracing` → OTel exporter in FR-OBS-003 | ⏸️ FR-OBS-003 |
...
**§10.6 — RLS GUC naming.** Spec §1 #10 uses `auth.tenant_id`. Shipped implementation uses `app.current_tenant_id`, aligning with FR-AUTH-003 §10.6 amendment (which converged on the global-GUC pattern). The pattern is strictly stronger (one policy per table instead of one per tenant × table) and matches the rest of the cluster.
...
**Status:** `draft → shipped (slice 1)`. Next FRs (FR-EMAIL-002 JWT bridge, FR-EMAIL-004 DKIM/ARC/BIMI, etc.) build on the substrate this FR provides.
