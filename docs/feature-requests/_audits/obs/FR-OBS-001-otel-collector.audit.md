...
First-pass had no PII filter at collector. FR-AI-022's typed-attribute-keys is primary, but defence-in-depth requires collector-side scrub. Resolved: §1 #11 normative + `attributes/pii_scrub` processor in pipeline + AC #10 + sev-1 alarm.
...
First-pass set up datasources but no hook for FR-OBS-002's dashboards. Resolved: §1 #5 + provisioning/dashboards mount in compose; FR-OBS-002 drops dashboards into the dir.
