...
source_decisions:
  ...
  - DEC-1005 2026-05-17 — Per-tenant CNAME requires `tenant_admin` role at TENANT level (not engagement_admin) — DNS misconfig has cross-engagement blast radius (mirrors FR-PORTAL-003 DEC-882 pattern)
  - DEC-1006 2026-05-17 — Email template overrides: per-tenant Tera template fragments for FR-EMAIL-001 templates (welcome, magic-link, password-reset, invoice-receipt); fallback chain tenant → CyberSkill default
...
**Why tenant_admin only for CNAME (§1 #15, DEC-1005)?** DNS misconfig affects every user landing on `portal.<tenant>.com`; engagement_admin role's scope is one engagement, not the tenant-wide DNS. Mirrors FR-PORTAL-003 DEC-882 pattern. Reduces blast-radius surface.
...
**Upstream (depends_on):**
- **FR-TEN-101** Self-serve signup — tenant exists before brand pack can be created.

**Cross-module (related_frs):**
- **FR-PORTAL-001** Scoped read-only views — consumes brand pack at render time.
- **FR-PORTAL-003** External IdP — brand applied at IdP redirect login page.
- **FR-PORTAL-005** Branded Genie — brand pack applied to embedded chat.
- **FR-TEN-103** Residency provisioning — CDN edge selection per residency.
...
**§11.15** Per-tenant rate limit (10 saves/day) uses Redis sliding-window like FR-TEN-101 §1 #5; same infrastructure.

**§11.16** Email-override fallback chain is FR-EMAIL-001 internal mechanism; this FR populates the override files via `portal_brand_packs.email_overrides` JSONB → on-disk Tera fragment.
