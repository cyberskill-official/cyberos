...
§1 #11 says per-Engagement isolation. But subject A in Engagements X, Y, Z — when X SCIM DELETEs, do we revoke X-scoped JWTs only? Resolved: §11.10 confirms each JWT carries `engagement_id` claim (FR-PORTAL-003 derivative); blacklist scoped to `(tenant_id, engagement_id)`; AC #12 verifies. Y + Z JWTs unaffected.
