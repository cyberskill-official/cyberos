...
source_decisions:
  - DEC-1170 2026-05-17 — Genie chat embedded in client-tenant portal (via FR-PORTAL-001 scoped views); narrows CUO answers to ONLY data the JWT's scope_grants allow (a client user sees their own projects + docs, not the consulting firm's full org)
  - DEC-1171 2026-05-17 — Scope_grants schema in JWT: array of `{ resource_type: enum, resource_ids: [uuid], permissions: [enum] }` per FR-AUTH-004 extension; closed enum for resource_type per DEC-1172
  ...
  - DEC-1175 2026-05-17 — Brand pack applied to Genie UI per FR-PORTAL-002: colours, logo, font; default-pack used if tenant hasn't configured one
  - DEC-1176 2026-05-17 — Session integration with FR-PORTAL-003: Genie session inherits IdP-auth subject + Engagement scope from the JWT; SCIM deprovision (FR-PORTAL-004) cascades to active Genie sessions
  ...
  - DEC-1184 2026-05-17 — IdP-auth subjects (`auth_method='external_idp'`) get 8h JWT TTL per FR-PORTAL-003 DEC-879; Genie session re-auths at expiry; conversation history preserved across sessions
...
5. **MUST** extend FR-AUTH-004 JWT mint with `scope_grants` claim per DEC-1171. Claim shape:
    ```json
    "scope_grants": [
    ...
    ```
   JWT mint resolves the caller's FR-AUTH-101 RBAC role → scope_grants from `engagement_memberships` table.
...
9. **MUST** apply per-Engagement brand from FR-PORTAL-002 per DEC-1175. The Genie UI inlines per-Engagement brand colours + logo via the `portal_brand_pack_active` lookup; default pack if none configured.
...
12. **MUST** invalidate Genie sessions on FR-PORTAL-004 SCIM deprovision per DEC-1176. The cascade target `Genie sessions` from FR-PORTAL-004 §1 #6 UPDATE `portal_genie_sessions SET revoked_at=now(), revoked_reason='scim_deprovision' WHERE caller_subject_id=$1 AND engagement_id=$2`. Active SSE connections close per FR-PORTAL-004 §1 #5 (WebSocket close 4001 equivalent for SSE = abort stream + emit `error` event).
...
**Why brand pack inheritance from FR-PORTAL-002 (§1 #9, DEC-1175)?** Single source of truth for tenant branding; Genie shouldn't have its own brand config. PORTAL-002 brand pack applies everywhere PORTAL renders, including Genie.
...
**Why 90-day retention on messages (§1 #14, DEC-1178)?** Long enough for the user to come back next sprint + re-find a useful answer; bounded to limit storage growth. Matches FR-TEN-101 signup_sessions pattern.
...
10. **Brand pack applied** — Genie UI fetches brand pack from FR-PORTAL-002; falls back to default if none.
11. **Rate limit 60/min** — 61st query in 60s → SSE event `{error: "rate_limited"}`.
12. **SCIM-cascade revoke** — FR-PORTAL-004 SCIM DELETE cascades to active Genie session → `revoked_at` set + active SSE aborted.
...
**Upstream (depends_on):**
- **FR-PORTAL-003** External IdP + SCIM JIT — IdP-auth subjects + JWT scope_grants pattern.
- **FR-CUO-101** LangGraph supervisor — CUO orchestrator invoked here; receives scope_grants context + enforces boundary via doctor invariant.

**Cross-module (related_frs):**
- **FR-PORTAL-001** Scoped read-only views — same scope_grants apply.
- **FR-PORTAL-002** Brand pack — Genie UI inherits brand.
- **FR-PORTAL-004** SCIM deprovision — cascade target.
...
- **Deferred:** Genie-to-tool action (Genie suggesting tool call requiring user confirm) — slice 4, requires FR-MCP-008 integration.
...
|---|---|---|---|
| CUO retrieves cross-tenant data | post-flight check | Response replaced with safe message; sev-1 audit | Operator investigates CUO; FR-CUO-101 doctor invariant should have caught |
...
| CUO emits sources without tenant_id metadata | post-flight needs the field | Sev-1 audit; conservative reject (assume violation) | FR-CUO-101 schema fix |
...
**§11.2** Rate limiter is sliding-window Redis (same pattern as FR-TEN-101 + FR-MCP-007).
...
**§11.9** Conversation history retention: 90 days; daily prune via `cyberos_pruner` role (same as FR-MCP-007 pattern).
...
**§11.14** Genie response streaming uses CUO's streaming endpoint (FR-CUO-101 derivative); non-streaming CUO models accumulate then emit single-event.
...
**§11.17** Per-tool MCP integration via FR-MCP-008 elicitation (slice 4): Genie suggests tool calls; user confirms via elicitation; tool runs; response continued.
