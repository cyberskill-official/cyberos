...
source_decisions:
  - DEC-1240 2026-05-17 — Closed enum `client_workflow_kind` = {new_project_request, billing_inquiry, support_ticket, feature_request, general_question}; CI cardinality asserts 5
  ...
  - DEC-1252 2026-05-17 — Email notification to submitter on every status change via FR-EMAIL-001 (per-tenant template overrides per FR-PORTAL-002)
...
sub_tasks:
  ...
  - "0.4h: workflows/attachments.rs (FR-DOC-001 presigned)"
...
7. **MUST** create CHAT thread per DEC-1241 via `chat_bridge.rs::create_thread(workflow_id)`:
    - Calls FR-CHAT-005 create-thread API in the Engagement-scoped channel.
...
10. **MUST** support file attachments per DEC-1246. Handler:
    - Returns presigned S3 URLs from FR-DOC-001 for each attachment slot at submission-start.
...
CREATE TYPE client_workflow_kind AS ENUM ('new_project_request','billing_inquiry','support_ticket','feature_request','general_question');
...
| CHAT thread creation fails | FR-CHAT-005 error | Workflow row rolled back; 500 | Operator investigates CHAT |
...
| Email send fails | FR-EMAIL-001 error | Status change still committed; email retried by FR-EMAIL-001 job | Inherent retry |
| Submitter deprovisioned mid-workflow (FR-PORTAL-004) | session revoked | Subsequent actions 401; workflow row remains | Engagement team continues internally |
...
**§11.6** Client reply via portal UI: server creates CHAT message via FR-CHAT-005 with type='client_reply' + author=submitter_subject_id.

**§11.7** Email templates per FR-PORTAL-002 brand pack: per-tenant override fallback chain.
