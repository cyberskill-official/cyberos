...
source_decisions:
  ...
  - DEC-1223 2026-05-17 — Identity verification: SSO-authenticated callers (FR-PORTAL-003) pass identity automatically; legacy email-password callers require step-up via email-confirmation link
  - DEC-1224 2026-05-17 — Async processing: DSAR requests queued via FR-MCP-007 Tasks primitive (long-running) — typical 1-72 hours for full bundle assembly across PROJ/INV/DOC/CHAT/audit history
  ...
  - DEC-1226 2026-05-17 — Delivery: signed URL to S3 with 7-day TTL; passphrase emailed separately via FR-EMAIL-001; caller MUST verify integrity via manifest signature
  - DEC-1227 2026-05-17 — Deletion requests trigger FR-TEN-104 tenant-erasure flow OR per-subject erasure via FR-AUTH-002 hard-purge (if subject is the only requester; not destroying tenant)
...
sub_tasks:
  ...
  - "0.4h: dsar/create.rs"
  - "0.5h: dsar/processor.rs (FR-MCP-007 task)"
...
6. **MUST** expose `POST /v1/portal/dsar/request` body `{ dsar_type, scope_description?, passphrase_hash? }`. Handler:
    ...
    - Email-password caller → status='identity_verification_pending' + email confirmation link via FR-EMAIL-001.
    - Enqueues processing via FR-MCP-007 task per DEC-1224.
...
10. **MUST** support deletion via FR-TEN-104 or FR-AUTH-002 hard-purge per DEC-1227. `dsar_type='deletion_request'` triggers internal review queue (sev-1) before destructive action; operator manually invokes hard-purge after legal sign-off.
...
**Why async via Tasks primitive (§1 #6, DEC-1224)?** Bundle assembly = cross-table scan + cross-tenant filter + ZIP encryption — easily minutes to hours for active subjects. Sync processing = 30s gateway timeout; async via FR-MCP-007 fits exactly. Reuses existing infrastructure.
...
|---|---|---|---|
| Identity verification email fails | FR-EMAIL-001 error | Status stays pending; daily reminder + 30d expiry | Caller requests resend |
| Bundle assembly task fails | FR-MCP-007 task failed | Sev-1 alert; status='received' again; operator investigates | Manual retry |
...
**§11.3** Task processor uses FR-MCP-007 worker pool; per-module=`portal`; max_concurrent=2 (heavy I/O).

**§11.4** Email delivery via FR-EMAIL-001 with priority=high; identity-verify email has 24h link expiry.
