...
source_decisions:
  ...
  - DEC-861 2026-05-17 — SAML 2.0 + OIDC reuse FR-AUTH-103 + FR-AUTH-104 protocol primitives; PORTAL-003 adds the per-Engagement layer + SCIM
  ...
  - DEC-868 2026-05-17 — SCIM CREATE → AUTH subject create + Engagement membership grant; SCIM UPDATE → attribute sync; SCIM DELETE → FR-PORTAL-004 deprovision flow (out-of-scope here)
...
9. **MUST** support OIDC RP per FR-AUTH-104 protocol layer + per-Engagement metadata at `/oidc/v1/{engagement_slug}/.well-known/openid-configuration`. The callback at `/oidc/v1/{engagement_slug}/callback` verifies:
    - PKCE code_verifier matches the stored challenge (per FR-AUTH-104 DEC-401).
    - `state` nonce matches the server-issued state (single-use, 5-min TTL).
    - ID token signature against IdP JWKS (cached 24h, kid-based rotation per FR-AUTH-104 DEC-402).
...
11. **MUST** JIT-provision via FR-AUTH-002 at first SSO sign-in per DEC-862. Flow:
    ...
    - If not exists: INSERT subject via FR-AUTH-002 with `auth_method='external_idp'`, `idp_config_id`, `external_id`, `email` (claim), `display_name` (claim), `role` (claim-mapped per #12).
    - Bind subject to Engagement via FR-AUTH-101 RBAC grant.
...
13. **MUST** expose SCIM 2.0 endpoints per RFC 7643 + RFC 7644 + DEC-867:
    ...
    - `DELETE /scim/v2/{engagement_slug}/Users/{id}` — soft-tombstone (delegate to FR-PORTAL-004 for full session-invalidation flow).
...
17. **MUST** enforce JWT session lifetime 8h for IdP-authenticated subjects per DEC-879 + DEC-883. FR-AUTH-004 JWT mint reads `subject.auth_method` — if `external_idp`, sets `exp = now + 8h`. At any sensitive operation, the handler checks `last_sso_at`; if > 8h ago AND `enforcement='required'`, redirect to IdP for re-authentication.
...
25. **MUST NOT** allow a JIT-provisioned subject to elevate role beyond claim-mapped per DEC-863. Subsequent role mutations via FR-AUTH-101 admin UI are permitted (tenant_admin can override claim-mapping), but the JIT path itself is closed.
...
27. **MUST** emit per-Engagement metric `portal_idp_sign_in_duration_seconds` to OBS (FR-OBS-005). Alarm sev-2 if p95 > 3 s sustained 10 min (latency indicator for IdP-side issues).
...
9. **Required enforcement disables password** — user in `required`-enforcement Engagement attempts FR-AUTH-004 password login → 403 + `sso_required` + IdP redirect URL.
...
**Upstream (depends_on):**
- **FR-AUTH-103** SAML SSO — SAML protocol primitives (Response parse, Assertion verify, cert chain).
- **FR-AUTH-104** OIDC SSO — OIDC RP primitives (ID token verify, JWKS rotation, PKCE).

**Cross-module (related_frs):**
- **FR-AUTH-002** Subject create — JIT calls into this.
- **FR-AUTH-004** JWT mint — first-login token after SSO.
- **FR-AUTH-101** RBAC — Engagement membership grants.
- **FR-PORTAL-001** Scoped read-only views — depends on subjects existing.
- **FR-PORTAL-002** Per-tenant brand pack — sign-in page brand applied.
...
**Downstream (blocks):**
- **FR-PORTAL-004** SCIM deprovision — needs SCIM endpoints + session linkage.
- **FR-PORTAL-005** Branded Genie — needs IdP-auth scope_grants.
...
- **Deferred:** SCIM 2.0 Group sync DELETE → cascade subject-membership revoke — slice 2, FR-PORTAL-004 path.
...
- **Deferred:** Per-Engagement custom domain for SP endpoint (`sso.acme.com/saml2/acs`) — slice 2 + FR-PORTAL-002 brand pack integration.
- **Deferred:** SAML SLO (Single Log-Out) — slice 2; slice 1 has SCIM DELETE deprovision via FR-PORTAL-004.
...
| OIDC ID token signature invalid | JWKS verify fails | 401 + `id_token_invalid` + `portal.idp_sign_in_failed` | JWKS rotation drift — cache refresh triggered (24h max stale per FR-AUTH-104) |
...
**§11.8** The `subjects.last_sso_at` column requires a migration to FR-AUTH-002's table — added via `services/auth/migrations/0XXX_subjects_last_sso_at.sql` referenced by this FR's modified_files.
...
**§11.13** Per-Engagement OIDC metadata at `/oidc/v1/{eng}/.well-known/openid-configuration` differs from per-Tenant OIDC of FR-AUTH-104 — PORTAL is the RP-facing-OUTWARD endpoint (IdP-side configures against this), while FR-AUTH-104 is the AUTH server-as-RP for internal SSO.
...
**§11.17** PII hashing: `email_hash16 = encode(substring(digest(coalesce(lower(email), '') || global_salt, 'sha256') from 1 for 8), 'hex')` — consistent with FR-TEN-101's hash function. Global salt rotates yearly; old hashes retained for forensic comparison.
