...
related_frs: [FR-SKILL-101, FR-SKILL-102, FR-SKILL-103, FR-SKILL-104, FR-SKILL-106, FR-MEMORY-101, FR-MEMORY-106, FR-MEMORY-107, FR-MEMORY-109, FR-MEMORY-111]
...
new_files:
  ...
  - skills/memory-capture/main.rs                              # skill binary (broker subprocess entrypoint)
  - skills/memory-capture/Cargo.toml
  - skills/memory-capture/src/lib.rs                           # public Rust SDK
  ...
  - skills/memory-capture/cli/cyberos-memory-capture           # bash wrapper for one-shot CLI use
  ...
  - skills/memory-capture/scripts/sign-bundle.sh              # uses FR-SKILL-102 signing infra
modified_files:
...
disallowed_tools:
  - call memory_writer directly from SDK (per DEC-200 — must go through broker per FR-SKILL-104)
...
sub_tasks:
  ...
  - "1.0h: src/emit.rs — bridge to broker via JSON-RPC; PII-scrub via FR-MEMORY-111 before send"
...
2. **MUST** carry a valid SKILL.md frontmatter per FR-SKILL-103 schema v1:
    ...
    - `sync_class: shareable` (rows emitted by this skill MAY sync to Cloud memory per FR-MEMORY-106).
...
8. **MUST** PII-scrub payload via FR-MEMORY-111's ruleset BEFORE broker call. Scrub failure → `CaptureError::PiiScrubFailed`; caller can retry with `payload_sanitized = true` flag (telling SDK that caller has already sanitised and we trust the input).
...
14. **MUST** be signed via FR-SKILL-102 release pipeline; the OCI tag is `oci://registry.cyberos.world/skills/memory-capture:1.0.0`; immutable.
...
impl MemoryCapture {
        ...
        -> Result<EmitOutcome, CaptureError>
    {
        ...
        // §1 #8: PII scrub (calls broker's MemoryEmit which itself runs FR-MEMORY-111;
...
16. **SKILL.md validates** — `cyberos skill validate skills/memory-capture/` → exit 0 (FR-SKILL-103 validator).
...
- **FR-SKILL-104 (upstream)** — capability broker enforces `allowed_tools: [MemoryEmit]`.
- **FR-SKILL-102** — OCI distribution + signature pipeline.
- **FR-MEMORY-101** — MemoryWriter (called via broker).
- **FR-MEMORY-106** — sync_class enforcement.
- **FR-MEMORY-107** — capture daemon (handles emit forwarding).
- **FR-MEMORY-109** — Claude Code hook is a SIBLING capture path (also writes via FR-MEMORY-107).
- **FR-MEMORY-111** — PII detection used inside SDK pre-scrub.
...
|---|---|---|---|
| Broker socket missing | UnixStream::connect Err | exp backoff retry; final `BrokerDown` | Wait for FR-MEMORY-110 to restart |
...
| `payload_sanitised: true` lies (raw PII slips through) | Broker's FR-MEMORY-111 scan catches it | Broker denies; caller error | Caller stops lying |
...
- `payload_sanitised: true` is honoured by the SDK but the broker still runs FR-MEMORY-111 scrub. Pre-scrub is a performance optimisation, not a security boundary.
