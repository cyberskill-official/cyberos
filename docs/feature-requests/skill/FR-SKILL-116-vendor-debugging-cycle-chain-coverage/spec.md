...
5. `build.sh` MUST run `check-chain-coverage.sh` against its own output as its final step and propagate a failure - a payload that under-covers its own workflow can no longer be produced. A reduced-profile payload (zero vendored skills - the documented doc-driven floor) is exempt: the check prints `chain SKIP: reduced profile` and exits 0; partial vendoring still fails. (Amended post-ship 2026-07-12: FR-CUO-209 t07 surfaced that the check as first shipped broke the reduced floor.)
