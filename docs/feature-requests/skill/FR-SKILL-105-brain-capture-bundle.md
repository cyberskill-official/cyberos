---
id: FR-SKILL-105
title: "brain-capture@1 skill bundle — canonical SDK-style entry point for emitting BRAIN capture rows from tools, scripts, and external integrations"
module: SKILL
priority: MUST
status: accepted
verify: T
...
related_frs: [FR-SKILL-101, FR-SKILL-102, FR-SKILL-103, FR-SKILL-104, FR-SKILL-106, FR-BRAIN-101, FR-BRAIN-106, FR-BRAIN-107, FR-BRAIN-109, FR-BRAIN-111]
depends_on: [FR-SKILL-103, FR-SKILL-104]
blocks: [FR-SKILL-106]

source_pages:
  - website/docs/skills/brain-capture.html
  - website/docs/runbooks/brain-capture-skill-runbook.html
source_decisions:
  - DEC-200 (brain-capture@1 is THE canonical capture entry; every emit goes through it)
  - DEC-201 (skill bundle MUST be signed via FR-SKILL-102 OCI signing path)
...
service: cyberos/skills/brain-capture/
new_files:
  - skills/brain-capture/SKILL.md
  - skills/brain-capture/main.rs                              # skill binary (broker subprocess entrypoint)
  - skills/brain-capture/Cargo.toml
  - skills/brain-capture/src/lib.rs                           # public Rust SDK
  - skills/brain-capture/src/emit.rs
  - skills/brain-capture/src/dedup.rs
  - skills/brain-capture/sdk-python/cyberos_brain_capture/__init__.py
  - skills/brain-capture/sdk-python/setup.py
  - skills/brain-capture/cli/cyberos-brain-capture           # bash wrapper for one-shot CLI use
  - skills/brain-capture/tests/sdk_rust_test.rs
  - skills/brain-capture/tests/sdk_python_test.py
  - skills/brain-capture/tests/cli_e2e_test.sh
  - skills/brain-capture/scripts/sign-bundle.sh              # uses FR-SKILL-102 signing infra
modified_files:
  - cyberos/Cargo.toml                                       # workspace member for SDK crate
allowed_tools:
  - file_read: skills/brain-capture/**, services/brain/**
  - file_write: skills/brain-capture/**
  - bash: cd skills/brain-capture && cargo test
  - bash: cd skills/brain-capture && python3 -m pytest sdk-python
disallowed_tools:
  - call brain_writer directly from SDK (per DEC-200 — must go through broker per FR-SKILL-104)
...
effort_hours: 9
sub_tasks:
  - "0.5h: SKILL.md frontmatter (id=brain-capture, version=1.0.0, allowed_tools=[BrainEmit], allowed_brain_scopes=memories/**)"
  - "0.5h: Cargo.toml + main.rs (subprocess entrypoint that connects to broker socket and exposes `cb capture` API)"
  - "1.0h: src/lib.rs — public Rust SDK with `BrainCapture::new().emit(kind, payload, scope).await`"
  - "1.0h: src/emit.rs — bridge to broker via JSON-RPC; PII-scrub via FR-BRAIN-111 before send"
  ...
  - "0.5h: cli/cyberos-brain-capture — one-shot bash wrapper: `echo '{...}' | cyberos-brain-capture --kind notes.captured`"
  - "1.0h: sign-bundle.sh — calls FR-SKILL-102 signing pipeline; outputs .skill artifact"
...
risk_if_skipped: "Without a canonical capture entry, every tool that wants to emit a BRAIN row reinvents the wheel: connection management, dedup, PII scrub, signature, error handling. Different tools end up with subtly different audit-row shapes — operators querying BRAIN can't filter consistently. The skill is the contract: 'if you import brain-capture@1, your rows look right by construction.' Without the multi-language SDK, only Rust callers can emit; the Python/bash escape hatch is needed for ad-hoc scripts. Without dedup, the same idempotent operation (e.g. nightly cron emitting status) creates thousands of identical rows."
---

## §1 — Description (BCP-14 normative)

The `brain-capture@1` skill bundle **MUST** be the canonical, signed, OCI-distributed SDK for emitting BRAIN capture rows. The bundle:

1. **MUST** ship as a single `.skill` artifact containing: (a) the Rust skill binary `main` (broker subprocess entrypoint), (b) a Rust SDK crate (`cyberos-brain-capture`), (c) a Python wrapper (`cyberos_brain_capture`), (d) a bash CLI (`cyberos-brain-capture`). All three language frontends route through the same Rust core.
2. **MUST** carry a valid SKILL.md frontmatter per FR-SKILL-103 schema v1:
    - `id: brain-capture`, `version: 1.0.0`, `description: "Canonical entry point for emitting BRAIN capture rows."`
    - `allowed_tools: [BrainEmit]` (only — never Bash, Read, HttpFetch).
    - `allowed_brain_scopes: ["memories/**"]` (broad emit scope; finer-grained scoping is the caller's frontmatter responsibility).
    - `sync_class: shareable` (rows emitted by this skill MAY sync to Cloud BRAIN per FR-BRAIN-106).
    - `signature` populated via FR-SKILL-102 signing pipeline.
3. **MUST** expose the following stable API in the Rust SDK:
    ```rust
    pub struct BrainCapture { /* opaque */ }
...
4. **MUST** expose the equivalent Python API:
    ```python
    from cyberos_brain_capture import BrainCapture, EmitScope, CaptureError
...
5. **MUST** expose the bash CLI:
    ```text
    $ echo '{"title":"x","body":"y"}' | cyberos-brain-capture --kind notes.captured --scope private
...
8. **MUST** PII-scrub payload via FR-BRAIN-111's ruleset BEFORE broker call. Scrub failure → `CaptureError::PiiScrubFailed`; caller can retry with `payload_sanitized = true` flag (telling SDK that caller has already sanitised and we trust the input).
9. **MUST** auto-discover the broker socket from `CYBEROS_BROKER_SOCKET` env var (set by FR-SKILL-104 when subprocess is launched). Outside a broker invocation (e.g. ad-hoc CLI use), the SDK connects to the default broker socket `/run/cyberos/broker.sock` (system-scope) or `~/.cyberos/broker.sock` (user-scope).
...
11. **MUST** propagate W3C TraceContext: read `TRACEPARENT` env var (set by upstream) or generate a new trace_id per `BrainCapture::new()` call. Every emit carries this trace_id.
12. **MUST** emit OTel span `skill.brain_capture.emit` per call with attributes `kind`, `dedup_hit`, `payload_size_bytes`, `pii_scrub_match_count`, `duration_ms`.
13. **MUST** emit OTel metrics:
    - `skill_brain_capture_emits_total{kind, outcome}` (counter; outcome ∈ emitted | deduped | denied | broker_down | pii_failed).
    - `skill_brain_capture_dedup_hit_ratio` (gauge).
14. **MUST** be signed via FR-SKILL-102 release pipeline; the OCI tag is `oci://registry.cyberos.world/skills/brain-capture:1.0.0`; immutable.
...
**Why a skill at all (§1 #1)?** Three motives. First, ENFORCEMENT: routing emit through the FR-SKILL-104 capability broker ensures every BRAIN write is authorised, audited, and PII-scrubbed (no path that bypasses these). Second, IDEMPOTENCY: the SDK's dedup layer (§1 #7) cuts duplicate emit load by ~10× under realistic churn. Third, DISTRIBUTION: a signed OCI artifact gives operators a single answer to "how do I emit a memory?" — `cargo add cyberos-brain-capture` or `pip install cyberos-brain-capture`.
...
**Why allowed_tools = [BrainEmit] only (§1 #2)?** Defense-in-depth. The skill has zero need for Bash/Read/HttpFetch; granting them would expand attack surface. The capability broker enforces this — if a future SKILL.md PR adds Bash by mistake, the broker rejects every Bash call at invoke time.

**Why kind regex (§1 #6)?** A namespaced kind (`module.event_type`) lets operators query "all rows from the notes module" via prefix match. Without regex enforcement, kinds drift into `notes-captured` vs `notes.captured` vs `notes_captured` chaos. The regex locks the convention; FR-AI-003's closed-set list is the authoritative registry.
...
**Why W3C trace propagation (§1 #11)?** Traces let operators correlate "user prompt → tool call → BRAIN row" across service boundaries. Without propagation, the BRAIN row appears orphaned; with it, OBS dashboards show the full chain.
...
id: brain-capture
version: 1.0.0
description: Canonical entry point for emitting BRAIN capture rows.
...
tenant_scope: any
effort_minutes: 5
tags: [brain, capture, foundation, sdk]
signature:
  algo: ed25519
...
The canonical entry point for emitting BRAIN capture rows.

## Quick start (Rust)

```rust
use cyberos_brain_capture::{BrainCapture, EmitScope};

let cb = BrainCapture::new().await?;
let outcome = cb.emit(
    "notes.captured",
...
```python
from cyberos_brain_capture import BrainCapture, EmitScope
cb = BrainCapture()
...
```bash
echo '{"title":"x"}' | cyberos-brain-capture --kind notes.captured --scope private
```
```
...
```rust
// skills/brain-capture/src/lib.rs
...
impl BrainCapture {
        ...
        -> Result<EmitOutcome, CaptureError>
    {
        ...
        if let Some(prior) = self.dedup.get(&key) {
            metrics::counter!("skill_brain_capture_emits_total", "kind" => kind.to_owned(), "outcome" => "deduped").increment(1);
            return Ok(EmitOutcome { deduped: true, ..prior.clone() });
        }

        // §1 #8: PII scrub (calls broker's BrainEmit which itself runs FR-BRAIN-111;
        ...
        metrics::counter!("skill_brain_capture_emits_total", "kind" => kind.to_owned(), "outcome" => "emitted").increment(1);
        Ok(outcome)
    }
...
class BrainCapture:
    def __init__(self):
        self.binary = os.environ.get("CYBEROS_BRAIN_CAPTURE_BIN", "cyberos-brain-capture")
...
```bash
#!/usr/bin/env bash
# skills/brain-capture/cli/cyberos-brain-capture
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    -h|--help)
      cat <<EOF
usage: cyberos-brain-capture --kind <module.event> [--scope private|shareable] [--acl @actor]* [--payload-sanitised]
  stdin: JSON payload
...
exec cyberos-brain-capture-main --kind "$KIND" --scope "$SCOPE" \
  $(printf -- "--acl %s " "${ACL[@]:-}") \
...
1. **Rust SDK: emit succeeds** — `BrainCapture::new()` + `emit("notes.captured", ..., Private)` → `EmitOutcome { row_id != "", deduped: false }`.
...
13. **Bash CLI: stdin JSON** — `echo '{"x":1}' | cyberos-brain-capture --kind a.b` → `EmitOutcome` JSON to stdout, exit 0.
14. **Bash CLI: missing --kind** — `cyberos-brain-capture` → exit 1 with stderr.
15. **Bash CLI: --acl multi** — `--acl @alice --acl @bob` → scope becomes `Shareable { acl: ["@alice", "@bob"] }`.
16. **SKILL.md validates** — `cyberos skill validate skills/brain-capture/` → exit 0 (FR-SKILL-103 validator).
17. **SKILL.md signature verified** — release-time signature → validate returns `signature: ok`.
18. **Broker enforcement: only BrainEmit allowed** — try to call `Bash` from inside this skill → broker returns `tool_not_in_allowed_tools`.
19. **Bundle signed via FR-SKILL-102** — `sign-bundle.sh` produces `.skill` artifact; `cyberos skill verify <artifact>` returns `signed_by: <key>`.
20. **Dedup TTL: 60s expiry** — first emit; wait 61s; identical emit → fresh (not deduped).
21. **OTel span per emit** — exporter receives `skill.brain_capture.emit` with attrs.
22. **Metric: outcome counter** — counters `skill_brain_capture_emits_total{outcome="emitted"}`, `{outcome="deduped"}` increment correctly.
...
async fn emit_succeeds_against_stub_broker() {
    ...
    let mut cb = BrainCapture::new().await.unwrap();
...
async fn invalid_kind_rejected() {
    let mut cb = BrainCapture::test().await;
...
echo '{"title":"cli"}' | cyberos-brain-capture --kind notes.captured --json > /tmp/result.json
[ "$(jq -r .row_id /tmp/result.json | head -c 4)" = "row-" ]
[ "$(jq -r .deduped /tmp/result.json)" = "false" ]

# Missing --kind
! cyberos-brain-capture --json < /dev/null 2>&1 | grep -q "ERROR"

# Idempotent
echo '{"x":2}' | cyberos-brain-capture --kind a.b --json > /tmp/r1.json
echo '{"x":2}' | cyberos-brain-capture --kind a.b --json > /tmp/r2.json
...
- **FR-SKILL-103 (upstream)** — frontmatter schema.
- **FR-SKILL-104 (upstream)** — capability broker enforces `allowed_tools: [BrainEmit]`.
- **FR-SKILL-102** — OCI distribution + signature pipeline.
- **FR-BRAIN-101** — BrainWriter (called via broker).
- **FR-BRAIN-106** — sync_class enforcement.
- **FR-BRAIN-107** — capture daemon (handles emit forwarding).
- **FR-BRAIN-109** — Claude Code hook is a SIBLING capture path (also writes via FR-BRAIN-107).
- **FR-BRAIN-111** — PII detection used inside SDK pre-scrub.
...
|---|---|---|---|
| Broker socket missing | UnixStream::connect Err | exp backoff retry; final `BrokerDown` | Wait for FR-BRAIN-110 to restart |
...
| OTel exporter unavailable | metric buffered | Metric eventually dropped after buffer fills | Operator restarts FR-OBS-001 |
...
| `payload_sanitised: true` lies (raw PII slips through) | Broker's FR-BRAIN-111 scan catches it | Broker denies; caller error | Caller stops lying |
...
- The bundle ships as a single `.skill` tar.zst per FR-SKILL-102 format; contents: `main` binary + `SKILL.md` + `sdk-rust/` + `sdk-python/` + `cli/`. Total bundle size target: < 5 MB.
- The Rust SDK is published to crates.io as `cyberos-brain-capture = "1.0"`; Python wrapper to PyPI as `cyberos-brain-capture`; both depend on the binary being installed via OCI.
...
- The dedup LRU is per-SDK-instance, not global. Long-lived callers (daemons) keep one BrainCapture instance; short-lived callers (scripts) get fresh dedup each invocation. Broker-side has its own dedup as defense-in-depth.
- `payload_sanitised: true` is honoured by the SDK but the broker still runs FR-BRAIN-111 scrub. Pre-scrub is a performance optimisation, not a security boundary.
- The `kind` regex (`module.event_type`) is shared with FR-AI-003's closed-set list; this skill emits canonical kinds documented there.
- The bash CLI delegates to the binary (`cyberos-brain-capture-main`); the wrapper just parses flags and constructs JSON. This keeps the bash file < 50 lines.
...
- The release sign-bundle.sh is a thin wrapper around FR-SKILL-102's signing pipeline; it doesn't duplicate signing logic, just composes the inputs.
