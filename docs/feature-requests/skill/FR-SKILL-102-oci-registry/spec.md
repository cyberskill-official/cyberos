...
9. **MUST** authenticate via FR-AUTH-004 JWT with `claims.scope_grants` containing `skill:publish` or `skill:pull`. Missing scope → 403.
10. **MUST** include `Idempotency-Key` header support on push (mirrors FR-AUTH-001 §1 #5). Repeat push with same key + same content → return existing manifest; same key + different content → 409 with `idempotency_key_reuse`.
...
- Per-publisher cosign keypair stored in publisher's local secret store (`~/.cyberos/cosign.key`); rotation per FR-AUTH-006-style sweeper.
...
- Idempotency-Key matches FR-AUTH-001 pattern; standard retry-safety.
