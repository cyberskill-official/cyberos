...
source_decisions:
  ...
  - "Layout must match the full-pair convention (feature-request-author/-audit file set) so FR-SKILL-118's parity checker covers it from day one."
...
# FR-SKILL-117: architectural-spike-author / -audit
...
6. Both skills MUST carry acceptance `TRIGGER_TESTS.md` with >= 6 cases each (>= 3 positive, >= 3 negative), following the conventions of the existing trigger-test files (FR-SKILL-112 lineage).
7. File layout MUST match the full-pair convention exactly as listed in `new_files`, so the FR-SKILL-118 parity checker passes over this pair with zero exemptions.
...
7. **Layout parity** (§1 #7) - every file in `new_files` exists; the FR-SKILL-118 parity check (or, before it lands, a manual `ls` against the feature-request pair layout) shows no missing file class.
...
- Structural check for AC 1, 3, 5, 6, 7: `bash tools/cyberos-init/check-chain-coverage.sh` pair rule (from FR-SKILL-116) plus `grep` assertions listed inline in the two TRIGGER_TESTS.md preambles (each acceptance file opens with its own "how to verify this pair" block, executable line by line).

## §6 - Implementation skeleton

Author SKILL.md follows the section order of implementation-plan-author (description, triggers, inputs, artefact spec, pipeline pointer, HALT semantics). RUBRIC.md follows feature-request-audit/RUBRIC.md's family/rule-ID formatting. Envelope JSONs mirror the existing pairs' shape (json-schema-lite objects with required arrays).

## §7 - Dependencies

None upstream (authoring is self-contained). Blocks FR-CUO-209 (the pair joins the vendored set there). FR-SKILL-118's parity checker adopts this pair automatically once both land.

## §8 - Example payloads

```yaml
# spike frontmatter (fixture)
spike_id: SPIKE-FR-MEMORY-130-1
fr_id: FR-MEMORY-130
...
None blocking. Whether ship-feature-requests gains an optional spike step between steps 2 and 3 is FR-CUO-209 territory (workflow change), not this pair's.
...
2. Evidence rot (cited file later deleted) - audit checks citations resolve AT AUDIT TIME; later rot is caught by FR-SKILL-119's anchor checker class, out of scope here.
...
Keep artefact version pinned at @1 in both SKILL.md files; version bumps follow the contracts CHANGELOG discipline. The pair is NOT added to build.sh here - vendoring is FR-CUO-209's single expansion, kept out of this FR to avoid two writers on build.sh:28.
