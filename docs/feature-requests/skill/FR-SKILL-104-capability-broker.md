---
id: FR-SKILL-104
title: "Capability broker — subprocess sandbox enforces allowed_tools + allowed_brain_scopes at invoke time; tool-name allowlist + path-glob allowlist + timeout enforcement"
module: SKILL
priority: MUST
status: accepted
verify: T
...
related_frs: [FR-SKILL-101, FR-SKILL-102, FR-SKILL-103, FR-SKILL-105, FR-BRAIN-101, FR-BRAIN-106, FR-AUTH-003]
...
new_files:
  ...
  - services/skill-broker/src/tools/brain.rs
...
disallowed_tools:
  ...
  - dispatch a BrainRead with path outside allowed_brain_scopes (per §1 #5)
...
sub_tasks:
  ...
  - "1.0h: enforce.rs — pre-dispatch checks: tool in allowed_tools? path in allowed_brain_scopes? request size ≤ 1MB?"
  ...
  - "1.0h: tools/brain.rs — BrainRead + BrainSearch + BrainEmit; path-glob check against allowed_brain_scopes"
  ...
  - "1.5h: broker_e2e_test.rs — invoke skill that calls Read + BrainEmit; verify enforce works; verify timeout"
...
risk_if_skipped: "Without enforcement, FR-SKILL-103's frontmatter is decorative — a skill declares `allowed_tools: [Read]` but can call Bash, BrainEmit, anything. Every skill becomes a potential privilege-escalation vector. Without per-call path-glob check on BRAIN, a skill granted `memories/projects/cyberos/**` can read `memories/people/founders/compensation`. Without timeout, a skill stuck in an infinite loop holds the broker's connection forever. Without sandbox sealing, the skill inherits broker's env vars (potentially containing the JWT signing key) — total auth compromise. This FR is load-bearing; skipping it makes the entire SKILL module security-theatre."
---
...
3. **MUST** enforce frontmatter at every `tool.call`:
    ...
    - For `BrainRead` + `BrainSearch`: every requested `path` MUST match at least one glob in `allowed_brain_scopes`.
...
5. **MUST** enforce `allowed_brain_scopes` path-glob check using `globset` (same crate FR-SKILL-103 uses for parse-time validation). Globs are evaluated against the post-canonicalisation BRAIN path (per AGENTS.md §0.4). The check happens AT EVERY `BrainRead`/`BrainSearch`/`BrainEmit` call — even within the same invocation; this prevents glob bypass via mid-session reconfiguration.
...
7. **MUST** emit BRAIN audit rows for every tool dispatch:
...
8. **MUST** propagate W3C TraceContext (per FR-AI-022) — every tool.call includes a `traceparent` field; the broker injects this into the underlying tool (e.g. `HttpFetch` adds it as a request header; `BrainEmit` writes it into the row payload).
9. **MUST** register the canonical tool set at broker startup. The registry is a static `MCP_TOOL_REGISTRY: BTreeMap<ToolName, Box<dyn Tool>>`. Native tools: `Bash`, `Read`, `Write`, `Edit`, `Glob`, `Grep`, `BrainRead`, `BrainSearch`, `BrainEmit`, `HttpFetch`, `HttpPost`. MCP-server-provided tools are registered dynamically when the broker connects to an MCP server (slice-3+).
...
14. **SHOULD** support `cyberos skill broker replay --invocation <id>` reconstructing a past invocation's tool-call sequence from BRAIN audit rows (slice-3+; placeholder).
...
**Why a broker process at all (§1 #1)?** Without a broker, every skill would need its own tool implementation. The broker centralises: tool implementations (one impl of `Bash`, not N), enforcement (one place to check `allowed_tools`), audit (one place to emit BRAIN rows), observability (one place to attach OTel spans). Skills become thin wrappers around `tool.call` JSON-RPC.
...
- **FD leak**: broker may have an open writer-process FD (per FR-BRAIN-101); skill could write to BRAIN bypassing the broker. `close_fds` prevents.
...
**Why path canonicalisation before glob check (§1 #5)?** A skill could try `BrainRead("memories/projects/cyberos/../people/founders/compensation")` — without canonicalisation, the glob `memories/projects/cyberos/**` might match (depending on glob library); with canonicalisation, the path normalises to `memories/people/founders/compensation` and the glob fails.
...
**Why args_hash, not args (§1 #7)?** Same reason FR-BRAIN-109 hashes Claude Code tool args — secrets in args (Bash commands like `curl -H "Bearer ..."`). Hash for dedup + audit; never store raw.

**Why default-deny everywhere (§1 #3)?** Default-allow is unsafe by construction. A skill that declares no `allowed_brain_scopes` should have no BRAIN access (not "everything"); a skill that declares no `x-allowed-files` should not be able to read arbitrary files. Frontmatter is opt-in for capabilities.
...
**Why per-tool span cascade (§1 #10)?** Aggregated dashboards: "how often does the `Bash` tool fail vs succeed?", "what's the p99 latency of BrainSearch in this skill?" Per-call spans roll up into per-tool histograms.
...
// Response: denied (enforcement violation)
{
  ...
  "error": {
    ...
    "data":    {"reason": "tool_not_in_allowed_tools", "tool": "Bash", "skill_id": "brain-capture"}
  }
}
```
...
pub enum EnforceError {
    ...
    #[error("BRAIN scope violation: path {path:?} not in allowed_brain_scopes")]
...
impl Enforcer {
    pub fn from_frontmatter(fm: &SkillFrontmatter) -> anyhow::Result<Self> {
        ...
        let mut brain_b = globset::GlobSetBuilder::new();
        for p in &fm.allowed_brain_scopes { brain_b.add(globset::Glob::new(p)?); }
        let brain_scope_globs = brain_b.build()?;
        ...
        Ok(Self { allowed_tools, disallowed_tools, brain_scope_globs, file_globs, domain_globs })
    }
    ...
    pub fn check_brain_path(&self, path: &str) -> Result<(), EnforceError> {
        // Path canonicalisation: handle `..` segments
        let canonical = canonicalise_brain_path(path);
        if !self.brain_scope_globs.is_match(&canonical) {
...
fn canonicalise_brain_path(path: &str) -> String {
...
pub struct Dispatcher {
    ...
    pub brain_writer: Arc<cyberos_brain_writer::BrainWriter>,
}
...
impl Dispatcher {
        ...
        match tool {
            "BrainRead" | "BrainSearch" | "BrainEmit" => {
                if let Some(p) = args.get("path").and_then(|v| v.as_str()) {
                    self.enforcer.check_brain_path(p)?;
                }
            }
            "Read" | "Write" | "Edit" => {
                if let Some(p) = args.get("path").and_then(|v| v.as_str()) {
    ...
    // emit_* methods elided; they write canonical::skill_* audit rows via brain_writer
}
```
...
1. **Tool call succeeds for allowed tool** — skill with `allowed_tools: [BrainRead]` calls `tool.call({tool: "BrainRead", args: {path: "memories/projects/cyberos/notes/x.md"}})` → success response.
...
4. **BRAIN scope check works** — skill with `allowed_brain_scopes: ["memories/projects/cyberos/**"]` reads `memories/projects/cyberos/x.md` → ok; reads `memories/people/y.md` → `scope_violation`.
...
19. **W3C trace propagation: BrainEmit** — emitted BRAIN row's payload.trace_id matches.
...
#[test]
fn brain_scope_glob_match() {
    let mut fm = SkillFrontmatter::test_empty();
    fm.allowed_brain_scopes = vec!["memories/projects/cyberos/**".into()];
    let e = Enforcer::from_frontmatter(&fm).unwrap();
    assert!(e.check_brain_path("memories/projects/cyberos/notes/x.md").is_ok());
    assert!(matches!(e.check_brain_path("memories/people/y.md"), Err(EnforceError::ScopeViolation { .. })));
}
...
fn path_traversal_caught() {
    ...
    fm.allowed_brain_scopes = vec!["memories/projects/cyberos/**".into()];
    let e = Enforcer::from_frontmatter(&fm).unwrap();
    let res = e.check_brain_path("memories/projects/cyberos/../people/y.md");
...
// services/skill-broker/tests/broker_e2e_test.rs
#[tokio::test]
async fn dispatch_brainread_succeeds() {
    let env = BrokerTestEnv::with_skill("test-read", &["BrainRead"], &["memories/projects/cyberos/**"]);
    let client = env.client().await;
    let resp = client.call("BrainRead", json!({"path": "memories/projects/cyberos/x.md"})).await.unwrap();
    assert!(resp["body"].is_string());
}

#[tokio::test]
async fn dispatch_bash_denied() {
    let env = BrokerTestEnv::with_skill("test-no-bash", &["BrainRead"], &[]);
    let client = env.client().await;
    let resp = client.call("Bash", json!({"cmd": "ls"})).await;
    assert!(resp.unwrap_err().to_string().contains("tool_not_in_allowed_tools"));
    let row = env.brain.latest("skill.tool_denied").await;
...
async fn timeout_enforced() {
    ...
    let timeout_row = env.brain.latest("skill.timeout").await;
...
- **FR-SKILL-101 (upstream)** — BRAIN integration (pre/post audit rows pattern).
- **FR-SKILL-102 (related)** — OCI registry distributes signed bundles; broker reads them.
- **FR-SKILL-105 (downstream)** — brain-capture@1 skill is the first canonical user.
- **FR-BRAIN-101** — BrainWriter (used by broker to emit audit rows).
- **FR-BRAIN-106** — sync_class semantics.
- **FR-AUTH-003** — RLS on BrainRead enforces tenant_id scope.
...
- Replay tool from BRAIN audit rows — slice 3+.
...
| BrainWriter unavailable | emit Err | Audit row dropped (logged); tool call still completes | Operator restores writer |
...
| Long-running tool (BrainSearch with 100k results) | timeout enforcement | SIGTERM at 90%; if tool yields response in grace, emit completed | Operator considers smaller queries |
...
| Audit row write fails mid-dispatch | brain_writer Err | Row queued; emit in background; tool call still completes | Operator restores BRAIN |
...
- The Unix socket pattern means broker process must stay alive for the duration of invocation; it owns the socket file. On broker crash, the file is orphaned; FR-BRAIN-110's sweeper cleans `/tmp/cyberos-skill-broker.*.sock` files older than 5 minutes.
...
- `skill.tool_denied` rows include `attempted_args_hash` (NOT raw args) for the same reason FR-BRAIN-109 hashes Claude Code tool args: args may contain secrets.
- `cyberos skill broker tail` is implemented as a BRAIN query subscription (the audit rows arrive in real-time); avoids needing a separate IPC channel.
