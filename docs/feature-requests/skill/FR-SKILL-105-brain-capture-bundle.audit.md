...
Global dedup needs Redis/IPC; per-instance is simpler. Resolved: §1 #7 + §11 note per-instance with 60s TTL + LRU(1000); broker-side dedup is the defense-in-depth (FR-BRAIN-107).

### ISS-003 — PII scrub: SDK-side vs broker-side
Both adds latency; only one risks gaps. Resolved: §1 #8 both — SDK pre-scrub for performance; broker FR-BRAIN-111 as authoritative. `payload_sanitised=true` flag lets callers skip SDK-side scrub when payload is known-clean.
