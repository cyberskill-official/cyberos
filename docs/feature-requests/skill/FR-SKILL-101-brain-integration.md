---
id: FR-SKILL-101
title: "Skill BRAIN integration — skill.invoked_started + skill.invoked_completed audit rows (skill.* namespace) + args_hash + trace_id propagation + panic-recovery"
module: SKILL
priority: MUST
status: accepted
verify: T
...
source_pages:
  - website/docs/modules/skill.html#brain-integration
source_decisions:
...
new_files:
  ...
  - services/skill-host/src/brain_bridge.rs
  - services/skill-host/src/invocation_context.rs
  - services/skill-host/src/audit_canonical.rs
  - services/skill-host/tests/brain_audit_test.rs
  - services/skill-host/tests/brain_audit_panic_test.rs
  - services/skill-host/tests/brain_audit_concurrent_test.rs
  - services/skill-host/tests/brain_audit_trace_propagation_test.rs
modified_files:
  - services/skill-host/src/supervisor.rs                  # call invoke_with_audit
allowed_tools:
  - file_read: services/skill-host/**
  - file_write: services/skill-host/**
  - bash: cd services/skill-host && cargo test brain_audit
disallowed_tools:
  - emit raw args in BRAIN row (per §1 #3 — args_hash only)
  - skip start OR completed row (per §1 #1 + #2 — both required)
  - silent panic (per §1 #5 — panic emits completed row)
  - bypass FR-AI-003 brain_writer (per §1 #6)

effort_hours: 6
sub_tasks:
  - "0.5h: brain_bridge.rs — invoke_with_audit wrapper"
...
The SKILL host **MUST** emit BRAIN audit rows before AND after every skill invocation. Each invocation:
...
4. **MUST** propagate `trace_id` from caller's OTel context (FR-AI-022). Both audit rows carry the same `trace_id`; investigators can join LLM trace + skill trace + audit row in BRAIN.
5. **MUST** use FR-AI-003 `brain_writer::emit()` for both rows. Failure to emit `_started` → skill NOT dispatched (caller gets 503). Failure to emit `_completed` → log sev-1 + DO NOT reverse skill effect (skills may have side effects; reversing isn't generally possible).
...
8. **MUST** support concurrent invocations — different skill calls use independent BRAIN write paths; no serialised lock around emit.
...
**Why _started fails block dispatch (§1 #5)?** Without auditable record, skill side effects are invisible. Refusing dispatch when audit can't record preserves auditability invariant. The trade-off is unavailability during BRAIN outages — acceptable because audit chain integrity is non-negotiable.
...
**Why concurrent invocations independent (§1 #8)?** Serialised lock around brain_writer would bottleneck high-throughput skill invocation. Independent emit paths allow parallel skills to audit independently.
...
pub async fn invoke_with_audit(
    ...
    brain_writer::emit(start_row).await
    ...
    if let Err(e) = brain_writer::emit(completed_row).await {
...
1. Skill invocation emits 2 BRAIN rows (started + completed).
...
8. Compensation-related skills (e.g., cuo.cfo.payroll-draft) emit rows even though body excluded from BRAIN.
...
async fn skill_invocation_emits_both_rows() {
    ...
    let started = brain_test_helper::find_latest("skill.invoked_started").unwrap();
    let completed = brain_test_helper::find_latest("skill.invoked_completed").unwrap();
...
async fn args_hash_matches_canonical_sha256() {
    ...
    let row = brain_test_helper::find_latest("skill.invoked_started").unwrap();
...
async fn 100_concurrent_invocations_produce_200_rows() {
    ...
    let started = brain_test_helper::count_rows_since("skill.invoked_started", recent()).await;
    let completed = brain_test_helper::count_rows_since("skill.invoked_completed", recent()).await;
...
async fn completed_emit_failure_logs_sev1_no_revert() {
    brain_test_helper::inject_emit_failure_for_kind("skill.invoked_completed");
...
- **FR-AI-003** — brain_writer::emit() bridge.
...
|---|---|---|---|
| BRAIN unreachable on _started | brain_writer error | Invocation refused; caller 503 | Operator investigates BRAIN |
| BRAIN unreachable on _completed | brain_writer error | Sev-1 log; skill effect not reversed | Operator investigates |
| BRAIN slow on _started (>50ms) | latency histogram | Invocation proceeds (latency-tolerated); sev-3 alarm | Investigate brain_writer |
...
| Audit emit during shutdown | brain_writer in-flight | Tx may abort | By design (not catastrophic) |
...
- Concurrent invocations independent — no lock serialisation around brain_writer.
- Compensation-skill bodies excluded from BRAIN (DEC-036) but invocation rows still emitted (the FACT of the call is auditable).
