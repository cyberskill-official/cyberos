...
allowed_tools:
  ...
  - file_write: modules/skill/**, tools/sweep-placeholders/**, modules/cuo/{cuo,tests}/**, docs/feature-requests/skill/**
...
disallowed_tools:
  ...
  - escape brackets via the FR-SKILL-113 migrate.sh path (that's the wrong fix — these are placeholders that need REAL values, not escape-encoding)
  - touch body XML form `<untrusted_content source="...">…</untrusted_content>` (body XML is the runtime wrapper per FR-SKILL-113; never touched)
...
8. The body XML form `<untrusted_content source="..." page="...">…</untrusted_content>` **MUST NOT** be touched anywhere. SKB-030 fires only on frontmatter values; body markup is preserved per FR-SKILL-113 §1 #3.
...
15. Registry version: this FR is a v0.2.6 increment (post-v0.2.5 introduced by FR-SKILL-113). The bump is documented in the repo-root `CHANGELOG.md` `[SKILL]` section.

## §2 — Why this design (rationale for humans)

**Why a separate FR rather than fold into FR-SKILL-113 (§1 #1)?** FR-SKILL-113 specifically migrated the `wrap_in` field via a mechanical pattern match. The stale placeholders span DOZENS of distinct fields with NO common pattern. Each one needs operator domain knowledge to substitute correctly. Bundling the work into 113 would have either inflated 113 from 12h to 28h (wrong scope grouping) or shipped 113 incomplete (leaving 134 portability bugs). Separating the FRs honours the audit-fix-audit discipline — 113 closed one specific bug class; 115 closes the residual.
...
**Why body XML preserved (§1 #8)?** Restates FR-SKILL-113's invariant. The body XML is the runtime wrapper; the frontmatter is the declaration. Touching body XML would break wrapping semantics for every production skill that processes external bytes (i.e. all of them).
...
4. **suggest.py proposes a stage letter** — for `feature-request-author` (body references SDP §2(b) Requirements), suggest_for_metadata_stage returns `'b'`.
5. **suggest.py reads CONTRACT_ECHO for artefact type** — for `feature-request-author`, the description's `<input>` → `feature_request source`.
...
20. **Cross-FR reciprocity** — FR-SKILL-113's `blocks:` list updated to include FR-SKILL-115; reciprocity sweep passes.
...
**Depends on:**
- **FR-SKILL-113** (XML-free frontmatter) — provides the `wrap_in_marker:` rename so the detector can correctly distinguish the now-fixed marker from stale placeholders.

**Blocks:** none (independent of FR-SKILL-114; orthogonal to FR-SKILL-111 + 112).

**Related:** all of FR-SKILL-111 / 112 / 113 / 114 — together they form the v0.2.5/v0.2.6 portability + foundation-discipline bundle.
...
description:     "Frontmatter field metadata.stage contains stale template placeholder syntax. Per FR-SKILL-115 §1 #1, every production skill must carry concrete substituted values."
...
| Registry version bump conflicts with concurrent FR-SKILL-117 marker namespace work | Operator coordinates timing | Version bump skipped | Bump on FR-SKILL-117 commit instead |
...
- **Why dependency on FR-SKILL-113 instead of independent?** FR-SKILL-113 introduced `wrap_in_marker:` — without that rename in place, the detector might confuse the OLD `wrap_in: <untrusted_content/>` form (which 113 already migrated) with stale placeholders. FR-SKILL-113 first; then FR-SKILL-115 picks up the residual.
