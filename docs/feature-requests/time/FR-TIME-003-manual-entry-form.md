...
build_envelope:
  ...
  disallowed_tools:
    ...
    - bypass FR-TIME-007 cap (per DEC-1402)
...
sub_tasks:
  ...
  - "1.1h: integration smoke + cross-FR test with FR-TIME-007"
...
2. **MUST** expose `POST /v1/time/entries/manual` body `{ engagement_id, project_id?, task_id?, entry_date, duration_seconds, description, reason, approval_override? }`. Handler:
   ...
   - FR-TIME-007 OT cap check per §1 #5.
   - Creates TIME entry via FR-TIME-001.
...
6. **MUST** chain into FR-TIME-007 OT cap check per DEC-1402. After per-day cap check, invoke `vn_labour::cap_check` for vn-1 Members. Breach → 412 + matching breach kind.
...
11. **MUST NOT** bypass FR-TIME-007 OT cap (per DEC-1402).
...
**Why chain into FR-TIME-007 (§1 #6, DEC-1402)?** Manual entry must respect all the same labour-law constraints as timer entries; otherwise it's the bypass path.
...
| Bulk import bypass | handler enforces same checks | Tested via FR-TIME-007 §11.8 lint pattern | CI catches |
...
**§11.3** Per-day cap check uses FR-TIME-007's aggregator (shared infrastructure).

**§11.4** FR-TIME-007 chain invoked AFTER per-day cap (most-granular first).
