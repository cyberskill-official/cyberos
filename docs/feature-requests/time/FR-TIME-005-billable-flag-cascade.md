...
18. **Audit sampled correctly** — `billable_resolved` at 1% per FR-OBS-006 tail.
...
| Audit row sampling drops critical event | FR-OBS-006 tail | Entry-override always emitted (not sampled) per AC #18 | Inherent |
...
**§11.4** Audit sampling at 1% via FR-OBS-006 except entry_override always emitted (Member intent).
