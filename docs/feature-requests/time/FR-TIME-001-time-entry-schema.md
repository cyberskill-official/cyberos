...
source_decisions:
  ...
  - DEC-223 (billable flag is materialised on the row but COMPUTED by FR-TIME-005 — this FR declares the column as `billable BOOLEAN NOT NULL DEFAULT false` with no constraint until FR-TIME-005 ships the cascade)
...
disallowed_tools:
  ...
  - hard-code billable cascade logic in this FR — that is FR-TIME-005's responsibility (per DEC-223)
  - enforce VN Labour Code OT caps at this FR — that is FR-TIME-007's responsibility (per spec scope)
...
1. **MUST** define the `time_entries` table with the following columns (full DDL in §3.1):
    ...
    - `engagement_id UUID NOT NULL` — the engagement billable target (per FR-PROJ-005; placeholder FK at slice 1).
    - `issue_id UUID NOT NULL` — the specific issue (per FR-PROJ-001).
    ...
    - `entry_status entry_status NOT NULL DEFAULT 'draft'` — closed enum: `draft | submitted | approved | reverted` (FR-TIME-006 transitions).
    - `billable BOOLEAN NOT NULL DEFAULT false` — set by FR-TIME-005's cascade; this FR declares the column only (per DEC-223).
    - `rate_card_snapshot JSONB` — populated at entry creation by FR-TIME-005; snapshot of the engagement rate-card at the row's instant (per DEC-224); empty `{}` at slice 1.
...
16. **MUST** support idempotent creation via `Idempotency-Key` header (same semantics as FR-AUTH-002 §1 #6).
...
21. **MUST** maintain the **rate_card_snapshot pattern** (per DEC-224). When FR-TIME-005 ships, it populates `rate_card_snapshot` at row creation with the engagement's then-current rate card. Mutations to the engagement's rate card NEVER alter past `rate_card_snapshot` values — the snapshot is frozen at the row's instant. Slice 1 ships the column with `{}` default; FR-TIME-005 fills.
...
25. **MUST** treat `created_by_subject_id` as the **actor** (who created the row), which MAY differ from `member_subject_id` (whose work is being recorded). For self-entry both are equal; for AM-on-behalf-of entry (FR-TIME-003 manual form) they differ. Both fields are immutable per row.
...
**Why entry_kind closed at 4 values (§1 #3)?** Working-time classification under VN Labour Code is `regular | overtime | weekend | holiday`. Each kind has different statutory rate multipliers (FR-REW-004 ships). Allowing a 5th (e.g. `night_shift`) is an ADR — the rate-multiplier table would need to extend. Closed enum prevents drift.
...
**Why slice-1 ships billable=false default (DEC-223)?** The billable cascade (FR-TIME-005) is a non-trivial 4-step decision involving the engagement's non-billable categories, the rate card's role default, and member overrides. Splitting it to its own FR keeps this schema FR focused on integrity. Default `false` is conservative — if the cascade fails to set the flag, the entry is treated as non-billable (no invoice line) rather than mis-billed. FR-TIME-005's tests assert the default is replaced on every entry.

**Why entry_currency on the row (DEC-229)?** Multi-currency tenants run engagements in VND, USD, SGD, etc. The entry's currency is the engagement's invoice currency at the row's instant. Snapshotting prevents the engagement's currency switch (rare but possible during contract renegotiation) from retroactively converting past entries. Invoice math is FR-INV-001's concern; this FR just preserves the source-of-truth currency.

**Why created_by_subject_id distinct from member_subject_id (§1 #25)?** Most entries are self-logged (member = creator). Some entries are AM-on-behalf-of (manual entry on someone else's behalf with their confirmation — FR-TIME-003 ships that flow). Both fields capture different facts: "whose work this records" vs "who pressed the button." Audit trails need both.
...
25. **rate_card_snapshot defaults to `{}`** — slice 1 default until FR-TIME-005 fills.
...
**Upstream:**
- **FR-AUTH-003** — RLS enforcement; same `current_setting('auth.tenant_id')` pattern.
- **FR-AUTH-101** — RBAC catalogue; `Resource::TimeEntry + Action::Write/Read` matrix entry.

**Downstream (8 placeholders):**
- **FR-TIME-002** — timer start/stop UI (creates entries via this FR's API).
- **FR-TIME-003** — manual entry form + VN Labour Code cap validation.
- **FR-TIME-005** — billable cascade (populates `billable` + `rate_card_snapshot`).
- **FR-TIME-006** — weekly approval flow (transitions `entry_status`).
- **FR-TIME-007** — VN Labour Code Art. 107 OT cap hard-block at write.
- **FR-TIME-009** — per-cycle billable rollup → INV.
- **FR-HR-008** — performance signal aggregator (read-only consumer).
...
```json
{
  ...
  "entry_kind": "regular",
  "entry_currency": "VND",
  "description": "Worked on FR-AUTH-101 spec review"
}
```
...
```json
{
  ...
  "billable": false,
  "entry_currency": "VND",
  "description_scrubbed": "Worked on FR-AUTH-101 spec review",
  "ts_start": "2026-05-16T09:00:00Z",
...
- **Holiday-list driving entry_kind defaulting** — FR-TIME-007 ships the holiday table.
- **VN Labour Code OT cap enforcement** — FR-TIME-007 (this FR allows the kind = overtime; the cap is FR-TIME-007's gate).
- **Billable cascade computation** — FR-TIME-005.
- **Approval flow transitions** — FR-TIME-006.
- **Per-cycle rollup emit to INV** — FR-TIME-009.
- **Time-entry edit UI** — FR-TIME-002 + FR-TIME-003.
...
| `billable` field set in create request | Handler ignores (slice 1; cascade sets) | Default false applied | FR-TIME-005 takes over |
...
- **PROJ issue FK is a logical foreign key** — declared in §1 #1 but not enforced as SQL FK at slice 1 (cross-service FK is operationally complex). FR-PROJ-001's data integrity is trusted; if an issue is deleted, time entries pointing at it become orphaned (still queryable, just decorative).
- **`engagement_id` similarly soft-FK** — same reason; FR-PROJ-005 owns the engagements table.
