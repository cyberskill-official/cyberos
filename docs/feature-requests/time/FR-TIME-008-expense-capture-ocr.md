...
source_decisions:
  - DEC-1450 2026-05-17 — Expense capture flow: Member uploads photo via mobile/web → S3 storage via FR-DOC-001 → AWS Textract OCR → hóa đơn parser extracts VN-specific fields (MST, total VND, date) OR generic receipt fields → Member confirms/edits → expense persisted + (optionally) attached as invoice line
  ...
  - DEC-1453 2026-05-17 — Member confirmation MANDATORY before persistence (per FR-MCP-008 elicitation pattern; AI-extracted data needs human verification)
...
build_envelope:
  ...
  disallowed_tools:
    ...
    - bill expense to client without engagement billable flag (per FR-INV-001 integration)

effort_hours: 8
sub_tasks:
  - "0.4h: 0009_expenses.sql + 0010_expense_policies.sql + 2 closed enums"
  - "0.4h: expense/mod.rs"
  - "0.5h: upload.rs (S3 presigned via FR-DOC-001)"
  - "0.6h: ocr_dispatcher.rs (FR-MCP-007 task)"
...
5. **MUST** expose `POST /v1/time/expenses/upload` body `{ engagement_id, kind, expected_currency }`. Handler:
   - Returns FR-DOC-001 presigned S3 URL.
   - Creates expense row status='pending_ocr'.
   - Emits `time.expense_captured` sev-3.

6. **MUST** trigger async OCR via FR-MCP-007 Tasks on photo-upload-complete webhook from S3:
   ...
   - Notifies Member (push via FR-PORTAL-007 PWA push).
...
12. **MUST** support invoice attach `POST /v1/time/expenses/{id}/attach-to-invoice` body `{ invoice_id }` per DEC-1457 derivative. Caller has `cfo` or `engagement_admin`. Handler:
    ...
    - Creates invoice_line row (line_kind='expense_reimbursement') via FR-INV-001.
...
**Why FR-MCP-007 async (§1 #6, DEC-1455)?** Textract analyse is 5-30s; sync would tie up gateway workers. Tasks primitive fits.
...
- **Deferred:** Foreign-currency auto-convert at upload — slice 3 (FR-INV-002 derivative).
...
| OCR task lost (FR-MCP-007 crash) | task retry | Re-run | Inherent |
...
| Expense rejected after invoice attach | state check | 409 — already invoiced | Use correction_to via FR-INV-001 |
| FR-PORTAL-007 push not sent | best-effort | Email fallback via FR-EMAIL-001 | Inherent |
...
**§11.7** Invoice attach delegates to FR-INV-001 line-add endpoint.

**§11.8** Push notification via FR-PORTAL-007 with payload `{ kind: "ocr_complete", expense_id }`.
