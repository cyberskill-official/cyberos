...
source_decisions:
  - DEC-1430 2026-05-17 — Per-cycle rollup runs at end of billing cycle (engagement-configured: monthly default); aggregates approved billable entries per (Member, role-on-engagement, project, task); produces JSON payload consumed by FR-INV-001 draft creation
...
sub_tasks:
  ...
  - "0.6h: integration with FR-INV-001 draft creation"

risk_if_skipped: "Without rollup, FR-INV-001 has no way to convert TIME entries into invoice lines → entire billing pipeline broken. Without DEC-1431 idempotency, double-billing on retry. Without DEC-1432 status filter, draft entries leak to invoices. Without DEC-1433 rate snapshot, FR-INV-001's rate-card snapshot DEC-1363 broken. The 6h effort completes the TIME→INV pipeline."
---
...
6. **MUST** return result shape consumable by FR-INV-001:
   ```json
   {
     "engagement_id": "...",
...
**Why rate-card snapshot in result (§1 #3, DEC-1433)?** FR-INV-001 needs the snapshot to fulfill DEC-1363. This FR provides it ready-to-copy.
...
14. **Long-running rollup → task** — > 30s engagement uses FR-MCP-007 pattern (slice 2 enhancement).
...
20. **JSON shape stable** — FR-INV-001 consumer tested against fixture.
...
- **Deferred:** Async via FR-MCP-007 Tasks for huge engagements — slice 2.
...
**§11.5** FR-INV-001 invokes rollup synchronously then creates invoice with result.
...
**§11.9** Trace_id propagated from FR-INV-001 caller through rollup to FR-INV-001 invoice row.
