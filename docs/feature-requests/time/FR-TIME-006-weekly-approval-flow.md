...
source_decisions:
  ...
  - DEC-1422 2026-05-17 — Locked once approved — no further mutation (corrections via FR-TIME-001 correction_to pattern only)
...
5. **MUST** expose Member submit `POST /v1/time/timesheets/{id}/submit`. Handler:
   ...
   - Triggers email to AM via FR-EMAIL-001.
...
11. **MUST** block entry writes for locked weeks per DEC-1422. FR-TIME-001 entry/create.rs modified to check `timesheets.status` for the week; locked → 412 + `week_locked`.

12. **MUST** trigger emails per DEC-1426 via FR-EMAIL-001:
...
**Why immediate auto-lock on approve (§1 #6, DEC-1422)?** Approved = ready-for-invoice. Lock prevents drift. Corrections via correction_to pattern (FR-TIME-001 derivative).
...
17. **Approved correction via correction_to** — works (FR-TIME-001 path).
...
| Email send fails | FR-EMAIL-001 retry | Audit logged; user notified | Inherent retry |
...
| Locked week with pending corrections | correction_to path | Allowed; preserves audit | FR-TIME-001 derivative |
...
| Email rate-limit hit | FR-EMAIL-001 queue | Delivery delayed | Inherent retry |
...
**§11.6** Email templates use FR-PORTAL-002 brand pack overrides for tenant theming.

**§11.7** Locked-week entry block at FR-TIME-001 create.rs entry-point.
