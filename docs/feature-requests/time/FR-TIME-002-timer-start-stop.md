...
build_envelope:
  ...
  modified_files:
    ...
    - services/auth/src/handlers/logout.rs                            # invoke timer commit on logout
...
7. **MUST** expose `POST /v1/time/timer/stop` body `{ timer_id, final_description? }`. Handler:
   ...
   - Creates TIME entry via FR-TIME-001 with snapped duration.
...
11. **MUST** auto-commit on logout per DEC-1384. FR-AUTH-004 logout handler invokes `time::timer::commit_all_for_subject(subject_id)`:
...
| Long-running timer (24h+) | absolute cap | Hard-cap at 24h via VN Labour Code (FR-TIME-007 derivative) | Member splits across days |
| Engagement permission lost mid-timer | RLS at commit | Commit may fail; sev-2 alert | Manual entry via FR-TIME-003 |
...
**§11.3** Logout handler at FR-AUTH-004 invokes `services/time/src/timer/commit.rs::commit_all_for_subject` synchronously before returning.
