...
4. **MUST** cascade on executed per DEC-1874 at `cascade_executor.rs::execute(termination)`:
   - FR-ESOP-005 vesting halt + forfeiture per GL/BL
   - FR-AUTH-101 deprovision (all roles revoked)
   - FR-PORTAL-008 offer DSAR export to ex-member
   - FR-PROJ-013 reassign open issues to manager
