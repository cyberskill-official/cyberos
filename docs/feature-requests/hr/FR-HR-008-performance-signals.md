...
3. **MUST** aggregate at `signal_aggregator.rs::aggregate(member, period)`:
   - proj_issue_velocity: count issues closed / period (FR-PROJ-013 read)
   - time_utilization_pct: billable_hours / total_hours (FR-TIME-001 read)
   - learn_completion_rate: courses_completed / assigned (FR-LEARN-001 read)
   - project_satisfaction_avg: avg from PROJ retro feedback
   - ot_burnout_flag: TRUE if OT hours > 80% of cap last 3 months (FR-TIME-007)

4. **MUST** run monthly cron at EOM per DEC-1862 via FR-MCP-007.
...
| FR-TIME-007 burnout calc fail | flag=false default | sev-2 | data fix |
