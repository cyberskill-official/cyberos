...
risk_if_skipped: "Without version-pinned policy, FR-TIME-007 OT enforcement + FR-REW-004 deductions drift over time → audit failures. Without DEC-1842 immutability, prior payslip recompute breaks (different rate replay). Without DEC-1843 statutory-vs-override distinction, tenants illegally lower SI rates."
---
...
3. **MUST** lookup at `loader.rs::get(tenant_id, kind, effective_at)`:
   ...
   - Determinism: same params → same result (critical for FR-REW-004 replay).
...
**Why version-pinned (DEC-1842)?** FR-REW-004 must reproduce historical payslips deterministically; mutable rates break replay.
