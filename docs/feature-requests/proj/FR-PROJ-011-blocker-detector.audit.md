...
Per AUTHORING.md §0 master rule: spec is now perfect — depth bounded by the genuine surface (parser × classification × auto-resolve × dwell × escalation × snooze × cycle × mention tracking × cross-engagement × PII), not by line targets.
