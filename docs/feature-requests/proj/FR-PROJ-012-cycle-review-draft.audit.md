...
Per feature-request-audit skill §0 master rule: spec is now perfect — depth bounded by the genuine surface (COO compose × stats × LLM × locale × persona × revisions × comparison × redaction × skip × learnings), not by line targets.
