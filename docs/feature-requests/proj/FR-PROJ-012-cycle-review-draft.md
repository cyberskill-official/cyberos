...
disallowed_tools:
  ...
  - call LLM with raw PII (use FR-MEMORY-111 redaction first)
...
7. **MUST** redact PII in input context via FR-MEMORY-111 before LLM call (issue descriptions may contain customer info; redact before composition).
...
18. **MUST** preserve all prior draft revisions as memory memory revisions (FR-MEMORY-101 versioning); the latest is the canonical draft; operators can revert to a prior revision.
...
**Why save as memory memory (DEC-332)?** Reviews are knowledge — searchable, citable, referenced in future cycle planning. Saving to memory with `sync_class: shareable` makes them findable via FR-MEMORY-108 search.
...
**Why redact before LLM (§1 #7)?** External LLM provider (Anthropic/OpenAI) shouldn't see customer PII. FR-MEMORY-111 redaction is the boundary. ZDR contract (FR-AI-015) ensures even after-redaction data isn't retained.
...
- `redact_titles` calls FR-MEMORY-111's `scan_and_redact` per issue title; results cached for the cycle.
- `compute_status_dwell_medians` queries `issue_status_history` (FR-PROJ-004) for transitions within cycle period.
- Force-regenerate creates a new memory revision via FR-MEMORY-101's content-addressed write; old revisions preserved by memory's natural append-only chain.
...
- LLM response redaction runs the same FR-MEMORY-111 ruleset; same redaction tokens as input.
