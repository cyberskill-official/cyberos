---
id: FR-PROJ-012
title: "Cycle-review draft generator — CUO/COO-persona LLM compose at cycle close with completion stats, blocker recap, and editable BRAIN draft"
module: PROJ
priority: MUST
status: accepted
verify: T
...
source_decisions:
  ...
  - DEC-331 (draft generated by COO persona via FR-AI-014; never auto-published; operator edits + accepts)
  - DEC-332 (review draft IS a BRAIN memory in shareable sync_class — team artifact)
...
modified_files:
  - services/proj-sync/src/main.rs                    # spawn cycle-close scheduler
allowed_tools:
  - file_read: services/proj-sync/**
  - file_write: services/proj-sync/{src,tests}/**
  - bash: cd services/proj-sync && cargo test cycle_review
disallowed_tools:
  - auto-publish review to BRAIN as final (per DEC-331 — draft state only)
  - call LLM with raw PII (use FR-BRAIN-111 redaction first)
...
sub_tasks:
  ...
  - "0.5h: emit as BRAIN memory at memories/projects/<eng>/cycle-reviews/<cycle_id>.md (draft)"
  - "0.5h: BRAIN audit row 'proj.cycle_review_drafted'"
...
2. **MUST** compute `CycleStats` for each detected cycle:
    ...
    - `cancelled_count`.
    - `velocity_estimate_points` (sum of estimate values on completed issues; per FR-PROJ-013 estimate field).
    - `velocity_actual_hours` (sum of FR-TIME-005 billable_hours on completed issues).
    - `blocker_count_resolved` (FR-PROJ-011 blockers resolved during cycle).
    - `blocker_count_stale_at_close` (active blockers at cycle end).
    - `time_in_status_p50` per status (median dwell).
3. **MUST** compose a markdown draft via FR-AI-014 COO persona using a fixed prompt template:
    ```
...
13. **MUST** include FR-PROJ-013 estimate-vs-actual delta in stats: `velocity_estimate_vs_actual_ratio` (estimate_points / actual_hours). Operators see whether estimates were optimistic / pessimistic.
...
15. **MUST** include FR-PROJ-011 stale-blocker recap by category: e.g. "3 blockers stale on customer feedback, 2 on design review." Helps operator identify systemic patterns.
...
18. **MUST** preserve all prior draft revisions as BRAIN memory revisions (FR-BRAIN-101 versioning); the latest is the canonical draft; operators can revert to a prior revision.
...
20. **MUST** redact LLM response too (not just input) before BRAIN save — LLM may echo or hallucinate PII from training data.
...
22. **MUST** include cycle's `learnings` section if the cycle had `learnings` field populated during the cycle (FR-PROJ-014 kanban allows operator to add running learnings); concatenated into the prompt input.
...
**Why save as BRAIN memory (DEC-332)?** Reviews are knowledge — searchable, citable, referenced in future cycle planning. Saving to BRAIN with `sync_class: shareable` makes them findable via FR-BRAIN-108 search.
...
**Why redact before LLM (§1 #7)?** External LLM provider (Anthropic/OpenAI) shouldn't see customer PII. FR-BRAIN-111 redaction is the boundary. ZDR contract (FR-AI-015) ensures even after-redaction data isn't retained.

**Why locale-aware (§1 #12)?** VN-team reviews in Vietnamese, English-team in English. Wrong locale = friction.

**Why estimate-vs-actual ratio (§1 #13)?** Operators tracking estimate accuracy need the ratio explicitly; baseline observation for FR-PROJ-013 calibration.
...
"#, cycle.id, cycle.engagement_id, persona_version,
    ...
    brain_writer::put_memory(&memory_path, full.as_bytes()).await?;
    Ok(memory_path)
}
```
...
5. **Blocker counts split active vs resolved** — accurate per FR-PROJ-011 state.
...
9. **BRAIN audit `cycle_review_drafted`** — row appears post-save.
...
24. **All revisions preserved** — 3 re-generations → 3 BRAIN memory revisions; latest = canonical (AC for §1 #18).
25. **Comparison to prior cycle in prompt** — prior accepted cycle stats included if available (AC for §1 #19).
26. **LLM response redacted** — fixture LLM output containing PII → BRAIN memory has redacted form (AC for §1 #20).
...
async fn cycle_close_generates_draft() {
    ...
    let mem = env.brain.read_memory(&format!(
...
async fn llm_failure_yields_stub() {
    ...
    let cycle = env.create_cycle().await;
    env.run_close_detector().await;
    let mem = env.brain.read_memory_for_cycle(cycle.id).await.unwrap();
    assert!(mem.contains("status: draft"));
...
async fn force_regenerate() {
    ...
    let revisions = env.brain.memory_revisions_for_cycle(cycle.id).await;
...
- **FR-PROJ-001** — cycles + issues tables.
- **FR-PROJ-004** — status FSM (Done detection).
- **FR-PROJ-011** — blocker_state for blocker counts.
- **FR-AI-014** — persona-stamped LLM calls.
- **FR-AI-015** — ZDR enforcement on the external LLM call.
- **FR-BRAIN-111** — PII redaction.
- **FR-CUO-101 (placeholder)** — future routing of draft notifications.
...
| BRAIN put_memory fails | sqlx Err | draft body NOT saved; sev-2 | Operator retries via force endpoint |
...
| Memory path collision (multiple drafts) | BRAIN handles revisions | All preserved | None |
...
| Revision count > 50 | BRAIN handles; UI paginates | None | None |
...
- The COO persona version (`coo@1.0.0`) is fetched from FR-AI-014 registry at compose time; stamped in frontmatter for reproducibility.
- The stats-only stub on LLM failure carries the JSON of CycleStats verbatim — operator can read and write the prose.
- `redact_titles` calls FR-BRAIN-111's `scan_and_redact` per issue title; results cached for the cycle.
- `compute_status_dwell_medians` queries `issue_status_history` (FR-PROJ-004) for transitions within cycle period.
- Force-regenerate creates a new memory revision via FR-BRAIN-101's content-addressed write; old revisions preserved by BRAIN's natural append-only chain.
...
- LLM response redaction runs the same FR-BRAIN-111 ruleset; same redaction tokens as input.
...
- The LLM call is rate-limited per tenant (FR-AI-014's budget); 1 review per cycle is well under any reasonable cap.
...
- The cycle review is `sync_class: shareable` because it's a team artifact; ACL can restrict if engagement has it (per FR-PROJ-002 engagement default ACL).
- Operator acceptance triggers FR-PROJ-002 `proj.decision`-style audit row with `decision_id` pointing at the cycle review memory.
