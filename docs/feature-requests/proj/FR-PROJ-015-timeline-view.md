...
source_decisions:
  ...
  - DEC-362 (Fixed-Fee milestones from FR-PROJ-007 surface as gold-bordered markers)
...
modified_files:
  - web/proj-client/src/router.tsx                  # /proj/timeline/:cycle_id route
allowed_tools:
...
disallowed_tools:
  ...
  - skip kbd parity for date-resize (per FR-PROJ-014 precedent)
...
sub_tasks:
  ...
  - "0.5h: MilestoneMarker.tsx — gold vertical bars at FR-PROJ-007 milestones"
  - "1.0h: kbd parity (J/K swimlanes; H/L days; Shift+Arrow resize ±1day)"
  - "1.0h: timeline_test.tsx — drag-resize + milestone render + kbd"
  - "0.5h: PATCH endpoint integration (LWW per FR-PROJ-003)"
...
3. **MUST** render each issue as a horizontal bar positioned by `starts_at..ends_at`. Bar height proportional to swimlane row height; bar colour from status enum (FR-PROJ-018 design tokens).
...
6. **MUST** PATCH date changes via FR-PROJ-003 LWW scalar handlers (issue.starts_at + issue.ends_at are LWW fields).
7. **MUST** render milestone markers from FR-PROJ-007 Fixed-Fee config: vertical gold-bordered line at `target_date`; hover tooltip shows milestone name + amount.
...
14. **MUST** pass axe-core (per FR-PROJ-018 a11y CI).
...
19. **MUST** include dependency arrows between issues (basic, slice-3 minimal version): when issue A `depends_on` issue B, render thin arrow from B's right edge to A's left edge. Full Gantt is FR-PROJ-016.
...
**Why LWW resize (DEC-361)?** Date changes are simple scalars; FR-PROJ-003 already provides LWW infrastructure. Two users dragging same bar → last-writer-wins with subject_id tie-break.
...
**Why minimal dependency arrows here (§1 #19)?** Operators in slice-3 want basic "X depends on Y" visualisation; full Gantt (critical path, parallel chains) is FR-PROJ-016.
...
- **FR-PROJ-003** — LWW scalar (starts_at, ends_at).
- **FR-PROJ-007** — milestone source.
- **FR-PROJ-014** — kbd-nav patterns reused.
- **FR-PROJ-018** — design tokens.
...
- Dependency arrows between issues (Gantt-style) — FR-PROJ-016.
...
- LWW PATCH endpoint reused from FR-PROJ-003 §3 (same as Kanban scalar updates).
...
- Snap-to-week/month uses Math.round on day delta; precision dates entered via Brief Modal (FR-PROJ-017).
...
- Weekend background uses CSS `:nth-child` of day-grid; static color from FR-PROJ-018 tokens.
