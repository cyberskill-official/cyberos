---
id: FR-PROJ-009
title: "BRAIN_LINK schema — Issue ↔ BRAIN memory linkage (cites | implements | supersedes) with bidirectional traversal and link-graph queries"
module: PROJ
priority: MUST
status: accepted
verify: T
...
related_frs: [FR-PROJ-001, FR-PROJ-010, FR-BRAIN-101, FR-BRAIN-108]
depends_on: [FR-PROJ-001]
blocks: [FR-PROJ-010]

source_pages:
  - website/docs/modules/proj.html#brain-link
source_decisions:
  - DEC-300 (link is a typed edge — cites | implements | supersedes — between Issue and a specific BRAIN memory path or row_id)
  - DEC-301 (links validated at write: target memory MUST exist + caller MUST have read scope per FR-SKILL-103 allowed_brain_scopes)
...
new_files:
  - services/proj-sync/migrations/0009_brain_links.sql
  - services/proj-sync/src/brain_link/mod.rs
  - services/proj-sync/src/brain_link/handlers.rs
  - services/proj-sync/tests/brain_link_test.rs
allowed_tools:
  - file_read: services/proj-sync/**, services/brain/**
  - file_write: services/proj-sync/{src,tests,migrations}/**
  - bash: cd services/proj-sync && cargo test brain_link
disallowed_tools:
...
effort_hours: 5
sub_tasks:
  - "0.5h: 0009_brain_links.sql migration"
...
3. **MUST** validate at link-create:
    - `memory_path` MUST exist (calls FR-BRAIN-108 search; 404 if missing → `Err(LinkError::TargetMissing)`).
    - Caller's frontmatter `allowed_brain_scopes` (FR-SKILL-103) MUST cover the path; otherwise `Err(LinkError::ScopeDenied)`.
...
5. **MUST** expose REST endpoints:
    - `POST /api/proj/issues/:id/brain-links` — create.
    - `DELETE /api/proj/issues/:id/brain-links/:link_id` with `reason` body — soft-remove.
    - `GET /api/proj/issues/:id/brain-links` — outgoing edges.
    - `GET /api/proj/brain-memories/:path/issues` — incoming edges (bidirectional traversal).
6. **MUST** emit BRAIN audit rows:
    - `proj.brain_link_created` on create.
    - `proj.brain_link_removed` on soft-delete.
...
9. **MUST** emit OTel metric `proj_brain_links_total{link_type, outcome}`; outcome ∈ created | removed | denied | dangling.
10. **MUST** support link annotations: an optional `annotation` field (max 500 chars) on link creation explaining the relationship beyond just the type (e.g. cites with annotation "see section 3 specifically"). PII-redacted via FR-BRAIN-111 before storage.
...
12. **MUST** include a `link_strength` field (`weak | medium | strong`) optional, default `medium`. `weak` = passing mention; `strong` = primary citation. Used by FR-PROJ-010 citation-drift to prioritise alerts.
13. **MUST** support batch link creation: `POST /api/proj/issues/:id/brain-links/batch` with array of links (max 50). Each item validated independently; partial success allowed (per-item status).
14. **MUST** expose link-graph traversal: `GET /api/proj/issues/:id/brain-links/graph?depth=N&types=cites,implements` returns a directed graph (issues + memories as nodes, links as edges) up to N hops. Default depth=2, max=5.
...
19. **MUST** support link "review" state: optionally, links can be marked `review_pending = true` requiring admin approval before they're considered active for traversal queries. Tenant policy `cyberos_proj_tenant_settings.brain_links_require_review` toggles default.
20. **MUST** include `proj.brain_link_traversed` audit row each time a graph-traversal query crosses an edge (for analytics on which links are heavily traversed). Sampled at 10% to avoid audit log spam.
...
**Why bidirectional traversal (§1 #5)?** Forward: "what does this issue link to?" Backward: "what issues link to this memory?" The latter is the load-bearing query for citation drift (FR-PROJ-010): "is this memory referenced anywhere?"
...
CREATE UNIQUE INDEX uniq_active_brain_link
    ON brain_links (issue_id, memory_path, link_type)
    WHERE removed_at IS NULL;
CREATE INDEX idx_brain_links_by_issue ON brain_links (issue_id) WHERE removed_at IS NULL;
CREATE INDEX idx_brain_links_by_memory ON brain_links (memory_path) WHERE removed_at IS NULL;

ALTER TABLE brain_links ENABLE ROW LEVEL SECURITY;
CREATE POLICY brain_links_tenant_isolation ON brain_links
...
pub async fn create_link(
...
) -> Result<BrainLink, LinkError> {
    // 1. Validate target exists
    let memory = brain_reader::find_memory(&memory_path).await
    ...
    let row: BrainLink = sqlx::query_as(
        "INSERT INTO brain_links (issue_id, memory_path, memory_row_id, link_type,
...
pub async fn remove_link(
    ...
    sqlx::query(
        "UPDATE brain_links SET removed_at = NOW(), removed_by_subject_id = $1, removal_reason = $2
    ...
    emit_brain_row("proj.brain_link_removed", serde_json::json!({
...
1. **Create cites link** — POST → 201; row in brain_links; type=cites.
...
5. **Scope-denied rejected** — POST to memory outside caller's allowed_brain_scopes → 403 ScopeDenied.
...
12. **Bidirectional: list-by-memory** — GET /api/proj/brain-memories/:path/issues → issues linking to it.
13. **BRAIN audit on create + remove** — both events emit corresponding rows.
...
- **FR-PROJ-001** — issues FK.
- **FR-PROJ-010 (downstream)** — citation drift uses these links.
- **FR-BRAIN-101** — BrainReader for validation.
- **FR-BRAIN-108** — search API (find_memory).
...
| Memory deleted after link created | dangling | Listed in query; FR-PROJ-010 flags | Operator removes link |
| BRAIN reader unreachable | find_memory Err | 503 | Operator restores BRAIN |
...
| Annotation with PII | redacted by FR-BRAIN-111 | stored redacted | None |
...
- `memory_row_id` is for linking to specific BRAIN audit chain rows (not memory paths). Some memories don't live at canonical paths (e.g. ad-hoc cross-references); row_id supports them.
- `subject_can_read_scope` integrates with FR-SKILL-103's allowed_brain_scopes when caller is a skill; for direct API callers, use FR-AUTH-003's tenant scope.
- Forward-only supersedes uses ns timestamps; BRAIN memory's `created_at_ns` is from its audit row (canonical).
- Soft-delete preserves history; FR-PROJ-008 records the link create/remove as history_event rows pegged to the linked issue.
...
- Annotation text is PII-redacted before storage (reuses FR-BRAIN-111 ruleset); raw form NOT retained.
...
- The annotation's 500-char limit matches FR-PROJ-002 reason field for consistency.
