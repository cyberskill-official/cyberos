...
related_frs: [FR-PROJ-001, FR-PROJ-003, FR-PROJ-004, FR-MEMORY-101]
depends_on: [FR-PROJ-001, FR-MEMORY-101, FR-PROJ-004]
blocks: []
...
- **FR-MEMORY-101** — MemoryWriter::emit_tx provides chain_anchor + row_id.
...
| Chain reader unreachable | verify returns false (treat as missing) | Marked unverified | Investigate FR-MEMORY-101 |
...
- memory emit returns `(chain_anchor, row_id)` synchronously per FR-MEMORY-101's writer protocol; the history_event UPDATE after emit fills these in.
...
- The summary endpoint's date range is bounded at 1 year to keep response time bounded; longer ranges should use FR-MEMORY-108 search directly.
