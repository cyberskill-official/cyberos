---
id: FR-PROJ-008
title: "BRAIN audit row per issue mutation — chained to PROJ history_event table with field-level diff and chain_anchor verification"
module: PROJ
priority: MUST
status: accepted
verify: T
...
related_frs: [FR-PROJ-001, FR-PROJ-003, FR-PROJ-004, FR-BRAIN-101]
depends_on: [FR-PROJ-001, FR-BRAIN-101, FR-PROJ-004]
blocks: []

source_pages:
  - website/docs/modules/proj.html#audit-trail
source_decisions:
  - DEC-290 (every issue mutation emits a BRAIN audit row + a PROJ history_event row; the two MUST be linked via chain_anchor)
  - DEC-291 (history_event captures field-level diff; BRAIN row carries the BRAIN-chain hash for tamper detection)
...
disallowed_tools:
  - emit BRAIN row without corresponding history_event row (per DEC-290 — they're a pair)
...
sub_tasks:
  ...
  - "0.5h: link BRAIN row's payload.history_event_id; history_event.chain_anchor = BRAIN row's chain hash"
...
3. **MUST** link the history_event ↔ BRAIN row bidirectionally:
    - history_event row carries `brain_row_id` and `chain_anchor`.
    - BRAIN audit row's payload carries `history_event_id` and `history_event_seq`.
    - Operators can pivot from either side.
4. **MUST** compute a per-field diff:
    - Scalars: `before` = prior value JSON; `after` = new value JSON.
    - Y.Text fields (`description`, `comment.body`): `before` = SHA-256 of prior content; `after` = SHA-256 of new content; raw content NOT stored (privacy + size). The Y.Doc snapshots (FR-PROJ-003) are the canonical text history.
...
6. **MUST** verify `chain_anchor` matches actual BRAIN-chain hash on query:
    - GET history endpoint walks `chain_anchor` per row → re-fetches BRAIN row → asserts SHA-256(canonical(brain_row_minus_chain) || prev_chain) == chain_anchor.
...
9. **MUST** emit `proj.issue_mutated` BRAIN audit row per mutation with payload `{issue_id, field, mutation_kind, before_hash, after_hash, by_subject_id, history_event_id, history_event_seq, trace_id}`.
...
11. **MUST** RLS-enforce (FR-AUTH-003).
12. **MUST** cache `chain_verified` flag per history_event row for 60s in memory (per request worker) to amortise BRAIN round-trips on hot endpoints (issue page load fetches 50+ history events).
...
15. **MUST** include `mutator_session_id` field correlating with FR-PROJ-002 decision_session_id — one PATCH that mutates 3 fields produces 3 history events sharing a session_id.
...
**Why dual-write (DEC-290)?** BRAIN audit rows live in the BRAIN chain (Layer 1 immutable, tenant-cross-cutting); history_event lives in Postgres (queryable, indexed by issue_id, tenant-scoped). The pair gives queryability + immutability — querying history is fast; the BRAIN row is the cryptographic proof.

**Why chain_anchor (DEC-291)?** Without it, a sufficiently-privileged operator could delete a history_event row and the BRAIN audit row separately, leaving no detection. The anchor binds them: deleting the history row leaves a BRAIN row pointing to it (detectable); tampering the BRAIN row breaks the chain hash (detectable per FR-BRAIN-101).

**Why hash Y.Text content (§1 #4)?** Issue descriptions can contain sensitive content (employee feedback, customer data). Storing raw before/after in history bloats the database AND duplicates the canonical Y.Doc snapshots (FR-PROJ-003). Hash is sufficient for "did the content change?" — clients fetch the actual text from Y.Doc snapshots.

**Why append-only (§1 #5)?** History tables that allow UPDATE/DELETE become "history-of-history" rabbit holes. Append-only is the universal pattern for audit logs; aligns with BRAIN chain semantics.
...
**Why same transaction (§1 #2)?** If history_event INSERT fails after BRAIN row already emitted, the two diverge silently. Transactional pair = atomicity: both or neither.

**Why 60s cache (§1 #12)?** Issue page load fetches 50+ history rows; without cache, that's 50+ BRAIN round-trips × ~10ms = 500ms page-load delay. 60s TTL is short enough that recent tampering surfaces quickly.
...
pub async fn emit_mutation(
    ...
    // 1. Insert history_event with PLACEHOLDER chain_anchor (filled after BRAIN emit)
    let event_id = uuid::Uuid::new_v4();
    let seq: i64 = sqlx::query_scalar(
        "INSERT INTO history_event (id, issue_id, field, before, after, mutation_kind,
                                     by_subject_id, occurred_at_ns, chain_anchor, brain_row_id, tenant_id)
    ...
    // 2. Emit BRAIN row (returns chain_anchor)
    ...
    let brain_emit = brain_writer::emit_tx("proj.issue_mutated", payload).await?;

    // 3. Update history_event with the actual chain_anchor + brain_row_id
    sqlx::query(
        "UPDATE history_event SET chain_anchor = $1, brain_row_id = $2 WHERE id = $3"
    ).bind(&brain_emit.chain_anchor).bind(&brain_emit.row_id).bind(event_id)
    ...
    Ok(HistoryEvent {
        ...
        chain_anchor: brain_emit.chain_anchor, brain_row_id: brain_emit.row_id,
    })
}
...
async fn verify_chain_anchor(row: &HistoryEvent) -> bool {
    // Fetch BRAIN row by brain_row_id; recompute chain hash
    match brain_reader::fetch_row(&row.brain_row_id).await {
...
5. **history_event linked to BRAIN row** — row's `brain_row_id` matches a BRAIN row with `kind="proj.issue_mutated"`.
6. **BRAIN row links back** — BRAIN row's `payload.history_event_id` matches the history_event UUID.
7. **chain_anchor verified** — happy query → all rows `chain_verified: true`.
8. **chain_anchor mismatch detected** — manually tamper a BRAIN row → query returns `chain_verified: false`; metric increments.
...
11. **Same-transaction pair** — if BRAIN emit fails, history_event also rolls back; observed via DB inspection after forced failure.
...
16. **chain_verified cached for 60s** — second query within 60s avoids re-fetching BRAIN row; cache hit metric increments (AC for §1 #12).
...
```rust
#[tokio::test]
async fn status_transition_emits_history_and_brain_pair() {
    ...
    let brain_row = env.brain.find_by_id(&events[0].brain_row_id).await.unwrap();
    assert_eq!(brain_row["payload"]["history_event_id"], events[0].id.to_string());
}
...
async fn chain_anchor_tamper_detected() {
    ...
    env.brain.tamper_row_for_test(/* the second mutation's brain row */).await;
...
- **FR-PROJ-001** — issues table FK.
- **FR-PROJ-003** — CRDT snapshot trigger.
- **FR-PROJ-004** — status transition handler.
- **FR-BRAIN-101** — BrainWriter::emit_tx provides chain_anchor + row_id.
...
|---|---|---|---|
| BRAIN emit fails mid-tx | sqlx tx rollback | history_event row not committed | None automatic; caller retries |
...
| BRAIN row deleted (impossible per AGENTS.md §3.5) | fetch_row Err | sev-1; marked unverified | None automatic |
...
| OTel exporter unavailable | buffered then dropped | Logged | Restore FR-OBS-001 |
| Chain reader unreachable | verify returns false (treat as missing) | Marked unverified | Investigate FR-BRAIN-101 |
...
| Sweep finds tamper but BRAIN row missing | treat as tamper | SEV-1 | Operator investigates |
...
- BRAIN emit returns `(chain_anchor, row_id)` synchronously per FR-BRAIN-101's writer protocol; the history_event UPDATE after emit fills these in.
...
- `cyberos_app` is the runtime SQL role (FR-AUTH-003); `cyberos_ops` (admin) bypasses the REVOKE.
- Chain verification on every query is O(N) BRAIN reads; for hot endpoints (issue page load), cache verified flags for 60s.
- `proj_chain_anchor_verifications_total{result="mismatch"}` > 0 triggers FR-OBS-007 sev-1 alert.
...
- session_id correlation with FR-PROJ-002 decision_session_id: both come from the same source (request-scoped header or client-generated); the proj handlers propagate.
...
- We considered storing the full BRAIN row alongside history_event to avoid the BRAIN round-trip on verify, but rejected: duplication = drift risk; the round-trip is fast (cached after first).
...
- The summary endpoint's date range is bounded at 1 year to keep response time bounded; longer ranges should use FR-BRAIN-108 search directly.
