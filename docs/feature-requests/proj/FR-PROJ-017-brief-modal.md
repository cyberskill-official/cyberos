...
sub_tasks:
  ...
  - "0.5h: HistoryDrawer.tsx — FR-PROJ-008 history_event list"
...
3. **MUST** bind description to Y.Text via FR-PROJ-003 YjsProvider and TipTap + `@tiptap/extension-collaboration`. Concurrent edits converge per Yjs.
...
5. **MUST** render meta sidebar with LWW scalars (FR-PROJ-003 §1 #6):
    - Status (uses FR-PROJ-014 StatusPicker).
    - Assignee dropdown.
    - Priority radio.
...
6. **MUST** render presence cursors:
    ...
    - Cursor flag throttled at 30 Hz per FR-PROJ-003 awareness.
    - Cursor expires 30s after last awareness heartbeat.
7. **MUST** provide a history drawer toggle (button + `H` shortcut):
    - When open, side panel shows FR-PROJ-008 history_event timeline.
...
28. **Sidebar quick-link actions** — click "Add Dep" in sidebar → opens FR-PROJ-016 dialog inline (AC for §1 #18).
...
- **FR-PROJ-004** — status transitions (StatusPicker).
- **FR-PROJ-008** — history_event source.
- **FR-PROJ-014** — opener (Kanban).
...
| Chain anchor mismatch | red warn icon | Sev-1 alarm via FR-PROJ-008 metric | Operator investigates |
...
| @lumi in comment with tenant lumi disabled | falls back to FR-CHAT-008 behavior | None | None |
...
- The 480px desktop side-panel width is a design token (FR-PROJ-018).
