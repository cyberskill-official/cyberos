...
First-pass §1 #5 mentioned "5xx on BRAIN emit → status change rolled back" but no transaction handling shown. Resolved: §1 #6 + §3 emit_decision_in_tx wraps DB+BRAIN in single tx; AC #7 + §5 test.
...
Operator-typed reason fields routinely include client names, emails, phone numbers. BRAIN audit retention is years; storing unredacted = years of accumulated PII risk. Resolved: §1 #13 + mandatory PII redact via FR-BRAIN-111 ruleset + AC #17 + test body.
...
Without index, queries became O(n) scans over the BRAIN outbox — interactive UX would lag. Resolved: §1 #19 + partial index `(tenant, issue, ts DESC) WHERE kind='proj.decision'` + AC #24 EXPLAIN ANALYZE check.

### ISS-014 — Immutability not enforced (strict-redo pass)
Original spec implied immutability via append-only chain but had no explicit handler rejection. Resolved: §1 #20 + 405 on PATCH/DELETE + REVOKE UPDATE/DELETE on brain_outbox role + AC #25 + #26.
...
A recipient with only the payload (e.g. in DSAR export) couldn't verify integrity without crawling the BRAIN chain. Resolved: §1 #22 + chain_anchor in payload + AC #28 + self-verify test.
