...
source_decisions:
  ...
  - DEC-252 (transitions are LWW-scalar via FR-PROJ-003 §1 #6; companion columns track when/who)
...
3. **MUST** validate transitions server-side BEFORE the LWW scalar write. Validation order:
    1. FSM check (this FR).
    2. LWW timestamp check (FR-PROJ-003 §1 #6).
    3. Apply.
...
9. **MUST** track per-status time-in-state for analytics (downstream of FR-PROJ-013 estimate calibration):
...
**Why time-in-status metric (§1 #9)?** "Average time in `in_review`" is the leading indicator of review-team throughput. Captured at transition-out time = exact + cheap. Downstream of FR-PROJ-013 calibration.
...
15. **Concurrent transitions serialised** — two clients send transitions on same issue concurrently → second client gets stale_write (FR-PROJ-003 LWW); history reflects single transition.
...
| Issue created with status != backlog | FR-PROJ-001 default; bypassed by direct SQL | Allowed; history starts mid-FSM | Operator audits |
...
- Re-open reason prompt is a modal/textarea in the picker; FR-PROJ-017 (Brief modal) provides the more polished version.
