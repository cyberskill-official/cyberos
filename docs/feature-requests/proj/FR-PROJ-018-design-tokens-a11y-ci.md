...
:root {
  ...
  /* — Color: Status (per FR-PROJ-004 IssueStatus) — */
  ...
  /* — Color: Critical-path (FR-PROJ-016) — */
...
- **FR-PROJ-014** — Kanban consumes status-color tokens.
- **FR-PROJ-015** — Timeline uses day-px token.
- **FR-PROJ-016** — Gantt uses critical-path-border token.
- **FR-PROJ-017** — Brief Modal uses side-panel-width token.
...
- The audit CLI integrates with FR-PROJ-008's CI gate; coverage below 90% = SEV-3 warning.
