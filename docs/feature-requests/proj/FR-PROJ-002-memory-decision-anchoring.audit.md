...
strict_redo_pass: 2026-05-16 P.M. (no-line-cap expansion per feature-request-audit skill §0; ISS-007..018 added)
---
...
Per feature-request-audit skill §0 master rule: spec is now perfect — depth bounded by the genuine surface (decision lifecycle × retraction × bulk-ops × link normalisation × tenant policy × engagement policy × required-reason × immutability × self-verification × session correlation), not by line targets.
