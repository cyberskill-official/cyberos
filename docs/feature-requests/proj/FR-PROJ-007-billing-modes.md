...
source_decisions:
  ...
  - DEC-282 (mode-aware rollup is the canonical input to FR-INV-001 invoice generation)
...
disallowed_tools:
  ...
  - rollup non-billable time entries into invoiceable amount (per FR-PROJ-006 snapshot)
...
The billing-mode layer **MUST** support exactly three modes per engagement with mode-aware rollups feeding FR-INV-001 invoice generation. The contract:

1. **MUST** define `billing_modes` table with columns: `id UUID PK`, `engagement_id UUID FK`, `mode` (`time_and_materials | fixed_fee | retainer`), `effective_from DATE`, `effective_to DATE` (nullable), `config JSONB` (mode-specific), `created_at`, `created_by_subject_id`, `tenant_id`. Versioned same as rate cards (FR-PROJ-005); INSERT-only.
...
12. **MUST** RLS-enforce (FR-AUTH-003).
...
21. **MUST** include an `engagement_metadata` field in `InvoiceRollup`: `{client_name, engagement_name, billing_address, vat_number}` from engagement (FR-PROJ-001) for downstream invoice rendering (FR-INV-001).
...
**Why versioned mode changes (§1 #1, DEC-280)?** Same logic as rate cards (FR-PROJ-005): UPDATE corrupts historical billing. A client invoiced at "T&M" in Q1 then "Fixed-Fee" in Q2 must show both modes in their respective periods.
...
**Why engagement_metadata in rollup (§1 #21)?** Downstream invoice rendering needs client name + billing address + VAT number. Pre-fetched in the rollup so FR-INV-001 doesn't re-query engagement.
...
- **FR-PROJ-005** — rate cards (T&M uses lookup_at).
- **FR-PROJ-006** — billable cascade (snapshot consumed in T&M rollup).
- **FR-INV-001 (downstream)** — invoice generation consumes `InvoiceRollup`.
- **FR-TIME-005** — time-entry snapshot field.
...
| Tax/VAT lookup deferred to FR-INV-001 | rollup excludes taxes | None | By design |
...
| Time entry without billable_snapshot (pre-FR-PROJ-006) | excluded from rollup | SEV-3 warning if found | Operator backfills |
...
- T&M rollup groups time entries by `(role, member_id)` to produce one InvoiceLine per cell; aggregation per FR-INV-001 spec.
- Fixed-Fee milestones tied to issue IDs (each milestone has 1+ issues that must be "done" per FR-PROJ-004 lifecycle); status change triggers rollup re-computation on next invoice.
...
- Milestone cancellation has two refund modes: (a) refund-to-client (issues credit note via FR-INV-001); (b) redistribute remaining milestone amounts (e.g. cancel m3 worth 20M, add 20M to m4-m5 pro-rata). Default is (a); operator chooses at cancellation time.
...
- Cancellation of an already-invoiced milestone requires manual credit-note via FR-INV-001 — automated handling deferred to slice 4+.
