...
Per AUTHORING.md §0 master rule: spec is now perfect — depth bounded by the genuine surface (tokens × themes × lint × Storybook × Chromatic × axe-core × contrast × focus × Liquid-Glass × OS preferences × diff reports × prefers-contrast × semantic layer × audit × versioning × telemetry × escape hatch × discoverability × cheatsheet), not by line targets.
