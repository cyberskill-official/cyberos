...
sub_tasks:
  ...
  - "0.5h: memory search integration (FR-MEMORY-108) — query decisions by issue_id"
...
1. **MUST** emit memory row `proj.decision` per Issue status transition (FR-PROJ-001 §1 #6's `proj.issue_status_changed` is REPLACED by this richer row). Payload: `issue_id`, `from_status`, `to_status`, `reason`, `decided_by_subject_id`, `prior_decision_chain`, `cross_module_links`, `tenant_id`, `request_id`.
...
4. **MUST** be referenceable from CHAT via `#issue/<id>` syntax — chat module's link expander (FR-CHAT-007) follows the link to the memory decision row.
5. **MUST** include `prior_decision_chain` field referencing the chain hash of the previous `proj.decision` for the SAME issue. First transition → `null`. Subsequent transitions form a per-issue chain queryable as "show all decisions on issue X" via FR-MEMORY-108.
...
9. **MUST** be queryable via FR-MEMORY-108 — searching for `kind:proj.decision AND issue_id:<id>` returns the chronological per-issue chain.
...
13. **MUST** redact PII from `reason` text before memory emit using FR-MEMORY-111 ruleset (same redactor as FR-CHAT-005). The redacted form is stored in the audit row; original is NOT retained. This is mandatory not optional — operators cannot disable.
...
19. **MUST** index `proj.decision` rows by `(tenant_id, issue_id, ts_ns)` in the memory search index (FR-MEMORY-108). The per-issue chain query MUST return in O(log n) time, not O(n) scan.
...
**Why FR-MEMORY-108 query integration (§1 #9)?** Decisions become first-class searchable artefacts. "Show me all decisions for engagement X" or "decisions by subject Y last quarter" — answerable directly.
...
10. CHAT @lumi can search decisions via FR-MEMORY-108: `kind:proj.decision AND issue_id:<id>`.
...
- **FR-AI-003** — memory_writer for audit row.
- **FR-MEMORY-106** — sync_class enforcement at sync boundary.
- **FR-MEMORY-108** — search-by-kind queryable.
...
| sync_class not respected at sync | FR-MEMORY-106 enforces | Filtered at sync boundary | By design |
...
- The PII redactor for reason text reuses the FR-MEMORY-111 ruleset; updates to the ruleset apply to PROJ without code changes.
...
- We considered streaming WAL changes to memory (like FR-CHAT-005's bridge pattern) for decisions but rejected: decisions are interactive (operator clicks); synchronous emit gives immediate feedback.
