...
source_decisions:
  ...
  - DEC-262 (billable_default is the rate-card's recommendation; overridable per FR-PROJ-006 cascade)
...
9. **MUST** enforce RLS per FR-AUTH-003 — operators can only see/write rate cards for their tenant's engagements.
...
18. **MUST** record `archived_at TIMESTAMPTZ` separate from supersession: an engagement-archived state (FR-PROJ-001) sets all the engagement's active rate cards' `archived_at = NOW()`; they remain queryable for historical reference but are excluded from `proj_rate_cards_active` gauge.
...
- **FR-PROJ-001** — `engagements` table FK target.
- **FR-PROJ-006 (downstream)** — billable cascade uses `lookup_at` to resolve rates.
- **FR-PROJ-007 (downstream)** — billing modes consume rate cards.
- **FR-AUTH-003** — RLS on `rate_cards`.
...
- Per-member rate overrides (Alice's senior engineer rate ≠ junior default) — slice 3+; lives in FR-PROJ-006 cascade.
...
- The `Currency::decimals()` helper is used by FR-PROJ-007 + FR-INV when rendering amounts; minor unit display formula = `value / 10^decimals`.
- Idempotency uses the same `admin_idempotency_keys` table pattern as FR-AUTH-001.
...
- We considered storing rates as `pg_numeric(20,8)` instead of BIGINT minor units; BIGINT is faster + matches the canonical pattern in FR-AUTH-001 + FR-PROJ-001.
...
- Engagement deletion (FR-PROJ-001) is `ON DELETE RESTRICT` for rate cards: operators must explicitly archive cards before engagement deletion.
