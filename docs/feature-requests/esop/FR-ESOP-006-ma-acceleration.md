...
source_decisions:
  - DEC-2300 2026-05-17 — Board declares M&A event → all active grants accelerated to fully_vested; Member notice via FR-EMAIL-009 within 5 business days
...
2. **MUST** require board threshold per DEC-2302 (reuse FR-ESOP-003 board threshold logic).
...
4. **MUST** notify members within 5 business days at `member_notifier.rs::notify(ma_event)` per DEC-2300:
   - For each affected member: send email via FR-EMAIL-009
