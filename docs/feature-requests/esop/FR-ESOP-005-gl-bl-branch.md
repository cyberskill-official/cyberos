...
source_decisions:
  - DEC-2290 2026-05-17 — Triggered by FR-HR-009 termination.executed; GL = full vesting up to term_date; BL = forfeit unvested + optional vested forfeit per policy
...
2. **MUST** trigger on FR-HR-009 termination.executed per DEC-2290.
...
5. **MUST** define table at migration `0005`:
   ...
   CREATE TABLE esop_leaver_outcomes (
     ...
     termination_id UUID NOT NULL,  -- FR-HR-009 ref
...
6. **MUST** expose endpoints:
   ```text
   POST /v1/esop/leaver-outcomes                   (auto from FR-HR-009 trigger; CFO/CEO can also draft)
