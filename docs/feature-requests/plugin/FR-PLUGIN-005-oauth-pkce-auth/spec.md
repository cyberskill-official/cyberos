...
related_frs: [FR-PLUGIN-001, FR-PLUGIN-002, FR-PLUGIN-006, FR-MCP-004, FR-AUTH-004, FR-AUTH-007]
depends_on: [FR-PLUGIN-001, FR-AUTH-004]
blocks: [FR-PLUGIN-007]

source_pages:
  - modules/plugin/INTEROP.md universal constraint 3
  - modules/plugin/manifest.schema.json (auth section)
  - docs/feature-requests/mcp/FR-MCP-004-oauth-pkce/spec.md (protocol shape)
...
build_envelope:
  ...
  modified_files:
    ...
    - services/auth/src/routes.rs (add /oauth/authorize + /oauth/token endpoints — placeholder until FR-AUTH-007 lands)
...
The PLUGIN module **MUST** implement OAuth 2.1 + RFC 7636 PKCE authentication for every plugin tool call. The bridge (FR-PLUGIN-002) MUST require a valid access token on every `tools/call` request; tokens MUST be obtained via a one-time install-time authorize flow against `auth.cyberskill.world`.
...
3. **MUST** sign access tokens with the AUTH service's JWKS-published key per FR-AUTH-004. Bridge verifies via cached JWKS (5-minute TTL).
...
5. **MUST** define and enforce the scope catalogue per DEC-2443:
   ...
   ```
   A tool call without the required scope MUST return error class `authz_denied` per FR-PLUGIN-002 clause 7. Scope-vs-tool matrix lives in `auth/scope_check.rs::REQUIRED_SCOPES`.
...
7. **MUST** store tokens via OS-native secret stores per DEC-2445:
   ...
   The fallback MUST log a warning to FR-OBS-001 OTel that secret store is unavailable.
...
9. **MUST** emit memory audit rows per FR-PLUGIN-006 for every auth event:
...
20. **api-key auth method rejected at install** — manifest with `auth.method: "api-key"` fails FR-PLUGIN-001 validation, install blocked.
...
- **Upstream:** FR-PLUGIN-001 (manifest declares `auth.method: "oauth-pkce"`); FR-AUTH-004 (JWT/JWKS issuance, shipped).
- **Downstream:** FR-PLUGIN-002 (bridge enforces scope on every call); FR-PLUGIN-006 (audit rows for auth events); FR-PLUGIN-007 (per-runtime adapter handles browser redirect to localhost callback).
- **Cross-module:** FR-MCP-004 (OAuth-PKCE protocol shape — clause 10 inherits); FR-AUTH-007 (planned: OAuth-PKCE authorize/token endpoints at AUTH service — placeholder until that FR ships).
...
| Refresh token reuse | AUTH detects duplicate (rotation invariant) | revoke all related grants | User re-authenticates; admin notified via FR-OBS-007 |
...
| Plugin manifest auth.method != "oauth-pkce" | FR-PLUGIN-001 schema check | install fails | Author fixes manifest |
