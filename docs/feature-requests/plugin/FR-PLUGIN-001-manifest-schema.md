...
related_frs: [FR-PLUGIN-002, FR-PLUGIN-003, FR-PLUGIN-004, FR-PLUGIN-005, FR-PLUGIN-006, FR-PLUGIN-007, FR-PLUGIN-008, FR-MCP-003, FR-SKILL-111]
depends_on: []
blocks: [FR-PLUGIN-002, FR-PLUGIN-003, FR-PLUGIN-004, FR-PLUGIN-005, FR-PLUGIN-006, FR-PLUGIN-007, FR-PLUGIN-008]
...
source_decisions:
  ...
  - DEC-2405 2026-05-19 — tool names MUST match `^cyberos\\.[a-z][a-z0-9]*\\.[a-z][a-z0-9_]*$` per FR-MCP-003 SEP-986
  ...
  - DEC-2408 2026-05-19 — Reference packer is Python (`modules/plugin/cyberos_plugin/`), Rust binary deferred to FR-PLUGIN-007; Python first matches CUO/memory reference-impl convention
...
12. **MUST NOT** allow `additionalProperties` at the top level of the manifest per DEC-2400. Unknown fields are rejected to keep the contract closed. Adapter-specific fields go under `targets[].extensions{}` (introduced in FR-PLUGIN-007).

13. **MUST NOT** silently coerce types (e.g. `version: 1.0` → `"1.0.0"`). Type mismatches fail validation with a clear error.

14. **MUST NOT** accept manifests where any tool's `annotations.destructive == true` but `scopes` does not include a corresponding write scope. Per FR-MCP-006 tool gating, destructive tools require explicit scope coverage.
...
**Why SEP-986 enforced at manifest level (DEC-2405, clause 6)?** FR-MCP-003 already enforces this at the gateway. Enforcing it at the manifest schema level catches violations earlier (author time, not runtime) and surfaces them in the same validator output as other schema errors. Defense-in-depth.
...
- **Downstream:** every other FR in this module reads `manifest.schema.json` for its contract. FR-PLUGIN-002 reads `tools[*]` for what MCP tools to register. FR-PLUGIN-003 reads `commands[*]`. FR-PLUGIN-004 reads `skills[*]`. FR-PLUGIN-005 reads `auth.*`. FR-PLUGIN-006 reads `audit.*`. FR-PLUGIN-007 reads `targets[*]`. FR-PLUGIN-008 reads `marketplace.*`.
- **Cross-module:** FR-MCP-003 (SEP-986 tool naming — clause 6 enforces it), FR-MCP-006 (tool annotation gating — clause 14 enforces it), FR-SKILL-111 (description enrichment — manifest `description` 60-480 char range matches the trigger-discovery discipline).
  ...
  "errors": [
    {
      ...
      "message": "Tool name 'execute_workflow' must follow SEP-986: 'cyberos.{module}.{verb}_{noun}'. Per FR-MCP-003."
    },
    {
      "json_pointer": "/signature/rekor_uuid",
      "constraint": "required",
      "message": "Missing Rekor transparency-log UUID. Bundles must be Sigstore-signed per FR-PLUGIN-001 §1 clause 8."
    }
  ]
}
```
...
- ~~Should the packer ship as Rust to match production?~~ → No, Python reference first per DEC-2408. Rust binary lands in FR-PLUGIN-007 alongside multi-runtime adapter work.
...
- §11.4 **Error message format.** Every validation error MUST include the JSON pointer to the failing field. Errors include a hint pointing back to this FR (e.g. "per FR-PLUGIN-001 clause 6") so the author can find the rationale.
...
- §11.7 **Marketplace pre-check.** When FR-PLUGIN-008 lands, `cyberos-plugin publish` runs `validate` before upload. This is duplicative with packer-time validation but is mandatory because plugins may be uploaded by tools other than the official packer.
