...
related_frs: [FR-PLUGIN-001, FR-PLUGIN-005, FR-PLUGIN-006, FR-MCP-001, FR-MCP-003, FR-MCP-006, FR-MCP-007]
depends_on: [FR-PLUGIN-001, FR-MCP-001, FR-MCP-003]
...
source_decisions:
  ...
  - DEC-2412 2026-05-19 — Bridge implements MCP 2025-11-25 spec — initialize + tools/list + tools/call + capabilities negotiation per FR-MCP-001
  ...
  - DEC-2414 2026-05-19 — Long-running tools (CUO execute_workflow) implement MCP Tasks primitive per FR-MCP-007 — async handle + status poll + resume-on-reconnect
  ...
  - DEC-2417 2026-05-19 — Bridge MUST emit OpenTelemetry spans for every tool call with attributes plugin_id, tool_name, tenant_id, trace_id; goes to FR-OBS-001 collector
...
2. **MUST** implement the MCP 2025-11-25 handshake per FR-MCP-001 + DEC-2412:
...
3. **MUST** expose exactly 8 tools at first ship per DEC-2413. Names follow SEP-986 per FR-MCP-003:
...
4. **MUST** wire each tool to its source-of-truth module per architecture:
   ...
   - SKILL tools → call `services/skill-broker/` (in flight per FR-SKILL-103) OR file-scan `modules/skill/` catalog if broker not ready

5. **MUST** implement MCP Tasks primitive per FR-MCP-007 for `cyberos.cuo.execute_workflow` + `cyberos.skill.invoke_skill` per DEC-2414. Long-running tools:
...
6. **MUST** be stateless across requests per DEC-2415. No in-memory session affinity; every request authenticates via OAuth-PKCE JWT (FR-PLUGIN-005) carrying `tenant_id` claim. The bridge MUST NOT cache identity between requests. Long-running tasks persist their state in Postgres, not in memory.
...
8. **MUST** emit OpenTelemetry spans per DEC-2417 for every tool call with attributes `cyberos.plugin_id`, `cyberos.tool_name`, `cyberos.tenant_id`, `cyberos.trace_id`, `cyberos.duration_ms`, `cyberos.outcome` (success / error_class). Spans flow to FR-OBS-001 collector via OTLP gRPC.

9. **MUST** honour tool-annotation gating per FR-MCP-006. Tools with `destructive: true` or `external_effect: true` annotations (`cyberos.memory.append_audit` for example) require the host to set `mcp/sampling/elicit_confirm: true` in the call envelope OR include a pre-signed confirmation token. Bridge rejects un-confirmed destructive calls with `authz_denied`.

10. **MUST** validate every `tools/call` request against the tool's declared `input_schema` per FR-PLUGIN-001 clause 1 before invoking the upstream. Schema-violation calls produce `input_validation` errors WITHOUT touching the upstream.
...
**Why MCP 2025-11-25 only (DEC-2412, clause 14)?** Mixing protocol versions in one binary multiplies code paths quadratically. FR-MCP-001 already commits the gateway to this version. Plugin bridge inherits.
...
**Why HTTP REST for memory (clause 4)?** memory service ships as a Fargate binary (per FR-MEMORY-104). HTTP REST is the natural client surface. Bridge can call it from any deployment context.

**Why Tasks primitive for long-running tools (DEC-2414, clause 5)?** CUO workflow chains run for 30 seconds to several minutes. A synchronous JSON-RPC response would either timeout or hold the connection. MCP Tasks primitive (FR-MCP-007) is the canonical solution: client receives a handle, polls status, optionally cancels. Reconnect-resume is essential for desktop hosts that may disconnect.
...
- **Upstream:** FR-PLUGIN-001 (manifest schema — bridge tools MUST match manifest tool list); FR-MCP-001 (MCP 2025-11-25 spec); FR-MCP-003 (SEP-986 naming).
- **Downstream:** FR-PLUGIN-003 (slash commands invoke bridge tools); FR-PLUGIN-007 (multi-runtime adapters bundle this binary).
- **Cross-module:** FR-MCP-006 (tool annotation gating — clause 9 enforces); FR-MCP-007 (Tasks primitive — clause 5 implements); FR-AUTH-004 (JWT verification, shipped); FR-MEMORY-101 (audit ingest, shipped); FR-OBS-001 (OTel collector — clause 8 emits to it).
...
| Postgres unavailable | sqlx connection error | upstream_unavailable for task ops; degraded mode for short tools | Recovery on next call; alerting via FR-OBS-007 |
| JWT signature invalid | auth middleware | authz_denied with hint "re-authenticate" | Client refreshes token |
| JWT expired | auth middleware exp claim check | authz_denied with hint "token expired" | Client refreshes via OAuth-PKCE (FR-PLUGIN-005) |
...
- §11.8 **OTel span attributes.** Each span captures: cyberos.plugin_id, cyberos.plugin_version, cyberos.tool_name, cyberos.tenant_id (hashed for privacy if FR-AI-011 PII redaction triggers), cyberos.trace_id, cyberos.duration_ms, cyberos.outcome. Spans flow via OTLP gRPC to FR-OBS-001 collector.
