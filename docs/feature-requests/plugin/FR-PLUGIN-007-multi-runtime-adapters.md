...
related_frs: [FR-PLUGIN-001, FR-PLUGIN-002, FR-PLUGIN-003, FR-PLUGIN-004, FR-PLUGIN-005, FR-PLUGIN-006, FR-PLUGIN-008]
depends_on: [FR-PLUGIN-002, FR-PLUGIN-004, FR-PLUGIN-005]
...
source_decisions:
  ...
  - DEC-2463 2026-05-19 — Adapter outputs are reproducible per FR-PLUGIN-001 clause 10 — same canonical manifest → same target bundle hash for a given target
  ...
  - DEC-2465 2026-05-19 — Adapter MUST omit playbooks (skills/) from Cursor bundle per FR-PLUGIN-004 clause 8 — Cursor's MCP integration does not render Skills
...
build_envelope:
  ...
  new_files:
    ...
    - services/plugin-host/src/bin/cyberos-plugin.rs (extends Python CLI from FR-PLUGIN-001 with Rust pack subcommand for multi-target)
...
The PLUGIN module **MUST** ship multi-runtime adapters at `services/plugin-host/src/adapters/<target>/`. Each adapter transforms the canonical CyberOS manifest (per FR-PLUGIN-001) into a target-runtime-native bundle. P1 targets: claude-code, cursor, cowork, codex-cli.
...
3. **MUST** be reproducible per DEC-2463. For a given canonical manifest + target, two pack invocations produce identical SHA-256 bundle hashes. Adapters MUST use the reproducibility helpers in `adapters/common.rs` (epoch mtimes, sorted entries, fixed permissions) — same as FR-PLUGIN-001 clause 10.
...
10. **MUST** emit a bundle-level Sigstore signature distinct per target per FR-PLUGIN-001 clause 8. The same canonical manifest produces 4 distinct signatures (one per target bundle).
...
- **Upstream:** FR-PLUGIN-002 (bridge binary referenced in 3 of 4 adapter bundles); FR-PLUGIN-004 (skills folder layout); FR-PLUGIN-005 (auth manifest section translation).
- **Downstream:** FR-PLUGIN-008 (marketplace publish reads target list per bundle).
- **Cross-module:** FR-PLUGIN-003 (commands surface per adapter); FR-PLUGIN-006 (audit endpoint per adapter target).
