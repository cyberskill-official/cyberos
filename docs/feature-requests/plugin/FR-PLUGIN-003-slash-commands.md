...
related_frs: [FR-PLUGIN-001, FR-PLUGIN-002, FR-PLUGIN-004]
depends_on: [FR-PLUGIN-001, FR-PLUGIN-002]
blocks: []
...
source_decisions:
  ...
  - DEC-2422 2026-05-19 — Command files MUST be host-portable — Claude Code + Cowork render natively; Cursor + Codex CLI use the SKILL.md compat layer in FR-PLUGIN-007
  - DEC-2423 2026-05-19 — Command argument schema MUST mirror the underlying MCP tool's input_schema — single source of truth per FR-PLUGIN-001
  - DEC-2424 2026-05-19 — Commands MUST include before-use trigger discipline per FR-SKILL-111 — description (60-480 chars) + 4 trigger examples
...
The PLUGIN module **MUST** ship 4 canonical slash-commands at `modules/plugin/commands/<name>.md`. Each command is a markdown file with YAML frontmatter binding the command to one or more MCP tools exposed by FR-PLUGIN-002.
...
4. **MUST** declare 4 example trigger phrases per command per FR-SKILL-111 + DEC-2424. Phrases live in the frontmatter `triggers:` array. Hosts that route by description match (Claude Code's skill router) use these to disambiguate.

5. **MUST** include a `description:` field of 60-480 characters per FR-SKILL-111. This is the host-rendered command summary. The 60-char floor forces meaningful copy; the 480-char ceiling forces conciseness.
...
6. **MUST** carry frontmatter conforming to this YAML shape:
   ...
   binds_to:
     - tool: cyberos.<module>.<verb>_<noun>   # SEP-986 from FR-PLUGIN-001
   ...
   destructive: <true|false>            # if any bound tool is destructive (per FR-MCP-006)
   ---
   ```
...
7. **MUST** validate via `modules/plugin/tests/test_commands_*.py` that:
   ...
   - Every `binds_to[*].tool` matches a tool name registered in FR-PLUGIN-002's static registry
...
**Why mirror tool input_schema (DEC-2423, clause 3)?** Two sources of truth for the same shape drift. The manifest schema (FR-PLUGIN-001) is canonical. Commands MUST derive from it.

**Why 4 trigger phrases (DEC-2424, clause 4)?** FR-SKILL-111 calibrated 4 as the minimum count that gives the description-match router enough fingerprint to disambiguate. Two phrases is too noisy; eight is overkill for a slash command that the user already typed.

**Why 60-480 char description (clause 5)?** Same as FR-SKILL-111 description-enrichment range. Below 60 = unhelpful blurb; above 480 = wall-of-text in cramped UI panels.
...
def validate_command_file(path: Path) -> List[ValidationError]:
    ...
    for bind in fm["binds_to"]:
        if bind["tool"] not in REGISTERED_TOOL_NAMES:
            errors.append(("binds_to", f"tool '{bind['tool']}' not in FR-PLUGIN-002 registry"))
    return errors
```
...
5. **Every binds_to.tool exists in FR-PLUGIN-002 registry** — test loads tool list, checks each bind.
    ...
    for f in COMMANDS_DIR.glob("cyberos-*.md"):
        ...
        for bind in fm["binds_to"]:
            assert bind["tool"] in REGISTERED, \
                f"{f.name}: tool '{bind['tool']}' not in FR-PLUGIN-002 registry"
```
...
- **Upstream:** FR-PLUGIN-001 (manifest schema declares `commands` array shape), FR-PLUGIN-002 (provides the bound tools).
- **Downstream:** FR-PLUGIN-007 (adapters export commands to per-runtime format — Claude Code reads `.md` directly; Cursor doesn't render commands but ignores them).
- **Cross-module:** FR-SKILL-111 (description enrichment discipline, shipped), FR-SKILL-112 (TRIGGER_TESTS.md pattern — the `triggers:` array follows the same shape); FR-MCP-006 (destructive flag mirrors tool annotation).
...
- ~~Should we add /cyberos-doctor for the bundle health check?~~ → No, FR-PLUGIN-001 ships `cyberos-plugin doctor` as a CLI; not surfaced as a slash command. Successor FR may add if user demand emerges.
...
- §11.6 **Cursor doesn't render slash commands.** Cursor's MCP integration surfaces tools but not commands. The FR-PLUGIN-007 Cursor adapter omits the `commands/` directory from the Cursor bundle. The canonical command files still ship in the canonical bundle for hosts that do render them.

- §11.7 **Claude Code custom command rendering.** Claude Code reads markdown from a `.claude/commands/` directory (relative to project). The Claude Code adapter (FR-PLUGIN-007) copies `modules/plugin/commands/*.md` to `dist/.claude/commands/` in the packed bundle.
