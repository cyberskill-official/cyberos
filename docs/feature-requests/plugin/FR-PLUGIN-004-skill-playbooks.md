...
related_frs: [FR-PLUGIN-001, FR-PLUGIN-002, FR-PLUGIN-003, FR-SKILL-111, FR-SKILL-112, FR-SKILL-113, FR-SKILL-114, FR-SKILL-115]
depends_on: [FR-PLUGIN-002, FR-SKILL-111]
...
source_decisions:
  ...
  - DEC-2431 2026-05-19 — Playbooks are routing+discipline, not tools — they teach hosts how to chain FR-PLUGIN-002 tools correctly; they do NOT add new MCP tools
  - DEC-2432 2026-05-19 — Every playbook MUST pass SKILL_BUNDLE_RUBRIC SKB-020..023 (description format + XML-free frontmatter) per FR-SKILL-111/113
  - DEC-2433 2026-05-19 — Every playbook MUST carry acceptance/TRIGGER_TESTS.md with 4 positive + 4 negative fixtures per FR-SKILL-112
...
build_envelope:
  ...
  disallowed_tools:
    - bypass FR-SKILL-111 description format (per DEC-2432)
...
The PLUGIN module **MUST** ship 12 skill playbooks at `modules/plugin/skills/<name>/SKILL.md` following the Anthropic Agent Skills spec. Each playbook is a markdown file with YAML frontmatter teaching hosts WHEN and HOW to chain the MCP tools exposed in FR-PLUGIN-002. Playbooks do NOT add new MCP tools — they are documentation that the host's skill router uses to inject just-in-time discipline into the model's prompt.
...
2. **MUST** conform to SKILL_BUNDLE_RUBRIC SKB-020..023 per DEC-2432 + FR-SKILL-111/113:
...
3. **MUST** carry `acceptance/TRIGGER_TESTS.md` per DEC-2433 + FR-SKILL-112 with at least:
...
4. **MUST** reference only tools registered in FR-PLUGIN-002 — validator test `test_playbooks_reference_valid_tools.py` checks every tool name mentioned in the playbook body against the 8-tool registry.
...
7. **MUST** pass FR-SKILL-114 baseline check — once SKILL.md hits v1.0 maturity, an `acceptance/BASELINE.md` MUST be created freezing the trigger-test pass rate so regressions are detectable.

8. **MUST** be host-portable per DEC-2434 — same SKILL.md works in Claude Code, Cowork, Codex CLI without modification. Cursor doesn't render Skills (its MCP integration surfaces tools only); the Cursor adapter (FR-PLUGIN-007) omits skills/.
...
11. **MUST NOT** rename a playbook within v1.x.y — rename = breaking change requiring major bump (mirrors FR-PLUGIN-003 clause 11).
...
- **Tools** (FR-PLUGIN-002) — *what* the plugin can do; called by the host's tool-calling subsystem.
- **Commands** (FR-PLUGIN-003) — *how the user invokes* it explicitly; a UI affordance.
...
**Why exactly 12 playbooks (DEC-2430)?** Same logic as FR-PLUGIN-003 — small, learnable, well-considered. 12 covers the major use-cases with room to teach edge-cases (memory-write-discipline, audit-emission-discipline). More playbooks dilute the router's discrimination.
...
**Why TRIGGER_TESTS (DEC-2433)?** FR-SKILL-112 introduced TRIGGER_TESTS.md as the way to measure routing accuracy. Without it, a description that triggers spuriously (false positive) or fails to trigger (false negative) goes unnoticed. With TRIGGER_TESTS, CI catches the regression.
...
**Why required body sections (clause 5)?** Same lazy-load discipline as FR-PLUGIN-003 commands — the body teaches the model when triggered. Without explicit "scopes required" and "side effects" sections, the model uses the playbook without knowing the consequences.

**Why mandatory BASELINE.md at v1.0 (clause 7)?** FR-SKILL-114 establishes this as the regression-prevention mechanism. Playbook descriptions drift as authors tune triggers; without a baseline pass-rate snapshot, the team can't tell if a tweak improved routing or broke it.
...
└── acceptance/
    ...
    └── BASELINE.md                (frozen pass rate at v1.0 promotion — added by FR-SKILL-114)
```
...
7. **Every description's verb stems are recognised (SKB-023)** — test runs against VERB_STEMS allowlist from FR-SKILL-115 tooling.
8. **Every TRIGGER_TESTS.md has 4+ positive + 4+ negative fixtures** — test parses lists.
9. **Every tool name referenced in body matches FR-PLUGIN-002 registry** — test greps `cyberos.<...>` patterns and validates.
    ...
    for skill_dir in SKILLS.iterdir():
        ...
        for match in pattern.findall(body):
            assert match in REGISTERED, \
                f"{skill_dir.name}: tool '{match}' not in FR-PLUGIN-002 registry"
```
...
- (later, at v1.0 promotion per FR-SKILL-114) acceptance/BASELINE.md
...
- **Upstream:** FR-PLUGIN-002 (provides the 8 tools referenced); FR-SKILL-111 (description enrichment discipline applied here); FR-SKILL-113 (XML-free frontmatter — SKB-022).
- **Downstream:** FR-PLUGIN-007 (multi-runtime adapters copy `skills/` into per-target bundle for hosts that support Skills).
- **Cross-module:** FR-SKILL-112 (TRIGGER_TESTS.md format), FR-SKILL-114 (BASELINE.md once v1.0), FR-SKILL-115 (placeholder sweep tooling reused for verb-stem check).
...
- ~~Should BASELINE.md ship in v1 or wait for v1.0 promotion?~~ → Wait for v1.0 per clause 7 + FR-SKILL-114 — baselines on pre-stable skills get bumped repeatedly and the discipline erodes.
...
| Verb stems unrecognised | SKB-023 allowlist | test fails | Author uses canonical verbs or extends VERB_STEMS via FR-SKILL-115 |
...
- §11.1 **Why 12 not 10 or 14.** 12 = 3 use-cases × 4 playbooks each, evenly balanced. 10 leaves discovery undercovered; 14 dilutes router discrimination. Sweet spot from FR-SKILL-111 calibration data.

- §11.2 **VERB_STEMS reuse.** FR-SKILL-115 shipped a verb-stem allowlist (`tools/sweep-placeholders/verb_stems.py`). The test imports it directly. Adding new verbs requires the FR-SKILL-115 extension procedure.
...
- §11.4 **Cursor + Codex CLI compatibility.** Cursor doesn't render Skills; the FR-PLUGIN-007 Cursor adapter strips the skills/ directory from the bundle. Codex CLI reads SKILL.md as-is. Both behaviours are tested in FR-PLUGIN-007.
...
- §11.8 **Avoid playbook author drift.** Annual review: re-check every playbook's description against shipped FR-PLUGIN-002 tool list. Add a CI annotation when a new tool lands without a playbook covering it.

---

*End of FR-PLUGIN-004 spec.*
