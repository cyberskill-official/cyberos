...
related_frs: [FR-PLUGIN-001, FR-PLUGIN-002, FR-PLUGIN-005, FR-PLUGIN-007, FR-PLUGIN-008, FR-MEMORY-101, FR-MEMORY-104, FR-OBS-001]
depends_on: [FR-PLUGIN-002, FR-MEMORY-101]
blocks: [FR-PLUGIN-008]
...
source_decisions:
  ...
  - DEC-2454 2026-05-19 — Retry policy is exponential backoff (250ms / 1s / 4s / 16s / 64s / 256s) up to 24h; after 24h, row marked failed and surfaces to FR-OBS-007 alert
  ...
  - DEC-2456 2026-05-19 — Audit-emission failures themselves MUST emit an OTel span tagged plugin.audit_emission_failed for FR-OBS-001 observability
...
1. **MUST** emit exactly 6 audit kinds per DEC-2450:
   - `plugin.installed` — on first successful OAuth-PKCE token exchange (per FR-PLUGIN-005 clause 9)
...
5. **MUST** implement retry per DEC-2454: exponential backoff `[250ms, 1s, 4s, 16s, 64s, 256s, 1024s, 4096s, ...]`; up to 24 hours total. After 24h, mark row `failed` and emit OTel alert (FR-OBS-007 picks up).
...
6. **MUST** scrub user data from audit bodies per DEC-2455. The audit row body MUST include exactly these fields (and nothing else):
   ...
   - `outcome` — "success" | one of 4 error classes from FR-PLUGIN-002 clause 7
...
**Why Postgres-backed outbox (DEC-2452, clause 3)?** In-memory queue loses on process restart. Filesystem queue requires its own atomicity model. Postgres is already in the bridge's stack (FR-PLUGIN-002 task table); reusing it is free.
...
**Why required OTel emission on emission failure (DEC-2456, clause 7)?** Audit outages are the most dangerous failure — they're silent. OTel emission ensures FR-OBS-007 + the dashboards know a tenant's audit chain is degraded. Without it, memory-write failures hide for hours.
...
- **Upstream:** FR-PLUGIN-002 (bridge calls emitter on each tool call); FR-MEMORY-101 (Layer-2 ingest pipeline, shipped).
- **Downstream:** FR-PLUGIN-008 (marketplace audit chain — publish events use the same emitter for plugin.published).
- **Cross-module:** FR-MEMORY-104 (memory HTTP REST, shipped); FR-OBS-001 (OTel collector for clause 7 spans); FR-OBS-007 (alertmanager picks up 24h-failure rows per DEC-2454).

---

## §8 — Example payloads

(See §3 for schema. Sample bodies in FR-PLUGIN-005 §8.)
