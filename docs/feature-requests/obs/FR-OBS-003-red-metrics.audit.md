...
FR-OBS-003 expanded from 165 lines to ~640. Added 6 §1 clauses (#4 standardised buckets, #8 custom dimensions, #9 cardinality guard, #10 init API, #11 CI lint completeness test, #12 SDK self-metrics). 8 §2 rationale paragraphs. Full Rust crate skeleton + macro + cardinality guard in §3. 15 ACs. 7 full Rust test bodies. 17 failure modes. 9 implementation notes.
