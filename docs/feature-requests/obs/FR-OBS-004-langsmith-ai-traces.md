...
disallowed_tools:
  - send raw (un-redacted) prompts to LangSmith — must be the FR-AI-011 redacted version (per §1 #4)
...
effort_hours: 6
sub_tasks:
  - "0.5h: ai_policy.langsmith_export bool field + FR-AI-005 schema extension"
...
2. **MUST** include in the exported payload:
    ...
    - REDACTED prompt (post-FR-AI-011 redaction; placeholders only — no raw PII).
...
9. **MUST** authenticate to LangSmith via bearer token (per-environment, rotated quarterly per FR-AUTH-006-style sweeper). Token in env var `LANGSMITH_API_TOKEN`; never logged.
...
12. **SHOULD** truncate redacted prompts > 100KB to first 100KB + `"...[truncated by FR-OBS-004]"` marker. LangSmith UI degrades on huge payloads; truncation preserves the diagnostic value.
...
**Why per-region LangSmith (§1 #4)?** FR-AI-016 residency pinning applies: a `Sg1` tenant's traces shouldn't ship to an EU-region LangSmith. Per-region deployment (`langsmith.sg-1.cyberos.world`, `langsmith.eu-1.cyberos.world`) keeps data in-region. Single global LangSmith would force all data to one region.
...
pub fn build_payload(
    ...
    let prompt = if redacted_prompt.0.len() > MAX_PROMPT_BYTES {
        format!("{}...[truncated by FR-OBS-004]", &redacted_prompt.0[..MAX_PROMPT_BYTES])
    } else { redacted_prompt.0 };
    let response = if redacted_response.0.len() > MAX_PROMPT_BYTES {
        format!("{}...[truncated by FR-OBS-004]", &redacted_response.0[..MAX_PROMPT_BYTES])
...
11. **Truncation > 100KB** — 200KB prompt → exported version is 100KB + `"...[truncated by FR-OBS-004]"`.
...
fn truncation_at_100kb() {
    ...
    assert!(payload.prompt.ends_with("...[truncated by FR-OBS-004]"));
}
...
- **FR-AI-022** — Source of trace_id for correlation.
- **FR-AI-011** — Source of redacted prompts/responses.
- **FR-AI-014** — Persona handle in metadata.
- **FR-AI-001** — Cost in metadata.
- **FR-AI-005** — Tenant policy schema includes `langsmith_export` field.
- **FR-AI-021** — CLI enables/disables per tenant + emits audit row.
...
| Exported PII (post-redaction bug) | `langsmith_no_pii_test` regex check | PR blocked | Investigate FR-AI-011 |
...
| Per-tenant opt-in audit row missing | FR-AI-021 emits | CLI bug | Investigate CLI |
...
- Per-tenant opt-in default false respects PDPL Art. 7. Operators enable per-tenant via FR-AI-021 CLI; opt-in is auditable.
