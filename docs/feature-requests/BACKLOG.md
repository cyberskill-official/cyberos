...
| `services/obs-collector/` | Rust workspace member (NEW 2026-05-19) | **FR-OBS-001 + FR-OBS-002 shipped.** `deploy/obs/` contains the live Docker Compose LGTM stack (CyberOS OTLP ingress gate + otelcol-contrib + Loki + Prometheus + Tempo + Grafana), retention configs, Grafana datasource provisioning, token rotation, health/smoke/auth/restart tests, and file-backed exporter queues. `cyberos-obs ingress` enforces per-service bearer-token-to-`service.name` binding for OTLP/HTTP and OTLP/gRPC before forwarding through an internal collector token. `cyberos-obs grafana-proxy` authenticates Grafana datasource JWTs, injects tenant filters into PromQL/LogQL/TraceQL, rejects user-supplied tenant labels with sev-1 audit events, emits per-query `obs.query_proxied` rows, and gives root-admin an audited unfiltered path. The live stack passes health, auth-required, service-token binding, Tempo smoke, and 100/100 buffer replay across Tempo outage plus collector restart. `cyberos-obs validate-config` now guards the pinned otelcol-contrib 0.110.0 schema drift found during live verification. | FR-OBS-003 (RED metrics SDK) + FR-OBS-004 (LangSmith) + FR-OBS-005 (TraceContext correlation) + FR-OBS-006 (tail sampling) + FR-OBS-007 (Alertmanager → CUO routing) + FR-OBS-008 (compliance views) + FR-OBS-009 (chain-of-custody manifest). |
