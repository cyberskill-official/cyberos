...
| `services/obs-collector/` | Rust workspace member (NEW 2026-05-19) | **FR-OBS-001..006 shipped.** `deploy/obs/` contains the live Docker Compose LGTM stack (CyberOS OTLP ingress gate + otelcol-contrib + Loki + Prometheus + Tempo + Grafana + self-hosted LangSmith), retention configs, Grafana datasource provisioning, token rotation, health/smoke/auth/restart tests, file-backed exporter queues, per-region LangSmith config, and tail-based sampling operator configs/scripts. `cyberos-obs ingress` enforces per-service bearer-token-to-`service.name` binding for OTLP/HTTP and OTLP/gRPC before forwarding through an internal collector token. `cyberos-obs grafana-proxy` authenticates Grafana datasource JWTs, injects tenant filters into PromQL/LogQL/TraceQL, rejects user-supplied tenant labels with sev-1 audit events, emits per-query `obs.query_proxied` rows, and gives root-admin an audited unfiltered path. `cyberos-obs validate-config` now guards the pinned otelcol-contrib 0.110.0 schema drift found during live verification and rejects missing/misordered trace sampling. AI Gateway exports redacted, tenant-opt-in LangSmith traces correlated to FR-AI-022 trace IDs. The shared OBS SDK now owns W3C TraceContext extraction/generation/injection, request log context spans, duration exemplars, and the OBS correlation CI gate. FR-OBS-006 adds deterministic tail-sampling policy evaluation for 100% errors/5xx/slow/flagged tenants plus 10% normal traces, route latency budgets, sampling metric constants, and the audited `cyberos-ai flag-tenant` operator command. | FR-OBS-007 (Alertmanager → CUO routing) + FR-OBS-008 (compliance views) + FR-OBS-009 (chain-of-custody manifest). |
