...
| `services/ai-gateway/` | Rust workspace member (NEW 2026-05-19) | **FR-AI-001 + FR-AI-002 + FR-AI-003 + FR-AI-004 + FR-AI-005 + FR-AI-006 + FR-AI-007 + FR-AI-008 + FR-AI-009 + FR-AI-010 + FR-AI-011 + FR-AI-012 + FR-AI-013 + FR-AI-015 + FR-AI-016 + FR-AI-017 + FR-AI-018 + FR-AI-021 + FR-AI-022 shipped.** FR-AI-003 (memory audit-row bridge): subprocess writer · canonical-JSON serialisation per AGENTS.md §6.2 · NFC normalisation · SHA-256 chain verification · path-traversal guard · 5s timeout w/ SIGTERM-then-SIGKILL · typed builders (precheck · invocation · invocation_failed · reconcile_started · reconcile_completed · reconcile_failed · hold_expired · hold_expired_started · hold_expired_completed · cleanup_run_completed · persona_loaded · zdr_violation · residency_violation) · `check_writer_available` startup health check. FR-AI-005 (tenant-policy YAML loader): `TenantPolicy` schema (cap · warn · hard_stop · provider · fallback_chain · timeout · residency · tenant_jurisdiction · residency_override · zdr · emergency_override · allowed_personas) · `ArcSwap`-backed lock-free cache · `notify` file-watcher with NFS polling-mode detection · 10 ACs covered via integration tests · CI gate via `gen-schema` binary · `cyberos-ai policy validate \| list \| serve` CLI · `EXAMPLE.tenant.yaml` reference. FR-AI-006 (model-alias resolution): closed six-alias registry · primary/fallback/override resolution · terminal policy-invariant failures · ZDR/residency/cost-table checks · metrics · deterministic/concurrent tests · sub-microsecond benchmark. FR-AI-007 (provider cost-table loader): YAML-backed USD rates · Decimal precision · aggregate validation · hot reload with last-known-good fallback · metrics · sub-microsecond lookup benchmark. FR-AI-008 (multi-provider router): Bedrock/Anthropic/OpenAI HTTP providers · retry + failover loop · deadline enforcement · `Retry-After` handling · trace-header propagation · response normalization · contract tests. FR-AI-009 (circuit breaker): per-provider/model breaker · half-open probes · router filtering/outcome recording · metrics · deterministic transition events · ~30ns hot-path benchmark. FR-AI-010 (streaming SSE): router streaming retry/failover · OpenAI/Anthropic/Bedrock SSE normalization · token/usage/done pipeline · first-token latency bench. FR-AI-011 (PII redaction): localhost-only Presidio sidecar client · loopback config validation · fail-closed router gate for normal/streaming calls · restoration map for tool-call args · no-prompt error/log sanitizer · redaction p95 bench. FR-AI-012 (VN PII): Presidio recognizers for CCCD/MST/phone/NĐD/address/bank account · per-type recall floor fixture · no-network lint · version endpoint · tenant PII allowlist suppression · regenerated policy schema. FR-AI-013 (recall CI): 230-row synthetic corpus (200 positive + 30 negative) · per-recognizer + aggregate recall gate · manifest version pins · JSON recall/precision artifacts · fixture invariants · no-real-PII pre-commit hook · self-gated GitHub Actions + quarterly refresh reminder. FR-AI-015 (ZDR enforcement): HTTPS-sourced attestation table · fail-closed alias enforcement · hot reload with revocation metrics · hard-stale override · `ai.zdr_violation` audit rows · weekly staleness workflow. FR-AI-016 (residency pinning): sg/eu/us/vn matcher · fail-closed Vn1 · per-alias residency overrides · missing-residency defaults/refusals · `ai.residency_violation` audit rows · 403 response mapping. FR-AI-017 (response cache): Redis-backed tenant-prefixed cache keys over redacted prompt/model/persona · TTL jitter · 1MB entry cap · Redis degradation fallback · router lookup/insert · cache_state audit field. FR-AI-018 (cache isolation gate): 256-case cross-tenant property test, seven regression scenarios, adversarial tenant corpus, 100-task concurrency test, dedicated CI gate with non-skip enforcement and JSON artefact. FR-AI-021 (operator CLI): Postgres-backed `cyberos-ai` usage/models/policy/breaker/expiry/invoice/memory commands · role-gated JWT auth · `--confirm` destructive guard · versioned JSON output · `ai.cli_*` audit rows · live fixture verification. FR-AI-022 (OTel traces): root chat/embed spans · persona/redact/cache/provider child spans · per-attempt retry events · W3C TraceContext + baggage propagation · PII-safe attribute lint · span-name docs · overhead guard. Workspace wired in `services/Cargo.toml`. | FR-AI-014 remains ready pending protocol path approval for persona seed files. FR-AI-019 remains ready pending live BGE-M3 model/checksum sidecar verification. FR-AI-020 (rerank) follows FR-AI-019 per slice 4 in §2 P0.1. |
| `services/obs-collector/` | Rust workspace member (NEW 2026-05-19) | **FR-OBS-001..007 shipped.** `deploy/obs/` contains the live Docker Compose LGTM stack (CyberOS OTLP ingress gate + otelcol-contrib + Loki + Prometheus + Tempo + Grafana + self-hosted LangSmith + Alertmanager + obs-router), retention configs, Grafana datasource provisioning, token rotation, health/smoke/auth/restart tests, file-backed exporter queues, per-region LangSmith config, tail-based sampling operator configs/scripts, and Alertmanager-to-CUO routing config. `cyberos-obs ingress` enforces per-service bearer-token-to-`service.name` binding for OTLP/HTTP and OTLP/gRPC before forwarding through an internal collector token. `cyberos-obs grafana-proxy` authenticates Grafana datasource JWTs, injects tenant filters into PromQL/LogQL/TraceQL, rejects user-supplied tenant labels with sev-1 audit events, emits per-query `obs.query_proxied` rows, and gives root-admin an audited unfiltered path. `cyberos-obs validate-config` now guards the pinned otelcol-contrib 0.110.0 schema drift found during live verification and rejects missing/misordered trace sampling. AI Gateway exports redacted, tenant-opt-in LangSmith traces correlated to FR-AI-022 trace IDs. The shared OBS SDK now owns W3C TraceContext extraction/generation/injection, request log context spans, duration exemplars, and the OBS correlation CI gate. FR-OBS-006 adds deterministic tail-sampling policy evaluation for 100% errors/5xx/slow/flagged tenants plus 10% normal traces, route latency budgets, sampling metric constants, and the audited `cyberos-ai flag-tenant` operator command. FR-OBS-007 adds `cyberos-obs-router`: webhook-secret-authenticated Alertmanager ingestion, CUO `obs.triage-alert@1` invocation, CHAT/PagerDuty routing with sev-1 dual-route, ack/escalation callbacks, dedup, metrics, and `obs.alert_triaged`/`obs.alert_acked` audit rows. | FR-OBS-008 (compliance views) + FR-OBS-009 (chain-of-custody manifest). |
...
- **P0 implementation phase kicks off — three new Rust workspace members shipped.** Per Stephen's "cyberos implement AI Gateway + OBS + MCP Gateway" directive (2026-05-19 evening session), three foundational P0 services now exist under `services/` as workspace members alongside the previously-shipped `auth`/`memory`/`skill-broker` crates:
  - `services/ai-gateway/` — **FR-AI-001 + FR-AI-002 + FR-AI-003 + FR-AI-004 + FR-AI-005 + FR-AI-006 + FR-AI-007 + FR-AI-008 + FR-AI-009 + FR-AI-010 + FR-AI-011 + FR-AI-012 + FR-AI-013 shipped end-to-end.** FR-AI-003 is the canonical Python memory Writer subprocess bridge with NFC normalisation + SHA-256 chain verification + path-traversal guard + 5s timeout w/ SIGTERM-then-SIGKILL + typed builders for the slice-1 closed set (`ai.precheck` · `ai.invocation` · `ai.invocation_failed` · `ai.reconcile_started` · `ai.reconcile_completed` · `ai.reconcile_failed` · `ai.hold_expired` · `ai.persona_loaded`). FR-AI-005 is the per-tenant YAML policy loader with `ArcSwap`-backed lock-free cache + `notify` file-watcher + 10 ACs covered by integration tests + `gen-schema` CI gate + `cyberos-ai policy validate \| list \| serve` CLI. FR-AI-006 adds closed-set model-alias resolution with primary/fallback/override semantics, ZDR/residency/cost-table enforcement, deterministic/concurrent tests, metrics, and sub-microsecond benchmark coverage. FR-AI-007 adds the YAML-backed provider cost table with Decimal rates, aggregate validation, hot reload, metrics, and sub-microsecond lookup benchmark coverage. FR-AI-008 adds Bedrock/Anthropic/OpenAI HTTP provider calls, retry/failover, deadline enforcement, `Retry-After`, trace-header propagation, and normalized responses. FR-AI-009 adds per-provider/model circuit breakers with half-open probes, router filtering/outcome recording, metrics, deterministic transition events, and ~30ns hot-path benchmark coverage. FR-AI-010 adds streaming router retry/failover, OpenAI/Anthropic/Bedrock SSE normalization, token/usage/done event flow, and a first-token latency bench. FR-AI-011 adds the Presidio sidecar redaction gate before normal/streaming provider dispatch, loopback-only sidecar config validation, no-prompt error/log sanitization, tool-call argument restoration maps, and a redaction latency bench. FR-AI-012 adds VN Presidio recognizers for CCCD/MST/phone/NĐD/address/bank account, per-type recall floor fixtures, sidecar version metadata, no-network recognizer lint, tenant PII allowlists, and regenerated policy schema. FR-AI-013 adds the CI recall gate with exact 50/30/40/20/40/20 recognizer distribution, 30 negatives, manifest version pins, fixture invariants, JSON artifacts, no-real-PII pre-commit hook, self-gated workflow, and quarterly refresh reminder.
  - `services/obs-collector/` — **FR-OBS-001 shipped after live LGTM verification.** `deploy/obs/` now contains the live Docker Compose stack, CyberOS OTLP ingress gate, backend configs, Grafana datasources, token rotation, and shell tests. The service-token gate has focused Rust tests plus local live verification for 401/403/200 behavior, Tempo smoke, self-metric emission, and 100/100 file-buffer replay across Tempo outage plus collector restart.
...
|---|---|:-:|:-:|---|---:|
| **FR-AI-001** | AI Gateway cost-ledger pre-call check | MUST | done | FR-AI-003, FR-AI-005, FR-AI-007 | 8h |
...
| FR-ID | Title | Pri | Status | Depends on | Effort |
|---|---|:-:|:-:|---|---:|
| **FR-AI-006** | Model-alias resolution (`chat.smart` → `bedrock:claude-3.5-sonnet`) with per-tenant override | MUST | done | FR-AI-005, FR-AI-007 | 6h |
...
| **FR-AI-019** | Self-hosted BGE-M3 embeddings (single L4 GPU pod) + CPU fallback | SHOULD | ready_to_implement | — | 8h |
...
| FR-ID | Title | Pri | Status | Depends on | Effort |
|---|---|:-:|:-:|---|---:|
| FR-OBS-004 | LangSmith integration for AI traces (tied to operational trace_id) | MUST | done | FR-AI-022 | 5h |
| FR-OBS-005 | Trace × log × metric × AI-trace correlation via W3C TraceContext propagation | MUST | done | FR-OBS-003, FR-OBS-004 | 6h |
| FR-OBS-006 | Tail-based sampling (100% on errors, 10% normal) via OTel Collector | SHOULD | done | **FR-OBS-001** | 4h |

#### Slice 3 — auto-runbook router + compliance surfaces

| FR-ID | Title | Pri | Status | Depends on | Effort |
|---|---|:-:|:-:|---|---:|
| FR-OBS-007 | Alert Manager → CUO `obs.triage-alert@1` skill routing (≥ 0.70 conf → CHAT; else PagerDuty) | MUST | done | FR-OBS-003 | 8h |
...
| FR-ID | Title | Pri | Status | Depends on | Effort |
|---|---|:-:|:-:|---|---:|
| FR-AUTH-001 | Tenant create (root-admin in tenant 0 calls `POST /v1/admin/tenants`) | MUST | ready_to_implement | — | 6h |
| FR-AUTH-002 | Subject create (`POST /v1/admin/subjects`) with bcrypt hashed password | MUST | ready_to_implement | FR-AUTH-001 | 5h |
| FR-AUTH-003 | RLS enforcement at every table (Postgres `current_setting('app.tenant')` predicate) | MUST | ready_to_implement | FR-AUTH-001 | 8h |
| FR-AUTH-004 | JWT issuance + JWKS endpoint (RS256) with `tenant_id` + `agent_persona` + `scope_grants` claims | MUST | ready_to_implement | FR-AUTH-002 | 6h |
| FR-AUTH-005 | Admin REST: list tenants + list subjects + revoke subject | MUST | ready_to_implement | FR-AUTH-001, FR-AUTH-002 | 5h |
| FR-AUTH-006 | `cyberos-auth bootstrap` CLI for tenant-0 root-admin (no UI required) | MUST | ready_to_implement | FR-AUTH-001 | 3h |
...
| FR-ID | Title | Pri | Status | Depends on | Effort |
|---|---|:-:|:-:|---|---:|
| FR-MCP-001 | MCP 2025-11-25 spec compliance — `tools/list`, `tools/call`, `capabilities` | MUST | ready_to_implement | FR-AUTH-004 | 12h |
...
| FR-ID | Title | Pri | Status | Depends on | Effort |
|---|---|:-:|:-:|---|---:|
| FR-MEMORY-101 | Layer-2 ingest pipeline (binlog → pgvector + Apache AGE) with chain-anchor verification | MUST | ready_to_implement | — | 6h |
| FR-MEMORY-102 | Layer-2 rebuild-from-Layer-1 CI gate with deterministic rebuild + spot-check | MUST | ready_to_implement | FR-MEMORY-101 | 6h |
| FR-MEMORY-103 | memory-sync daemon — laptop A ↔ Cloud memory ↔ laptop B with sync_class gating | MUST | ready_to_implement | FR-MEMORY-101 | 4h |
| FR-MEMORY-104 | Tauri 2.x desktop app — signed/notarised client + tray + quick capture | MUST | ready_to_implement | FR-MEMORY-103 | 6h |
| FR-MEMORY-105 | cyberos doctor watched-folders integrity invariants | MUST | ready_to_implement | FR-MEMORY-101 | 4h |
| FR-MEMORY-106 | memory sync_class enforcement — private vs shareable + ACL filtering | MUST | ready_to_implement | FR-MEMORY-101, FR-MEMORY-103 | 4h |

#### Stage 2 — Capture daemon (FS watcher + Cowork hook + Claude Code hook)

| FR-ID | Title | Pri | Status | Depends on | Effort |
|---|---|:-:|:-:|---|---:|
| FR-MEMORY-107 | memory capture daemon FS watcher with rate-limit + content-dedup + backpressure | MUST | ready_to_implement | FR-MEMORY-102 | 12h |
| FR-MEMORY-108 | memory search API — vector + graph + full-text recall with RRF fusion | MUST | ready_to_implement | FR-MEMORY-107 | 6h |
| FR-MEMORY-109 | Claude Code hook capture emits prompt/tool/stop memories with trace correlation | MUST | ready_to_implement | FR-MEMORY-107 | 5h |
| FR-MEMORY-110 | Capture daemon supervision — health check + restart on crash | MUST | ready_to_implement | FR-MEMORY-107 | 4h |
...
| FR-PROJ-002 | memory-anchored `proj.decision` row per Issue state change | MUST | ready_to_implement | FR-PROJ-001 | 16h |
...
| FR-AUTH-103 | SAML 2.0 SSO (per-tenant IdP config) | MUST | ready_to_implement | FR-AUTH-004 | 12h |
...
| FR-AUTH-106 | Impossible-travel detection + adaptive challenge | SHOULD | ready_to_implement | FR-AUTH-002 | 8h |
