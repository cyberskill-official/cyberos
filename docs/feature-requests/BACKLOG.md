...
| `services/obs-collector/` | Rust workspace member (NEW 2026-05-19) | **FR-OBS-001 ready_to_test.** `deploy/obs/` now contains the slice-1 Docker Compose LGTM stack (CyberOS OTLP ingress gate + otelcol-contrib + Loki + Prometheus + Tempo + Grafana), retention configs, Grafana datasources, token rotation, health/smoke/auth/restart tests, and file-backed exporter queues. `cyberos-obs ingress` enforces per-service bearer-token-to-`service.name` binding for OTLP/HTTP and OTLP/gRPC before forwarding through an internal collector token. `cyberos-obs validate-config` + `validate-tokens` pre-flight gates remain green. Local live ingress verification passed; full LGTM smoke is pending Docker image pull on the host. | FR-OBS-002 (Grafana tenant-aware proxy) + FR-OBS-003 (RED metrics SDK) + FR-OBS-004 (LangSmith) + FR-OBS-005 (TraceContext correlation) + FR-OBS-006 (tail sampling) + FR-OBS-007 (Alertmanager → CUO routing) + FR-OBS-008 (compliance views) + FR-OBS-009 (chain-of-custody manifest). |
