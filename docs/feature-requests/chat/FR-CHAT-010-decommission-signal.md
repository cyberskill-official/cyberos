...
- **FR-CHAT-006** — `imported_slack` source flag.
- **FR-CHAT-007** — `imported_zalo` source flag.
- **FR-OBS-007** — sev-3 routing.
...
| OBS alert burst on multi-tenant transition (50 tenants Ready same day) | 50 SEV-3 alerts | digest grouping (FR-OBS-007) | None |
...
- For tenants with multiple workspaces imported (FR-CHAT-006/007 workspace_id), the signal aggregates across workspaces. Per-workspace decommission is slice-4+.
