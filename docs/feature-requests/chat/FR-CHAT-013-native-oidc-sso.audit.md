...
### ISS-004 - The kick was the FR-CHAT-002 overclaim (RESOLVED)
Resolved: DEC-2503 + §1 #6 + §9 - revoke blocks re-login immediately (provider authorize refuses); an open Mattermost session dies via back-channel logout (FR-AUTH-110 slice 2) or the session TTL; SCIM (FR-PORTAL-004) is instant cross-app. Stated plainly, no overclaim.
...
6 of 6 concerns resolved. The supersede linkage (FR-CHAT-002 closed; this FR's `supersedes` + the banner) is clean, the kick boundary is honest, and the free-vs-enterprise connector reality is handled rather than assumed. The FR's depth is bounded by its real surface (one provider, one connector, config + an owner-run sign-in), not by a line target.
