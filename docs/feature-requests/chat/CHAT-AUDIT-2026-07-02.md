...
| 4 | Desktop / web-push notifications | High | M | C + B + X | Browser `Notification` + VAPID web-push; APNS/FCM for mobile is separate (`push.rs`, FR-CHAT-011). Depends on #1. |
...
| 16 | Admin / moderation console + retention + DSAR export + report/suspend | Medium | L | B + C | Only role-delete + audit read exist today (`messages.rs`, `auditlog.rs`); FRs exist (FR-CHAT-012). |
...
- "Missing" means not found in the read files (`services/chat/src/*`, `services/chat/migrations/*`, `apps/web/src/*`). A few features exist as authored FRs under `docs/feature-requests/chat/` (mobile push, DSAR export, Lumi mention, imports) but have no code in the module today; those are flagged inline.
