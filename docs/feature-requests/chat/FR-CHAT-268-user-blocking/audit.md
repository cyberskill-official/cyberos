...
FR-CHAT-268 specifies user blocking: 15 normative §1 clauses, one migration (`chat_blocks`, directional,
...
often the administrator. If `blocked_by` were applied uniformly, the admin would open FR-CHAT-269's queue and
find the reported content redacted - by their own block. Resolved: §1 #12 carves the moderation queue out
explicitly, §7 makes it a normative constraint *on FR-CHAT-269* rather than a note, and AC #12 asserts the
