...
source_decisions:
  ...
  - DEC-481 (reuse FR-CHAT-006 step pattern; Zalo-specific parser per source)
...
**Why NFC normalisation (§1 #4)?** Vietnamese text often has combining diacritics (NFD) mixed with composed forms (NFC). Search + comparison need consistent form. NFC is the canonical choice (matches FR-CHAT-004 tokeniser).
...
**Why preserve membership events (§1 #24)?** FR-CHAT-008 mention resolution and FR-CHAT-012 DSAR both need user-channel membership timeline. Zalo exports surface these as `<div class="event">` elements; capturing them mirrors the Slack importer's channelmember support.
...
### §6.1 — Reuse of FR-CHAT-006 infrastructure
...
### §6.12 — Failure routing matrix (mirrors FR-CHAT-006 §6.12)

Same as FR-CHAT-006 with adjustments:
...
- Step 6 (verify) samples per FR-CHAT-006 §1 #28.

---

## §7 — Dependencies

- **FR-CHAT-006** — checkpoint table + step pattern reused.
- **FR-CHAT-005** — bridge picks up imported posts.
...
- `unicode-normalization` crate provides `nfc()` iterator. NFC is the canonical form for Vietnamese text — FR-CHAT-004's bigram tokenizer assumes NFC. Without normalisation, a search for "cà phê" wouldn't match a NFD-encoded message containing the same word.
...
- We chose UUID-v7 for `import_jobs.id` so per-tenant import lists sort chronologically — same rationale as FR-CHAT-006.
...
- For long-running imports, we considered emitting a per-conversation progress row (e.g. `chat.import_conversation_completed`) but the heartbeat from FR-CHAT-005 plus the per-step `chat.import_step_completed` already covers operator visibility.
