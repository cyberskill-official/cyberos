...
A user typing `@lumi do A and @lumi do B` could either trigger two CUO calls (wasteful) or one (sensible). Original spec didn't say. Resolved: §1 #20 + count_mentions helper + single CUO invocation; AC #26 + test body verify; BRAIN audit records mention_count for visibility.
...
Original spec metric `chat_lumi_mentions_total{outcome=error}` increments but no BRAIN audit row, so operator post-mortem requires log digging. Resolved: §1 #23 + `chat.lumi_error` BRAIN row with error_class + redacted error_message + SEV-2 escalation on sustained rate; AC #29 + test body verify; §6.10 documents severity routing.

## §3 — Resolution

All 16 mechanical concerns addressed. **Score = 10/10.**

Per AUTHORING.md §0 master rule: spec is now perfect — depth bounded by the genuine surface (LLM persona flow + tenant gating + rate limiting + placeholder UX + multi-hop tracing + builtin commands + budget capture + error observability), not by line targets.
