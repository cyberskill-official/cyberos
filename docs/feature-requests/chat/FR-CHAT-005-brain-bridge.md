---
id: FR-CHAT-005
title: "BRAIN bridge — Postgres logical replication from chat to BRAIN Layer-3 ingest with p95 ≤ 5s latency"
module: CHAT
priority: MUST
status: accepted
verify: T
...
source_decisions:
  - DEC-460 (Postgres logical replication; chat → bridge process → BRAIN; never direct app-side write)
  - DEC-461 (sync_class derived from channel privacy; private channel = sync_class private)
  - DEC-462 (p95 chat-msg-to-BRAIN ≤ 5s; sev-2 alarm above)

language: rust 1.81
service: cyberos/services/chat-brain-bridge/
new_files:
  - services/chat-brain-bridge/Cargo.toml
  - services/chat-brain-bridge/src/main.rs
  - services/chat-brain-bridge/src/replication.rs
  - services/chat-brain-bridge/src/map.rs
  - services/chat-brain-bridge/tests/bridge_test.rs
modified_files:
  - infra/terraform/modules/tenant_chat/rds.tf      # enable wal_level=logical
  - services/chat/sql/init-bridge-publication.sql
allowed_tools:
  - file_read: services/chat-brain-bridge/**
  - file_write: services/chat-brain-bridge/{src,tests}/**, services/chat/sql/**
  - bash: cd services/chat-brain-bridge && cargo test
disallowed_tools:
  - skip PII redaction before BRAIN write (per FR-BRAIN-111)
  - write to BRAIN bypassing FR-BRAIN-107 capture daemon (per AGENTS.md §14.1)
...
sub_tasks:
  ...
  - "1.5h: map.rs — map MM post → BRAIN row {kind: chat.message, payload: {channel_id, user_id, body_redacted, sync_class, trace_id}}"
  - "0.5h: integration with FR-BRAIN-107 capture daemon via Unix socket"
  - "0.5h: BRAIN audit kinds (chat.message | chat.channel_created | chat.channel_archived)"
  - "0.5h: latency tracking: msg.create_at vs BRAIN seq emit time"
  - "1.5h: bridge_test.rs — happy + lag detection + PII redaction"
  - "0.5h: OTel histogram chat_brain_bridge_latency_seconds"
risk_if_skipped: "Chat messages stay in chat DB forever; never indexed for cross-chat search / cross-module retrieval. Without async bridge, app-side dual-write = transaction complexity. Without redaction, customer PII lands in BRAIN. Without 5s SLA, search is stale at the surface. Logical replication keeps the chat app's write path fast (no synchronous external call)."
---

## §1 — Description (BCP-14 normative)

The bridge **MUST** consume Postgres logical replication from chat DB and emit BRAIN rows via FR-BRAIN-107 socket. The contract:
...
4. **MUST** parse incoming WAL events:
    - `INSERT posts` → emit `chat.message` BRAIN row.
...
6. **MUST** redact PII via FR-BRAIN-111 ruleset BEFORE emit.
7. **MUST** propagate W3C trace_id: read from post's `props.cyberos_trace_id` if present; else generate new at bridge.
8. **MUST** track latency: measure `now() - post.create_at` at emit time; emit histogram `chat_brain_bridge_latency_seconds`.
9. **MUST** emit BRAIN audit `chat.bridge_lag_alert` when p95 latency > 5s sustained 60s (sev-2 routing via FR-OBS-007).
...
12. **MUST** emit OTel metrics:
    - `chat_brain_bridge_messages_total{kind, outcome}` (counter; outcome ∈ emitted | redaction_failed | brain_write_failed).
    - `chat_brain_bridge_latency_seconds`.
    - `chat_brain_bridge_lag_lsn` (gauge; chat WAL LSN minus consumed LSN).
13. **MUST** handle BRAIN socket unavailable: bridge pauses (does NOT advance LSN); resumes when socket up; alarm sev-2 if down > 60s.
14. **MUST** be idempotent at the message level: re-emit of the same `(tenant_id, post_id, version)` triple MUST produce a BRAIN row whose `dedup_key` field equals the prior row's `dedup_key`. FR-BRAIN-107 uses `dedup_key` to drop duplicates that arise from restart-mid-LSN-advance.
15. **MUST NOT** advance the replication slot past an LSN whose BRAIN emit ack has not been received. The bridge maintains a `pending_acks: BTreeMap<Lsn, AckHandle>` and only calls `pg_replication_slot_advance` for LSNs whose ack has resolved. This is the source of the exactly-once-modulo-dedup guarantee.
...
18. **MUST** emit `chat.user_joined_channel` and `chat.user_left_channel` rows from `channelmembers` table INSERT / DELETE events. These are required by FR-CHAT-008 mention resolution and FR-CHAT-012 DSAR export.
...
21. **MUST** support graceful shutdown via SIGTERM:
    ...
    - Drain in-flight BRAIN emits with a 10s grace window.
    - Persist final LSN to BRAIN as `chat.bridge_shutdown` row.
    - Exit 0.
...
25. **MUST NOT** read the chat DB's `Sessions` table — login events are owned by FR-CHAT-002, not by the bridge. The publication scope is explicitly `posts, channels, reactions, channelmembers, fileinfo`; no other tables.
...
**Why p95 ≤ 5s (DEC-462)?** Users searching BRAIN expect "I just said it in chat" to be findable within seconds. 5s is the calibrated SLA; alarm above forces investigation.

**Why persist LSN (§1 #10)?** Restart-loss = silent data loss. LSN persistence = exactly-once delivery (modulo BRAIN-write idempotency).

**Why pause on BRAIN down (§1 #13)?** Advancing LSN without successful BRAIN write = data loss on permanent failure. Pause = back-pressure + data integrity.

**Why dedup_key (§1 #14)?** Even with pause-on-failure, a bridge crash AFTER BRAIN ack but BEFORE LSN advance causes re-emit on restart. FR-BRAIN-107 needs a deterministic key to dedup — we compute it as `sha256(tenant_id || post_id || version)` so the same logical event always produces the same key.

**Why pending_acks BTreeMap (§1 #15)?** BRAIN emits are async; if we advance the LSN past an unacked message, a crash loses it. The BTreeMap keeps LSNs in order so we can only advance to the highest-acked contiguous LSN.
...
**Why channelmember events (§1 #18)?** FR-CHAT-008 mentions resolve from channelmembers (who can see a mention?). FR-CHAT-012 DSAR needs the timeline of "what channels did user X access." Without these events, those FRs would have to query the chat DB directly — coupling we want to avoid.
...
**Why heartbeat row (§1 #20)?** Operators answering "is the bridge running" without OBS dashboard access need a BRAIN-side signal. A heartbeat every 30s is far cheaper than tailing logs.
...
**Why scope publication explicitly (§1 #25)?** Default `ALTER PUBLICATION chat_bridge FOR ALL TABLES` would include Mattermost's `Sessions` table — leaking login events to BRAIN that should be owned by FR-CHAT-002. Explicit table list keeps the contract surface narrow.
...
```toml
[package]
name    = "cyberos-chat-brain-bridge"
version = "0.1.0"
edition = "2021"
rust-version = "1.81"
...
cyberos-brain-client = { path = "../../crates/cyberos-brain-client" }
cyberos-brain-pii    = { path = "../../crates/cyberos-brain-pii" }
...
```sql
-- services/chat/sql/init-bridge-publication.sql
-- Owned by FR-CHAT-005; executed once per tenant at first deploy
-- via cyberos-chat-bridge --init-publication.

BEGIN;

-- Publication scope is EXPLICIT — not FOR ALL TABLES.
-- Per FR-CHAT-005 §1 #25, Sessions table is NOT in this set.
...
#[derive(Parser, Debug)]
#[command(name = "cyberos-chat-brain-bridge")]
struct Args {
    ...
    /// BRAIN writer Unix socket / TCP endpoint (FR-BRAIN-107).
    #[arg(long, env = "CYBEROS_BRAIN_WRITER_SOCK")]
    brain_socket: String,
...
async fn main() -> Result<()> {
    ...
    let brain = cyberos_brain_client::connect(&args.brain_socket).await?;
    let shutdown_rx = install_signal_handlers().await;

    let mut backoff = Duration::from_millis(500);
    loop {
        match run_bridge(&args, &brain, shutdown_rx.clone()).await {
            ...
            Err(e) => {
                ...
                metrics::counter!("chat_brain_bridge_reconnects_total",
...
async fn replica_safety_check(client: &tokio_postgres::Client) -> Result<()> {
    ...
    if in_recovery {
        cyberos_brain_client::emit(BrainRow {
...
pub async fn run_replication_loop(
    ...
    tenant_id: uuid::Uuid,
    brain: &cyberos_brain_client::Client,
        ...
        tokio::select! {
            ...
            _ = shutdown_rx.changed() => {
                if *shutdown_rx.borrow() {
                    ...
                    brain.emit(brain_row_shutdown(tenant_id, highest_committed_lsn)).await?;
            ...
            _ = heartbeat_ticker.tick() => {
                ...
                brain.emit(brain_row_heartbeat(tenant_id, highest_committed_lsn, lag)).await?;
                metrics::gauge!("chat_brain_bridge_lag_lsn").set(lag.bytes as f64);
            }
            ...
            msg = stream.next() => {
                ...
                match parsed {
                    ReplicationMessage::XLogData(xlog) => {
                        ...
                        let row   = map_event_to_brain_row(event.clone(), tenant_id)?;

                        // PII redaction BEFORE emit. Failure = drop + audit.
                        let normalised = normalise_mattermost_specials(&row.body);
                        let redacted = match cyberos_brain_pii::scan_and_redact(&normalised, &[]).await {
                            Ok(r) => r,
                            Err(e) => {
                                error!(?e, post_id = ?event.post_id(), "pii redaction failed; dropping");
                                brain.emit(brain_row_redaction_failed(tenant_id, event.post_id(), e.to_string())).await?;
                                metrics::counter!("chat_brain_bridge_messages_total",
                                    "kind" => row.kind.clone(),
                        ...
                        brain.emit_with_ack(final_row, ack_tx).await?;
...
pub fn map_event_to_brain_row(
    ...
    let row = match event {
        ...
        ReplicationEvent::ChannelInsert(c) => BrainRow {
        ...
        ReplicationEvent::ChannelArchive(c) => BrainRow {
        ...
        ReplicationEvent::ReactionInsert(r) => BrainRow {
        ...
        ReplicationEvent::ReactionDelete(r) => BrainRow {
        ...
        ReplicationEvent::ChannelMemberInsert(m) => BrainRow {
        ...
        ReplicationEvent::ChannelMemberDelete(m) => BrainRow {
        ...
        ReplicationEvent::FileInfoInsert(f) => BrainRow {
...
```rust
// services/chat-brain-bridge/src/lsn.rs
...
3. **INSERT post → chat.message row in BRAIN within 5s**.
...
10. **PII redacted (email in message → <EMAIL> in BRAIN row)**.
11. **trace_id from props.cyberos_trace_id when present**.
12. **trace_id generated when absent**.
13. **LSN advances on successful BRAIN write**.
14. **LSN NOT advanced on BRAIN failure** — fixture: kill brain socket; insert message; restart; replay.
...
21. **Edit emits chat.message_edited with prior_dedup_key** — fixture: insert post; update message; observe BRAIN row whose payload `prior_dedup_key` equals dedup_key of original (AC for §1 #16).
22. **Reaction insert → chat.message_reacted** — fixture: insert reaction; observe BRAIN row with `emoji_name` (AC for §1 #17).
23. **channelmember insert → chat.user_joined_channel** — fixture: add user to channel; observe BRAIN row (AC for §1 #18).
24. **channelmember delete → chat.user_left_channel** — fixture: remove user from channel; observe BRAIN row (AC for §1 #18).
25. **Read-replica connection refused at startup** — fixture: point DSN at replica; observe SEV-1 `chat.bridge_misconfigured` BRAIN row + process exit non-zero (AC for §1 #19).
26. **Heartbeat row every 30s** — fixture: idle bridge; observe `chat.bridge_heartbeat` row at t≈30s (AC for §1 #20).
27. **SIGTERM drains in 10s** — fixture: send 100 messages then SIGTERM; observe all 100 emitted to BRAIN before process exits; observe `chat.bridge_shutdown` row (AC for §1 #21).
...
31. **Sessions table NOT in publication** — fixture: insert into Sessions; observe no BRAIN row (AC for §1 #25).
32. **Crash mid-LSN-advance produces duplicate emit, dedup_key identical** — fixture: kill bridge after BRAIN ack but before slot advance; restart; observe two emits with same dedup_key; FR-BRAIN-107 dedup retains one.
...
34. **chat.bridge_lag_alert payload includes lag_bytes + lag_seconds** — fixture: stall BRAIN; wait for alarm; observe payload completeness.
...
`tests/bridge_test.rs` spins up a real Postgres + fake BRAIN socket via `testcontainers`. Helpers are in `tests/common/mod.rs`.
...
impl TestEnv {
    ...
    // ... ensure_channel, ensure_user, kill_brain_socket, restart_brain_socket, ...
}
```
...
async fn ac3_happy_message_to_brain() {
    let env = TestEnv::new().await;
    let post_id = env.insert_chat_post("O", "Hello, world").await;
    let row = env.brain.wait_for_kind_predicate("chat.message",
...
async fn ac4_message_deleted() {
    ...
    env.brain.wait_for_kind("chat.message", Duration::from_secs(6)).await.unwrap();
    env.update_post(&post_id, "delete_at", now_ns()).await;
    env.brain.wait_for_kind_predicate("chat.message_deleted",
        |r| r["payload"]["post_id"] == post_id,
        Duration::from_secs(6)).await.unwrap();
}
```
...
async fn ac7_8_9_sync_class(#[case] channel_type: &str, #[case] expected: &str) {
    ...
    let row = env.brain.wait_for_kind("chat.message", Duration::from_secs(6)).await.unwrap();
...
### AC #14 — LSN pause on BRAIN failure

```rust
#[tokio::test(flavor = "multi_thread")]
async fn ac14_lsn_not_advanced_on_brain_failure() {
    let env = TestEnv::new().await;
    env.brain.stop().await;
    ...
    assert_eq!(env.bridge_consumed_lsn().await, initial_lsn,
        "LSN advanced while BRAIN socket was down");
    env.brain.start().await;
    let row = env.brain.wait_for_kind("chat.message", Duration::from_secs(15)).await.unwrap();
...
async fn ac20_lsn_advance_gated_on_contiguous_acks() {
    let env = TestEnv::new().await;
    env.brain.set_ack_policy(AckPolicy::AckOnly(vec![/* second emit only */]));
...
async fn ac21_message_edited_prior_dedup_key() {
    ...
    let row_v1 = env.brain.wait_for_kind("chat.message", Duration::from_secs(6)).await.unwrap();
    let dk1 = row_v1["payload"]["dedup_key"].as_str().unwrap().to_owned();

    env.update_post_message(&post_id, "v2").await;
    let row_edit = env.brain.wait_for_kind("chat.message_edited", Duration::from_secs(6)).await.unwrap();
...
async fn ac25_read_replica_refused() {
    ...
    let row = env.brain.wait_for_kind("chat.bridge_misconfigured",
...
async fn ac26_heartbeat_every_30s() {
    let env = TestEnv::new().await;
    let t1 = env.brain.wait_for_kind("chat.bridge_heartbeat",
        Duration::from_secs(35)).await.unwrap();
    let t2 = env.brain.wait_for_kind_after(t1.ts_ns, "chat.bridge_heartbeat",
        Duration::from_secs(35)).await.unwrap();
...
async fn ac27_sigterm_drains_in_10s() {
    ...
    let rows = env.brain.collected_rows_of_kind("chat.message").await;
    assert_eq!(rows.len(), 100, "drained {} of 100 messages", rows.len());
    let shutdown = env.brain.collected_rows_of_kind("chat.bridge_shutdown").await;
...
async fn ac31_sessions_not_in_publication() {
    ...
    env.pg.client().await.execute(
        "INSERT INTO Sessions(id, token, user_id, create_at)
              VALUES ('s1', 'tok', 'u1', $1)",
        &[&now_ns()]).await.unwrap();
    let result = env.brain.wait_for_any(Duration::from_secs(5)).await;
    assert!(matches!(result, Err(_)) || result.unwrap().kind != "chat.session_started",
        "bridge leaked Sessions table to BRAIN");
}
```
...
async fn ac32_crash_mid_advance_dedups_on_restart() {
    ...
    let row1 = env.brain.wait_for_kind("chat.message", Duration::from_secs(6)).await.unwrap();
    ...
    let row2 = env.brain.wait_for_kind_after(row1.ts_ns, "chat.message",
        Duration::from_secs(10)).await.unwrap();
    assert_eq!(row2["payload"]["dedup_key"], dk,
        "dedup_key non-deterministic across restart");
    // FR-BRAIN-107 will dedup these on the read side; we just verify the key matches.
}
```
...
Reasons classified for the `chat_brain_bridge_reconnects_total` counter:
- `pg_disconnect` — Postgres connection lost.
- `brain_disconnect` — BRAIN socket lost.
...
### §6.5 — BRAIN client interface

The bridge consumes `cyberos-brain-client::emit_with_ack(row, ack_tx)` (a workspace crate, owned by FR-BRAIN-107). The client serialises the row, frames it per FR-BRAIN-107 wire format, sends, and resolves the `ack_tx` when the BRAIN writer acks. Ack timeout is 5s; on timeout the ack resolves `false` (which causes the bridge to stop advancing the LSN past that emit).

### §6.6 — PII allowlist propagation

FR-AGENTS.md §3.6 allows tenant-scoped PII allowlists. The bridge reads `<brain-root>/manifest.json::tenants[tenant_id].pii_allowlist` at startup and passes it to `cyberos_brain_pii::scan_and_redact(body, &allowlist)`. Reload on SIGHUP (or BRAIN-side hot-reload event).

### §6.7 — Init publication command

`cyberos-chat-bridge --init-publication --chat-dsn <dsn>` runs `init-bridge-publication.sql` and exits. Invoked once by FR-CHAT-003 Terraform via `null_resource` after RDS is `available`. Idempotent (DROP IF EXISTS + CREATE).
...
The bridge process is launched with `CYBEROS_TENANT_ID=<uuid>` env var. The replication slot is named with that UUID. The BRAIN client connects with a per-tenant credential. There are three layers of enforcement:
...
3. **BRAIN write scope:** the BRAIN client refuses to emit a row whose `tenant_id` doesn't match its credential's tenant.
...
- **FR-CHAT-003** — RDS host for the publication.
- **FR-BRAIN-107** — capture daemon socket consumer.
- **FR-BRAIN-111** — PII redaction.
- **FR-OBS-007** — alarm routing.
...
| BRAIN socket down | emit Err | LSN paused; pending_acks grows; SEV-2 after 60s | Operator restores BRAIN; bridge auto-resumes |
| BRAIN socket slow (ack > 5s) | ack timeout | LSN paused; pending_acks grows | Investigate BRAIN writer load; consider FR-BRAIN-107 scale-up |
...
| Duplicate emit | bridge crash mid-LSN-advance | dedup_key identical; FR-BRAIN-107 dedups | None |
| Tenant_id wrong | startup config | bridge starts but emits with wrong tenant | BRAIN client refuses (defense-in-depth); SEV-1 |
...
| pgoutput protocol-version mismatch | parse Err with version code | startup fails; SEV-1 | Coordinate with FR-BRAIN-107 schema update |
...
| Mattermost upgrade adds a new table consumed by FR-CHAT-008 | not in current publication | new events not emitted until publication ALTER | Operator runs ALTER PUBLICATION + bridge restart |
...
| Network partition Fargate ↔ BRAIN socket | emit Err | LSN paused; SEV-2 after 60s | Heals automatically |
...
| Heartbeat row schema rejected by BRAIN (FR-AI-003 closed set drift) | BRAIN 422 | heartbeat lost; logged warn | Coordinate with FR-AI-003 to allow `chat.bridge_heartbeat` |
...
- Bridge runs as separate Fargate task per tenant; ECS launches via FR-CHAT-003 module. Co-located with the chat server in the same task definition so they scale together (load on chat = load on bridge).
- The 5-second SLA is end-to-end (chat insert → BRAIN row); typical observed ~500ms. The budget breaks down as: Postgres WAL write (1ms) + bridge pgoutput decode (5ms) + PII redact (30ms) + BRAIN socket emit + ack (~100ms typical) = ~140ms median. The 5s headroom absorbs occasional BRAIN GC pauses and PII-rule cold starts.
- Bridge_lag_alert dedup'd by FR-OBS-007 (no spam during sustained outage). FR-OBS-007 expects the `chat.bridge_lag_alert` row's payload to carry `sustained_seconds`; this gates re-alarming.
...
- `dedup_key = sha256(tenant_id || post_id || version)` where `version` is `update_at`. This means a post and its edit produce different keys, which is intentional — the edit is a new event to FR-BRAIN-107.
- The `pending_acks: BTreeMap<u64, oneshot::Receiver<bool>>` is bounded implicitly by BRAIN's ack rate. If BRAIN slows to a crawl, the map grows; at 100K entries memory pressure becomes notable (~16MB). We don't set a hard cap because dropping pending entries would lose data; instead, the bridge_lag_alert fires long before memory becomes an issue.
...
- We don't use `wal2json` even though FR-CHAT-003 preloads it, because `pgoutput` is more efficient (binary protocol vs JSON parsing); `wal2json` is preloaded for other downstream consumers (slice 4+ analytics pipeline).
...
- The `metrics-exporter-prometheus` is paired with the Cloudwatch agent in FR-CHAT-003 obs sidecar; metrics live in CloudWatch namespace `CyberOS/Chat/Bridge`.
- We chose `uuid::Uuid::now_v7()` (time-ordered UUIDs) over `v4` for fake-BRAIN socket test IDs so test failure logs are chronologically sortable.
...
- Why we emit `chat.bridge_shutdown` instead of just letting the process exit: operators investigating "did the bridge crash or shut down cleanly?" need a BRAIN-side signal. Heartbeat absence is ambiguous (could be network); explicit shutdown row removes ambiguity.
...
- `dedup_key` lives in the row body, not as a top-level field, because FR-BRAIN-107's wire protocol carries it in the payload envelope; the FR-BRAIN-107 dedup index then promotes it to a top-level filter.
- The `force-replay` runbook (`cyberos-chat-bridge force-replay --from-lsn <lsn>`) is the recovery path for PII redaction-rule corrections: the bridge replays a range of LSNs, producing rows with the same dedup_keys; FR-BRAIN-107's dedup will collapse to the most recent (replay-replaces-original semantic is owned by FR-BRAIN-107).
