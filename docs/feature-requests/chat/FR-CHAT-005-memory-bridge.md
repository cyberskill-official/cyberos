...
related_frs: [FR-CHAT-003, FR-CHAT-006, FR-CHAT-008, FR-CHAT-012, FR-MEMORY-101, FR-MEMORY-107, FR-MEMORY-111]
...
disallowed_tools:
  - skip PII redaction before memory write (per FR-MEMORY-111)
  - write to memory bypassing FR-MEMORY-107 capture daemon (per AGENTS.md §14.1)
...
sub_tasks:
  ...
  - "0.5h: integration with FR-MEMORY-107 capture daemon via Unix socket"
...
The bridge **MUST** consume Postgres logical replication from chat DB and emit memory rows via FR-MEMORY-107 socket. The contract:
...
6. **MUST** redact PII via FR-MEMORY-111 ruleset BEFORE emit.
...
9. **MUST** emit memory audit `chat.bridge_lag_alert` when p95 latency > 5s sustained 60s (sev-2 routing via FR-OBS-007).
...
14. **MUST** be idempotent at the message level: re-emit of the same `(tenant_id, post_id, version)` triple MUST produce a memory row whose `dedup_key` field equals the prior row's `dedup_key`. FR-MEMORY-107 uses `dedup_key` to drop duplicates that arise from restart-mid-LSN-advance.
...
**Why dedup_key (§1 #14)?** Even with pause-on-failure, a bridge crash AFTER memory ack but BEFORE LSN advance causes re-emit on restart. FR-MEMORY-107 needs a deterministic key to dedup — we compute it as `sha256(tenant_id || post_id || version)` so the same logical event always produces the same key.
...
**Why scope publication explicitly (§1 #25)?** Default `ALTER PUBLICATION chat_bridge FOR ALL TABLES` would include Mattermost's `Sessions` table — leaking login events to memory that should be owned by FR-CHAT-002. Explicit table list keeps the contract surface narrow.
...
struct Args {
    ...
    /// memory writer Unix socket / TCP endpoint (FR-MEMORY-107).
...
32. **Crash mid-LSN-advance produces duplicate emit, dedup_key identical** — fixture: kill bridge after memory ack but before slot advance; restart; observe two emits with same dedup_key; FR-MEMORY-107 dedup retains one.
...
async fn ac32_crash_mid_advance_dedups_on_restart() {
    ...
    // FR-MEMORY-107 will dedup these on the read side; we just verify the key matches.
}
```
...
The bridge consumes `cyberos-memory-client::emit_with_ack(row, ack_tx)` (a workspace crate, owned by FR-MEMORY-107). The client serialises the row, frames it per FR-MEMORY-107 wire format, sends, and resolves the `ack_tx` when the memory writer acks. Ack timeout is 5s; on timeout the ack resolves `false` (which causes the bridge to stop advancing the LSN past that emit).
...
- **FR-MEMORY-107** — capture daemon socket consumer.
...
| memory socket slow (ack > 5s) | ack timeout | LSN paused; pending_acks grows | Investigate memory writer load; consider FR-MEMORY-107 scale-up |
...
| Duplicate emit | bridge crash mid-LSN-advance | dedup_key identical; FR-MEMORY-107 dedups | None |
...
| pgoutput protocol-version mismatch | parse Err with version code | startup fails; SEV-1 | Coordinate with FR-MEMORY-107 schema update |
...
| Heartbeat row schema rejected by memory (FR-AI-003 closed set drift) | memory 422 | heartbeat lost; logged warn | Coordinate with FR-AI-003 to allow `chat.bridge_heartbeat` |
...
- `dedup_key = sha256(tenant_id || post_id || version)` where `version` is `update_at`. This means a post and its edit produce different keys, which is intentional — the edit is a new event to FR-MEMORY-107.
...
- `dedup_key` lives in the row body, not as a top-level field, because FR-MEMORY-107's wire protocol carries it in the payload envelope; the FR-MEMORY-107 dedup index then promotes it to a top-level filter.
- The `force-replay` runbook (`cyberos-chat-bridge force-replay --from-lsn <lsn>`) is the recovery path for PII redaction-rule corrections: the bridge replays a range of LSNs, producing rows with the same dedup_keys; FR-MEMORY-107's dedup will collapse to the most recent (replay-replaces-original semantic is owned by FR-MEMORY-107).
