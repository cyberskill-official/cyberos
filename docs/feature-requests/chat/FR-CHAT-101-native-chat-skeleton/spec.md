...
created: 2026-06-29
supersedes: [FR-CHAT-001, FR-CHAT-002, FR-CHAT-003, FR-CHAT-004, FR-CHAT-005, FR-CHAT-006, FR-CHAT-007, FR-CHAT-008, FR-CHAT-009, FR-CHAT-010, FR-CHAT-011, FR-CHAT-012, FR-CHAT-013]
related_frs: [FR-AUTH-110, FR-AUTH-004, FR-AUTH-005, FR-AUTH-101]
depends_on: [FR-AUTH-110, FR-AUTH-004]
blocks: []
...
modified_files:
  ...
  - docs/feature-requests/chat/README.md             # retire FR-CHAT-001..013, point at the native series

retires:
  - "The Mattermost-fork CHAT module (FR-CHAT-001..012) and its OIDC config (FR-CHAT-013). services/chat is repurposed from the Mattermost fork to the native cyberos-chat crate; the old Mattermost scaffolding is archived, not built."
...
2. **MUST** authenticate every request with the CyberOS access token the FR-AUTH-110 provider issues,
   verified against the FR-AUTH-004 JWKS (RS256) - the same verification `obs-compliance-view` and the MCP
...
from the FR-AUTH-110 provider, Postgres with the tenant-RLS pattern, the audit chain, the obs SDK, and the
...
3. Vietnamese search (the FR-CHAT-004 goal, re-homed natively) and file attachments.
...
Upstream: FR-AUTH-110 (the provider that issues the identity token), FR-AUTH-004 (the JWKS chat verifies
against). Reuses the `cyberos-audit-chain` and `cyberos-obs-sdk` shared crates and the tenant-RLS idiom.

Retires: the Mattermost-fork CHAT module - FR-CHAT-001 through FR-CHAT-012 and FR-CHAT-013 - is superseded by
