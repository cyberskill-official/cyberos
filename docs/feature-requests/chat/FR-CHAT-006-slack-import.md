...
3. **MUST** execute 8 explicit steps in order:
    ...
    2. **Users**: parse users.json → JIT-provision Mattermost users (FR-CHAT-002 pattern; idempotent on slack_user_id).
...
The importer authenticates to MM as a per-tenant import bot (created by FR-CHAT-002 admin REST, marked `is_bot=true`). The bot has `system_admin` role for the duration of the import only; an operator post-import workflow demotes it to `system_user` after `chat.import_finished` audit row fires.
...
- Why we mark posts with `cyberos_imported=true`: FR-CHAT-005 bridge uses this to short-circuit push notifications. Without it, every historical message would push-notify every user in the channel.
