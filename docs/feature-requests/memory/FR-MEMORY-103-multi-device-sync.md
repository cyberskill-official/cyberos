...
blocks: [FR-MEMORY-104, FR-MEMORY-106, FR-MEMORY-105]
...
sub_tasks:
  ...
  - "0.5h: disputed_pair row creation on conflict (FR-MEMORY-105 surfaces UI)"
  ...
  - "1.0h: Tauri-app integration (FR-MEMORY-104) hooks"
...
4. **MUST** detect conflicts via CRDT triple `(content_hash, ts_ns, originator_device)`. Conflict = same target memory id with two different writes from different devices within overlapping wall-clock windows. On conflict, write a `disputed_pair` row with both versions; FR-MEMORY-105 surfaces the resolution UI.
...
12. **MUST** authenticate to Cloud memory via per-device bearer token (rotated quarterly; managed via FR-AUTH-006-style sweeper). Compromised token → revoke at Cloud memory; device cannot sync until new token issued.
...
- **FR-MEMORY-101** — Layer 2 sees synced rows downstream.
- **FR-MEMORY-104** — Tauri app embeds memory-sync.
- **FR-MEMORY-105** — Disputed-pair UI (slice 2+).
- **FR-MEMORY-106** — sync-class enforcement at write time (this FR enforces at sync boundary).
...
| Conflict | CRDT detect | disputed_pair row created | User picks via FR-MEMORY-105 UI |
