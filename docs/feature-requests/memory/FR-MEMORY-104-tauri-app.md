...
blocks: [FR-MEMORY-105]   # placeholder — disputed-pair UI FR, not yet specified
...
sub_tasks:
  ...
  - "1.0h: src/main.rs entry + sync_supervisor (embeds FR-MEMORY-103 daemon as tokio task)"
  ...
  - "1.0h: routes/search/+page.svelte — local search UI (FR-MEMORY-108 API)"
...
3. **MUST** run memory-sync (FR-MEMORY-103) as an internal Tauri-managed process via `tauri::async_runtime::spawn`. The supervisor monitors health; auto-restart on panic with exponential backoff.
...
5. **MUST** show on the main dashboard:
    ...
    - Disputed pair count (badge; clicks to FR-MEMORY-105 resolution UI).
...
- **FR-MEMORY-103** — sync daemon embedded.
- **FR-MEMORY-105 (downstream)** — disputed-pair UI link.
