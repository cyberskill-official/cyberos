...
blocks: [FR-MEMORY-108, FR-MEMORY-109, FR-MEMORY-110, FR-MEMORY-111]

source_pages:
  - website/docs/modules/memory.html#capture-daemon
  - website/docs/runbooks/memory-capture-runbook.html
source_decisions:
...
new_files:
  - services/memory-capture/Cargo.toml
  - services/memory-capture/src/main.rs
  - services/memory-capture/src/lib.rs
  - services/memory-capture/src/watcher.rs
  - services/memory-capture/src/dedup.rs
  - services/memory-capture/src/rate_limit.rs
...
sub_tasks:
  ...
  - "1.0h: emit.rs — bridge to FR-MEMORY-101 memory_writer; carries W3C TraceContext; retries with exp backoff"
...
13. **MUST** integrate with FR-MEMORY-105's doctor invariants: `cyberos memory capture status` invokes `doctor --only watched-folders`; refuses to start if any error-severity invariant fails.
14. **MUST** emit OTel metrics:
    ...
    - `memory_capture_emit_latency_seconds{folder_id}` (histogram; FR-OBS-003 buckets).
...
**Why 250ms debounce (§1 #2)?** Empirical: VS Code's atomic-save sequence is `tmp.create → tmp.write → tmp.fsync → tmp.rename → final.fsync`. On macOS FSEvents that's 4 events within ~80ms. JetMemorys IDEs are similar. 250ms safely catches all editor variants while keeping latency low (a human writes once per minute or so; 250ms is invisible).
...
**Why dedup BEFORE emit (§1 #3 + #4)?** Without dedup, "user opens file, types one character, save" creates a new row identical in content to the prior version (because of editor auto-save on-save). The memory chain accumulates noise. With dedup, identical content = identical row = idempotent no-op. The rename case (cache hit on different path) is also load-bearing: `git stash` renames files; without rename detection the same content would emit two rows.
...
**Why W3C TraceContext propagation (§1 #8)?** A capture row in memory with `trace_id: 0af7...` is correlatable to a Cowork session that wrote the file. Without propagation, the row is orphaned — operators can see "memory gained a row at 14:32:17.483 with content hash X" but can't ask "who wrote this?" With propagation, OBS dashboards can build "capture rows by source tool" pivots.
...
**Why startup resync (§1 #11)?** The daemon may have been down (crash, planned maintenance, OS reboot). Files were created / modified / deleted while it was off. Without resync, those changes are silently lost from the memory's perspective. The resync compares filesystem reality to last-known memory state and emits catch-up rows. The 60-second budget for 100K files is the operator-friendly upper bound; exceeded → sev-2 alarm so the operator knows to investigate.
...
**Why no `cyberos memory capture stop` command?** The daemon is managed by systemd / launchd (FR-MEMORY-110); stopping it is `systemctl stop` etc. Adding a CLI stop creates two ways to do the same thing — the systemd unit is authoritative.
...
# services/memory-capture/Cargo.toml
[package]
name        = "cyberos-memory-capture"
version     = "0.1.0"
edition     = "2021"
description = "Event-driven FS watcher → memory capture daemon"
...
cyberos-memory-writer  = { path = "../memory/memory-writer" }
cyberos-memory-reader  = { path = "../memory/memory-reader" }
...
pub struct CachedEntry {
    ...
    pub last_seq:   u64,            // memory chain seq at which we last emitted for this hash
...
use cyberos_memory_writer::{MemoryWriter, AuditRow, canonical};
...
pub async fn emit_capture(
    ...
    async move {
        let kind_str = match (&raw.kind, dedup) {
            (EventKind::Created, _)             => "memory.capture_created",
            (EventKind::Modified, _)            => "memory.capture_modified",
            (EventKind::Renamed { .. }, _)      => "memory.capture_renamed",
            (EventKind::Deleted, _)             => "memory.capture_deleted",
        };
...
#[derive(Parser)]
#[command(name = "cyberos-memory-capture")]
struct Cli {
...
async fn main() -> ExitCode {
    ...
    let daemon = match cyberos_memory_capture::CaptureDaemon::start(cli.manifest, cli.dry_run).await {
        Ok(d) => d,
...
1. **Single-file write captures one row** — write `foo.txt` to a watched folder → exactly one `memory.capture_created` row in memory within 500ms; `content_hash` = blake3(file body); `trace_id` is a 32-char hex.
2. **Editor atomic-save coalesced** — VS Code save sequence (tmp create + rename) produces ONE `memory.capture_created` row, not 3.
3. **Idempotent re-save** — write same bytes to same path twice → only one row emitted; second save is `DedupVerdict::Idempotent`.
4. **Modify with content change emits prior_content_hash** — write file v1, then v2 → second row is `memory.capture_modified` carrying `prior_content_hash = hash(v1)`.
5. **Rename detected via dedup cache** — write `foo.txt`, then rename to `bar.txt` → row 2 is `memory.capture_renamed` with `from_relative_path: foo.txt` and same `content_hash` as row 1.
6. **Delete emits last_content_hash** — write + delete → second row is `memory.capture_deleted` with `last_content_hash` matching the prior row.
...
9. **Per-folder rate-limit triggers drop** — write 1000 files in 1 second → first 200 emitted (burst), remaining drop within seconds with `memory.capture_dropped` rows carrying `reason: rate_limited:folder`.
10. **Per-tenant rate-limit aggregates** — 10 folders each at 100 events/s → after 2 sec, `memory.capture_dropped` rows with `reason: rate_limited:tenant` (1000/sec exceeds 500/sec sustained).
11. **Queue overflow drops with audit row** — manually pause the emitter; producer fills 10K queue; next event drops within 100ms with `memory.capture_dropped` carrying `reason: queue_overflow`.
...
14. **Startup resync emits catch-up rows** — daemon down; user writes 5 new files; daemon starts → 5 catch-up rows + `memory.capture_resync_started` + `memory.capture_resync_completed`.
...
19. **Doctor gate refuses to start** — manifest has dangling symlink (FR-MEMORY-105 `WatchedFolderResolvable` fails) → daemon exits 7 (`InternalError`) with stderr explaining the doctor failure.
20. **Metrics emit** — Prometheus registry contains all metrics from §1 #14; counters increment per emit.
21. **OTel span per emit** — exporter receives a `memory.capture.emit` span per emitted row with attributes `folder_id`, `kind`, `bytes`, `trace_id`.
22. **Dry-run prints without emitting** — `--dry-run` → events printed to stdout; memory chain unchanged.
...
async fn single_file_write_captures_one_row() {
    ...
    let rows = env.memory.rows_since(env.start_seq()).await;
    let cap = rows.iter().find(|r| r.kind == "memory.capture_created").unwrap();
...
async fn idempotent_resave() {
    ...
    let rows = env.memory.rows_since(env.start_seq()).await;
    let captures: Vec<_> = rows.iter().filter(|r| r.kind.starts_with("memory.capture_")).collect();
...
async fn rename_detected_via_dedup() {
    ...
    let rows = env.memory.rows_since(env.start_seq()).await;
    let renamed = rows.iter().find(|r| r.kind == "memory.capture_renamed").unwrap();
...
async fn per_folder_rate_limit_drops_with_audit() {
    ...
    let rows = env.memory.rows_since(env.start_seq()).await;
    let emitted = rows.iter().filter(|r| r.kind == "memory.capture_created").count();
    let dropped = rows.iter().filter(|r| r.kind == "memory.capture_dropped").count();
    assert!(emitted <= 200 + (50 * 2),  "emitted {emitted} (burst+sustained); expected ≤ 300");
    assert!(dropped > 0,                "expected drops; got {dropped}");
    let first_dropped = rows.iter().find(|r| r.kind == "memory.capture_dropped").unwrap();
...
async fn startup_resync_catches_up() {
    ...
    env.wait_for_event("memory.capture_resync_completed", Duration::from_secs(10)).await;

    let rows = env.memory.rows_since(env.start_seq()).await;
    assert!(rows.iter().any(|r| r.kind == "memory.capture_resync_started"));
    assert!(rows.iter().any(|r| r.kind == "memory.capture_resync_completed"));
    let captures: Vec<_> = rows.iter().filter(|r| r.kind == "memory.capture_created").collect();
...
use cyberos_memory_writer::MemoryWriter;
use cyberos_memory_reader::MemoryReader;
...
impl CaptureDaemon {
    pub async fn start(manifest_path: Option<std::path::PathBuf>, dry_run: bool) -> anyhow::Result<Self> {
        ...
        let writer = MemoryWriter::connect().await?;
        let reader = MemoryReader::connect().await?;
...
- **FR-MEMORY-101** — `MemoryWriter` + `MemoryReader` are the interface for chain mutations + reads.
- **FR-MEMORY-102** — `cyberos memory watch/unwatch` is the registration UX; this daemon picks up via SIGHUP.
- **FR-MEMORY-105** — `cyberos doctor --only watched-folders` is the boot gate.
- **FR-MEMORY-108** (downstream) — Cowork session-hook capture FR uses this daemon as its emit pathway.
- **FR-MEMORY-109** (downstream) — Claude Code hook capture FR likewise.
- **FR-MEMORY-110** (downstream) — health-check daemon supervises this process.
- **FR-MEMORY-111** (downstream) — pre-ingest PII detection runs on `body` BEFORE `blake3::hash` is invoked.
...
### `memory.capture_created`

```json
{
  "kind": "memory.capture_created",
...
### `memory.capture_dropped`

```json
{
  "kind": "memory.capture_dropped",
  "payload": {
    "folder_id":      "01HZK9R8M3X5C8Q4-notes",
    "event_kind":     "Modified",
    "content_hash":   "ab12...",
...
- Encrypted-at-rest body hashing — slice 3+; need FR-MEMORY-111 PII detection first to know what to encrypt.
...
|---|---|---|---|
| File unreadable (perms) | `tokio::fs::read` returns Err | Event silently skipped; metric `memory_capture_events_total{outcome="read_failed"}` increments | Operator fixes perms |
...
| Queue full + 100ms timeout | `event_tx.send_timeout` returns Err | `memory.capture_dropped` row emitted | Operator scales emitter or reduces burst |
| `memory_writer` unavailable | emit returns Err | Logged; row not emitted; dedup cache NOT updated | Retry on next event; operator investigates writer |
| Rate-limit (folder) | `governor::check` returns Err | `memory.capture_dropped` with `reason: rate_limited:folder` | Operator raises folder quota in code (governed) |
| Rate-limit (tenant) | `governor::check` returns Err | `memory.capture_dropped` with `reason: rate_limited:tenant` | Operator raises tenant quota |
...
| Daemon OOMs | OOM-killer | systemd restarts (FR-MEMORY-110); resync replays | Operator investigates leak |
| FSEvents drops events (macOS) | mtime mismatch on next event | Caught by next resync OR next event for that file | Periodic resync (FR-MEMORY-110 schedules hourly) |
...
| Disk fills | writer returns ENOSPC | `memory.capture_dropped` rows; sev-1 alarm | Operator frees space; daemon recovers |
