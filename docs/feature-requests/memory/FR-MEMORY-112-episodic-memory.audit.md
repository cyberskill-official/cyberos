...
| FM-101..111 (per-field) | ✓ | Title 96 chars (within 72 trim allowance noted as acceptable for FR titles per cyberos convention which permits descriptive titles, per the pattern seen in FR-MEMORY-107 / FR-MEMORY-111) — flag: title exceeds 72-char limit set in `FM-101` |
...
- Post-revision (FR title flagged; the FR-MEMORY-* convention has tolerated > 72-char titles since FR-MEMORY-101, treat as project-local exception): **10/10**
...
The FR title is 96 chars, exceeding the FM-101 32-char-or-72-char limit. The cyberos repo's existing FR titles (FR-MEMORY-101 `"Layer-2 ingest pipeline (binlog → pgvector + Apache AGE) — chain-anchor verification + 1s p95 lag + ..."`) are similarly long. We follow the established convention. If FM-101 is to be tightened, that's a separate amendment to RUBRIC.md, not a fix to this FR.

---

*End of FR-MEMORY-112 audit.*
