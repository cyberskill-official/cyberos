...
source_pages:
  - website/docs/modules/memory.html#pii-pre-ingest
...
source_decisions:
  ...
  - DEC-171 (canonical ruleset is shared between FR-MEMORY-109 hook + this FR; one regex catalogue, two consumers)
...
new_files:
  - services/memory-capture/src/pii.rs
...
sub_tasks:
  ...
  - "0.5h: integrate into FR-MEMORY-107 emit.rs (call pii::scan_and_redact BEFORE blake3 hashing — hash the redacted body)"
  - "0.5h: integrate FR-MEMORY-109 redactor delegation (delete inline regexes; use ruleset.rs)"
...
risk_if_skipped: "Without pre-ingest scan, capture rows land in memory with raw PII. Vietnamese data protection law (PDPL 2025) treats CCCD as restricted personal data; storing it without authorisation is a regulatory violation. Operators writing Slack-style messages routinely paste customer info ('Tran Quoc Khanh CCCD 079123...'); without scan, memory becomes a PII honeypot. The 99.5% recall gate is the load-bearing CI signal that says 'capture is safe to ship'; without it, redaction quality silently regresses on every change. Without a shared ruleset (DEC-171), the AI Gateway redactor + capture redactor + Claude Code hook redactor drift apart — three sources of truth = inconsistent privacy posture."
---

## §1 — Description (BCP-14 normative)

The pre-ingest PII detection layer **MUST** sit between the FR-MEMORY-107 capture daemon's event classifier and its memory-writer call, scanning every byte that would be written to the chain. The contract:
...
12. **MUST** emit OTel metrics:
    ...
    - `memory_pii_scan_duration_seconds{ner_invoked}` (histogram; FR-OBS-003 buckets).
...
**Why pre-ingest, not post-ingest (§1 #1)?** Once PII lands in the memory chain it is, by AGENTS.md §3.4, immutable — the only way to remove it is `delete(path, "purge")` which requires explicit chat-turn approval. That's a heavy lift for routine PII slip-ups. Pre-ingest catches the problem at the boundary; the chain stays clean by construction.
...
**Why one shared ruleset (§1 #2 + DEC-171)?** Three places redact PII: the capture daemon (this FR), the Claude Code hook (FR-MEMORY-109), the AI Gateway (FR-AI-011/012). If each maintains its own regex catalogue, drift is inevitable — fixing a CCCD regex in one place doesn't fix it in the others. One crate, three importers. The CI test that bans inline regex literals in callers locks this in mechanically.
...
**Why 99.5% recall (§1 #9)?** Compliance commitment: the memory never stores raw PII matching our ruleset. 99.5% recall means ≤ 0.5% of PII spans slip through — those are typically edge cases (CCCDs with unusual formatting). The remaining 0.5% are caught by FR-AI-013's post-ingest recall gate AND by quarterly red-team review. 100% recall is impossible (open language; Presidio bounded). 99.5% is the calibrated threshold.
...
**Why ≤ 50ms p95 (§1 #13)?** Capture is on FR-MEMORY-107's hot path; total event-to-emit budget is ~250ms (FR-MEMORY-107 §1 #2 debounce + redaction + dedup + emit). 50ms for PII scan keeps the latency tax bounded.
...
pub async fn scan_and_redact(body: &str, allowlist: &[regex::Regex]) -> ScanResult {
    ...
    if ner_invoked {
        match presidio_bridge::scan(body).await {
            ...
            Err(e) => {
                ...
                metrics::counter!("memory_pii_scan_failed_total", "reason" => "ner_subprocess_died").increment(1);
            }
        }
    }
    ...
    metrics::histogram!("memory_pii_scan_duration_seconds", "ner_invoked" => ner_invoked.to_string())
        .record(elapsed.as_secs_f64());
    for (_, tag) in &deduped { metrics::counter!("memory_pii_matches_total", "tag" => format!("{tag:?}")).increment(1); }
...
9. **NER skipped on short input** — input < 32 bytes with no regex match → NER not invoked; metric `memory_pii_scan_duration_seconds{ner_invoked="false"}` increments.
...
15. **Fail-closed on NER subprocess crash** — kill the python subprocess mid-scan → `ScanResult.redacted_body == "<SCAN_FAILED>"`; `memory.capture_pii_scan_failed` row emitted; caller skips chain write.
...
18. **OTel span emitted per scan** — exporter receives `memory.pii.scan` with `body_bytes`, `match_count`, `ner_invoked`, `duration_ms` attrs.
19. **Metric: per-tag counter** — running fixtures → `memory_pii_matches_total{tag="EmailAddress"}` non-zero; bounded cardinality.
20. **CLI: cyberos memory pii test** — reads file or stdin; prints span-highlighted output (terminal colour) + JSON match list.
21. **Hash invariance under PII change** — same redacted body but different raw PII (e.g. `"<EMAIL>"` substituted for two different real emails) → same blake3 hash; idempotent dedup at FR-MEMORY-107.
22. **AI gateway uses the same ruleset** — FR-AI-011 + FR-AI-012's PII module imports from `cyberos_memory_capture::pii::ruleset`; verified via build-graph test.
23. **Claude Code hook uses the same ruleset** — FR-MEMORY-109's `redact.rs` delegates to `ruleset::scan_with_regex`.
...
- **FR-MEMORY-107 (upstream)** — capture emit pipeline; PII scan slots in just before blake3.
- **FR-MEMORY-109 (sibling)** — Claude Code hook delegates redaction to this ruleset.
...
### `memory.capture_pii_scan_failed`

```json
{
  "kind": "memory.capture_pii_scan_failed",
  "payload": {
...
| Invalid UTF-8 in input | Already handled at FR-MEMORY-107 boundary | N/A | N/A |
...
- The `ruleset.rs` module is the single source of truth. Callers (capture, hook, AI gateway) MUST `use cyberos_memory_capture::pii::ruleset;` — no inline regex literals. CI grep test enforces.
...
- The `cyberos_presidio` Python module is a thin wrapper around Microsoft's Presidio (`presidio-analyzer` + `presidio-anonymizer`). Its `requirements.txt` pins versions; `cyberos memory pii update-model` (slice 3+) will refresh.
