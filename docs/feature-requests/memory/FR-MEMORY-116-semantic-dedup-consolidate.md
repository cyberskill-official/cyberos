...
related_frs: [FR-MEMORY-115, FR-MEMORY-108]
depends_on: [FR-MEMORY-115]
blocks: []
...
source_decisions:
  ...
  - DEC-221 (Default = `--dry-run`; operator must explicitly pass `--apply` to merge proposals. Mirrors FR-MEMORY-115's operator-review gate)
...
disallowed_tools:
  ...
  - bypass the FR-MEMORY-115 protocol-amendment §7.7 anchor check when `--apply` is used (the same anchor check applies)
...
sub_tasks:
  ...
  - "1.0h: --threshold flag plumbing to the duplicates detector (default 0.92 per FR-MEMORY-115 §1 #3)"
  ...
  - "1.0h: integration test against the seeded_memory_with_dupes fixture from FR-MEMORY-115 (validates the cross-FR composition works end-to-end)"
risk_if_skipped: "FR-MEMORY-116 is a strict subset of FR-MEMORY-115's capabilities — it ships nothing FR-MEMORY-115 doesn't already provide. The reason it's a distinct FR is operator ergonomics: many operators run `cyberos consolidate` on a nightly cron and want dedup to happen there, not as a separate `cyberos dream` invocation. Without FR-MEMORY-116, operators have to either (a) run two cron jobs (consolidate + dream) and live with the duplication of audit-chain work, or (b) skip dedup during routine maintenance and accept the duplicate accumulation. The 6-hour effort is dominantly tests + CLI glue — the semantic logic is already in FR-MEMORY-115. Skipping means we'd later have to re-explain to operators that dedup lives in a separate command, which is a documentation tax. Cheaper to ship the convenience surface."
---
...
2. **MUST** default to `--dry-run` when `--semantic-dedup` is passed. Operator must explicitly add `--apply` to merge dedup proposals. Mirrors FR-MEMORY-115's operator-review gate (DEC-221).
...
5. **MUST** tag the resulting `dream.start` / `dream.complete` / `dream.proposal_applied` audit rows with `extra.invocation: "consolidate"` so FR-MEMORY-120's history view can distinguish dedup-from-consolidate from dedup-from-explicit-dream-run.
...
9. **MUST** be idempotent — re-running `cyberos consolidate --semantic-dedup --apply` with identical store state produces zero new chain rows (same `dream apply` idempotency from FR-MEMORY-115 #10).
...
**Why subset, not separate path (§1 #3, DEC-220).** Two independent code paths drift. One is shared. The dream pipeline (FR-MEMORY-115) is the canonical surface; semantic-dedup-in-consolidate is the convenience surface. Both call the same detector + applier.
...
**Why `extra.invocation` instead of a new audit kind (DEC-222).** New audit kinds expand the schema surface; new code paths in walkers / readers / FR-MEMORY-120 history. `extra.invocation` is one bag-key, leaves walkers untouched, and lets the history view filter by invocation source.
...
13. **Reuses FR-MEMORY-115 detector** — code-level test asserts the import path `cyberos.core.dream.detectors.duplicates.run` is invoked from consolidate.py; no duplicate detector code exists in consolidate.py. *(traces_to: §1 #3, DEC-220)*
14. **Cross-FR integration smoke** — `seeded_memory_with_dupes` fixture (FR-MEMORY-115) → `cyberos consolidate --semantic-dedup --dry-run` produces the SAME diff (same proposal_ids) as `cyberos dream --detectors duplicates --dry-run`. *(traces_to: §1 #3)*
...
async def test_semantic_dedup_dry_run_produces_diff(seeded_memory_with_dupes):
    ...
    # (only `dream.start` + `dream.complete` from FR-MEMORY-115's runner)
...
- **FR-MEMORY-115 (depends on)** — `duplicates.run` + `dream_apply` + AGENTS.md §7.7. This FR is a CLI wrapper around them.
- **FR-MEMORY-108 (transitively)** — semantic backend (sentence-transformers) is the substrate for cosine sim.
...
- Per-tenant threshold override (KYC tenants want tighter dedup) — slice 4+ once FR-MEMORY-117 lands.
...
| Duplicates detector errors | FR-MEMORY-115's error path | dream.detector_failed row + partial diff | Operator inspects + re-runs |
| Apply transaction fails mid-way | FR-MEMORY-115's transactional rollback | head unchanged | Re-run after fixing |
| Re-apply on changed state | FR-MEMORY-115 PreconditionFailed | exception, no writes | Re-run dedup detector + re-apply |
