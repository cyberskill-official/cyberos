...
Per FR-AI-021 §3 note, module-specific codes start at 200 (AUTH), 300 (memory). I initially wrote `exit 6 (InvariantFailure)` — collides with shared 6=SchemaViolation. Resolved: §1 #10 + §6 `exit_code` = `200` (memory-range InvariantFailure); §10 every error-severity row.
