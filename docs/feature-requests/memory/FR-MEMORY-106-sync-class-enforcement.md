...
blocks: [FR-MEMORY-107]   # placeholder — read-time ACL filtering FR, not yet specified
...
modified_files:
  - services/memory-sync/src/sync.rs                       # call should_sync() at every direction
allowed_tools:
...
- **FR-MEMORY-103** — sync.rs integration point.
- **FR-MEMORY-107 (downstream)** — read-time ACL filtering uses similar logic.
...
- v1 transitional support exists for migration; eventual deprecation via FR-MEMORY-204 (after v1 usage drops to <1% of rows).
