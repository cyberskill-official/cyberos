...
The FR correctly produces FR-MEMORY-121 events rather than inventing a shape, and its `depends_on [FR-MEMORY-121, FR-AUTH-002, FR-CHAT-101]` / `blocks [FR-MEMORY-123]` matches the canonical BRAIN/EVAL map. Live-realism is strong: every wiring point (the `emit_token_issued` neighbour, the `Presence` count map, the `CHAT_AUDIT_DATABASE_URL`/`audit_pool` plumbing, the `chat_messages` pointer store, the deploy compose, the p0 runbook) is an existing, named artifact in the repo, so the spec is implementable against the code as it stands.

---

*End of FR-MEMORY-122 audit.*
