...
risk_if_skipped: "memory search is the read API for everything: KB retrieval (FR-KB-007), CUO ambient context, OBS auto-triage. Without it, all those modules fail. Without RLS+ACL, search leaks across tenants/actors. Without chain_anchor verification, Layer 1 tampering goes undetected at query time. Without graceful degrade, BGE outage takes down search entirely."
---
...
10. **MUST** verify `chain_anchor` for each returned result against Layer 1 (per FR-MEMORY-101 §1 #4). Mismatch → drop the result + emit sev-1 OBS event `memory_search_chain_anchor_mismatch{tenant_id, seq}`.
...
- **FR-MEMORY-101** — Layer 2 + chain_anchor.
- **FR-AI-019** — BGE-M3 sidecar.
- **FR-AI-020** — BGE-rerank-v2-m3.
- **FR-AUTH-003** — RLS enforcement.
- **FR-AUTH-004** — JWT.
- **FR-MEMORY-106** — sync_class informs ACL filter (related).
