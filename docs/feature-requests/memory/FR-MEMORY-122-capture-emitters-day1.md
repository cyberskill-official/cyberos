---
id: FR-MEMORY-122
title: "capture emitters — wire AUTH (sign-in, presence) + CHAT (message create/edit/delete, channel/DM activity) to emit FR-MEMORY-121 interaction-events, turn ON the chat→brain audit link (CHAT_AUDIT_DATABASE_URL), define the emitter contract for PROJ/EMAIL/APP/MCP, gated by the consent check; day-1 wide capture"
module: MEMORY
priority: MUST
status: draft
verify: T
...
related_frs: [FR-MEMORY-121, FR-MEMORY-101, FR-MEMORY-123, FR-EVAL-001, FR-AUTH-002, FR-AUTH-004, FR-CHAT-101, FR-OBS-003, FR-APP-005]
depends_on: [FR-MEMORY-121, FR-AUTH-002, FR-CHAT-101]
blocks: [FR-MEMORY-123]
...
source_decisions:
  ...
  - DEC-2712 (every emitter goes through FR-MEMORY-121 emit(), which is consent-gated on the FR-EVAL-001 notice acknowledgment — no emitter captures a subject who has not acknowledged)
...
disallowed_tools:
  - emit an interaction-event by hand-building an l1_audit_log row instead of calling FR-MEMORY-121 emit() (bypasses the consent gate + validation + schema)
  ...
  - block or fail the underlying interaction (sign-in, message send) on a capture error (capture is best-effort, per FR-MEMORY-121 §1 #7)

effort_hours: 10
sub_tasks:
  - "1.0h: cyberos-capture crate — CaptureEmitter trait + Capturer{pool} wrapper that builds the FR-MEMORY-121 event + calls emit(); shared by auth + chat (and future modules)"
  ...
  - "0.5h: emitter metrics — reuse FR-MEMORY-121 counters; add memory_capture_emitter_calls_total{module,event_type,outcome}"
...
5. **MUST** turn ON the chat→brain audit link (DEC-2713). CHAT already reads `CHAT_AUDIT_DATABASE_URL` into `audit_pool` and logs `"unset; chat audit events are logged, not chained"` when absent (P0 left it unset). This FR sets that variable to the brain's audit DB (the MEMORY `l1_audit_log` database, Supabase Postgres in prod) so chat interaction-events chain into MEMORY, changes the `unset` log from `warn` to `info`, and documents the variable as required in production. The chat capture path routes through `cyberos-capture` → FR-MEMORY-121 `emit()` over that pool.
6. **MUST** route every emitter through FR-MEMORY-121 `emit()` so the consent gate (FR-MEMORY-121 §1 #8) and validation apply uniformly. An emitter physically cannot skip the gate, because it has no other way to write the row. A subject who has not acknowledged the FR-EVAL-001 notice produces `Skipped` outcomes and zero rows, regardless of which module emitted.
...
12. **MUST** emit OTel metrics (in addition to the FR-MEMORY-121 counters): `memory_capture_emitter_calls_total{module, event_type, outcome}` (outcome ∈ `recorded | skipped_consent | invalid | emit_error`) and `memory_capture_backfill_events_total{outcome}`. These give a per-emitter view of live capture and of backfill progress.
...
14. **MUST** set `trace_id` on emitted events when a request trace is in scope (FR-OBS-003 traceparent), so a captured interaction correlates with its request span and log line. When no trace is in scope (e.g. a websocket presence edge), `trace_id` is `null` — never fabricated.
...
**Why turn on the chat→brain link now (DEC-2713)?** The brain plan names this exact gap: chat content is in chat's DB but, because P0 left `CHAT_AUDIT_DATABASE_URL` off, none of it is mirrored to the brain. The infrastructure is already there — chat reads the variable, holds an `audit_pool`, and writes through `cyberos-audit-chain` when it is set. Turning it on is a configuration flip plus routing chat's capture through the FR-MEMORY-121 emit path, not new plumbing. Until it is on, the brain has no chat signal at all.
...
/// chained l1_audit_log row via FR-MEMORY-121 emit() (consent-gated, validated, best-effort).
#[derive(Clone)]
pub struct Capturer {
    pool: PgPool,
}
...
impl Capturer {
    ...
    /// Best-effort: returns the FR-MEMORY-121 outcome. Callers log + swallow on error;
...
//
// FR-MEMORY-122 §1 #3 — emit the auth.signed_in interaction-event alongside the audit token row.
...
See §3 (shared crate, AUTH + CHAT emitters, the link flip, backfill). Each module's `capture.rs` is the per-module surface; `cyberos-capture` is the shared mechanism; everything routes through FR-MEMORY-121 `emit()`.

---

## §7 — Dependencies

- **FR-MEMORY-121** — the event shape, the typed builder, `emit()`, the consent gate, and the contract these emitters produce/obey. This FR adds no shape; it produces FR-MEMORY-121 events.
- **FR-AUTH-002 / FR-AUTH-004** — the subjects + sessions AUTH attributes events to; the existing audit pool + `emit_token_issued`/`_failed` call sites the new emitters sit next to.
- **FR-CHAT-101** — the cyberos-chat service (channels/DMs/messages/attachments/presence) the chat emitters hook into; the existing `CHAT_AUDIT_DATABASE_URL` / `audit_pool` plumbing DEC-2713 turns on.
- **FR-EVAL-001** — owns the acknowledgment ledger the consent gate reads (via FR-MEMORY-121); capture for a subject begins only after acknowledgment.
- **FR-OBS-003** — the traceparent `trace_id` emitters attach when a request trace is in scope.
- **Downstream:** FR-MEMORY-123 (ingestion consumes the events these emitters produce); FR-APP-005 (the audit viewer surfaces them).
...
| Emitter inlines raw body (bug) | FR-MEMORY-121 size cap + body-bytes histogram | `Invalid` at the limit; metric flags below | Author fixes emitter to use content_ref |
...
| Two emitters for one domain event (bug) | FR-MEMORY-121 event_id unique index | one wins; SEV-3 warn | Author fixes call site |
| AUTH audit pool and brain pool differ | both are the same l1_audit_log DB | rows land together | By design (shared deployment) |
| Chat DB and audit DB out of sync (pointer dangles) | resolve-time check in consumer | consumer treats missing target as redacted | FR-MEMORY-123 handles gracefully |
...
| OBS exporter down | buffered then dropped | metric gap; rows still written | Restore FR-OBS-001 |
...
- This FR produces FR-MEMORY-121 events; it defines no new shape. The shared `cyberos-capture` crate is the mechanism (build → `emit()`); each module's `capture.rs` is the thin domain translation. New modules add a `capture.rs`, not a new pattern — that is the whole DEC-2714 point.
...
- **Why limited:** this FR is the point where real employee work-activity begins flowing into the BRAIN — sign-ins, presence, and chat activity for CyberSkill staff — which is the data foundation for the later AI-assisted evaluation (FR-EVAL-003/004). The emitters run no model and make no decision, but they feed a limited-risk AI use (workplace evaluation support), so they are classified and governed as part of that chain.
...
- **Consent before capture (enforced, not promised):** every emitter writes only through FR-MEMORY-121 `emit()`, which is hard-gated on the FR-EVAL-001 notice acknowledgment. There is no emit path that bypasses the gate (hand-built audit rows are forbidden by `disallowed_tools`), so no subject is captured before they have acknowledged the monitoring notice — live or in backfill. This is disclosed monitoring made structural.
- **Minimisation + purpose limitation:** emitters carry only the bounded attributes their event needs (e.g. `channel_kind`, `method`, `source_ip_hash16`), reference content rather than copying it, and inherit FR-MEMORY-121's 2 KiB attributes cap and body-bytes watch. The purpose is to record the work, not to surveil the person; the closed event vocabulary bounds what can be recorded.
- **Access + tenancy:** captured events are `l1_audit_log` rows and inherit the per-tenant RLS, so a tenant's records never cross to another. Who may read them (manager + HR + the employee for their own record) is enforced by the consuming surfaces (FR-EVAL-005), not loosened here.
...
- **No autonomous decision:** these emitters record interactions; they never score, rank, or decide anything about a person. Every consequential use runs through the human-in-the-loop evaluation workflow (FR-EVAL-004).

---

*End of FR-MEMORY-122.*
