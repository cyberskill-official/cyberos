...
related_frs: [FR-MEMORY-103, FR-MEMORY-115, FR-MEMORY-117]
depends_on: [FR-MEMORY-117]
blocks: []
...
disallowed_tools:
  ...
  - bypass the FR-MEMORY-117 ACL check for put_if (per §1 #5)
...
12. **MUST** require the AGENTS.md §3.1 amendment APPROVED via `APPROVE protocol change P21 §3.1` chat-turn before `put_if` is callable. Same anchor-check pattern as FR-MEMORY-115 / 117: `Writer.put_if(...)` raises `ProtocolAmendmentMissingError` if §3.1 hasn't been extended to include `put_if`.
...
14. **SHOULD** support a batch variant `put_if_all([(path, body, meta, precond), ...])` that succeeds-all or rejects-all transactionally. Slice-4 stretch — useful for FR-MEMORY-115 dream-apply (which already has its own transactional path).
...
**Why aux row even on rejection (§1 #4, §1 #7, DEC-242).** Dream-applier (FR-MEMORY-115) and FS watcher (FR-MEMORY-107) need to count rejections to detect contention storms. Operators investigating "why did agent X's writes silently disappear?" need a queryable log. The aux row is cheap; visibility is high-value.

**Why preserve `put` semantics on success (§1 #6).** `put_if` shouldn't pollute the audit chain with a new row shape — downstream consumers (walker, doctor, FR-MEMORY-115's pattern detector, FR-MEMORY-120's history view) all already understand `put` rows. Making `put_if`'s success row identical to a `put` row keeps every consumer working without changes.
...
- **FR-MEMORY-117 (depends on)** — ACL check runs first. This FR requires `put_if` to honour STORE.yaml ACL.
- **FR-MEMORY-115 (related)** — dream-applier's preconditions are conceptually identical; the applier could be refactored to use `put_if` (slice-4 cleanup).
- **FR-MEMORY-103 (related)** — multi-device sync uses optimistic concurrency at the chain level (`prev_chain`); this FR adds it at the memory-file level for finer granularity.
...
- Batch `put_if_all([...])` — §1 #14; slice 4 (FR-MEMORY-115 dream-applier already has its own batch path; a general-purpose batch primitive is a slice-4 design discussion).
...
- **Why we don't expose `put_if` as separate `kind` in the audit row.** Downstream consumers (walker, doctor, FR-MEMORY-115 detectors, FR-MEMORY-120 history) all already understand `put`. A new kind would force every consumer to handle two row shapes. The success row's invisibility is the load-bearing simplification.
