...
related_frs: [FR-BRAIN-101, FR-BRAIN-102, FR-BRAIN-103, FR-BRAIN-104, FR-BRAIN-107, FR-BRAIN-110]
depends_on: [FR-BRAIN-101, FR-BRAIN-103, FR-BRAIN-104]
blocks: [FR-BRAIN-107, FR-BRAIN-110]

# ───── Source contracts ─────
source_pages:
  - website/docs/modules/brain.html#doctor
  - website/docs/runbooks/brain-doctor-runbook.html
source_decisions:
  ...
  - DEC-130 (watched-folders are first-class BRAIN citizens; doctor MUST validate every registered folder)
  - DEC-131 (per-folder manifest fragment; symlink resolution; cross-volume drift detection)

# ───── Build envelope ─────
language: rust 1.81
service: cyberos/services/brain/
new_files:
  - services/brain/src/doctor/watched_folders.rs
  - services/brain/src/doctor/invariants_v2.rs
  - services/brain/memory.invariants.yaml.d/watched-folders-v2.yaml
  - services/brain/tests/doctor_watched_folders_test.rs
  - services/brain/tests/doctor_invariants_v2_test.rs
modified_files:
  - services/brain/src/doctor/mod.rs                       # register watched-folders module
  - services/brain/src/doctor/cli.rs                       # surface new invariant ids in `cyberos doctor --json`
  - services/brain/memory.invariants.yaml                  # include directive for watched-folders-v2.yaml
  - services/brain/docs/doctor.md                          # document the 5 new invariants
allowed_tools:
  - file_read: services/brain/**
  - file_write: services/brain/{src,tests,memory.invariants.yaml.d,docs}/**
  - bash: cd services/brain && cargo test doctor
  - bash: cd services/brain && cargo run --bin cyberos -- doctor --json
disallowed_tools:
...
sub_tasks:
  ...
  - "0.5h: integration into FR-BRAIN-110 health check (doctor exit code propagates)"
risk_if_skipped: "Without these invariants, a watched-folder registration can silently rot: the folder is unmounted, the symlink dangles, two registrations point at the same realpath, the folder's HEAD diverges from root, or its manifest schema drifts. None of these are caught by core BRAIN invariants (those only audit `<memory-root>/audit/*.binlog` and HEAD). The first symptom of any of these is a capture-write failure with a confusing error message — operator wastes an hour discovering the dangling symlink instead of seeing it on `cyberos doctor`. FR-BRAIN-110's health-check daemon depends on this invariant set to know when to refuse-to-start."
---
...
1. **MUST** add invariant `WatchedFolderManifestPresent` (severity `error`): for every entry in `manifest.watched_folders[]`, the file `<folder>/manifest.json` MUST exist, be readable, and parse as valid JSON conforming to `memory.schema.json#/definitions/WatchedFolderManifest`. Failure → agent state transitions to `FROZEN_RECOVERABLE` (per AGENTS.md §12); repair via `cyberos brain watch <path>` re-bootstraps the fragment OR `cyberos brain unwatch <path>` removes the registration.

2. **MUST** add invariant `WatchedFolderResolvable` (severity `error`): for every entry in `manifest.watched_folders[]`, the path MUST resolve through every symlink (per AGENTS.md §0.4) and the final target MUST exist (`stat()` succeeds, not ENOENT). Failure → `FROZEN_RECOVERABLE`; repair via remount the volume OR `cyberos brain unwatch <path>` if the folder is permanently gone.

3. **MUST** add invariant `WatchedFolderNotShadowed` (severity `error`): no two entries in `manifest.watched_folders[]` MAY resolve to the same realpath. Ambiguous registration is forbidden by construction. Failure → `FROZEN_RECOVERABLE`; repair via `cyberos brain unwatch` on one of the duplicates (the operator picks which).
...
10. **MUST** integrate into `cyberos doctor` exit code: any error-severity failure → exit 2 (`InvariantFailure`); any warning-only failures → exit 0 with warnings printed; no failures → exit 0. Exit codes per the shared `cyberos-cli-exit::ExitCode` (slot reserved at `200` range for BRAIN-specific codes per FR-AI-021 §3 note; `200 = InvariantFailure`).

11. **MUST** emit OTel span `brain.doctor.invariant_check` per invariant call with attributes `invariant_id`, `severity`, `status`, `duration_ms`, `affected_path_count` (when applicable). Aggregated by FR-OBS-005 for trend dashboards.
...
13. **SHOULD** emit metric `brain_doctor_invariants_failed_total{invariant_id, severity}` (counter) and `brain_doctor_invariant_duration_seconds{invariant_id}` (histogram with FR-OBS-003 standardised buckets).
...
- `NotShadowed` — operator ran `brain watch` twice with different relative paths that resolve to the same realpath.
...
**Why deterministic JSON output (§1 #8)?** CI gates downstream of doctor (e.g. FR-BRAIN-110's health check, FR-OBS-007's auto-triage) diff doctor's output across runs. Non-deterministic output → false-positive diffs → operators ignore real failures. Sort-by-realpath is the canonical iteration order.

**Why 5-second latency budget (§1 #9)?** Doctor runs as part of FR-BRAIN-110's daemon health check at startup AND on a 60-second interval. 5 seconds is the upper bound that lets the daemon remain responsive; missing the budget triggers a sev-2 alarm. For ≤ 50 folders the budget is generous; the threshold exists to catch pathological cases (e.g. 10,000 dangling symlinks would blow the budget).
...
invariants:
  - id: WatchedFolderManifestPresent
    ...
    repair_hint: "cyberos brain watch <path>   # re-bootstrap the fragment"
  ...
  - id: WatchedFolderResolvable
    ...
    state_on_failure: FROZEN_RECOVERABLE
    repair_hint: "remount the volume OR cyberos brain unwatch <path>"
  ...
  - id: WatchedFolderNotShadowed
    ...
    state_on_failure: FROZEN_RECOVERABLE
    repair_hint: "cyberos brain unwatch <one of the duplicates>"
...
```rust
// services/brain/src/doctor/mod.rs (excerpt)
const INVARIANT_ORDER: &[&dyn Invariant] = &[
    ...
    // Watched-folders (100..199) — registered here per FR-BRAIN-105
...
```json
{
  ...
  "invariants": [
    ...
    },
    {
      ...
      "repair_hint": "cyberos brain watch /Users/stephencheng/Documents/work-notes"
    },
    {
      "id": "WatchedFolderManifestSchema",
      "severity": "warning",
...
15. **OTel span per invariant** — exporter receives 5 spans named `brain.doctor.invariant_check` with attribute `invariant_id`; assert via in-memory exporter.
16. **Metric increments on failure** — failure path increments `brain_doctor_invariants_failed_total{invariant_id, severity}`; assert via in-process Prometheus registry.
...
```rust
// services/brain/src/doctor/mod.rs  (orchestrator excerpt)
...
```rust
// services/brain/src/doctor/cli.rs
...
pub fn exit_code(results: &[InvariantResult]) -> i32 {
    if results.iter().any(|r| r.severity == Severity::Error && r.status == Status::Fail) {
        200  // shared cyberos-cli-exit::ExitCode (BRAIN-range InvariantFailure)
    } else { 0 }
}
```

```yaml
# services/brain/memory.invariants.yaml (master file, append include)
include:
...
- **FR-BRAIN-101 (upstream)** — defines `manifest.watched_folders[]` schema, the data this FR audits.
- **FR-BRAIN-102 (related)** — `cyberos brain watch/unwatch` is the repair path for `WatchedFolderManifestPresent` failures.
- **FR-BRAIN-103 (related)** — multi-device sync writes folder HEADs; if it bypasses the canonical writer, this FR catches it.
- **FR-BRAIN-107 (downstream)** — FS watcher's startup uses `doctor --only watched-folders` as a gate; refuses to start if any error-severity invariant fails.
- **FR-BRAIN-110 (downstream)** — health-check daemon runs doctor on a 60-second interval and reports failures via OTel.
...
[error] WatchedFolderManifestPresent: FAIL — 1 watched folder(s) missing or unreadable manifest.json
        ...
        repair:   cyberos brain watch /Users/stephencheng/Documents/work-notes
...
| Dangling symlink (target deleted) | `WatchedFolderResolvable` fails | exit 200, FROZEN_RECOVERABLE | Operator `cyberos brain unwatch <path>` |
...
| Folder manifest.json deleted | `WatchedFolderManifestPresent` fails | exit 200, FROZEN_RECOVERABLE | Operator `cyberos brain watch <path>` re-bootstraps |
...
| Folder HEAD file size != 8 bytes | `WatchedFolderHEADConsistent` skips that folder | exit 0 if no other failures | Operator investigates; may need `cyberos brain watch` |
...
| Invariant exceeds latency budget | metric `brain_doctor_invariant_duration_seconds` p95 alarm | sev-2 alarm via FR-OBS-007 | Operator investigates folder count + filesystem health |
