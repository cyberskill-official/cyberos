---
id: FR-BRAIN-103
title: "brain-sync daemon — laptop A ↔ Cloud BRAIN ↔ laptop B with sync_class gating + CRDT conflict + 10K offline buffer + device-id stamp"
module: BRAIN
priority: MUST
status: accepted
verify: T
...
related_frs: [FR-BRAIN-101, FR-BRAIN-104, FR-BRAIN-105, FR-BRAIN-106]
depends_on: [FR-BRAIN-101]
blocks: [FR-BRAIN-104, FR-BRAIN-106, FR-BRAIN-105]
...
new_files:
  - services/brain-sync/Cargo.toml
  - services/brain-sync/src/main.rs
  - services/brain-sync/src/sync.rs
  - services/brain-sync/src/crdt.rs
  - services/brain-sync/src/protocol/mod.rs
  - services/brain-sync/src/protocol/grpc.rs
  - services/brain-sync/src/buffer.rs
  - services/brain-sync/src/sync_class_filter.rs
  - services/brain-sync/src/compensation_guard.rs
  - services/brain-sync/proto/brain_sync.proto
  - services/brain-sync/tests/sync_test.rs
  - services/brain-sync/tests/sync_class_test.rs
  - services/brain-sync/tests/conflict_test.rs
  - services/brain-sync/tests/offline_buffer_test.rs
  - services/brain-sync/tests/compensation_guard_test.rs
modified_files: []
allowed_tools:
  - file_read: services/brain-sync/**
  - file_write: services/brain-sync/**
  - bash: cd services/brain-sync && cargo test
disallowed_tools:
...
sub_tasks:
  ...
  - "0.5h: disputed_pair row creation on conflict (FR-BRAIN-105 surfaces UI)"
...
4. **MUST** detect conflicts via CRDT triple `(content_hash, ts_ns, originator_device)`. Conflict = same target memory id with two different writes from different devices within overlapping wall-clock windows. On conflict, write a `disputed_pair` row with both versions; FR-BRAIN-105 surfaces the resolution UI.
...
7. **MUST** maintain offline buffer of up to 10,000 rows when Cloud BRAIN is unreachable. Buffer is FIFO; oldest rows evicted on overflow with sev-2 alert + metric `brain_sync_buffer_overflow_total`. Evicted rows are NOT lost (they're in the local Layer 1 chain) — they just don't propagate until manual reconciliation.
8. **MUST** emit OTel metrics:
    - `brain_sync_lag_seconds{device_id}` (histogram; round-trip).
    - `brain_sync_conflicts_total` (counter).
    - `brain_sync_buffered_rows{device_id}` (gauge).
    - `brain_sync_buffer_overflow_total{device_id}` (counter; sev-2 alarm).
    - `brain_sync_pushed_rows_total{device_id, sync_class}` (counter).
    - `brain_sync_compensation_rejected_total` (counter; sev-1 alarm — DEC-036 violation attempt).
...
11. **MUST** detect online/offline state via TCP probe to Cloud BRAIN every 30s; transition online→offline → start buffering; offline→online → flush buffer + resume.
12. **MUST** authenticate to Cloud BRAIN via per-device bearer token (rotated quarterly; managed via FR-AUTH-006-style sweeper). Compromised token → revoke at Cloud BRAIN; device cannot sync until new token issued.
...
**Why Cloud BRAIN as star-topology hub (DEC-100)?** Peer-to-peer sync (every device → every device) scales as O(N²) connections; centralised hub is O(N). Cloud BRAIN is the trust root + delivery hub. Devices online intermittently get all updates from the hub on reconnect.
...
```rust
// services/brain-sync/src/sync.rs
pub async fn sync_loop(local: &LocalBrain, cloud: &CloudBrainClient, config: &SyncConfig) -> anyhow::Result<()> {
    loop {
...
pub async fn detect_conflict(new_row: &Row, local: &LocalBrain) -> Result<Option<DisputedPair>, anyhow::Error> {
...
1. Append row on device A → within 5s, row visible on device B (via Cloud BRAIN round-trip).
...
5. Compensation rows refused at sync boundary; sev-1 metric `brain_sync_compensation_rejected_total` increments.
...
async fn offline_buffers_50_rows_then_syncs_on_reconnect() {
    ...
    let buffered: u64 = otel_test_helper::gauge_value("brain_sync_buffered_rows", &[]);
    assert_eq!(buffered, 50);
    mock_cloud.go_online();
    tokio::time::sleep(Duration::from_secs(3)).await;
    let final_buffered: u64 = otel_test_helper::gauge_value("brain_sync_buffered_rows", &[]);
...
async fn compensation_row_rejected_at_sync_boundary() {
    ...
    tokio::time::sleep(Duration::from_secs(2)).await;
    assert_eq!(mock_cloud.received_count(), 0);
    let metric: u64 = otel_test_helper::counter_value("brain_sync_compensation_rejected_total", &[]);
    assert_eq!(metric, 1);
}
...
async fn buffer_overflow_evicts_oldest() {
    ...
    let metric: u64 = otel_test_helper::counter_value("brain_sync_buffer_overflow_total", &[]);
...
- **FR-BRAIN-101** — Layer 2 sees synced rows downstream.
- **FR-BRAIN-104** — Tauri app embeds brain-sync.
- **FR-BRAIN-105** — Disputed-pair UI (slice 2+).
- **FR-BRAIN-106** — sync-class enforcement at write time (this FR enforces at sync boundary).
- Cloud BRAIN service (separate, P1+).
...
sev-1 brain_sync_compensation_rejected_total incremented
```
...
sev-2 brain_sync_buffer_overflow_total incremented
```
...
- Cross-Cloud-BRAIN federation (multi-region) — slice 5+.
...
| Cloud BRAIN down | Same | Same | Self-heals |
| Conflict | CRDT detect | disputed_pair row created | User picks via FR-BRAIN-105 UI |
...
- Cloud BRAIN is the centralised hub (star topology); per-device sync via gRPC stream.
...
- Per-device bearer token rotated quarterly via FR-AUTH-006-style sweeper.
...
*End of FR-BRAIN-103. Status: draft (10/10 target).*
