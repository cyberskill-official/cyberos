...
related_frs: [FR-BRAIN-101, FR-BRAIN-110, FR-AI-019]
depends_on: [FR-BRAIN-101]
blocks: [FR-BRAIN-107]

source_pages:
  - website/docs/modules/brain.html#rebuild
source_decisions:
...
new_files:
  - services/brain/src/bin/rebuild_layer2.rs
  - services/brain/src/rebuild/mod.rs
  - services/brain/src/rebuild/spot_check.rs
  - services/brain/src/rebuild/determinism.rs
  - services/brain/src/cli/rebuild_cli.rs
  - services/brain/tests/rebuild_test.rs
  - services/brain/tests/rebuild_determinism_test.rs
  - services/brain/tests/rebuild_resume_test.rs
  - .github/workflows/brain-rebuild.yml
modified_files:
  - services/brain/src/layer2/ingest.rs                    # add `run_one_pass` for rebuild
allowed_tools:
  - file_read: services/brain/**
  - file_write: services/brain/{src,tests}/**, .github/workflows/brain-rebuild.yml
  - bash: cd services/brain && cargo build --bin rebuild_layer2
  - bash: cd services/brain && cargo test rebuild
disallowed_tools:
...
sub_tasks:
  ...
  - "1.0h: brain-rebuild.yml CI workflow (4-core ubuntu-22.04 + pgvector + AGE + BGE sidecar)"
...
1. **MUST** be CI-gated on every PR touching `services/brain/**` OR `crates/cyberos-obs-sdk/**` (the latter affects ingest instrumentation) OR the workflow file itself.
2. **MUST** spin up a fresh Postgres instance (pgvector + AGE extensions) AND a BGE-M3 sidecar for the rebuild duration. The CI job uses `services` declaration in `.github/workflows/brain-rebuild.yml`.
...
7. **MUST** include `cyberos-brain rebuild-layer2 --tenant <id> [--dry-run]` CLI for operator preview. `--dry-run` shows what would be rebuilt without executing.
...
11. **MUST** apply production safety guard — running `rebuild_layer2` against a production Postgres requires `--prod-confirmed-aware` flag AND interactive Y confirmation (similar to FR-AUTH-006 §1 #11). Production rebuild WIPES then RECREATES Layer 2 — destructive even though derivable.
...
14. **SHOULD** emit OTel metrics:
    - `brain_rebuild_duration_seconds{tenant_id}` (histogram).
    - `brain_rebuild_rows_ingested{tenant_id}` (gauge).
    - `brain_rebuild_spot_check_pass{tenant_id}` (gauge; 0-100).
    - `brain_rebuild_failures_total{stage}` (counter).

---

## §2 — Why this design (rationale for humans)

**Why CI gate on every brain PR (§1 #1)?** Layer 2 derivability is the load-bearing invariant. Without CI enforcement, a subtle bug (e.g., new column in ingest path that isn't deterministic) ships to production AND breaks rebuild capability — discovered only when ops actually needs to rebuild (typically during incident).
...
```yaml
# .github/workflows/brain-rebuild.yml
name: BRAIN Layer-2 Rebuild Gate
on:
  pull_request:
    paths:
      - 'services/brain/**'
...
- **FR-BRAIN-101** — ingest_loop's `run_one_pass` is invoked.
- **FR-AI-019** — BGE-M3 sidecar.
- **FR-AUTH-003** — RLS pattern.
...
*End of FR-BRAIN-102. Status: draft (10/10 target).*
