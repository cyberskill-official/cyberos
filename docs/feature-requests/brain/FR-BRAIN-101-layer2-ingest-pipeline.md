...
related_frs: [FR-BRAIN-102, FR-BRAIN-108, FR-BRAIN-110, FR-AI-019, FR-AUTH-003, FR-OBS-001]
depends_on: [FR-AI-019, FR-AUTH-003]
blocks: [FR-BRAIN-102, FR-BRAIN-103, FR-BRAIN-106, FR-BRAIN-108, FR-BRAIN-105, FR-PROJ-008]

source_pages:
  - website/docs/modules/brain.html#layer-2
source_decisions:
...
new_files:
  ...
  - services/brain/src/main.rs
  - services/brain/src/layer2/mod.rs
  - services/brain/src/layer2/ingest.rs
  - services/brain/src/layer2/binlog_tail.rs
  - services/brain/src/layer2/pgvector.rs
  - services/brain/src/layer2/age.rs
  - services/brain/src/layer2/cursor.rs
  - services/brain/src/layer2/chain_anchor.rs
  - services/brain/src/layer2/entity_extract.rs
  - services/brain/migrations/0001_layer2.sql
  - services/brain/migrations/0002_layer2_cursor.sql
  - services/brain/tests/layer2_ingest_test.rs
  - services/brain/tests/layer2_chain_anchor_test.rs
  - services/brain/tests/layer2_cursor_resume_test.rs
  - services/brain/tests/layer2_idempotency_test.rs
  - services/brain/tests/layer2_tenant_isolation_test.rs
modified_files: []
allowed_tools:
  - file_read: services/brain/**
  - file_write: services/brain/{src,tests,migrations}/**
  - bash: cd services/brain && cargo test layer2
...
sub_tasks:
  ...
  - "1.0h: layer2/chain_anchor.rs — SHA-256 of canonical row bytes (matches BRAIN canonical-JSON)"
  ...
  - "1.0h: integration with FR-BRAIN-102 (rebuild from Layer 1)"
risk_if_skipped: "BRAIN search (FR-BRAIN-108) doesn't exist. KB retrieval, OBS dashboards, CUO ambient context all fail. Layer 2 is the read scale-out layer. Without 1s p95 lag, freshly-written memories are invisible to search for too long; the user-perceived 'I just saved this; why can't I find it?' regression. Without chain_anchor, Layer 1 tampering goes undetected at query time."
---
...
2. **MUST** compute BGE-M3 embedding via FR-AI-019 sidecar for every memory-body row; insert into pgvector with `(tenant_id, seq, id, kind, path, ts_ns, embedding 1024-dim, chain_anchor 32-bytes)`.
...
4. **MUST** tag every Layer 2 row with `chain_anchor = SHA-256(canonical(Layer 1 row at seq N))` for tamper detection. Read paths (FR-BRAIN-108) verify chain_anchor matches Layer 1 before returning results; mismatch → sev-1 + drop the result.
...
6. **MUST** target ingest staleness ≤ 1 second p95 from binlog append to Layer 2 visibility (DEC-074). Steady-state SLO measured via `brain_layer2_ingest_lag_seconds` histogram.
7. **MUST** emit OTel metrics:
    - `brain_layer2_ingest_lag_seconds{tenant_id}` (histogram; SLO p95 < 1s).
    - `brain_layer2_ingest_failures_total{tenant_id, reason}` (counter; reason ∈ bge_down | age_fail | postgres_error | chain_anchor_mismatch).
    - `brain_layer2_throughput_rows_per_sec{tenant_id}` (gauge).
    - `brain_layer2_cursor_advance_total{tenant_id}` (counter).
    - `brain_layer2_age_pending_retry_total{tenant_id}` (gauge; entries needing AGE backfill).
8. **MUST** support tenant isolation via `tenant_id` column + RLS (FR-AUTH-003 pattern). The `layer2_memories` table is in `TENANT_SCOPED_TABLES` registry; RLS USING + WITH CHECK clauses applied per FR-AUTH-003 §1 #2.
...
15. **SHOULD** support a `--rebuild` flag for FR-BRAIN-102 CI gate: starts from seq 0 for all tenants; ignores existing cursor; re-ingests everything from Layer 1.
...
6. **Embedding sidecar (FR-AI-019) failure → ingest retries with backoff; doesn't crash** — kill BGE; ingest queues retries; continues.
...
8. **CI rebuild test (FR-BRAIN-102) recreates Layer 2 from Layer 1 in <30min** for slice-1 data volume.
...
async fn bge_down_triggers_retry_no_crash() {
    ...
    let metric: u64 = otel_test_helper::counter_value("brain_layer2_ingest_failures_total", &[("reason", "bge_down")]);
    assert!(metric > 0);
    // ingest still alive
...
async fn chain_anchor_mismatch_sev1() {
    ...
    let metric: u64 = otel_test_helper::counter_value("brain_layer2_ingest_failures_total", &[("reason", "chain_anchor_mismatch")]);
    assert!(metric > 0);
}
...
- **FR-AI-019** — BGE-M3 sidecar (embeddings).
- **FR-AUTH-003** — RLS pattern + TENANT_SCOPED_TABLES registry (add `layer2_memories`).
- **FR-OBS-001** — OTel metrics.
- **FR-BRAIN-102 (downstream)** — rebuild CI gate uses `--rebuild` flag.
...
```text
sev-1  brain_layer2_chain_anchor_mismatch
...
| Embedding dim mismatch (BGE returns wrong dim) | sqlx schema check | INSERT fails; sev-2 | Investigate FR-AI-019 |
...
- spaCy NER + custom recognizers extract PERSON/ORG/PLACE/CONCEPT. Custom recognizers reuse FR-AI-012's VN_* set for Vietnamese-specific entities.
- The `--rebuild` flag is the FR-BRAIN-102 hook. CI rebuilds Layer 2 from Layer 1 to ensure derivability.
...
*End of FR-BRAIN-101. Status: draft (10/10 target).*
