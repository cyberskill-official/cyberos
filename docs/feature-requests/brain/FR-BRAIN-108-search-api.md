...
related_frs: [FR-BRAIN-101, FR-BRAIN-106, FR-AI-019, FR-AI-020, FR-AUTH-003, FR-AUTH-004]
depends_on: [FR-BRAIN-101, FR-BRAIN-107]
blocks: [FR-KB-007]   # placeholder — "Ask this page" KB Q&A FR, not yet specified (downstream consumer)

source_pages:
  - website/docs/modules/brain.html#search
source_decisions:
...
new_files:
  - services/brain/src/search/mod.rs
  - services/brain/src/search/vector.rs
  - services/brain/src/search/graph.rs
  - services/brain/src/search/fulltext.rs
  - services/brain/src/search/rrf.rs
  - services/brain/src/search/rerank.rs
  - services/brain/src/search/acl_filter.rs
  - services/brain/src/search/chain_anchor_verify.rs
  - services/brain/src/search/explain.rs
  - services/brain/migrations/0003_pgroonga.sql
  - services/brain/tests/search_test.rs
  - services/brain/tests/search_acl_test.rs
  - services/brain/tests/search_fallback_test.rs
  - services/brain/tests/search_chain_anchor_test.rs
  - services/brain/tests/search_p95_benchmark_test.rs
  - services/brain/tests/search_vn_query_test.rs
modified_files:
  - services/brain/src/handlers/mod.rs                   # add search route
allowed_tools:
  - file_read: services/brain/**
  - file_write: services/brain/{src,tests,migrations}/**
  - bash: cd services/brain && cargo test search
disallowed_tools:
...
risk_if_skipped: "BRAIN search is the read API for everything: KB retrieval (FR-KB-007), CUO ambient context, OBS auto-triage. Without it, all those modules fail. Without RLS+ACL, search leaks across tenants/actors. Without chain_anchor verification, Layer 1 tampering goes undetected at query time. Without graceful degrade, BGE outage takes down search entirely."
---

## §1 — Description (BCP-14 normative)

The BRAIN service **MUST** expose `GET /v1/brain/search` returning ranked memories. Each request:
...
2. **MUST** dispatch to 3 backends in parallel via `tokio::try_join!`:
    - **Vector**: BGE-M3 embed query (FR-AI-019) → cosine similarity search in `layer2_memories` HNSW index.
...
3. **MUST** combine via Reciprocal Rank Fusion (RRF) with k=60 (standard parameter); take top 50 candidates; rerank via FR-AI-020 BGE-reranker-v2-m3 → final top-N.
...
10. **MUST** verify `chain_anchor` for each returned result against Layer 1 (per FR-BRAIN-101 §1 #4). Mismatch → drop the result + emit sev-1 OBS event `brain_search_chain_anchor_mismatch{tenant_id, seq}`.
11. **MUST** authenticate via FR-AUTH-004 JWT; extract tenant_id + actor_id from claims.
...
14. **SHOULD** emit OTel metrics:
    - `brain_search_requests_total{tenant_id, outcome}` (counter; outcome ∈ ok | partial_degrade | full_failure | empty).
    - `brain_search_latency_ms{tenant_id}` (histogram; SLO p95 < 250ms).
    - `brain_search_backend_latency_ms{backend}` (histogram per vector/graph/fulltext).
    - `brain_search_chain_anchor_mismatch_total{tenant_id}` (counter; sev-1 alarm).
    - `brain_search_acl_filtered_total{tenant_id}` (counter; how many results filtered).
    - `brain_search_rerank_improvement` (histogram; rerank-position-delta tracking).
...
6. ACL filter: memories with `acl: [@alice]` invisible to @bob; metric `brain_search_acl_filtered_total` increments.
...
async fn acl_excludes_unauthorised_actor() {
    ...
    let metric: u64 = otel_test_helper::counter_value("brain_search_acl_filtered_total", &[("tenant_id", &tenant.to_string())]);
    assert!(metric > 0);
}
...
async fn chain_anchor_mismatch_drops_result_and_sev1() {
    ...
    let metric: u64 = otel_test_helper::counter_value("brain_search_chain_anchor_mismatch_total", &[("tenant_id", &tenant.to_string())]);
    assert!(metric > 0);
}
...
- **FR-BRAIN-101** — Layer 2 + chain_anchor.
- **FR-AI-019** — BGE-M3 sidecar.
- **FR-AI-020** — BGE-rerank-v2-m3.
- **FR-AUTH-003** — RLS enforcement.
- **FR-AUTH-004** — JWT.
- **FR-BRAIN-106** — sync_class informs ACL filter (related).
...
```http
GET /v1/brain/search?q=Decree%2013&kind=decisions&limit=10&explain=true HTTP/1.1
...
ERROR seq=12345 stored_anchor=abc... recomputed=def...
      brain_search_chain_anchor_mismatch; result dropped
sev-1 brain_search_chain_anchor_mismatch_total{tenant_id=...} incremented
```
...
- BGE-reranker-v2-m3 polishes top-50 with cross-encoder precision (FR-AI-020).
...
*End of FR-BRAIN-108. Status: draft (10/10 target).*
