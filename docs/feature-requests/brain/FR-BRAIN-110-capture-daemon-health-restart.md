---
id: FR-BRAIN-110
title: "BRAIN capture daemon supervision — systemd + launchd units + /healthz + watchdog + crash-restart with exponential backoff + sweeper cron"
module: BRAIN
priority: MUST
status: building
verify: T
...
related_frs: [FR-BRAIN-105, FR-BRAIN-107, FR-BRAIN-108, FR-BRAIN-109, FR-OBS-001, FR-OBS-007]
depends_on: [FR-BRAIN-105, FR-BRAIN-107, FR-BRAIN-109]
blocks: []

source_pages:
  - website/docs/modules/brain.html#daemon-supervision
  - website/docs/runbooks/brain-capture-runbook.html#supervision
source_decisions:
  ...
  - DEC-163 (hourly sweeper prunes /tmp/cyberos-brain-claude-traces, stale dedup cache, old metric snapshots)

language: rust 1.81 + systemd + launchd plist
service: cyberos/services/brain-capture/
new_files:
  - services/brain-capture/src/healthz.rs
  - services/brain-capture/src/sweeper.rs
  - services/brain-capture/install/systemd/cyberos-brain-capture.service
  - services/brain-capture/install/launchd/world.cyberos.brain-capture.plist
  - services/brain-capture/install/install-daemon.sh
  - services/brain-capture/install/uninstall-daemon.sh
  - services/brain-capture/tests/healthz_test.rs
  - services/brain-capture/tests/sweeper_test.rs
  - services/brain-capture/tests/restart_e2e_test.sh
modified_files:
  - services/brain-capture/src/main.rs                  # bind /healthz; start sweeper; install signal handlers
  - services/brain-capture/src/lib.rs                   # CaptureDaemon::health() + ::run_sweeper()
allowed_tools:
  - file_read: services/brain-capture/**
  - file_write: services/brain-capture/{src,install,tests}/**
...
effort_hours: 6
sub_tasks:
  - "0.5h: cyberos-brain-capture.service unit file (Restart=on-failure; RestartSec=exp backoff cap 5m via systemd RestartSteps)"
  - "0.5h: world.cyberos.brain-capture.plist (KeepAlive + ThrottleInterval + ExitTimeOut)"
...
1. **MUST** ship a systemd unit `cyberos-brain-capture.service` (Linux) AND a launchd plist `world.cyberos.brain-capture.plist` (macOS). The installer (`install-daemon.sh`) detects the OS and copies the appropriate file to:
    - Linux: `/etc/systemd/system/cyberos-brain-capture.service` (system-scope) OR `~/.config/systemd/user/cyberos-brain-capture.service` (user-scope, default for non-root install).
    - macOS: `~/Library/LaunchAgents/world.cyberos.brain-capture.plist` (user-scope; system-scope deferred to slice 3).
2. **MUST** auto-restart on non-zero exit per OS-native semantics:
    ...
    - launchd: `KeepAlive: true`, `ThrottleInterval: 60` (launchd does not natively support exponential backoff; we implement it in the daemon's own startup logic by sleeping `min(2^crash_count, 300)` seconds before binding sockets, where `crash_count` is read from `/tmp/cyberos-brain-capture-crashes`).
...
4. **MUST** return 503 when ANY of the following are true:
    - Any FR-BRAIN-105 error-severity invariant fails (checked every 60s; result cached).
    ...
    - The BRAIN writer subprocess (per FR-BRAIN-101) is unreachable.
5. **MUST** run a sweeper task on a 60-second tokio interval that prunes:
    - `/tmp/cyberos-brain-claude-traces/<uuid>` files older than 1 hour (FR-BRAIN-109 trace cache).
    - Dedup-cache entries whose `stored_at` exceeded TTL (5 minutes per FR-BRAIN-107 §1 #4).
    - Old metric snapshots in `/tmp/cyberos-brain-metrics/` older than 24 hours.
    - Crash-count file `/tmp/cyberos-brain-capture-crashes` if last reset > 1 hour ago (allows recovery from a transient crash storm without sticky exp-backoff).
6. **MUST** install signal handlers per AGENTS.md operational conventions:
    ...
    - `SIGHUP` → reload manifest (per FR-BRAIN-107 §1 #15); does not exit.
...
7. **MUST** track crash count in `/tmp/cyberos-brain-capture-crashes` (single u32 LE; incremented at startup; reset to 0 after 5 minutes of stable uptime). The daemon uses this to compute its own startup-delay backoff on launchd.
8. **MUST** emit OTel metrics:
    - `brain_capture_daemon_uptime_seconds` (gauge).
    - `brain_capture_daemon_restart_count_total` (counter; survives restart via the crash-count file).
    - `brain_capture_sweeper_pruned_total{kind}` (counter; kind ∈ trace_cache | dedup | metric_snapshot | crash_count_reset).
    - `brain_capture_health_endpoint_total{status}` (counter; status ∈ healthy | unhealthy).
9. **MUST** emit OTel span `brain.capture.sweeper.tick` per sweep with attributes `pruned_trace_cache`, `pruned_dedup`, `pruned_metric_snapshots`, `duration_ms`.
10. **MUST** emit a `brain.capture_supervisor_event` BRAIN audit row when the daemon process starts (kind=`started`), reloads manifest (kind=`reloaded`), or exits (kind=`exited`, `exit_code`, `uptime_seconds`). This gives operators a first-class audit trail for "when did the daemon last go down?"
...
12. **MUST** support `cyberos brain capture status` CLI which hits `/healthz` and pretty-prints the response (human-friendly format with colour-coded badges).
13. **MUST** support `cyberos brain capture logs [--follow] [--lines N]` which tails the daemon log file (`~/Library/Logs/cyberos-brain-capture.log` on macOS, `journalctl -u cyberos-brain-capture` on Linux).
...
15. **SHOULD** publish `cyberos-brain-capture.service` to a Homebrew formula (macOS) and a deb/rpm package (Linux) — slice 3+.
...
**Why supervisor_event audit rows (§1 #10)?** Operators investigating "BRAIN had a gap between 14:00 and 14:23" need to answer "was the daemon down?" Without a first-class audit row, they have to cross-reference systemd journalctl with BRAIN — friction. The row turns the supervisor's behaviour into queryable data.
...
**Why no `--restart-now` CLI command?** The OS init system owns the lifecycle. `systemctl restart cyberos-brain-capture` (Linux) and `launchctl kickstart -k gui/$UID/world.cyberos.brain-capture` (macOS) are the canonical paths. Adding a Cyberos-level wrapper duplicates them; operators benefit from learning the OS commands once.

**Why `cyberos brain capture logs` (§1 #13)?** Cross-platform log access is a friction point: `journalctl -u name --follow` on Linux vs tailing a file on macOS. The CLI command abstracts the difference.
...
```ini
# install/systemd/cyberos-brain-capture.service
[Unit]
Description=CyberOS BRAIN capture daemon
After=network.target
Documentation=https://docs.cyberos.world/runbooks/brain-capture-runbook.html

[Service]
Type=simple
ExecStart=/usr/local/bin/cyberos-brain-capture --foreground
Restart=on-failure
RestartSec=5s
...
EnvironmentFile=-/etc/cyberos/brain-capture.env
...
```xml
<!-- install/launchd/world.cyberos.brain-capture.plist -->
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
  <key>Label</key>             <string>world.cyberos.brain-capture</string>
  <key>ProgramArguments</key>  <array><string>/usr/local/bin/cyberos-brain-capture</string><string>--foreground</string></array>
  <key>RunAtLoad</key>         <true/>
  <key>KeepAlive</key>         <true/>
  <!-- launchd's anti-flap; daemon's own backoff handles longer delays -->
  <key>ThrottleInterval</key>  <integer>60</integer>
  <key>ExitTimeOut</key>       <integer>10</integer>
  <key>StandardOutPath</key>   <string>/Users/USERNAME/Library/Logs/cyberos-brain-capture.log</string>
  <key>StandardErrorPath</key> <string>/Users/USERNAME/Library/Logs/cyberos-brain-capture.err</string>
  <key>WorkingDirectory</key>  <string>/tmp</string>
  <key>EnvironmentVariables</key>
  <dict>
    <key>RUST_LOG</key>        <string>info,cyberos_brain_capture=debug</string>
...
pub async fn handler(state: Arc<HealthState>) -> (StatusCode, Json<HealthReport>) {
    ...
    metrics::counter!("brain_capture_health_endpoint_total", "status" => status).increment(1);
...
pub async fn run_sweeper_loop() {
    ...
    loop {
        ...
        let pruned_traces  = prune_dir_older_than("/tmp/cyberos-brain-claude-traces", Duration::from_secs(3600));
        let pruned_metrics = prune_dir_older_than("/tmp/cyberos-brain-metrics",       Duration::from_secs(86400));
        let reset_crashes  = maybe_reset_crash_count("/tmp/cyberos-brain-capture-crashes", Duration::from_secs(300));
        tracing::debug!(
            pruned_trace_cache = pruned_traces,
            pruned_metric_snapshots = pruned_metrics,
            crash_count_reset = reset_crashes,
            duration_ms = start.elapsed().as_millis() as u64,
            "brain.capture.sweeper.tick"
        );
        metrics::counter!("brain_capture_sweeper_pruned_total", "kind" => "trace_cache").increment(pruned_traces as u64);
        metrics::counter!("brain_capture_sweeper_pruned_total", "kind" => "metric_snapshot").increment(pruned_metrics as u64);
        if reset_crashes {
            metrics::counter!("brain_capture_sweeper_pruned_total", "kind" => "crash_count_reset").increment(1);
        }
    }
}
...
case "$(uname -s)" in
  Linux)
    UNIT=cyberos-brain-capture.service
    ...
    ;;
  Darwin)
    PLIST="world.cyberos.brain-capture.plist"
...
2. **systemd: daemon restarts on non-zero exit** — `kill -SEGV $(pidof cyberos-brain-capture)` → process restarts within `RestartSec=5s` ± 2s; `MainPID` differs.
3. **systemd: exp backoff cap at 5min** — crash 10× rapidly → 10th restart waits ~5 min (within ±10s).
4. **launchd: daemon starts on `launchctl load`** — fixture installs plist; `launchctl print gui/$UID/world.cyberos.brain-capture` shows state=running.
...
11. **Sweeper prunes trace cache** — write `/tmp/cyberos-brain-claude-traces/<uuid>` with mtime 2h ago → next tick removes it; metric `brain_capture_sweeper_pruned_total{kind="trace_cache"}` increments.
12. **Sweeper prunes metric snapshots** — fixture file in `/tmp/cyberos-brain-metrics/` with mtime 25h ago → pruned.
13. **Sweeper resets crash count after stable uptime** — write crash-count file with mtime 6 min ago → sweeper resets to 0.
14. **SIGTERM graceful shutdown** — send SIGTERM with full queue → daemon drains queue (≤ 5s), emits `brain.capture_supervisor_event` kind=`exited`, exits 0.
...
17. **supervisor_event row on start** — fresh start → exactly one `brain.capture_supervisor_event` row with kind=`started`.
...
20. **uninstall-daemon.sh total cleanup** — uninstall → unit/plist removed; daemon stopped; `/tmp/cyberos-brain-capture-crashes` removed.
21. **cyberos brain capture status** — pretty-prints `/healthz` JSON with colour badges (PASS/WARN/FAIL per reason).
22. **cyberos brain capture logs --follow --lines 100** — tails the appropriate log source; works on both Linux + macOS.
23. **OTel metrics: uptime + restart count** — Prometheus registry has both metrics; restart count survives a SIGSEGV→restart cycle.
24. **FR-OBS-007 alert fires** — 503 for 60s → sev-2 alert with `service: brain-capture` label.
...
sleep 2
[ "$(systemctl --user is-active cyberos-brain-capture)" = "active" ]

# Crash + restart
ORIG_PID=$(systemctl --user show -p MainPID --value cyberos-brain-capture)
kill -9 "$ORIG_PID"
sleep 7
NEW_PID=$(systemctl --user show -p MainPID --value cyberos-brain-capture)
[ "$ORIG_PID" != "$NEW_PID" ]
[ "$(systemctl --user is-active cyberos-brain-capture)" = "active" ]

# Crash 10× rapidly → 10th restart should be at 5 min cap (sample 3 to keep CI under 5 min)
for i in 1 2 3; do
  kill -9 $(systemctl --user show -p MainPID --value cyberos-brain-capture)
  sleep 1
done
# After 3 crashes, RestartSec should be 30s (per RestartSteps); verify within ±5s
START=$(date +%s)
while [ "$(systemctl --user is-active cyberos-brain-capture)" != "active" ]; do
  sleep 1
...
```rust
// services/brain-capture/src/main.rs (excerpt — daemon entrypoint)
...
async fn main() -> cyberos_cli_exit::ExitCode {
    ...
    // Doctor gate (FR-BRAIN-105 §1 #13)
    if let Err(e) = run_doctor_gate().await {
        tracing::error!(?e, "doctor invariant failure; refusing to start");
        return ExitCode::InternalError;
    }

    // Start the daemon (FR-BRAIN-107) and supervisory pieces
    let daemon = cyberos_brain_capture::CaptureDaemon::start(cli.manifest, cli.dry_run).await
    ...
    tokio::spawn(async move {
        axum::serve(healthz_listener, cyberos_brain_capture::healthz::router(health_state)).await.unwrap();
    });

    // Spawn sweeper
    tokio::spawn(cyberos_brain_capture::sweeper::run_sweeper_loop());
...
- **FR-BRAIN-105 (upstream)** — `cyberos doctor --only watched-folders` is the gate at boot AND the periodic check feeding `/healthz`.
- **FR-BRAIN-107 (upstream)** — capture daemon being supervised; this FR adds the supervision wrapper.
- **FR-BRAIN-108 (related)** — Cowork session capture FR is downstream consumer (its hook posts to this daemon's socket).
- **FR-BRAIN-109 (related)** — Claude Code hook capture; same socket consumer + the `/tmp/cyberos-brain-claude-traces/` cache this FR sweeps.
...
### `brain.capture_supervisor_event` (start)

```json
{
  "kind": "brain.capture_supervisor_event",
  "payload": {
    "event":     "started",
    "version":   "0.1.0",
    "pid":       4823,
...
| `cyberos brain capture status` can't reach /healthz | connection refused | CLI prints error; exit 1 | Operator checks daemon is running |
| `cyberos brain capture logs` on macOS but log file path wrong | tail Err | CLI prints error | Operator updates plist path or accepts default |
...
- The crash-count file location (`/tmp/cyberos-brain-capture-crashes`) is intentionally NOT in the BRAIN root — it's runtime state, not memory. /tmp is the right scope.
- The `supervisor_event` rows are written via `BrainWriter::emit` (not via the daemon's own queue) because they may need to fire AFTER the queue stops (e.g. exit path). They're synchronous with-deadline; if BRAIN is down they're dropped (logged) — acceptable since the supervisor restart-state is also surfaced via systemd journal.
- The `cyberos brain capture logs` CLI shells out to `journalctl` on Linux and `tail -F` on macOS; portable cross-platform log streaming would need a Rust journal-reader crate (`systemd@0.10`), deferred.
- `install-daemon.sh` and `uninstall-daemon.sh` are bash for the same reasons as FR-BRAIN-109's `install-hooks.sh`: small, calls standard tools (`systemctl`, `launchctl`, `sed`, `install`).
