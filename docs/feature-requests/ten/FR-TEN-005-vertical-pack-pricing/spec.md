...
source_decisions:
  - DEC-1300 2026-05-17 — Vertical packs (e.g. "Legal Pack", "Construction Pack" per FR-SKILL-107) priced as flat per-pack-per-month add-on; NOT per-seat (consultancies often have 1 partner managing 50 client engagements through the same pack — per-seat would double-bill)
  ...
  - DEC-1303 2026-05-17 — Installation triggers prorated invoice via FR-TEN-003 (stripe) or FR-TEN-102 (vnd); uninstall triggers prorated credit (Stripe) or skip-next-billing (vnd)
...
6. **MUST** expose `POST /v1/admin/tenants/{tid}/packs/install` body `{ pack_id }`. Handler:
   - Validates pack exists in FR-SKILL-107 registry.
   - Tier-gate check per §1 #7.
   - 50-pack-limit check per DEC-1307.
   - Duplicate check (partial unique).
   - Resolve price per §1 #8.
   - Prorate invoice via FR-TEN-003 (stripe-rail) or FR-TEN-102 (vnd-rail).
...
9. **MUST** expose `POST /v1/admin/tenants/{tid}/packs/{install_id}/uninstall` per DEC-1305:
   ...
   - Skill bundle remains accessible 14 days (FR-SKILL-107 reads our status).
...
11. **MUST** monthly recurring charge at billing-cycle-anchor (consistent with FR-TEN-003 DEC-788). Per active install, dispatch charge via configured rail. On failure: increment retry; after FR-TEN-003-style 3 retries → status='suspended_billing_failed' + emit `ten.pack_billing_failed` sev-1.
...
20. **MUST NOT** charge VND tenant via Stripe rail or vice versa (consistent with FR-TEN-003 DEC-784 + FR-TEN-102 DEC-973).
...
13. **VND tenant uses FR-TEN-102 rail** — pack billing routes through VND PSPs.
...
|---|---|---|---|
| Pack not in registry | FR-SKILL-107 lookup | 404 | Caller fixes pack_id |
...
| Currency change attempt on existing install | billing_currency immutable per FR-TEN-003 | Schema rejection | Inherent |

---

## §11 — Implementation notes

**§11.1** Pack install creates a row in `vertical_pack_installs` AND notifies FR-SKILL-107 to activate the bundle.
