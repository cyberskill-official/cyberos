...
disallowed_tools:
  ...
  - allow tenant-admin to initiate hostile-termination fast-track (per DEC-511 — CEO+CLO+CSO required via FR-TEN-202)
...
4. **MUST** enforce the closed FSM transition matrix (per DEC-500 + DEC-502):
    ...
    - `terminating_a → terminating_b` (scheduled advance after 30 days; OR FR-TEN-202 fast-track).
    - `terminating_b → terminated` (scheduled advance after 60 days IN B + CSO+CLO dual-signoff).
    - `active → terminating_b` (FR-TEN-202 hostile-termination fast-track; CEO+CLO+CSO sign-off).
...
11. **MUST** ship `POST /v1/ten/offboarding/finalize-termination` (per DEC-506). Body: `{tenant_slug, signer1_subject_id, signer2_subject_id, reason, confirmation: "I_AUTHORISE_IRREVERSIBLE_DELETE"}`. Validates:
    ...
    - `signer1` has role `cseco` (Chief Security Officer) per FR-AUTH-101.
...
19. **MUST** ensure the read-only gate caches per-tenant state with 60-second TTL (matching FR-AUTH-109's pattern). Cache invalidated on state transition.
...
**Why FR-TEN-202 fast-track bypasses 30-day terminating_a (DEC-511)?** Hostile termination (regulatory order, court injunction) demands faster action than 90-day grace allows. CEO+CLO+CSO triple-sign-off (vs CSO+CLO dual for normal flow) reflects the elevated authority. This FR ships the FSM that FR-TEN-202 consumes; the fast-track handler is FR-TEN-202.
...
BEGIN
    ...
    IF NOT (
        ...
        OR (OLD.offboarding_state = 'active'      AND NEW.offboarding_state = 'terminating_b')   -- FR-TEN-202 hostile
...
pub fn is_valid_transition(from: OffboardingState, to: OffboardingState) -> bool {
    ...
    matches!((from, to),
        (Active, TerminatingA)
        | (Active, TerminatingB)       // FR-TEN-202 hostile
...
pub async fn finalize_termination(
    ...
    // Trigger FR-TEN-106 attestation row (out of scope here; placeholder)
    Ok(StatusCode::OK)
}
```
...
7. **active → terminated direct forbidden** — must pass through B (only FR-TEN-202 fast-track allows active → terminating_b).
...
**Upstream:**
- **FR-TEN-001** — tenant provisioning (this FR consumes tenants table).

**Downstream (2 placeholders):**
- **FR-TEN-105** — signed-bundle export (consumes terminating_a state to gate exports).
- **FR-TEN-106** — permanent-delete attestation row (consumes terminating_b → terminated transition).
...
- **FR-TEN-202 fast-track via active → terminating_b** — this FR ships FSM; TEN-202 ships the triple-signoff handler.
