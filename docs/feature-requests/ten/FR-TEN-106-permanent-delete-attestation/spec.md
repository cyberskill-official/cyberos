...
source_decisions:
  - DEC-1340 2026-05-17 — Permanent-delete attestation = the final, irreversible step in tenant lifecycle; transitions tenant from `terminating` (FR-TEN-104) to fully hard-purged; dual-sign (CSO + CLO) required because the action is unrecoverable
...
build_envelope:
  ...
  disallowed_tools:
    ...
    - skip bundle precondition (per FR-TEN-105 DEC-1330)
...
5. **MUST** expose `POST /v1/admin/tenants/{tid}/permanent-delete/initiate` body `{ reason }`. Caller has `cso` OR `tenant_admin` role. Handler validates preconditions:
   - `tenants.status = 'terminating'` (set by FR-TEN-104) — else 412 + `tenant_not_terminating`.
   - `now() - tenant.terminating_at >= 30 days` — else 412 + `cool_off_not_elapsed`.
   - Recent bundle within 90d per FR-TEN-105 §1 #14 — else 412 + `bundle_export_required_before_attestation` (referenced from FR-TEN-105 cross-FR contract).
...
10. **MUST** cascade purge in fixed order per DEC-1343 + DEC-1348:
    ...
    3. **`kms_keys`** — `kms schedule-key-deletion` for tenant's keys (signing key from FR-TEN-105, encryption keys); 30d grace before AWS final deletion.
...
**Why 30-day cool-off (§1 #5, DEC-1341)?** Buyer's remorse + accidental termination. 30 days gives the tenant time to reverse termination (FR-TEN-104 reverse flow) before destruction.

**Why bundle precondition (§1 #5, DEC-1341 + FR-TEN-105 DEC-1330)?** Recovery path. Bundle = last copy of tenant data. Without recent bundle, no way to restore if deletion was wrongful.
...
| Tenant not terminating | precondition | 412 | Initiate termination via FR-TEN-104 |
