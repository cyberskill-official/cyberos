...
source_decisions:
  - DEC-2410 2026-05-17 — Fast-track FR-HR-009 termination bypassing standard CEO+CFO co-sign with CEO+CLO+CSO triple-sign for hostile actors (legal trigger justification required)
...
3. **MUST** require legal trigger doc via FR-DOC-001 per DEC-2412 — case ref + brief description.

4. **MUST** cascade via fast-path FR-HR-009 termination + AUTH revocation at `cascade.rs::execute(override)` per DEC-2410 — bypass standard CEO+CFO sign (this override IS the sign).
...
6. **MUST** define table at migration `0011`:
   ...
   CREATE TABLE ten_hostile_overrides (
     ...
     legal_doc_id UUID NOT NULL,  -- FR-DOC-001 ref
...
11. **MUST NOT** skip legal doc per DEC-2412 (FR-DOC-001 FK enforced).
