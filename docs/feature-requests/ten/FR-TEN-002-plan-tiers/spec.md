...
new_files:
  - docs/feature-requests/ten/PLAN_CAPS.md
...
source_decisions:
  ...
  - DEC-783 2026-05-16 — Plan-change handler issues a FR-TEN-004 metering event tagged with `plan_change` for audit linkage
...
risk_if_skipped: "Without explicit plan tiers, every tenant gets the same caps; either too generous (we leak money) or too restrictive (we lose customers). Plan tiers are the primary commercial differentiator and the substrate for FR-TEN-003 Stripe billing."
---

## §1 — Description (BCP-14 normative)

The TEN service **MUST** define exactly three hardcoded plan tiers — Starter, Team, Enterprise — with per-tier caps on the four FR-TEN-004 metering axes, expose a plan-change API, and persist tier transitions as append-only history.
...
| enterprise | NULL (∞) | NULL (∞) | 50_000_000 | 1 TiB |
   ...
   NULL means unlimited for that axis. Enterprise has a finite ai_tokens cap because tokens map to provider pass-through costs and "unlimited" would expose us to cost explosions (DEC-780). Storage above 1 TiB on Enterprise is billed per-GB (out of scope here; see FR-TEN-003).

3. **MUST** default new tenants to `starter` at FR-TEN-001 provisioning (DEC-783 ref). The default is wired in `services/ten/src/handlers/tenant_create.rs` — no other code path may write `plan_tier` at insert.

4. **MUST** expose `POST /v1/admin/tenants/{id}/plan` to change a tenant's plan. The handler requires:
   - The caller's role is `tenant_admin` (per FR-AUTH-101) for upgrade/downgrade of their own tenant, OR `cyberskill_founder` for any tenant.
...
15. **MUST** record the plan change in the FR-TEN-004 metering event stream with a synthetic event of axis-independent kind `plan_change` carrying `idempotency_key = "plan_change_<history_id>"` (DEC-783). This enables billing-side reconciliation between plan history and metering events.
...
18. **MUST** maintain a `tenants.next_scheduled_change` nullable column for deferred downgrades. A row is set when a downgrade is requested with `effective: "next_period"`; the period-close job (FR-TEN-004 #22) applies the change as part of period freeze. After application, the column is cleared.
...
**§2.12  Why deferred downgrades go through period-close.** Clause #18 + FR-TEN-004 #22. The metering period-close handler is already the moment of accounting truth (seats + storage snapshot). Applying the plan change at that same boundary keeps all billing-relevant changes atomic — the tenant's bill for the new period uses the new tier.
...
**§2.21  Why we don't ship a plan-comparison UI as part of this FR.** Out of scope. The marketing site + the tenant-admin SPA (FR-TEN-107) own the UI; this FR is the data + API substrate.
...
3. New tenant default `plan_tier = starter` after FR-TEN-001 provisioning.
...
26. Per-tenant `metering_caps_yaml` override stronger than plan caps (FR-TEN-004 integration).
27. `dry_run=true` returns preview without DB write, audit row, or rate-limit consumption.
28. Plan change emits a FR-TEN-004 metering event with kind=plan_change linked by history_id.
...
- `metering_event_link_test` — plan change emits FR-TEN-004 event with history_id.
...
**Depends on**: FR-TEN-001 (provisioning CLI + tenants table).

**Blocks**: FR-TEN-005 (vertical-pack pricing add-on).
...
- **OQ-4** (open): vertical-pack pricing (FR-TEN-005) layers add-ons on top of base plan; the interaction model needs FR-TEN-005 + FR-TEN-003 to be finalized.
...
**§11.4** The period_end helper queries the tenant's billing timezone + cycle to compute the next period boundary. Mirrors FR-TEN-004 #18 pattern.

**§11.5** The downgrade-violation check queries the materialized `metering_current_period` view (FR-TEN-004 #20) for each axis and compares to target tier's cap.
...
**§11.12** Plan changes for terminated tenants (FR-TEN-104) are rejected at the handler (404). The trigger would catch it too via RLS, but the handler's explicit check is more user-friendly.
...
**§11.14** The CI test `caps_drift_test` reads constants from `caps.rs` via cargo-expand and compares to a documented table in `docs/feature-requests/ten/PLAN_CAPS.md` (one-source-of-truth audit).
...
**§11.20** The founder-tenant detection is via the `is_founder_tenant` boolean; the role `cyberskill_founder` is a subject role (per FR-AUTH-101). A subject with role `cyberskill_founder` can operate on any tenant via the founder-override handler; a non-founder cannot change a founder tenant via any path.
