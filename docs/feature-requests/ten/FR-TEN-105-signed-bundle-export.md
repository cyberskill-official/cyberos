...
source_decisions:
  - DEC-1320 2026-05-17 — Tenant offboarding (FR-TEN-104 termination flow) MUST produce a deterministic exportable bundle of all tenant data + signed manifest before permanent deletion; this is the auditable evidence chain
  - DEC-1321 2026-05-17 — Bundle format: ZIP with sorted entries + fixed mtime 2000-01-01T00:00:00Z + mode 0o644 (matches AGENTS.md §10 portability pattern + FR-TEN-103 export deterministic guarantee)
...
risk_if_skipped: "Without signed-bundle export, tenant offboarding (FR-TEN-104) has no auditable evidence trail — when the tenant claims data wasn't fully exported or was tampered with, we have no proof. Without DEC-1322 per-tenant signing keys, one compromised key invalidates all tenants' bundles. Without DEC-1330 pre-deletion gate, FR-TEN-106 permanent-delete could happen without a recovery option = data-loss incident. Without DEC-1321 deterministic zip, re-export shows hash mismatches → tenant disputes integrity. Without DEC-1331 audit chain inclusion, post-deletion legal queries have no evidence."
---
...
3. **MUST** define `tenant_signing_keys` at migration `0027`: `(tenant_id UUID PRIMARY KEY, public_key_pem TEXT NOT NULL, private_key_kms_blob BYTEA NOT NULL, kms_key_id TEXT NOT NULL, generated_at TIMESTAMPTZ NOT NULL DEFAULT now())`. One key per tenant; generated at FR-TEN-001 provisioning per DEC-1322.

4. **MUST** generate Ed25519 keypair at tenant provisioning per DEC-1322. Modification to FR-TEN-001 orchestrator: after tenant row created, generate keypair via `ed25519-dalek`, KMS-encrypt private key, insert `tenant_signing_keys` row.
...
5. **MUST** expose `POST /v1/admin/tenants/{tid}/bundle/export` body `{ reason }`. Caller has `tenant_admin` OR `cso` role. Handler:
   ...
   - Enqueues build task via FR-MCP-007.
...
14. **MUST** enforce pre-deletion gate per DEC-1330. FR-TEN-106 attestation handler checks:
...
22. **MUST NOT** permanent-delete tenant without valid recent bundle (per DEC-1330 gate enforced in FR-TEN-106).
...
|---|---|---|---|
| Build task fails | FR-MCP-007 error | status='failed' + sev-1 audit | Operator retries |
...
| Build interrupted by deploy | task retry via FR-MCP-007 checkpoint | Resumes from last checkpoint | Inherent |
...
**§11.6** Build task uses FR-MCP-007 worker pool; checkpoints per-source so crash mid-build resumes.
...
**§11.9** Per-tenant residency: bundle stored in tenant's residency S3 per FR-TEN-103 trip-wire.

**§11.10** Chain-of-custody anchor at FR-OBS-009 cross-system; future-proofs auditor questions.

---

*End of FR-TEN-105 spec.*
