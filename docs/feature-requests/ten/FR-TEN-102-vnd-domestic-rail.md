...
source_decisions:
  ...
  - DEC-971 2026-05-17 — Per-PSP webhook signature verification (per FR-INV-005 pattern): VnPay HMAC-SHA512 over query params; Momo HMAC-SHA256 over JSON body; ZaloPay HMAC-SHA256 over key1
  ...
  - DEC-973 2026-05-17 — vn-1 tenant CANNOT cross-rail to Stripe (mirrors FR-TEN-003 DEC-784 — VND tenants are VND-only)
  - DEC-974 2026-05-17 — Founder tenant skip applies (FR-TEN-003 DEC-805 derivative) — internal-rail tenants bypass VND rail too
  ...
  - DEC-986 2026-05-17 — Per-PSP webhook dispatch via NATS (mirrors FR-TEN-003 DEC-793): inv layer captures + verifies, NATS publishes to `tenant.<slug>.ten.vnd.<psp>.<event>`, TEN dispatcher consumes
  - PDPL Law 91/2025 Art. 7 (consent — PSP token auth recorded as explicit consent + versioned at vnd_consents table inline within tenant_consents per FR-TEN-101)
...
build_envelope:
  ...
  modified_files:
    - services/ten/src/handlers/plan_change.rs                         # for VND tenants, push plan_change as charge-side adjustment (FR-TEN-002 next period)
    ...
    - services/inv/src/webhook/                                        # add momo.rs + zalopay.rs (VnPay covered in FR-INV-005)
  ...
  disallowed_tools:
    - charge a non-VND tenant via VND rail (per DEC-973 — symmetric to FR-TEN-003 cross-rail block)
...
sub_tasks:
  ...
  - "0.5h: dunning.rs (state machine mirror of FR-TEN-003)"
...
risk_if_skipped: "Without VND rail, vn-1 tenants cannot pay — every signup in Vietnam fails at the payment step (per FR-TEN-101 §1 #13's 503 placeholder). This kills the entire VN go-to-market (the founder's home market + the largest single-country opportunity in early commercial). Without DEC-968 hóa đơn issuance, every charge violates VN Decree 123 — illegal billing + tax-authority enforcement risk + commercial-licence revocation. Without DEC-971 per-PSP webhook signature verification, attackers forge fake successful payments. Without DEC-963 KMS-wrapped tokens, a DB leak exposes recurring-charge tokens that can drain customer wallets. Without DEC-983 gap-free hóa đơn numbering, tax authority audit fails (Circular 78 §3 mandates gap-free). Without DEC-967 overage modeling, FR-TEN-004 metering events for vn-1 tenants are stranded (no rail to bill overages). Without DEC-973 cross-rail block, payment-route logic could attempt Stripe on a VND tenant + leak data cross-region. Without DEC-981's billing_contact_phone capture, PSPs cannot SMS-OTP for token auth = signup fails. The 12h effort lands the VN commercial primitive; without it, vn-1 residency exists but cannot bill."
---
...
7. **MUST** enforce RLS with both USING and WITH CHECK on all 5 VND tables, scoped to `tenant_id = current_setting('auth.tenant_id')::uuid` AND `current_setting('auth.residency') = 'vn-1'` (FR-TEN-103 trip-wire integration).
...
9. **MUST** support the token-bind flow at signup per DEC-963. Flow:
   ...
   Total signup-flow target: <30s same as FR-TEN-101 SLO, accounting for ~10s PSP redirect+auth time.
...
11. **MUST** charge overage at period_close per DEC-967. The `vnd/overage.rs::charge_for_period(tenant_id, period_end)`:
    ...
    - 1-hour push window (per FR-TEN-003 DEC-810); 24h retry deadline.

12. **MUST** advance dunning per DEC-965. Mirrors FR-TEN-003 §1 #11:
    ...
    - On `suspended`: trigger FR-TEN-104 tenant suspension + emit `ten.tenant_billing_suspended_vnd` sev-1.
...
13. **MUST** expose `POST /v1/admin/tenants/{id}/vnd/refund` for CFO-gated refunds per DEC-966. Body: `{ original_charge_ref, amount_vnd, reason }`. Validations:
    - Caller has `cfo` role per FR-AUTH-101.
...
15. **MUST** support per-PSP webhook ingestion via FR-INV-005 extension (modified_files: `services/inv/src/webhook/momo.rs` + `zalopay.rs` + their signature verifiers; VnPay already covered by FR-INV-005). The INV layer:
...
21. **MUST NOT** charge the founder tenant per DEC-974. Guard mirrors FR-TEN-003 §1 #22 — emits sev-3 `ten.vnd_founder_skip` informational row.

22. **MUST** lock `billing_currency` immutable post-provisioning per DEC-977 (already enforced by FR-TEN-003's `trg_billing_currency_immutable` trigger).
...
**Why async PSP responses + Postgres LISTEN/NOTIFY (§1 #10, DEC-976)?** VN PSPs return `200 + processing` immediately; real outcome arrives via webhook within seconds-to-5min. Polling = wasted load; LISTEN/NOTIFY = real-time wake-up when the webhook handler INSERTs. Same pattern as FR-INV-005's webhook dispatch.
...
**Why dunning state machine parallel to FR-TEN-003 (§1 #12, DEC-965)?** Operational simplicity. Two rails (Stripe + VND) with identical dunning state machines mean operators learn one model. Per-PSP dunning differences would explode the operator-mental-model surface.
...
**Why cross-rail block (§1 #20, DEC-973)?** Tenant `billing_currency` is locked at provisioning (FR-TEN-003 DEC-798). A VND tenant attempting Stripe rail = bug, either client-side or server-side. Fail closed at the rail entry; trip-wire catches at schema level too (FR-TEN-103 §1 #8 cross-residency trigger).
...
7. **Dunning state machine** — three consecutive `charge_failed` → `retry_3`; fourth → `suspended` + FR-TEN-104 suspension.
...
**Upstream (depends_on):**
- **FR-TEN-003** Stripe billing — provides billing-rail abstraction this FR consumes (rail trait pattern + plan_change push hook + dunning state machine pattern + refund flow pattern); founder-skip + cross-rail block guards consumed.
- **FR-INV-005** VietQR webhook — VnPay webhook handler shape consumed; extended for Momo + ZaloPay variants.

**Cross-module (related_frs):**
- **FR-TEN-001** Provisioning — billing_contact_phone capture at vn-1 signup.
- **FR-TEN-002** Plan tiers — VND prices in catalog (PRICE_CATALOG VND entries).
- **FR-TEN-004** 4-axis metering — period_close hook invokes vnd::overage::charge_for_period.
- **FR-TEN-101** Self-serve signup — VND path now functional (FR-TEN-101 §1 #13's 503 placeholder resolved).
- **FR-TEN-103** Residency provisioning — vn-1 routing for PSP credentials + KMS.
...
- **Deferred:** Annual billing cycle for VND — slice 3 (monthly only at slice 2, mirrors FR-TEN-003).
...
| RLS bypassed (residency drift detected) | FR-TEN-103 trip-wire | INSERT/UPDATE blocked; sev-1 `ten.cross_residency_write_blocked` | Inherent — defense-in-depth |
...
**§11.10** The dunning state machine reuses FR-TEN-003's `DunningState` enum (Ok/Retry1/Retry2/Retry3/Suspended); same advancement semantics; same un-suspend on success.
...
**§11.15** The 7-day idempotency cache TTL (mirror of FR-TEN-003 DEC-807) — Stripe's idempotency window is 24h, VND PSPs vary (some don't enforce window). 7d is forensically generous + matches existing pattern.
...
**§11.19** OBS dashboard per-PSP success-rate watching alerts on sustained <95% (3 PSP outages within a week = sev-1 escalation per FR-OBS-007).

**§11.20** Cargo dependency on `hmac` + `sha2` (for VnPay HMAC-SHA512); already in workspace from FR-INV-005's VietQR pattern.
...
**§11.22** The `vnd_event_dispatch_log` mirrors FR-TEN-003's `stripe_event_dispatch_log` table shape; consistent operational model across rails.

---

*End of FR-TEN-102 spec.*
