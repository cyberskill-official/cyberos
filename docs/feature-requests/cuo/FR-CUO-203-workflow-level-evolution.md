...
5. **MUST** route workflow-refinement proposals to the SAME `docs/proposals/open/` directory as skill proposals (FR-CUO-201). They are distinguishable by `frontmatter.kind: workflow_refinement` vs `skill_refinement`.
6. **MUST** extend FR-CUO-202's proposal classifier to handle workflow diffs:
...
Skill stripes from FR-CUO-201 have format `<skill_slug>:<signal_id>:<hash>`; workflow stripes have format `<persona>/<workflow_slug>:<signal_id>:<hash>`. The presence of `/` in workflow stripes makes them disjoint from skill stripes — no collision possible. `cyberos-cuo proposal list` SHOULD group by kind (skill vs workflow) for readability.
...
In scope: per-workflow metrics aggregator, threshold-signal evaluator (reusing FR-CUO-200's signal-function pattern), workflow stripe computer, proposal authoring (LLM with workflow self-reflection prompt), classifier extension, CLI subcommands, audit row emission.
...
- **FR-CUO-200** — read-only harness foundation
- **FR-CUO-201** — proposal emitter + stripe dedup (reused for workflow stripes)
- **FR-CUO-202** — proposal applier (extended in §1 #6 for workflow diff buckets)
...
ALL workflow-level diffs default to queue (§1 #6 says "All workflow diffs default to QUEUE — workflows are higher-stakes than individual skills"). Auto-apply requires an explicit `--auto-workflow-diffs` flag on `cyberos-cuo proposal apply`, AND the proposal bucket must still classify as `condition_tune` or below. Pattern changes (e.g. `linear → time_critical`) are MAJOR + always queue regardless of flags. Stripe-repeat-halt (inherited from FR-CUO-201) means the harness halts immediately rather than proposing a second workflow edit for the same root cause.
...
6. The classifier (extension of FR-CUO-202) returns `step_addition` → `minor` + queue for a proposal that inserts a new step in the `skill_chain:` block. *(traces_to: §1 #6)*
7. Repeat occurrence of the same workflow stripe (per FR-CUO-201's halt-on-repeat rule) halts the workflow with `HITL_HALT`. *(traces_to: §1 #5 via FR-CUO-201 §1 #7)*
