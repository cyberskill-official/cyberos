...
source_decisions:
  - DEC-2370 2026-05-17 — On FR-CUO-104 chain step failure, walker invokes registered compensating action for each completed step in REVERSE order; partial-execution preserved (not rolled back if no compensation registered)
...
risk_if_skipped: "Without rollback, FR-CUO-104 partial executions leave systems in inconsistent state. Without DEC-2370 reverse order, compensations applied wrong direction. Without DEC-2372 missing-compensation preservation, naive rollback breaks systems where compensation unsafe."
---
...
2. **MUST** trigger on FR-CUO-104 step failure (chain status=failed) per DEC-2370.

3. **MUST** execute at `executor.rs::rollback(chain_id)` per DEC-2370:
   - Fetch completed steps in REVERSE order
   - For each: look up compensating action via FR-SKILL-001 registry
...
6. **MUST** update FR-CUO-104 chain status to `partial_rolled_back` post-completion per DEC-2373.
