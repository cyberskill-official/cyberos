...
depends_on: [FR-CUO-200, FR-CUO-201, FR-CUO-202, FR-CUO-203]
---
...
2. The loop MUST reuse the FR-CUO-201/202/203 propose cycle and MUST run it against the golden sets only. It MUST NOT read or touch production tenant data.

3. A proposed change MUST be applied only if all three hold: (a) it passes the AWH gate (author then test then gate, green); (b) it falls inside the evolution envelope of clause 4; (c) FR-CUO-202 classifies it low-risk. If any condition fails, the change MUST NOT be applied.
...
8. The loop MUST emit memory audit kinds through the FR-CUO-201 audit path: `cuo.dream_started`, `cuo.dream_proposal`, `cuo.dream_applied`, `cuo.dream_halted_hitl`, `cuo.dream_reverted`, each carrying the run id, the target artifact, and the outcome.
...
Keep evolution human-triggered only (the FR-CUO-200..203 status quo). Rejected: it does not meet the "improve while offline" ask. Idle time stays unused and every refinement waits on attention.
...
- Source: memory audit rows (FR-CUO-201 path) and the AWH gate log.
...
- FR-CUO-200 harness read-only report - the outcome-distribution metrics the loop reads.
- FR-CUO-201 refinement proposals - the proposal shape and the audit path the loop reuses.
- FR-CUO-202 auto-bump low-risk - the risk classifier that gates auto-apply.
- FR-CUO-203 workflow-level evolution - the workflow-edit proposals the loop can apply.
...
- Tools used: Claude (Cowork), authoring this FR from Stephen's capability request and the existing FR-CUO-200..203 harness specs.
