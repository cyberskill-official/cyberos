# memory Auto-Sync — Universal Personal + Lumi's memory
...
> - **Layer-2 service** → [`../services/memory/README.md`](../services/memory/README.md) (Rust scaffold, Wave 1)
> - **Multi-device sync FR** → [`feature-requests/memory/FR-MEMORY-103-multi-device-sync.md`](feature-requests/memory/FR-MEMORY-103-multi-device-sync.md)
> - **Per-stage FRs** → [`feature-requests/memory/`](feature-requests/memory/) (FR-MEMORY-101…111)
>
...
Every person who triggers the protocol gets one **Personal memory** — an append-only, audit-chained, offline-first long-term memory store that captures everything they do across every folder on every machine they own (files they edit, decisions they lock, discussions they have, notes they write). The store is **portable**: copy `~/.cyberos-memory/` between laptops, desktops, phones, and your memory comes with you. Personal memories do **two-way sync** with **Lumi's memory** (the cloud-hosted org-tenant memory, also called CUO's memory or CyberSkill's memory) — shareable memories push up to Lumi; team-shared memories pull down to you. Over time, Lumi aggregates patterns across team members, deduplicates, and **auto-evolves** a wisdom layer that exceeds any one contributor. CyberSkill (and any tenant) gets a compounding moat: the longer the org uses the platform, the smarter Lumi becomes.
...
| Name | Definition |
|---|---|
| **Personal memory** | A user's individual `.cyberos-memory/` store. One per human per machine, but portable; copying the folder is equivalent to moving the memory. The single source of truth for that user's local memory. |
| **Lumi** | The Genie persona that fronts CUO (Chief Universal Officer). One face, one voice, ten C-level skills hot-loaded on demand. Lumi is *the assistant*; Lumi's memory is *the substrate Lumi reads/writes*. |
| **Lumi's memory** | The cloud-hosted org-tenant memory. Also called **CUO's memory** or **CyberSkill's memory** depending on conversational context. Same store, three names for three audiences. |
| **Shared memory** | A memory record with `sync_class: shareable` (or `team-public` / `org-only`) that flows from Personal memory → Lumi's memory and is visible to other team members. |
| **Private memory** | A memory record with `sync_class: private` that stays in the Personal memory forever. Never reaches Lumi. |
| **Multi-memory power** | The capability that emerges once N personal memories feed one Lumi's memory — pattern recognition, dedup, synthesis, auto-evolved wisdom. |
| **The protocol** | The set of rules in [`AGENTS.md`](../memory/docs/AGENTS.md), [`memory.schema.json`](../memory/docs/memory.schema.json), and this document that together define how a memory behaves. Open standard. |

Going forward in this document: "memory" without qualifier means *the protocol*. "Personal memory" and "Lumi's memory" are the two concrete instances.
...
┌──────────────────────────────────────────────────────────────────────────┐
│                                                                          │
│            LAYER 3 · Lumi's memory (Cloud, multi-tenant)                  │
│            ┌─────────────────────────────────────────────┐               │
│            │  org_tenant/                                │               │
...
│              memories      │             │   memories                    │
│                            │             │                               │
│   LAYER 2 · Sync orchestrator  (per Personal memory, runs in background)  │
│            ┌─────────────────────────────────────────────┐               │
...
│                            │ tail audit chain                            │
│                            │                                             │
│   LAYER 1 · Personal memory  (one per human, portable, offline-first)     │
│            ┌─────────────────────────────────────────────┐               │
│            │  ~/.cyberos-memory/                         │               │
│            │    ├── manifest.json                        │               │
│            │    ├── HEAD                                 │               │
...
## §4 — Personal memory (universal — any folder, any human)

### §4.1 Core invariants

- **One per human.** Not one per project. The Personal memory is the user's *singular* long-term memory across everything they do on every machine they own.
- **Lives at a known XDG-style location.** Default: `$HOME/.cyberos-memory/` on macOS / Linux / WSL; `%APPDATA%\cyberos-memory\` on Windows. Override via `$CYBEROS_MEMORY_HOME`.
- **Offline-first.** Every read and write is local-disk-only. Network is optional and asynchronous (Layer 2 sync orchestrator).
- **Portable.** `cp -r ~/.cyberos-memory/ /Volumes/USB/` is a complete backup. Move the folder between machines; the memory moves with it. No DB-rebuild step.
- **Append-only.** Per AGENTS.md §3: all mutations are `put`/`move`/`delete`. No in-place edits, no migrations that mutate prior frames.
- **Audit-chained.** MMR + Ed25519 STH. Every memory is provably from the user's memory, in order.
- **Universal protocol — works on any folder.** You activate the memory once; it captures activity across all *watched folders*. Watched folders are a config list maintained inside `manifest.json`.

### §4.2 Activation

The Personal memory is activated once per human-machine:

```bash
# First time, on any new machine:
cyberos memory init                    # creates ~/.cyberos-memory/
cyberos memory watch ~/Projects        # add Projects/ as a watched folder
cyberos memory watch ~/Documents       # add Documents/
cyberos memory watch ~/Desktop         # add Desktop/

# Subsequent times — auto-discovers via manifest.json
cyberos memory status                  # see watched folders + last sync
```
...
| Slack / Zalo (via MCP) | messages tagged `@memory` or `@lumi` | `discussions` | `shareable` (default) |
...
| Email / Calendar (via MCP) | sent/received with `@memory` tag | `discussions` | folder default |
...
|---|---|---|
| `private` | Personal memory only — never pushed | Random browser scraps, half-finished notes, tool-output snippets |
| `personal` | Personal memory — syncs across user's own machines (laptop ↔ desktop ↔ phone) but NOT to Lumi | Personal notes, drafts, exploratory work |
| `shareable` | Eligible for push to Lumi's memory; subject to ACL filter at push time | Decisions, completed deliverables, meeting outcomes |
| `team-public` | Pushed to Lumi's memory AND visible to all org members via shared scope | Locked policy decisions, RFC approvals, OKR commits |
| `org-only` | Pushed to Lumi's memory but restricted by RBAC role | Compensation decisions, hiring decisions, financial state |
...
### §4.5 Portability — moving a Personal memory
...
| Lumi's memory 2-way sync (Layer 2) | continuous, online | minutes |

For users who don't yet have Lumi's memory deployed (org of one, offline-first, no cloud subscription), iCloud Drive sync is the recommended cross-device path. The protocol's sync-conflict detection (`layout-no-sync-conflict-siblings` invariant) catches iCloud's "filename (Mac)" / "filename (iPhone)" sibling creation and surfaces it to the user.
...
| **CLI flush** | `cyberos memory capture <path-or-text>` for one-off manual capture | On-demand |
...
acl:
  ...
  - subject: org:cyberskill
    rights: [read]
memory_chain_hash: <set by Writer>
memory_chain_prev: <set by Writer>
related: []
auto_capture: true
---
```
...
## §6 — Lumi's memory (Layer 3 — Cloud, shared)

### §6.1 What it is

A cloud-hosted multi-tenant CyberOS deployment whose primary product is *being the sync target for its tenant's team members' Personal memories*. The same memory protocol applies — same `Writer`, same `audit/binlog`, same MMR — but the store is shared and deployment-managed.

### §6.2 Per-tenant isolation

- One Lumi's memory per tenant (org).
...
- Read scoping: every cross-module call passes a `tenant_id` claim; Lumi's memory reads/writes are scoped to that tenant.

### §6.3 What Lumi sees

Lumi (the Genie / CUO) has *read access* to Lumi's memory as part of its `allowed_memory_scopes` (per AGENTS.md §3.6 / SKILL.md frontmatter):
...
Lumi's memory is *the killer feature* of the TEN module. The reviewer flagged: "ship a thin TEN-billing slice at P2 instead of P4" — Lumi's memory is the *product* that thin slice sells.

Existing TEN spec at `website/docs/modules/ten.html` covers per-tenant subdomain, Stripe/VietQR billing, branded shell. Add: Lumi's memory per tenant, provisioned at tenant create-time, scaled at tenant add-seat-time.
...
```
   ...
   └──────────────────┘                                                 │
              ...
              ▼                                                         │
   ┌──────────────────┐                                                 │
   │ pushed           │  POSTed to Lumi's memory; received OK            │
   └──────────────────┘                                                 │
              │                                                         │
              │ Lumi's memory replays + chains; returns confirm + hash  │
              ▼                                                         │
   ┌──────────────────┐                                                 │
   ...
   ┌──────────────────┐
   │ remote-write     │  another user's memory pushed a memory to Lumi   │
   └──────────────────┘
              │
...
1. **No "merge" at the file level** — the local memory never overwrites its own audit chain. A foreign record is a new local row that *references* the foreign one.
2. **Chain-position wins** — if two users both author memory ids that collide (e.g. both decided to call something `decisions/2026-05-14-foo.md`), the later-pushed wins by chain position on Lumi's memory. The losing record is preserved as a `superseded_by:` reference.
...
- **Offline tolerance:** Sync windows are best-effort. Personal memory works fully offline; the orchestrator queues until network returns.
...
| `manual` | `cyberos memory sync` | User-controlled |

Switch via `cyberos memory sync-mode normal`.

---

## §8 — Multi-memory power + auto-evolve (§3 Layer 3 enrichment)

### §8.1 The pattern

Once N personal memories feed one Lumi's memory, three new affordances become possible:
...
Runs nightly. Walks the Lumi's memory ledger for the prior 24 hours. Produces:
...
The synthesis output is itself a memory and follows the protocol. It's transparent — every claim Lumi's synthesis makes is hash-anchored to the underlying personal-memory-originated rows. No black-box AI summarisation.

### §8.3 Compounding moat

This is the strategic payoff of the entire design:

- A new user joining a Lumi's memory inherits *the org's accumulated wisdom* from day one.
- Lumi's quality of responses to any user improves as the org's collective memory grows.
- Switching cost climbs over time — your org's memory is the most valuable asset on the cloud bill.
...
```
                  ┌────────────────────────────────────────┐
                  │ Multi-memory auto-evolve (Lumi wisdom)  │  ← Stage 5
                  └─────────────────────┬──────────────────┘
                                        │
                  ┌─────────────────────┴──────────────────┐
                  │ 2-way sync (Personal ↔ Lumi's memory)   │  ← Stage 4
                  └─────────────────────┬──────────────────┘
                                        │
                  ┌─────────────────────┴──────────────────┐
                  │ Lumi's memory deployment                │  ← Stage 3
                  ...
                  └─────────────────────┬──────────────────┘
                                        │
                  ┌─────────────────────┴──────────────────┐
                  │ Personal memory — universal protocol    │  ← Stage 1
...
| **Stage 1** | Personal memory — universal | Extend memory module: `memory init`, `memory watch`, `memory status`, multi-folder watch in manifest.json | **Now** — no external dep |
...
| **Stage 3** | Lumi's memory deployment | TEN module + AUTH module + AI Gateway (per reviewer's reorder) | **P2 (M+9)** — gated on those three modules shipping |
| **Stage 4** | 2-way sync orchestrator | Stage 3 + sync state machine + JWT-based wire protocol | **P2 (M+9) — runs in parallel to Stage 3** |
| **Stage 5** | Multi-memory auto-evolve | Stage 4 + synthesis sub-skill | **P3 (M+12)** |
...
```bash
cyberos memory watch ~/Projects/cyberos --default-sync-class shareable
cyberos memory watch ~/Documents/Personal --default-sync-class private
```
...
cyberos memory reclass <memory-id> private

# Mark a private memory as shareable (will sync on next window):
cyberos memory reclass <memory-id> shareable
```

### §10.4 Right to forget

Per AGENTS.md §3.6 + §17.1, GDPR Art. 17 erasure is supported via `delete(path, "purge")`. For Lumi's memory, this propagates: a purge on Personal memory emits a `purge_propagate` request that, after AUTH-verifying the requester, redacts the corresponding record on Lumi's memory. The *fact* of erasure remains audit-chained on both memories (per §3.6 — purge of purge is forbidden).
...
### §11.1 P13 Stage 1 — universal personal memory

- **§4.6 (new)** — `MEMORY_HOME` resolution. Default `$HOME/.cyberos-memory/`. Override via `$CYBEROS_MEMORY_HOME`. Resolved once at process start; cached for the session.
- **manifest.json schema extension** — add `watched_folders: [{path, default_sync_class, since}]` array.
- **CLI additions** — `memory init`, `memory watch <path>`, `memory unwatch <path>`, `memory status`, `memory capture <path-or-text>`.
...
- **§14.3 (new)** — bi-directional sync protocol. Builds on §14.1 (interop) and §14.2 (cross-memory import). Adds: sync state machine, JWT wire format, `manifest.imports.lumi.last_pulled_seq` / `manifest.pushes.lumi.last_pushed_seq`.
...
Personal memory ships first (Stage 1 + 2) — useful to *anyone* with a laptop, including non-CyberOS users. Lumi's memory ships at P2 — useful to *any org*, with CyberOS's 22 modules as the value-add.
...
Personal memory as Stage 1 should be **OSS day-1**. Free CLI. Apache 2.0. No login required. This is the OSS distribution mechanism the strategy doc already calls out (Level 1 — Internal → Level 2 — OSS distribution).

Cloud memory (Stage 3+) is the commercial product — sold by tenant, by user-count, by storage-tier. Lumi's memory capacity scales the price.

### §12.3 Compounding moat

This is *the* answer to the reviewer's GTM #5 score. The marketplace is premature; the *memory* is the moat. A tenant six months into using Lumi's memory has accumulated context that no competitor can replicate. Switching cost = the value of the org's memory.
...
Recommendation: **adopt Personal memory (Stage 1+2) as a P0 extension of the memory module** rather than as a new module. It doesn't add to the 22-module count; it deepens module #1.
...
| Shared-cloud memory name in user-facing copy | **Lumi's memory** (warm, persona-attached) for end-user surfaces; **CUO's memory** for technical/architectural docs; **CyberSkill's memory** as the deployment-specific instance. |
| CLI brand | `cyberos memory ...` — keep the protocol generic; do not brand the CLI as "lumi" (the protocol stands alone from any one assistant). |
| Manifest header naming | `manifest.json` key `memory_owner: { name, email, lumi_tenant }` — explicit ownership for portability + sync auth. |
| Marketing name for the protocol | **The memory Protocol** (capitalised). Document as "memory" only when context is clear; "the memory Protocol" in marketing + spec contexts. |
...
| Week | Slice | Output |
|---|---|---|
| 1 | Personal-memory protocol extension | `cyberos memory init/watch/unwatch/status` CLI. Manifest schema + invariants. Tests. |
...
After week 4, every CyberSkill team member has automatic, offline-first, portable, audit-chained capture across their laptop and team conversations. Lumi's memory (Stages 3-5) lights up at P2.
...
### §15.1 CLI brand → `cyberos memory ...`

Single binary. All memory-protocol commands ship under `cyberos memory *`. No standalone `memory` binary. Reasons:
...
`cyberos memory init` creates the store but watches nothing. The user must run `cyberos memory watch <path>` to opt each folder in. Rationale: privacy floor is maximally conservative. The "remember everything" guarantee only applies to folders the user explicitly opted in. Footgun risk = zero on first install.
...
|---|---|---|
| Week 1 | Personal-memory protocol extension (`memory init/watch/unwatch/status/capture`) | CLI + invariants + tests |
...
`cyberos memory watch <path> --default-sync-class <private|personal|shareable|team-public|org-only>`. No interactive prompt. If `--default-sync-class` is omitted, the value is `private`. Per-memory override remains available via `cyberos memory reclass <memory-id> <class>`.
...
| No active Cowork session | Desktop notification (macOS UNUserNotificationCenter / Linux libnotify / Windows ToastNotification). Click-through opens the relevant `cyberos memory pending` view. |
| Non-urgent items | Accumulate in `cyberos memory pending`; user reviews on own cadence (e.g. end-of-day). |
...
- **Non-urgent** (new watched folder reaching sync window for first time, summary digest): always `cyberos memory pending`; no notification.
...
2. **Encryption-at-rest for Personal memory** — per AGENTS.md §5.4 the `Envelope` schema supports `cipher`. Should `cyberos memory init` default to encrypted-store? (Default-no for ergonomics; opt-in via `--encrypted`.)
3. **`memory status` UX** — what does the user see? Watched-folder list, last-sync time, pending-PII count, audit-row count, doctor invariants — full digest in one screen?
...
*End of design.* This is the lock for the memory auto-sync vision. Stages 1+2 are buildable now; Stages 3-5 ride the P0+P2 critical path.
