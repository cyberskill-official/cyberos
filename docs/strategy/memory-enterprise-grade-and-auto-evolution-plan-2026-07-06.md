...
- F4. No semantic fact layer. There is no extraction of durable facts, no Mem0-style ADD/UPDATE/DELETE/NOOP decision on write, no dedup or contradiction handling in the serving path; FR-MEMORY-116 covers the file store only.
...
- F10. No reranker, no MMR, no temporal ranking. FR-MEMORY-108's own title promises BGE reranking; `search.rs` and `recall.rs` ship RRF only. Score is pure rank fusion with no recency, importance, or diversity terms.
...
- F19. The real consent gate is unwired: DEC-2702's default-deny stub is what ships; FR-MEMORY-122's FR-EVAL-001-backed gate does not exist yet, and a stray use of the test-only `AllowAll` would silently capture everyone.
- F20. PII protection is regex-only, Python-only. `pre_ingest_pii.py` implements "Presidio-style" patterns with a swap-in note for the real analyzer; FR-MEMORY-111's target (Presidio EN + VN recognizers, >= 99.5% recall) is unmet, and the Rust cloud ingest path applies no PII pass at all.
...
- F25. Offline sync is scaffolding. `sync_supervisor.rs` spawns `cyberos.core.memory_sync_daemon`, which does not exist anywhere in the tree (the file header admits it); the desktop stays "online" with no syncing. FR-MEMORY-103's daemon (laptop A <-> cloud <-> laptop B, sync_class gating, 10k-file offline queue) is unbuilt.
...
- R2. Port the Park-et-al combined score into brain recall: final = 0.4 x normalized relevance + 0.3 x importance + 0.3 x recency, with recency as exponential decay on `last_accessed_at` (FR-MEMORY-113 already fixes the weights; `ranking.py` and `decay.py` are the reference implementations). Normalize each component to [0,1] before weighting and keep RRF as the candidate-stage fuser.
...
- R12. Add a cross-encoder reranker stage over the fused top 50: bge-reranker-v2-m3 served by the existing embed-sidecar (FR-MEMORY-108 already names BGE rerank). Budget one batched sidecar call per recall; skip when degraded.
...
- R29. Semantic dedup in the serving path: nightly job finds fact pairs with cosine > 0.95, merges via the R5 op pipeline (UPDATE/DELETE ops with provenance), mirroring `consolidate.py`/FR-MEMORY-116 semantics.
...
- R31. Write-time importance scoring (FR-MEMORY-114): rate each fact/episode 1-10 with a cheap model against an anchored rubric (examples pinned in the prompt so ratings do not drift high), stored in `importance` and used by R2 ranking and the reaper.
...
- R33. Port the dream loop to the BRAIN: run the four FR-MEMORY-115 detectors (duplicates, stale, patterns, verify) as nightly jobs over brain tables, emitting proposals to a review queue, with the applier gated exactly like the existing cap-4 dream loop (disabled by default, evidence required, every apply chained). The Python `dream/` package is the reference implementation.
...
- R73. (P0, critical) Replace header-trust with FR-AUTH-004 JWT verification middleware on `/v1/memory/*`: tenant and subject come from verified claims; `x-tenant-id` remains only as an internal hop header signed by the gateway if needed. Add a negative test proving a forged header cannot cross tenants (F15).
...
- R78. Ship real PII protection where content enters the cloud: a Presidio sidecar (analyzer + anonymizer) invoked by the Rust ingest path before any content-bearing body is chained, with the custom VN recognizers FR-MEMORY-111 specifies (CCCD/CMND ids, VN phone formats, addresses), held to the FR's >= 99.5% recall bar on a labeled set; store `pii_flags[]` per row to drive retention and masking (F20).
...
- R103. Wire the day-1 emitters (FR-MEMORY-122): chat messages, auth events, proj activity, obs incidents, each with `content_ref` pointers into their owning stores, and wire the real consent gate to the FR-EVAL-001 acknowledgment ledger at the same time (F19). Capture volume is the raw material for everything else in this plan.
...
- R105. Lumi reads memory only through the recall API with her own subject identity and access grants, never raw SQL; every evaluation cites `audit_row_id`s (FR-EVAL-003), and the citation check is enforced at the evaluation engine, not by convention.
...
Suggested FR numbering for net-new work: FR-MEMORY-125 auth+RLS hardening, 126 fact layer + op pipeline, 127 content-aware ingestion + PII sidecar, 128 hybrid ranking stack, 129 first-party sync, 130 retention + erasure + PDPL, 131 evolution loop + evals, 132 MCP/memory-tool surface. Existing FR-MEMORY-112..120 fold into these rather than shipping file-store-only.
