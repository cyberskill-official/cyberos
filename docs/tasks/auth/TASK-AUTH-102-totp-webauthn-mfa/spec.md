...
related_tasks: [TASK-AUTH-002, TASK-AUTH-004, TASK-AUTH-101, TASK-AI-003, TASK-MEMORY-101, TASK-AUTH-105, TASK-AUTH-106, TASK-OBS-007]
depends_on: [TASK-AUTH-002]
blocks: [TASK-AUTH-105, TASK-AUTH-106]
...
**Why 10 recovery codes + bcrypt cost 12 (DEC-485)?** 10 codes = enough for "I lost my phone" + multiple recovery events; bcrypt cost 12 matches TASK-AUTH-002's password hash + makes offline brute-force impractical even if codes leak.
...
**Why founder ALWAYS requires MFA (DEC-128 + DEC-491)?** Founder role grants cross-module privileged read — single most-sensitive role. MFA-always is the design assertion baked into TASK-AUTH-101's `requires_webauthn` flag.
...
- **TASK-AUTH-101** — RBAC; founder role MFA-always; root-admin role required for unlock.
- **TASK-AI-003** — memory audit bridge.
- **TASK-MEMORY-111** — PII scrubbing.
- **TASK-AI-005** — per-tenant policy YAML (mfa_required_roles).
- **TASK-OBS-007** — sev-1 alarms on lockout; sev-2 on recovery consumption + reuse.
...
- **10 recovery codes + bcrypt cost 12** — same parameter as TASK-AUTH-002 password.
