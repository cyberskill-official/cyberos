...
related_tasks: [TASK-AUTH-002, TASK-AUTH-005, TASK-AUTH-102, TASK-AUTH-101, TASK-MEMORY-111]
depends_on: [TASK-AUTH-002, TASK-AUTH-102]
blocks: []
...
source_decisions:
  ...
  - DEC-743 2026-05-16 — On detection, require fresh MFA (TASK-AUTH-102 challenge) — do NOT block login outright
...
11. **MUST** persist every login (challenged + allowed + blocked) to the `login_history_geo` append-only table per TASK-AUTH-103's existing login_history pattern, extended with `(latitude, longitude, country_iso2, asn, is_anonymous_proxy, travel_decision, speed_kmh_vs_prior)`. The table is REVOKE UPDATE, DELETE FROM cyberos_app + privileged auth_writer role.
...
15. **MUST** route the adaptive challenge through TASK-AUTH-102 (the existing MFA challenge issuer). The travel-evaluator creates a challenge with `kind = "impossible_travel"`, returns the challenge_id to the client. The client submits MFA proof via `POST /v1/auth/mfa/challenge/{id}/verify` per TASK-AUTH-102. On verify-success, the AUTH session JWT is finally issued and a `login_history_geo` row is recorded with `travel_decision = 'allowed_after_challenge'`.
...
17. **MUST** restrict policy mutation (`impossible_travel_speed_kmh`, `impossible_travel_action`, `block_anonymous_ip`, `impossible_travel_ip_allowlist_cidrs`) to the `security_admin` role per TASK-AUTH-101 + a sev-2 memory audit row with reason ≥10 chars.
...
**Depends on**: TASK-AUTH-002 (subject_create — login flow), TASK-AUTH-102 (MFA challenge — adaptive challenge issuer).

**Blocks**: nothing (terminal leaf in P2 auth hardening; complements TASK-AUTH-107).

**Blast radius if broken**:
- **False positive**: legitimate user challenged unnecessarily; bounded by allowlist + ASN bypass.
- **False negative**: impossible-travel signal missed; defense-in-depth via TASK-AUTH-102 MFA still active.
...
| 10 | MFA challenge issuance fails (TASK-AUTH-102 down) | upstream error | 1 | Login rejected with 503 + sev-1 |
...
| 31 | Subject deleted between cache write and read | foreign key | 3 | Bounded by TASK-AUTH-105 deletion ordering |
| 32 | Tenant terminated (TASK-TEN-104) — old login geos still query | RLS + tenant state | 3 | Read-only after termination; allowed |
...
**§11.6** The challenge_id returned in the `challenge_required` response is the same challenge type as TASK-AUTH-102 user-initiated MFA — the verify endpoint is shared.
...
**§11.17** Tracing spans wrap each `travel::evaluate` call with the subject_id + decision. The span shows up in TASK-OBS-001 OTEL traces, enabling forensic walks across login + travel + MFA spans.
...
**§11.22** Per-tenant policy cache (60s TTL) is invalidated via pg_notify on policy mutation, mirroring the TASK-AUTH-107 pattern.

---

*End of TASK-AUTH-106 spec.*
