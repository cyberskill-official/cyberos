...
related_tasks: [TASK-AUTH-004, TASK-AUTH-101, TASK-AUTH-108, TASK-AI-003, TASK-MEMORY-101, TASK-OBS-007]
...
source_decisions:
  - DEC-440 (grace window 30 days from TASK-AUTH-101 ship date; configurable per tenant via tenant policy override; minimum 7 days; maximum 90 days)
  - DEC-441 (stub-era tokens are TASK-AUTH-004 tokens issued before TASK-AUTH-101 ship — they lack the `rbac_v` claim entirely; presence of `rbac_v` claim = post-TASK-AUTH-101 token)
  ...
  - DEC-449 (refresh trigger — when TASK-AUTH-004 refresh sees a token without `rbac_v`, the refreshed token MUST include `rbac_v` at current value; the verifier records the refresh in `auth_token_refresh_log`)
...
risk_if_skipped: "Without the migration enforcer, TASK-AUTH-101's `rbac_v` claim is advisory — stub-era tokens continue valid indefinitely, defeating the purpose of replay-resistance. Operators lose visibility into 'how many users haven't refreshed yet'; OBS has no alarm signal when stub-token rejection starts climbing. Without DEC-448's clear rejection reason, clients see opaque 401s + don't know to refresh. Without DEC-449's refresh hook, refreshed tokens still lack rbac_v + the grace never actually ends. Without DEC-445's extension override, an operator with a stuck client base has no escape valve. The 5h effort closes the loop: TASK-AUTH-101 ships the claim, TASK-AUTH-109 enforces it after grace."
---

## §1 — Description (BCP-14 normative)

The AUTH service **MUST** enforce the TASK-AUTH-101 stub → full migration via the 30-day grace window + rejection enforcer + per-tenant extension override. Each requirement:
...
3. **MUST** enforce RLS with both `USING` and `WITH CHECK` on `auth_migration_state` (root-admin only — same pattern as TASK-TEN-001's tenants table; consumers read via internal helper) and `auth_token_refresh_log` (tenant-scoped).
...
**Why per-tenant cutover_at not global (DEC-442)?** Different tenants ship at different dates. Existing tenants get cutover_at = ship_date + 30 days at TASK-AUTH-101 ship; new tenants get cutover_at = provision_date + 30 days. Global cutover would force later-provisioned tenants into immediate cutover — operationally cruel.
...
**Why refresh hook on TASK-AUTH-004 path (DEC-449)?** Refresh is the natural cutover point — old token returns access token + refresh token; new access token gets rbac_v injected. No client code change needed; refresh is opaque from client perspective.
...
**Why `auth_provisioner` SQL role (§1 #17)?** Same defense-in-depth pattern as TASK-TEN-001. App code can't accidentally mutate migration state; only the explicit migration handler (with provisioner role) can.
...
1. **Migration state seeded on TASK-AUTH-101 ship** — every existing tenant gets cutover_at = now+30d.
2. **New tenants seeded on provisioning** — cutover_at = provision_time + 30d via TASK-TEN-001 hook.
...
- **TASK-TEN-001** — provisioning hook that seeds per-tenant cutover_at.
...
| Sev-2 alarm threshold drift | OBS rule version | None | Coordinate with TASK-OBS-007 |
...
| Refresh log table size growth | partition (deferred) | None at slice 1 | TASK-OBS-006 retention rule |
...
- **Refresh hook on TASK-AUTH-004 path** — transparent client-side cutover.
