...
related_tasks: [TASK-AUTH-004, TASK-AUTH-101, TASK-AI-014, TASK-AI-016, TASK-AI-003, TASK-MEMORY-101, TASK-CUO-101, TASK-AUTH-109]
...
source_decisions:
  - DEC-420 (Lumi JWT shape is additive on TASK-AUTH-004 + TASK-AUTH-101 — adds `agent_persona`, `tenant_residency`, `lumi_org_tenant`, `persona_version`, `sync_class_allowed` claims; existing claims preserved unchanged)
  - "DEC-421 (`agent_persona` claim format: `cuo-<persona-key>@<semver>` matching TASK-AI-014 stamping; persona-key ∈ TASK-AUTH-101 closed enum {agent-persona, founder, cfo, cto, coo, chro, cmo, cpo, cso, cseco, clo, cdo, dpo, caio})"
  - DEC-422 (`tenant_residency` claim is the per-tenant residency code from TASK-AI-016 closed enum: vn-1 | sg-1 | eu-1 | us-1; mismatch with route's residency → 451 unavailable for legal reasons)
  ...
  - DEC-424 (`persona_version` claim is a semver string matching TASK-AI-014's pinned version; verifiers may reject tokens with persona_version older than 2 minor versions behind current)
...
3. **MUST** enforce that `agent_persona` claim's persona-key ∈ TASK-AUTH-101 agent-persona-role-family enum (per DEC-421). Verifier parses prefix `cuo-` + key + `@` + semver; invalid format → 401 `agent_persona_malformed`; unknown persona-key → 401 `agent_persona_unknown`.

4. **MUST** verify `persona_version` against the live version registry from TASK-AI-014 (per DEC-424 + DEC-429). Verifier compares token's `persona_version` against `cuo-<persona-key>@<live-semver>` from the registry; if token's version is more than 2 minor versions behind → 401 `persona_version_stale` + emit `auth.lumi_persona_version_stale` memory row (sev-2 — signals refresh-token reuse beyond rotation window).
...
22. **MUST** validate that `lumi_org_tenant` slug matches the pattern `^[a-z][a-z0-9-]{2,40}[a-z0-9]$` per TASK-TEN-001's slug regex. Malformed → 401 `lumi_org_tenant_malformed`.
...
- **TASK-AI-014** — persona-version registry (consumed by staleness check).
- **TASK-AI-016** — residency policy.
- **TASK-AI-005** — per-tenant policy YAML (consumed for sync_class policy gate).
- **TASK-AI-003** — memory audit bridge.
- **TASK-MEMORY-111** — PII scrubbing of lumi_org_tenant.
...
| Refresh path missing | TASK-AUTH-004 dependency | Existing path | Designed |
...
- **Lumi JWT additive on TASK-AUTH-004** — 5 new claims, 8 existing preserved; 13 total.
...
- **Same signing key as TASK-AUTH-004 at slice 1** — operational simplicity; distinct iss provides boundary.
