...
related_tasks: [TASK-AUTH-002, TASK-AUTH-003, TASK-AUTH-004, TASK-AUTH-005, TASK-AUTH-006, TASK-AUTH-108, TASK-AUTH-109, TASK-CRM-001, TASK-HR-001, TASK-KB-001, TASK-REW-001, TASK-DOC-001, TASK-OKR-001]
depends_on: [TASK-AUTH-005]
blocks: [TASK-AUTH-108, TASK-AUTH-109, TASK-CRM-001, TASK-DOC-001, TASK-HR-001, TASK-KB-001, TASK-OKR-001, TASK-REW-001, TASK-TIME-001, TASK-INV-005, TASK-AUTH-104, TASK-INV-003, TASK-INV-004]
...
subtasks:
  ...
  - "0.5h: migrate.rs — stub→full helper used by TASK-AUTH-109 + grace-window validator"
...
13. **MUST** reject any JWT whose `roles` claim contains a string that does not parse to a `Role` variant. The verifier (used by every consuming service per TASK-AUTH-004 §1 #7) treats this as a tampered token: `401 UNAUTHORIZED {"error":"invalid_token","reason":"unknown_role_in_claim"}`. The check is at the verifier, not downstream — failing closed.
...
INSERT INTO roles (name, display, reserved, requires_webauthn, scope_summary, lands_in_slice) VALUES
  ...
  ('cpo',                'CPO',                FALSE, FALSE, 'Product brief + roadmap + task-author canonical',         4),
...
-- v1 was the implicit pre-TASK-AUTH-101 catalogue (5 roles); v2 ships here.
...
BEGIN
    ...
    -- memory audit row emission via memory_writer bridge (TASK-AI-003) is handled at the migration commit hook.
    RETURN NULL;
END;
$$ LANGUAGE plpgsql;
...
pub async fn assign_role(
    ...
    claims: Claims,                              // injected by TASK-AUTH-004 middleware
...
18. **JWT carries roles + rbac_v** — newly issued token (after TASK-AUTH-101 ships) has both claims populated; older tokens (pre-TASK-AUTH-101) lack `rbac_v` and are accepted during grace window.
...
21. **Stub compatibility** — every (stub_role, resource, action) tuple from the pre-TASK-AUTH-101 matrix is present in the new matrix; no removed tuples (additive only).
...
async fn stub_role_permissions_strictly_additive() {
    ...
    for (role, res, act) in &pre_matrix.allowed {
        assert!(post_matrix.allowed.contains(&(*role, *res, *act)),
            "TASK-AUTH-101 removed pre-existing tuple ({role:?}, {res:?}, {act:?})");
    }
}
```
...
### 8.5 — JWT claims (post-TASK-AUTH-101)
...
| POST founder for subject without passkey | Handler `webauthn::has_factor` check | 409 `webauthn_required` | Enrol passkey first (TASK-AUTH-105) |
...
| Bcrypt path changed (TASK-AUTH-114 lands) and role gate breaks for founder | Test `rbac_founder_webauthn_gate_test` after bcrypt swap | CI fails | Coordinate TASK-AUTH-114 + TASK-AUTH-101 changes |
