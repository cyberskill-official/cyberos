...
related_tasks: [TASK-AUTH-002, TASK-AUTH-003, TASK-AUTH-004, TASK-AUTH-005, TASK-AUTH-006]
depends_on: []
blocks: [TASK-AUTH-002, TASK-AUTH-003, TASK-AUTH-005, TASK-AUTH-006, TASK-PROJ-001, TASK-TEN-001]
...
-- Tenant 0 (root) inserted by TASK-AUTH-006 bootstrap CLI; not by this endpoint.
```
...
-- Sweeper job (TASK-AUTH-006 owns) deletes rows older than 24h.
```
...
- memory module reachable via outbox table (TASK-AUTH-006 sets up the memory_writer subprocess).
...
| Idempotency table grows unbounded | sweeper job missing | Storage growth | TASK-AUTH-006 cron sweeps rows > 24h |
...
- Bootstrap of tenant 0 happens via TASK-AUTH-006 CLI, not via this endpoint (chicken-and-egg: tenant 0 needs to exist before root-admin in tenant 0 can call this endpoint).
...
*End of TASK-AUTH-001. Status: draft (10/10 target).*
