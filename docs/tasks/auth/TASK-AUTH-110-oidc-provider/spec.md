...
related_tasks: [TASK-AUTH-004, TASK-AUTH-005, TASK-AUTH-101, TASK-AUTH-104, TASK-MCP-004, TASK-PORTAL-003, TASK-PORTAL-004]
depends_on: [TASK-AUTH-004, TASK-AUTH-104, TASK-AUTH-005, TASK-AUTH-101]
blocks: [TASK-PORTAL-003]

source_pages:
  - website/docs/modules/auth.html#provider
source_decisions:
  - DEC-2480 (AUTH is the central first-party identity provider; CHAT/Mattermost, PORTAL, and other first-party apps federate to AUTH via OIDC Authorization Code + PKCE - the inverse of TASK-AUTH-104, where AUTH is the client of Google)
  - DEC-2481 (reuse TASK-AUTH-004 `auth_signing_keys` + `/.well-known/jwks.json` for id_token + access_token signing; the provider mints NO second key system - one JWKS for the whole platform)
  - DEC-2482 (the authorize endpoint authenticates the human via an AUTH SSO session cookie when present (silent SSO); when absent it brokers to the tenant's primary upstream IdP via TASK-AUTH-104 (Google), and on return resumes the authorize and issues the downstream code - so Google stays the ultimate authenticator and AUTH is the broker)
  ...
  - DEC-2484 (Authorization Code flow + PKCE S256 mandatory; implicit + hybrid flow forbidden, mirroring TASK-AUTH-104 DEC-401)
  ...
  - DEC-2486 (id_token carries: sub = CyberOS subject_id, email, name/preferred_username, tenant_id, roles per TASK-AUTH-101; aud = the RP client_id; iss = the AUTH issuer; signed RS256 against auth_signing_keys)
  - DEC-2487 (userinfo endpoint returns the same identity claims for the access_token bearer; access_token is the TASK-AUTH-004 RS256 JWT scoped `openid`)
  - DEC-2488 (authorize is revoke-gated: a subject revoked via TASK-AUTH-005 (deny-list) CANNOT obtain a new code - the kick blocks re-authentication into every first-party app; existing downstream sessions are killed by the slice-2 back-channel-logout follow-up, NOT this slice)
  ...
  - DEC-2497 (only tenant-admin per TASK-AUTH-101 may register/modify RP clients; client_secret never returned after creation, only a one-time reveal at create)
  ...
  - DEC-2499 (first-party RPs MAY receive a refresh_token (offline_access) reusing TASK-AUTH-004 rotation; for CHAT/Mattermost it is optional since Mattermost mints its own session - default OFF per RP, opt-in flag on the client record)
...
10. **MUST** build the id_token (DEC-2486): claims `iss` (= configured issuer, DEC-2498), `sub` (= CyberOS `subject_id`), `aud` (= RP `client_id`), `exp` (+1h), `iat`, `auth_time`, `nonce` (echoed if supplied), `email`, `email_verified`, `name`, `preferred_username`, `tenant_id`, `roles` (TASK-AUTH-101 array). Signed RS256 against `auth_signing_keys` with the current `kid` (DEC-2481). NO second key system.
...
15. **MUST** restrict RP registration to tenant-admin (DEC-2497): `POST /v1/auth/op/rp-clients` (create), `PATCH .../{id}`, `DELETE .../{id}` (soft-delete `is_active=false`) require role `tenant-admin` per TASK-AUTH-101. The plaintext `client_secret` is returned exactly once at create; never again. Emit `auth.op_rp_client_changed` (excluding the secret value).
...
18. **MUST** PII-scrub `email`, `subject_id`, `reason` via the memory PII layer before chain commit; audit rows carry hash16 forms (mirrors TASK-AUTH-104 §1 #15).
...
25. **MUST** make refresh tokens opt-in per RP (DEC-2499): `allow_refresh=false` by default; CHAT/Mattermost does not need it (Mattermost mints its own session post-login). When true, `offline_access` scope grants a refresh_token rotated per TASK-AUTH-004.

26. **MUST** revoke SSO sessions on subject revoke: when TASK-AUTH-005 revokes a subject, all that subject's `auth_sso_sessions` rows are marked `revoked_at` (so silent SSO stops too, not only new logins). Emit `auth.op_sso_session_revoked`. (This is the slice-1 reach of the kick into the broker; the reach into already-issued downstream app sessions is slice 2.)
...
Why reuse the TASK-AUTH-004 keys and JWKS (DEC-2481). The platform already signs RS256 and publishes one JWKS. A second key system for the provider would fragment verification and drift on rotation. One issuer, one key set, one place to rotate.

Why broker through the existing session or Google, not a new password prompt (DEC-2482). The human already proves who they are to CyberOS via Google (TASK-AUTH-104). The provider should not re-ask. If the user has an AUTH SSO session, authorize is silent. If not, it sends them through the same Google flow that already works, then issues the downstream code. Google stays the root authenticator; AUTH is the broker that turns that into a CyberOS-shaped identity every app understands.
...
Why PKCE mandatory and implicit forbidden (DEC-2484). Same reasoning as TASK-AUTH-104: authorization-code interception is the classic attack, PKCE binds the code to the initiator, and implicit/hybrid leak tokens through the URL. OAuth 2.1 deprecates them. We only ship the safe profile.
...
/// (DEC-2498); `jwks_uri` is the existing TASK-AUTH-004 /.well-known/jwks.json (DEC-2481).
...
/// Signs with the current TASK-AUTH-004 signing key (RS256). The `kid` header MUST match a key
...
//      else -> stash this authorize request, 302 into TASK-AUTH-104 initiate (upstream broker)
//              on TASK-AUTH-104 callback: create sso session (DEC-2489), resume here        (upstream_broker)
...
1. **Discovery served** - `/.well-known/openid-configuration` returns the pinned OIDC profile; `issuer` equals the configured value; `jwks_uri` is the existing TASK-AUTH-004 JWKS.
...
7. **Upstream broker** - with no cookie, authorize drives TASK-AUTH-104 Google login, creates an SSO session, then issues the code.
...
async fn revoked_subject_cannot_get_code(ctx: TestCtx) {
    ...
    ctx.revoke_subject(sub.id).await;                       // TASK-AUTH-005
...
§3 is the skeleton. The provider reuses the TASK-MCP-004 `oauth` substrate (pkce, jwt-minting against `auth_signing_keys`, code/token store shape, discovery shape, error enum) per DEC-2493; the net-new is the OIDC layer (id_token, userinfo, openid-configuration), the authorize human-auth brokering into TASK-AUTH-104, and the SSO cookie/session. The 7 memory row builders follow the canonical pattern.
...
Upstream:
- TASK-AUTH-004 - signing keys + JWKS (id_token + access_token signed against these; one key system).
- TASK-AUTH-104 - OIDC client (the upstream Google login that authorize brokers to when there is no SSO session).
- TASK-AUTH-005 - revoke / deny-list (the kick the authorize/token/userinfo gates enforce).
- TASK-AUTH-101 - role enum (roles claim in the id_token).
...
Cross-module:
- TASK-AUTH-002 - subject lookup for claims.
- TASK-AI-003 / memory - audit bridge.
- TASK-OBS-007 - alarm on a denied-revoked spike (expected post-layoff; sustained from one IP may be probing).
...
- **Refresh-token rotation for RPs** - slice 2 (the TASK-AUTH-004 rotation already exists; this wires it per RP).
- **Third-party consent UI + RFC 7591 Dynamic Client Registration** - slice 3 (first-party RPs are admin-registered).
- **SCIM auto-deprovision** - TASK-PORTAL-004 (instant cross-app removal beyond the revoke gate).
...
| No SSO session, no cookie | cookie absent | broker to TASK-AUTH-104 | Designed |
...
| JWKS/signing key missing | TASK-AUTH-004 | 500 + sev-1 | Key health |
...
- One issuer, one JWKS - reuse TASK-AUTH-004; never a second key system.
- authorize brokers: silent via the SSO cookie, else through TASK-AUTH-104 Google, then resume.
...
- Reuse the TASK-MCP-004 oauth substrate; spend the budget on the OIDC layer + brokering + SSO cookie.
