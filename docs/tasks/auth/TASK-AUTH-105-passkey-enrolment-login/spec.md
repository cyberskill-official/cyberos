...
related_tasks: [TASK-AUTH-002, TASK-AUTH-004, TASK-AUTH-101, TASK-AUTH-102, TASK-AI-003, TASK-MEMORY-101, TASK-OBS-007]
depends_on: [TASK-AUTH-102]
blocks: [TASK-DOC-006]
...
source_decisions:
  ...
  - DEC-548 (cross-platform key recovery — if subject loses ALL synced passkeys + cross-platform keys, TASK-AUTH-102 recovery codes are the only recovery path; passkey-only subjects without recovery codes risk lockout; UI MUST warn during enrolment)
  - "DEC-549 (per-tenant `passkey_required_for_roles: [<role>...]` policy in tenant_policy YAML — when subject's roles intersect, password login is blocked; founder always per TASK-AUTH-101 DEC-128)"
...
**Why same WebAuthn factor table from TASK-AUTH-102 (§1 #7, modified_files)?** Passkeys ARE WebAuthn credentials. Storing in the same `mfa_factors` table preserves the unified factor view (subject's full credential inventory in one place). The passkey-specific lifecycle log is supplementary.
...
**Why downgrade_gate cached inline at TASK-AUTH-004 issuer (§1 #20)?** Hot path — JWT issuance happens on every login. Cache (60s TTL matching TASK-AUTH-109 pattern) keeps it fast. Invalidate on passkey enrol/remove + opt-out change.
...
**Why no SLO / passkey-side logout (§1 #14)?** Passkey is for authentication; session lifecycle is TASK-AUTH-004's concern. Logout = expire token; no passkey-specific action needed.
...
-- Extend mfa_factors (from TASK-AUTH-102) with passkey-specific columns
...
pub async fn login_finish(
    ...
    // (3) Verify signature via TASK-AUTH-102 webauthn module + counter-monotonicity
    ...
    match result {
        ...
        Err(crate::mfa::webauthn::VerifyError::ClonedAuthenticator) => {
            // Per TASK-AUTH-102 §1 #10 — revoke factor + sev-1 audit
...
10. **Login finish with cloned authenticator** → factor revoked + sev-1 audit per TASK-AUTH-102.
...
(API contract above is the skeleton; 8 memory row builders follow canonical pattern; webauthn-rs handles cryptographic plumbing via TASK-AUTH-102.)
...
**Upstream:**
- **TASK-AUTH-102** — TOTP + WebAuthn MFA (passkeys ARE WebAuthn discoverable credentials; reuses factor table + verifier).

**Downstream:** none at slice 1.

**Cross-module:**
- **TASK-AUTH-002** — subject (PRIMARY KEY for factor + enrolment rows).
- **TASK-AUTH-004** — JWT issuer (downgrade-gate hook).
- **TASK-AUTH-101** — RBAC (founder = passkey-required; root-admin for opt-out + force-remove).
- **TASK-AI-003** — memory audit bridge.
- **TASK-MEMORY-111** — PII scrubbing (display_name).
- **TASK-AI-005** — per-tenant policy YAML (passkey_required_for_roles).
- **TASK-OBS-007** — sev-2 alarm rules.
...
| Counter monotonicity violation (cloned key) | TASK-AUTH-102 verifier | Factor revoked + sev-1 audit | Re-enrol |
...
| Counter alarms not firing | OBS rule | CI fails | Coordinate with TASK-OBS-007 |

---

## §11 — Implementation notes

- **Passkey = WebAuthn discoverable credential** — same factor table as TASK-AUTH-102.
...
- **Cache 60s on downgrade gate** — matches TASK-AUTH-109 pattern.
