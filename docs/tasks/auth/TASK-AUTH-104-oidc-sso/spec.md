...
related_tasks: [TASK-AUTH-002, TASK-AUTH-004, TASK-AUTH-101, TASK-AI-003, TASK-MEMORY-101, TASK-AUTH-103, TASK-TEN-101, TASK-PORTAL-003]
depends_on: [TASK-AUTH-004, TASK-AUTH-101]
blocks: [TASK-TEN-101, TASK-PORTAL-003]
...
source_decisions:
  ...
  - DEC-403 (JIT (Just-In-Time) subject provisioning — first OIDC login auto-creates AUTH subject via TASK-AUTH-002; claim → role mapping per per-tenant config)
...
risk_if_skipped: "Without OIDC SSO, every tenant employee provisions an AUTH subject manually (password reset friction + onboarding delay). TASK-TEN-101's self-serve signup needs OIDC for federated identity at signup. TASK-PORTAL-003's external IdP integration with JIT user provisioning is the canonical enterprise-tenant entry pattern. Without DEC-401's PKCE requirement, the authorization-code interception attack is open; without DEC-402's JWKS rotation, IdP key rotation breaks login until manual rebuild; without DEC-408's state + nonce verification, CSRF + replay attacks succeed; without DEC-411's id_token signature verification, attacker-crafted tokens grant access. The 10h effort lands the standards-compliant SSO with all the security guarantees the spec mandates."
---
...
**Upstream:**
- **TASK-AUTH-004** — JWT issuance + JWKS (used after OIDC callback for our own JWT).
- **TASK-AUTH-101** — RBAC role enum (claim mapper validates against).

**Downstream (2 placeholders):**
- **TASK-TEN-101** — self-serve signup form (consumes OIDC for federated identity).
- **TASK-PORTAL-003** — external IdP integration with JIT user provisioning.

**Cross-module:**
- **TASK-AUTH-002** — subject create (JIT provisioning calls internal helper).
- **TASK-AI-003** — memory audit bridge.
- **TASK-MEMORY-111** — PII scrubbing for sub_claim + email.
- **TASK-OBS-007** — sev-2 alarm on signature failures > 5/h.
...
- **Refresh token rotation** — handled by TASK-AUTH-004 separately.
- **SCIM provisioning** — TASK-PORTAL-004.
- **SAML 2.0 SSO** — TASK-AUTH-103 (sibling).
...
- **claim_mapping_yaml validates roles at save** — closed TASK-AUTH-101 enum.
