...
related_tasks: [TASK-DOC-001, TASK-DOC-006, TASK-DOC-002, TASK-DOC-003, TASK-DOC-004, TASK-EMAIL-009, TASK-AI-003, TASK-MEMORY-111]
depends_on: [TASK-DOC-001, TASK-DOC-006]
blocks: []
...
source_decisions:
  ...
  - DEC-1754 2026-05-17 — Per-signer TASK-DOC-006 verification BEFORE signature applied; signature_status='signed' only after both verify+sign succeed
...
8. **MUST** define tables at migration `0006`:
   ...
   CREATE TABLE doc_signers (
     ...
     verification_id UUID,  -- FK to TASK-DOC-006
     ca_routing TEXT,
...
10. **MUST** emit 6 memory audit kinds per DEC-1756. PII per TASK-MEMORY-111: signer_id (uuid) ok; signature_payload hashed.

11. **MUST** thread trace_id through workflow → invite → verify → sign → audit.

12. **MUST NOT** apply signature without successful TASK-DOC-006 verification per DEC-1754.
