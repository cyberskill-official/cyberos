...
The spec lands the branded Genie chat for client-tenant portal on top of TASK-PORTAL-003 (IdP-auth) + TASK-CUO-101 (LangGraph supervisor). Final form: 1,090 lines, 23 §1 normative clauses (2 migrations, scope_grants JWT claim extension, query handler with SSE streaming, pre-flight + post-flight cross-tenant boundary checks, TASK-PORTAL-002 brand inheritance, TASK-PORTAL-004 SCIM cascade target, per-Engagement persona override, conversation history with 90-day retention, 5 memory audit kinds), 20 acceptance criteria, 10 verification tests, 22 failure-mode rows, 20 implementation notes.
