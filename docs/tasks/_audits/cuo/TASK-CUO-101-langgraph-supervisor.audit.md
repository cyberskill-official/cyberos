...
First-pass would auto-invoke any skill at confidence ≥ 0.70. Resolved: §1 #7 + DEC-170 — `destructive: true` annotation in skill catalog triggers capability broker TASK-SKILL-104 Elicitation flow; supervisor refuses to override regardless of confidence; AC #12.
...
First-pass omitted the field at slice 2; TASK-CUO-104 would have to handle both shapes. Resolved: §1 #25 + #26 + AC #18 — field is present-but-null at slice 2; one shape forever.
...
First-pass stored raw query. Resolved: §1 #24 + `apply_memory_pii_rules` per TASK-MEMORY-111; AC #19. Raw query retained transient in OTel span.
