...
### ISS-005 — task-audit skill §3.4 rule 11 (money as BIGINT minor) — spec uses `rust_decimal::Decimal` not BIGINT minor
...
- §6 skeleton's metrics module preamble (added per TASK-AI-006 ISS-003 pattern) is the right level of detail.
...
- ISS-005 RESOLVED (2026-05-16, task-audit skill compliance pass): §11 note added explaining Decimal-vs-BIGINT boundary; rule 11 applies at TASK-AI-001 storage tier, satisfied there; §2 cross-link added.
- ISS-006 RESOLVED (2026-05-16, task-audit skill compliance pass): `failures.sort_by(...)` added in §6; §4 AC #16 added asserting byte-identical Vec<FileFailure> across runs.

**Score = 10/10.** Ship as-is. Ready to transition `draft → accepted`.

---

*End of TASK-AI-007 audit (final). Status: PASS at 10/10.*
