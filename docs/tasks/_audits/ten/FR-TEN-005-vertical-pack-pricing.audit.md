...
The spec lands per-pack add-on pricing for the vertical-pack marketplace on top of TASK-TEN-002 plan tiers + TASK-SKILL-107 pack registry. 690 lines, 20 §1 normative clauses, 20 ACs, 4 verification tests, 16 failure-mode rows, 10 implementation notes. 3 migrations, 6 endpoints, 5 memory audit kinds.
...
§11.9 says API; better is direct DB read by TASK-SKILL-107 since both on same DB. Resolved: §11.9 — TASK-SKILL-107 reads `vertical_pack_installs.status` directly via RLS-shared connection.
