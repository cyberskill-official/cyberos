...
First draft permitted free-string markers. Risk: typo proliferation (`untrusted_cotent`, `untrustedContent`). **Resolved:** §1 #2 + §3 Rust enum `MarkerName` + JSONSchema `enum: ["untrusted_content"]` freeze the namespace at v1; new markers require explicit task + variant addition.
...
- `depends_on: [TASK-SKILL-103]` — present + verified (accepted per `TASK-SKILL-103-frontmatter-extension.audit.md`). Reciprocity update for 103's `blocks:` queued for the same housekeeping commit that includes TASK-SKILL-111 + TASK-SKILL-112.
- `related_tasks:` enumerates TASK-SKILL-103 (parent), TASK-SKILL-111 (overlapping bracket-check), TASK-SKILL-112 (independent), TASK-SKILL-114 (orthogonal — promotion artefact). All four exist in the catalog.
...
- **Body XML preserved.** Spot-checked `task-author/SKILL.md` body: `<untrusted_content` appears 2 times in markdown prose (CONTRACT_ECHO example + §3 PLAN phase reference) — both untouched. SK-040 catch-all enforces frontmatter only.
- **134 pre-existing placeholder leaks discovered** (separate from this task's scope). Stale `<SDP §2 stage letter>` etc. in `metadata.stage` field across ~134 production SKILL.md files. Not regressions from this task; the migrate.sh regex was specifically anchored to `wrap_in: <untrusted_content/>` and did NOT touch other XML-bracket fields. Operator note: future TASK-SKILL-115 sweep should target `metadata.stage` + other placeholder fields.
- **RUBRIC location.** Same as TASK-SKILL-111/112: FM-115/FM-116 landed as SKB-040..042 in [SKILL_BUNDLE_RUBRIC.md](https://cyberos-wiki.cyberskill.world/modules/skill/appendices.html) (Appendix L).
