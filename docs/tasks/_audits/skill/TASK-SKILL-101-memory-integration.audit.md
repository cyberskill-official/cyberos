...
TASK-SKILL-101 expanded from 61 lines to ~620. Added 6 §1 clauses (#7 catch_unwind for panic; #8 concurrent independence; #9 tenant_id in both rows; #10 duration even on panic; #11 metrics; expanded #5 with start-vs-completed failure semantics). 7 §2 rationale paragraphs. Full Rust types + builders + invocation_context in §3. 15 ACs. 8 full Rust test bodies. 14 failure modes. 9 implementation notes.
