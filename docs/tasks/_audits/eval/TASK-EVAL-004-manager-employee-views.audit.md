...
Inventing a panel-specific access rule would create two sources of truth. Resolved: §1 #3 composes the TASK-EVAL-001 view grant with AUTH's manager-of; AC #5; §11 compose-not-invent note; DEC-2624.
...
Approve/override/response must be reconstructable. Resolved: §1 #10 `eval.assessment_{approved,overridden,response_recorded}` chained into l1_audit_log (TASK-PROJ-008 / TASK-MEMORY-123 pattern); AC #17; §8 example payloads.

### ISS-008 — Hidden backend creep
A surfacing panel could quietly grow server logic in the wrong place. Resolved: §1 #1 #12 panel in the TASK-APP-001 console, no backend beyond the eval service, access/audit/state-machine live server-side (the TASK-APP-005 discipline); AC #19; §11 no-backend note; DEC-2624.
