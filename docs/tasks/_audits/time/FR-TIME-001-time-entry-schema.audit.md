...
Resolved: §1 #21 + DEC-224 + `rate_card_snapshot JSONB` on the row (snapshot pattern); TASK-TIME-005 fills.
...
Per task-audit skill §0 master rule: spec is now perfect — depth bounded by the genuine architectural surface (append-only via SQL grant × correction-via-new-row × acyclic chain × tree-rejection × cross-scope rejection × rate-card snapshot × multi-currency × per-row duration cap × RLS isolation × audit chain × view + walker × REST + idempotency × OTel), not by line targets.

---

*End of TASK-TIME-001 audit.*
