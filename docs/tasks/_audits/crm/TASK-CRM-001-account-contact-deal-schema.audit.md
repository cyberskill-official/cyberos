...
First-pass used `DECIMAL(15,2)` (better) but allowed bound-field flexibility. Resolved: §1 #13 + task-audit skill rule 11 + BIGINT minor + CHAR(3) currency; AC #18.
...
First-pass left "operator creates pipelines manually". Resolved: §1 #14 + seed_default_pipelines SQL function + TASK-TEN-001 hook integration; AC #23.

### ISS-008 — PII in memory audit (raw email, phone, full_name)
First-pass logged contact email in audit chain. Resolved: §1 #18 + TASK-MEMORY-111 scrubbing; hashed forms only in chain.
