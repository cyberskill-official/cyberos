...
authoring_md_compliance: 2026-06-29 (mirrors TASK-AUTH-104 structure; ≥6 canonical ISSes; first-party-IdP profile)
---

## §1 - Verdict summary

TASK-AUTH-110 makes AUTH a first-party OIDC provider so CHAT (Mattermost) and PORTAL federate to one CyberOS identity - the inverse of TASK-AUTH-104. Scope: 26 normative clauses over 4 tables (rp_clients + auth_codes single-use + sso_sessions + append-only op_login_history), OIDC + RFC 8414 discovery, authorize that brokers the human via an SSO cookie or upstream Google (TASK-AUTH-104) and is revoke-gated, token exchange minting id_token + access_token against the existing TASK-AUTH-004 keys, userinfo, an admin-only first-party RP registry with locked redirect_uris and one-time secret reveal, PKCE S256 mandatory with implicit/hybrid forbidden, exact redirect_uri match with reject-without-redirect, a pinned issuer checked at boot, and 7 memory audit kinds. The design's defining strength is honesty about the kick boundary: slice 1 blocks re-authentication and revokes the SSO session, slice 2 (back-channel logout) kills live downstream sessions. It reuses the TASK-MCP-004 OAuth substrate rather than rebuilding it. The verdict is PASS; one item is left as a deliberate impl-time decision, recorded below.
