...
TASK-MEMORY-106 expanded from 72 lines to ~700. Added 5 §1 clauses (#7 deterministic; #8 property test 10K rows; #9 pre-network call placement; #10 CLI dry-run; #11 metrics). 8 §2 rationale paragraphs. Full Rust types + structural exclusion + integration in §3. 17 ACs. 7 unit tests + 3 proptest cases. 14 failure modes. 8 implementation notes.
...
Tracking v1 adoption needs a metric for eventual deprecation. Resolved: §1 #11 + memory_sync_v1_transitional_total{value} counter; AC #11.
...
Operators debugging filter decisions need a dry-run. Resolved: §1 #10 + cyberos-memory validate-sync-class subcommand; AC #16.
