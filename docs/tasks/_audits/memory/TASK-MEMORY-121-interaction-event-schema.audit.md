...
### ISS-009 — TASK-EVAL-001 not yet shipped (forward dependency)
The consent gate reads a ledger TASK-EVAL-001 owns, which does not exist yet. Resolved: §7 + §11 document the stub (operator-seeded, default capture-OFF — the safe default) that the gate resolves against until TASK-EVAL-001 lands; only the stub is replaced, not the call site. The dependency is declared in `depends_on`.
...
The ID renumber (the prior `TASK-MEMORY-121` awh-gate-result task moved to TASK-MEMORY-124, with TASK-APP-005's dependency + prose and CHANGELOG.md updated) is recorded in the task's `renumbered_from`/`renumbered_note` frontmatter and is consistent across the repo's non-generated sources.

---

*End of TASK-MEMORY-121 audit.*
