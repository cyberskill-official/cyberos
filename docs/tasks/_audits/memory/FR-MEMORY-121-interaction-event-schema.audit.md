...
The ID renumber (the prior `TASK-MEMORY-121` awh-gate-result FR moved to TASK-MEMORY-124, with TASK-APP-005's dependency + prose and CHANGELOG.md updated) is recorded in the FR's `renumbered_from`/`renumbered_note` frontmatter and is consistent across the repo's non-generated sources.
