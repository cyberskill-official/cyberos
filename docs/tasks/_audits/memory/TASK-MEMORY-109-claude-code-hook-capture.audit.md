...
Fail-open = leak risk; fail-closed = capture loss. Resolved: §1 #5 fail-closed + `memory.claude_capture_redaction_failed` row; AC #12 covers the panic path.

### ISS-004 — Latency vs reliability tradeoff for chain write
Direct chain write from hook = correctness but 200ms latency; queued via socket = fast but eventual emit. Resolved: §1 #6 + #10 socket IPC to TASK-MEMORY-107 daemon; AC #15 latency budget; §2 explains the architectural choice.
