...
Per task-audit skill §0 master rule: spec is now perfect — depth bounded by the genuine architectural surface (2 closed enums × HMAC-SHA256 + constant-time compare × idempotency × replay window × append-only + privileged column-level grant × memo parser × payload-hash storage × server-side timestamp × 5 memory audit kinds × VND-only × per-tenant URL × 60s secret-rotation overlap × probe detection × CFO-gated rotation), not by line targets.

---

*End of TASK-INV-005 audit.*
