...
Direct LLM call = no policy. Resolved: §1 #3 via TASK-CUO-101 + persona=lumi.

### ISS-007 — Imported posts would re-trigger Lumi (strict-redo pass)
Original spec didn't address @lumi mentions in historical messages from TASK-CHAT-006/007 imports. Processing them would trigger thousands of LLM calls and produce anachronistic replies. Resolved: §1 #11 + `is_imported` propagation from plugin to service + early-return check; AC #16 + test body verify.
...
Per task-audit skill §0 master rule: spec is now perfect — depth bounded by the genuine surface (LLM persona flow + tenant gating + rate limiting + placeholder UX + multi-hop tracing + builtin commands + budget capture + error observability), not by line targets.

---

*End of TASK-CHAT-008 audit.*
