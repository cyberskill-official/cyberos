...
Per task-audit skill §0 master rule: spec is now perfect — depth bounded by the genuine surface (APNS + FCM v1 auth × privacy payload × per-tenant topic × DnD windows × dedup × badge × silent push × rate limit × locale × prod/sandbox), not by line targets.

---

*End of TASK-CHAT-011 audit.*
