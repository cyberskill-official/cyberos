...
### ISS-016 — Trace_id propagation unspecified (strict-redo pass, task-audit skill §3.7 rule 22)
...
Per task-audit skill §0 master rule: spec is now perfect — depth bounded by the genuine surface (interactive picker UX × per-message context × dedup × date-range × ACL × memory splitting × dry-run × full audit lifecycle), not by line targets.

---

*End of TASK-CHAT-009 audit.*
