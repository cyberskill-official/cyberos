...
The generic OpenID connector is Enterprise-gated in the open-source build. Resolved: DEC-2501 + §3.2 use the GitLab connector with overridden endpoint URLs (the standard free-edition method), with the Enterprise OpenID path in §3.3 as the alternative - the task does not assume a connector the free build lacks.

### ISS-003 - JIT must create a real user, not a simulation (RESOLVED)
The closed task's provisioner was in-memory. Resolved: DEC-2502 + §1 #3 - Mattermost's native sign-in creates a real user row + session from sub/email/preferred_username; the task explicitly contrasts this with the simulation.
...
6 of 6 concerns resolved. The supersede linkage (TASK-CHAT-002 closed; this task's `supersedes` + the banner) is clean, the kick boundary is honest, and the free-vs-enterprise connector reality is handled rather than assumed. The task's depth is bounded by its real surface (one provider, one connector, config + an owner-run sign-in), not by a line target.
