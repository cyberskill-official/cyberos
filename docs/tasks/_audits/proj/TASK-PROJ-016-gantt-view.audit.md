...
Per task-audit skill §0 master rule: spec is now perfect — depth bounded by the genuine surface (dep CRUD × cycle detect × critical path × slack × parent rollup × kbd × validate CLI × near-cycle × PDF export × batch × completion % × undo), not by line targets.

---

*End of TASK-PROJ-016 audit.*
