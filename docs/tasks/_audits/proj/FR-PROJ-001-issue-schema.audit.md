...
First-pass §3 had RLS migration but didn't update TASK-AUTH-003 registry. New tenant create wouldn't auto-provision policy. Resolved: modified_files includes templates.rs update.
...
| #7 RLS via TASK-AUTH-003 pattern | #9 | All 4 migrations enable RLS + FORCE RLS + `tenant_scoped` policy using `app.current_tenant_id`; issue_links policy joins through `issues` | ✅ |
...
| #14 OTel metrics | — | Metric names declared in §1; emission via `tracing` in the binary; full OTel wiring is TASK-OBS-003 territory | ⏸️ TASK-OBS-003 |
...
**§10.6.a — RLS GUC naming.** Spec §3 uses `app.tenant_id`. Implementation uses `app.current_tenant_id`, aligning with TASK-AUTH-003 §10.6 amendment.
