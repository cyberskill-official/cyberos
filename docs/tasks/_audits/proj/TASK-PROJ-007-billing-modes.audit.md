...
Per task-audit skill §0 master rule: spec is now perfect — depth bounded by the genuine surface (3 modes × proration × milestone CRUD × overage tracking × holidays × mixed currency × preview × p95 budget × metadata pre-fetch), not by line targets.

---

*End of TASK-PROJ-007 audit.*
