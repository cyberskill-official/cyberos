...
A PATCH that changes 3 fields produces 3 history events with no link back to the originating request. Resolved: §1 #15 + session_id (correlates with TASK-PROJ-002) + AC #19.
...
Per task-audit skill §0 master rule: spec is now perfect — depth bounded by the genuine surface (dual-store linkage × chain-anchor verification × cache × sweep × pagination × session correlation × summary × PII × source × concurrency check), not by line targets.

---

*End of TASK-PROJ-008 audit.*
