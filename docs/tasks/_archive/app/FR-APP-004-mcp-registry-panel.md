...
depends_on: [TASK-APP-001, TASK-AUTH-004, TASK-MCP-001, TASK-MCP-002, TASK-MCP-003, TASK-MCP-004, TASK-MCP-005, TASK-MCP-006]
---
...
4. The panel MUST sit behind the TASK-APP-001 auth gate: it is reachable only within a valid operator session, and it MUST reuse the existing auth flow (TASK-AUTH-004 JWT, TASK-AUTH-104 OIDC for IdP operators) rather than defining its own credential path.

5. The panel MUST render the per-module server health using the closed set of states the mcp-gateway defines (TASK-MCP-002): `healthy`, `degraded`, `unhealthy`, `deregistered`. It MUST NOT invent additional states, collapse the four into a binary, or infer a health value the gateway did not return.

6. The panel MUST render each tool's spec annotations as the gateway reports them in `tools/list` (TASK-MCP-001): `readOnlyHint`, `destructiveHint`, `idempotentHint`, `openWorldHint`, and the SEP-986 tool name (TASK-MCP-003). A tool the gateway marks `destructiveHint: true` MUST be shown as destructive in the panel, not flattened to a generic entry.
...
10. On an expired or rejected session (a 401 from the gateway) the panel MUST return the operator to sign-in through the TASK-APP-001 gate rather than rendering a partial or stale view with a dead token.
...
- Source: the mcp-gateway request log and the OTel `mcp_gateway_request_total` counter (TASK-MCP-001).
...
- TASK-APP-001 APP CDS web console - the shell, navigation, auth gate, CDS tokens, and Caddy deployment path this panel extends; the panel is a screen set inside that SPA, not a new app.
- TASK-MCP-001 mcp-gateway 2025-11-25 spec compliance - the `tools/list` federated catalog the tools view renders, the tool annotations it shows, and the `GET /mcp/healthz` aggregate counts the health view reads.
- TASK-MCP-002 per-module server registration and heartbeat lifecycle - the `GET /v1/mcp/servers` status list and the closed `healthy` / `degraded` / `unhealthy` / `deregistered` health states the registry and health views render.
- TASK-MCP-003 SEP-986 naming validator - the `cyberos.{module}.{verb}_{noun}` tool-name form the tools view displays as the gateway reports it.
- TASK-MCP-004 OAuth 2.1 PKCE - the OAuth clients the OAuth view surfaces and the audience-bound token model behind the gateway's auth.
- TASK-MCP-005 Protected Resource Metadata - the `/.well-known/oauth-protected-resource` and per-module documents the OAuth view reads.
- TASK-MCP-006 tool-annotation gating - referenced for the standing destructive-tool contract in clause 8: any future tool-trigger screen routes destructive tools through this confirmation gate.
- TASK-AUTH-004 JWT and JWKS - the operator session token the auth shell holds and the panel presents on every authenticated gateway call.
- Cross-cutting: the existing Caddy front that terminates the other CyberOS surfaces and the mcp-gateway; the panel ships as part of the TASK-APP-001 static bundle behind the same front, not a new ingress.
