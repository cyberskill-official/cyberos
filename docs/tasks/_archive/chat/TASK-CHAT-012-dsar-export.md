...
related_tasks: [TASK-CHAT-005, TASK-MEMORY-101, TASK-AUTH-005]
...
subtasks:
  ...
  - "1.0h: manifest.rs — list every message with chain_anchor from TASK-CHAT-005 memory row"
...
17. **MUST** include push device registrations (TASK-CHAT-011): historical record of which devices the subject registered, with `{platform, registered_at, deregistered_at}`. Tokens themselves NOT included (security: tokens are device credentials).
18. **MUST** include Lumi interactions (TASK-CHAT-008): every `chat.lumi_invoked` memory row where the subject was the user_id. Payload preserves both the redacted body AND the response.
19. **MUST** include retro captures (TASK-CHAT-009): every memory the subject created OR was a participant in.
...
while IFS= read -r line; do
    ...
    if [[ "$anchor" == "null" ]]; then
        echo "SKIP: $post_id (no chain anchor; pre-TASK-CHAT-005 message)"
        continue
    fi
...
The chat-dsar service runs as a Fargate task in each tenant's CHAT cluster (TASK-CHAT-003). It exposes:
...
- **TASK-CHAT-005** — memory_anchor source.
- **TASK-MEMORY-101** — chain verification reference.
- **TASK-AUTH-005** — admin role check.
...
| memory chain_anchor missing for pre-TASK-CHAT-005 messages | anchor field is null | export degraded; SKIP in verify | Operator backfills (slice-4+) |
...
- S3 KMS uses tenant's CMK (per TASK-CHAT-003 §1 #7). Cross-tenant decryption is cryptographically prevented.
