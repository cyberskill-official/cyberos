---
id: TASK-CHAT-013
title: "CHAT native OIDC SSO - Mattermost federates to the TASK-AUTH-110 CyberOS OIDC provider via its built-in OIDC/OAuth connector (replaces the closed TASK-CHAT-002 AuthBridge plugin)"
module: CHAT
priority: MUST
status: superseded
...
supersedes: [TASK-CHAT-002]
related_tasks: [TASK-CHAT-001, TASK-CHAT-002, TASK-CHAT-003, TASK-AUTH-110, TASK-AUTH-005, TASK-AUTH-101, TASK-PORTAL-004]
depends_on: [TASK-CHAT-001, TASK-AUTH-110, TASK-CHAT-003]
blocks: []

source_pages:
  - website/docs/modules/chat.html#auth
source_decisions:
  - DEC-2500 (Mattermost federates to the TASK-AUTH-110 CyberOS OIDC provider through Mattermost's OWN native connector - not a plugin. TASK-CHAT-002's AuthBridge approach is closed: a Mattermost plugin cannot replace the core /api/v4/users/login route, and the shipped plugin is a non-working simulation)
  ...
  - DEC-2503 (the kick: a CyberOS revoke (TASK-AUTH-005) makes the provider's authorize refuse - re-login is blocked into chat and every first-party app; an already-open Mattermost session dies via TASK-AUTH-110 back-channel logout (its slice 2) or the Mattermost session TTL; instant cross-app deprovision is SCIM = TASK-PORTAL-004)
  ...
  - DEC-2506 (drop the TASK-CHAT-002 build patches 010-disable-builtin-auth + 011-load-authbridge-plugin; builtin password sign-up is turned off via Mattermost's own config (EnableSignUpWithEmail=false, the SSO-only settings), not a patch or plugin)
  - DEC-2507 (roles: the id_token roles claim (TASK-AUTH-101) is available to map CyberOS roles to Mattermost system/team roles where the connector supports it; the mapping is operator-configured, not hard-coded)
  - DEC-2508 (the cyberos-chat RP client_secret is the one-time reveal from the TASK-AUTH-110 registry, stored only in Mattermost config/env, never committed; the redirect_uri is registered byte-exact)
...
effort_hours: 6
subtasks:
  - "1.0h: register the cyberos-chat RP in the TASK-AUTH-110 provider; capture the one-time secret"
...
CHAT (the Mattermost fork, TASK-CHAT-001) **MUST** authenticate users by federating to the TASK-AUTH-110 CyberOS OIDC provider through Mattermost's own native OIDC/OAuth connector, replacing the closed TASK-CHAT-002 AuthBridge plugin. Each requirement:

1. **MUST** use Mattermost's native connector, not a plugin (DEC-2500). TASK-CHAT-002 is closed because a Mattermost plugin cannot intercept the core `/api/v4/users/login` route (plugins serve only under `/plugins/<id>/`) and the shipped plugin is a non-working simulation. SSO in Mattermost is configured in the server (GitLab/OpenID/SAML connectors), never a plugin.
...
4. **MUST** register `cyberos-chat` as a relying party in the TASK-AUTH-110 provider (`POST /v1/admin/op/rp-clients`), with the Mattermost sign-in-complete URL as the single registered redirect_uri, byte-exact (DEC-2508). The one-time client_secret is stored only in Mattermost config/env, never committed.
...
9. **MUST** make the `roles` claim (TASK-AUTH-101) available for operator-configured mapping to Mattermost system/team roles where the connector supports it (DEC-2507). No hard-coded role mapping.

10. **MUST** verify by an owner-run live browser sign-in (DEC-2509): a user with no session opens chat, is sent to the provider, brokered through Google (the TASK-AUTH-110 broker), returned with a session, lands in chat as a real provisioned user; then a revoke blocks the next sign-in. The connector configuration is checked into deploy config; there is no unit test for a browser SSO handshake.
...
Why the native connector, not the plugin. TASK-CHAT-002 assumed a Mattermost plugin could replace login. It cannot: a plugin only receives requests under `/plugins/<id>/`, so the AuthBridge's attempt to intercept `/api/v4/users/login` never fires, and the shipped code is a simulation - no Mattermost SDK, in-memory fake users, a fabricated session string. Mattermost has a real, supported way to do SSO: its server-side OAuth/OpenID connectors. We use that.
...
Why the kick is honest. Revoking a CyberOS subject blocks the next sign-in immediately, because the provider's authorize refuses. It does not, by itself, kill a Mattermost session already open - Mattermost holds its own session after login. We are explicit: instant logout needs back-channel logout (the provider's slice 2) or SCIM (TASK-PORTAL-004); the session TTL bounds the gap meanwhile.
...
Why drop the patches. With the native connector doing auth, the two TASK-CHAT-002 patches that disabled builtin auth and loaded the dead plugin are unnecessary; builtin sign-up is turned off through Mattermost's own settings, which is the supported lever.
...
4. **redirect_uri exact** - a mismatched redirect is rejected by the provider (TASK-AUTH-110), never redirected.
...
Upstream:
- TASK-AUTH-110 - the CyberOS OIDC provider Mattermost federates to (built + green).
- TASK-CHAT-001 - the Mattermost fork being configured.
- TASK-CHAT-003 - per-tenant deployment (provides the HTTPS endpoint OIDC needs).
- TASK-AUTH-005 - revoke (the kick the provider enforces).
- TASK-AUTH-101 - roles claim for optional role mapping.

Supersedes:
- TASK-CHAT-002 - the AuthBridge plugin (closed; non-working simulation).

Downstream / related:
- TASK-PORTAL-004 - SCIM auto-deprovision for instant cross-app removal.
- TASK-AUTH-110 slice 2 - back-channel logout, for instant session kill in chat.
...
- **Instant session kill in chat** - TASK-AUTH-110 slice 2 (back-channel logout) + TASK-PORTAL-004 (SCIM). Until then, revoke blocks re-login and the session TTL bounds the window.
...
- Native Mattermost connector, never a plugin (TASK-CHAT-002 closed).
...
- Kick = revoke blocks re-login now; instant session kill is back-channel logout (slice 2) / SCIM (TASK-PORTAL-004).
- HTTPS end to end; the __Host- cookie needs TLS.
- Single-tenant one-team for slice 1; multi-tenant sync later.
- Secret one-time from the TASK-AUTH-110 registry, env-only, never committed.
