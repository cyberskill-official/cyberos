...
related_tasks: [TASK-CHAT-003, TASK-CHAT-005, TASK-MEMORY-108]
...
**Why RLS at query time (§1 #6)?** Mattermost's ACL is its own layer; TASK-AUTH-003 RLS is ours. The query MUST satisfy BOTH — Mattermost team membership AND tenant_id RLS. Skipping either is a leak: Mattermost-only allows cross-tenant scan; RLS-only allows cross-team scan within a tenant.
...
**Build flow:**
1. Add PGroonga to RDS parameter group via TASK-CHAT-003 Terraform module.
2. Run init migrations on RDS bootstrap.
3. Build plugin via Mattermost SDK (`mattermost plugin manifest`).
4. Install plugin via TASK-CHAT-001 patch loading mechanism.
...
- **TASK-CHAT-003 (upstream)** — RDS host; Terraform module enables PGroonga extension via parameter group.
...
- Cross-channel search via vector embeddings (memory search delegation per TASK-MEMORY-108) — slice 4+; semantic queries cross over.
...
|---|---|---|---|
| PGroonga extension missing | `CREATE EXTENSION` Err on bootstrap | RDS bootstrap fails; sev-1 | Operator runs migration manually; or TASK-CHAT-003 module updated |
...
| Audit row emit fails | memory socket down | Search still succeeds; audit lost; sev-2 | Operator restores TASK-MEMORY-107 |
...
| 100K+ message corpus exceeds budget | latency p95 > 200ms | Sev-2 alarm; TASK-CHAT-003 tier upgrade recommended | Operator scales |
...
| Unicode normalisation drift (NFC vs NFD) | bigram mismatch | False negatives | Plugin applies NFC at query time (matches TASK-CHAT-007 import pattern) |
...
- The rate limiter is in-process per-tenant via `governor` crate (same pattern as TASK-AI-021 + TASK-SKILL-108). Cross-pod state would need Redis (slice 4+).
- `chat.search_query` audit rows aggregate to a per-tenant per-day "search heatmap" via TASK-OBS-008 compliance view — informs which terms users search most.
...
- Unicode NFC normalisation: VN text from imports (TASK-CHAT-006/007) may arrive NFD; plugin applies `string.ToValidUTF8` + relies on Postgres's `unaccent` extension being NOT installed (since we want diacritics to match exactly). Diacritic-stripping fallback is slice-4+.

---

*End of TASK-CHAT-004.*
