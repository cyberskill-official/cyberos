...
related_tasks: [TASK-CHAT-001, TASK-CHAT-002, TASK-CHAT-004, TASK-CHAT-005, TASK-OBS-001]
depends_on: [TASK-CHAT-001, TASK-CHAT-002]
blocks: [TASK-CHAT-004, TASK-CHAT-005, TASK-CHAT-011]
...
source_decisions:
  ...
  - DEC-443 (egress to TASK-AUTH-004 JWKS + TASK-MEMORY-101 MemoryWriter via VPC endpoint, not public internet)
...
subtasks:
  ...
  - "0.5h: observability.tf — CloudWatch log group + TASK-OBS-001 collector sidecar"
...
**Why CloudWatch alarms (§1 #13)?** OBS layer (TASK-OBS-007) handles app-level alerts; AWS-native alarms catch infra-level. Two layers for resilience.
...
# Cross-account PrivateLink to TASK-AUTH-004 JWKS service.
...
- **TASK-CHAT-001** — image source.
- **TASK-CHAT-002** — authbridge plugin baked into image.
- **TASK-CHAT-004 (downstream)** — search uses RDS.
- **TASK-CHAT-005 (downstream)** — memory bridge uses Postgres logical replication.
- **TASK-OBS-001** — collector sidecar.
- **TASK-AUTH-004** — JWKS source.
- **TASK-MEMORY-101** — memory writer socket.
...
```json
{
  ...
  "payload": {
    ...
    "operator_action": "verify TASK-MEMORY-101 socket is up and tenant subnet has route"
  }
}
```
...
| RDS CPU > 80% sustained | `rds_cpu` CloudWatch alarm | Sev-2 | Scale up tier OR identify query causing CPU via TASK-OBS-005 |
...
| Plugin (TASK-CHAT-002) hot-reload triggers Fargate task replace | task definition revision bumped | Rolling restart; <30s downtime per task | None — ALB drains |
