...
related_tasks: [TASK-CHAT-005, TASK-CHAT-009, TASK-AI-014, TASK-CUO-101, TASK-MEMORY-101]
depends_on: [TASK-CHAT-005]
blocks: [TASK-CHAT-009]

source_pages:
  - website/docs/modules/chat.html#lumi
source_decisions:
  - DEC-490 (@lumi is the canonical assistant mention; case-insensitive; works in channel + DM)
  - DEC-491 (mention extracted from message body via regex; routed to TASK-CUO-101 supervisor)
...
subtasks:
  ...
  - "1.0h: chat-lumi service — receives webhook; routes to CUO supervisor (TASK-CUO-101)"
...
22. **MUST** redact CUO response before memory audit emit, separately from request redaction. The redaction ruleset is the same TASK-MEMORY-111 ruleset; responses can leak data the model echoed from the prompt.
...
25. **MUST** record the CUO budget consumed per invocation in memory payload (`budget_tokens_used`, `budget_dollars_estimate`); TASK-AI-014 surfaces these for billing.
...
**Why redaction before memory (§1 #6)?** Users may @lumi with sensitive info; TASK-MEMORY-111 ruleset scrubs.

**Why skip imported posts (§1 #11)?** Importing 100k historical Slack/Zalo messages would trigger 100k LLM calls if Lumi processed historical mentions — $$$ and wrong (the original conversation already happened; replying now is anachronistic). The `cyberos_imported` props marker from TASK-CHAT-006/007 carries through; we check it explicitly.
...
**Why CUO budget in audit (§1 #25)?** Per-tenant billing for AI usage requires per-invocation cost capture. TASK-AI-014 aggregates these into invoice line items.
...
Budget fields come from TASK-AI-014's per-invocation accounting. The Rust service receives them as part of the CUO response payload. If TASK-AI-014 returns no budget info (older CUO version), the audit row carries `budget_tokens_used: null` + a SEV-3 warning is logged.
...
`cyberos_chat_tenant_settings` is read on EVERY webhook. We cache the row in-process with 30s TTL to avoid hammering Postgres. Cache invalidation on `cyberos_chat_tenant_settings.updated_at` change is via TASK-AUTH-005 webhook (same pattern as tenant_map in TASK-CHAT-002).
...
The plugin install step (TASK-CHAT-002 install workflow) creates a MM user `Lumi` with `is_bot=true`, `cyberos_system_user=true` props. The bot user's ID is read once at plugin OnActivate and cached in `lumi_system_user_id()`.
...
Single errors are SEV-3 (logged, counter incremented). Sustained errors (>1/min) escalate via TASK-OBS-007 alarm to SEV-2. This avoids paging operators on transient CUO timeouts while still surfacing structural failures.
...
- **TASK-CHAT-005** — bridge picks up the reply too.
- **TASK-CHAT-009** — retro-capture is sibling flow.
- **TASK-CUO-101 (placeholder)** — supervisor.
- **TASK-AI-014** — persona-stamped LLM via CUO.
...
| Lumi system user missing | first post fails 404 | SEV-1 chat.lumi_error; install incomplete | Operator provisions via TASK-CHAT-002 admin REST |
...
| CUO budget unavailable in response | TASK-AI-014 older version | audit row has budget_tokens_used=null + SEV-3 | Upgrade CUO |
...
- Reply system user = "Lumi" with avatar; provisioned at plugin install via TASK-CHAT-002 admin REST. The avatar PNG ships in the plugin bundle.
- Slice-3 CUO stub returns canned acknowledgement; full LLM compose in P2 via TASK-CUO-101. The Lumi-mention contract is stable; only the CUO backend evolves.
...
- The placeholder + edit pattern means the same MM post_id is preserved across the placeholder + final reply. Downstream consumers (TASK-CHAT-005 bridge) see the edit as a chat.message_edited event; that's fine.
...
- The CUO call timeout (60s) is calibrated against TASK-CUO-101's stated SLA. Longer would block the placeholder forever; shorter would prematurely fail.
...
- Channel context (`channel_member_count`, `user_is_admin`) is sent in CUO request but NOT stored in memory audit (per TASK-MEMORY-111 minimal-data principle). CUO uses it for one decision then discards.
