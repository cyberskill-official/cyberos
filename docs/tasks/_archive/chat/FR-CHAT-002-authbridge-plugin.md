---
id: TASK-CHAT-002
title: "cyberos-chat-authbridge plugin — Mattermost auth delegates to TASK-AUTH-004 JWT with tenant_id propagation and SCIM-free provisioning"
module: CHAT
priority: MUST
status: superseded
superseded_by: [TASK-AUTH-110, TASK-CHAT-013]
closed: 2026-06-29
closed_reason: "superseded - the AuthBridge plugin approach does not work (a Mattermost plugin cannot replace the core login route, and the shipped plugin is a non-working simulation); the unified path is Mattermost's native OIDC connector federating to the TASK-AUTH-110 provider, specified in TASK-CHAT-013"
verify: T
phase: P1
...
related_tasks: [TASK-CHAT-001, TASK-CHAT-003, TASK-AUTH-004, TASK-AUTH-005, TASK-AUTH-110]
depends_on: [TASK-CHAT-001, TASK-AUTH-004]
blocks: [TASK-CHAT-003]

source_pages:
  - website/docs/modules/chat.html#auth
source_decisions:
  - DEC-430 (Mattermost auth replaced by JWT validation against TASK-AUTH-004 JWKS; no Mattermost users table writes)
...
disallowed_tools:
  ...
  - skip JWT signature verification (per TASK-AUTH-004)

effort_hours: 10
subtasks:
  - "0.5h: plugin.json manifest"
  - "1.0h: main.go — HTTP middleware intercepting /api/v4/users/login and /api/v4/users/me/sessions"
  - "1.0h: jwks_cache.go — fetch + cache TASK-AUTH-004 /.well-known/jwks.json; 1h TTL"
  - "1.5h: JWT validation per TASK-AUTH-004 spec (signature, exp, nbf, iss, tenant_id claim required)"
...
> own native OIDC connector federating to the TASK-AUTH-110 CyberOS OIDC provider, specified in
> [[TASK-CHAT-013]]. The DEC-430/431/432 ids below also collided with TASK-AUTH-110's first draft and were
> renumbered there (DEC-2480+); these originals stand as the CHAT-002 record.

## §1 — Description (BCP-14 normative)

The authbridge plugin **MUST** delegate Mattermost authentication to TASK-AUTH-004 JWT with tenant propagation. The contract:
...
```json
{
  ...
  "description": "Delegates Mattermost authentication to TASK-AUTH-004 JWT with tenant propagation",
  "version": "1.0.0",
...
// Backed by cyberos_chat_tenant_map (TASK-AUTH-005 admin REST owns writes).
...
// Invalidate is called from TASK-AUTH-005 webhook on mapping change.
...
28. **Tenant-team mapping cache invalidation** — TASK-AUTH-005 webhook → `tenantMap.Invalidate(tenant_id)` → subsequent login does a fresh DB lookup (verified by stub-DB-counter assertion).
...
Per task-audit skill §3.8 rule 25 (audit-before-action), the memory row MUST be emitted BEFORE the CreateSession call returns to the client. The pattern is:
...
### §6.3 — `cyberos_chat_tenant_map` schema (referenced by TASK-AUTH-005)

```sql
-- Owned by TASK-AUTH-005 admin REST; read by TASK-CHAT-002.
...
The unique partial index satisfies task-audit skill §3.4 rule 14 (at most one active row per tenant). The `archived_at` column is the audit-trail for re-mapping operations.
...
| `cyberos_tenant_id`  | UUID string | JWT `tenant_id` | Only via tenant-switch flow (TASK-AUTH-005). |
| `cyberos_jit`        | bool string | "true" on creation | Never. |
| `cyberos_first_login_at` | ISO-8601 | now() | Never. |

Downstream plugins (TASK-CHAT-004 search, TASK-CHAT-008 mention) read `Props["cyberos_tenant_id"]` for tenant scoping. The plugin MUST NOT overwrite these props on subsequent logins — only on first JIT provision.
...
`cyberos_jti` is stored so downstream logout (Mattermost `RevokeSession`) can publish the JTI to the AUTH-004 revocation list — see TASK-AUTH-005.
...
- **TASK-CHAT-001** — fork.
- **TASK-AUTH-004** — JWT + JWKS endpoint.
- **TASK-AUTH-005** — admin REST for team-tenant mapping CRUD.
- **TASK-CHAT-003 (downstream)** — Fargate deployment runs this plugin.
...
- SSO with WebAuthn passkey via TASK-AUTH-004 — slice 4+.
...
| JWKS kid rotation | KeyFunc cache miss | Synchronous refresh; if new kid present, login succeeds; else 401 | Operator confirms TASK-AUTH-004 rotation; should be transparent |
...
| Tenant team not mapped in `cyberos_chat_tenant_map` | tenantMap.Lookup returns err | 500 `tenant_not_mapped`; SEV-2 page | Operator adds mapping via TASK-AUTH-005 admin REST |
| TASK-AUTH-005 webhook for mapping change lost | Stale cache TTL 5min | Stale mapping for ≤5min; new login uses stale team_id | Operator manually invalidates via plugin admin endpoint |
...
| Revocation deny-list returns false positive | jti incorrectly marked revoked | 401 `revoked` for legitimate user | Operator removes jti from deny-list via TASK-AUTH-005; cache TTL 60s |
...
| memory audit row schema rejection (kind not whitelisted) | memory returns 422 | Logged; login still succeeds; metric `audit_emit_failures_total{reason=schema}` | Coordinate with TASK-AI-003 closed-set extension |
...
- The revocation lookup uses HTTP polling with a 60s LRU cache rather than a streaming subscription. Rationale: simpler operationally; 60s revocation lag is acceptable for a B2B product (TASK-AUTH-004 §1 #19 sets the SLA at 5 minutes); the streaming complexity isn't justified.
...
- We chose ES256 over RS256 for JWT signing (in coordination with TASK-AUTH-004): smaller signatures (64 bytes vs 256 bytes), better mobile-bandwidth profile, faster verification (constant-time scalar mul vs modular exponentiation).
...
- The plugin does NOT issue refresh tokens — the JWT is the only credential. Mattermost session refresh happens by re-presenting the JWT to `/login`. This is a deliberate choice: TASK-AUTH-004 owns the refresh-token lifecycle; the chat plugin stays stateless and lean.
