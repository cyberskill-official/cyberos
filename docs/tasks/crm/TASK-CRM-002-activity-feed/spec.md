...
related_tasks: [TASK-CRM-001, TASK-EMAIL-006, TASK-EMAIL-009, TASK-CHAT-005, TASK-AI-003, TASK-MEMORY-111]
depends_on: [TASK-CRM-001, TASK-EMAIL-006]
blocks: []
...
6. **MUST** support manual `note_added` + `call_logged` via POST endpoint:
   ```text
   POST /v1/crm/activities  (CRO/AM via TASK-AUTH-101)
   GET  /v1/crm/contacts/{id}/activities?limit=50  (paginated, desc time)
   GET  /v1/crm/accounts/{id}/activities?limit=50
   ```

7. **MUST** emit 3 memory audit kinds per DEC-1625. PII: summary text SHA-256 hashed per TASK-MEMORY-111.
