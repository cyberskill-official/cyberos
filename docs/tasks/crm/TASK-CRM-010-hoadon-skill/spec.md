...
related_tasks: [TASK-CRM-004, TASK-INV-001, TASK-INV-007, TASK-SKILL-109, TASK-MEMORY-111]
depends_on: [TASK-INV-007, TASK-CRM-004]
blocks: [TASK-TIME-008]
...
subtasks:
  ...
  - "1.3h: integration with TASK-INV-007 + smoke test"

risk_if_skipped: "Without CRM-side trigger, CFO must manually create invoice + emit hóa đơn for every won deal (operational burden). Without DEC-1702 idempotency, retry storms cause duplicate emit (illegal per Decree 123). Without DEC-1703 delegation, CRM duplicates hóa đơn logic (TASK-INV-007 is canonical)."
---
...
7. **MUST** emit 4 memory audit kinds per DEC-1704. PII per TASK-MEMORY-111: deal_value SHA-256 hashed; ids ok.

8. **MUST** thread trace_id from deal-hook / manual call → orchestrator → TASK-INV-001 → TASK-INV-007 → audit.
...
11. **MUST NOT** reimplement TASK-INV-007 logic per DEC-1703 — orchestration only.
