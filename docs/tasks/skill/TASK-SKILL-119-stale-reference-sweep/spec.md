...
related_tasks: [TASK-SKILL-115, TASK-SKILL-118, TASK-DOCS-002]
depends_on: []
blocks: []
source_pages:
...
source_decisions:
  ...
  - "ship-tasks.md:199 still claims the code-review pair 'doesn't exist yet' (it exists and is vendored); :137 references a future Issue Request artefact as bare TBD."
  - "Distinct from TASK-SKILL-115 (done), which swept <placeholder> syntax; this sweeps dead cross-references and adds the checker that keeps them dead."
...
4. Anchor exists only in the rendered site, not the markdown (generated pages) - scope is repo markdown; generated-site anchors are TASK-DOCS-002's builder concern.
