...
related_tasks: [TASK-SKILL-103, TASK-SKILL-104, TASK-SKILL-105, TASK-SKILL-108, TASK-SKILL-110, TASK-MEMORY-111]
depends_on: [TASK-SKILL-104, TASK-SKILL-108]
blocks: [TASK-SKILL-110]
...
- **TASK-SKILL-104** — broker enforces `allowed_tools: [MemoryEmit]`; verifies skill cannot call HttpFetch.
- **TASK-SKILL-105** — memory-capture SDK used internally for audit emit.
- **TASK-SKILL-108** — vietnam-mst-validate is a sibling VN-pack skill; both depend on shared `transliterate()` + `redact()` helpers (in `cyberos-vn-common` crate).
- **TASK-SKILL-110** — vietnam-vat-invoice depends on this skill for embedding QR in hóa đơn.
- **TASK-MEMORY-111** — PII detection ruleset includes `VnBankAccount` tag.
...
- The bank registry is hand-curated from NAPAS's public BIN list (last refresh: 2026-04). Quarterly refresh is owned by TASK-SKILL-109's release captain.
- The transliteration map is shared with TASK-SKILL-108 + TASK-SKILL-110 via `cyberos-vn-common` crate (created in slice-3 release).
