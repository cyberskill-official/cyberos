...
related_tasks: [TASK-SKILL-117, TASK-SKILL-118, TASK-CUO-209, TASK-IMP-068]
depends_on: []
blocks: [TASK-CUO-209]
source_pages:
...
5. Payload built by an older build.sh (no check embedded) - CI (TASK-IMP-068) runs the current checkout's check against the fresh build, so stale build scripts cannot bypass it on main.
