...
related_tasks: [TASK-SKILL-101, TASK-SKILL-102, TASK-SKILL-103, TASK-SKILL-105, TASK-MEMORY-101, TASK-MEMORY-106, TASK-AUTH-003]
depends_on: [TASK-SKILL-103]
blocks: [TASK-SKILL-105, TASK-SKILL-108, TASK-SKILL-109, TASK-SKILL-110]
...
subtasks:
  ...
  - "0.5h: registry.rs — global MCP_TOOL_REGISTRY; TASK-SKILL-104 owns registration; TASK-SKILL-103 validates names against it"
...
- **FD leak**: broker may have an open writer-process FD (per TASK-MEMORY-101); skill could write to memory bypassing the broker. `close_fds` prevents.
...
**Why args_hash, not args (§1 #7)?** Same reason TASK-MEMORY-109 hashes Claude Code tool args — secrets in args (Bash commands like `curl -H "Bearer ..."`). Hash for dedup + audit; never store raw.
...
**Why x-prefixed extensions for files + domains (§1 #3)?** The v1 schema (TASK-SKILL-103) is frozen at 5 required + 8 optional fields. Adding `allowed_files` + `allowed_domains` as required would break existing skills; making them v2 fields requires a major bump. The `x-` extension namespace (per TASK-SKILL-103 §1 #5) is the right escape hatch — these are de facto required for skills that read files or call HTTP, but v1 schema doesn't enforce them.
...
- **TASK-MEMORY-101** — MemoryWriter (used by broker to emit audit rows).
- **TASK-MEMORY-106** — sync_class semantics.
- **TASK-AUTH-003** — RLS on MemoryRead enforces tenant_id scope.
...
- The Unix socket pattern means broker process must stay alive for the duration of invocation; it owns the socket file. On broker crash, the file is orphaned; TASK-MEMORY-110's sweeper cleans `/tmp/cyberos-skill-broker.*.sock` files older than 5 minutes.
