...
related_tasks: [TASK-SKILL-103, TASK-SKILL-104, TASK-SKILL-105, TASK-MEMORY-103]
depends_on: [TASK-SKILL-105]
blocks: [TASK-SKILL-107]

source_pages:
  - website/docs/skills/memory-sync.html
source_decisions:
  - DEC-400 (slice-3 ships skill scaffold + stub broker call; full sync orchestration is TASK-MEMORY-103 + P2 Stage 4)
...
disallowed_tools:
  - implement sync logic in this skill (per DEC-400 — TASK-MEMORY-103 owns)
...
The `memory-sync@1` skill bundle **MUST** scaffold the sync-trigger surface; full orchestration deferred to TASK-MEMORY-103 + P2. The contract:
...
4. **Bash CLI prints warning** — `cyberos-memory-sync` → stderr contains "slice-3" + "TASK-MEMORY-103".
...
- **TASK-MEMORY-103 (downstream sync owner)** — full sync logic ships here in P2.
...
- Slice-4: delegate to TASK-MEMORY-103 Unix socket for real sync.
...
| OTel exporter down | metric buffered | Logged | Restore TASK-OBS-001 |
...
- The `memory.sync_requested` kind is intentionally distinct from `memory.sync_*` rows TASK-MEMORY-103 will emit; this is the *operator request*, not the daemon's *action*.

---

*End of TASK-SKILL-106.*
