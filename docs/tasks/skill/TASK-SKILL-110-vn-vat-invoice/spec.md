...
related_tasks: [TASK-SKILL-103, TASK-SKILL-104, TASK-SKILL-105, TASK-SKILL-108, TASK-SKILL-109, TASK-MEMORY-111, TASK-AUTH-003]
depends_on: [TASK-SKILL-104, TASK-SKILL-108, TASK-SKILL-109]
blocks: []
...
subtasks:
  ...
  - "1.5h: xml_schema_test.rs — happy path + 5 mandatory fields missing + buyer MST invalid (via TASK-SKILL-108 cross-check)"
...
**Why ed25519 (§1 #6)?** GDT accepts RSA-2048 and ECDSA; ed25519 (RFC 8032) is faster, smaller, and standardised. GDT's spec allows ed25519 since 2024. Our cert pipeline (TASK-AUTH-004 reuse) is already ed25519.
...
2. **Buyer MST validated upfront** — request with buyer.mst="9999999999" (invalid checksum) → TASK-SKILL-108 returns ChecksumFailed → mapped to `Err(BuyerMstInactive { status: "checksum_failed" })`.
...
- **TASK-SKILL-104** — broker enforces `allowed_tools` + domain allowlist on hoadondientu.gdt.gov.vn.
- **TASK-SKILL-105** — memory-capture SDK used for audit rows.
- **TASK-SKILL-108** — MST validation (buyer.mst checked upfront).
- **TASK-SKILL-109** — VietQR for embedded payment-collection (often appended to PDF render).
- **TASK-AUTH-003** — RLS on tenant cert lookup (tenant can only sign with own cert).
- **TASK-MEMORY-111** — PII rules for buyer info redaction.
...
- Embed VietQR in PDF render — slice 4+; uses TASK-SKILL-109.
...
|---|---|---|---|
| Buyer MST inactive | TASK-SKILL-108 returns Inactive | `BuyerMstInactive` | Caller surfaces; user re-enters or confirms manually |
