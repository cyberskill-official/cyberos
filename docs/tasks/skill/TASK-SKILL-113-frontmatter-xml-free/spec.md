...
related_tasks: [TASK-SKILL-103, TASK-SKILL-111, TASK-SKILL-112, TASK-SKILL-114]
depends_on: [TASK-SKILL-103]
blocks: []
...
service: cyberos/services/skill-broker/  (TASK-SKILL-103 validator extension) + modules/skill/task-audit/  (RUBRIC update) + modules/skill/_template/  (template) + tools/migrate-wrap-in/  (sweep script)
new_files:
...
subtasks:
  ...
  - "0.5h: task-audit skill §3.13 entry — frontmatter-comment-hygiene gets a sibling rule frontmatter-string-form-only"
...
+    pub wrap_in_marker:  MarkerName, // v2 (v0.2.5) — string-form marker name; see TASK-SKILL-113
...
+        "description": "v1 marker namespace (TASK-SKILL-113). String-form only — XML brackets are rejected."
+      },
...
### Auditor rules — additions to `modules/skill/task-audit/RUBRIC.md`
...
**Statement:** SKILL.md frontmatter MUST carry `wrap_in_marker:` (renamed from legacy `wrap_in:` in registry v0.2.5; per TASK-SKILL-113 §1 #1). Value MUST match `^[a-z][a-z0-9_]*$` and MUST be one of the registered v1 markers (today: `"untrusted_content"` only).
...
```bash
#!/usr/bin/env bash
# tools/migrate-wrap-in/migrate.sh — TASK-SKILL-113 mechanical sweep.
...
case "$MODE" in
  ...
  --verify)
    ...
    missing=$(find "$SKILLS_DIR" -name 'SKILL.md' -path '*/task-author/*' -o -name 'SKILL.md' -path '*/task-audit/*' \
...
+  wrap_in_marker: "untrusted_content"   # TASK-SKILL-113 — registry v0.2.5
...
20. **README Part 2.1 row updated** — frontmatter table shows `wrap_in_marker:` with TASK-SKILL-113 cross-link.
21. **README Part 2.5 new sub-section added** — "What changed in registry v0.2.5" documenting the rename + rationale.
22. **task-audit skill §3.13 rule added** — frontmatter-string-form-only rule.
...
fn quoted_string_with_brackets_does_not_false_positive() {
    // YAML string-quoted `<` should NOT trigger (defensive — but per TASK-SKILL-113 §1 #1 we still reject)
...
```bash
# Added under modules/skill/task-audit/acceptance/
...
5. README Part 2.5 ("What changed in registry v0.2.5") is a 6-8 paragraph sub-section: motivation (1 para), what changed (2 para — field rename + auditor rule split), how to migrate (2 para — script + verify), future markers (1 para — pointer to TASK-SKILL-117+ namespace expansion).
...
description:     "Frontmatter uses legacy v1 form `wrap_in: <untrusted_content/>` which contains forbidden XML brackets. Per TASK-SKILL-113 §1 #1, rename to `wrap_in_marker: \"untrusted_content\"`."
...
| TASK-SKILL-103 broker doesn't validate `wrap_in_marker` because it's wired only for the old `wrap_in` field | New marker field passes schema but no enum check fires | Defensive auditor rule FM-116 catches | Update broker code + JSONSchema in lockstep |
...
| Partner connector receives a half-migrated bundle | Partner's loader rejects on the XML field at boot | Partner ships nothing; CyberOS team alerted | Tighten the OCI registry TASK-SKILL-102 gate to refuse uploads with legacy-form fields |
...
| JSONSchema mirror not synced with Rust types | CI agent runs both validators against fixtures; mismatch on any → CI fails | Build broken | Sync schema.rs ↔ skill.schema.json via `cargo xtask schema` (per TASK-SKILL-103 §11) |
