...
related_tasks: [TASK-SKILL-101, TASK-SKILL-102, TASK-SKILL-104, TASK-SKILL-105, TASK-SKILL-111, TASK-SKILL-112, TASK-SKILL-113, TASK-SKILL-114, TASK-SKILL-115, TASK-MEMORY-106, TASK-AUTH-003]
depends_on: [TASK-SKILL-101]
blocks: [TASK-SKILL-104, TASK-SKILL-105, TASK-SKILL-111, TASK-SKILL-112, TASK-SKILL-113, TASK-SKILL-114, TASK-SKILL-115]
...
**Why allowed_memory_scopes as globs (§1 #2)?** A skill that touches "all my projects" needs `memories/projects/**`; a skill that touches "only Cyberos" needs `memories/projects/cyberos/**`. Globs are the right shape — granular, declarative, well-understood. `globset` is the canonical Rust glob crate; reusing it ensures consistency with TASK-MEMORY-106's sync-class globs.

**Why allowed_tools as enum (§1 #2)?** If we accepted arbitrary strings, typos would creep in (`Bsah`, `memoryread`). Enum tokens are validated; the canonical set is small (~10 native tools + MCP names via TASK-SKILL-104). Adding a new tool means: enum variant + broker impl + version bump.

**Why signature optional (§1 #3)?** Local dev needs to iterate fast — adding/checking signature on every change kills the flow. The OCI gate (TASK-SKILL-102) enforces signature on registry-uploaded bundles; local dev runs unsigned. Two scopes: trust-local (loose) and trust-registry (strict).
...
pub enum ToolName {
    ...
    HttpFetch, HttpPost,
    // MCP tool names registered at broker startup per TASK-SKILL-104
...
pub fn validate(fm: &SkillFrontmatter, body_canonical: &str, broker_version: &str) -> Result<(), FrontmatterError> {
    ...
    for (i, t) in fm.allowed_tools.iter().enumerate() {
        if let ToolName::McpTool = t {
            ...
            // For simplicity in this skeleton we trust serde's pre-validation; TASK-SKILL-104 owns the registry check.
        }
    }
...
- **TASK-AUTH-003** — RLS for tenant_scope=pinned enforcement (broker injects tenant_id into memory reads).
...
- The signature verification reuses the ed25519 implementation from `ed25519-dalek` (already in workspace via TASK-OBS-009).
- `serde_yaml` allows duplicate keys silently in some versions; we pin to a version that errors on duplicates (≥ 0.9).
- The `McpTool` `#[serde(other)]` variant is a forward-compat trick — for v1 we accept any string and defer registry validation to TASK-SKILL-104. In v2 we may upgrade to a strict registry-time check.
- `effort_minutes` is an advisory cap; the actual SIGTERM enforcement lives in TASK-SKILL-104. Default cap (when unspecified) is 30 minutes.
