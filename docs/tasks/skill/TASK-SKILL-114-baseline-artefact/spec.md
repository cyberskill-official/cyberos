...
related_tasks: [TASK-SKILL-103, TASK-SKILL-111, TASK-SKILL-112, TASK-SKILL-113]
...
service: modules/skill/_template/  (scaffold) + modules/skill/task-audit/  (RUBRIC + audit rule) + modules/cuo/cuo/  (baseline analyser)
new_files:
...
allowed_tools:
  ...
  - file_write: modules/skill/{_template,task-audit}, modules/cuo/{cuo,tests}, docs/tasks/skill/**
...
subtasks:
  ...
  - "0.5h: task-audit/RUBRIC.md — add FM-114 baseline-present-at-v1 rule (severity: error on v1.0.0+; warning on v0.x; auto-fix: never — numbers must be measured)"
  - "0.5h: task-audit skill §3.11 documentation-discipline expansion — add promotion-readiness sub-rule"
...
**Verdict:** all three thresholds passed. Skill is v1.0-eligible per TASK-SKILL-114 §1 #6.

## Token-budget transparency

Per-invocation token budget for `task-author`:
...
### Auditor rule — addition to `modules/skill/task-audit/RUBRIC.md`

```markdown
### FM-114 — baseline-present-at-v1

**Statement:** Skills at `skill_version >= 1.0.0` MUST carry `BASELINE.md` per TASK-SKILL-114 §1. The artefact justifies the promotion + anchors future drift detection.
...
```python
"""Validate BASELINE.md artefacts per TASK-SKILL-114."""
...
**Depends on:**
- **TASK-SKILL-103** (frontmatter-extension) — provides the broker that gates partner_connector skills on BASELINE.md presence.

**Blocks:** none.

**Related:**
- **TASK-SKILL-111** (description trigger enrichment) — orthogonal; description is loaded by host classifier; baseline is read by promotion auditor.
- **TASK-SKILL-112** (TRIGGER_TESTS.md) — orthogonal; routing test vs promotion artefact.
- **TASK-SKILL-113** (XML-free frontmatter) — orthogonal; frontmatter shape vs new artefact.
...
description:     "Skills at v1.0+ MUST carry BASELINE.md per TASK-SKILL-114 §1. The artefact justifies the v0.x → v1.0 promotion."
...
```bash
$ python -m cyberos.cuo.baseline modules/skill/task-author/
✓ task-author — baseline valid (all 3 thresholds met)
...
| `next_review_due` set far in future (e.g. 2099) | Validator accepts (no upper bound) | Stale-by-default | task-audit skill §3.11 prescribes default +12 months |
