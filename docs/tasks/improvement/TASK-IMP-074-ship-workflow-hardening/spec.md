...
source_pages:
  ...
  - "modules/cuo/chief-technology-officer/workflows/ship-tasks.md (workflow_version 2.4.0; one-FR-at-a-time queue; no batch selection, no unlock rescan)"
...
1. Any commit whose staged changes touch `docs/tasks/**`, `CHANGELOG.md`, or `VERSION` **MUST** regenerate `docs/status/` (via the existing `migrate-frs.sh --page` fast path) and stage the result in the same commit, wired into `.githooks/pre-commit`. The status page can never again lag the FR frontmatter it renders.
...
1. Committing a change under `docs/tasks/` regenerates and stages `docs/status/` in that same commit (hook run observed; `git show --stat` includes docs/status/index.html alongside the FR change).
...
Upstream none; downstream: a follow-up FR may teach `cyberos update`/plugin/MCP clients to compare `rules_sha` (clause 10). Batched with TASK-IMP-075 (non-overlapping cones: workflow/hooks/CI-docs vs apps/desktop Rust) — the first sanctioned use of §11a.
...
| Unlock rescan picks an FR whose spec drifted since audit | ship-manifest fr_sha256 staleness rule (TASK-CUO-206) triggers restart at step 1 | no stale-spec shipping | existing manifest machinery |
