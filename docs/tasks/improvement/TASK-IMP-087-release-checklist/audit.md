...
ISSUE ISS-001 (TRACE-002, wontfix-info): ACs are ops-verified with recorded-grep
rationale (a single operator markdown document; suite out of scope by design). Sufficient.
ISSUE ISS-002 (QA-004, wontfix-info): the primary metric measures document shape, not
release outcome - correct on purpose: working the lines IS the release, operator-owned;
stated in Success Metrics.
