...
related_tasks: [TASK-IMP-068, TASK-IMP-070, TASK-PLUGIN-008, TASK-SKILL-201, TASK-APP-001]
depends_on: [TASK-IMP-068]
blocks: [TASK-IMP-070]
source_pages:
  - .github/workflows/release.yml
  - tools/install/README.md
  - tools/install/bootstrap.sh
  - tools/install/rollout.sh
...
service: .github/workflows/ + tools/install/
new_files:
...
1. A script `tools/install/release-assets.sh <payload-dir> <out-dir>` MUST produce, from a built payload: `cyberos-payload-<ver>.tar.gz` (deterministic: sorted paths, numeric owner 0:0, fixed mtime `2000-01-01T00:00:00Z`, gzip -n), `cyberos-<ver>.plugin` (byte-copy of `cyberos.plugin`), stable-name aliases `cyberos-payload.tar.gz` and `cyberos.plugin` (byte-identical copies, so `releases/latest/download/<stable-name>` always resolves), and `SHA256SUMS` covering all four. `<ver>` MUST be read from `<payload-dir>/VERSION`.
