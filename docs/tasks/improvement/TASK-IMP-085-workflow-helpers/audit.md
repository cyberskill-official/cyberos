...
ISSUE ISS-001 (TRACE-001, fixed, MACHINE-CAUGHT): task-lint flagged clause 1.9 (the suite
mandate) as uncited by any AC on the first run - the exact miss class the tool was built
for, caught in the tool-builder batch itself. AC 10 added (ops verification via run_all
glob discovery). Lint re-run: clean, exit 0.
ISSUE ISS-002 (QA-005, wontfix-info): the "one combined tool" alternative is thin by
design - the versioning-independence argument is the whole point; recorded as sufficient.
