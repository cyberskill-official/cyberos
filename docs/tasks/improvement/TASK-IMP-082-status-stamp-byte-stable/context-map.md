...
## Blast radius
file_count: 4 (1 renderer, 1 new suite, 2 peer-suite assertion updates) | module_count: 2 (tools/docs-site, scripts/tests) | cross_module_edges: scripts/tests suite -> tools/docs-site renderer + modules/templates fixtures; renderer -> none new (node:crypto is stdlib)
module_placement_warning: null (task declares service tools/docs-site; the suite location scripts/tests/test_render_stamp.sh is fixed by spec §1.8)
Behavioral radius: every status-page render everywhere (cyberos repo, hook flows, consumer repos via payload). The only observable change is the stamp VALUE on the three surfaces; layout, lenses, chunks, KPI math untouched (spec Out of scope). audit-fleet's pinned re-render is unaffected because the pin path is preserved verbatim.
