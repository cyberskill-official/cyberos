...
related_tasks: [TASK-EMAIL-001, TASK-EMAIL-008, TASK-AI-003, TASK-MCP-006, TASK-MEMORY-111]
depends_on: [TASK-EMAIL-001, TASK-AI-003]
...
subtasks:
  ...
  - "0.5h: integration with TASK-EMAIL-008"
...
8. **MUST** emit 5 memory audit kinds per DEC-1604. PII per TASK-MEMORY-111: variable values SHA-256 hashed; ids ok.

9. **MUST** thread trace_id through plan → extract → check → execute → audit.

10. **MUST** integrate with TASK-EMAIL-008 — Genie action_proposer wraps its TASK-AI-003 calls in `camel::execute(plan, email)` instead of direct calls.
