...
related_tasks: [TASK-EMAIL-001, TASK-EMAIL-003, TASK-EMAIL-004, TASK-AUTH-004, TASK-AUTH-101, TASK-AI-003, TASK-MEMORY-111]
depends_on: [TASK-EMAIL-001, TASK-AUTH-004]
blocks: []
...
source_decisions:
  ...
  - DEC-1461 2026-05-17 — JWT-as-password pattern: client supplies JWT as IMAP/SMTP password field; authbridge validates against TASK-AUTH-004 issuer; rejects expired/wrong-audience
  - "DEC-1462 2026-05-17 — Per-tenant mailbox naming: `<subject_id>@<tenant_slug>.cyberos.world`; per-tenant CNAME (TASK-PORTAL-002) MAY alias to vanity domain"
...
subtasks:
  ...
  - "0.6h: jwt_validator.rs (TASK-AUTH-004 JWKS fetch + cache)"
  - "0.5h: mailbox_resolver.rs (subject_id → mailbox path)"
  - "0.4h: cache.rs (Redis 60s TTL)"
  - "0.4h: revocation_consumer.rs (NATS from TASK-PORTAL-004)"
...
5. **MUST** subscribe to TASK-PORTAL-004 SCIM revoke NATS events per DEC-1465. Consumer invalidates Redis cache for subject + emits `email.auth_failed` reason='subject_revoked' for next attempt.
...
7. **MUST** emit 4 memory audit kinds per DEC-1466. PII-scrub source_ip via TASK-MEMORY-111.
