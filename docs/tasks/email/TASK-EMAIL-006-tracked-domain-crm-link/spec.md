...
related_tasks: [TASK-EMAIL-001, TASK-CRM-001, TASK-AI-003, TASK-MEMORY-111]
depends_on: [TASK-EMAIL-001, TASK-CRM-001]
blocks: [TASK-CRM-002]
...
source_decisions:
  - DEC-1570 2026-05-17 — Auto-link triggered ONLY on inbound message from sender domain matching tracked_domains list per tenant; outbound not linked here (TASK-EMAIL-009 sets crm_id at send)
...
8. **MUST** add `link_origin` column to TASK-EMAIL-001 messages table:
   ```sql
...
9. **MUST** emit 3 memory audit kinds per DEC-1575. PII per TASK-MEMORY-111: contact email/name SHA-256 hashed; domain (already public) ok.

10. **MUST** thread trace_id from inbound processor → matcher → CRM upsert → audit.

11. **MUST NOT** link untracked domains per DEC-1572.

12. **MUST NOT** link outbound (handled by TASK-EMAIL-009 at send-intent) per DEC-1570.
...
- §11.3 TASK-AI-003 prompt: "What company owns the domain {domain}? Reply with name only, no commentary."
