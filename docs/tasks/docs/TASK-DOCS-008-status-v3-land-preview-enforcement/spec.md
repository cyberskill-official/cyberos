---
id: TASK-DOCS-008
title: "Land status-v3 preview + enforcement trio on main behind PR"
...
verify: T
phase: Status-v3 Phase 0
owner: Stephen Cheng (CTO)
created: 2026-07-27
shipped: null
memory_chain_hash: null
related_tasks: [TASK-DOCS-009, TASK-DOCS-010]
depends_on: []
blocks: [TASK-DOCS-009, TASK-DOCS-010]
source_pages:
...
new_files:
  - docs/status-v3-preview/
  - scripts/check_task_link.sh
...
# TASK-DOCS-008: Land status-v3 preview + enforcement trio on main behind PR

## Summary

Deliver plan item T1 (Phase 0) from `docs/plans/PLAN-status-v3-platform-2026-07-27/plan.md`: P0 gate: preview folder, check_task_link.sh, commit-msg hook, and traceability.yml committed on feat/status-v3-platform with RULE-compliant messages; suite green on scripts touch.
...
Execute Phase 0 for T1 exactly as specified in the plan, preserving non-negotiables: THE RULE, no history rewrite, determinism, offline-first, consumer sovereignty. Every commit cites `TASK-DOCS-008`.
...
- Primary: acceptance core is true — P0 gate: preview folder, check_task_link.sh, commit-msg hook, and traceability.yml committed on feat/status-v3-platform with RULE-compliant messages; suite green on scripts touch.
- Guardrail: no hand-edited derived payload copies; pair/parity and payload hooks remain the mirror path.

## Scope

In: plan §7 T1 and its Phase 0 steps.
...
Upstream: none.
Downstream: TASK-DOCS-009, TASK-DOCS-010.
