---
id: TASK-DOCS-018
title: "deploy.yml trigger widening + served-page freshness check"
...
# TASK-DOCS-018: deploy.yml trigger widening + served-page freshness check

## Summary

Deliver plan item T11 (Phase 3) from `docs/plans/PLAN-status-v3-platform-2026-07-27/plan.md`: post-deploy curl asserts v3 markup + fp.
...
Execute Phase 3 for T11 exactly as specified in the plan, preserving non-negotiables: THE RULE, no history rewrite, determinism, offline-first, consumer sovereignty. Every commit cites `TASK-DOCS-018`.
...
- Primary: acceptance core is true — post-deploy curl asserts v3 markup + fp.
- Guardrail: no hand-edited derived payload copies; pair/parity and payload hooks remain the mirror path.

## Scope

In: plan §7 T11 and its Phase 3 steps.
