---
id: TASK-DOCS-011
title: "Epoch/shorthand/ledger commit classification in generator"
...
# TASK-DOCS-011: Epoch/shorthand/ledger commit classification in generator

## Summary

Deliver plan item T4 (Phase 1) from `docs/plans/PLAN-status-v3-platform-2026-07-27/plan.md`: fixtures green for epoch duplicates, shorthand/lists, ledger precedence, exempts; ~68% coverage reproduced on mothership.
...
Execute Phase 1 for T4 exactly as specified in the plan, preserving non-negotiables: THE RULE, no history rewrite, determinism, offline-first, consumer sovereignty. Every commit cites `TASK-DOCS-011`.
...
- Primary: acceptance core is true — fixtures green for epoch duplicates, shorthand/lists, ledger precedence, exempts; ~68% coverage reproduced on mothership.
- Guardrail: no hand-edited derived payload copies; pair/parity and payload hooks remain the mirror path.

## Scope

In: plan §7 T4 and its Phase 1 steps.
