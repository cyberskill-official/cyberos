---
id: TASK-DOCS-014
title: "Legacy hash redirect map + noscript regeneration from feed"
...
# TASK-DOCS-014: Legacy hash redirect map + noscript regeneration from feed

## Summary

Deliver plan item T7 (Phase 2) from `docs/plans/PLAN-status-v3-platform-2026-07-27/plan.md`: new DOM assertions for legacy hashes and noscript table from feed.
...
Execute Phase 2 for T7 exactly as specified in the plan, preserving non-negotiables: THE RULE, no history rewrite, determinism, offline-first, consumer sovereignty. Every commit cites `TASK-DOCS-014`.
...
- Primary: acceptance core is true — new DOM assertions for legacy hashes and noscript table from feed.
- Guardrail: no hand-edited derived payload copies; pair/parity and payload hooks remain the mirror path.

## Scope

In: plan §7 T7 and its Phase 2 steps.
