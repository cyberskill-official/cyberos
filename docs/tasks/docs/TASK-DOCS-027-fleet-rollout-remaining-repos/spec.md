---
id: TASK-DOCS-027
title: "Fleet rollout remaining repos + exception handling"
...
related_tasks: [TASK-DOCS-026, TASK-DOCS-029]
depends_on: [TASK-DOCS-026]
blocks: [TASK-DOCS-029]
source_pages:
...
# TASK-DOCS-027: Fleet rollout remaining repos + exception handling

## Summary

Deliver plan item T20 (Phase 7) from `docs/plans/PLAN-status-v3-platform-2026-07-27/plan.md`: P7 gate report with per-repo rows; exceptions reverted.
...
Execute Phase 7 for T20 exactly as specified in the plan, preserving non-negotiables: THE RULE, no history rewrite, determinism, offline-first, consumer sovereignty. Every commit cites `TASK-DOCS-027`.
...
- Primary: acceptance core is true — P7 gate report with per-repo rows; exceptions reverted.
- Guardrail: no hand-edited derived payload copies; pair/parity and payload hooks remain the mirror path.

## Scope

In: plan §7 T20 and its Phase 7 steps.
...
Upstream: TASK-DOCS-026.
Downstream: TASK-DOCS-029.
