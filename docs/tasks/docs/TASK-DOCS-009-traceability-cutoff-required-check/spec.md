---
id: TASK-DOCS-009
title: "Traceability required check + cutoff registry in config.yaml"
...
verify: T
phase: Status-v3 Phase 0/4
owner: Stephen Cheng (CTO)
created: 2026-07-27
shipped: null
memory_chain_hash: null
related_tasks: [TASK-DOCS-008, TASK-DOCS-019, TASK-DOCS-020]
depends_on: [TASK-DOCS-008]
blocks: [TASK-DOCS-019, TASK-DOCS-020]
source_pages:
...
# TASK-DOCS-009: Traceability required check + cutoff registry in config.yaml

## Summary

Deliver plan item T2 (Phase 0/4) from `docs/plans/PLAN-status-v3-platform-2026-07-27/plan.md`: strict mode blocks unlinked commit in scratch repo; cutoff resolves env -> config.yaml -> install-time default.
...
Execute Phase 0/4 for T2 exactly as specified in the plan, preserving non-negotiables: THE RULE, no history rewrite, determinism, offline-first, consumer sovereignty. Every commit cites `TASK-DOCS-009`.
...
- Primary: acceptance core is true — strict mode blocks unlinked commit in scratch repo; cutoff resolves env -> config.yaml -> install-time default.
- Guardrail: no hand-edited derived payload copies; pair/parity and payload hooks remain the mirror path.

## Scope

In: plan §7 T2 and its Phase 0/4 steps.
...
Downstream: TASK-DOCS-019, TASK-DOCS-020.
