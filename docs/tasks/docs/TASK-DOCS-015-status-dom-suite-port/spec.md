---
id: TASK-DOCS-015
title: "Port v3 DOM suite to tools/docs-site/tests with jsdom"
...
related_tasks: [TASK-DOCS-013, TASK-DOCS-016]
depends_on: [TASK-DOCS-013]
blocks: [TASK-DOCS-016]
source_pages:
...
# TASK-DOCS-015: Port v3 DOM suite to tools/docs-site/tests with jsdom

## Summary

Deliver plan item T8 (Phase 2) from `docs/plans/PLAN-status-v3-platform-2026-07-27/plan.md`: 47+ assertions in run_all against emitted page; jsdom is docs-site test-only dep.
...
Execute Phase 2 for T8 exactly as specified in the plan, preserving non-negotiables: THE RULE, no history rewrite, determinism, offline-first, consumer sovereignty. Every commit cites `TASK-DOCS-015`.
...
- Primary: acceptance core is true — 47+ assertions in run_all against emitted page; jsdom is docs-site test-only dep.
- Guardrail: no hand-edited derived payload copies; pair/parity and payload hooks remain the mirror path.

## Scope

In: plan §7 T8 and its Phase 2 steps.
...
Upstream: TASK-DOCS-013.
Downstream: TASK-DOCS-016.
