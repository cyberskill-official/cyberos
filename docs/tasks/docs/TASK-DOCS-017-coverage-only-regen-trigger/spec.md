---
id: TASK-DOCS-017
title: "Wide regen trigger + --coverage-only fast path"
...
verify: T
phase: Status-v3 Phase 3
owner: Stephen Cheng (CTO)
created: 2026-07-27
shipped: null
memory_chain_hash: null
related_tasks: [TASK-DOCS-013]
...
# TASK-DOCS-017: Wide regen trigger + --coverage-only fast path

## Summary

Deliver plan item T10 (Phase 3) from `docs/plans/PLAN-status-v3-platform-2026-07-27/plan.md`: P3 scenarios pass; code-only path <1s; loud-warn-never-block preserved.
...
Execute Phase 3 for T10 exactly as specified in the plan, preserving non-negotiables: THE RULE, no history rewrite, determinism, offline-first, consumer sovereignty. Every commit cites `TASK-DOCS-017`.
...
- Primary: acceptance core is true — P3 scenarios pass; code-only path <1s; loud-warn-never-block preserved.
- Guardrail: no hand-edited derived payload copies; pair/parity and payload hooks remain the mirror path.

## Scope

In: plan §7 T10 and its Phase 3 steps.
