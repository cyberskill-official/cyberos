---
id: TASK-DOCS-012
title: "Ledger relocation to docs/tasks/_state + task-lint validation"
...
related_tasks: [TASK-DOCS-010, TASK-DOCS-021]
depends_on: [TASK-DOCS-010]
blocks: [TASK-DOCS-021]
source_pages:
...
# TASK-DOCS-012: Ledger relocation to docs/tasks/_state + task-lint validation

## Summary

Deliver plan item T5 (Phase 1) from `docs/plans/PLAN-status-v3-platform-2026-07-27/plan.md`: unknown-task ledger entry fails lint; unknown hash prefix warns.
...
Execute Phase 1 for T5 exactly as specified in the plan, preserving non-negotiables: THE RULE, no history rewrite, determinism, offline-first, consumer sovereignty. Every commit cites `TASK-DOCS-012`.
...
- Primary: acceptance core is true — unknown-task ledger entry fails lint; unknown hash prefix warns.
- Guardrail: no hand-edited derived payload copies; pair/parity and payload hooks remain the mirror path.

## Scope

In: plan §7 T5 and its Phase 1 steps.
