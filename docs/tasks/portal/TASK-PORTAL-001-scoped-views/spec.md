...
related_tasks: [TASK-PORTAL-002, TASK-PORTAL-003, TASK-PORTAL-004, TASK-PORTAL-005, TASK-PORTAL-006, TASK-PORTAL-008, TASK-TEN-101, TASK-AUTH-004, TASK-AUTH-101, TASK-PROJ-001, TASK-INV-001, TASK-DOC-001, TASK-CHAT-005, TASK-AI-003, TASK-MEMORY-111, TASK-MEMORY-106, TASK-OBS-005]
depends_on: [TASK-TEN-101]
blocks: [TASK-PORTAL-007, TASK-PORTAL-008]
...
source_decisions:
  ...
  - DEC-1208 2026-05-17 — All view-read events emit `portal.view_read` audit row at sev-3 (sampled at 1% via TASK-OBS-006); detail-row reads at sev-2 (always emitted — caller saw specific resource)
  ...
  - DEC-1213 2026-05-17 — Exported view (CSV/Excel) via `GET /v1/portal/views/{view_kind}/export?format=csv|xlsx` per TASK-PORTAL-008-derivative streaming; ≤ 10k rows per export; over-limit → use DSAR
...
17. **MUST** thread trace_id end-to-end per task-audit skill rule 22-24.

18. **MUST** stream exports in chunked-transfer encoding per §1 #7. Memory footprint bounded — no full-result-set buffering.

19. **MUST** enforce read-only — view endpoints never accept POST/PUT/PATCH/DELETE (returns 405). Writes route through TASK-PORTAL-006 workflows.

20. **MUST** validate `sync_class` filter is enforced at the SQL view level per DEC-1202 + TASK-MEMORY-106. Handler does NOT trust client-supplied sync_class filter; the view's predicate is the gate.
...
23. **MUST NOT** return rows with `sync_class IN ('private','team-internal')` per TASK-MEMORY-106. View definition enforces; handler-side check defense-in-depth.
...
**Upstream (depends_on):**
- **TASK-TEN-101** Self-serve signup — tenant + Engagement exists before views.

**Cross-module (related_tasks):**
- **TASK-PORTAL-002** Brand pack — view UI inherits brand.
- **TASK-PORTAL-003** External IdP — caller_subject_id from IdP-auth JWT.
- **TASK-PORTAL-004** SCIM deprovision — revoked sessions return 401.
- **TASK-PORTAL-005** Branded Genie — uses same scope_grants pattern.
- **TASK-PORTAL-006** Client-initiated workflows — write counterpart to read views.
- **TASK-PORTAL-008** DSAR — full data export when > 10k rows needed.
- **TASK-PROJ-001** Projects — source schema; sync_class filter joins here.
- **TASK-INV-001** Invoices — source schema.
- **TASK-DOC-001** Documents — source schema.
- **TASK-CHAT-005** Channels — source schema; messages too.
- **TASK-AUTH-101** RBAC — engagement_memberships table consumed.
- **TASK-MEMORY-106** sync_class enforcement — view definitions consume.
- **TASK-AI-003** memory audit — 6 new kinds.
- **TASK-MEMORY-111** PII scrubbing — filter hash only in chain.

**Downstream (blocks):**
- **TASK-PORTAL-007** PWA — needs views to display.
- **TASK-PORTAL-008** DSAR — references view shape for export.
...
| Export > 10k rows | count pre-check | 413 + `export_too_large; use_dsar` | DSAR via TASK-PORTAL-008 |
