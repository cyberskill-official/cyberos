...
related_tasks: [TASK-PORTAL-003, TASK-PORTAL-001, TASK-PORTAL-005, TASK-AUTH-002, TASK-AUTH-004, TASK-AUTH-101, TASK-TEN-104, TASK-CHAT-005, TASK-AI-003, TASK-MEMORY-111, TASK-OBS-007]
depends_on: [TASK-PORTAL-003]
blocks: []
...
source_decisions:
  ...
  - DEC-1086 2026-05-17 — Per-Engagement isolation: SCIM DELETE invalidates ONLY sessions scoped to that Engagement (per TASK-PORTAL-003); user with multi-Engagement membership keeps sessions in other Engagements
  ...
  - DEC-1090 2026-05-17 — Audit-row PII: subject_id retained in chain (anti-correlation needed for forensic); email PII-scrubbed via TASK-MEMORY-111
...
19. **MUST** thread W3C `traceparent` across SCIM DELETE → sync phase → async phase → cascade targets → reconciliation (task-audit skill rule 22-24). Single trace_id per deprovision operation.
...
**Upstream (depends_on):**
- **TASK-PORTAL-003** External IdP + SCIM JIT — SCIM Users endpoint extended here for DELETE.

**Cross-module (related_tasks):**
- **TASK-PORTAL-001** Scoped views — cascade target.
- **TASK-PORTAL-005** Branded Genie chat — cascade target.
- **TASK-AUTH-002** Subject create — subject tombstone semantics.
- **TASK-AUTH-004** JWT mint + validate — blacklist consulted at validate.
- **TASK-AUTH-101** RBAC — role gates.
- **TASK-TEN-104** Lifecycle — deprovision may trigger if last admin lost.
- **TASK-CHAT-005** CHAT membership — cascade target.
- **TASK-AI-003** memory audit-row bridge — 7 new kinds.
- **TASK-MEMORY-111** PII scrubbing — email hash in chain.
- **TASK-OBS-007** Auto-runbook — sev-1 SLA-breach + orphan-session alerts route to CHAT.
...
**§11.10** Per-Engagement JWT scoping: each JWT carries `engagement_id` claim (TASK-PORTAL-003 derivative); blacklist scoped to `(tenant_id, engagement_id)` partition.
