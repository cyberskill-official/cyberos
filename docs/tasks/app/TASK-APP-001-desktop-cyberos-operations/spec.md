---
id: TASK-APP-001
title: Desktop CyberOS operations - build payload, init/update projects from the UI
...
4. The Tauri commands MUST validate that the configured checkout contains `tools/install/build.sh` (and the payload `dist/cyberos/init.sh` for check/init) before running, returning a structured error otherwise.
