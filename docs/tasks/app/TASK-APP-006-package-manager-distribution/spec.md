...
5. Neither manifest **MUST** be submitted (i.e. no PR opened against `homebrew/homebrew-cask` or `microsoft/winget-pkgs`) as a direct or automatic consequence of this FR landing — submission is a distinct, later, Stephen-gated action per §1's `disallowed_tools`, exactly mirroring the TASK-APP-005 Flathub-PR gate.
...
- **Downstream:** none currently drafted.
  ...
  - No such FR is drafted, scheduled, or otherwise planned as of this writing — the dependency is noted here purely so a future author knows to add `depends_on: [TASK-APP-006]` rather than discovering the relationship after the fact.
- **Cross-module:** none.
...
- **`related_tasks` references this FR as the last item in the same batch as TASK-APP-003/004/005, all now landed** — unlike TASK-APP-003/004/005's own §9 forward-reference notes, this FR is authored last in the batch, so all its `related_tasks` entries resolve to already-existing files; no forward-reference disclosure is needed here, and this is noted for symmetry with the prior three FRs' §9 sections rather than because a gap exists.
