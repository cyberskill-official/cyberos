...
The original §1 #2 read as though TASK-APP-003 and TASK-APP-004 "produced" the `.dmg`/NSIS `.exe` this FR's manifests point at — in fact both installers are produced by the pre-existing, always-on `desktop` job in `release.yml` (confirmed lines 69–139, no opt-in gate), which predates and runs independently of TASK-APP-003's/TASK-APP-004's own additionally-gated MAS `.pkg`/MSIX artifacts. Left uncorrected, a reader could wrongly conclude this FR depends on TASK-APP-003/004's gating flags (`MAS_RELEASE`, `MSSTORE_RELEASE`) landing or being enabled, which it does not. Resolved: §1 #2 now names the `desktop` job explicitly and states this FR's manifests have no dependency on either sibling FR's own artifacts or flags; §7's "Upstream" line was tightened to match; §1, §7.
