...
related_tasks: [TASK-PROJ-002, TASK-PROJ-003, TASK-PROJ-004, TASK-PROJ-017, TASK-PROJ-018]
depends_on: [TASK-PROJ-002]
blocks: [TASK-PROJ-018]

source_pages:
  - website/docs/modules/proj.html#kanban
source_decisions:
  - DEC-350 (drag-to-column triggers TASK-PROJ-004 transition; illegal transitions snap back)
...
subtasks:
  ...
  - "1.0h: optimistic UI update + reconcile on TASK-PROJ-002 confirmation"
...
15. **MUST** support bulk operations via multi-select: Shift-click selects range; Cmd/Ctrl-click toggles; bulk drag moves all selected. Bulk transitions emit one memory row per issue (per TASK-PROJ-002 §1 #16).
...
- **TASK-PROJ-004** — status FSM + transition endpoint.
- **TASK-PROJ-017 (sibling)** — Brief Modal opened on Enter.
- **TASK-PROJ-018 (downstream)** — design tokens + a11y CI.
...
- Color tokens for column headers come from TASK-PROJ-018.
