...
related_tasks: [TASK-PROJ-001, TASK-AI-003, TASK-MEMORY-106, TASK-MEMORY-108]
depends_on: [TASK-PROJ-001, TASK-AI-003]
blocks: [TASK-PROJ-003, TASK-PROJ-004, TASK-PROJ-013, TASK-PROJ-014, TASK-PROJ-015, TASK-PROJ-016, TASK-TIME-004]

source_pages:
  - website/docs/modules/proj.html#memory-anchoring
source_decisions:
  - DEC-215 (audit-before-action; memory failure rolls back DB mutation)
...
subtasks:
  ...
  - "0.5h: memory row builder for proj.decision kind"
  - "0.5h: OTel metrics emission"
  - "1.5h: Tests — emit + reason + prior_chain + ACL + cross-module links + rollback on memory failure"
  - "0.5h: memory search integration (TASK-MEMORY-108) — query decisions by issue_id"
risk_if_skipped: "Issue state changes are silent — chain has no per-decision rationale. Audit reviews can't answer 'why was this done done?'. Without prior_chain, per-issue history requires expensive join. Without audit-before-action, partial states (DB updated but memory not) corrupt the chain."
---

## §1 — Description (BCP-14 normative)

Every PROJ Issue state change **MUST** be a memory-anchored decision, not just a DB row update. The integration:

1. **MUST** emit memory row `proj.decision` per Issue status transition (TASK-PROJ-001 §1 #6's `proj.issue_status_changed` is REPLACED by this richer row). Payload: `issue_id`, `from_status`, `to_status`, `reason`, `decided_by_subject_id`, `prior_decision_chain`, `cross_module_links`, `tenant_id`, `request_id`.
...
4. **MUST** be referenceable from CHAT via `#issue/<id>` syntax — chat module's link expander (TASK-CHAT-007) follows the link to the memory decision row.
5. **MUST** include `prior_decision_chain` field referencing the chain hash of the previous `proj.decision` for the SAME issue. First transition → `null`. Subsequent transitions form a per-issue chain queryable as "show all decisions on issue X" via TASK-MEMORY-108.
6. **MUST** apply audit-before-action invariant: the entire status change (DB UPDATE + memory row emit) runs in a single Postgres transaction; memory emit failure rolls back the DB UPDATE. Partial states forbidden.
...
9. **MUST** be queryable via TASK-MEMORY-108 — searching for `kind:proj.decision AND issue_id:<id>` returns the chronological per-issue chain.
...
16. **MUST** treat a bulk-status-change (multiple issues PATCHed in one request) as ONE memory row per issue, NOT one aggregate row. The audit trail must be per-issue queryable.
...
19. **MUST** index `proj.decision` rows by `(tenant_id, issue_id, ts_ns)` in the memory search index (TASK-MEMORY-108). The per-issue chain query MUST return in O(log n) time, not O(n) scan.
...
23. **MUST** track `decision_session_id`: a single user PATCH that changes status + reason + assignee in one operation generates one memory row for the status change but they share a session_id with other ops (e.g. comment add). Allows reconstructing "what happened in this PATCH request" across multiple audit rows.
...
**Why decision-not-just-update (DEC-215)?** A status change in DB is silent — no rationale captured, no chain entry. As a "decision" in memory, it's queryable, auditable, search-indexable, AND linked into the per-issue history. The decision IS the truth; the DB row is the projection.

**Why audit-before-action (DEC-215)?** Partial states (DB updated but memory not) corrupt the audit chain — the issue shows status=done but no decision row explains it. Single transaction with memory-emit-blocks-commit eliminates the partial state.
...
**Why max 500-char reason (§1 #3)?** Reasons are rationale, not essays. Long-form decision documents go in memory as separate `decisions/` files. 500 chars enforces conciseness.
...
**Why 100ms emit budget (§1 #11)?** Status changes are interactive (operator clicks "Done"). 100ms is invisible; longer feels sluggish. Budget covers DB UPDATE + memory write + chain fetch.

**Why TASK-MEMORY-108 query integration (§1 #9)?** Decisions become first-class searchable artefacts. "Show me all decisions for engagement X" or "decisions by subject Y last quarter" — answerable directly.

**Why mandatory PII redaction (§1 #13)?** Reason text is operator-typed; it may include client names, emails, phone numbers. memory audit retention is years; storing unredacted = years of accumulated PII risk. Redact at write-time, not at read-time, so the persisted form is always safe.
...
**Why chain_anchor_in_payload (§1 #22)?** Audit row self-verification: a recipient with the payload alone can recompute + compare. Without it, recipient needs access to memory's chain index.
...
pub async fn emit_decision_in_tx(
    ...
    // §1 #6 audit-before-action — emit_in_tx blocks on memory write
    memory_writer::emit_in_tx(tx, row).await
        .map_err(|e| DecisionError::MemoryEmitFailed(e.to_string()))?;
...
pub async fn fetch_prior_chain(tx: &mut PgTransaction<'_>, issue_id: Uuid) -> Result<Option<String>, DecisionError> {
    let row: Option<(Vec<u8>,)> = sqlx::query_as(
        "SELECT chain FROM memory_outbox
...
pub async fn emit_retraction(
    ...
    let original = sqlx::query!(
        "SELECT payload, chain FROM memory_outbox
         WHERE kind = 'proj.decision'
...
-- Index on the memory outbox for (tenant, issue, ts) per §1 #19.
CREATE INDEX IF NOT EXISTS idx_memory_outbox_proj_decision
    ON memory_outbox ((payload_json::jsonb->>'tenant_id'),
...
1. PATCH issue status → memory row `proj.decision` emitted with full payload.
...
7. Audit-before-action: memory emit failure → DB UPDATE rolled back; status NOT changed.
8. Default `sync_class: shareable`, `acl: []`.
9. Tenant-admin can override ACL to restrict (e.g., `acl: ["@hr-team"]`) per-decision.
10. CHAT @lumi can search decisions via TASK-MEMORY-108: `kind:proj.decision AND issue_id:<id>`.
...
21. **Bulk PATCH = one row per issue** — PATCH `/v1/proj/issues?ids=a,b,c` with same status → 3 memory rows, one per issue (AC for §1 #16).
...
29. **decision_session_id correlates ops** — multi-mutation PATCH produces multiple memory rows sharing `session_id` (AC for §1 #23).
...
async fn status_change_emits_decision_row() {
    ...
    let row = memory_test_helper::find_latest("proj.decision").unwrap();
...
async fn prior_decision_chain_links() {
    ...
    let first = memory_test_helper::find_latest("proj.decision").unwrap();
    ...
    let second = memory_test_helper::find_latest("proj.decision").unwrap();
...
#[tokio::test]
async fn memory_emit_failure_rolls_back_status() {
    let (pool, claims) = test_setup_with_engagement().await;
    let issue = create_test_issue(&pool, &claims, IssueStatus::Triage).await;
    ...
    }, None, &claims.claims, &pool, "req").await.expect_err("expected MemoryEmitFailed");
    assert!(matches!(err, IssueError::Decision(DecisionError::MemoryEmitFailed(_))));
...
async fn no_decision_on_no_status_change() {
    ...
    let count = memory_test_helper::count_rows_since("proj.decision", recent()).await;
...
async fn search_decisions_by_issue() {
    ...
    let search_results = memory_search::search(&format!("kind:proj.decision AND issue_id:{}", issue.id), claims.tenant_id).await.unwrap();
...
async fn ac19_retraction_emits_separate_row() {
    ...
    let issue = create_test_issue(&pool, &claims, IssueStatus::Triage).await;
    patch_status(&issue, IssueStatus::Todo, &claims, &pool).await.unwrap();
    let original = memory_test_helper::find_latest("proj.decision").unwrap();
    ...
    let retraction = memory_test_helper::find_latest("proj.decision_retracted").unwrap();
    ...
    let original_again = memory_test_helper::find_by_decision_id(decision_id).unwrap();
...
async fn ac21_bulk_patch_one_row_per_issue() {
    ...
    let rows = memory_test_helper::find_all_since("proj.decision", recent()).await;
...
- **TASK-PROJ-001** — Issue schema + status FSM.
- **TASK-AI-003** — memory_writer for audit row.
- **TASK-MEMORY-106** — sync_class enforcement at sync boundary.
- **TASK-MEMORY-108** — search-by-kind queryable.
...
```http
GET /v1/memory/search?q=kind:proj.decision%20AND%20issue_id:issue-... HTTP/1.1
→ 200 OK
{
  "items": [
...
|---|---|---|---|
| memory unreachable | memory_writer error | tx rollback; 503 audit-before-action | Operator investigates memory |
...
| Concurrent PATCH races | optimistic lock at TASK-PROJ-001 | First wins; second 412 | By design |
| Decision search by issue returns out of order | memory search not chain-aware | Caller sorts by ts_ns | By design |
...
| memory slow (>100ms emit) | OTel histogram | Sev-3 alarm | Investigate memory_writer |
| Concurrent decisions for same issue | independent emit | Both succeed; chain may interleave | By design |
| sync_class not respected at sync | TASK-MEMORY-106 enforces | Filtered at sync boundary | By design |
...
- 500-char reason limit enforces conciseness; long-form rationales go in memory as separate decisions/ files.
...
- The PII redactor for reason text reuses the TASK-MEMORY-111 ruleset; updates to the ruleset apply to PROJ without code changes.
- We chose UUIDv7 for decision_id (not v4) so per-issue history queries can sort by decision_id directly (v7 is time-ordered).
- Retraction as a separate row was chosen over mutation because: (a) audit-chain integrity is non-negotiable; (b) operators expect "the original decision exists forever; supersession is a new fact"; (c) it parallels TASK-CHAT-006's `chat.message_edited` pattern.
...
- Immutability is enforced via Postgres GRANT/REVOKE (cyberos_app has SELECT + INSERT only on memory_outbox), not just handler code. Defense in depth.
- The required-reason policy is per-status, not per-tenant boolean, so different statuses can have different requirements.
- chain_anchor_in_payload lets recipients verify a single decision without crawling the whole chain — useful for embedding decisions in DSAR exports (TASK-CHAT-012 sibling pattern).
- decision_session_id is generated client-side (or by the orchestrator TASK-CUO-101) and propagated; the proj handler trusts it.
...
- The 100ms p95 emit budget covers DB UPDATE + prior_chain fetch + PII redact + memory row write + chain commit. Each is ~20ms typical.
- We considered streaming WAL changes to memory (like TASK-CHAT-005's bridge pattern) for decisions but rejected: decisions are interactive (operator clicks); synchronous emit gives immediate feedback.

---

*End of TASK-PROJ-002. Status: done (implemented 2026-05-23).*
