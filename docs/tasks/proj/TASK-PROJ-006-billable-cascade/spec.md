...
related_tasks: [TASK-PROJ-001, TASK-PROJ-005, TASK-PROJ-007, TASK-TIME-005, TASK-AUTH-003]
depends_on: [TASK-PROJ-005]
blocks: [TASK-PROJ-007, TASK-TIME-005]
...
subtasks:
  ...
  - "0.5h: snapshot embedded in time entry row (already in TASK-TIME-005)"
...
- **TASK-PROJ-005** — rate card lookup at Tier 3.
- **TASK-PROJ-007 (downstream)** — billing modes consume resolved billable flag.
- **TASK-TIME-005** — time-entry snapshot field stores this resolution.
- **TASK-AUTH-003** — RLS.
...
- The `billable_snapshot` field on time entry is the bill-truth; cascading downstream (TASK-PROJ-007 billing modes) trusts it without re-resolving.
