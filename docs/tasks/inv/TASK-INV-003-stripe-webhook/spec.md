...
related_tasks: [TASK-AUTH-101, TASK-AI-003, TASK-MEMORY-101, TASK-INV-005, TASK-INV-006, TASK-TEN-003, TASK-OBS-007]
depends_on: [TASK-AUTH-101]
blocks: [TASK-INV-006, TASK-TEN-003]
...
source_decisions:
  ...
  - DEC-466 (multi-currency support — Stripe sends amount + currency; stored as BIGINT minor + CHAR(3) currency per task-audit skill rule 11; supports USD, EUR, SGD, GBP at slice 1)
...
1. **MUST** extend the `payment_receipts` table (from TASK-INV-005) — `receipt_source` enum already includes `'stripe'`; no schema change here. New rows from Stripe handler use `receipt_source='stripe'` and populate `bank_code='STRIPE'` (Stripe is added as a new value to bank_code enum via a small migration ALTER TYPE).
...
5. **MUST** be **append-only** on `stripe_event_log` at SQL grant (per DEC-463 + task-audit skill rule 12). `REVOKE UPDATE, DELETE ON stripe_event_log FROM cyberos_app`.
...
13. **MUST** PII-scrub `customer.email`, `customer.name`, `description` via TASK-MEMORY-111 BEFORE chain commit. Raw values retained in tenant Postgres rows; memory chain holds hashed forms (`customer_email_hash16`, `customer_name_hash16`).
...
25. **MUST** populate `payment_receipts.invoice_id` from `event.data.object.metadata.invoice_id` (per DEC-472) when present + the invoice exists in tenant scope. Missing metadata or invoice not found → `invoice_id=NULL`; TASK-INV-006 cash app handles matching async.
...
-- Extend bank_code enum to include STRIPE (was missing from TASK-INV-005's 15 VN banks)
...
async fn handle_payment_succeeded(
    ...
    // (2) Insert payment_receipts row (mirrors TASK-INV-005 shape)
...
async fn handle_invoice_finalized(state: &crate::AppState, tenant_id: Uuid, evt: &StripeEvent, payload_sha: &str) -> anyhow::Result<i64> {
    // INSERT log row only; TASK-TEN-003 consumes for subscription tier transitions
    todo!()
}
...
**Upstream:**
- **TASK-AUTH-101** — CFO role for secret rotation.

**Downstream (2 placeholders):**
- **TASK-INV-006** — cash application reads payment_receipts.
- **TASK-TEN-003** — Stripe billing integration consumes the event log for subscription state.

**Cross-module:**
- **TASK-INV-001** — invoices table FK target for `invoice_id`.
- **TASK-INV-005** — shares the `payment_receipts` schema + `receipt_source` enum.
- **TASK-AI-003** — memory audit bridge.
- **TASK-MEMORY-111** — PII scrubbing.
- **TASK-OBS-007** — sev-2 alarm rule.
...
| Invoice metadata missing | optional field | invoice_id=NULL | TASK-INV-006 cash app |
...
| Refund without original payment_intent | log + emit | Manual reconciliation | TASK-INV-006 |
...
| Webhook arrives before payment_intent (race) | metadata.invoice_id lookup may fail | invoice_id=NULL | TASK-INV-006 |
...
- **Subscription events log only at slice 1** — TASK-TEN-003 consumes for state.
