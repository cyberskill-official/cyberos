...
related_tasks: [TASK-INV-007, TASK-INV-001, TASK-TEN-102, TASK-AI-003, TASK-MEMORY-111]
depends_on: [TASK-INV-007]
blocks: []
...
3. **MUST** require `customer_agreement_doc_id` (TASK-DOC-001 reference) per DEC-1530 — without it, return 422.
...
5. **MUST** submit form to GDT within 24h of decision per DEC-1533 — via TASK-MCP-007 task (GDT may ack async).
...
8. **MUST** support replace flow: when `replacement_invoice_id` supplied, trigger TASK-INV-007 emit for replacement BEFORE marking original cancelled; ensures continuity.

9. **MUST** emit 4 memory audit kinds per DEC-1535. PII per TASK-MEMORY-111 (reason text scrubbed of customer names → SHA256 hash of full notes).
