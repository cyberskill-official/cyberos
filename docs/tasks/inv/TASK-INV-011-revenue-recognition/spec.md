...
related_tasks: [TASK-INV-001, TASK-INV-002, TASK-INV-009, TASK-AI-003, TASK-MEMORY-111]
...
8. **MUST** emit 4 memory audit kinds per DEC-1565. PII scrub per TASK-MEMORY-111: amount/period_revenue SHA-256 hashed; engagement_id (uuid) ok.
...
1. **4 methods supported + cardinality test**. 2. **Schedule built at engagement creation**. 3. **EOM rollforward via TASK-MCP-007 cron**. 4. **Journal entries balance (DR sum = CR sum)**. 5. **Snapshots immutable (no UPDATE/DELETE grant)**. 6. **Prior-period adjustment via new snapshot**. 7. **rust_decimal for amounts (no f64)**. 8. **4 memory audit kinds**. 9. **PII scrubbed (amount SHA256)**. 10. **RLS denies cross-tenant**. 11. **Trace_id preserved**. 12. **Multi-currency: schedule in engagement currency; report base via TASK-INV-002**. 13. **time_based: equal monthly split**. 14. **milestone_based: recognize on milestone complete event**. 15. **pct_completion: recompute on TASK-TIME-002 entry change**. 16. **point_in_time: full on invoice send**. 17. **Manual rollforward CFO-only**. 18. **Idempotent (UNIQUE on tenant+engagement+period)**. 19. **Failure mid-rollforward → snapshot=failed + sev-1**. 20. **Reconcile_status auto-updates (pending→partial→complete)**.
...
| pct_completion negative hours | scheduler validate | sev-2 + skip period | TASK-TIME-002 fix |
