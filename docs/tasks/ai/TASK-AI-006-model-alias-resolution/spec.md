...
10. **Override misses ZDR** — Given override target is non-ZDR and `zdr_required: true`, return `Err(ZdrViolation)`. Override doesn't bypass policy invariants. 10a. **Override misses residency** — Given override target's region (e.g., `Bedrock us-east-1`) doesn't match `policy.residency: Sg1`, `resolve` MUST return `Err(ResidencyViolation { resolved_region: Some("us-east-1"), policy_residency: Sg1 })` from the override path. Same terminal semantics as #9 and #10 — no fallthrough to primary.
