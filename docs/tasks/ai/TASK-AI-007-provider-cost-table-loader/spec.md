...
related_tasks: [TASK-AI-001, TASK-AI-002, TASK-AI-005, TASK-AI-006, TASK-AI-008]
depends_on: []
blocks: [TASK-AI-001, TASK-AI-002, TASK-AI-006, TASK-AI-008]
...
disallowed_tools:
  ...
  - mix currencies in the cost table (USD only; TASK-INV-002 handles FX)
...
9. **MUST** keep cost rates in **USD only**. Multi-currency conversion happens at invoice time (TASK-INV-002), not at cost-ledger time. Mixing currencies here would force every precheck/reconcile comparison to FX-convert, which is non-deterministic across timezones and breaks reproducibility.
...
12. **MUST** support a per-model `is_embedding: true` flag (in `CostRate`) where `output_per_1k_usd == 0.0` — embeddings have no output tokens by definition. This is informational metadata that downstream callers (TASK-AI-002 reconcile) can use to short-circuit output cost calculation.
...
14. **MUST** expose `cost_table::loaded_at() -> Option<DateTime<Utc>>` for the operator CLI (TASK-AI-021 `models pricing` subcommand) to show when the table was last loaded.
15. **MUST NOT** persist cost rates anywhere except in-memory (no Postgres write at slice 2). The TEN module's per-tenant override table (TASK-TEN-006, P2) introduces DB-backed overrides; the YAML stays canonical and is read first.

---

## §2 — Why this design (rationale for humans)

**Why YAML and not a database?** Same reasoning as TASK-AI-005's policy loader: cost rates change roughly weekly (a new model launches, Bedrock pricing updates, OpenAI deprecates `gpt-4-turbo`). Git-tracked YAML gives every rate change a PR + reviewer + diff. The audit trail of "who lowered the chat.smart rate on 2026-04-15?" is built into git blame, not in a separate audit log. The TEN module (TASK-TEN-006, deferred to P2) eventually adds a Postgres-backed override for per-tenant negotiated rates (e.g., enterprise tenants who get 10% off), but the YAML stays as the canonical bootstrap source and the fallback when TEN's DB is unreachable.
...
**Why aggregate validation failures?** Same operator-experience reason as TASK-AI-005 §1 #11. An operator misconfiguring 3 entries (say, three new models added with copy-paste errors) gets all 3 failures in one error message. Without aggregation: fix one, redeploy, find the next, redeploy, find the third. Three deploys to find three errors. With aggregation: one deploy, all three visible.

**Why USD only?** Provider invoices arrive in USD. AI providers (Bedrock, Anthropic, OpenAI) all publish prices in USD. Tenant billing is in tenant currency (VND for Vietnamese tenants, EUR for EU, SGD for Singapore, etc.) — that conversion happens at invoice issue time in TASK-INV-002, using SBV's daily FX rate published at 16:00 ICT. Mixing currencies at the cost-ledger layer would force every comparison ("is this call over the tenant's monthly cap?") to FX-convert, which is non-deterministic across timezones AND breaks reproducibility (the same audit row could yield different "in cap" verdicts on different days if FX moved).

**Why `Decimal` not `f64`?** Floating point is wrong for money. `gpt-4o-mini`'s rate is `$0.00015/1k`. A call that uses 1000 prompt tokens + 500 completion tokens costs `1.0 * 0.00015 + 0.5 * 0.0006 = 0.00045`. In `f64`, you get `0.00044999999999999996` due to representation. In `Decimal`, you get exactly `0.000450`. Aggregating thousands of these for a month, the `f64` error compounds; the `Decimal` is exact. This is a "decimal everywhere" project — cost ledger uses `Decimal`, TASK-AI-001's holds use `Decimal`, TASK-AI-002's reconciliation uses `Decimal`. Consistency.
...
**Why expose `is_embedding` metadata?** TASK-AI-002's reconcile path computes `actual_usd = prompt_tokens × input_rate + completion_tokens × output_rate`. For embeddings, `completion_tokens` is always 0 by definition (embeddings produce vectors, not tokens). Without `is_embedding`, TASK-AI-002 has no way to validate that a non-zero `completion_tokens` value on an embedding response is a bug. With it, TASK-AI-002 can assert `is_embedding ⇒ completion_tokens == 0` and surface the bug at reconcile time.
...
rates:
  ...
  bge:                                          # self-hosted (TASK-AI-019) — cost = 0
    bge-m3:
...
- Filesystem supports `notify` (inotify on Linux, FSEvents on macOS); deployment env must NOT be a network FS that lacks event support (S3FS, etc.). Same constraint as TASK-AI-005.
...
### Caller in TASK-AI-006 (alias resolve)
...
### Caller in TASK-AI-002 (reconcile — uses is_embedding to validate)
...
### Caller in TASK-AI-021 (operator CLI `models pricing`)
...
1. **~~Should the cost table support multi-currency?~~** — RESOLVED: USD only (§1 #9 + §2 rationale). FX is TASK-INV-002's job.
...
5. **~~Should we have is_embedding metadata?~~** — RESOLVED: yes (§1 #12 + §2). Helps TASK-AI-002 validate provider responses.
...
*End of TASK-AI-007. Status: draft (10/10 target, expanded from compressed first-pass per workflow correction 2026-05-15). Run `task-audit` next.*
