...
related_tasks: [TASK-AI-001, TASK-AI-002, TASK-AI-006, TASK-AI-008, TASK-AI-009, TASK-AI-016, TASK-AI-019]
depends_on: [TASK-AI-019]
blocks: [TASK-KB-006]
...
subtasks:
  ...
  - "0.5h: Model checksum verification at startup (mirrors TASK-AI-019 §1 #11 pattern)"
  - "0.5h: GPU detection + CPU fallback variant"
  - "1.0h: Adapter (`RerankProvider`) implementing the `Provider` trait extension"
  - "0.5h: Per-region sidecar URL config (mirrors TASK-AI-019 §1 #14)"
  ...
  - "0.5h: Mid-run GPU failover detection (mirrors TASK-AI-019 §1 #15)"
...
risk_if_skipped: "KB retrieval relies on raw embedding similarity (cosine) without rerank lift. Top-10 KB results include embedding-similar-but-actually-irrelevant noise; auto-runbook quality (TASK-OBS-007) and KB Q&A precision both degrade by ~15-25%. Mitigation: priority is COULD — KB module functions without rerank, just at lower precision. Descope-friendly under TASK-AI-016 if memory/KB ships behind schedule. Cost of NOT shipping: KB precision floor degrades; managed-rerank fallback (Cohere ~$0.001/search × 50 tenants × 10K searches/mo = $500/mo) becomes the operational alternative."
---

## §1 — Description (BCP-14 normative)

The AI Gateway service **MAY** offer a BGE-reranker-v2-m3 cross-encoder service as a sidecar (sibling to TASK-AI-019's embedding sidecar). The reranker and surrounding contract obey the following:

1. **MUST** be invokable via the `rerank.fast` alias from TASK-AI-006. `RerankProvider` is registered as a `Provider` enum variant in TASK-AI-008's router.
...
4. **MUST** report `cost: 0.0` to the cost-ledger per call (self-hosted = no marginal cost; amortised infra tracked separately per TASK-AI-019 §1 #6 pattern).
5. **MUST** emit one `ai.invocation` memory audit row per call, via the `canonical::invocation_rerank` builder added to TASK-AI-003's memory bridge. Row payload: `tenant_id`, `agent_persona`, `model: "bge-reranker-v2-m3"`, `model_sha256`, `actual_usd: 0.0`, `latency_ms`, `query_token_count`, `candidate_count`, `total_token_count`, `device`, `request_id`. Mirrors TASK-AI-002's invocation-row schema with rerank-specific fields.
...
8. **MUST** be deployed **per-region** matching the TASK-AI-019 sidecar pattern. URL config lives in `services/ai-gateway/config/embeddings.yaml` under `bge_rerank_sidecars`. Per-region deployment satisfies TASK-AI-016 residency pinning by construction; absence of a sidecar in the required region surfaces as `RouterError::NoSidecarForRegion` (handled by TASK-AI-016 §1 #6).
9. **MUST** verify the BGE-reranker-v2-m3 model SHA-256 checksum at sidecar startup against `embeddings/checksums/bge-reranker-v2-m3.sha256`. Mismatch refuses startup; gateway refuses to bind. Same supply-chain defence rationale as TASK-AI-019 §1 #11.
...
**Why marginal cost = 0 in cost-ledger (§1 #4)?** Same reasoning as TASK-AI-019 §1 #6: the GPU is rented monthly; one additional rerank call costs $0 marginally. Amortised infra cost is tracked OUTSIDE the per-call cost ledger via TASK-AI-022's accounting channel. This keeps cost-ledger semantics clean ("per-call dollars to provider") and avoids arbitrary amortisation choices.
...
pub async fn run() -> Result<(), Error> {
    // ... existing including TASK-AI-019 ...
...
- **TASK-AI-008** — router::Provider trait extended with `call_rerank`.
- **TASK-AI-009** — circuit breaker wraps RerankProvider per-sidecar-instance.
- **TASK-AI-016** — per-region deployment satisfies residency by construction.
- **TASK-KB-007 (downstream placeholder)** — KB module calls rerank after embedding-similarity pre-filter.
...
- Hardware: shared L4 GPU with TASK-AI-019 (BGE-M3 ~2GB + BGE-reranker-v2-m3 ~600MB ≈ 2.6GB VRAM, well within L4's 24GB).
...
bge:
  bge-reranker-v2-m3:
    input: 0.0
    output: 0.0
    notes: "Self-hosted; marginal cost = 0. Amortised infra cost shared with bge-m3 (TASK-AI-019)."
```
...
- Multi-region rerank load-balancing — TASK-AI-016 area; current model is strict per-region pinning.
...
| No sidecar in required region | URL lookup miss | `NoSidecarForRegion` cascades to TASK-AI-016 | Operator deploys sidecar in region OR caller accepts fallback |
...
*End of TASK-AI-020. Status: draft (10/10 target).*
