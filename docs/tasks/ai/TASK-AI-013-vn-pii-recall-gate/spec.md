...
related_tasks: [TASK-AI-011, TASK-AI-012, TASK-AI-022]
depends_on: [TASK-AI-012, TASK-AI-011]
blocks: []
...
subtasks:
  ...
  - "0.5h: test_precision_warning.py — precision reported per recognizer; trend hook for TASK-AI-022"
...
### Fixture file format (YAML — reconciled to TASK-AI-012 §6)

```yaml
# services/ai-gateway/pii/fixtures/vn_pii_200_samples.yaml
# 200 positive samples + 30 negative samples. Curated quarterly.
# Sample-count distribution per TASK-AI-013 §1 #1: 50 CCCD / 30 MST / 40 PHONE / 20 NDD / 40 ADDRESS / 20 BANK_ACCOUNT + 30 negative.
...
# Aggregate floor (TASK-AI-012 §1 #1 post-ISS-004 also requires aggregate).
...
9. **Recognizer version pin asserted** — Before running recall, `test_recall_gate.py` calls `GET /recognizers/version` (TASK-AI-012 §1 #14), parses the response, and asserts each entity's version matches `fixture_manifest.yaml`'s `recognizer_versions` field. Mismatch raises `fixture_version_mismatch` with non-zero exit.
...
11. **Aggregate recall floor enforced** — In addition to per-recognizer, `test_recall_gate.py` asserts the aggregate `correct / total >= 0.99` (matches TASK-AI-012 §1 #1 post-ISS-004).
...
17. **Trend chart hook (TASK-AI-022)** — The recall gate's JSON artefact is consumed by TASK-AI-022's dashboard with no schema changes; `validate_corpus_format.py --schema-out` is the contract.
...
@pytest.fixture(scope="module")
def analyzer():
    """Initialise Presidio with all 6 VN recognizers per TASK-AI-012."""
        ...
        with:
          script: |
            ...
            await github.rest.issues.create({
              ...
              body: [
                `Per TASK-AI-013 §1 #4, the VN PII fixture must be regenerated quarterly.`,
                ``,
  ...
  - repo: local
    hooks:
      - id: no-real-pii-in-corpus
        name: VN PII corpus — block real-CCCD-like commits (TASK-AI-013 §1 #14)
...
- `recognizer_versions` in `fixture_manifest.yaml` MUST match the semver strings emitted by `GET /recognizers/version` (TASK-AI-012 §1 #14).
...
- `CONFIDENCE_HIGH`, `CONFIDENCE_MED`, `CONFIDENCE_LOW` from `recognizers/confidence.py` (TASK-AI-012 §3) drive the negative-sample test's threshold.
...
- ASEAN expansion (Khmer / Lao / Thai recognizers + their fixtures) — slice 6 / TASK-AI-022 area.
...
| New VN PII format emerges (production discovery) | Customer-facing leak OR TASK-AI-022 trend chart alarms | Recognizer misses; sev-2 alert | Add sample(s) to fixture; update recognizer; bump fixture_version; PR through gate |
...
| New VN province code not in `province_codes.py` (TASK-AI-012 dep) | CCCD recall drops on that province's samples → recall gate fails on VN_CCCD | CI block on CCCD recall | Add province code to TASK-AI-012's `province_codes.py`; re-run gate |
| Test set bias (over-representation of labeled samples) | Production recall lower than fixture recall by >2pp | Out-of-band monitoring (TASK-AI-022) | Quarterly regen consciously includes harder samples |
...
| Network call accidentally added in a test (e.g., `requests.get(...)` in a fixture loader) | `vn-pii-recall.yml` egress-monitor step OR `test_no_network_imports.py` (TASK-AI-012 ISS-002 style) extended to `tests/` directory | CI fails on offline-only assertion | Engineer removes the network call |
...
*End of TASK-AI-013. Status: draft (10/10 target).*
