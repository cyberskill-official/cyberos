...
related_tasks: [TASK-AI-001, TASK-AI-005, TASK-AI-006, TASK-AI-008, TASK-AI-016]
...
subtasks:
  ...
  - "1.0h: zdr::is_zdr() + zdr::attestation_for() + ArcSwap registry (same pattern as TASK-AI-005, TASK-AI-014)"
  ...
  - "0.5h: alias::resolve integration (TASK-AI-006 §1 #6 wiring)"
...
| Tenant policy `zdr_required` missing | TASK-AI-005 schema default | Defaults to false (no enforcement for that tenant) | Operator updates tenant policy YAML if ZDR is required |
...
| Audit row emit fails (memory bridge down) | `memory_writer::emit` returns Err | Refusal still proceeds; sev-1 log ("ZDR refused but audit row failed") | Operator investigates memory; TASK-AI-003 §10 covers |
