...
related_tasks: [TASK-AI-001, TASK-AI-006, TASK-AI-007, TASK-AI-008, TASK-AI-009, TASK-AI-016, TASK-AI-020]
depends_on: []
blocks: [TASK-AI-020, TASK-MEMORY-101, TASK-KB-005]
...
subtasks:
  ...
  - "0.5h: Circuit-breaker integration (TASK-AI-009 wraps BgeProvider)"
...
2. **MUST** be invokable via the `embed.standard` and `embed.code` aliases (TASK-AI-006). `BgeProvider` is registered as one of the `Provider` enum variants in TASK-AI-008's router.
...
6. **MUST** report `cost: 0.0` to the cost-ledger per BGE call (self-hosted = no marginal cost). The amortised infra cost (L4 GPU rental ~$360/month) is tracked OUTSIDE the per-call cost ledger via a separate accounting channel (TASK-AI-022 follow-up). This split is documented in §2.
7. **MUST** integrate with the TASK-AI-009 circuit breaker: sustained sidecar failures (5 consecutive HTTP 5xx OR 5 consecutive timeouts) trigger breaker opening; subsequent calls fail fast (no sidecar contact) until the half-open probe succeeds. The breaker state is per-sidecar-instance (not per-tenant).
...
**Why expose `model_name` + `model_sha256` in every response (§1 #13)?** memory Layer 2 stores embeddings in a vector index. If we silently switch from `bge-m3@1.0` to `bge-m3@1.1`, old embeddings in the index are now in a different vector space — retrieval quality silently degrades. Embedding the model identity in every response gives downstream consumers the signal to invalidate / re-embed. This is the same lock-step versioning principle as TASK-AI-014's persona handle and TASK-AI-017's cache schema version.
...
13. **Circuit breaker integrates** — 5 consecutive sidecar 5xx responses → breaker opens; subsequent calls fail fast with `BreakerOpen`; TASK-AI-008 routing falls back to next provider.
...
15. **No sidecar in required region returns ResidencyViolation** — Request with `region=Vn1` (no Vn1 sidecar deployed) → `RouterError::ResidencyViolation` (delegated to TASK-AI-016).
...
- **TASK-AI-008** — router::Provider trait includes `call_embed`; BgeProvider is one variant.
- **TASK-AI-009** — circuit breaker wraps BgeProvider (per-sidecar-instance state).
- **TASK-AI-016** — region-pinned sidecar selection; absent sidecar in required region surfaces as residency violation.
- **TASK-AI-020 (downstream)** — BGE rerank shares the sidecar infrastructure (different model checkpoint; same sidecar pattern).
- **TASK-AI-022 (downstream)** — amortised infra cost dashboarding.
...
| No sidecar for required region | `bge_provider::call_embed` lookup miss | `RouterError::ResidencyViolation` cascades to TASK-AI-016 | Operator deploys sidecar in region OR tenant accepts alternative |
...
- The cost-ledger reports zero per-call (§1 #6) because the marginal cost IS zero. Amortised infra accounting is a SEPARATE ledger surfaced in TASK-AI-022's dashboards. Conflating them would either over-charge per-call (if amortised) or under-report infra (if zeroed).
- Per-region sidecar deployment satisfies TASK-AI-016 by construction. A `Vn1` tenant's embeddings cannot leave Vietnam if the sidecar is in `ap-southeast-1` because there's no Vn1 sidecar deployed (slice 4); the request fails with `ResidencyViolation` honestly rather than silently routing across borders.
...
*End of TASK-AI-019. Status: draft (10/10 target).*
