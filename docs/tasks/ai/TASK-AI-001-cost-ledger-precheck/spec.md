...
related_tasks: [TASK-AI-002, TASK-AI-003, TASK-AI-004, TASK-AI-005]
depends_on: [TASK-AI-003, TASK-AI-005, TASK-AI-007]
blocks: [TASK-AI-002, TASK-AI-004, TASK-TEN-004]
...
8. **MUST** load the cost-rate table from `services/ai-gateway/config/cost_rates.yaml` at gateway startup (loader machinery from TASK-AI-005 is reused). The hardcoded `const HashMap` is REMOVED for slice 1; cost rates live in YAML from day 1.
...
11. **MUST** emit the `ai.precheck` memory audit row synchronously (subprocess fork; ~30ms latency). TASK-AI-008 (slice 2 PyO3 spike) explores in-process PyO3 for the latency gain; until then, the audit-before-action invariant outweighs the latency cost.
...
13. **MUST** check `policy.ai_policy.allowed_personas` (if non-null) against `req.agent_persona`; refuse with `403 FORBIDDEN` and `{error: "persona_not_allowed", allowed: [...]}` if not in the list. This enforces the per-tenant persona-pinning policy from TASK-AI-005's schema.
...
|---|---|
| `precheck → Allow` then provider call succeeds | `200 OK` + chat response body; post-reconcile fires (TASK-AI-002) |
...
### Response — Allow path (HTTP 200, after provider call succeeds — TASK-AI-002 handles the actual provider call + post-reconcile)
...
4. **~~Audit-row sync vs async~~** → §1 #11 (synchronous in slice 1; TASK-AI-008 spikes PyO3).
...
|---|---|---|---|
| Tenant policy file missing | `policy::load_for_tenant` returns `PolicyMissing` | `503 POLICY_NOT_FOUND` | Operator adds the YAML; gateway re-reads on file-watch event (TASK-AI-005 §1 #4) |
...
| Budget cap exceeded | `current.spent_usd + estimated_usd > monthly_cap_usd` | `402 PAYMENT_REQUIRED` | Operator raises cap or tenant waits to next period; TASK-AI-021 CLI exposes `cyberos-ai policy raise-cap` |
...
| Hold UNIQUE constraint hit on idempotency_key | Retry with same `idempotency_key` from caller | Returns existing hold | Idempotent by design (§1 #4 of TASK-AI-001 — return the existing hold_id) |
