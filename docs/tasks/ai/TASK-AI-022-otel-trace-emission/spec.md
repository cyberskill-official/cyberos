...
related_tasks: [TASK-AI-001, TASK-AI-002, TASK-AI-008, TASK-AI-009, TASK-AI-010, TASK-AI-014, TASK-AI-017, TASK-AI-019, TASK-AI-021, TASK-OBS-001, TASK-OBS-004, TASK-OBS-005, TASK-OBS-006]
depends_on: [TASK-AI-008, TASK-AI-003, TASK-OBS-001]
blocks: [TASK-OBS-004]
...
1. **MUST** initialise the OTel SDK at gateway startup with an OTLP gRPC exporter pointed at `http://localhost:4317` (the OBS collector deployed by TASK-OBS-001). Initialisation failure is fatal — gateway refuses to bind. Sampler is `AlwaysOn` at the gateway; tail-based reduction happens in the OBS collector per TASK-OBS-006.
...
7. **MUST** propagate trace context to outgoing provider HTTP calls via the W3C `traceparent` header. Provider responses (Bedrock, OpenAI) propagate trace context downstream where supported (LangSmith via TASK-OBS-004 ingests these). The propagation function lives in `propagation.rs` and is invoked by every provider adapter.
8. **MUST** emit span EVENTS (not separate spans) for retries within a single provider_call span: `retry.attempt` event with `attempt_num`, `backoff_ms`, `prior_status_code`. Retries on the same provider attempt within TASK-AI-009's retry budget produce events on the same span; failover to a different provider creates a NEW provider_call span (per §1 #5).
...
- **TASK-AI-010** — Streaming bypasses cache; `cache_state: skipped` attribute on root span.
...
- OTel collector at `localhost:4317` (TASK-OBS-001 deployment).
...
- Trace export to Jaeger/Tempo/Honeycomb via OTLP is standard; TASK-OBS-001 owns the collector configuration. The gateway is collector-agnostic — it speaks OTLP and lets the collector route.
...
*End of TASK-AI-022. Status: draft (10/10 target).*
