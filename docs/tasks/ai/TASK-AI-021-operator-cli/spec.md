...
related_tasks: [TASK-AI-001, TASK-AI-002, TASK-AI-003, TASK-AI-004, TASK-AI-005, TASK-AI-007, TASK-AI-008, TASK-AI-009, TASK-AI-014, TASK-AI-015, TASK-AI-022]
depends_on: [TASK-AI-005, TASK-AI-008, TASK-AI-002, TASK-AI-004, TASK-AI-009]
blocks: []
...
subtasks:
  ...
  - "0.5h: `expiry status` + `expiry repair --confirm` (TASK-AI-004 dedupe)"
...
4. **MUST** emit a memory audit row for ANY mutating operation (policy set, breaker reset, failover drill, expiry repair, memory emit) via the dedicated `canonical::cli_*` builders. The row carries `command`, `args`, `operator_id` (from token), `request_id`, `outcome`, AND a SHA-256 of the full command line for replay identification. Audit-before-action invariant from TASK-AI-001 §1 #6 applies.
...
9. **MUST** include the subcommand catalogue (each subcommand has `--help` listing its specific flags):
   ...
   | `cyberos-ai expiry repair --confirm` | Dedupe duplicate `ai.hold_expired` rows (TASK-AI-004) | Yes | admin | `ai.cli_expiry_repaired` |
...
13. **MUST** include `policy validate` as a no-network operation: parses the YAML, runs TASK-AI-005's schema validator, prints errors (line-by-line if multiple) OR success. Useful for pre-deploy CI gates that validate policy YAMLs before merge.
...
Re-exported from the shared crate **`cyberos-cli-exit`** (single source of truth across all CyberOS CLIs — see also TASK-AUTH-006). CI scripts depend on these numerical values; they are a stable contract once locked.
...
10. **`cyberos-ai expiry repair --confirm`** removes duplicate `ai.hold_expired` rows (TASK-AI-004 dedupe).
...
16. **`policy validate` no-network** — Parses YAML; calls TASK-AI-005 schema validator; prints errors line-by-line on failure (exit 5) or `valid` on success (exit 0).
...
- **TASK-AI-022 (downstream)** — OTel CLI metrics consumed by the operator dashboard.
...
- `expiry repair` directly closes the slice-1 limitation in TASK-AI-004 AC #9 (manual SQL was the workaround). The CLI command makes this a routine ops task instead of a database query.
...
*End of TASK-AI-021. Status: draft (10/10 target).*
