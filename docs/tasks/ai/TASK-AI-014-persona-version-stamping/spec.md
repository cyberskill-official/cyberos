...
related_tasks: [TASK-AI-001, TASK-AI-003, TASK-AI-005, TASK-AI-008, TASK-AI-022]
depends_on: [TASK-AI-003]
blocks: []
...
- Per-tenant persona override (`tenant.persona_override["cuo-cpo"] = "<custom-handle>"`) — slice 5; TASK-AI-005 schema extension.
- Signed personas (Ed25519 over source_hash) — TASK-AI-022; current SHA-256 tamper-check is sufficient for boundary detection but doesn't survive a compromised disk-writer.
...
| memory read failure at init | `memory_writer::read_path` error | `PersonaInitError::MemoryReadFailed` → init fails → gateway refuses to bind | Operator investigates memory (TASK-AI-003 §10 covers); restart gateway after memory recovery |
...
| Allowed_tools field references a non-existent MCP tool | Runtime detection at tool-call time (TASK-MCP-006) | Tool call refused; sev-2 log | Operator updates persona OR registers the tool |
