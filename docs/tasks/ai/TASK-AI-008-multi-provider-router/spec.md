...
related_tasks: [TASK-AI-001, TASK-AI-002, TASK-AI-006, TASK-AI-007, TASK-AI-009, TASK-AI-010, TASK-AI-015, TASK-AI-021]
depends_on: [TASK-AI-006, TASK-AI-007, TASK-AI-002]
blocks: [TASK-AI-009, TASK-AI-010, TASK-AI-021, TASK-AI-011, TASK-AI-017, TASK-AI-022, TASK-CUO-101]
...
source_decisions:
  - docs/tasks/ai/TASK-AI-006-model-alias-resolution/spec.md §1 (ResolvedModel.fallback_position)
  - docs/tasks/ai/TASK-AI-007-provider-cost-table-loader/spec.md (cost basis)
...
effort_hours: 10
subtasks:
  - "1.0h: Provider trait (call_chat, call_embed) + ProviderKind::as_metric_label() (added to TASK-AI-006 schema if absent)"
  - "1.5h: 3 impls (Bedrock, Anthropic, OpenAI) — each ~150 lines; covers happy path + error mapping"
  - "1.0h: router::call_provider entry — accepts ResolvedModel from TASK-AI-006, builds chain, dispatches"
...
1. **MUST** accept (a) the `ChatCompleteRequest`, (b) the `ResolvedModel` from TASK-AI-006, (c) a tokio `Instant` deadline, (d) a reference to the active `TenantPolicy` (used to walk the fallback chain). The deadline is enforced strictly — if it elapses mid-call, the function returns `Err(RouterError::DeadlineExceeded)` within 50ms.
2. **MUST** dispatch to the resolved provider's impl via the `Provider` trait. Three impls in slice 2: `BedrockProvider`, `AnthropicProvider`, `OpenAIProvider`. Vertex (Gemini) lands in slice 4 (TASK-AI-017 area). The trait MUST live in `src/router/mod.rs`; provider-specific code MUST live in `src/router/<provider>.rs` with no cross-impl `use` statements.
...
9. **MUST** treat HTTP `404` (model not found) as terminal — no retry, no failover. A 404 means the resolved model name doesn't exist at the provider; failover would mask the configuration error. The cost-table loader (TASK-AI-007) and alias resolver (TASK-AI-006) are supposed to prevent this; if a 404 reaches the router, the alias resolver has a bug.
...
11. **MUST** normalize all three providers' response shapes into one `ProviderResponse { id, usage, choices, finish_reason, latency_ms, cache_state, attempts }`. Downstream code (TASK-AI-002 reconcile) sees a single struct. The normalization MUST live in `src/router/normalize.rs` and MUST be exhaustive over each provider's documented shape — unknown fields are dropped silently, missing required fields produce `Err(InvalidResponse)`.
...
**Why is HTTP 404 terminal?** A 404 means the resolved model name doesn't exist at the provider. This can only happen if the cost-table loader (TASK-AI-007) and the alias resolver (TASK-AI-006) disagree about what models are valid. Failing over would mask the bug. Returning 404 to the caller (which becomes `ProviderError` in TASK-AI-002) and triggering a sev-2 log lets operators catch the configuration drift before it spreads.
...
/// Same as call_provider but returns a streaming response. Slice 3 (TASK-AI-010) wires this.
...
filter to skip providers whose circuit breaker is open (TASK-AI-009)
...
16. **Streaming stub returns Err** — In slice 2, calling `call_provider_streaming` MUST return `Err(StreamingNotImplemented)`. (Slice 3's TASK-AI-010 changes this AC to a positive test.)
17. **Traceparent propagated to outbound (task-audit skill §3.7)** — Header-capture mock provider records the inbound HTTPS `traceparent` value of every call. A round-trip test: inbound `cost_ledger::precheck` opens a span with `traceparent: 00-<inbound-trace-id>-<inbound-span-id>-01`; the mock provider's recorded `traceparent` MUST share the same `<trace-id>` (32-hex) component. The `<span-id>` MUST differ (child span) but `<trace-id>` MUST match.
18. **Router does not mutate policy (task-audit skill §3.10 rule 30)** — `Arc::strong_count(&policy)` before and after `router::call_provider(.., &policy, ..)` MUST be identical (no transient clone retained). Asserts the MUST NOT "mutate policy from inside the router."
...
- **TASK-AI-009 (downstream)** — Circuit breaker. The router's `build_provider_chain` MAY filter out providers with open breakers (slice 3). In slice 2 the breaker isn't yet wired; the router consumes the full chain.
...
// attempts vec accessible via tracing context (not in error variant in slice 2; TASK-AI-002 reads it via channel):
[
    AttemptRecord { provider: Bedrock, attempt_num: 1, fallback_position: 0,
...
```rust
// In slice 2 (TASK-AI-008):
let err = router::call_provider_streaming(&req, &resolved, deadline, &policy).await.unwrap_err();
assert!(matches!(err, RouterError::StreamingNotImplemented));

// In slice 3 (TASK-AI-010 wires this — replaces stub with full SSE pipeline).
```
...
- Tenant-configurable failover budget (TASK-AI-021).
- Vertex (Gemini) provider impl (TASK-AI-017).
- Per-provider rate-limit pre-check before issuing the request (TASK-AI-009 area).
...
| All providers in chain failed | exhausted retries on all | `Err(AllProvidersFailed { attempts })` | Caller sees `503` from gateway; TASK-AI-002 refunds |
| Deadline exceeded mid-call | `Instant::now() >= effective_deadline` | `Err(DeadlineExceeded)` | TASK-AI-002 reconciles as `Cancelled` (refund) |
...
| Concurrent calls hit provider rate limit | 429 across many calls | Each individually retries; circuit-breaker (TASK-AI-009) opens after threshold | Self-healing with circuit-breaker |
...
*End of TASK-AI-008. Status: draft (10/10 target).*
