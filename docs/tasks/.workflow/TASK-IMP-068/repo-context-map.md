...
- error_type: bash `set -euo pipefail`, explicit numeric exit codes (0 ok / 10 policy fail / 2 unreadable) - pinned_in: tools/cyberos-install/init.sh, scripts/caf_gate.sh
- logging: `cyberos-install:` / `cyberos:` prefixed echo lines, errors to stderr - pinned_in: tools/cyberos-install/build.sh, .pre-commit-hooks/cyberos-payload-build.sh
...
## Files outside the immediate domain (tools/cyberos-install/)
...
## Blast radius
file_count: 9 | module_count: 3 (tools/cyberos-install, .github, .githooks) | cross_module_edges: hooks->build.sh, CI->build.sh+check
