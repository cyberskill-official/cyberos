...
- build.sh guard: explicit `cyberos: ERROR: VERSION missing or not X.Y.Z` on refusal.
...
- payload-gate.yml + version.yml: step-level visibility native to Actions; step names carry the intent. PII: none in scope (versions and paths only); no redaction policy needed.
