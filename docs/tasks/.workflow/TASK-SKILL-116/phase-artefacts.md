...
## Repo context map
Patterns: bash set -uo pipefail + exit 0/10/2 contract + `cyberos-init:` prefix (pinned_in: check-version-sync.sh, TASK-IMP-068); vendored set = single $skills string at build.sh:28; chain doc = modules/cuo/.../ship-tasks.md skill_chain (`skill: <name>` entries); command docs name skills as backticked tokens.
