...
## Repo context map
Patterns: exit 0/10/2 + `cyberos-install:` prefix (TASK-IMP-068 lineage); release.yml = tag-driven job set (desktop/android/ios), top-level permissions contents: write; bootstrap legacy env CYBEROS_PACK_URL + top-level-dir tarballs; rollout takes payload dir as $1.
