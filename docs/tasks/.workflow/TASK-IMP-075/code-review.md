...
Expected-pending (honest): cargo check both ways - no Rust toolchain in this environment; first real run is Stephen's machine or the first MAS_RELEASE build. Attribute syntax is the only risk surface and it is minimal. Verdict needed: "TASK-IMP-075 review: approved" or rejected+reason.
