...
related_tasks: [TASK-LEARN-003, TASK-REW-008, TASK-MCP-007, TASK-MEMORY-111]
depends_on: [TASK-LEARN-003]
blocks: []
...
source_decisions:
  - DEC-2140 2026-05-17 — At quarter close (Mar 31 / Jun 30 / Sep 30 / Dec 31), sum per-member VP from TASK-LEARN-003 weekly snapshots → emit aggregate share to TASK-REW-008 for BP fund split
...
7. **MUST** emit 4 memory audit kinds per DEC-2144. PII per TASK-MEMORY-111: vp values SHA-256; member_id + shares ok.
...
- §11.1 Cron via TASK-MCP-007 `kind: 'learn.vp_rew_handoff'`, Q-end+1d at 04:00.
- §11.2 Aggregator pure function: `(member_vp_snapshots, total_vp) → shares`.
- §11.3 REW emit via HTTP POST to TASK-REW-008 endpoint with retry+ack.
