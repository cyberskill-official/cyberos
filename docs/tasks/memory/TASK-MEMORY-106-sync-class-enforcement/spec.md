...
# placeholder — read-time ACL filtering task, not yet specified
...
modified_files:
  # call should_sync() at every direction
  - services/memory-sync/src/sync.rs
allowed_tools:
  - file_read: services/memory-sync/**
  - file_write: services/memory-sync/**
  - bash: cd services/memory-sync && cargo test sync_class
disallowed_tools:
  - sync compensation/equity rows under any sync_class (per DEC-036 — structural exclusion)
  #1 + #2 + #4)
  - skip should_sync() check on any sync direction (per §1
  #3)
  - bypass ACL on shareable rows with non-empty acl (per §1
  #4 — log + count)
  - silently apply transitional v1 mapping (per §1
...
subtasks:
  ...
  - "0.5h: cyberos-memory validate-sync-class CLI for ops to dry-run"
...
The memory-sync daemon **MUST** enforce `meta.sync_class` filtering on every sync direction (push + pull + import + cross-memory merge). Each filter call:

1. **MUST** push only `sync_class == "shareable"` rows to Cloud memory. Rows with `private | local-only` are filtered at push.
...
**Why filter at EVERY boundary (§1 #1 + #2 + #5)?** Push filter prevents Cloud receiving private rows. Pull filter prevents local importing private rows. Both are necessary because Cloud memory's filtering bug could push private rows TO us — pull filter catches that. Defense-in-depth at every direction.
...
**Why audit-row per filter (§1 #6)?** Compliance audits need to answer "what was filtered out + why?" Without the audit row, filter decisions are invisible. With the row, ops + auditors can answer the question via standard memory query.
...
for row in pending {
    ...
    match decision {
        ...
        SyncDecision::Refuse(reason) => {
            memory::emit(canonical::sync_row_filtered(&row, Direction::Push, reason)).await?;
...
for remote in pulled {
    ...
    match decision {
        ...
        SyncDecision::Refuse(reason) => {
            memory::emit(canonical::sync_row_filtered(&remote, Direction::Pull, reason)).await?;
...
13. Filter decisions emit `memory.sync_row_filtered` audit row with reason.
...
16. CLI `cyberos-memory validate-sync-class --file row.yaml --actor @alice` returns SyncDecision JSON.
17. Sev-1 alarm on `memory_sync_compensation_excluded_total > 0/day` (upstream bug).
...
fn v1_transitional_metric_increments() {
    ...
    let metric: u64 = otel_test_helper::counter_value("memory_sync_v1_transitional_total", &[("value", "publishable")]);
    assert_eq!(metric, 1);
}
```
...
fn cli_validate_sync_class_returns_json() {
    ...
    let output = std::process::Command::new(env!("CARGO_BIN_EXE_cyberos_memory"))
...
- **TASK-MEMORY-103** — sync.rs integration point.
- **TASK-MEMORY-107 (downstream)** — read-time ACL filtering uses similar logic.
...
```text
$ cyberos-memory validate-sync-class --file row.yaml --actor @alice
{
  "decision": "refuse",
  "reason": {
    "acl_mismatch": {
...
| Audit row emit fails | memory_writer error | Sev-2; row still filtered | Operator investigates memory |
...
- v1 transitional support exists for migration; eventual deprecation via TASK-MEMORY-204 (after v1 usage drops to <1% of rows).
...
- The `memory.sync_row_filtered` audit row is the compliance primitive. Auditors can answer "what was filtered out?" via standard memory query.
...
*End of TASK-MEMORY-106. Status: done (implemented 2026-05-23).*
