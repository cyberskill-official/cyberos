...
related_tasks: [TASK-MEMORY-101, TASK-MEMORY-108, TASK-MEMORY-122, TASK-MEMORY-123, TASK-EVAL-001, TASK-AUTH-002, TASK-AUTH-004, TASK-CHAT-101, TASK-OBS-008]
depends_on: [TASK-MEMORY-101, TASK-EVAL-001]
blocks: [TASK-MEMORY-122, TASK-MEMORY-123]
...
disallowed_tools:
  ...
  - emit an interaction-event for a subject that has not acknowledged the TASK-EVAL-001 notice (per DEC-2702 — the consent gate is mandatory)
...
subtasks:
  ...
  - "1.0h: interaction/consent_gate.rs — has_acknowledged(pool, tenant, subject) reading the TASK-EVAL-001 notice-ack ledger; emit() returns Skipped{reason:consent_not_acknowledged} when false"
...
risk_if_skipped: "Without one frozen event shape, every module (AUTH, CHAT, PROJ, EMAIL, APP, MCP) invents its own audit payload, and the BRAIN ingestion (TASK-MEMORY-123) and the evaluation engine (TASK-EVAL-003) have to special-case each one forever — the 'record all interactions' goal degrades into per-module archaeology. Without content_ref discipline, raw message/document/email bodies land verbatim in a years-retained audit chain (a PDPD minimisation breach). Without the consent gate defined here, capture can start before the TASK-EVAL-001 notice is acknowledged — the exact covert-monitoring failure the governance phase exists to prevent. Without versioning, the first schema change silently breaks every downstream emitter that pinned the old shape."
---
...
10. **MUST** bound the row size: `attributes` ≤ 2 KiB serialised and the whole body ≤ 16 KiB. Over-size → `EmitError::Invalid`. This keeps the audit chain and the TASK-MEMORY-101 ingest cheap and stops a module from smuggling raw content through `attributes`.
...
- **TASK-MEMORY-101** — the layer-1 audit chain + reconcile + layer-2 ingest these rows ride on; `cyberos-audit-chain` anchor compatibility.
- **TASK-EVAL-001** — owns the monitoring-notice + acknowledgment ledger the consent gate reads (`eval_monitoring_ack`, `eval_current_notice`). Until it lands, the gate resolves against a documented operator-seeded stub (§11).
- **TASK-AUTH-002 / TASK-AUTH-004** — `subject_id` and `session_id` come from AUTH subjects + sessions (jti).
- **TASK-OBS-003** — `trace_id` is the W3C traceparent the request middleware sets.
- **TASK-OBS-008** — the `event_type` generated column on `l1_audit_log` these rows populate; TASK-APP-005 viewer reads it.
- **Downstream:** TASK-MEMORY-122 (emitters depend on this contract), TASK-MEMORY-123 (ingestion reads this shape), TASK-EVAL-003 (evaluation indexes these dimensions).
...
- Per-`event_type` retention classes (e.g. presence pruned sooner than IP-activity) — owned by TASK-EVAL-001 governance/retention, applied at ingest in TASK-MEMORY-123.
...
| Notice version bumped (mass re-ack needed) | consent_skipped spike | Everyone suppressed until re-ack | By design; operator re-circulates notice (TASK-EVAL-001) |
...
| TASK-EVAL-001 ledger absent (pre-landing) | stub resolves default | operator-seeded default applies | Replace stub when TASK-EVAL-001 lands |
...
| `op='view'` row deleted by privileged role | chain reconcile (TASK-MEMORY-101) | detected as a chain gap | Incident response |
...
| OTel exporter down | buffered then dropped | metrics gap; rows still written | Restore TASK-OBS-001 |
