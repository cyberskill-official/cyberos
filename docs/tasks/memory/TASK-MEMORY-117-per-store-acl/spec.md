...
related_tasks: [TASK-MEMORY-103, TASK-MEMORY-106, TASK-MEMORY-112, TASK-MEMORY-115]
depends_on: []
blocks: [TASK-MEMORY-118]
...
13. **MUST** require the AGENTS.md §14.4 amendment APPROVED via `APPROVE protocol change P20 §14.4` chat-turn before runtime ACL enforcement is enabled. Same runtime-check pattern as TASK-MEMORY-115's §7.7: the writer checks for the §14.4 anchor in AGENTS.md at construction; absent → ACL enforcement is in WARN-ONLY mode (rejections logged + aux row emitted, but writes proceed). Once §14.4 is anchored, full enforcement is active.
...
**Why mtime-keyed cache (§1 #9).** STORE.yaml files are rarely changed but frequently consulted. In-memory cache avoids re-parsing YAML on every write. mtime-based invalidation matches how `cyberos doctor` and TASK-MEMORY-107 (FS watcher) detect external mutations.
