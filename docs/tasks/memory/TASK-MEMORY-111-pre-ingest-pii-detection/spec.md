...
related_tasks: [TASK-MEMORY-107, TASK-MEMORY-109, TASK-AI-011, TASK-AI-012, TASK-AI-013, TASK-OBS-007]
depends_on: [TASK-MEMORY-107, TASK-AI-012]
blocks: [TASK-TEN-004]
...
subtasks:
  ...
  - "0.5h: integrate into TASK-MEMORY-107 emit.rs (call pii::scan_and_redact BEFORE blake3 hashing — hash the redacted body)"
  - "0.5h: integrate TASK-MEMORY-109 redactor delegation (delete inline regexes; use ruleset.rs)"
  - "0.5h: TASK-AI-011 + TASK-AI-012 module sync (move their regexes to ruleset.rs)"
...
The pre-ingest PII detection layer **MUST** sit between the TASK-MEMORY-107 capture daemon's event classifier and its memory-writer call, scanning every byte that would be written to the chain. The contract:
...
**Why 99.5% recall (§1 #9)?** Compliance commitment: the memory never stores raw PII matching our ruleset. 99.5% recall means ≤ 0.5% of PII spans slip through — those are typically edge cases (CCCDs with unusual formatting). The remaining 0.5% are caught by TASK-AI-013's post-ingest recall gate AND by quarterly red-team review. 100% recall is impossible (open language; Presidio bounded). 99.5% is the calibrated threshold.
...
**Why ≤ 50ms p95 (§1 #13)?** Capture is on TASK-MEMORY-107's hot path; total event-to-emit budget is ~250ms (TASK-MEMORY-107 §1 #2 debounce + redaction + dedup + emit). 50ms for PII scan keeps the latency tax bounded.
...
21. **Hash invariance under PII change** — same redacted body but different raw PII (e.g. `"<EMAIL>"` substituted for two different real emails) → same blake3 hash; idempotent dedup at TASK-MEMORY-107.
22. **AI gateway uses the same ruleset** — TASK-AI-011 + TASK-AI-012's PII module imports from `cyberos_memory_capture::pii::ruleset`; verified via build-graph test.
23. **Claude Code hook uses the same ruleset** — TASK-MEMORY-109's `redact.rs` delegates to `ruleset::scan_with_regex`.
...
- **TASK-MEMORY-107 (upstream)** — capture emit pipeline; PII scan slots in just before blake3.
- **TASK-MEMORY-109 (sibling)** — Claude Code hook delegates redaction to this ruleset.
- **TASK-AI-011 / TASK-AI-012 / TASK-AI-013** — AI Gateway redactor + VN PII plugin + recall-floor CI gate; shared ruleset.
- **TASK-OBS-007** — alarms when scan_failed_total spikes.
...
| Invalid UTF-8 in input | Already handled at TASK-MEMORY-107 boundary | N/A | N/A |
