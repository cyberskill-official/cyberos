...
related_tasks: [TASK-MEMORY-101, TASK-MEMORY-108, TASK-MEMORY-113, TASK-MEMORY-114, TASK-MEMORY-117, TASK-MEMORY-121, TASK-MEMORY-122, TASK-AI-019, TASK-AI-020, TASK-AI-022, TASK-AUTH-003, TASK-AUTH-004, TASK-EVAL-001, TASK-OBS-001]
depends_on: [TASK-MEMORY-121, TASK-MEMORY-122]
...
source_decisions:
  - DEC-2720 (WIDE day-1 capture feeds the brain; the ingest worker consumes the TASK-MEMORY-121 interaction-event log produced by the TASK-MEMORY-122 emitters; no event type is special-cased out of ingestion)
  - DEC-2721 (the l1_audit_log hash chain stays the SYSTEM OF RECORD; the vector index + summaries are a DERIVED fast lens — rebuildable from Layer 1, never authoritative, Layer 1 wins on any conflict; reuse TASK-MEMORY-101 DEC-070 invariant)
  - DEC-2722 (ACCESS-RESTRICTED brain: embeddings + summaries inherit tenant RLS AND the per-subject access rules from TASK-EVAL-001; recall MUST NOT leak across the access boundary even when the vector neighbour is semantically closest)
  - DEC-2723 (residency-pinned, spend-capped embeddings via the ai-gateway — the worker never calls a model provider directly; the gateway enforces region + ZDR + the tenant spend cap; reuse TASK-AI-022 policy surface)
  ...
  - DEC-2726 (every recall result carries a provenance pointer back to the exact l1_audit_log row(s) it was derived from, so TASK-EVAL-003 can cite exact events; a summary cites the event range it compacted)
...
disallowed_tools:
  ...
  - return a recall result whose subject the caller is not entitled to under TASK-EVAL-001 (per DEC-2722 — access boundary is load-bearing)
...
subtasks:
  ...
  - "1.0h: brain/event_cursor.rs — per-tenant cursor over the TASK-MEMORY-121 event stream; restart resume"
  - "2.0h: brain/embed_client.rs — ai-gateway embeddings call (TASK-AI-019 via TASK-AI-022 policy); residency + spend-cap headers; backoff + pending_embed_retry"
  ...
  - "2.0h: brain/access_scope.rs — TASK-EVAL-001 per-subject access predicate applied AFTER tenant RLS; recall caller entitlement check; deny-by-default on unknown subject"
  - "1.0h: brain/provenance.rs — map every embedding row + summary to its l1_audit_log source row(s); chain_anchor carried for tamper detection on recall (reuse TASK-MEMORY-101 verify)"
  ...
  - "1.5h: tests — brain_recall_access_scope_test (tenant RLS + TASK-EVAL-001 subject scope; closest neighbour the caller can't see is excluded; deny-by-default)"
...
10. **MUST** verify `chain_anchor` for each returned hit against Layer 1 (reuse TASK-MEMORY-101 §1 #4 / TASK-MEMORY-108 §1 #10): recompute `SHA-256(canonical(Layer 1 row at source_seq))`; on mismatch, drop the hit and emit a sev-1 `memory_brain_chain_anchor_mismatch{tenant_id, source_seq}` event. The derived index is never trusted over a tampered chain.
...
20. **SHOULD** weight recall by TASK-MEMORY-114 write-time importance and TASK-MEMORY-113 recency decay where available, so an evaluation surfaces material recent evidence ahead of stale noise; absence of those signals MUST NOT block recall (graceful fallback to raw cosine + RRF).
...
**Why chain_anchor verify on recall (§1 #10, reuse TASK-MEMORY-101/108).** The derived index can lag a Layer-1 tamper. Recomputing the anchor from the current Layer-1 row at read time catches drift: a mismatch means the chain under this hit changed, so the hit is dropped and a sev-1 fires rather than feeding stale or tampered evidence into an evaluation.

**Why importance + recency weighting is a SHOULD, not a MUST (§1 #20).** TASK-MEMORY-114 (write-time importance) and TASK-MEMORY-113 (recency decay) sharpen ranking, but the brain must work before they are wired in this tenant. Treating them as graceful enhancements keeps the dependency edge soft: recall degrades to raw cosine + RRF without them, and improves automatically once they are present.
...
CREATE TABLE brain_event_embedding (
    ...
    source_seq         BIGINT NOT NULL,              -- TASK-MEMORY-121 event seq (the cursor key)
    audit_row_id       TEXT   NOT NULL,              -- provenance pointer into l1_audit_log
    subject_id         UUID   NOT NULL,              -- whose interaction (TASK-EVAL-001 access subject)
    channel_id         UUID,                         -- where (chat channel / module surface), nullable
    kind               TEXT   NOT NULL,              -- event kind from TASK-MEMORY-121
...
pub struct RecallQuery {
    pub q: String,
    pub subject_scope: Option<Vec<Uuid>>,   // optional narrowing; still re-checked against TASK-EVAL-001
...
pub async fn ingest_one_tenant(tenant_id: Uuid, pool: &PgPool, gw: &EmbedClient) -> anyhow::Result<()> {
    ...
    let events = event_stream::read_after(tenant_id, cursor).await?;   // TASK-MEMORY-121 source
...
pub async fn recall(q: RecallQuery, tenant_id: Uuid, caller: &Caller,
    ...
    let mut out = Vec::new();
    for h in hits {
        // §1 #10: read-time chain_anchor verify against Layer 1 (reuse TASK-MEMORY-101)
        if !chain_anchor_verify::matches_layer1(&h, pool).await? {
            metrics::chain_anchor_mismatch(tenant_id, h.source_seq);
            continue;
        }
        // §1 #8: tenant RLS already applied at SQL; now the TASK-EVAL-001 per-subject access predicate.
...
impl EmbedClient {
    ...
    /// ZDR flag, model alias, and spend cap (TASK-AI-022). Never contacts a provider directly.
...
1. **Event ingested → embedding visible** — append a TASK-MEMORY-121 event; within the ingest-lag SLO a `brain_event_embedding` row exists with the event's `audit_row_id` and `tier='hot'`.
...
10. **TASK-EVAL-001 subject scope excludes the closest neighbour** — seed an event for subject S the caller may NOT see that is the top cosine neighbour of the query; recall EXCLUDES it (not deranks) and increments `memory_brain_recall_access_denied_total{reason="subject_scope"}`.
11. **Deny-by-default on unknown subject** — recall over a subject with no TASK-EVAL-001 entitlement returns 0 hits for that subject; `reason="unknown_subject"` increments.
...
19. **Store-ACL on summary writes** — the `brain-ingest` actor writing a subject summary into an ACL-`deny` subtree is rejected with a `memory.acl_denied` aux row (reuse TASK-MEMORY-117).
...
22. **Importance + recency weighting when present** — with TASK-MEMORY-114 / TASK-MEMORY-113 signals available, recent high-importance evidence ranks ahead of stale noise; absent those signals, recall still returns (raw cosine + RRF).
...
async fn recall_excludes_closest_neighbour_outside_access_scope() {
    // The single closest cosine neighbour belongs to a subject the caller may NOT see (TASK-EVAL-001).
    let env = BrainTestEnv::new().await;
...
- **TASK-MEMORY-121** — interaction-event schema; the event shape this worker consumes (`source_seq`, subject, channel, kind, body, `audit_row_id`).
- **TASK-MEMORY-122** — capture emitters; produce the events into the stream this worker tails.
- **TASK-MEMORY-101** — Layer-2 ingest base + `chain_anchor` compute/verify + canonical Layer-1 bytes (reused for read-time tamper detection).
- **TASK-MEMORY-108** — search building blocks reused (RRF, PGroonga full-text fallback, snippet build).
- **TASK-MEMORY-113 / TASK-MEMORY-114** — recency decay + write-time importance signals (SHOULD-weight recall; soft edge).
- **TASK-MEMORY-117** — per-store ACL; governs the subtree the worker writes subject summaries into.
- **TASK-AI-019** — BGE-M3 embedding model (invoked only via the gateway).
- **TASK-AI-020** — BGE reranker (optional precision polish on recall candidates).
- **TASK-AI-022** — ai-gateway tenant policy: residency pin, ZDR, model alias, spend cap (the embeddings path the worker calls).
- **TASK-AUTH-003** — RLS pattern + `TENANT_SCOPED_TABLES` registry (add the three brain tables).
- **TASK-AUTH-004** — JWT (tenant_id + caller identity on recall).
- **TASK-EVAL-001** — per-subject access rules (the recall access predicate; **blocks** nothing here but is consulted at recall time).
...
- Cross-encoder rerank (TASK-AI-020) wired into the recall candidate set by default — slice 2.
- Cold-tier object-storage offload (raw bodies to S3-compatible store, pointer kept) — slice 4+.
- Recall personalisation by reviewer role (manager vs HR vs self-view) beyond the TASK-EVAL-001 boundary — slice 4+.
...
| TASK-EVAL-001 subject scope excludes top neighbour | access predicate | Hit excluded (not deranked); `subject_scope` counter | By design |
| Unknown subject (no entitlement record) | access predicate deny-by-default | 0 hits for that subject; `unknown_subject` counter | Operator grants entitlement in TASK-EVAL-001 |
...
| Embedding dim mismatch (gateway returns wrong dim) | sqlx schema check | INSERT fails; sev-2 | Investigate TASK-AI-019 / gateway |
...
| Store-ACL denies summary write | TASK-MEMORY-117 check | `memory.acl_denied` aux row; summary not written to that subtree | Operator grants `brain-ingest` write on the subtree |
| TASK-MEMORY-121 stream lags / stalls | cursor not advancing + lag metric | Recall serves stale (older) evidence; sev-3 on lag SLO breach | Investigate emitters (TASK-MEMORY-122) |
...
- The access boundary is load-bearing, not cosmetic: tenant RLS at the DB plus the TASK-EVAL-001 per-subject predicate at recall, applied as an EXCLUDE (a closest neighbour the caller may not see never appears) with deny-by-default on unknown subjects. This is the technical expression of the strategy note's "access control" and "human in the loop" principles and Vietnam's PDPD purpose-limitation.
- Provenance is what makes the brain defensible: every hit carries a pointer to the exact, hash-chained audit row(s) it came from, so TASK-EVAL-003 cites events, not vibes, and a reviewer or the employee can verify each citation against the immutable chain.
- Embeddings and summaries are generated ONLY through the ai-gateway, which pins residency, enforces ZDR, and charges the tenant spend cap (TASK-AI-022). Over-cap degrades to `pending_*` with backoff; the worker has no code path that contacts a model provider directly, by design.
...
*End of TASK-MEMORY-123. Status: draft (enters draft → ready_to_implement via task-audit).*
