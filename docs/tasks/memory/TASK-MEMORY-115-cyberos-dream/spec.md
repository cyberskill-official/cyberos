...
related_tasks: [TASK-MEMORY-112, TASK-MEMORY-113, TASK-MEMORY-114, TASK-MEMORY-116, TASK-MEMORY-119, TASK-MEMORY-120, TASK-CUO-105]
depends_on: [TASK-MEMORY-112, TASK-MEMORY-113, TASK-MEMORY-114]
blocks: [TASK-MEMORY-116]
...
source_decisions:
  ...
  - DEC-213 (Dream inputs are EITHER recent audit rows OR the session-transcript ledger from TASK-MEMORY-119; TASK-MEMORY-115 ships with audit-row input as default; transcript input is a stretch via TASK-MEMORY-119 integration)
...
subtasks:
  ...
  - "5.0h: cyberos/core/dream/detectors/patterns.py — cross-session pattern detection over `episode.logged` rows (TASK-MEMORY-112 dependency); identifies recurring task/outcome combos; emits `new` proposals under `memories/refinements/`"
...
3. **MUST** support the following four detector types (closed enum DEC-214) that produce a `DreamProposal`:
    ...
    | `patterns` | `new` | finds recurring task/outcome combos across `episode.logged` rows (TASK-MEMORY-112 dep); proposes a NEW memory under `memories/refinements/` summarising the pattern |
...
9. **MUST** select the active LLM invoker via the same chain as TASK-MEMORY-114 (`--invoker {mock|anthropic}` → `CYBEROS_DREAM_INVOKER` → `manifest.dream.invoker` → default `mock` if no API key). Same `CYBEROS_DISABLE_LLM=1` escape hatch.
...
14. **MUST** preserve full memory integrity on dream apply — if any single proposal's write fails (e.g. ACL denied per TASK-MEMORY-117), the entire apply transaction rolls back to the snapshot HEAD seq. No half-applied diffs.
...
16. **MUST** track quality metrics over time. Each `dream.complete` row's `quality_metrics` MUST include: `{proposals_count_by_kind, applied_count_by_kind, fallback_count, avg_invoker_latency_ms, scope_size_memories, scope_size_audit_rows}`. These feed TASK-MEMORY-120's `cyberos history` retrospective view.
...
**Why per-proposal aux row instead of one bulk row (§1 #15).** TASK-MEMORY-120 (`cyberos history`) needs per-path provenance: "this memory was tombstoned by dream X, proposal Y." Bulk rows force history to re-parse the payload to find which path was affected. Per-proposal rows are cheap (audit chain is designed for high cardinality) and make history queries trivial.
...
async def run(
    ...
    diff = DreamDiff(
        ...
        scope=scope or "*",
        since=(started_at - since).isoformat(),
        input_sessions=[],  # populated when TASK-MEMORY-119 lands
        proposals=dedup,
        metrics={
...
TASK-MEMORY-115. Dream-runner and dream-applier are distinct identities;
...
| Single proposal write fails (ACL denied per TASK-MEMORY-117) | writer exception in transaction | entire batch rolls back | Adjust ACL or skip that proposal id |
