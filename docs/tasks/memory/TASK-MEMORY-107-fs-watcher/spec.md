...
related_tasks: [TASK-MEMORY-101, TASK-MEMORY-102, TASK-MEMORY-104, TASK-MEMORY-105, TASK-MEMORY-108, TASK-MEMORY-109, TASK-MEMORY-110, TASK-MEMORY-111]
depends_on: [TASK-MEMORY-102, TASK-MEMORY-105, TASK-MEMORY-106]
blocks: [TASK-MEMORY-108, TASK-MEMORY-109, TASK-MEMORY-110, TASK-MEMORY-111]
...
subtasks:
  ...
  - "1.0h: emit.rs — bridge to TASK-MEMORY-101 memory_writer; carries W3C TraceContext; retries with exp backoff"
...
**Why no `cyberos memory capture stop` command?** The daemon is managed by systemd / launchd (TASK-MEMORY-110); stopping it is `systemctl stop` etc. Adding a CLI stop creates two ways to do the same thing — the systemd unit is authoritative.
...
19. **Doctor gate refuses to start** — manifest has dangling symlink (TASK-MEMORY-105 `WatchedFolderResolvable` fails) → daemon exits 7 (`InternalError`) with stderr explaining the doctor failure.
...
- **TASK-OBS-003, TASK-OBS-005** — metric + trace conventions.
...
- Encrypted-at-rest body hashing — slice 3+; need TASK-MEMORY-111 PII detection first to know what to encrypt.
...
| Daemon OOMs | OOM-killer | systemd restarts (TASK-MEMORY-110); resync replays | Operator investigates leak |
| FSEvents drops events (macOS) | mtime mismatch on next event | Caught by next resync OR next event for that file | Periodic resync (TASK-MEMORY-110 schedules hourly) |
