...
related_tasks: [TASK-MEMORY-108, TASK-MEMORY-112, TASK-MEMORY-113, TASK-MEMORY-115, TASK-MEMORY-119]
depends_on: []
blocks: []
...
8. **MUST** include rows for ALL chain kinds that touch the path, not just `put/move/delete`. Specifically: `episode.logged` (TASK-MEMORY-112), `memory.importance_scored` (TASK-MEMORY-114), `dream.proposal_applied` (TASK-MEMORY-115), `memory.acl_denied` and `memory.precondition_failed` (TASK-MEMORY-117/118 attempt rows). These rows surface as "annotation events" — they appear in the history but don't trigger a body diff.
...
- **TASK-MEMORY-108 (related)** — audit chain reader infrastructure is reused. History is a different projection over the same data.
- **TASK-MEMORY-112 (related)** — `episode.logged` aux rows are rendered as history entries.
- **TASK-MEMORY-113 (related)** — `meta.importance` field diffs surface in `frontmatter_diff`.
- **TASK-MEMORY-114 (related)** — `memory.importance_scored` rows are rendered as history entries.
- **TASK-MEMORY-115 (related)** — `dream.proposal_applied` rows surface; `extra.dream_id` annotation rendered.
- **TASK-MEMORY-117 (related)** — `memory.acl_denied` rows surface as attempt-history.
- **TASK-MEMORY-118 (related)** — `memory.precondition_failed` rows surface similarly.
- **TASK-MEMORY-119 (related)** — `extra.session_id` annotation rendered.
