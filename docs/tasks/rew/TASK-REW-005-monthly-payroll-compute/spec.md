...
related_tasks: [TASK-REW-001, TASK-REW-002, TASK-REW-004, TASK-REW-006, TASK-REW-009, TASK-MEMORY-111]
...
source_decisions:
  - DEC-2190 2026-05-17 — Monthly payroll compute orchestrates: P1+P2 + accrued P3 → gross → deductions (TASK-REW-004) → net per member
...
7. **MUST** emit 6 memory audit kinds per DEC-2194. PII per TASK-MEMORY-111: amounts SHA256.
...
11. **MUST** be deterministic — replay produces byte-identical totals per TASK-REW-002.
...
- §11.1 Compute reads encrypted comp via TASK-REW-001 decrypt (CFO-only context); uses session-scoped key.
...
- §11.5 TASK-REW-006 PDF generates per payslip_row post-commit.
