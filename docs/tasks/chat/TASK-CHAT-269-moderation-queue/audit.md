...
TASK-CHAT-269 specifies the workspace moderation queue: 21 normative §1 clauses, one migration (retention),
three administrator routes, a fail-closed role gate that is chat's first consumer of the TASK-AUTH-101 `roles`
...
The draft checked `claims.roles.contains("tenant-admin")` against a `#[serde(default)]` vector, and TASK-AUTH-101
allows a grace window in which tokens may lack the RBAC claims entirely. So every token issued before
TASK-AUTH-101 shipped - all of them still valid - would have deserialised to `roles: []`... and an early
...
the same three-valued-logic trap that killed the partial unique index in TASK-CHAT-267 (its ISS-007), wearing a
...
TASK-CHAT-267's snapshot is justified while the report is open, because the sender can destroy the original. Once
...
- Implementation predates this leg (commit f62b018, parallel session); the ship queue surfaced the
  ...
  docs/tasks/.workflow/TASK-CHAT-269/review-packet.md); 3 recorded path deviations matching
  repo conventions.
