...
- TASK-MCP-008 + TASK-MCP-006: migration 0016 (mcp_elicitations) + src/elicitation_pg.rs store-of-record. The
...
- TASK-MCP-007: worker pool, NATS progress, checkpoints + resume, the long_running annotation + tools/call
...
Flip on green:
- TASK-MCP-004 OAuth 2.1 + PKCE: implementing -> done. The full flow is built and gate-green (authorize,
...
- TASK-MCP-007 tasks primitive: deferred = mcp_tasks table + RLS, worker pool, NATS progress, checkpoints
  and resume, the long_running annotation + tools/call async routing, idempotency, rate limit, TTL sweep.
- TASK-MCP-008 elicitation: deferred = mcp_elicitations table + RLS, KMS, NATS, LISTEN/NOTIFY, S3
  file_upload, rate limit, sweeper, prune.

Unchanged (separate remaining slice):
- TASK-MCP-003 sep986 naming: stays implementing. Slice 3 left: the CI grep gate (scripts +
