...
related_tasks: [TASK-MCP-001, TASK-MCP-002, TASK-MCP-003, TASK-MCP-004, TASK-MCP-005, TASK-MCP-007, TASK-MCP-008, TASK-AUTH-004, TASK-AI-003, TASK-MEMORY-111, TASK-OBS-007]
...
source_decisions:
  ...
  - DEC-1045 2026-05-17 — Bypass token: trusted internal callers (per TASK-AUTH-004 JWT carrying `mcp_gating_bypass=true` scope claim) skip gating entirely; restricted to system-tenant callers only
  ...
  - DEC-1048 2026-05-17 — Tool annotations from TASK-MCP-002 registration are STORED as part of the module-registration row; annotation drift between registry + tool schema is forensic event sev-1
...
subtasks:
  ...
  - "0.3h: gating/elicit.rs — TASK-MCP-008 delegate (placeholder 503 at slice 2)"
...
18. **MUST** PII-scrub audit rows per DEC-1060 + task-audit skill rule 18. `request_payload_sha256` only in memory chain; raw payload in MCP gateway logs (RLS-scoped, 7-day retention).
...
24. **MUST NOT** allow bypass-token scope grant outside system-tenant per DEC-1045. TASK-AUTH-004 JWT mint guard: if `tenant_id != system_tenant` AND `mcp_gating_bypass` in scope_grants → reject mint with `invalid_scope_for_tenant`.
...
**Why elicit-mode placeholder via 503 at slice 2 (§1 #13, DEC-1055)?** TASK-MCP-008 (Elicitation) is on the roadmap but not yet shipped. Returning 503 instead of silently degrading to confirm mode (a) makes the missing capability visible, (b) doesn't violate the tenant_admin's policy choice (they picked elicit; we don't pretend confirm is equivalent), (c) gives operators a metric to track when TASK-MCP-008 demand materialises.

**Why scope-claim approach for bypass (§1 #14, DEC-1045)?** Scope claims are issued at JWT mint (TASK-AUTH-004 controlled); they're tamper-evident (signed); they're auditable (scope grants logged at mint time). Alternative (separate bypass-token endpoint) duplicates auth surface. Scope claim integrates with existing TASK-AUTH-004 + TASK-AUTH-101 RBAC.

**Why versioned policy (§1 #3)?** Same rationale as TASK-PORTAL-002 brand pack versioning — tenant_admin wants rollback safety on commercial-grade config changes. Versioning is cheap (one row per save) + valuable (undo button).
...
**Upstream (depends_on):**
- **TASK-MCP-001** spec compliance — tools/call dispatch path; gating intercepts here.

**Cross-module (related_tasks):**
- **TASK-MCP-002** Per-module registration — annotations stored in registry.
- **TASK-MCP-003** SEP-986 naming — tool_id format validated in policy.
- **TASK-MCP-004** OAuth 2.1 PKCE — JWT bearer with scope_grants for bypass.
- **TASK-MCP-005** PRM — gating-supported tool scopes appear in per-module PRM.
- **TASK-MCP-007** Tasks primitive — destructive tasks confirm at start; subsequent polls don't re-confirm.
- **TASK-MCP-008** Elicitation — `elicit` mode delegates here (slice-2 placeholder).
- **TASK-AUTH-004** JWT mint — bypass-scope guard at mint.
- **TASK-AI-003** memory audit-row bridge — 5 new kinds.
- **TASK-MEMORY-111** PII scrubbing — payload SHA only in chain.
- **TASK-OBS-007** Auto-runbook — sev-1 drift + policy events route to CHAT.
...
| Elicit mode requested but TASK-MCP-008 not shipped | mode resolver | 503 + `elicitation_not_yet_supported` | Tenant_admin temporarily falls back to confirm mode |
...
**§11.2** Pending confirmations are pruned by `cyberos_pruner` role's DELETE grant + scheduled job (same pattern as TASK-TEN-003 `stripe_api_calls`).

**§11.3** Bypass scope is granted ONLY via TASK-AUTH-004's admin mint endpoint with explicit operator approval; not a self-service grant.
...
**§11.10** Decision audit at sev-3 sampled at 1% via TASK-OBS-006 tail-sampling for routine flow; rejected decisions emitted at 100% (security-relevant).
...
**§11.19** Per-tenant audit log retention: 90 days (matches TASK-TEN-101 signup_sessions pattern); after 90d, summary statistics retained, individual rows pruned.

---

*End of TASK-MCP-006 spec.*
