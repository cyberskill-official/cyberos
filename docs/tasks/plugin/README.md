...
This is **distinct** from the MCP gateway (`docs/tasks/mcp/TASK-MCP-001..008`) which owns the protocol layer. PLUGIN sits on top: packaging, runtime adapters, capability advertisement, install-time auth, audit emission, marketplace distribution.
...
| [TASK-PLUGIN-003](TASK-PLUGIN-003-slash-commands/spec.md) | MUST | 1 | 4 | Canonical slash-commands — `/cyberos-run`, `/cyberos-memory`, `/cyberos-skill-list`, `/cyberos-route` markdown definitions |
| [TASK-PLUGIN-004](TASK-PLUGIN-004-skill-playbooks/spec.md) | MUST | 2 | 6 | Skill playbooks bundle — Anthropic-Agent-Skills SKILL.md files teaching hosts how to chain plugin tools correctly |
| [TASK-PLUGIN-005](TASK-PLUGIN-005-oauth-pkce-auth/spec.md) | MUST | 2 | 8 | Plugin OAuth-PKCE authentication — install-time authorize + refresh-token rotation against `auth.cyberskill.world` |
| [TASK-PLUGIN-006](TASK-PLUGIN-006-memory-audit-emission/spec.md) | MUST | 2 | 6 | memory audit emission — every install/update/uninstall/invoke produces a `plugin.*` audit row; idempotent retry queue |
| [TASK-PLUGIN-007](TASK-PLUGIN-007-multi-runtime-adapters/spec.md) | MUST | 3 | 10 | Multi-runtime adapters — `cyberos-plugin pack --target {claude-code,cursor,cowork,codex-cli}` emitters; deferred targets in P2 |
| [TASK-PLUGIN-008](TASK-PLUGIN-008-marketplace-distribution/spec.md) | SHOULD | 3 | 6 | Marketplace distribution — `cyberos-plugin publish` pushes signed bundle to `plugins.cyberskill.world` + mirrors to `agentskills.io`; revenue-share + vetted-badge |
...
- **MCP**: TASK-PLUGIN-002 → TASK-MCP-001 (protocol spec compliance), TASK-MCP-003 (SEP-986 naming), TASK-MCP-006 (tool annotation gating); TASK-PLUGIN-005 → TASK-MCP-004 (OAuth-PKCE flow shape)
...
- **TEN**: TASK-TEN-005 (vertical pack pricing) needs TASK-PLUGIN-008 for the publish + revenue-share surface
- **PORTAL**: TASK-PORTAL-005 (branded Genie chat) needs TASK-PLUGIN-007 Cowork adapter so the white-label client ships as a plugin
