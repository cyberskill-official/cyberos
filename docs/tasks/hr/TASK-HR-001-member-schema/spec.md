...
source_decisions:
  ...
  - DEC-202 (CCCD id encrypted at rest with separate KMS keyspace per TASK-HR-003; column declared here as `cccd_encrypted BYTEA`)
  - DEC-203 (compensation fields — base_salary, bonus, equity_units — categorically forbidden in this schema; CI gate TASK-REW-010 asserts)
  - DEC-204 (leave_balance is materialised view of TASK-HR-004 leave_entries; never written directly here — read-only column)
...
disallowed_tools:
  ...
  - allow direct UPDATE of `leave_balance` (per DEC-204; column is a materialised view of TASK-HR-004 leave_entries)
  ...
  - store raw CCCD ID in the schema (per DEC-202; only `cccd_encrypted BYTEA` is permitted — encryption done at TASK-HR-003 boundary)
...
3. **MUST** declare the closed `member_status` PostgreSQL enum with exactly 6 values (per DEC-200): `'candidate'`, `'probation'`, `'active'`, `'on_leave'`, `'suspended'`, `'terminated'`. Adding a 7th value is an ADR (mirrors TASK-AUTH-101's role-catalogue discipline).
...
7. **MUST** trigger emission of exactly one `hr.member_status_changed` memory audit row per status transition (per DEC-208), atomically with the DB write (audit-before-action per task-audit skill rule 25). The row carries `{member_id, tenant_id, from_status, to_status, changed_by_subject_id_hash16, reason, trace_id, ts_ns}`.
...
16. **MUST** support idempotent creation via `Idempotency-Key` header (same semantics as TASK-AUTH-002 §1 #6). Repeat POST with same key + same body → return existing member. Repeat POST with same key + different body → 409 `idempotency_key_reuse`.
...
26. **MUST** ship the `sabbatical_accrued_days(start_date DATE)` SQL function with deterministic output: returns `0` if `years_of_service < 5`; otherwise returns `LEAST(years_of_service - 5 + 1, 30)`. Pure function; same input → same output (per task-audit skill rule 27).
...
**Cross-module:**
- **TASK-AUTH-101** — RBAC; `Resource::HrMember` + `Action::Admin/Read` are in the matrix.
- **TASK-AI-003** — memory audit bridge; receives `hr.member_created`, `hr.member_updated`, `hr.member_status_changed`, `hr.member_cccd_accessed`.
- **TASK-MEMORY-111** — PII detection; applied to `full_name` + `email` before chain commit.
...
| CCCD photo URL accidentally stored as TEXT | TASK-HR-003's keyspace enforcement | Out-of-scope here | TASK-HR-003 |
...
- **`level::executive` is the ceiling** — `founder` is the AUTH RBAC role, distinct from the HR level. Founders are typically `level=executive` Members PLUS hold the `founder` role per TASK-AUTH-101.
