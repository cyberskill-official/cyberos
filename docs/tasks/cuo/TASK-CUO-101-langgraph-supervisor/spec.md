...
source_decisions:
  ...
  - DEC-161 (LiteLLM as the LLM abstraction inside CUO; routes provider calls THROUGH AI Gateway TASK-AI-008, never direct)
  ...
  - DEC-166 (persona JWT carries `agent_persona: cuo-<persona-key>@<semver>` per TASK-AI-014; the supervisor validates it at every state entry)
  ...
  - DEC-170 (CUO never invokes a destructive skill on confidence alone; the capability broker TASK-SKILL-104 enforces destructive-confirm regardless of supervisor decision)
...
allowed_tools:
  ...
  - file_read: docs/tasks/ai/TASK-AI-008-multi-provider-router/spec.md
...
disallowed_tools:
  ...
  - silently auto-invoke a destructive skill (per DEC-170 — capability broker TASK-SKILL-104 enforces; supervisor refuses regardless)
...
subtasks:
  ...
  - "1.0h: nodes/record.py — build cuo.routing_decision row; chain via memory_writer TASK-MEMORY-101 + TASK-AI-003"
  - "0.5h: litellm_proxy.py — LiteLLM-shaped client that routes through AI Gateway TASK-AI-008"
  - "0.5h: persona.py — 11-persona catalogue (Genie + 10 C-level) + defer-to-human matrix"
  - "0.5h: checkpointer.py — in-memory checkpointer scaffold (Postgres lands in TASK-CUO-102)"
...
10. **MUST** validate the caller's JWT carries `agent_persona: cuo-<persona-key>@<semver>` matching the requested persona. Missing or mismatched persona claim → `403 FORBIDDEN` (exit 77) with body `{"error":"persona_mismatch","claimed":"<x>","requested":"<y>"}`. Validation uses TASK-AUTH-101's `RoleMatrix` for `agent-persona` role + TASK-AI-014's persona-version stamping.
...
24. **MUST** PII-scrub the `query` field of the memory audit row using `cyberos-memory-pii` rules per task-audit skill rule 18, BEFORE chain commit. The original query is retained in the supervisor's OTel span (transient, < 30-day retention via TASK-OBS-006 tail sampling); the memory row holds the scrubbed form for long-term storage.

25. **MUST** support **multi-step chain entry stub**: the supervisor returns `{decision, next_step: null}` at slice 2. TASK-CUO-104 (topological chain walk, slice 3+) consumes `next_step` to compose multi-skill flows. The slice-2 stub MUST set `next_step = null` unconditionally — never a stale value.

26. **MUST** emit `cuo.routing_decision` row carrying `next_step: null` at slice 2; the field is **PRESENT but null** (not absent). This contract guarantees forward-compatibility with TASK-CUO-104's chain-walk consumer; absent-field would force TASK-CUO-104 to handle both shapes.
...
**Why `next_step: null` field explicitly present (§1 #25, §1 #26)?** TASK-CUO-104 ships multi-step chain walks; the consumer reads `decision.next_step` to plan the next supervisor invocation. If slice 2 omits the field, TASK-CUO-104 must handle both shapes (field-present + field-absent), which is two code paths to test forever. Setting it explicitly to `null` makes the contract one-shape — forward-compatible without ambiguity.
...
class CuoState(TypedDict):
    ...
    next_step: None              # slice 2 always null; TASK-CUO-104 fills
...
def build_supervisor_graph() -> StateGraph:
    ...
    return g.compile(checkpointer=InMemoryCheckpointer())  # Postgres in TASK-CUO-102
```
...
```python
# cuo/cuo/supervisor/litellm_proxy.py
"""LiteLLM-shaped client that ALWAYS routes through AI Gateway TASK-AI-008.
...
async def litellm_call_via_gateway(prompt: str, tenant_id: str) -> str:
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        resp = await client.post(
            ...
            json={
                "tenant_id": tenant_id,
                "model": "haiku@cuo-supervisor",   # alias resolved by TASK-AI-006
...
def build_routing_decision_row(state: CuoState) -> dict:
    ...
    """
    pii_scrubbed_query = apply_memory_pii_rules(state["query"])  # TASK-MEMORY-111
    return {
        ...
        "next_step": None,    # slice 2 contract; TASK-CUO-104 fills
...
12. **Destructive skill bypass blocked** — skill annotated `destructive: true` at confidence 1.0 → capability broker TASK-SKILL-104 gates via Elicitation; supervisor's invoke node refuses to override.
...
- **TASK-INV-010** — dunning draft CUO.
- **TASK-OKR-006, TASK-OKR-007** — Monday digest + quarterly retro CUO drafts.
- **TASK-PROJ-011, TASK-PROJ-012** — blocker notification + cycle-review draft (already specced; they consume CUO via webhook).
- **TASK-RES-004** — hiring memo CUO draft.

**Cross-module (informational):**
- **TASK-AI-014** — persona-version stamping; `cuo-<persona-key>@<semver>` is validated at parse node.
- **TASK-AUTH-101** — RBAC catalogue; `agent-persona` role must be present in claims.
- **TASK-AI-003** — memory audit bridge; receives `cuo.routing_decision`, `cuo.llm_cascade_timeout`, `cuo.persona_defer_block`, `cuo.dry_run`, `cuo.persona_unknown`.
- **TASK-SKILL-104** — capability broker; gates destructive-skill invocation regardless of supervisor decision.
- **TASK-MEMORY-111** — PII detection; applied to `query` field before chain commit.
- **TASK-AI-022** — OTel trace emission; supervisor spans correlate via `traceparent`.
...
- **Multi-step chain walk** — TASK-CUO-104. Slice 2 ships `next_step: null` stub.
- **Postgres checkpointer** — TASK-CUO-102. Slice 2 ships in-memory scaffold.
- **LLM cascade replay** — TASK-CUO-103 captures full prompt + seed for Art. 12 replay; slice 2 stores only model + path.
...
| Persona JWT version mismatch (`cuo-cfo@0.3.0` vs catalogue `cuo-cfo@0.4.1`) | parse-node check via TASK-AI-014 | 403 with `persona_version_mismatch` | Refresh-token cycle |
...
| Destructive skill picked at high confidence | `invoke` node checks `destructive: true` annotation | Capability broker TASK-SKILL-104 gates via Elicitation | User confirms via Elicitation flow |
...
| Caller's `traceparent` malformed | parse-node validation | Generate fresh trace at trust boundary per task-audit skill rule 22 | None — designed |
...
- **`InMemoryCheckpointer` is intentional for slice 2** — checkpointing is for state recovery on restart, but slice-2 requests are stateless per request (no multi-step chains yet). Adding the Postgres dependency now would buy us complexity without value; TASK-CUO-102 ships when chains land.
...
- **W3C `traceparent` is generated at the trust boundary** when caller doesn't provide one (per task-audit skill rule 22). The supervisor entry is a trust boundary — chat/email/etc. may or may not propagate.
- **`OTel sampling 1%` for steady-state spans, 100% for non-success** — the high-volume happy path is sampled to keep storage costs sane; the rare error path is always captured for debugging. Matches TASK-OBS-006 tail-sampling pattern.
...
- **Persona JWT format `cuo-<persona-key>@<semver>`** — matches TASK-AI-014; the version part lets us roll out persona prompt changes without breaking existing tokens (with grace window).
...
- **`apply_memory_pii_rules` is the TASK-MEMORY-111 entry point** — same rules used by every memory audit row builder; consistent PII scrubbing across modules.
- **`current_trace_id_hex` formats via `{}` not `{:?}`** — task-audit skill rule 24; Display, not Debug.
- **The CLI's exit codes follow `cyberos-cli-exit`** — shared crate per task-audit skill rule 9; tests assert numeric values to catch drift if the shared crate's mapping changes.
