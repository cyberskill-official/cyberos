...
related_tasks: [TASK-CUO-101, TASK-CUO-103, TASK-MEMORY-111]
...
5. **MUST** expose endpoints:
   ```text
   GET /v1/cuo/runs/{run_id}/checkpoints   (TASK-CUO-103 replay reads these)
   ```

6. **MUST** emit 3 memory audit kinds per DEC-2324. PII per TASK-MEMORY-111: state_json hashed; ids ok.
...
- §11.4 Archive cron via TASK-MCP-007 monthly; DROP PARTITION for oldest.
