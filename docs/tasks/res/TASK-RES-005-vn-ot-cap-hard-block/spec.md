...
related_tasks: [TASK-HR-005, TASK-RES-002, TASK-TIME-007, TASK-MEMORY-111]
depends_on: [TASK-HR-005]
blocks: []
...
3. **MUST** define consent table at migration `0005`:
   ...
   CREATE TABLE res_ot_consent (
     ...
     consent_doc_id UUID,  -- TASK-DOC-001 reference
...
4. **MUST** integrate with TASK-RES-002 validator — checker called pre-commit.
...
6. **MUST** emit 4 memory audit kinds per DEC-2064. PII per TASK-MEMORY-111: hours SHA-256 hashed.

7. **MUST** thread trace_id from TASK-RES-002 propose → checker → audit.
...
- §11.2 Cap values: regular_weekly_cap, consent_cap, annual_cap fetched via TASK-HR-005 policy.
...
- §11.5 Integration test simulates TASK-RES-002 propose flow end-to-end.
