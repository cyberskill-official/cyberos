...
subtasks:
  ...
  - "0.5h: PII-scrub processor (regex+attribute drop; defence-in-depth vs TASK-AI-022 §1 #15 caller-side prevention)"
  ...
  - "0.5h: Grafana provisioning (datasources + folder for TASK-OBS-002 dashboards)"
...
| Cross-service trace context broken | Tempo shows orphan spans | Investigate W3C traceparent propagation (TASK-AI-022) | Engineer fixes propagation gap |
| PII detected by scrub | metric `obs_collector_pii_scrub_total` increment | Sev-1 + dropped from spans | Investigate caller; add typed-key per TASK-AI-022 §1 #15 |
...
| `tenant_id` resource attr missing | Span ingested without label | Logs/traces unsearchable by tenant | Caller MUST set resource attr (TASK-AI-022) |
...
- The PII-scrub processor is defence-in-depth. Caller-side prevention (TASK-AI-022 §1 #15 typed attribute keys + AST lint) is the primary control. The scrub catches regressions; sev-1 alarm forces investigation.
...
- Retention floors (30d/90d/7d) reflect operational + compliance lookback windows. P2 extends with S3 backend (Loki) + Mimir (Prometheus) + sampled Tempo (TASK-OBS-006).
- Tenant_id resource attribute is the TASK-OBS-002 query primitive — without it, tenant-aware filtering is impossible. TASK-AI-022 + every CyberOS service MUST emit it.
...
*End of TASK-OBS-001. Status: draft (10/10 target).*
