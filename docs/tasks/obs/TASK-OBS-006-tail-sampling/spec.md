...
related_tasks: [TASK-OBS-001, TASK-OBS-005, TASK-AI-022, TASK-AI-021]
...
subtasks:
  ...
  - "1.0h: flag_tenant.sh + flag_tenant.rs hook (TASK-AI-021 integration)"
  ...
  - "1.0h: per-route p99 latency-budget computation (read from TASK-OBS-003 metrics)"
...
```rust
// services/ai-gateway/src/cli/flag_tenant.rs (TASK-AI-021 subcommand)
...
- **TASK-OBS-001** — OTel collector running.
- **TASK-OBS-005** — trace_id propagation (sampling decision is per-trace, not per-span).
- **TASK-AI-021** — operator CLI for flag-tenant.
...
- Per-tenant sampling override beyond flag — slice 4+ with TASK-AI-005 schema extension.
...
- Tail-based sampling at the collector is the sole sampling decision point — services emit 100% (TASK-AI-022 §1 #10), the collector reduces.
...
- The CLI `cyberos-ai flag-tenant` is the operator surface; emits memory audit row; 24h auto-unflag is TASK-AI-021 enhancement (slice 4+).

---

*End of TASK-OBS-006. Status: draft (10/10 target).*
