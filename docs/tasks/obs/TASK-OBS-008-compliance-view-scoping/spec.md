...
related_tasks: [TASK-OBS-002, TASK-OBS-009, TASK-AI-022, TASK-AUTH-004]
...
subtasks:
  ...
  - "1.0h: auth.rs — auditor-role JWT check (extends TASK-AUTH-004 verify)"
...
**Why PII-placeholder defence-in-depth (§1 #12)?** Audit chain SHOULD contain only placeholders (TASK-AUTH-002 + TASK-AUTH-004 + TASK-AI-014 enforce). But regression-bugs could leak. Regex scan at response time catches the regression before the auditor sees raw PII.
...
- The 4 views map 1:1 to the most-common audit regulations. Adding a 5th (PCI DSS) is TASK-OBS-010 placeholder.
...
*End of TASK-OBS-008. Status: draft (10/10 target).*
