...
related_tasks: [TASK-TEN-001, TASK-TEN-002, TASK-TEN-003, TASK-TEN-004, TASK-TEN-102, TASK-TEN-103, TASK-TEN-104, TASK-TEN-107, TASK-AUTH-001, TASK-AUTH-002, TASK-AUTH-004, TASK-AUTH-101, TASK-AUTH-104, TASK-AUTH-107, TASK-PORTAL-001, TASK-PORTAL-002, TASK-AI-003, TASK-MEMORY-111, TASK-EMAIL-001, TASK-OBS-007]
depends_on: [TASK-AUTH-104, TASK-TEN-001, TASK-TEN-002, TASK-TEN-003]
blocks: [TASK-TEN-107, TASK-PORTAL-001, TASK-PORTAL-002]
...
source_decisions:
  ...
  - DEC-821 2026-05-17 — Email-OTP verification before tenant creation; 6-digit code, 10-min TTL, max 5 attempts; OIDC sign-up path (via TASK-AUTH-104) skips OTP because IdP already verified
  - DEC-822 2026-05-17 — Tenant slug is user-chosen with real-time uniqueness check; slug regex per TASK-TEN-001 DEC-321 (`^[a-z][a-z0-9-]{2,40}[a-z0-9]$`); 50 auto-suggestions on collision
  - DEC-823 2026-05-17 — Plan tier defaults to Starter (TASK-TEN-002 DEC-778); user may upgrade to Team/Enterprise inline; Enterprise requires manual sales contact (signup limited to Starter+Team for self-serve)
  ...
  - DEC-825 2026-05-17 — Residency derived from billing_currency: VND→vn-1, SGD→sg-1, EUR→eu-1, GBP→eu-1, USD→us-1 (consistent with TASK-TEN-003 DEC-785)
  - DEC-826 2026-05-17 — Payment method captured via Stripe Elements (for stripe-rail) or VnPay redirect (for VND-rail per TASK-TEN-102 — placeholder) in the same flow; no card details ever touch our backend (PCI SAQ-A scope)
  ...
  - DEC-831 2026-05-17 — TASK-AUTH-107 HIBP breach-check applied to email at signup (informational warning, not blocker) — leaked email still allowed but operator alerted
  ...
  - DEC-835 2026-05-17 — OIDC sign-up path (TASK-AUTH-104 "Sign up with Google/Microsoft") creates the root-admin subject WITHOUT a password; subsequent logins are OIDC-only until tenant_admin enables password fallback
  - DEC-836 2026-05-17 — First-login JWT minted via TASK-AUTH-004 with `signup_session_id` claim; cookies set on the public signup hostname (`signup.cyberos.world` redirects to `<slug>.cyberos.world` after final step)
  ...
  - "DEC-841 2026-05-17 — Founder bypass path: `POST /v1/admin/tenants` (TASK-TEN-001's ops CLI flow) STILL works alongside this; self-serve does NOT replace ops provisioning — they are parallel entry points"
...
build_envelope:
  ...
  new_files:
    ...
    - services/ten/src/signup/oidc_callback.rs                         # TASK-AUTH-104 OIDC return → tenant create
    ...
    - services/ten/src/signup/welcome_email.rs                         # delegates to TASK-EMAIL-001
...
subtasks:
  ...
  - "0.6h: oidc_callback.rs — TASK-AUTH-104 return path → headless tenant creation (skip OTP)"
  - "0.4h: abuse_guard.rs — squatting detector + IP block"
  - "0.4h: welcome_email.rs — TASK-EMAIL-001 delegate + 7d-expiring deeplink"
...
7. **MUST** apply TASK-AUTH-107 HIBP breach-check on the email at `signup_start` (per DEC-831). Result is informational — `breached: true` is appended to the response as `{ ..., breach_warning: { breach_count: N, latest_breach: "<date>" } }`. The signup proceeds; the warning surfaces in the UI as "We noticed your email appears in breaches; you may want to use a different email."
...
20. **MUST** thread W3C `traceparent` across the entire flow (task-audit skill rule 22 + 23 + 24). The `signup_session_id` is logged as a span attribute on every span; a single `trace_id` in the response makes support escalations resolvable.
...
26. **MUST NOT** persist plaintext passwords or plaintext OTPs anywhere. OTPs are HMAC-SHA256-hashed under a server-side secret before Redis insert. Auto-generated root-admin passwords are bcrypt-hashed in `subjects` (TASK-AUTH-002) and emailed once via TASK-EMAIL-001 (DEC-834).
...
GET    /v1/signup/oidc-callback          (public, TASK-AUTH-104 return)
```
...
**Upstream (depends_on):**
- **TASK-AUTH-104** OIDC SSO — Google/Microsoft sign-up path.
- **TASK-TEN-001** Provisioning — `provision_tenant` invoked at signup_complete step 8.
- **TASK-TEN-002** Plan tiers — `plan_tier` selection + per-tier subscription.
- **TASK-TEN-003** Stripe billing — `ensure_customer` + `ensure_subscription` at step 9.

**Cross-module (related_tasks):**
- **TASK-TEN-004** 4-axis metering — first metering events fire post-signup.
- **TASK-TEN-102** VND domestic rail — VNP path placeholder until TASK-TEN-102 ships.
- **TASK-TEN-103** 4-residency provisioning — residency derivation aligns.
- **TASK-TEN-104** Lifecycle — rollback path may invoke termination.
- **TASK-TEN-107** Tenant-admin SPA — first destination post-signup.
- **TASK-AUTH-001** Tenant create — invoked indirectly via TASK-TEN-001.
- **TASK-AUTH-002** Subject create — root-admin creation.
- **TASK-AUTH-004** JWT mint — first-login token.
- **TASK-AUTH-101** RBAC — root-admin gets `tenant_admin` role.
- **TASK-AUTH-107** HIBP breach-check — informational warning at signup.
- **TASK-EMAIL-001** Transactional email — OTP + welcome email + magic-link.
- **TASK-PORTAL-001/002** — depend on TEN-101 for tenant-scoped brand pack flow.
- **TASK-AI-003** memory audit — 9 new kinds register here.
- **TASK-MEMORY-111** PII scrubbing — email + IP scrubbed in chain rows.
- **TASK-OBS-007** Auto-runbook — sev-1/sev-2 alerts route to CHAT/PagerDuty.

**Downstream (blocks):**
- **TASK-TEN-107** — tenant-admin SPA depends on signup completing.
- **TASK-PORTAL-001** — scoped read-only views require self-served tenants.
- **TASK-PORTAL-002** — per-tenant brand pack requires self-served tenants.
...
| OTP delivery fails (email service down) | TASK-EMAIL-001 returns error | 503 + `otp_send_failed`; session state remains 'started'; user can resend after rate-limit window | Email service recovers; retry within 10min OTP window or new OTP |
...
| Stripe API 5xx during confirm | api_client retry exhausted (TASK-TEN-003 §1 #15) | 502 + `stripe_unavailable`; session.state remains 'email_verified'; user can retry | Stripe recovers; idempotency key prevents double-charge on retry |
...
| AUTH root-admin creation fails inside the atomic tx | create_subject returns error WITHIN the Phase-C tx | `ROLLBACK` undoes tenant + consent INSERTs (no tenant exists post-rollback); Stripe SetupIntent voided; signup_session.state='rolled_back' reason='root_admin_create_failed'; sev-1 logged | Operator investigates; user contacted via billing_contact_email; TASK-TEN-104 NOT invoked (no tenant to terminate) |
| Welcome email send fails | TASK-EMAIL-001 returns error post-commit | signup is COMPLETE (tx already committed); email retry job runs background; user can request resend via /v1/signup/resend-welcome | Email recovers; user logs in via JWT in response anyway |
...
| OIDC ID token signature invalid | TASK-AUTH-104 verification fails | 401 + `oidc_invalid_token`; session.state remains 'started' | User retries OIDC flow |
