...
related_tasks: [TASK-AUTH-001, TASK-AUTH-101, TASK-AI-016, TASK-AI-003, TASK-MEMORY-101, TASK-TEN-002, TASK-TEN-104, TASK-TEN-103]
depends_on: [TASK-AUTH-001]
blocks: [TASK-TEN-002, TASK-TEN-004, TASK-TEN-101, TASK-TEN-103, TASK-TEN-104]
...
source_decisions:
  - DEC-320 (ops-driven provisioning CLI is slice 1; self-serve signup TASK-TEN-101 ships at P3)
  ...
  - DEC-323 (S3 bucket prefix: per-residency bucket + `<tenant_id>/` prefix — composes with TASK-DOC-001 + TASK-EMAIL-001 residency routing)
  - DEC-324 (default residency = vn-1 at slice 1; explicit `--residency <sg-1|eu-1|us-1|vn-1>` flag override; TASK-TEN-103 ships full 4-residency provisioning)
  - DEC-325 (initial root-admin subject created with tenant-admin role via TASK-AUTH-001's POST /v1/admin/tenants flow; password auto-generated + emitted to operator stdout once)
  - DEC-326 (tenant.status closed enum: provisioning, active, suspended, terminating, terminated — slice 1 ships all 5; transitions in TASK-TEN-104)
  - DEC-327 (memory audit kinds: ten.tenant_provisioned, ten.tenant_suspended, ten.tenant_resumed, ten.tenant_terminating, ten.tenant_terminated, ten.tenant_branding_updated, ten.tenant_residency_changed — slice 1 emits ten.tenant_provisioned; others are TASK-TEN-104)
  - DEC-328 (REVOKE UPDATE, DELETE on tenant + tenant_status_history from cyberos_app — append-only enforced at SQL grant; status changes via TASK-TEN-104)
...
disallowed_tools:
  ...
  - ship self-serve signup at slice 1 (per DEC-320 — TASK-TEN-101 ships at P3)
...
subtasks:
  ...
  - "0.4h: provisioning/auth_bootstrap.rs — call TASK-AUTH-001 + create root-admin"
...
2. **MUST** declare the closed `tenant_status` Postgres enum with exactly 5 values (per DEC-326): `'provisioning'`, `'active'`, `'suspended'`, `'terminating'`, `'terminated'`. State transitions land in TASK-TEN-104; slice 1 sets `provisioning` → `active` at end of successful provisioning.
...
20. **MUST** create the tenant's memory audit-chain anchor (per DEC-332): emit a special chain-bootstrap row that becomes the genesis row for the tenant's audit chain. TASK-AI-003's memory_writer initialises the chain head from this row; subsequent rows chain to it.

21. **MUST** create the AUTH side via TASK-AUTH-001's internal helper `auth::admin::tenants::provision_tenant(slug, display_name, root_admin_email, root_admin_display_name, root_admin_password_hash, operator_subject_id)` returning `(tenant_id, root_admin_subject_id)`. This helper is exposed from TASK-AUTH-001 specifically for TASK-TEN-001's use; it is NOT exposed via REST (operator privilege only).
...
24. **MUST** validate `--root-admin-email` against the standard email regex (same as TASK-AUTH-002 §1 #2) AND check it's NOT already an existing AUTH subject in any tenant.
...
26. **MUST** support `--dry-run` flag: validates all inputs + checks slug availability + checks residency policy + simulates each step, but DOES NOT write anything. Returns exit 0 if all checks pass; exit 65/73/77 on issues. Used by TASK-TEN-101's signup form preflight (slice 3+).
...
- **TASK-TEN-104** — 90-day offboarding contract (uses the closed TenantStatus enum).

**Cross-module:**
- **TASK-AUTH-002** — subject create (consumed via the AUTH bootstrap helper for root-admin creation).
- **TASK-AUTH-101** — RBAC catalogue (root-admin role + tenant-admin role assignment).
- **TASK-AI-016** — residency policy (consumes `tenant_residency_map` table).
- **TASK-DOC-001** — document repository (consumes `tenant_residency_map` for body bucket lookup).
- **TASK-EMAIL-001** — Stalwart mail server (consumes `tenant_residency_map` for body bucket lookup).
- **TASK-AI-003** — memory audit bridge (receives `ten.tenant_provisioned` row + chain anchor).
...
Deferred:
- **Plan tier closed enum** — TASK-TEN-002 ships full 3-tier validation.
...
| AUTH internal helper signature drift | type error at compile | Build fails | Coordinate TASK-AUTH-001 + TASK-TEN-001 changes |
...
- **Ops-driven at slice 1** — self-serve signup (TASK-TEN-101) lands at P3. CLI covers our P2 vertical-pack volume.
...
- **`tenant_residency_map` is the lookup contract** — TASK-DOC-001 + TASK-EMAIL-001 + TASK-AI-016 all read from this table. Centralising prevents drift.
...
- **`--dry-run` mode** — validates everything, mutates nothing. Powers TASK-TEN-101's signup-form preflight.
...
- **Operator JWT verification** — slice 1 reads `auth.is_root_admin` GUC; slice 2 (TASK-TEN-101) adds full OAuth flow.
