...
- Generator: `tools/docs-site/render-status-hub.mjs` emits status-hub@3 (tabless canvas)
  ...
  Legacy dual emission (`status-legacy.html` / `CYBEROS_STATUS_LEGACY`) removed at **1.13.0**.
...
The preview folder was deleted in the same change that landed the swap. Confirmed still
absent at 1.13.0 closeout (`docs/status-v2-preview/` / `docs/status-v3-preview/`).
