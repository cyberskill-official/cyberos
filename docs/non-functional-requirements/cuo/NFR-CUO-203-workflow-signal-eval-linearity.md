...
A workflow with 1000 historical runs across 5 signals shouldn't take >1s to evaluate. Quadratic behaviour (e.g. for each workflow, walk ALL rows including other workflows') would mean the harness's workflow section becomes the slowest part of the report once workflows accumulate 50+ runs each. The O(N) walk discipline is the same one TASK-MEMORY-120's `cyberos history` enforces — both are linear walks over the audit chain with per-row classification.
...
The "queue by default" rule (§1 #5) is operator-mandated — Stephen explicitly chose this in chat. Workflows are read by orchestration; a typo could change every future ship-tasks run.
...
Queue-default: `test_workflow_diffs_default_to_queue` (deferred — requires extension of TASK-CUO-202's classifier to handle workflow stripe ids; the classifier currently handles only skill stripes).
