---
id: NFR-SKILL-004
title: "SKILL audit-chain coverage — every skill invocation MUST emit ≥ 1 BRAIN row"
module: SKILL
category: compliance
priority: MUST
verification: T
...
1. Every skill invocation (success, denial, error) **MUST** result in at least one Layer-1 BRAIN row carrying `{tenant_id, actor_id, skill_name, skill_version, capability_used, started_at, ended_at, outcome, output_digest?}`.
...
3. If the audit emit fails (BRAIN unreachable, disk full), the skill runtime **MUST** retain the audit row on local disk in `skill/audit-pending/<uuid>.json` and replay on next BRAIN healthcheck.
4. Reconciliation: number of invocations (per `cyberos-skill stats`) **MUST** match number of audit rows (per BRAIN `SELECT count(*) FROM l2_memory WHERE kind='skill.invoke'`) within 0.001% over a 7-day window.
...
- Chaos test (T) — kills BRAIN connection mid-invocation; asserts skill completes, row spooled, replayed within 60s.
...
- Spool replay rate > 1% sustained → sev-3; BRAIN connectivity degraded; investigate.
