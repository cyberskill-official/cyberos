...
4. Any attempted escape (denied syscall, denied egress) **MUST** be logged to memory with `kind=skill.cap_bypass_attempt` carrying the denied call + skill identity.
