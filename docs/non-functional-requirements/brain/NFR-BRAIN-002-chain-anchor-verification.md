---
id: NFR-BRAIN-002
title: "BRAIN chain-anchor verification — every l2_memory read re-checks anchor; mismatch → quarantine"
module: BRAIN
category: security
priority: MUST
verification: T
...
owner: CSO
created: 2026-05-18
related_frs: [FR-BRAIN-101, FR-BRAIN-102]
---
...
4. A doctor full-chain scan (`cyberos-brain doctor`) **MUST** verify every anchor in O(N) time and complete < 30s on a 100k-row store (NFR-BRAIN-004).
5. Quarantined rows **MUST NOT** be deleted automatically — they remain for forensic review by CSO. Manual `cyberos-brain release-quarantine <row_id>` restores after CSO sign-off.

## §2 — Why this constraint

The chain anchor is the platform's tamper-evidence primitive. Without read-time verification, an attacker who writes a malicious row to Postgres bypasses the BRAIN audit chain (the chain only protects writes via the ingest pipeline; direct DB write would corrupt the chain silently). Read-time verify catches direct-DB-write tampering. The < 200µs p99 budget keeps verify invisible inside the much larger query roundtrip. Quarantine instead of delete preserves the evidence for forensic analysis — deleting would lose the attack signature.

## §3 — Measurement

- Counter `brain_l2_anchor_verify_total{result}` where result ∈ {`ok`, `mismatch`}.
- Gauge `brain_l2_quarantine_rows_total` — should be near-zero.
- Histogram `brain_l2_anchor_verify_seconds` — p99 < 0.0002.
- Sev-1 alarm on `result=mismatch > 0`.

## §4 — Verification

- Property test `services/brain/tests/chain_anchor_test.rs` (T) — 1000 random rows; asserts every legit row verifies; injects 100 tampered rows; asserts all quarantine.
- Doctor smoke (T) — `cyberos-brain doctor` full scan on 100k-row test store; asserts no false-positive quarantines.
