---
id: NFR-BRAIN-006
title: "BRAIN STH publication frequency — consolidate every 24h or 100MB whichever first"
module: BRAIN
category: compliance
priority: MUST
verification: T
...
owner: CSO
created: 2026-05-18
related_frs: [FR-BRAIN-102, FR-BRAIN-101]
---

## §1 — Statement (BCP-14 normative)

1. The BRAIN consolidation process **MUST** publish a Signed Tree Head (STH) — a Merkle root over the consolidated window's rows, signed by the platform key — every 24 hours of wall-clock time OR every 100MB of new rows ingested since the last STH, whichever threshold is hit first.
...
- Counter `brain_sth_published_total{result}` per STH attempt.
- Gauge `brain_sth_age_seconds` — time since last STH; alarm at > 86400 + 3600 (1h grace beyond 24h target).
- Gauge `brain_sth_bytes_since_last` — alarm at > 100 * 1024 * 1024 + grace.
- BRAIN doctor invariant: STH chain unbroken end-to-end.

## §4 — Verification

- Integration test `services/brain/tests/sth_publication_test.rs` (T) — drives 100MB of writes; asserts STH published before threshold; drives time-fast-forward 24h; asserts time-based STH published.
- Doctor full-chain verify (T) — `cyberos-brain doctor --verify-sth-chain` walks every STH and verifies signatures + prev-hashes.
