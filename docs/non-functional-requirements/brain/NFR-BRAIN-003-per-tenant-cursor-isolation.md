---
id: NFR-BRAIN-003
title: "BRAIN per-tenant cursor isolation — cursor advance for tenant A never affects tenant B"
module: BRAIN
category: security
priority: MUST
verification: T
...
owner: CSO
created: 2026-05-18
related_frs: [FR-BRAIN-101, FR-BRAIN-103]
---

## §1 — Statement (BCP-14 normative)

1. The Layer-2 ingest cursor (`services/brain/src/layer2/cursor.rs`) **MUST** be scoped per (tenant_id, actor_id) — there is no global cursor that crosses tenants.
...
- Per-tenant gauge `brain_layer2_cursor_pos{tenant_id, actor_id}`.
- Counter `brain_layer2_cursor_advance_total{tenant_id}` — should increment only for the advancing tenant.
- BRAIN doctor invariant: no two cursors share (tenant_id, actor_id).

## §4 — Verification

- Property test `services/brain/tests/cursor_isolation_test.rs` (T) — drives 1000 random advance sequences across 50 tenants; asserts each tenant's cursor moves only on its own advances.
