...
5. Every circuit transition **MUST** emit a BRAIN audit row `ai_gateway.circuit.{open|close|probe}` carrying `{provider, route, trigger_error_class, ts, prev_state, next_state}`. These rows are immutable and survive consolidation.
...
- The 3-in-30s and 60s-recovery contract is verified by inspecting the BRAIN audit rows: the temporal delta between `circuit.open` and the third `provider_request.error` row **MUST** be ≤ 30s; the delta between `circuit.open` and the next `circuit.probe` row **MUST** be ≥ 60s and < 61s.
...
- Chaos drill `deploy/obs/runbooks/ai-gateway-provider-outage.md` (D) — quarterly — kills a provider mid-day and asserts the circuit opens within 30s, recovers within 90s, and no BRAIN audit row is missing from the transition log.
...
- BRAIN audit row missing for any circuit transition → sev-1 (chain-of-custody violation); halt new requests on that route until audit catches up.
