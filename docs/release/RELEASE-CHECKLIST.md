...
| B3 | IMP-15.3 plugin zip loaded into a live session | operator | open | 2026-07-19 PARTIAL - row stays open. VERIFIED: plugin installs and registers (Plugins menu lists CyberOS; installed manifest name=cyberos version=1.0.0; 8 commands + 57 skills load; descriptions render from our frontmatter). `/install` PASSES end-to-end - run via Claude Code CLI against `~/Projects/CyberSkill/sachviet` (a docs-only repo, git-init'd first per install.sh:1049). All components landed: machine, gates.env, backlog seed, CHANGELOG, status page, pre-commit hook, agent surface, BRAIN, managed .gitignore - and the installed `rules_sha` fbbc899a...3b00052 matches the shipped payload exactly. NOT YET RUN: `/create-tasks`, `ship-tasks`. ROW WORDING IS WRONG: `ship-tasks` ships as a SKILL, not a command - there is no `/ship-tasks` slash command; invoke the skill by name. HOST DEFECT (not ours, no payload change needed): the FIRST slash command in a fresh Cowork task is dropped with `Unknown command: /<plugin>:<cmd>`; the 2nd and later dispatch normally. Proven across 2 tasks x 3 commands (install/version/help). Workaround: send any message first. Plugin artifact verified correct on disk - only one plugin on the machine claims commands/install.md. SURFACE LIMIT: Cowork runs in an isolated cloud sandbox with no access to the local filesystem (no /Users, CYBEROS_PAYLOAD unset, desktop bridge not connected), so `/install` cannot reach a local repo from there. It failed closed and diagnosed this correctly rather than half-installing. The CLI is the surface where install actually works - finish this row there. |
