...
| Folder | Purpose |
|---|---|
| `tasks/` | The task corpus: 605 task specs across 29 domains (spec + `_audits/` companions). `BACKLOG.md` is the index of remaining active work; task frontmatter `status` is the record of truth. Improvement tasks (`type: improvement`) live in the same tree (`improvement/` + `(improvement)` tags in the backlog). |
...
| `batches/` | Per-batch economics ledgers (members, wall time, route-backs; feeds the status hub's batch economics panel). |
| `deploy/` | Runbooks: `RELEASE.md` (the release process), go-live, VPS topology, CI/local checks, auth SSO. |
| `goals/` | Standing goals re-verified after `done` (`verify-goals.mjs` inputs, TASK-IMP-109). |
| `plans/` | Plan artefacts (`plan@1`) from the plan-author front door. |
| `reference/` | Getting started, glossary, risk-register, and `status-feed@1` contract. |
| `runbooks/` | Operator runbooks (secrets rotation, envelope review, **traceability**). |
| `release/` | Release checklist. |
| `status/` | Generated status-hub@3 page (`index.html` + assets + `data/`; hooks/gates — do not hand-edit). |
