...
§3.2  **V1 aliases** (one release cycle): `create`, `str_replace`, `insert`, `rename` MAY be used and will be mapped at the implementation layer to `put`/`move`. `view` is implicit on read and MAY emit an audit row but does not change state. New code SHOULD use the canonical names.
...
§6.5  **Legacy chain bridge.** When `manifest.json:migration.legacy_last_chain` is set, the first record in the new binlog MUST have `prev_chain` equal to that value (with `sha256:` prefix stripped). The cumulative chain across the legacy → v2 boundary MUST be continuous.

§6.6  Forbidden ledger operations: in-place edit of a written row; re-ordering of rows; deletion of rows; rewriting the tail past the last intact frame. Recovery from corruption is via consolidation (§7), not row mutation.
