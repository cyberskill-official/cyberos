...
MEMORY, SKILL, and AI are green under the gate. CUO, AUTH, CHAT, PROJ, and EMAIL are staged and run by the bootstrap script. The other modules are still draft (not yet implemented), so they are gated when they are built. Run `scripts/awh_gate_coverage.py` for the live table.
