...
- Version stamping. `scripts/stamp-release-version.mjs --apply` runs first in both the desktop and Android jobs and propagates the platform `VERSION` into the artifacts that carry their own version field: `tauri.conf.json`, `apps/web/package.json`, and the Android `versionName` + `versionCode` (derived as `major*10000 + minor*100 + patch`, so 1.2.0 becomes 10200). These had silently drifted - `tauri.conf.json` still said 1.0.0 at VERSION 1.2.0 - and Play rejects any upload whose `versionCode` it has already seen, so publishing could never have worked twice. Check drift any time with `node scripts/stamp-release-version.mjs`.
