...
- Auto-bump: `.github/workflows/version.yml` runs on every push to `main`. It computes the next version from the new Conventional Commits with `scripts/cyberos-version.mjs` (feat -> minor; fix/perf/revert/refactor -> patch; `!` or `BREAKING CHANGE:` -> major; chore/docs/ci/test/build/style do not bump). When a bump is due it commits `chore(release): vX.Y.Z` back to `main` (no `[skip ci]` since FR-IMP-071: the bump commit must be visible to push-triggered workflows so a tag pointing at it fires `release.yml` natively and `deploy.yml` refreshes the docs at the new VERSION) (updating `VERSION` + `CHANGELOG.md`). It never tags, builds installers, or deploys. The baseline is the last commit that touched `VERSION`, so each push only counts what is new since the previous bump.
...
- Store build numbers come from `BUILD_NUMBER`, not from `VERSION`. The Android `versionCode` and the iOS `CURRENT_PROJECT_VERSION` are both stamped from the root `BUILD_NUMBER` file, a monotonic counter bumped alongside `VERSION` by `scripts/cyberos-version.mjs`. They are separate numbers on purpose: the marketing version can move wherever a product decision takes it (it was rolled back to 0.1.0 for the pre-1.0 run-up), but a store build number can only ever go up. Play permanently remembers every `versionCode` it has accepted - including 10700 - and rejects anything at or below it, so a derived code would have made the rollback unshippable on Android forever. `BUILD_NUMBER` is seeded above that mark and the stamper hard-fails if it ever drops to or below it.
