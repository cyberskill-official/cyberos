...
- **remote connector** - `node .cyberos/mcp/cyberos-mcp.mjs --http 8799` - a zero-dependency MCP streamable-HTTP endpoint (POST JSON-RPC, `GET /healthz` for probes) that agent UIs' "custom connector" dialogs can point at. Tools exposed: `task_init`, `task_gates`, `task_status`, `ship_task`.
...
3. Auth: the transport ships UNAUTHENTICATED - do not expose it publicly without either proxy-level auth (basic/bearer at nginx) or an allowlist. The ship_task/task_init tools execute repo workflows; treat the endpoint like CI credentials.
