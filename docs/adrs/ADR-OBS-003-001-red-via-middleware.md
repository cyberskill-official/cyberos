...
- Wired now: `auth` and `memory`. Deferred until the surface exists: `ai-gateway` (add the same one-line layer + `init` + `TenantCtx` when its HTTP listener is built) and `chat` (a pinned image, instrumented where its source lives).
- Export path: `obs_sdk::init` builds the instruments off the global meter; until a service installs an OTel meter provider with an OTLP metric exporter (pointing at the FR-OBS-001 collector), `record_request` records to a no-op meter. Wiring that provider, and validating end to end against the live obs stack (`deploy/obs/`), is the remaining piece for metrics to actually flow.
