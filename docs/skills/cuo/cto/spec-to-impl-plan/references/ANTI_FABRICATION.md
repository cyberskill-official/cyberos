...
Anti-fabrication discipline for the `spec-to-impl-plan` skill. The skill MUST ask clarifying questions when the input is ambiguous, NOT invent facts the source material doesn't support.
...
Run `cyberos skill-quality calibration spec-to-impl-plan` to see current calibration data. The skill's predicted-vs-actual `needs_human` rate should not diverge > 2×.
