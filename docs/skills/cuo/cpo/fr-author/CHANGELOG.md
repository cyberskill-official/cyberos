...
## v0.2.2 — 2026-05-06 (Tier-2/3 audit fixes; PATCH — wire-protocol contract + reference-doc divergence note)

### Added

- `depends_on_contracts:` extended with `nats-subjects/v1` (wire-protocol contract). Previously this skill emitted three NATS subjects (`cuo.fr_author.fr_written`, `…batch_complete`, `…hitl_pause`) without a declared contract. Now consumed via the new `cyberos/docs/contracts/nats-subjects/` registered in v1.0.0.
- `references/README.md` — index of the eight reference docs in this folder, plus a per-skill divergence note explaining why `HITL_PROTOCOL.md`, `UNTRUSTED_CONTENT.md`, `ANTI_FABRICATION.md`, `EU_AI_ACT_DECISION_TREE.md` differ between fr-author and fr-audit (intentional; deferred consolidation to v0.3.0). Surfaced this divergence proactively so the next reader doesn't waste cycles on "which copy is canonical?"
...
User-approved Tier-2/3 follow-ups to the v0.2.1 pre-deployment audit. Specific findings absorbed: B1 (per-skill divergence — documented as intentional after byte-level diff confirmed all four shared docs differ), B2 (NATS subjects undocumented — promoted to a wire-protocol contract under `cyberos/docs/contracts/nats-subjects/`), C1 (batch_size soft-cap commentary already present in schema description).

### Backwards compatibility

PATCH bump — no contract changes for this skill's own envelopes, no manifest schema changes, no body semantics changes. The `depends_on_contracts:` addition is additive (the contract did not previously exist; skills without the declaration have no contract to reference). Existing v0.2.1 manifests resume cleanly.
...
- `references/HASHING.md` reference in CONTRACT_ECHO replaced with the actual location of hashing rules (`references/MANIFEST_SCHEMA.md §3.1`). The HASHING.md file does not and never did exist; the reference was a dead link in v0.1.0/v0.2.0 inherited from the source v2.0.0 prompt.
- `references/OUTPUT_FORMATS.md` reference in WORKER step W2 replaced with explicit pointers to the actual format documents (`PLAN_RENDER.md`, `HITL_PROTOCOL.md`, `MANIFEST_SCHEMA.md` plus the contract template). OUTPUT_FORMATS.md was a dead link.
- `envelopes/fr-author.input.json` aligned with SKILL.md frontmatter: `required` array trimmed from 6 fields to 3 (`requirements_files`, `output_dir`, `manifest_path`); the other three (`batch_size`, `caller_persona`, `trace_id`) were declared optional in frontmatter v0.2.0 but still required by the schema. The schema now matches.
- SKILL.md `expects.optional_fields` extended to include `chain_to` (which is in the input schema but was undocumented in frontmatter).
- Skill version + CONTRACT_ECHO line + output envelope example bumped 0.2.0 → 0.2.1.
- Acceptance fixture folder added (`acceptance/README.md`) explaining the v0.3.0 milestone — fixtures are pending the runtime/harness build per registry README Part 13.3 + Recipe 8.

### Driver

Pre-deployment structural audit (registry README Part 7's manual-fine-tune playbook applied without production telemetry, since the runtime hasn't shipped yet). Surfaced 6 Tier-1 issues across both fr-author and fr-audit; 4 of them affect this skill. Each fix prevents a near-future failure: dead links would break the runtime's link-validator at first invocation; schema/frontmatter mismatch would reject every well-formed envelope from the supervisor.

### Backwards compatibility

PATCH bump — no contract changes. v0.2.0 manifests resume cleanly. Output envelope shape unchanged (only the example value of `skill_version` field bumped in the SKILL.md prose). Acceptance fixture folder is additive.
...
- Frontmatter blocks per registry README v0.2.0 Part 2:
  ...
  - `depends_on_contracts:` — pins `feature-request@v1` at the new path `cyberos/docs/contracts/feature-request/`.
...
- All references to `cuo/_shared/feature-request-template/` updated to the new contract location `cyberos/docs/contracts/feature-request/template.md` (declared via `depends_on_contracts:` rather than hard-coded paths in the body). Affected: §"PLAN phase", §"WORKER phase" step W2, §"Failure modes" BOOT-008 row, §"Citations" Template-source line, the CONTRACT_ECHO `template_version` line, the output envelope's `skill_version` field.
...
- `PIPELINE.md` — three worked chain examples (`fr-author` → `fr-audit`, audit-only, and the future `fr-author` → `fr-audit` → `cuo/cto/fr-to-tech-spec`).
...
- Audit loop **removed** from this skill. Audit now lives in `cuo/cpo/fr-audit` and is invoked via the LangGraph supervisor's edge (when chained) or by direct invocation (when standalone). The W4 step in the WORKER loop renamed from `INVOKE AUDIT` to `EMIT EVENT` — the skill emits a NATS subject `cuo.fr_author.fr_written` and lets the supervisor decide whether to chain.
- `prompt_revision: fr_create_and_audit@2.0.0` → `fr_author@2.0.0`.
- Manifest schema field `prompt_revisions.fr_create_and_audit` → `skill_revisions.fr_author`. v2.0.0 manifests load cleanly with a `MIGRATE_FORWARD` audit row written on first invocation.
...
- Round-trip pipeline test: PRD → fr-author → fr-audit → resume → pass, recorded in `cuo/cpo/fr-author/PIPELINE.md` §1.
- All references/*.md files trace 1:1 to the source §-headings, with the prefix renaming (s/fr_create_and_audit/fr_author/) the only systematic diff.
