# `fr-manifest@2` — single source of truth for fr-author state

> Sourced verbatim from `feature-request/FR_CREATE_AND_AUDIT.md` v2.0.0 §3 (modulo s/`fr_create_and_audit`/`fr_author`/g — see `CHANGELOG.md`).
...
- **`audit_hash`** — copied from the chained `fr-audit` skill's report (`audited_file_sha256` field) after each audit run. Optional in `fr-author` standalone mode.
...
```json
{
  ...
  "skill_revisions": {
    "fr_author": "fr_author@2.0.0",
    "_note": "MUST match the prompt_revision literal in cuo/cpo/fr-author/SKILL.md CONTRACT_ECHO. Mismatch → CONTRACT_DRIFT (FAILURE_MODES.md). The template version (feature_request@1, loaded from cyberos/docs/contracts/feature-request/template.md via depends_on_contracts:) advances lockstep with this skill — they are not separately versioned."
  },
  "inputs": {
    "requirements_files": [
