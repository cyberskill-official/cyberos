# `fr-author` standalone interview script

> Used by the runtime when `fr-author` is invoked **standalone** (no upstream pipeline envelope) — to fill the `expects.required_fields` from the user via chat. When invoked **chained**, this file is ignored; the upstream skill's output envelope supplies the same fields machine-readably.
...
If the user replies "actually, I don't have requirements yet — help me write one first," the supervisor MUST NOT proceed with `fr-author`. Instead it routes to a future `cuo/cpo/prd-draft` skill (not yet authored as of v0.2.0) or surfaces "There's no PRD-drafting skill yet; would you like to write the PRD by hand and come back?"

## Audit row

Each Q→A round produces one `genie.action_log` row of `row_kind: question`. The final answer-set produces one `row_kind: act` summarising the inputs the supervisor synthesised before invoking `fr-author`.
