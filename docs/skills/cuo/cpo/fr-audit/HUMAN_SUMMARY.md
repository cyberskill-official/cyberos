...
> Rendered to chat after each audit batch when the skill runs in **standalone** mode. Chained-mode runs (e.g. after `fr-create`) skip this — the supervisor folds the verdict into the upstream skill's summary.
...
> Type `fix FR-NNN` to dispatch a fix-iteration with `cuo/cpo/fr-create`
...
Same as `fr-create/HUMAN_SUMMARY.md` §"Tone rules", plus:
...
When chained from `fr-create`, the supervisor renders one line per audited FR back into the upstream skill's summary block:

```
fr-create wrote FR-007.md (artefact_write evt_…)
