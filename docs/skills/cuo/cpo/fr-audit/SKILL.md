...
produces:
  ...
  output_kind: artefact           # *.audit.md is an artefact; needs_human verdict emits 'question'; invariant breach emits 'refinement_proposal'
...
exposable_as:
  internal:           true
  agent_plugin:       true
  mcp_tool:           true        # auto-emit; the rubric makes a great MCP tool for partner CIs
  partner_connector:  false       # gated; pending partner-exposure DEC

# ── Audit hook (SRS §6.7) ────────────────────────────────────────────
audit:
  emit_to: genie.action_log
  row_kind: artefact_write        # one row per audit report; plus 'question' rows on HITL; plus 'self_refinement_proposal' on invariant breach
...
self_audit:
  ...
  anomaly_signals:
    ...
    user_correction_streak:    {threshold: 2, window: 5}    # human said "this rule is wrong" twice
    rule_reversal_streak:      {threshold: 1, window: 1}    # any time a passed FR is later corrected to fail = signal
    needs_human_rate_above:    {threshold: 0.5, window: 10}
    deterministic_drift:       {threshold: 1, window: 1}    # same FR + same rubric → different verdicts ever = breach
  on_breach:
...
human_fine_tune:
  ...
  review_required:
    ...
    on_rubric_rule_added: true    # NEW rules require cpo + (clo if rule touches EU AI Act / legal)
    on_rubric_rule_removed: true  # removing rules requires cpo + cseco (security-relevance check)
  signals_to_initiate:
  ...
  required_artifacts:
    - changelog_entry
    - acceptance_test_added       # new regression FR + expected audit verdict under acceptance/
    - rubric_rule_diff            # if RUBRIC.md changed, include the rule-by-rule diff
...
Single phase: `AUDIT`. There is no PLAN or WORKER concept here — every invocation runs the rubric + loop on each `fr_path`. Re-entrancy is anchored on each FR's audit report's `audited_file_sha256`:

- If the existing audit's `audited_file_sha256 == sha256(current_fr)`: resume in place; carry forward all issues and statuses, including `needs_human` answers.
- If hash differs: FR was edited externally. Reset every issue with `status ∈ {open, needs_human}` to `open` and re-evaluate. Preserve `fixed`/`wontfix` for diff context.
...
After looping over every `fr_path`, emit `AUDIT_BATCH_SUMMARY` (output envelope above). If any FR is `needs_human`, emit `HITL_BATCH_REQUEST` (per `HITL_PROTOCOL.md`) AFTER the summary, aggregating issues across all paused FRs.
...
- Use Levenshtein ≤2 for ambiguous enum corrections, but only on non-compliance-sensitive fields (NOT `eu_ai_act_risk_class`, NOT `ai_authorship`).
...
For each FR: locate → hash → load-or-init audit report → apply rubric → fix or escalate → re-audit → terminate. Each FR gets a sibling `<fr_path>.audit.md`. The skill emits `AUDIT_BATCH_SUMMARY` listing per-FR `overall_status` (`pass` | `needs_human` | `fail`).
