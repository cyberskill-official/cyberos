# `fr-create` human-summary template
...
When `fr-create` chains into `fr-audit`, this summary is NOT rendered. The supervisor instead emits a single-line trace:

```
fr-create → fr-audit  ·  3 FRs handed off  ·  trace_id={…}
```
