...
4. **Implementation files** — any file in the project that implements the protocol (e.g., `outputs/brain_writer.py` for §7.2; `cyberos/.protocol-signing-key` for §0.5) must be reviewed for compliance and updated if the change affects its behaviour.
...
- **Orphan manifest update** — an `op:"str_replace"` row against `manifest.json` updating `audit_chain_head` without a paired terminal op `∈ {consolidation_run, protocol_upgrade, protocol_rollback, session.end}` later in the ledger means crash mid-write. Append `op:"rejected" reason:"crash-mid-manifest-update"` and require the user to acknowledge before accepting new writes. (The original §4.7 wording predated `protocol_upgrade`/`protocol_rollback`/`session.end` as legitimate manifest-pinning terminators; per Bundle F this now lists all four.)
