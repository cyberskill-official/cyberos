/**
 * Events published by @cyberos/genie.
 *
 * Naming: `genie.{verb}.{noun}` — see SRS §5.4. Add an entry below
...
export type GenieEventName = never; // populate as you add events
...
export async function emitPlaceholder(_ctx: RequestContext): Promise<void> {
  ...
  await publish({
    ctx: _ctx,
    source: "GENIE",
    name: "genie.placeholder.emitted",
    payload: {},
  });
}
