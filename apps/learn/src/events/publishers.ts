/**
 * Events published by @cyberos/learn.
 *
 * Naming: `learn.{verb}.{noun}` — see SRS §5.4. Add an entry below
...
export type LearnEventName = never; // populate as you add events
...
export async function emitPlaceholder(_ctx: RequestContext): Promise<void> {
  ...
  await publish({
    ctx: _ctx,
    source: "LEARN",
    name: "learn.placeholder.emitted",
    payload: {},
  });
}
