/**
 * Events published by @cyberos/email.
 *
 * Naming: `email.{verb}.{noun}` — see SRS §5.4. Add an entry below
...
export type EmailEventName = never; // populate as you add events
...
export async function emitPlaceholder(_ctx: RequestContext): Promise<void> {
  ...
  await publish({
    ctx: _ctx,
    source: "EMAIL",
    name: "email.placeholder.emitted",
    payload: {},
  });
}
