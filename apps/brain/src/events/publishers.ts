/**
 * Events published by @cyberos/brain.
 *
 * Naming: `brain.{verb}.{noun}` — see SRS §5.4. Add an entry below
...
export type BrainEventName = never; // populate as you add events
...
export async function emitPlaceholder(_ctx: RequestContext): Promise<void> {
  ...
  await publish({
    ctx: _ctx,
    source: "BRAIN",
    name: "brain.placeholder.emitted",
    payload: {},
  });
}
