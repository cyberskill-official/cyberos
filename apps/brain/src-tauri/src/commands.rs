...
//!   - `search_brain`    — POSTs to the local Rust BRAIN service (FR-BRAIN-108),
...
//!                         Today the tenant resolves to "default"; FR-BRAIN-105+ wires
//!                         real multi-tenant resolution.
//!   - `get_sync_state`   — reads `~/.cyberos-memory/{tenant}/sync/last-status.json`
//!                         (matches `modules/memory/cyberos/core/brain_sync.py::LAST_STATUS_REL`).
...
const BRAIN_SEARCH_URL: &str = "http://127.0.0.1:7901/v1/brain/search";
...
#[tauri::command]
pub async fn search_brain(query: String, limit: u32) -> Result<Vec<SearchHit>, String> {
    ...
    let resp = client
        .post(BRAIN_SEARCH_URL)
        .json(&SearchRequest { query: &query, limit })
        .send()
        .await
        .map_err(|e| format!("brain search request failed: {e}"))?;

    if !resp.status().is_success() {
        return Err(format!("brain search HTTP {}", resp.status()));
    }

    resp.json::<Vec<SearchHit>>()
        .await
        .map_err(|e| format!("brain search decode failed: {e}"))
}
...
/// `default`; multi-tenant resolution arrives with FR-BRAIN-105+.
