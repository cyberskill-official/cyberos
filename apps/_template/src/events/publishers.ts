/**
 * Events published by {{PACKAGE}}.
 *
 * Naming: `{{NAMESPACE}}.{verb}.{noun}` — see SRS §5.4. Add an entry below
...
export type {{TYPE}}EventName = never; // populate as you add events
...
export async function emitPlaceholder(_ctx: RequestContext): Promise<void> {
  ...
  await publish({
    ctx: _ctx,
    source: "{{MODULE}}",
    name: "{{NAMESPACE}}.placeholder.emitted",
    payload: {},
  });
}
