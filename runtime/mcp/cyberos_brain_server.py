#!/usr/bin/env python3
"""
cyberos_brain_server.py — read-only MCP server for the .cyberos-memory BRAIN.
...
Exposes 4 read-only tools to MCP clients (Claude Code, Cursor, etc.):
  ...
  - brain_search    keyword search across slug/tags/scope/body
  - brain_show      list memories with metadata + optional filters
  - brain_get       fetch a single memory by memory_id or relative path
  - brain_stats     bucket counts + sync-class breakdown
...
`brain_writer.py` as the canonical write path.

This server respects:
  - §0.1 real-filesystem-only (resolves brain via .cyberos-memory walk)
...
Wire it into a client (Claude Code example) via .claude/mcp-config.json:

    {
      "mcpServers": {
        "cyberos-brain": {
          "command": "python3",
          "args": ["/abs/path/to/runtime/mcp/cyberos_brain_server.py"],
          "env": {"CYBEROS_BRAIN_ROOT": "/abs/path/to/cyberos"}
        }
      }
    }

If CYBEROS_BRAIN_ROOT is unset, the server walks up from CWD to find
...
Run standalone for testing:
    echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.0"}}}' \
      | python3 runtime/mcp/cyberos_brain_server.py
"""
...
SERVER_NAME = "cyberos-brain"
SERVER_VERSION = "0.1.0"
PROTOCOL_VERSION = "2024-11-05"


# ---------------------------------------------------------------------------
# BRAIN access (read-only)
# ---------------------------------------------------------------------------

def resolve_brain_root() -> Path:
    env = os.environ.get("CYBEROS_BRAIN_ROOT")
    if env:
        p = Path(env)
    ...
    while cur != cur.parent:
        if (cur / ".cyberos-memory").is_dir():
            return cur
        cur = cur.parent
    raise RuntimeError("no .cyberos-memory/ found; set CYBEROS_BRAIN_ROOT")
...
def tool_brain_search(args: dict) -> dict:
    ...
    for rel, fm, body, _ in iter_memories(brain_root):
...
def tool_brain_show(args: dict) -> dict:
    ...
    brain_root = resolve_brain_root()
    rows = []
    for rel, fm, _, _ in iter_memories(brain_root):
        if scope and not rel.startswith(scope):
            continue
...
def tool_brain_get(args: dict) -> dict:
    ...
    brain_root = resolve_brain_root()
    for rel, fm, body, text in iter_memories(brain_root):
...
def tool_brain_stats(args: dict) -> dict:
    ...
    tomb = 0
    total = 0
    for _, fm, _, _ in iter_memories(brain_root):
        total += 1
...
TOOLS = {
    ...
    "brain_stats": {
        "fn": tool_brain_stats,
        "description": "Bucket counts + sync-class breakdown + tombstone count for the current BRAIN.",
...
def handle_tools_call(params: dict) -> dict:
    ...
    if name.startswith("brain_") and name not in ("brain_search", "brain_show", "brain_get", "brain_stats"):
        # Defensive: refuse anything that looks like a write
        return {"content": [{"type": "text", "text": json.dumps({"error": "this MCP server is read-only; use brain_writer.py for writes"})}], "isError": True}
    try:
