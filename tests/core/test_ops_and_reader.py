...
def test_create_then_read(store: Path) -> None:
    with Writer(store) as writer:
        ops.create(writer, "memories/facts/FACT-001.md", _make_memory("FACT-001"), actor="t")
...
def test_create_rejects_duplicate(store: Path) -> None:
    with Writer(store) as writer:
        ops.create(writer, "memories/facts/x.md", _make_memory("x"), actor="t")
        with pytest.raises(FileExistsError):
            ops.create(writer, "memories/facts/x.md", _make_memory("x"), actor="t")


def test_str_replace_rejects_ambiguous(store: Path) -> None:
    with Writer(store) as writer:
        body = b"---\n{}\n---\nrepeat\nrepeat\n"
        target = store / "memories/x.md"
        target.parent.mkdir(parents=True)
        target.write_bytes(body)
        with pytest.raises(ValueError):
            ops.str_replace(writer, "memories/x.md", b"repeat", b"once", actor="t")


def test_rename_dst_collision(store: Path) -> None:
    with Writer(store) as writer:
        ops.create(writer, "memories/facts/a.md", _make_memory("a"), actor="t")
        ops.create(writer, "memories/facts/b.md", _make_memory("b"), actor="t")
        with pytest.raises(FileExistsError):
            ops.rename(writer, "memories/facts/a.md", "memories/facts/b.md", actor="t")


def test_path_traversal_blocked(store: Path) -> None:
    with Writer(store) as writer:
        with pytest.raises(ops.PathTraversal):
            ops.create(writer, "../escape.md", _make_memory("x"), actor="t")
        with pytest.raises(ops.PathTraversal):
            ops.create(writer, "/absolute.md", _make_memory("x"), actor="t")
            ...
            while not stop.is_set():
                ...
                writer.submit(
                    __import__("cyberos.core.writer", fromlist=["AuditRecord"]).AuditRecord(
                        op="str_replace", path=target, actor="churn", content_sha256="0" * 64,
                    )
                )
                i += 1
                time.sleep(0.001)
