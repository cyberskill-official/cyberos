...
from cyberos.core.ops import create, delete, move, put, str_replace
...
def test_build_with_activity_counts_by_op_actor_prefix(tmp_path):
    ...
    with Writer(store) as w:
        ...
        create(w, "memories/drift/x1.md", b"a", actor="coding-agent", kind="drift")
        str_replace(w, "memories/facts/f1.md", b"a", b"ab", actor="stephen")
    ...
    # 1 each: put×2, create, str_replace, rename(move), delete
    # Note: `move` op emits an "op=rename" record by default in ops.py
    # for v1-compat. Either way, our counts must add up to 6.
...
def test_build_highlights_include_decisions_drift_purges_renames(tmp_path):
    ...
    # but f1.md MAY appear as the SOURCE path of the subsequent rename row.
    # Assert that intent precisely: no `put` highlight under facts/.
    put_highlights = [
        h for h in d.highlights
        if h.op in ("put", "create", "str_replace", "insert")
    ]
    fact_put_highlights = [
    ...
    # Rename / purge appear regardless of path prefix.
    rename_or_move_present = any(h.op in ("rename", "move") for h in d.highlights)
    purge_present = any(h.extra.get("mode") == "purge" for h in d.highlights)
    assert rename_or_move_present
