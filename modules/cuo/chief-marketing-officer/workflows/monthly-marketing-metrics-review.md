---
workflow_id: chief-marketing-officer/monthly-marketing-metrics-review
workflow_version: 1.0.0
purpose: Monthly marketing KPI review — pipeline contribution, channel CAC, brand-search trends, campaign post-mortems.
...
inputs:
  ...
  - { name: prior_artefact, source: memory search for prior `monthly-marketing-metrics-review` outputs, format: markdown }

outputs:
  - { name: monthly_marketing_metrics_review_artefact, format: ad-hoc-md, recipient: persona owner + downstream consumers }

skill_chain:
  - { step: 1, skill: product-metrics-review-author, inputs_from: brief, outputs_to: mm_draft }
  - { step: 2, skill: product-metrics-review-audit, inputs_from: mm_draft, outputs_to: mm_final }
...
# Monthly Marketing Metrics Review — `cmo` workflow

> Monthly marketing KPI review — pipeline contribution, channel CAC, brand-search trends, campaign post-mortems.

## When to invoke

CUO routes here when the user says things like:

- "Run the CMO monthly marketing metrics review"
- "Monthly monthly marketing metrics review"
- "Run the monthly-marketing-metrics-review workflow"

## How to invoke

```bash
cyberos-cuo execute chief-marketing-officer/monthly-marketing-metrics-review \
    --output-dir <dir> \
    --invoker llm \
    --memory-emit
```
...
**Step 1 — `product-metrics-review-author`**: produces the artefact body in the shape declared by `modules/skill/product-metrics-review-author/CONTRACT.md`. Consumes the `brief` input (workflow-supplied) plus any prior-step hand-off.

**Step 2 — `product-metrics-review-audit`**: validates the upstream artefact against the per-skill RUBRIC. Returns `rubric_outcome: {score, pass, fixes}` plus a verdict (pass / needs_human / fail / exhausted).
...
- Skill chain anchors: see `modules/skill/product-metrics-review-author/` + `modules/skill/product-metrics-review-audit/`
