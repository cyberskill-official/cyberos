...
skill_chain:
  ...
  - { step: 28, skill: awh-gate,                                   inputs_from: { task: next_task, module: next_task.module, goldenset: "modules/<module>/.awh/goldenset.yaml", baseline: "modules/<module>/.awh/eval-baseline.json" }, outputs_to: awh_gate_report, judgment: mechanical, description: "Out-of-band independent rerun (the check step 27 is NOT). `awh eval <goldenset> --base-dir . --seeds 1 --baseline <baseline> --max-regression 0.0` reruns the task's §1 cited tests plus the module suite against the sealed, read-only baseline. GREEN (no task regressed) is REQUIRED to reach the done-flip; RED routes the task back to ready_to_implement per STATUS-REFERENCE §1.3 with routed_back_count += 1. Tests sealed via `awh lock modules/<module>/tests`. Emits memory.awh_gate_result. Deterministic executor: `tools/awh` (TASK-IMP-125)." }
  - { step: 29, skill: caf-gate,                                 inputs_from: { task: next_task, module: next_task.module, audit_profile: "modules/<module>/audit-profile.yaml", audit_baseline: "modules/<module>/.caf/" }, outputs_to: caf_gate_report, judgment: mechanical, description: "Code-audit gate (absorbed from CyberSkill/code-audit-framework). Deterministic floor: `bash scripts/caf_gate.sh <module>` runs the module's TARGET HEALTH via tools/caf/core/evals/verify-target.sh (the module's own RUN_COMMANDS - build/lint/typecheck/test - from modules/<module>/audit-profile.yaml, fail-closed) AND, when a sealed audit exists at modules/<module>/.caf/, `code-audit-validate --run modules/<module>/.caf --fail-on High` (no new High/Critical finding vs the sealed baseline). CLEAN is REQUIRED alongside the awh gate to reach the done-flip; RED routes the task back to ready_to_implement per STATUS-REFERENCE §1.3 with routed_back_count += 1. Catches the class awh cannot: build/lint breaks, route 404s, changed data contracts (the CCAF/kymondongiap class). Emits memory.caf_gate_result. See docs/verification/caf-absorption-design.md. Deterministic executor: `scripts/caf_gate.sh` (TASK-IMP-125)." }
...
- **Review acceptance** (`reviewing → ready_to_test`, steps 19-20): the agent produces the code-review packet (steps 17-18) with every §1 clause mapped to a named test, then HALTS. A human records the approval verdict, which advances the cell. Prefer `task-state.mjs transition <id> reviewing ready_to_test --verdict-by <actor> --verdict-evidence <path>` (TASK-IMP-144: frontmatter + flip + receipt + IMP-143 verdict artifact). Equivalent: set frontmatter first, then `backlog-mutate.mjs flip …` with the same flags. A bare flip of this transition refuses with exit 8 (TASK-CUO-303).
- **Final acceptance** (`testing → done`, steps 30-31): the agent brings every machine gate green (coverage, TRACE-004, awh, caf), then HALTS. A human records the acceptance verdict. The agent NEVER self-sets `done`. Same mechanics via `task-state.mjs transition` (preferred) or `backlog-mutate.mjs flip <id> testing done --verdict-by <actor> --verdict-evidence <path>` — the flags are how the recorded verdict advances the cell; a bare flip refuses with exit 8, and on a resolvable BRAIN store the gated flip appends one `status_overridden` audit row before the index moves.

**Gated-flip checklist (truth precedes index — TASK-IMP-120 + batch/8 friction; state engine — TASK-IMP-144):**

1. Write a non-empty evidence file. If it lives under `docs/batches/`, it MUST open with YAML frontmatter (`batch:` + `members:`) so `render-status-hub.mjs` can parse every file in that directory — or keep acceptance notes under the task folder instead.
2. Prefer `node tools/install/docs-tools/task-state.mjs transition <id> <from> <to> --verdict-by <actor> --verdict-evidence <path>` (writes frontmatter, flip, transition receipt under `docs/tasks/_state/receipts/`, and a content-addressed verdict artifact under `docs/tasks/_verdicts/`).
3. Or: set the task's `spec.md` frontmatter `status:` to the destination value **first**, then `backlog-mutate.mjs flip <id> <from> <to> --verdict-by <actor> --verdict-evidence <path>`.
4. Regenerate `docs/status/` (or rely on the pre-commit hook) before treating the hub as truth.

**Batch "approve all":** one shared evidence file MAY back N gated flips (one utterance, N `status_overridden` rows pointing at the same reason path). Prefer that over per-task copies when the operator's verdict is batch-wide.
...
Closed (TASK-IMP-144): editing `status:` in frontmatter and regenerating BACKLOG without a flip receipt is refused — `regen_backlog` exits non-zero and writes nothing. The former CUO-303 "accepted residual" is closed on the 1.x line.
...
- **`mechanical` = a deterministic executor produces the step's result with no model deciding anything** (TASK-IMP-125). The agent runs the tool and transcribes. The label is never applied on a feels-deterministic basis — it is anchored to an on-disk executor the payload names for that skill, **wherever that executor lives** (not keyed on `docs-tools/`):

  | Steps | Skill | Executor that does the work | Where the payload says so |
  |---|---|---|---|
  ...
  | 28 | `awh-gate` | `tools/awh` | skill_chain step 28 description — Deterministic executor: `tools/awh` |
  | 29 | `caf-gate` | `scripts/caf_gate.sh` | skill_chain step 29 description — Deterministic executor: `scripts/caf_gate.sh` |
...
- **A wrong level is a drift bug, not an outage.** A step marked `mechanical` whose executor is later replaced by a model is simply wrong until someone fixes it — the suite arm (`test_mechanical_steps_are_helper_backed`) is what notices, by re-proving the executor exists and is still the thing the payload names for that skill.
