...
inputs:
  ...
  - { name: prior_artefact, source: BRAIN search for prior `monthly-revenue-forecast` outputs, format: markdown }
...
**Step 1 — `forecast-author`**: produces the artefact body in the shape declared by `modules/skill/forecast-author/CONTRACT.md`. Consumes the `brief` input (workflow-supplied) plus any prior-step hand-off.
