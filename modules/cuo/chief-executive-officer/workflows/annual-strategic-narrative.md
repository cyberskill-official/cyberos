...
inputs:
  ...
  - { name: prior_artefact, source: memory search for prior `annual-strategic-narrative` outputs, format: markdown }
...
**Step 1 — `strategy-document-author`**: produces the artefact body in the shape declared by `modules/skill/strategy-document-author/SKILL.md`. Consumes the `brief` input (workflow-supplied) plus any prior-step hand-off.
