...
inputs:
  ...
  - { name: prior_okrs,         source: last quarter's okr-set@1 artefact,                 format: okr-set@1 }
...
skill_chain:
  - { step: 1, skill: okr-set-author, inputs_from: { vision_brief: vision_brief, prior_okrs: prior_okrs, function_inputs: function_inputs }, outputs_to: okrs_draft }
  - { step: 2, skill: okr-set-audit,  inputs_from: okrs_draft, outputs_to: company_okrs }

escalates_to:
  - { persona: cuo/chief-of-staff, when: "okr-set-audit fires QA-MEASURE-001 — KR not measurable; CoS owns OKR-governance fix" }
...
audit_hooks:
  ...
  - HITL pause at step 2 if okr-set-audit fires on overlapping KRs across functions
---
...
- **Step 1 `okr-set-author`** — drafts company OKRs per Doerr OKR template (Objectives qualitative + 3-5 Key Results quantitative). Pause for PLAN approval.
- **Step 2 `okr-set-audit`** — validates against `okr_set_rubric@1.0` (each O has 3-5 measurable KRs; KR thresholds are ambitious; cross-function alignment check).
