...
inputs:
  ...
  - { name: prior_artefact, source: BRAIN search for prior `annual-capacity-rebase` outputs, format: markdown }
...
**Step 1 — `capacity-plan-author`**: produces the artefact body in the shape declared by `modules/skill/capacity-plan-author/CONTRACT.md`. Consumes the `brief` input (workflow-supplied) plus any prior-step hand-off.
