...
inputs:
  ...
  - { name: prior_artefact, source: memory search for prior `quarterly-operating-review` outputs, format: markdown }
...
**Step 1 — `operating-model-author`**: produces the artefact body in the shape declared by `modules/skill/operating-model-author/SKILL.md`. Consumes the `brief` input (workflow-supplied) plus any prior-step hand-off.
