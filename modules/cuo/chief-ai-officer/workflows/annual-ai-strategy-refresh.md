...
inputs:
  ...
  - { name: prior_artefact, source: memory search for prior `annual-ai-strategy-refresh` outputs, format: markdown }
...
**Step 1 — `ai-strategy-author`**: produces the artefact body in the shape declared by `modules/skill/ai-strategy-author/SKILL.md`. Consumes the `brief` input (workflow-supplied) plus any prior-step hand-off.
