---
workflow_id: chief-of-staff/quarterly-ceo-priorities-review
workflow_version: 1.0.0
purpose: Quarterly CEO-priorities tracker review — what's slipping, what needs reallocation, what to kill.
...
inputs:
  ...
  - { name: prior_artefact, source: memory search for prior `quarterly-ceo-priorities-review` outputs, format: markdown }

outputs:
  - { name: quarterly_ceo_priorities_review_artefact, format: ad-hoc-md, recipient: persona owner + downstream consumers }

skill_chain:
  - { step: 1, skill: decision-log-author, inputs_from: brief, outputs_to: priorities_draft }
  - { step: 2, skill: decision-log-audit, inputs_from: priorities_draft, outputs_to: priorities_final }
...
# Quarterly Ceo Priorities Review — `chief-of-staff` workflow

> Quarterly CEO-priorities tracker review — what's slipping, what needs reallocation, what to kill.

## When to invoke

CUO routes here when the user says things like:

- "Run the CHIEF OF STAFF quarterly ceo priorities review"
- "Quarterly quarterly ceo priorities review"
- "Run the quarterly-ceo-priorities-review workflow"

## How to invoke

```bash
cyberos-cuo execute chief-of-staff/quarterly-ceo-priorities-review \
    --output-dir <dir> \
    --invoker llm \
    --memory-emit
```
...
**Step 1 — `decision-log-author`**: produces the artefact body in the shape declared by `modules/skill/decision-log-author/SKILL.md`. Consumes the `brief` input (workflow-supplied) plus any prior-step hand-off.
