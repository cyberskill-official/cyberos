...
inputs:
  ...
  - { name: prior_artefact, source: memory search for prior `monthly-leadership-onboarding` outputs, format: markdown }
...
**Step 1 — `onboarding-pack-author`**: produces the artefact body in the shape declared by `modules/skill/onboarding-pack-author/SKILL.md`. Consumes the `brief` input (workflow-supplied) plus any prior-step hand-off.
