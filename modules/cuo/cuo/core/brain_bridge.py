"""BRAIN bridge — emits CUO supervisor decisions to the BRAIN audit chain.

Per cuo/docs/AGENTS.md §1.1 step 4: "Records the chain decision in the BRAIN
...
sibling memory module. It opens the writer against the local BRAIN
...
or BRAIN dir missing), `emit_chain_result()` becomes a no-op and returns a
NoBrainResult with the reason. Callers should treat BRAIN emission as best-
...
class BrainEmitResult:
    """Outcome of emitting a ChainResult to the BRAIN audit chain."""
    ...
    def __repr__(self) -> str:
        if not self.emitted:
            return f"BrainEmitResult(SKIPPED, reason={self.reason_skipped!r})"
        return f"BrainEmitResult(emitted, rows={self.rows_written}, head={self.chain_head_after[:8]}...)"
...
def _try_import_memory_writer():
    ...
    # cuo file lives at modules/cuo/cuo/core/brain_bridge.py
...
def emit_chain_result(
    ...
    *,
    ...
    """Emit a completed workflow chain to the BRAIN audit chain.
    ...
    Args:
        ...
        skill_root: path to `skill/` (used to locate the sibling BRAIN root).
        actor: actor name attached to every row. Default "cuo-supervisor".
        brain_root: optional override for `.cyberos-memory/` location.
            If None, auto-locates via `_find_brain_root(skill_root)`.

    Returns:
        BrainEmitResult — `emitted=True` on success, `emitted=False` with
        `reason_skipped` if the writer is unavailable or BRAIN missing.
    """
    # Late import to avoid hard dependency for non-BRAIN callers.
    imported = _try_import_memory_writer()
    if imported is None:
        return BrainEmitResult(
            emitted=False,
            reason_skipped="cyberos.core.writer not importable — BRAIN emission requires the memory module",
        )
    Writer, WriterConfig, AuditRecord = imported

    if brain_root is None:
        brain_root = _find_brain_root(skill_root)
        if brain_root is None:
            return BrainEmitResult(
                emitted=False,
                reason_skipped=f"BRAIN root not found near {skill_root}",
            )
    ...
    if not audit_dir.is_dir():
        return BrainEmitResult(
            emitted=False,
            reason_skipped=f"BRAIN audit/ subdir missing at {audit_dir}",
        )
    ...
    try:
        cfg = WriterConfig()
        writer = Writer(brain_root, config=cfg)
        writer.open()
...
def brain_is_available(skill_root: Path) -> bool:
    """Check whether BRAIN emission would succeed for this skill_root.
    ...
    if _try_import_memory_writer() is None:
        return False
    brain = _find_brain_root(skill_root)
    if brain is None:
        return False
    return (brain / "audit").is_dir()
