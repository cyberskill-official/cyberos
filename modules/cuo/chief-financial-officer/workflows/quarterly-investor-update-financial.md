---
workflow_id: chief-financial-officer/quarterly-investor-update-financial
workflow_version: 1.0.0
purpose: Quarterly investor financial update — actuals vs forecast, runway, key ratios, narrative on variances.
...
inputs:
  ...
  - { name: prior_artefact, source: memory search for prior `quarterly-investor-update-financial` outputs, format: markdown }

outputs:
  - { name: quarterly_investor_update_financial_artefact, format: ad-hoc-md, recipient: persona owner + downstream consumers }

skill_chain:
  - { step: 1, skill: investor-update-author, inputs_from: brief, outputs_to: iu_draft }
  - { step: 2, skill: investor-update-audit, inputs_from: iu_draft, outputs_to: iu_final }
...
# Quarterly Investor Update Financial — `cfo` workflow

> Quarterly investor financial update — actuals vs forecast, runway, key ratios, narrative on variances.

## When to invoke

CUO routes here when the user says things like:

- "Run the CFO quarterly investor update financial"
- "Quarterly quarterly investor update financial"
- "Run the quarterly-investor-update-financial workflow"

## How to invoke

```bash
cyberos-cuo execute chief-financial-officer/quarterly-investor-update-financial \
    --output-dir <dir> \
    --invoker llm \
    --memory-emit
```
...
**Step 1 — `investor-update-author`**: produces the artefact body in the shape declared by `modules/skill/investor-update-author/CONTRACT.md`. Consumes the `brief` input (workflow-supplied) plus any prior-step hand-off.

**Step 2 — `investor-update-audit`**: validates the upstream artefact against the per-skill RUBRIC. Returns `rubric_outcome: {score, pass, fixes}` plus a verdict (pass / needs_human / fail / exhausted).
...
- Skill chain anchors: see `modules/skill/investor-update-author/` + `modules/skill/investor-update-audit/`
