...
inputs:
  ...
  - { name: prior_artefact, source: memory search for prior `annual-product-vision-refresh` outputs, format: markdown }
...
**Step 1 — `product-roadmap-author`**: produces the artefact body in the shape declared by `modules/skill/product-roadmap-author/SKILL.md`. Consumes the `brief` input (workflow-supplied) plus any prior-step hand-off.
