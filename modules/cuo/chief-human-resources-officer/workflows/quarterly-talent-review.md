---
workflow_id: chief-human-resources-officer/quarterly-talent-review
workflow_version: 1.0.0
purpose: Quarterly 9-box talent review — top performers, succession, performance management actions, equity refresh recommendations.
...
inputs:
  ...
  - { name: prior_artefact, source: memory search for prior `quarterly-talent-review` outputs, format: markdown }

outputs:
  - { name: quarterly_talent_review_artefact, format: ad-hoc-md, recipient: persona owner + downstream consumers }

skill_chain:
  - { step: 1, skill: workforce-plan-author, inputs_from: brief, outputs_to: talent_review_draft }
  - { step: 2, skill: workforce-plan-audit, inputs_from: talent_review_draft, outputs_to: talent_review_final }
...
# Quarterly Talent Review — `chro` workflow

> Quarterly 9-box talent review — top performers, succession, performance management actions, equity refresh recommendations.

## When to invoke

CUO routes here when the user says things like:

- "Run the CHRO quarterly talent review"
- "Quarterly quarterly talent review"
- "Run the quarterly-talent-review workflow"

## How to invoke

```bash
cyberos-cuo execute chief-human-resources-officer/quarterly-talent-review \
    --output-dir <dir> \
    --invoker llm \
    --memory-emit
```
...
**Step 1 — `workforce-plan-author`**: produces the artefact body in the shape declared by `modules/skill/workforce-plan-author/CONTRACT.md`. Consumes the `brief` input (workflow-supplied) plus any prior-step hand-off.

**Step 2 — `workforce-plan-audit`**: validates the upstream artefact against the per-skill RUBRIC. Returns `rubric_outcome: {score, pass, fixes}` plus a verdict (pass / needs_human / fail / exhausted).
...
- Skill chain anchors: see `modules/skill/workforce-plan-author/` + `modules/skill/workforce-plan-audit/`
