...
inputs:
  - { name: active_sdds,           source: engineering team (all in-flight SDDs), format: sdd@1 (set) }
  - { name: architecture_vision,   source: cuo/chief-architect/annual-architecture-vision, format: strategy-doc@1 }
  - { name: adr_history,           source: ADR repository, format: adr@1 (set) }

outputs:
  - { name: design_review,         format: sdd@1 (review chapter), recipient: cuo/chief-architect + cuo/cto + engineering leads }

skill_chain:
  - { step: 1, skill: sdd-author, inputs_from: { active_sdds: active_sdds, architecture_vision: architecture_vision, adr_history: adr_history }, outputs_to: review_draft }
  - { step: 2, skill: sdd-audit,  inputs_from: review_draft, outputs_to: design_review }
