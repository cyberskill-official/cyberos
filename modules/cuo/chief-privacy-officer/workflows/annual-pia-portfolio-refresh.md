---
workflow_id: chief-privacy-officer/annual-pia-portfolio-refresh
workflow_version: 1.0.0
purpose: Annual portfolio refresh — re-PIA every product touching personal data, refresh records of processing.
...
inputs:
  ...
  - { name: prior_artefact, source: memory search for prior `annual-pia-portfolio-refresh` outputs, format: markdown }
...
# Annual Pia Portfolio Refresh — `cpo-privacy` workflow

> Annual portfolio refresh — re-PIA every product touching personal data, refresh records of processing.

## When to invoke

CUO routes here when the user says things like:

- "Run the CPO PRIVACY annual pia portfolio refresh"
- "Annual annual pia portfolio refresh"
- "Run the annual-pia-portfolio-refresh workflow"

## How to invoke

```bash
cyberos-cuo execute chief-privacy-officer/annual-pia-portfolio-refresh \
    --output-dir <dir> \
    --invoker llm \
    --memory-emit
```
...
**Step 1 — `privacy-impact-assessment-author`**: produces the artefact body in the shape declared by `modules/skill/privacy-impact-assessment-author/SKILL.md`. Consumes the `brief` input (workflow-supplied) plus any prior-step hand-off.
