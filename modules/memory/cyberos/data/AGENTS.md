...
§0.4  `<memory-root>/` is the real local-filesystem path `.cyberos/memory/store/` at the project root, resolved through every symlink. The store lives under the unified, gitignored `.cyberos/` module tree (alongside `.cyberos/cuo/` and the `.cyberos/memory/` protocol docs); the extra `store/` level keeps the live store distinct from the vendored protocol files, so a machine update MUST refresh the docs without touching `store/`. Backward compatibility (normative): if `.cyberos/memory/store/` does not exist but a legacy `.cyberos-memory/` store exists at the project root, that legacy path IS the `<memory-root>/` and implementations MUST honor it unchanged; a fresh store SHALL be created at `.cyberos/memory/store/`. Resolution order is therefore an explicit override (`--store` / `CYBEROS_STORE`), then the nearest `.cyberos/memory/store/` walking up from the working directory, then the nearest legacy `.cyberos-memory/`. Sandbox/ephemeral paths are listed in `memory.invariants.yaml` (`layout-no-sandbox-path`); a store on any such path SHALL be rejected unless `CYBEROS_HOST_MOUNT_PREFIX` exempts it.
