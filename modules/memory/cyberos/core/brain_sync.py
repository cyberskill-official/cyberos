"""FR-BRAIN-103 — Multi-device sync daemon.

Bidirectional sync between a Personal BRAIN (this device) and Lumi (the
cloud-hosted org BRAIN). The personal-BRAIN client:
...
(``POST /v1/auth/lumi/issue``). The daemon's environment variables:
  ...
  * ``BRAIN_SYNC_LUMI_URL`` — base URL of the Lumi service, e.g.
    ``https://lumi.cyberskill.world``.
  * ``BRAIN_SYNC_LUMI_TOKEN`` — the issued JWT (rotated by the auth side).
  * ``BRAIN_SYNC_INTERVAL_SECS`` — poll cadence (default 60).
...
Reliability:
  ...
  * Every push + pull cycle emits a heartbeat-style status row at
    ``<store>/sync/last-status.json`` so the supervisor (FR-BRAIN-110)
    and the doctor invariants can see how the daemon is doing without
    needing a heavy logging stack.

Slice 1 ships:

  * The :class:`BrainSync` orchestrator + push + pull loops.
...
# ---------------------------------------------------------------------------
# Retry policy mirrors FR-BRAIN-110's restart policy.
# ---------------------------------------------------------------------------
...
def _http_post_json(url: str, token: str, body: dict, timeout: float = 10.0) -> dict:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "cyberos-brain-sync/0.1",
        },
    )
    try:
...
@dataclass
class BrainSync:
    """The brain-sync daemon orchestrator. One per personal BRAIN."""
...
def run_forever(
    ...
    *,
    ...
    BRAIN reader — keeps this module decoupled from the writer/reader).
    """
    lumi_url = lumi_url or os.environ.get("BRAIN_SYNC_LUMI_URL")
    lumi_token = lumi_token or os.environ.get("BRAIN_SYNC_LUMI_TOKEN")
    interval = int(interval_secs or os.environ.get("BRAIN_SYNC_INTERVAL_SECS", DEFAULT_INTERVAL_SECS))
    if not lumi_url or not lumi_token:
        raise RuntimeError(
            "brain-sync daemon requires BRAIN_SYNC_LUMI_URL + BRAIN_SYNC_LUMI_TOKEN"
        )

    sync = BrainSync(store=store, lumi_url=lumi_url, lumi_token=lumi_token)
    while True:
        try:
