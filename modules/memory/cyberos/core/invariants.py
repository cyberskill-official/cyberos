...
``memory/docs/memory.invariants.yaml``. The walker
...
def check_layout_root_canonical(store: Path) -> tuple[bool, str]:
    ...
    for entry in store.iterdir():
        ...
        if entry.is_dir() and name not in _CANONICAL_TOP_LEVEL_DIRS:
            ...
            # — they live under memory/docs/ normally but a synthetic
...
def check_ledger_link(store: Path) -> tuple[bool, str]:
    ...
    try:
        n = verify_segments(segs, start_prev=_GENESIS_CHAIN)
...
_REPAIRABLE_INVARIANTS: dict[str, str] = {
    ...
    "layout-shard-uniformity":
        "reshard unsharded memory files into the canonical <kind>/<hex>/<hex>/ "
        "buckets in place",
...
def _repair_shard_uniformity(store: Path) -> tuple[bool, str]:
    """Reshard un-bucketed memory files into canonical hex buckets.

    Doing this in-process keeps the doctor's repair operation a single
    atomic decision the user can audit. For shard-only repair we don't
    touch frontmatter — just move the files into <kind>/<hex>/<hex>/.
    """
    import hashlib
    import shutil
    mem = store / "memories"
...
def _repair_index_manifest(store: Path) -> tuple[bool, str]:
    ...
    target.write_text(
        _json.dumps({"last_applied_seq": last_seq}, sort_keys=True),
        encoding="utf-8",
    )
