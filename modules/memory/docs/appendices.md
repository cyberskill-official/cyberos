---
title: Memory — Appendices & Extended Reference — CyberOS
...
> Extended design rationale, history, and proposal tracker. For the concise module overview, see [`README.md`](<README.md>).
...
The autosync subsystem reconciles three concerns:
  ...
  2. **Background maintenance.** Cross-platform scheduler integration (launchd / systemd-user / Task Scheduler) runs nightly walk + weekly consolidate.
...
### Open questions
  ...
  3. **Q3 — Recovery if a STH is lost/corrupted.** Re-derive from raw rows (expensive) vs trust the next STH (requires gap acknowledgement).
