"""Tests for FR-BRAIN-103 multi-device sync daemon (pure-function pieces)."""
...
from cyberos.core.brain_sync import (
    DEAD_LETTER_REL,
    LAST_STATUS_REL,
    PULL_CURSOR_REL,
    BrainSync,
...
def test_push_once_writes_status_and_counts(tmp_path: Path) -> None:
    ...
    sync = BrainSync(store=tmp_path, lumi_url="https://lumi.test", lumi_token="t")
    with patch("cyberos.core.brain_sync._http_post_json", return_value={"ok": True}):
...
def test_push_retries_and_dead_letters_on_persistent_failure(tmp_path: Path) -> None:
    ...
    with patch("cyberos.core.brain_sync._http_post_json",
               side_effect=SyncTransportError("nope")), \
         patch("cyberos.core.brain_sync.time.sleep", lambda _x: None):
        counters = sync.push_once(rows)
...
def test_pull_once_admits_and_writes_cursor(tmp_path: Path) -> None:
    ...
    with patch("cyberos.core.brain_sync._http_get_json", return_value=resp):
...
def test_pull_once_records_network_error(tmp_path: Path) -> None:
    sync = BrainSync(store=tmp_path, lumi_url="https://lumi.test", lumi_token="t")
    with patch("cyberos.core.brain_sync._http_get_json",
