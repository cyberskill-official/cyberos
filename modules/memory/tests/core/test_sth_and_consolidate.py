...
def test_consolidate_refuses_over_failing_walk(store: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("CYBEROS_HOST_MOUNT_PREFIX", "/")
    # Deliberately corrupt the manifest so the manifest-validates-against-schema
    # invariant fails (unparseable JSON).
    (store / "manifest.json").write_text("not valid json", encoding="utf-8")
    report = run(store)
