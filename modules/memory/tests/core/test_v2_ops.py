...
def test_canonical_ops_emit_canonical_names(store: Path) -> None:
    """put/move emit canonical op names — no v1 aliases remain."""
    with Writer(store) as w:
        ops.put(w, "memories/facts/A.md", _body(), actor="t")
        ops.move(w, "memories/facts/A.md", "memories/facts/B.md", actor="t")
    with MmapWalker(store / "audit" / "current.binlog") as walker:
        records = list(walker.iter_records())
    assert [r.op for _o, r in records] == ["put", "move"]
