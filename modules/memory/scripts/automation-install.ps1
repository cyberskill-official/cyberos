...
Write-Host "  scripts : $Repo\memory\scripts\automation\"
...
New-Job -Name $NightlyTaskName `
    -Script (Join-Path $Repo "memory\scripts\automation\cyberos-nightly.ps1") `
    -When (Get-Date "01:09") -Cadence Daily

New-Job -Name $WeeklyTaskName `
    -Script (Join-Path $Repo "memory\scripts\automation\cyberos-weekly.ps1") `
