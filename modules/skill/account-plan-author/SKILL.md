---
# ── Identity ─────────────────────────────────────────────────────────
name: account-plan-author
description: >-
  Author a per-strategic-account plan (context + org chart + current spend + expansion thesis + threats + quarterly action plan + executive sponsor coverage). CRO-Revenue or CCO-Customer authored. Use when user asks to "draft a account plan" or "create the account plan". Do NOT use for "audit existing account plan" (use account-plan-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: account-plan_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['account-plan-audit']; empty list disables chaining
...
# account-plan-author — ACCOUNT-PLAN generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `account-plan@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`account-plan-audit`](../account-plan-audit/SKILL.md) by default.

`prompt_revision: account-plan_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a ACCOUNT-PLAN."
- "Generate v2 of the ACCOUNT-PLAN from the updated source."
- "Draft a ACCOUNT-PLAN for <project>."

If the user asks to *audit an existing ACCOUNT-PLAN*, route to `account-plan-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  ACCOUNT-PLAN-{NNN}-{slug}.md
...
```json
{
  ...
  "artefacts_written": [
    {"id": "ACCOUNT-PLAN-001", "path": "./account-plans/ACCOUNT-PLAN-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "account-plan-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/account-plan/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `account-plan_author.account-plan_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `account-plan-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `account-plan-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   account-plan-author
Input:
