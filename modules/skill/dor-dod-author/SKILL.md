...
description: |
  ...
  naturally into dor-dod-audit.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: b
  cyberos-template: dor-dod@1
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['dor-dod-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        dor-dod
    ...
    pin_path:  cyberos/skill/contracts/dor-dod/
  - id:        nats-subjects
    version:   v1
...
# dor-dod-author — DOR-DOD generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `dor-dod@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`dor-dod-audit`](../dor-dod-audit/SKILL.md) by default.
...
If the user asks to *audit an existing DOR-DOD*, route to `dor-dod-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        dor-dod-author
skill_version:                   1.0.0
prompt_revision:                 dor-dod_author@1.0.0
template_version:                dor-dod@1   (loaded from cyberos/skill/contracts/dor-dod/template.md)
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "dor-dod-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/dor-dod/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `dor-dod-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `dor-dod-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   dor-dod-author
Input:
