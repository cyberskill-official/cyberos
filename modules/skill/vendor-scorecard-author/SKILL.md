---
# ── Identity ─────────────────────────────────────────────────────────
name: vendor-scorecard-author
description: >-
  Author the monthly / quarterly vendor scorecard (vendor list + spend by category + SLA attainment + sustainability score + consolidation opportunities). COO or CPO-Procurement authored. Use when user asks to "draft a vendor scorecard" or "create the vendor scorecard". Do NOT use for "audit existing vendor scorecard" (use vendor-scorecard-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: vendor-scorecard_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['vendor-scorecard-audit']; empty list disables chaining
...
# vendor-scorecard-author — VENDOR-SCORECARD generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `vendor-scorecard@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`vendor-scorecard-audit`](../vendor-scorecard-audit/SKILL.md) by default.

`prompt_revision: vendor-scorecard_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a VENDOR-SCORECARD."
- "Generate v2 of the VENDOR-SCORECARD from the updated source."
- "Draft a VENDOR-SCORECARD for <project>."

If the user asks to *audit an existing VENDOR-SCORECARD*, route to `vendor-scorecard-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        vendor-scorecard-author
skill_version:                   1.0.0
prompt_revision:                 vendor-scorecard_author@1.0.0
...
naming_pattern:                  VENDOR-SCORECARD-{NNN}-{slug}.md
...
```json
{
  ...
  "artefacts_written": [
    {"id": "VENDOR-SCORECARD-001", "path": "./vendor-scorecards/VENDOR-SCORECARD-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "vendor-scorecard-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/vendor-scorecard/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `vendor-scorecard_author.vendor-scorecard_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
- If chained to `vendor-scorecard-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `vendor-scorecard-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   vendor-scorecard-author
Input:
