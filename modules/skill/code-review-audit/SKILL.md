...
metadata:
  ...
  cyberos-rubric-version: code_review_rubric@1.0
...
# code-review-audit — CODE-REVIEW auditor

> Standalone trigger that runs `code-review_rubric@1.0` against one or
> more existing `code-review@1` markdowns and writes a sibling
> `code-review.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`code-review-author`](../code-review-author/SKILL.md).

`prompt_revision: code-review_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `CODE-REVIEW`s."
- "Has `CODE-REVIEW-007` changed since the last audit?"
- "Tell me which `CODE-REVIEW`s would fail acceptance today."

Also invoked automatically by the supervisor when `code-review-author`'s output envelope sets `next_skill_recommendation: code-review-audit` (the default chain).
...
audit_rubric_version:            code-review_rubric@1.0
...
```json
{
  "artefact_paths": ["./code-reviews/CODE-REVIEW-001-foo.md", "./code-reviews/CODE-REVIEW-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "code-review-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./code-reviews/CODE-REVIEW-001-foo.md", "audit_path": "./code-reviews/CODE-REVIEW-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./code-reviews/CODE-REVIEW-002-bar.md", "audit_path": "./code-reviews/CODE-REVIEW-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
- Cross-check any sibling `review-findings.json`: its record count MUST equal the markdown's finding count, and a mismatch REDS the audit (`XREF-FINDINGS-COUNT`, authority: TASK-IMP-112 §1.4).
...
Input:
  artefact_paths:  [./team-a/CODE-REVIEW-001-something.md, ./team-b/CODE-REVIEW-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
