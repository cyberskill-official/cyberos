...
metadata:
  ...
  module: skill
  stage: cross
  cyberos-template: knowledge-pipeline@1
...
# knowledge-pipeline-audit — KNOWLEDGE_PIPELINE auditor

> Standalone trigger that runs `knowledge-pipeline_rubric@1.0` against one or
> more existing `knowledge-pipeline@1` markdowns and writes a sibling
> `knowledge-pipeline.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`knowledge-pipeline-author`](../knowledge-pipeline-author/SKILL.md).

`prompt_revision: knowledge-pipeline_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `KNOWLEDGE_PIPELINE`s."
- "Has `KNOWLEDGE_PIPELINE-007` changed since the last audit?"
- "Tell me which `KNOWLEDGE_PIPELINE`s would fail acceptance today."

Also invoked automatically by the supervisor when `knowledge-pipeline-author`'s output envelope sets `next_skill_recommendation: knowledge-pipeline-audit` (the default chain).
...
```json
{
  "artefact_paths": ["./knowledge-pipelines/KNOWLEDGE_PIPELINE-001-foo.md", "./knowledge-pipelines/KNOWLEDGE_PIPELINE-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "knowledge-pipeline-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./knowledge-pipelines/KNOWLEDGE_PIPELINE-001-foo.md", "audit_path": "./knowledge-pipelines/KNOWLEDGE_PIPELINE-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./knowledge-pipelines/KNOWLEDGE_PIPELINE-002-bar.md", "audit_path": "./knowledge-pipelines/KNOWLEDGE_PIPELINE-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   knowledge-pipeline-audit
Input:
  artefact_paths:  [./team-a/KNOWLEDGE_PIPELINE-001-something.md, ./team-b/KNOWLEDGE_PIPELINE-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
