...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['product-roadmap-audit']; empty list disables chaining
...
# product-roadmap-author — PRODUCT-ROADMAP generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `product-roadmap@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`product-roadmap-audit`](../product-roadmap-audit/SKILL.md) by default.

`prompt_revision: product-roadmap_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a PRODUCT-ROADMAP."
- "Generate v2 of the PRODUCT-ROADMAP from the updated source."
- "Draft a PRODUCT-ROADMAP for <project>."

If the user asks to *audit an existing PRODUCT-ROADMAP*, route to `product-roadmap-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        product-roadmap-author
skill_version:                   1.0.0
prompt_revision:                 product-roadmap_author@1.0.0
...
naming_pattern:                  PRODUCT-ROADMAP-{NNN}-{slug}.md
...
```json
{
  ...
  "artefacts_written": [
    {"id": "PRODUCT-ROADMAP-001", "path": "./product-roadmaps/PRODUCT-ROADMAP-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "product-roadmap-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/product-roadmap/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `product-roadmap_author.product-roadmap_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `product-roadmap-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `product-roadmap-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   product-roadmap-author
Input:
