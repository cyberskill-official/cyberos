...
We do NOT validate the year-of-birth as "plausible" (e.g. not in the future) because the skill is structural-only — same posture as `vietnam-mst-validate`. A live registry lookup against the Cơ sở Dữ liệu Quốc gia về Dân cư (CSDLQGDC) via VNeID is the way to confirm an issued CCCD; that requires partner credentials.
