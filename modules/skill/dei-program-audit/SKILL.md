---
# ── Identity ─────────────────────────────────────────────────────────
name: dei-program-audit
description: |
  Audit one or more existing dei-program@1 markdowns against
...
metadata:
  ...
  cyberos-template: dei-program@1
...
# dei-program-audit — DEI-PROGRAM auditor

> Standalone trigger that runs `dei-program_rubric@1.0` against one or
> more existing `dei-program@1` markdowns and writes a sibling
...
> Chains naturally after [`dei-program-author`](../dei-program-author/SKILL.md).
...
Also invoked automatically by the supervisor when `dei-program-author`'s output envelope sets `next_skill_recommendation: dei-program-audit` (the default chain).
