---
# ── Identity ─────────────────────────────────────────────────────────
name: automation-roadmap-author
description: >-
  Author the quarterly automation roadmap (opportunity portfolio + per-opportunity tech choice + ROI tracking + governance + risk register). UiPath / Automation Anywhere / Power Automate vendor patterns. Chief Automation Officer authored. Use when user asks to "draft a automation roadmap" or "create the automation roadmap". Do NOT use for "audit existing automation roadmap" (use automation-roadmap-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: automation-roadmap_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['automation-roadmap-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        automation-roadmap
    ...
    pin_path:  cyberos/skill/contracts/automation-roadmap/
  - id:        nats-subjects
    version:   v1
...
# automation-roadmap-author — AUTOMATION-ROADMAP generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `automation-roadmap@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`automation-roadmap-audit`](../automation-roadmap-audit/SKILL.md) by default.

`prompt_revision: automation-roadmap_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a AUTOMATION-ROADMAP."
- "Generate v2 of the AUTOMATION-ROADMAP from the updated source."
- "Draft a AUTOMATION-ROADMAP for <project>."

If the user asks to *audit an existing AUTOMATION-ROADMAP*, route to `automation-roadmap-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  AUTOMATION-ROADMAP-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./automation-roadmaps/",
...
```json
{
  ...
  "artefacts_written": [
    {"id": "AUTOMATION-ROADMAP-001", "path": "./automation-roadmaps/AUTOMATION-ROADMAP-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "automation-roadmap-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/automation-roadmap/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `automation-roadmap_author.automation-roadmap_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `automation-roadmap-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `automation-roadmap-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   automation-roadmap-author
Input:
