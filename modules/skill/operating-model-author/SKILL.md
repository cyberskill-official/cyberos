...
description: >-
  Author the annual operating model (operating principles + function design + process architecture + decision rights + RACI + SLA architecture + governance cadence). COO authored; CEO approved. Use when user asks to "draft a operating model" or "create the operating model". Do NOT use for "audit existing operating model" (use operating-model-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: operating-model_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['operating-model-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        operating-model
    ...
    pin_path:  cyberos/skill/contracts/operating-model/
  - id:        nats-subjects
    version:   v1
...
# operating-model-author — OPERATING-MODEL generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `operating-model@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`operating-model-audit`](../operating-model-audit/SKILL.md) by default.

`prompt_revision: operating-model_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a OPERATING-MODEL."
- "Generate v2 of the OPERATING-MODEL from the updated source."
- "Draft a OPERATING-MODEL for <project>."

If the user asks to *audit an existing OPERATING-MODEL*, route to `operating-model-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 operating-model_author@1.0.0
template_version:                operating-model@1   (loaded from cyberos/skill/contracts/operating-model/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  OPERATING-MODEL-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./operating-models/",
  "manifest_path": "./operating-models/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "OPERATING-MODEL-001", "path": "./operating-models/OPERATING-MODEL-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "operating-model-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/operating-model/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `operating-model_author.operating-model_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
- If chained to `operating-model-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `operating-model-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   operating-model-author
Input:
