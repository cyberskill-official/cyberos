---
# ── Identity ─────────────────────────────────────────────────────────
name: nps-program-audit
description: |
  Audit one or more existing nps-program@1 markdowns against
...
metadata:
  ...
  cyberos-template: nps-program@1
...
# nps-program-audit — NPS-PROGRAM auditor

> Standalone trigger that runs `nps-program_rubric@1.0` against one or
> more existing `nps-program@1` markdowns and writes a sibling
...
> Chains naturally after [`nps-program-author`](../nps-program-author/SKILL.md).
...
Also invoked automatically by the supervisor when `nps-program-author`'s output envelope sets `next_skill_recommendation: nps-program-audit` (the default chain).
