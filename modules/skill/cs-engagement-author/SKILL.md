...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['cs-engagement-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        cs-engagement
    ...
    pin_path:  cyberos/skill/contracts/cs-engagement/
  - id:        nats-subjects
    version:   v1
...
# cs-engagement-author — CS-ENGAGEMENT generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `cs-engagement@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`cs-engagement-audit`](../cs-engagement-audit/SKILL.md) by default.
...
If the user asks to *audit an existing CS-ENGAGEMENT*, route to `cs-engagement-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        cs-engagement-author
skill_version:                   1.0.0
prompt_revision:                 cs-engagement_author@1.0.0
template_version:                cs-engagement@1   (loaded from cyberos/skill/contracts/cs-engagement/template.md)
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "cs-engagement-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/cs-engagement/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `cs-engagement-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `cs-engagement-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   cs-engagement-author
Input:
