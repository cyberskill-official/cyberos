---
# ── Identity ─────────────────────────────────────────────────────────
name: safety-report-audit
description: >-
  Audit one or more existing safety-report@1 markdowns against safety-report_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after safety-report-author. Use when user asks to "audit this safety report" or "check the safety report". Do NOT use for "draft a new safety report" (use safety-report-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: safety-report@1
  cyberos-rubric-version: safety-report_rubric@1.0
...
# safety-report-audit — SAFETY-REPORT auditor

> Standalone trigger that runs `safety-report_rubric@1.0` against one or
> more existing `safety-report@1` markdowns and writes a sibling
> `safety-report.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`safety-report-author`](../safety-report-author/SKILL.md).

`prompt_revision: safety-report_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `SAFETY-REPORT`s."
- "Has `SAFETY-REPORT-007` changed since the last audit?"
- "Tell me which `SAFETY-REPORT`s would fail acceptance today."

Also invoked automatically by the supervisor when `safety-report-author`'s output envelope sets `next_skill_recommendation: safety-report-audit` (the default chain).
...
```json
{
  "artefact_paths": ["./safety-reports/SAFETY-REPORT-001-foo.md", "./safety-reports/SAFETY-REPORT-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "safety-report-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./safety-reports/SAFETY-REPORT-001-foo.md", "audit_path": "./safety-reports/SAFETY-REPORT-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./safety-reports/SAFETY-REPORT-002-bar.md", "audit_path": "./safety-reports/SAFETY-REPORT-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   safety-report-audit
Input:
  artefact_paths:  [./team-a/SAFETY-REPORT-001-something.md, ./team-b/SAFETY-REPORT-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
