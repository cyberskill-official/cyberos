...
description: >-
  Audit one or more existing data-governance@1 markdowns against data-governance_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after data-governance-author. Use when user asks to "audit this data governance" or "check the data governance". Do NOT use for "draft a new data governance" (use data-governance-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: data-governance@1
  cyberos-rubric-version: data-governance_rubric@1.0
...
# data-governance-audit — DATA-GOVERNANCE auditor

> Standalone trigger that runs `data-governance_rubric@1.0` against one or
> more existing `data-governance@1` markdowns and writes a sibling
> `data-governance.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`data-governance-author`](../data-governance-author/SKILL.md).

`prompt_revision: data-governance_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `DATA-GOVERNANCE`s."
- "Has `DATA-GOVERNANCE-007` changed since the last audit?"
- "Tell me which `DATA-GOVERNANCE`s would fail acceptance today."

Also invoked automatically by the supervisor when `data-governance-author`'s output envelope sets `next_skill_recommendation: data-governance-audit` (the default chain).
...
audit_rubric_version:            data-governance_rubric@1.0
...
```json
{
  "artefact_paths": ["./data-governances/DATA-GOVERNANCE-001-foo.md", "./data-governances/DATA-GOVERNANCE-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "data-governance-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./data-governances/DATA-GOVERNANCE-001-foo.md", "audit_path": "./data-governances/DATA-GOVERNANCE-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./data-governances/DATA-GOVERNANCE-002-bar.md", "audit_path": "./data-governances/DATA-GOVERNANCE-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   data-governance-audit
Input:
  artefact_paths:  [./team-a/DATA-GOVERNANCE-001-something.md, ./team-b/DATA-GOVERNANCE-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
