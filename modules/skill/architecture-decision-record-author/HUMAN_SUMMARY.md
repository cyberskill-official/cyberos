# `architecture-decision-record-author` — human summary format
...
```
ADR batch <batch_run_id> complete

Wrote N ADR(s):
  - ADR-001: <slug> — PASS (audit verdict: pass, 0 open issues)
  - ADR-002: <slug> — HITL_PAUSE (2 blocking issues — see HITL_BATCH_REQUEST below)
  - ADR-003: <slug> — PASS (audit verdict: pass, 1 warning)

Manifest:    <output_dir>/manifest.json
Total time:  <seconds>s
Next:        architecture-decision-record-audit will run automatically on PASS items. HITL items wait for your reply.
```
