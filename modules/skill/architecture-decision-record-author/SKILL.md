...
description: |
  Author one or more Architecture Decision Records (adr@1) from an
  approved SRS plus architectural-spike output, capturing significant
  technical decisions with context, options considered, decision,
  consequences. Aligned with Michael Nygard's ADR format and arc42
  documentation conventions. Covers Software Development Process.md
  §2(d) System architecture and high-level design. Chains naturally
  into architecture-decision-record-audit, threat-model-author, and software-design-document-author.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: d
  cyberos-template: architecture-decision-record@1
  cyberos-rubric-target: adr_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['architecture-decision-record-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        adr
    ...
    pin_path:  cyberos/skill/contracts/architecture-decision-record/
  - id:        nats-subjects
    version:   v1
...
# architecture-decision-record-author — ADR generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `architecture-decision-record@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`architecture-decision-record-audit`](../architecture-decision-record-audit/SKILL.md) by default.

`prompt_revision: adr_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a ADR."
- "Generate v2 of the ADR from the updated source."
- "Draft a ADR for <project>."

If the user asks to *audit an existing ADR*, route to `architecture-decision-record-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 adr_author@1.0.0
template_version:                architecture-decision-record@1   (loaded from cyberos/skill/contracts/architecture-decision-record/template.md)
...
```json
{
  ...
  "output_dir": "./adrs/",
  "manifest_path": "./adrs/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "ADR-001", "path": "./adrs/ADR-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "architecture-decision-record-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/architecture-decision-record/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `adr_author.adr_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `architecture-decision-record-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `architecture-decision-record-audit` (and other downstream consumers).
