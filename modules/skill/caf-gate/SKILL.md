...
description: >-
  ...
  CLEAN or RED result. Used by chief-technology-officer/ship-feature-requests at step 29, after the
  awh-gate (step 28) and before the done flip (step 30). CLEAN is required to reach done; RED routes
  the feature request back to ready_to_implement per STATUS-REFERENCE section 1.3. Use when the user
  ...
  feature-request-audit); this skill catches the class awh cannot see - a build or lint break, a
...
allowed_memory_scopes:
  read:
    ...
    - module:*
  write:
    - project:fr/{fr_id}/caf-gate
audit:
  row_kind: caf_gate_result
  required_fields: [fr_id, module, outcome, target_health, findings_high, harness_version]
...
Step 29 of `chief-technology-officer/ship-feature-requests`, between the awh-gate (step 28) and the
...
1. Resolve the feature request's module and its `modules/<module>/audit-profile.yaml`.
...
3. Read the verdict. CLEAN (target health passes and no new High/Critical) is required to proceed to
   the done flip. RED routes the feature request back to `ready_to_implement` per STATUS-REFERENCE
   section 1.3 with `routed_back_count += 1`.
4. Emit one `caf_gate_result` row into the memory audit chain carrying `{fr_id, module, outcome,
