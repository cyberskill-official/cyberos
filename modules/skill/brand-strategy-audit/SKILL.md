---
# ── Identity ─────────────────────────────────────────────────────────
name: brand-strategy-audit
description: >-
  Audit one or more existing brand-strategy@1 markdowns against brand-strategy_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after brand-strategy-author. Use when user asks to "audit this brand strategy" or "check the brand strategy". Do NOT use for "draft a new brand strategy" (use brand-strategy-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: brand-strategy@1
  cyberos-rubric-version: brand-strategy_rubric@1.0
...
# brand-strategy-audit — BRAND-STRATEGY auditor

> Standalone trigger that runs `brand-strategy_rubric@1.0` against one or
> more existing `brand-strategy@1` markdowns and writes a sibling
> `brand-strategy.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`brand-strategy-author`](../brand-strategy-author/SKILL.md).

`prompt_revision: brand-strategy_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `BRAND-STRATEGY`s."
- "Has `BRAND-STRATEGY-007` changed since the last audit?"
- "Tell me which `BRAND-STRATEGY`s would fail acceptance today."

Also invoked automatically by the supervisor when `brand-strategy-author`'s output envelope sets `next_skill_recommendation: brand-strategy-audit` (the default chain).
...
audit_rubric_version:            brand-strategy_rubric@1.0
...
```json
{
  "artefact_paths": ["./brand-strategys/BRAND-STRATEGY-001-foo.md", "./brand-strategys/BRAND-STRATEGY-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "brand-strategy-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./brand-strategys/BRAND-STRATEGY-001-foo.md", "audit_path": "./brand-strategys/BRAND-STRATEGY-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./brand-strategys/BRAND-STRATEGY-002-bar.md", "audit_path": "./brand-strategys/BRAND-STRATEGY-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   brand-strategy-audit
Input:
  artefact_paths:  [./team-a/BRAND-STRATEGY-001-something.md, ./team-b/BRAND-STRATEGY-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
