...
description: >-
  Audits the customer-360 / CDP (Customer Data Platform) state and proposes the next-quarter architecture refinement. Inventory of customer-data sources, identity resolution match-rate, master entity model, activation surfaces, consent + governance posture, data-quality scorecards, gap analysis. Per CDP Institute reference architecture + Segment / RudderStack / mParticle CDP patterns + IAB GVL/TCF + ISO/IEC 19944 + DAMA-DMBOK MDM chapter. Author a CUSTOMER_360 markdown from source artefact(s). Generates a versioned customer-360@1 file under output_dir, with per-claim authority markers and provenance to the source. Chains naturally into customer-360-audit by default. Use when user asks to "draft a customer 360" or "create the customer 360". Do NOT use for "audit existing customer 360" (use customer-360-audit instead). Author a CUSTOMER_360 markdown from source artefact(s). Generates a versioned customer-360@1 file under output_dir, with per-claim authority markers and provenance to the source. Chains natural...
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: customer-360_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['customer-360-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        customer-360
    ...
    pin_path:  cyberos/skill/contracts/customer-360/
  - id:        nats-subjects
    version:   v1
...
# customer-360-author — CUSTOMER_360 generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `customer-360@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`customer-360-audit`](../customer-360-audit/SKILL.md) by default.

`prompt_revision: customer-360_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a CUSTOMER_360."
- "Generate v2 of the CUSTOMER_360 from the updated source."
- "Draft a CUSTOMER_360 for <project>."

If the user asks to *audit an existing CUSTOMER_360*, route to `customer-360-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 customer-360_author@1.0.0
template_version:                customer-360@1   (loaded from cyberos/skill/contracts/customer-360/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  CUSTOMER_360-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./customer-360s/",
  "manifest_path": "./customer-360s/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "CUSTOMER_360-001", "path": "./customer-360s/CUSTOMER_360-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "customer-360-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/customer-360/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `customer-360_author.customer-360_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `customer-360-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `customer-360-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   customer-360-author
Input:
