---
# ── Identity ─────────────────────────────────────────────────────────
name: regulatory-filing-audit
description: >-
  Audits a regulatory-filing@1 artefact against regulatory_filing_rubric@1.0. Validates FM + SEC structure with regulator-specific section ordering (SEC 10-K Items 1-15 / FDA IND CTD modules / AI Act Annex IV / GDPR Art. 33 / Vietnam Decree 13/2023 PDPD), material-disclosure sourcing, and exhibit cross-references. Audit one or more existing regulatory-filing@1 markdowns against regulatory-filing_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after regulatory-filing-author. Use when user asks to "audit this regulatory filing" or "check the regulatory filing". Do NOT use for "draft a new regulatory filing" (use regulatory-filing-author instead). Audit one or more existing regulatory-filing@1 markdowns against regulatory-filing_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256....
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: regulatory-filing@1
  cyberos-rubric-version: regulatory-filing_rubric@1.0
...
depends_on_contracts:
  - id:        regulatory-filing
    ...
    pin_path:  cyberos/skill/contracts/regulatory-filing/
  - id:        nats-subjects
    version:   v1
...
# regulatory-filing-audit — REGULATORY_FILING auditor

> Standalone trigger that runs `regulatory-filing_rubric@1.0` against one or
> more existing `regulatory-filing@1` markdowns and writes a sibling
> `regulatory-filing.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`regulatory-filing-author`](../regulatory-filing-author/SKILL.md).

`prompt_revision: regulatory-filing_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `REGULATORY_FILING`s."
- "Has `REGULATORY_FILING-007` changed since the last audit?"
- "Tell me which `REGULATORY_FILING`s would fail acceptance today."

Also invoked automatically by the supervisor when `regulatory-filing-author`'s output envelope sets `next_skill_recommendation: regulatory-filing-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        regulatory-filing-audit
skill_version:                   1.0.0
prompt_revision:                 regulatory-filing_audit@1.0.0
template_version:                regulatory-filing@1   (loaded from cyberos/skill/contracts/regulatory-filing/template.md)
audit_rubric_version:            regulatory-filing_rubric@1.0
...
```json
{
  "artefact_paths": ["./regulatory-filings/REGULATORY_FILING-001-foo.md", "./regulatory-filings/REGULATORY_FILING-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "regulatory-filing-author",
    "manifest_path": "./regulatory-filings/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./regulatory-filings/REGULATORY_FILING-001-foo.md", "audit_path": "./regulatory-filings/REGULATORY_FILING-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./regulatory-filings/REGULATORY_FILING-002-bar.md", "audit_path": "./regulatory-filings/REGULATORY_FILING-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   regulatory-filing-audit
Input:
  artefact_paths:  [./team-a/REGULATORY_FILING-001-something.md, ./team-b/REGULATORY_FILING-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
