# `operating_model_rubric@1.0` — machine-checkable Operating Model rubric
...
| `FM-004` | `template` equals `operating-model@1` | error | true |
...
Per the `contracts/operating-model/template.md` H2 list. Each H2 in the template MUST be present + non-empty in the authored artefact. See template for the canonical headings.

| rule_id | Check | Severity |
| ------- | ----- | -------- |
| `SEC-001` | Every H2 declared in `contracts/operating-model/template.md` is present in the authored artefact | error |
...
- `cyberos/skill/contracts/operating-model/CONTRACT.md` + `template.md`
