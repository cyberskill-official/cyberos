...
description: >-
  Audit one or more existing operating-model@1 markdowns against operating-model_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after operating-model-author. Use when user asks to "audit this operating model" or "check the operating model". Do NOT use for "draft a new operating model" (use operating-model-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: operating-model@1
  cyberos-rubric-version: operating-model_rubric@1.0
...
# operating-model-audit — OPERATING-MODEL auditor

> Standalone trigger that runs `operating-model_rubric@1.0` against one or
> more existing `operating-model@1` markdowns and writes a sibling
> `operating-model.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`operating-model-author`](../operating-model-author/SKILL.md).

`prompt_revision: operating-model_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `OPERATING-MODEL`s."
- "Has `OPERATING-MODEL-007` changed since the last audit?"
- "Tell me which `OPERATING-MODEL`s would fail acceptance today."

Also invoked automatically by the supervisor when `operating-model-author`'s output envelope sets `next_skill_recommendation: operating-model-audit` (the default chain).
...
audit_rubric_version:            operating-model_rubric@1.0
...
```json
{
  "artefact_paths": ["./operating-models/OPERATING-MODEL-001-foo.md", "./operating-models/OPERATING-MODEL-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "operating-model-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./operating-models/OPERATING-MODEL-001-foo.md", "audit_path": "./operating-models/OPERATING-MODEL-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./operating-models/OPERATING-MODEL-002-bar.md", "audit_path": "./operating-models/OPERATING-MODEL-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   operating-model-audit
Input:
  artefact_paths:  [./team-a/OPERATING-MODEL-001-something.md, ./team-b/OPERATING-MODEL-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
