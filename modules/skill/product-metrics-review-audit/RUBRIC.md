# product-metrics-review_rubric@1.0 — compact universal rubric
...
| FM-002 | `artefact_type` matches `product-metrics-review@1` |
...
| ID | Rule |
|---|---|
| SEC-001 | Every H2 in `contracts/product-metrics-review/template.md` is present in the authored artefact |
| SEC-002 | Every H2 in the authored artefact is non-empty |
| SEC-003 | H2 ordering matches `template.md` |
| SEC-004 | Heading hierarchy is well-formed |

## COND family — conditional comment-block rules

| ID | Rule |
|---|---|
| COND-001 | Every `<!-- conditional: ... -->` block in `template.md` is satisfied per its declared trigger |

## QA family — quality heuristics

| ID | Rule |
|---|---|
| QA-CITE | Every claim that cites an external standard / framework names the standard |
...
| QA-VAGUE | No vague qualifiers without a number or named exception |
...
| ID | Rule |
|---|---|
| STALE-001 | `provenance.source_hash` matches the live sha256 of the source at `provenance.source_path`; if not, fire STALE handling |
