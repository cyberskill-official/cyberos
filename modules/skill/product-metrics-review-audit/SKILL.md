...
metadata:
  ...
  module: skill
  stage: cross
  cyberos-template: product-metrics-review@1
...
# product-metrics-review-audit — PRODUCT_METRICS_REVIEW auditor

> Standalone trigger that runs `product-metrics-review_rubric@1.0` against one or
> more existing `product-metrics-review@1` markdowns and writes a sibling
> `product-metrics-review.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`product-metrics-review-author`](../product-metrics-review-author/SKILL.md).

`prompt_revision: product-metrics-review_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `PRODUCT_METRICS_REVIEW`s."
- "Has `PRODUCT_METRICS_REVIEW-007` changed since the last audit?"
- "Tell me which `PRODUCT_METRICS_REVIEW`s would fail acceptance today."

Also invoked automatically by the supervisor when `product-metrics-review-author`'s output envelope sets `next_skill_recommendation: product-metrics-review-audit` (the default chain).
...
audit_rubric_version:            product-metrics-review_rubric@1.0
...
```json
{
  "artefact_paths": ["./product-metrics-reviews/PRODUCT_METRICS_REVIEW-001-foo.md", "./product-metrics-reviews/PRODUCT_METRICS_REVIEW-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "product-metrics-review-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./product-metrics-reviews/PRODUCT_METRICS_REVIEW-001-foo.md", "audit_path": "./product-metrics-reviews/PRODUCT_METRICS_REVIEW-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./product-metrics-reviews/PRODUCT_METRICS_REVIEW-002-bar.md", "audit_path": "./product-metrics-reviews/PRODUCT_METRICS_REVIEW-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   product-metrics-review-audit
Input:
  artefact_paths:  [./team-a/PRODUCT_METRICS_REVIEW-001-something.md, ./team-b/PRODUCT_METRICS_REVIEW-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
