---
# ── Identity ─────────────────────────────────────────────────────────
name: statement-of-work-audit
description: >-
  Audit one or more existing statement-of-work@1 markdowns against sow_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after statement-of-work-author. Use when user asks to "audit this statement of work" or "check the statement of work". Do NOT use for "draft a new statement of work" (use statement-of-work-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-version: sow_rubric@1.0
...
depends_on_contracts:
  - id:        sow
    ...
    pin_path:  cyberos/skill/contracts/statement-of-work/
  - id:        nats-subjects
    version:   v1
...
# statement-of-work-audit — SOW auditor

> Standalone trigger that runs `sow_rubric@1.0` against one or
> more existing `statement-of-work@1` markdowns and writes a sibling
> `sow.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`statement-of-work-author`](../statement-of-work-author/SKILL.md).

`prompt_revision: sow_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `SOW`s."
- "Has `SOW-007` changed since the last audit?"
- "Tell me which `SOW`s would fail acceptance today."

Also invoked automatically by the supervisor when `statement-of-work-author`'s output envelope sets `next_skill_recommendation: statement-of-work-audit` (the default chain).
...
skill_version:                   1.0.0
prompt_revision:                 sow_audit@1.0.0
template_version:                statement-of-work@1   (loaded from cyberos/skill/contracts/statement-of-work/template.md)
audit_rubric_version:            sow_rubric@1.0
...
```json
{
  "artefact_paths": ["./sows/SOW-001-foo.md", "./sows/SOW-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "statement-of-work-author",
    "manifest_path": "./sows/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./sows/SOW-001-foo.md", "audit_path": "./sows/SOW-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./sows/SOW-002-bar.md", "audit_path": "./sows/SOW-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   statement-of-work-audit
Input:
  artefact_paths:  [./team-a/SOW-001-something.md, ./team-b/SOW-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
