...
description: >-
  ...
  feature request's section-1 cited tests plus its module suite via `awh eval` against a sealed,
...
allowed_memory_scopes:
  read:
    ...
    - module:*
  write:
    - project:fr/{fr_id}/awh-gate
audit:
  row_kind: awh_gate_result
  required_fields: [fr_id, module, outcome, weighted_pass, harness_version, sealed_acceptance_hash]
...
gate, and a feature-request audit, but each of those runs in the authoring context, so the model
...
1. Resolve the feature request's module and its golden set at `modules/<module>/.awh/goldenset.yaml`.
...
3. Read the verdict. GREEN (no task regressed against the sealed baseline) is required to proceed
   to the done flip. RED routes the feature request back to `ready_to_implement` per
   STATUS-REFERENCE section 1.3 with `routed_back_count += 1`.
4. Emit one `awh_gate_result` row into the memory audit chain carrying `{fr_id, module, outcome,
