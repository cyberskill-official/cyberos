---
# ── Identity ─────────────────────────────────────────────────────────
name: decision-log-author
description: >-
  Author / maintain the company decision log (decision-id + context + options + decision + owner + reversibility + outcome-review-date). Chief of Staff maintained. Use when user asks to "draft a decision log" or "create the decision log". Do NOT use for "audit existing decision log" (use decision-log-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: decision-log_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['decision-log-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        decision-log
    ...
    pin_path:  cyberos/skill/contracts/decision-log/
  - id:        nats-subjects
    version:   v1
...
# decision-log-author — DECISION-LOG generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `decision-log@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`decision-log-audit`](../decision-log-audit/SKILL.md) by default.

`prompt_revision: decision-log_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a DECISION-LOG."
- "Generate v2 of the DECISION-LOG from the updated source."
- "Draft a DECISION-LOG for <project>."

If the user asks to *audit an existing DECISION-LOG*, route to `decision-log-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        decision-log-author
skill_version:                   1.0.0
prompt_revision:                 decision-log_author@1.0.0
template_version:                decision-log@1   (loaded from cyberos/skill/contracts/decision-log/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  DECISION-LOG-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./decision-logs/",
  "manifest_path": "./decision-logs/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "DECISION-LOG-001", "path": "./decision-logs/DECISION-LOG-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "decision-log-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/decision-log/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `decision-log_author.decision-log_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
- If chained to `decision-log-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `decision-log-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   decision-log-author
Input:
