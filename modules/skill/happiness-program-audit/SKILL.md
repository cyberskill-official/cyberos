---
# ── Identity ─────────────────────────────────────────────────────────
name: happiness-program-audit
description: >-
  Audit one or more existing happiness-program@1 markdowns against happiness-program_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after happiness-program-author. Use when user asks to "audit this happiness program" or "check the happiness program". Do NOT use for "draft a new happiness program" (use happiness-program-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: happiness-program@1
  cyberos-rubric-version: happiness-program_rubric@1.0
...
depends_on_contracts:
  - id:        happiness-program
    ...
    pin_path:  cyberos/skill/contracts/happiness-program/
  - id:        nats-subjects
    version:   v1
...
# happiness-program-audit — HAPPINESS-PROGRAM auditor

> Standalone trigger that runs `happiness-program_rubric@1.0` against one or
> more existing `happiness-program@1` markdowns and writes a sibling
> `happiness-program.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`happiness-program-author`](../happiness-program-author/SKILL.md).

`prompt_revision: happiness-program_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `HAPPINESS-PROGRAM`s."
- "Has `HAPPINESS-PROGRAM-007` changed since the last audit?"
- "Tell me which `HAPPINESS-PROGRAM`s would fail acceptance today."

Also invoked automatically by the supervisor when `happiness-program-author`'s output envelope sets `next_skill_recommendation: happiness-program-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        happiness-program-audit
skill_version:                   1.0.0
prompt_revision:                 happiness-program_audit@1.0.0
template_version:                happiness-program@1   (loaded from cyberos/skill/contracts/happiness-program/template.md)
audit_rubric_version:            happiness-program_rubric@1.0
...
```json
{
  "artefact_paths": ["./happiness-programs/HAPPINESS-PROGRAM-001-foo.md", "./happiness-programs/HAPPINESS-PROGRAM-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "happiness-program-author",
    "manifest_path": "./happiness-programs/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./happiness-programs/HAPPINESS-PROGRAM-001-foo.md", "audit_path": "./happiness-programs/HAPPINESS-PROGRAM-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./happiness-programs/HAPPINESS-PROGRAM-002-bar.md", "audit_path": "./happiness-programs/HAPPINESS-PROGRAM-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   happiness-program-audit
Input:
  artefact_paths:  [./team-a/HAPPINESS-PROGRAM-001-something.md, ./team-b/HAPPINESS-PROGRAM-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
