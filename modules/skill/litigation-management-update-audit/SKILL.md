...
metadata:
  ...
  module: skill
  stage: cross
  cyberos-template: litigation-management-update@1
...
# litigation-management-update-audit — LITIGATION_MGMT_UPDATE auditor

> Standalone trigger that runs `litigation-mgmt-update_rubric@1.0` against one or
> more existing `litigation-management-update@1` markdowns and writes a sibling
...
> Chains naturally after [`litigation-management-update-author`](../litigation-management-update-author/SKILL.md).
...
Also invoked automatically by the supervisor when `litigation-management-update-author`'s output envelope sets `next_skill_recommendation: litigation-management-update-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        litigation-management-update-audit
skill_version:                   1.0.0
prompt_revision:                 litigation-mgmt-update_audit@1.0.0
...
```json
{
  ...
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "litigation-management-update-author",
...
```
Persona: cuo-cpo
Skill:   litigation-management-update-audit
Input:
