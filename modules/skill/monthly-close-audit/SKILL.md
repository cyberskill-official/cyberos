---
# ── Identity ─────────────────────────────────────────────────────────
name: monthly-close-audit
description: >-
  Audit one or more existing monthly-close@1 markdowns against monthly-close_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after monthly-close-author. Use when user asks to "audit this monthly close" or "check the monthly close". Do NOT use for "draft a new monthly close" (use monthly-close-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: monthly-close@1
  cyberos-rubric-version: monthly-close_rubric@1.0
...
# monthly-close-audit — MONTHLY-CLOSE auditor

> Standalone trigger that runs `monthly-close_rubric@1.0` against one or
> more existing `monthly-close@1` markdowns and writes a sibling
> `monthly-close.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`monthly-close-author`](../monthly-close-author/SKILL.md).

`prompt_revision: monthly-close_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `MONTHLY-CLOSE`s."
- "Has `MONTHLY-CLOSE-007` changed since the last audit?"
- "Tell me which `MONTHLY-CLOSE`s would fail acceptance today."

Also invoked automatically by the supervisor when `monthly-close-author`'s output envelope sets `next_skill_recommendation: monthly-close-audit` (the default chain).
...
audit_rubric_version:            monthly-close_rubric@1.0
...
```json
{
  "artefact_paths": ["./monthly-closes/MONTHLY-CLOSE-001-foo.md", "./monthly-closes/MONTHLY-CLOSE-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "monthly-close-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./monthly-closes/MONTHLY-CLOSE-001-foo.md", "audit_path": "./monthly-closes/MONTHLY-CLOSE-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./monthly-closes/MONTHLY-CLOSE-002-bar.md", "audit_path": "./monthly-closes/MONTHLY-CLOSE-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   monthly-close-audit
Input:
  artefact_paths:  [./team-a/MONTHLY-CLOSE-001-something.md, ./team-b/MONTHLY-CLOSE-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
