# `monthly_close_rubric@1.0` — machine-checkable Monthly Close rubric
...
| `FM-004` | `template` equals `monthly-close@1` | error | true |
...
Per the `contracts/monthly-close/template.md` H2 list. Each H2 in the template MUST be present + non-empty in the authored artefact. See template for the canonical headings.

| rule_id | Check | Severity |
| ------- | ----- | -------- |
| `SEC-001` | Every H2 declared in `contracts/monthly-close/template.md` is present in the authored artefact | error |
...
- `cyberos/skill/contracts/monthly-close/CONTRACT.md` + `template.md`
