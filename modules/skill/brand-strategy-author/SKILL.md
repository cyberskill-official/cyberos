---
# ── Identity ─────────────────────────────────────────────────────────
name: brand-strategy-author
description: >-
  Author the annual brand strategy (positioning + brand architecture + brand voice + visual system + messaging pillars + awareness goals). Aaker brand-equity + BAV BrandAsset Valuator aligned. CMO or Chief Brand Officer authored. Use when user asks to "draft a brand strategy" or "create the brand strategy". Do NOT use for "audit existing brand strategy" (use brand-strategy-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: brand-strategy_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['brand-strategy-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        brand-strategy
    ...
    pin_path:  cyberos/skill/contracts/brand-strategy/
  - id:        nats-subjects
    version:   v1
...
# brand-strategy-author — BRAND-STRATEGY generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `brand-strategy@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`brand-strategy-audit`](../brand-strategy-audit/SKILL.md) by default.

`prompt_revision: brand-strategy_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a BRAND-STRATEGY."
- "Generate v2 of the BRAND-STRATEGY from the updated source."
- "Draft a BRAND-STRATEGY for <project>."

If the user asks to *audit an existing BRAND-STRATEGY*, route to `brand-strategy-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        brand-strategy-author
skill_version:                   1.0.0
prompt_revision:                 brand-strategy_author@1.0.0
template_version:                brand-strategy@1   (loaded from cyberos/skill/contracts/brand-strategy/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  BRAND-STRATEGY-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./brand-strategys/",
  "manifest_path": "./brand-strategys/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "BRAND-STRATEGY-001", "path": "./brand-strategys/BRAND-STRATEGY-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "brand-strategy-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/brand-strategy/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `brand-strategy_author.brand-strategy_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
- If chained to `brand-strategy-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `brand-strategy-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   brand-strategy-author
Input:
