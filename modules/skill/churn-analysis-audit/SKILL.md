...
metadata:
  ...
  module: skill
  stage: cross
  cyberos-template: churn-analysis@1
...
# churn-analysis-audit — CHURN_ANALYSIS auditor

> Standalone trigger that runs `churn-analysis_rubric@1.0` against one or
> more existing `churn-analysis@1` markdowns and writes a sibling
> `churn-analysis.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`churn-analysis-author`](../churn-analysis-author/SKILL.md).

`prompt_revision: churn-analysis_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `CHURN_ANALYSIS`s."
- "Has `CHURN_ANALYSIS-007` changed since the last audit?"
- "Tell me which `CHURN_ANALYSIS`s would fail acceptance today."

Also invoked automatically by the supervisor when `churn-analysis-author`'s output envelope sets `next_skill_recommendation: churn-analysis-audit` (the default chain).
...
```json
{
  "artefact_paths": ["./churn-analysiss/CHURN_ANALYSIS-001-foo.md", "./churn-analysiss/CHURN_ANALYSIS-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "churn-analysis-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./churn-analysiss/CHURN_ANALYSIS-001-foo.md", "audit_path": "./churn-analysiss/CHURN_ANALYSIS-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./churn-analysiss/CHURN_ANALYSIS-002-bar.md", "audit_path": "./churn-analysiss/CHURN_ANALYSIS-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   churn-analysis-audit
Input:
  artefact_paths:  [./team-a/CHURN_ANALYSIS-001-something.md, ./team-b/CHURN_ANALYSIS-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
