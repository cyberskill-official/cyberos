...
name: dor-dod-audit
description: |
  Audit one or more existing dor-dod@1 markdowns against
...
# dor-dod-audit — DOR-DOD auditor

> Standalone trigger that runs `dor-dod_rubric@1.0` against one or
> more existing `dor-dod@1` markdowns and writes a sibling
...
> Chains naturally after [`dor-dod-author`](../dor-dod-author/SKILL.md).
...
Also invoked automatically by the supervisor when `dor-dod-author`'s output envelope sets `next_skill_recommendation: dor-dod-audit` (the default chain).
