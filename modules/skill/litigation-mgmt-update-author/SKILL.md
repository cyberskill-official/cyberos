---
# ── Identity ─────────────────────────────────────────────────────────
name: litigation-mgmt-update-author
description: Authors a quarterly (or per-board-meeting) litigation management status update. Covers active matters, exposure quantification (USD potential damages), settlement posture, outside-counsel costs vs ACC Value-Challenge benchmark, reserve recommendation, board-chapter content. Per Litify case-mgmt model + ABA Litigation Section best practices.
  ...
  versioned litigation-mgmt-update@1 file under output_dir, with per-claim authority
  markers and provenance to the source. Chains naturally into
  litigation-mgmt-update-audit by default. Refuses to author when upstream artefact
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['litigation-mgmt-update-audit']; empty list disables chaining
...
# litigation-mgmt-update-author — LITIGATION_MGMT_UPDATE generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `litigation-mgmt-update@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`litigation-mgmt-update-audit`](../litigation-mgmt-update-audit/SKILL.md) by default.

`prompt_revision: litigation-mgmt-update_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a LITIGATION_MGMT_UPDATE."
- "Generate v2 of the LITIGATION_MGMT_UPDATE from the updated source."
- "Draft a LITIGATION_MGMT_UPDATE for <project>."

If the user asks to *audit an existing LITIGATION_MGMT_UPDATE*, route to `litigation-mgmt-update-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        litigation-mgmt-update-author
skill_version:                   1.0.0
prompt_revision:                 litigation-mgmt-update_author@1.0.0
...
naming_pattern:                  LITIGATION_MGMT_UPDATE-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./litigation-mgmt-updates/",
...
```json
{
  ...
  "artefacts_written": [
    {"id": "LITIGATION_MGMT_UPDATE-001", "path": "./litigation-mgmt-updates/LITIGATION_MGMT_UPDATE-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "litigation-mgmt-update-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/litigation-mgmt-update/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `litigation-mgmt-update_author.litigation-mgmt-update_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `litigation-mgmt-update-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `litigation-mgmt-update-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   litigation-mgmt-update-author
Input:
