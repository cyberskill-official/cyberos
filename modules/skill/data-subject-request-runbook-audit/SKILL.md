---
# ── Identity ─────────────────────────────────────────────────────────
name: data-subject-request-runbook-audit
description: >-
  Audit one or more existing data-subject-request-runbook@1 markdowns against dsr-runbook_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after data-subject-request-runbook-author. Use when user asks to "audit this data subject request runbook" or "check the data subject request runbook". Do NOT use for "draft a new data subject request runbook" (use data-subject-request-runbook-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: data-subject-request-runbook@1
  cyberos-rubric-version: dsr-runbook_rubric@1.0
...
# data-subject-request-runbook-audit — DSR-RUNBOOK auditor

> Standalone trigger that runs `dsr-runbook_rubric@1.0` against one or
> more existing `data-subject-request-runbook@1` markdowns and writes a sibling
> `dsr-runbook.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`data-subject-request-runbook-author`](../data-subject-request-runbook-author/SKILL.md).

`prompt_revision: dsr-runbook_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `DSR-RUNBOOK`s."
- "Has `DSR-RUNBOOK-007` changed since the last audit?"
- "Tell me which `DSR-RUNBOOK`s would fail acceptance today."

Also invoked automatically by the supervisor when `data-subject-request-runbook-author`'s output envelope sets `next_skill_recommendation: data-subject-request-runbook-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        data-subject-request-runbook-audit
skill_version:                   1.0.0
prompt_revision:                 dsr-runbook_audit@1.0.0
...
audit_rubric_version:            dsr-runbook_rubric@1.0
...
```json
{
  "artefact_paths": ["./dsr-runbooks/DSR-RUNBOOK-001-foo.md", "./dsr-runbooks/DSR-RUNBOOK-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "data-subject-request-runbook-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./dsr-runbooks/DSR-RUNBOOK-001-foo.md", "audit_path": "./dsr-runbooks/DSR-RUNBOOK-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./dsr-runbooks/DSR-RUNBOOK-002-bar.md", "audit_path": "./dsr-runbooks/DSR-RUNBOOK-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   data-subject-request-runbook-audit
Input:
  artefact_paths:  [./team-a/DSR-RUNBOOK-001-something.md, ./team-b/DSR-RUNBOOK-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
