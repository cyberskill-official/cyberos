# `dsr_runbook_rubric@1.0` — machine-checkable DSR Runbook rubric
...
| `FM-004` | `template` equals `data-subject-request-runbook@1` | error | true |
...
Per the `contracts/data-subject-request-runbook/template.md` H2 list. Each H2 in the template MUST be present + non-empty in the authored artefact. See template for the canonical headings.

| rule_id | Check | Severity |
| ------- | ----- | -------- |
| `SEC-001` | Every H2 declared in `contracts/data-subject-request-runbook/template.md` is present in the authored artefact | error |
...
- `cyberos/skill/contracts/data-subject-request-runbook/CONTRACT.md` + `template.md`
