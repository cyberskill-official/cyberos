...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['enps-program-audit']; empty list disables chaining
...
# enps-program-author — ENPS-PROGRAM generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `enps-program@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`enps-program-audit`](../enps-program-audit/SKILL.md) by default.
...
If the user asks to *audit an existing ENPS-PROGRAM*, route to `enps-program-audit` instead. If both, the supervisor chains the two.
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "enps-program-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/enps-program/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `enps-program-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `enps-program-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   enps-program-author
Input:
