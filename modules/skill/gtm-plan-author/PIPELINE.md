# `gtm-plan-author` — pipeline

This document describes how `gtm-plan-author` chains with upstream and downstream skills.
...
|---|---|---|
| `gtm-plan-audit` | Default after every `artefact_written` event | `next_skill_recommendation: gtm-plan-audit` in output envelope. |
| `<next-stage>-author` | After `gtm-plan-audit` returns PASS | Supervisor reads the audit's output envelope and queues the next stage. |
...
- `cyberos/skill/contracts/gtm-plan/` — the artefact template this skill generates.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/gtm-plan-audit/` — the sibling audit skill that validates this skill's output.
