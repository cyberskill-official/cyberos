---
# ── Identity ─────────────────────────────────────────────────────────
name: remote-policy-audit
description: >-
  Audit one or more existing remote-policy@1 markdowns against remote-policy_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after remote-policy-author. Use when user asks to "audit this remote policy" or "check the remote policy". Do NOT use for "draft a new remote policy" (use remote-policy-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: remote-policy@1
  cyberos-rubric-version: remote-policy_rubric@1.0
...
# remote-policy-audit — REMOTE-POLICY auditor

> Standalone trigger that runs `remote-policy_rubric@1.0` against one or
> more existing `remote-policy@1` markdowns and writes a sibling
> `remote-policy.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`remote-policy-author`](../remote-policy-author/SKILL.md).

`prompt_revision: remote-policy_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `REMOTE-POLICY`s."
- "Has `REMOTE-POLICY-007` changed since the last audit?"
- "Tell me which `REMOTE-POLICY`s would fail acceptance today."

Also invoked automatically by the supervisor when `remote-policy-author`'s output envelope sets `next_skill_recommendation: remote-policy-audit` (the default chain).
...
```json
{
  "artefact_paths": ["./remote-policys/REMOTE-POLICY-001-foo.md", "./remote-policys/REMOTE-POLICY-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "remote-policy-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./remote-policys/REMOTE-POLICY-001-foo.md", "audit_path": "./remote-policys/REMOTE-POLICY-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./remote-policys/REMOTE-POLICY-002-bar.md", "audit_path": "./remote-policys/REMOTE-POLICY-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   remote-policy-audit
Input:
  artefact_paths:  [./team-a/REMOTE-POLICY-001-something.md, ./team-b/REMOTE-POLICY-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
