...
description: >-
  Audit one or more existing model-card@1 markdowns against model-card_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after model-card-author. Use when user asks to "audit this model card" or "check the model card". Do NOT use for "draft a new model card" (use model-card-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: model-card@1
  cyberos-rubric-version: model-card_rubric@1.0
...
# model-card-audit — MODEL-CARD auditor

> Standalone trigger that runs `model-card_rubric@1.0` against one or
> more existing `model-card@1` markdowns and writes a sibling
> `model-card.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`model-card-author`](../model-card-author/SKILL.md).

`prompt_revision: model-card_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `MODEL-CARD`s."
- "Has `MODEL-CARD-007` changed since the last audit?"
- "Tell me which `MODEL-CARD`s would fail acceptance today."

Also invoked automatically by the supervisor when `model-card-author`'s output envelope sets `next_skill_recommendation: model-card-audit` (the default chain).
...
```json
{
  "artefact_paths": ["./model-cards/MODEL-CARD-001-foo.md", "./model-cards/MODEL-CARD-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "model-card-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./model-cards/MODEL-CARD-001-foo.md", "audit_path": "./model-cards/MODEL-CARD-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./model-cards/MODEL-CARD-002-bar.md", "audit_path": "./model-cards/MODEL-CARD-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
Input:
  artefact_paths:  [./team-a/MODEL-CARD-001-something.md, ./team-b/MODEL-CARD-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
