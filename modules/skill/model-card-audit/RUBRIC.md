# `model_card_rubric@1.0` — machine-checkable Model Card rubric
...
| `FM-004` | `template` equals `model-card@1` | error | true |
...
Per the `contracts/model-card/template.md` H2 list. Each H2 in the template MUST be present + non-empty in the authored artefact. See template for the canonical headings.

| rule_id | Check | Severity |
| ------- | ----- | -------- |
| `SEC-001` | Every H2 declared in `contracts/model-card/template.md` is present in the authored artefact | error |
...
- `cyberos/skill/contracts/model-card/CONTRACT.md` + `template.md`
