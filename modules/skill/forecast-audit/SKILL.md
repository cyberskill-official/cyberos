...
name: forecast-audit
description: >-
  Audit one or more existing forecast@1 markdowns against forecast_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after forecast-author. Use when user asks to "audit this forecast" or "check the forecast". Do NOT use for "draft a new forecast" (use forecast-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: forecast@1
  cyberos-rubric-version: forecast_rubric@1.0
...
# forecast-audit — FORECAST auditor

> Standalone trigger that runs `forecast_rubric@1.0` against one or
> more existing `forecast@1` markdowns and writes a sibling
> `forecast.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`forecast-author`](../forecast-author/SKILL.md).

`prompt_revision: forecast_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `FORECAST`s."
- "Has `FORECAST-007` changed since the last audit?"
- "Tell me which `FORECAST`s would fail acceptance today."

Also invoked automatically by the supervisor when `forecast-author`'s output envelope sets `next_skill_recommendation: forecast-audit` (the default chain).
...
```json
{
  "artefact_paths": ["./forecasts/FORECAST-001-foo.md", "./forecasts/FORECAST-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "forecast-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./forecasts/FORECAST-001-foo.md", "audit_path": "./forecasts/FORECAST-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./forecasts/FORECAST-002-bar.md", "audit_path": "./forecasts/FORECAST-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   forecast-audit
Input:
  artefact_paths:  [./team-a/FORECAST-001-something.md, ./team-b/FORECAST-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
