---
# ── Identity ─────────────────────────────────────────────────────────
name: clinical-protocol-author
description: |
  Author a per-trial clinical protocol (background + design + endpoints + eligibility + treatment + safety monitoring + statistical analysis plan + ethics/regulatory). ICH-GCP E6(R3). FDA IND / EMA CTR submission-ready. Chief Medical Officer authored.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: clinical-protocol_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['clinical-protocol-audit']; empty list disables chaining
...
# clinical-protocol-author — CLINICAL-PROTOCOL generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `clinical-protocol@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`clinical-protocol-audit`](../clinical-protocol-audit/SKILL.md) by default.

`prompt_revision: clinical-protocol_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a CLINICAL-PROTOCOL."
- "Generate v2 of the CLINICAL-PROTOCOL from the updated source."
- "Draft a CLINICAL-PROTOCOL for <project>."

If the user asks to *audit an existing CLINICAL-PROTOCOL*, route to `clinical-protocol-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  CLINICAL-PROTOCOL-{NNN}-{slug}.md
...
```json
{
  ...
  "artefacts_written": [
    {"id": "CLINICAL-PROTOCOL-001", "path": "./clinical-protocols/CLINICAL-PROTOCOL-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "clinical-protocol-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/clinical-protocol/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `clinical-protocol_author.clinical-protocol_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `clinical-protocol-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `clinical-protocol-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   clinical-protocol-author
Input:
