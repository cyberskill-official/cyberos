...
name: retro-audit
description: |
  Audit one or more existing retro@1 markdowns against
  retro_rubric@1.0. Produces a sibling .audit.md per artefact plus
...
# retro-audit — RETRO auditor

> Standalone trigger that runs `retro_rubric@1.0` against one or
> more existing `retro@1` markdowns and writes a sibling
...
> Chains naturally after [`retro-author`](../retro-author/SKILL.md).
...
Also invoked automatically by the supervisor when `retro-author`'s output envelope sets `next_skill_recommendation: retro-audit` (the default chain).
