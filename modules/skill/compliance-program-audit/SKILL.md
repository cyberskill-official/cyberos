---
# ── Identity ─────────────────────────────────────────────────────────
name: compliance-program-audit
description: >-
  Audit one or more existing compliance-program@1 markdowns against compliance-program_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after compliance-program-author. Use when user asks to "audit this compliance program" or "check the compliance program". Do NOT use for "draft a new compliance program" (use compliance-program-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: compliance-program@1
  cyberos-rubric-version: compliance-program_rubric@1.0
...
depends_on_contracts:
  - id:        compliance-program
    ...
    pin_path:  cyberos/skill/contracts/compliance-program/
  - id:        nats-subjects
    version:   v1
...
# compliance-program-audit — COMPLIANCE-PROGRAM auditor

> Standalone trigger that runs `compliance-program_rubric@1.0` against one or
> more existing `compliance-program@1` markdowns and writes a sibling
> `compliance-program.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`compliance-program-author`](../compliance-program-author/SKILL.md).

`prompt_revision: compliance-program_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `COMPLIANCE-PROGRAM`s."
- "Has `COMPLIANCE-PROGRAM-007` changed since the last audit?"
- "Tell me which `COMPLIANCE-PROGRAM`s would fail acceptance today."

Also invoked automatically by the supervisor when `compliance-program-author`'s output envelope sets `next_skill_recommendation: compliance-program-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        compliance-program-audit
skill_version:                   1.0.0
prompt_revision:                 compliance-program_audit@1.0.0
template_version:                compliance-program@1   (loaded from cyberos/skill/contracts/compliance-program/template.md)
audit_rubric_version:            compliance-program_rubric@1.0
...
```json
{
  "artefact_paths": ["./compliance-programs/COMPLIANCE-PROGRAM-001-foo.md", "./compliance-programs/COMPLIANCE-PROGRAM-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "compliance-program-author",
    "manifest_path": "./compliance-programs/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./compliance-programs/COMPLIANCE-PROGRAM-001-foo.md", "audit_path": "./compliance-programs/COMPLIANCE-PROGRAM-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./compliance-programs/COMPLIANCE-PROGRAM-002-bar.md", "audit_path": "./compliance-programs/COMPLIANCE-PROGRAM-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   compliance-program-audit
Input:
  artefact_paths:  [./team-a/COMPLIANCE-PROGRAM-001-something.md, ./team-b/COMPLIANCE-PROGRAM-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
