...
description: |
  ...
  baseline. Chains naturally into deploy-checklist-audit.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: i
  cyberos-template: deploy-checklist@1
  cyberos-rubric-target: deploy_checklist_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['deploy-checklist-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        deploy-checklist
    ...
    pin_path:  cyberos/skill/contracts/deploy-checklist/
  - id:        nats-subjects
    version:   v1
...
# deploy-checklist-author — DEPLOY-CHECKLIST generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `deploy-checklist@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`deploy-checklist-audit`](../deploy-checklist-audit/SKILL.md) by default.

`prompt_revision: deploy-checklist_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a DEPLOY-CHECKLIST."
- "Generate v2 of the DEPLOY-CHECKLIST from the updated source."
- "Draft a DEPLOY-CHECKLIST for <project>."

If the user asks to *audit an existing DEPLOY-CHECKLIST*, route to `deploy-checklist-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        deploy-checklist-author
skill_version:                   1.0.0
prompt_revision:                 deploy-checklist_author@1.0.0
template_version:                deploy-checklist@1   (loaded from cyberos/skill/contracts/deploy-checklist/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  DEPLOY-CHECKLIST-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./deploy-checklists/",
  "manifest_path": "./deploy-checklists/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "DEPLOY-CHECKLIST-001", "path": "./deploy-checklists/DEPLOY-CHECKLIST-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "deploy-checklist-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/deploy-checklist/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `deploy-checklist_author.deploy-checklist_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `deploy-checklist-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `deploy-checklist-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   deploy-checklist-author
Input:
