---
# ── Identity ─────────────────────────────────────────────────────────
name: breach-notification-author
description: >-
  Author a data-breach notification under regulatory deadline (incident summary + data classes exposed + affected subjects + regulator notification plan + data-subject notification plan + containment + remediation). CPO-Privacy + CISO + CLO co-authored. GDPR Art. 33 72h deadline. Use when user asks to "draft a breach notification" or "create the breach notification". Do NOT use for "audit existing breach notification" (use breach-notification-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: breach-notification_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['breach-notification-audit']; empty list disables chaining
...
# breach-notification-author — BREACH-NOTIFICATION generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `breach-notification@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`breach-notification-audit`](../breach-notification-audit/SKILL.md) by default.

`prompt_revision: breach-notification_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a BREACH-NOTIFICATION."
- "Generate v2 of the BREACH-NOTIFICATION from the updated source."
- "Draft a BREACH-NOTIFICATION for <project>."

If the user asks to *audit an existing BREACH-NOTIFICATION*, route to `breach-notification-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  BREACH-NOTIFICATION-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./breach-notifications/",
...
```json
{
  ...
  "artefacts_written": [
    {"id": "BREACH-NOTIFICATION-001", "path": "./breach-notifications/BREACH-NOTIFICATION-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "breach-notification-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/breach-notification/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `breach-notification_author.breach-notification_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `breach-notification-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `breach-notification-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   breach-notification-author
Input:
