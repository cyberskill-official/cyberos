...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['kri-dashboard-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        kri-dashboard
    ...
    pin_path:  cyberos/skill/contracts/kri-dashboard/
  - id:        nats-subjects
    version:   v1
...
# kri-dashboard-author — KRI-DASHBOARD generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `kri-dashboard@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`kri-dashboard-audit`](../kri-dashboard-audit/SKILL.md) by default.
...
If the user asks to *audit an existing KRI-DASHBOARD*, route to `kri-dashboard-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        kri-dashboard-author
skill_version:                   1.0.0
prompt_revision:                 kri-dashboard_author@1.0.0
template_version:                kri-dashboard@1   (loaded from cyberos/skill/contracts/kri-dashboard/template.md)
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "kri-dashboard-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/kri-dashboard/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `kri-dashboard-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `kri-dashboard-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   kri-dashboard-author
Input:
