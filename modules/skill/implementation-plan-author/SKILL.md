...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['implementation-plan-audit']; empty list disables chaining
...
# implementation-plan-author — IMPL-PLAN generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `implementation-plan@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`implementation-plan-audit`](../implementation-plan-audit/SKILL.md) by default.

`prompt_revision: impl-plan_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a IMPL-PLAN."
- "Generate v2 of the IMPL-PLAN from the updated source."
- "Draft a IMPL-PLAN for <project>."

If the user asks to *audit an existing IMPL-PLAN*, route to `implementation-plan-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  IMPL-PLAN-{NNN}-{slug}.md
...
```json
{
  ...
  "artefacts_written": [
    {"id": "IMPL-PLAN-001", "path": "./impl-plans/IMPL-PLAN-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "implementation-plan-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/implementation-plan/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `impl-plan_author.impl-plan_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `implementation-plan-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `implementation-plan-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   implementation-plan-author
Input:
