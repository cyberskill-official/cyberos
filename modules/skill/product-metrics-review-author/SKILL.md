...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['product-metrics-review-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        product-metrics-review
    ...
    pin_path:  cyberos/skill/contracts/product-metrics-review/
  - id:        nats-subjects
    version:   v1
...
# product-metrics-review-author — PRODUCT_METRICS_REVIEW generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `product-metrics-review@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`product-metrics-review-audit`](../product-metrics-review-audit/SKILL.md) by default.

`prompt_revision: product-metrics-review_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a PRODUCT_METRICS_REVIEW."
- "Generate v2 of the PRODUCT_METRICS_REVIEW from the updated source."
- "Draft a PRODUCT_METRICS_REVIEW for <project>."

If the user asks to *audit an existing PRODUCT_METRICS_REVIEW*, route to `product-metrics-review-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        product-metrics-review-author
skill_version:                   1.0.0
prompt_revision:                 product-metrics-review_author@1.0.0
template_version:                product-metrics-review@1   (loaded from cyberos/skill/contracts/product-metrics-review/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  PRODUCT_METRICS_REVIEW-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./product-metrics-reviews/",
  "manifest_path": "./product-metrics-reviews/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "PRODUCT_METRICS_REVIEW-001", "path": "./product-metrics-reviews/PRODUCT_METRICS_REVIEW-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "product-metrics-review-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/product-metrics-review/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `product-metrics-review_author.product-metrics-review_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `product-metrics-review-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `product-metrics-review-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   product-metrics-review-author
Input:
