# `product-metrics-review-author` — human summary format
...
```
PRODUCT_METRICS_REVIEW batch <batch_run_id> complete

Wrote N PRODUCT_METRICS_REVIEW(s):
  - PRODUCT_METRICS_REVIEW-001: <slug> — PASS (audit verdict: pass, 0 open issues)
  - PRODUCT_METRICS_REVIEW-002: <slug> — HITL_PAUSE (2 blocking issues — see HITL_BATCH_REQUEST below)
  - PRODUCT_METRICS_REVIEW-003: <slug> — PASS (audit verdict: pass, 1 warning)

Manifest:    <output_dir>/manifest.json
Total time:  <seconds>s
Next:        product-metrics-review-audit will run automatically on PASS items. HITL items wait for your reply.
```
