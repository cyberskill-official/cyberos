---
# ── Identity ─────────────────────────────────────────────────────────
name: clinical-protocol-audit
description: >-
  Audit one or more existing clinical-protocol@1 markdowns against clinical-protocol_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after clinical-protocol-author. Use when user asks to "audit this clinical protocol" or "check the clinical protocol". Do NOT use for "draft a new clinical protocol" (use clinical-protocol-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: clinical-protocol@1
  cyberos-rubric-version: clinical-protocol_rubric@1.0
...
depends_on_contracts:
  - id:        clinical-protocol
    ...
    pin_path:  cyberos/skill/contracts/clinical-protocol/
  - id:        nats-subjects
    version:   v1
...
# clinical-protocol-audit — CLINICAL-PROTOCOL auditor

> Standalone trigger that runs `clinical-protocol_rubric@1.0` against one or
> more existing `clinical-protocol@1` markdowns and writes a sibling
> `clinical-protocol.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`clinical-protocol-author`](../clinical-protocol-author/SKILL.md).

`prompt_revision: clinical-protocol_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `CLINICAL-PROTOCOL`s."
- "Has `CLINICAL-PROTOCOL-007` changed since the last audit?"
- "Tell me which `CLINICAL-PROTOCOL`s would fail acceptance today."

Also invoked automatically by the supervisor when `clinical-protocol-author`'s output envelope sets `next_skill_recommendation: clinical-protocol-audit` (the default chain).
...
skill_version:                   1.0.0
prompt_revision:                 clinical-protocol_audit@1.0.0
template_version:                clinical-protocol@1   (loaded from cyberos/skill/contracts/clinical-protocol/template.md)
audit_rubric_version:            clinical-protocol_rubric@1.0
...
```json
{
  "artefact_paths": ["./clinical-protocols/CLINICAL-PROTOCOL-001-foo.md", "./clinical-protocols/CLINICAL-PROTOCOL-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "clinical-protocol-author",
    "manifest_path": "./clinical-protocols/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./clinical-protocols/CLINICAL-PROTOCOL-001-foo.md", "audit_path": "./clinical-protocols/CLINICAL-PROTOCOL-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./clinical-protocols/CLINICAL-PROTOCOL-002-bar.md", "audit_path": "./clinical-protocols/CLINICAL-PROTOCOL-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   clinical-protocol-audit
Input:
  artefact_paths:  [./team-a/CLINICAL-PROTOCOL-001-something.md, ./team-b/CLINICAL-PROTOCOL-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
