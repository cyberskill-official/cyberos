...
metadata:
  ...
  module: skill
  stage: cross
  cyberos-template: non-disclosure-agreement-triage@1
...
# non-disclosure-agreement-triage-audit — NDA_TRIAGE auditor

> Standalone trigger that runs `nda-triage_rubric@1.0` against one or
> more existing `non-disclosure-agreement-triage@1` markdowns and writes a sibling
...
> Chains naturally after [`non-disclosure-agreement-triage-author`](../non-disclosure-agreement-triage-author/SKILL.md).
...
Also invoked automatically by the supervisor when `non-disclosure-agreement-triage-author`'s output envelope sets `next_skill_recommendation: non-disclosure-agreement-triage-audit` (the default chain).
...
```json
{
  ...
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "non-disclosure-agreement-triage-author",
...
```
Persona: cuo-cpo
Skill:   non-disclosure-agreement-triage-audit
Input:
