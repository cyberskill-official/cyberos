...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['requirements-traceability-matrix-audit']; empty list disables chaining
...
# requirements-traceability-matrix-author — RTM generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `requirements-traceability-matrix@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`requirements-traceability-matrix-audit`](../requirements-traceability-matrix-audit/SKILL.md) by default.

`prompt_revision: rtm_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a RTM."
- "Generate v2 of the RTM from the updated source."
- "Draft a RTM for <project>."

If the user asks to *audit an existing RTM*, route to `requirements-traceability-matrix-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  RTM-{NNN}-{slug}.md
...
```json
{
  ...
  "artefacts_written": [
    {"id": "RTM-001", "path": "./rtms/RTM-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "requirements-traceability-matrix-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/requirements-traceability-matrix/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `rtm_author.rtm_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `requirements-traceability-matrix-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `requirements-traceability-matrix-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   requirements-traceability-matrix-author
Input:
