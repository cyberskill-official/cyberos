# `requirements-traceability-matrix-author` — human summary format
...
```
RTM batch <batch_run_id> complete

Wrote N RTM(s):
  - RTM-001: <slug> — PASS (audit verdict: pass, 0 open issues)
  - RTM-002: <slug> — HITL_PAUSE (2 blocking issues — see HITL_BATCH_REQUEST below)
  - RTM-003: <slug> — PASS (audit verdict: pass, 1 warning)

Manifest:    <output_dir>/manifest.json
Total time:  <seconds>s
Next:        requirements-traceability-matrix-audit will run automatically on PASS items. HITL items wait for your reply.
```
