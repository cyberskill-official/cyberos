...
metadata:
  ...
  cyberos-template: <artifact>@1
  cyberos-rubric-version: <artifact>_rubric@1.0
...
# <artifact>-audit — <ARTIFACT> auditor

> Standalone trigger that runs `<artifact>_rubric@1.0` against one or
> more existing `<artifact>@1` markdowns and writes a sibling
> `<artifact>.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`<artifact>-author`](../<artifact>-author/SKILL.md).

`prompt_revision: <artifact>_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `<ARTIFACT>`s."
- "Has `<ARTIFACT>-007` changed since the last audit?"
- "Tell me which `<ARTIFACT>`s would fail acceptance today."

Also invoked automatically by the supervisor when `<artifact>-author`'s output envelope sets `next_skill_recommendation: <artifact>-audit` (the default chain).
...
```json
{
  "artefact_paths": ["./<artifact>s/<ARTIFACT>-001-foo.md", "./<artifact>s/<ARTIFACT>-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "<artifact>-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./<artifact>s/<ARTIFACT>-001-foo.md", "audit_path": "./<artifact>s/<ARTIFACT>-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./<artifact>s/<ARTIFACT>-002-bar.md", "audit_path": "./<artifact>s/<ARTIFACT>-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
Input:
  artefact_paths:  [./team-a/<ARTIFACT>-001-something.md, ./team-b/<ARTIFACT>-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
