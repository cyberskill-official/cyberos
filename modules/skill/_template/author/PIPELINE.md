# `<artifact>-author` — pipeline

This document describes how `<artifact>-author` chains with upstream and downstream skills.
...
|---|---|---|
| `<artifact>-audit` | Default after every `artefact_written` event | `next_skill_recommendation: <artifact>-audit` in output envelope. |
| `<next-stage>-author` | After `<artifact>-audit` returns PASS | Supervisor reads the audit's output envelope and queues the next stage. |
...
|---|---|---|
| `<artifact>_author.<artefact>_written` | `{artefact_id, artefact_path, artefact_hash, source_hash}` | After every successful W3 WRITE. |
| `<artifact>_author.batch_complete` | `{batch_run_id, artefacts_written, batch_outcome}` | At the end of a WORKER batch. |
| `<artifact>_author.hitl_pause` | `{artefact_id, blocking_issues}` | When the batch halts on HITL. |
| `<artifact>_author.amendment_request` | `{amendment_id, risk_class, change_description}` | When the author proposes a plan amendment. |
...
- `cyberos/skill/contracts/<artifact>/` — the artefact template this skill generates.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/<artifact>-audit/` — the sibling audit skill that validates this skill's output.
