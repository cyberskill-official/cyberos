...
> **Purpose:** assert which user phrasings MUST route to this skill (positive) and which MUST NOT route here (negative). Per TASK-SKILL-112, every production skill (`status: accepted` or higher) carries this file. The CUO supervisor's `classify_act` node is the unit under test.
>
...
- Negative triggers fall into three pools per TASK-SKILL-112 §1 #6: sibling-routing (a), cross-persona (b), no-skill (c — `→ none`).
