# `go-to-market-plan-author` — human summary format
...
```
GTM_PLAN batch <batch_run_id> complete

Wrote N GTM_PLAN(s):
  - GTM_PLAN-001: <slug> — PASS (audit verdict: pass, 0 open issues)
  - GTM_PLAN-002: <slug> — HITL_PAUSE (2 blocking issues — see HITL_BATCH_REQUEST below)
  - GTM_PLAN-003: <slug> — PASS (audit verdict: pass, 1 warning)

Manifest:    <output_dir>/manifest.json
Total time:  <seconds>s
Next:        go-to-market-plan-audit will run automatically on PASS items. HITL items wait for your reply.
```
