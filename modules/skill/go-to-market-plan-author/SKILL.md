...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['go-to-market-plan-audit']; empty list disables chaining
...
> into [`go-to-market-plan-audit`](../go-to-market-plan-audit/SKILL.md) by default.
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "go-to-market-plan-audit"
}
```
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `go-to-market-plan-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `go-to-market-plan-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   go-to-market-plan-author
Input:
