---
# ── Identity ─────────────────────────────────────────────────────────
name: data-strategy-audit
description: >-
  Audit one or more existing data-strategy@1 markdowns against data-strategy_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after data-strategy-author. Use when user asks to "audit this data strategy" or "check the data strategy". Do NOT use for "draft a new data strategy" (use data-strategy-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: data-strategy@1
  cyberos-rubric-version: data-strategy_rubric@1.0
...
# data-strategy-audit — DATA-STRATEGY auditor

> Standalone trigger that runs `data-strategy_rubric@1.0` against one or
> more existing `data-strategy@1` markdowns and writes a sibling
> `data-strategy.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`data-strategy-author`](../data-strategy-author/SKILL.md).

`prompt_revision: data-strategy_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `DATA-STRATEGY`s."
- "Has `DATA-STRATEGY-007` changed since the last audit?"
- "Tell me which `DATA-STRATEGY`s would fail acceptance today."

Also invoked automatically by the supervisor when `data-strategy-author`'s output envelope sets `next_skill_recommendation: data-strategy-audit` (the default chain).
...
audit_rubric_version:            data-strategy_rubric@1.0
...
```json
{
  "artefact_paths": ["./data-strategys/DATA-STRATEGY-001-foo.md", "./data-strategys/DATA-STRATEGY-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "data-strategy-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./data-strategys/DATA-STRATEGY-001-foo.md", "audit_path": "./data-strategys/DATA-STRATEGY-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./data-strategys/DATA-STRATEGY-002-bar.md", "audit_path": "./data-strategys/DATA-STRATEGY-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   data-strategy-audit
Input:
  artefact_paths:  [./team-a/DATA-STRATEGY-001-something.md, ./team-b/DATA-STRATEGY-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
