...
metadata:
  ...
  module: skill
  stage: cross
  cyberos-template: delivery-review@1
...
# delivery-review-audit — DELIVERY_REVIEW auditor

> Standalone trigger that runs `delivery-review_rubric@1.0` against one or
> more existing `delivery-review@1` markdowns and writes a sibling
> `delivery-review.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`delivery-review-author`](../delivery-review-author/SKILL.md).

`prompt_revision: delivery-review_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `DELIVERY_REVIEW`s."
- "Has `DELIVERY_REVIEW-007` changed since the last audit?"
- "Tell me which `DELIVERY_REVIEW`s would fail acceptance today."

Also invoked automatically by the supervisor when `delivery-review-author`'s output envelope sets `next_skill_recommendation: delivery-review-audit` (the default chain).
...
audit_rubric_version:            delivery-review_rubric@1.0
...
```json
{
  "artefact_paths": ["./delivery-reviews/DELIVERY_REVIEW-001-foo.md", "./delivery-reviews/DELIVERY_REVIEW-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "delivery-review-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./delivery-reviews/DELIVERY_REVIEW-001-foo.md", "audit_path": "./delivery-reviews/DELIVERY_REVIEW-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./delivery-reviews/DELIVERY_REVIEW-002-bar.md", "audit_path": "./delivery-reviews/DELIVERY_REVIEW-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   delivery-review-audit
Input:
  artefact_paths:  [./team-a/DELIVERY_REVIEW-001-something.md, ./team-b/DELIVERY_REVIEW-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
