...
description: |
  Author a feasibility memo + project plan + RAID log + communication
  plan (project-plan@1) from an approved SRS plus constraints (budget,
  timeline, regulatory). Covers Software Development Process.md §2(c)
  Feasibility study and project planning. Outputs technical
  feasibility, schedule (PMBOK 8th ed. performance domains; PRINCE2 7th
  ed. business case theme), RAID log, communication plan, kickoff
  agenda. Chains naturally into project-plan-audit and stage-gate-author.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: c
  cyberos-template: project-plan@1
  cyberos-rubric-target: project_plan_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['project-plan-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        project-plan
    ...
    pin_path:  cyberos/skill/contracts/project-plan/
  - id:        nats-subjects
    version:   v1
...
# project-plan-author — PROJECT-PLAN generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `project-plan@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`project-plan-audit`](../project-plan-audit/SKILL.md) by default.

`prompt_revision: project-plan_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a PROJECT-PLAN."
- "Generate v2 of the PROJECT-PLAN from the updated source."
- "Draft a PROJECT-PLAN for <project>."

If the user asks to *audit an existing PROJECT-PLAN*, route to `project-plan-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 project-plan_author@1.0.0
template_version:                project-plan@1   (loaded from cyberos/skill/contracts/project-plan/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  PROJECT-PLAN-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./project-plans/",
  "manifest_path": "./project-plans/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "PROJECT-PLAN-001", "path": "./project-plans/PROJECT-PLAN-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "project-plan-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/project-plan/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `project-plan_author.project-plan_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `project-plan-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `project-plan-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   project-plan-author
Input:
