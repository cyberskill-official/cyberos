---
# ── Identity ─────────────────────────────────────────────────────────
name: knowledge-asset-audit
description: >-
  Audit one or more existing knowledge-asset@1 markdowns against knowledge-asset_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after knowledge-asset-author. Use when user asks to "audit this knowledge asset" or "check the knowledge asset". Do NOT use for "draft a new knowledge asset" (use knowledge-asset-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: knowledge-asset@1
  cyberos-rubric-version: knowledge-asset_rubric@1.0
...
# knowledge-asset-audit — KNOWLEDGE-ASSET auditor

> Standalone trigger that runs `knowledge-asset_rubric@1.0` against one or
> more existing `knowledge-asset@1` markdowns and writes a sibling
> `knowledge-asset.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`knowledge-asset-author`](../knowledge-asset-author/SKILL.md).

`prompt_revision: knowledge-asset_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `KNOWLEDGE-ASSET`s."
- "Has `KNOWLEDGE-ASSET-007` changed since the last audit?"
- "Tell me which `KNOWLEDGE-ASSET`s would fail acceptance today."

Also invoked automatically by the supervisor when `knowledge-asset-author`'s output envelope sets `next_skill_recommendation: knowledge-asset-audit` (the default chain).
...
```json
{
  "artefact_paths": ["./knowledge-assets/KNOWLEDGE-ASSET-001-foo.md", "./knowledge-assets/KNOWLEDGE-ASSET-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "knowledge-asset-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./knowledge-assets/KNOWLEDGE-ASSET-001-foo.md", "audit_path": "./knowledge-assets/KNOWLEDGE-ASSET-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./knowledge-assets/KNOWLEDGE-ASSET-002-bar.md", "audit_path": "./knowledge-assets/KNOWLEDGE-ASSET-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   knowledge-asset-audit
Input:
  artefact_paths:  [./team-a/KNOWLEDGE-ASSET-001-something.md, ./team-b/KNOWLEDGE-ASSET-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
