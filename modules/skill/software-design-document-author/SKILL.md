...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['software-design-document-audit']; empty list disables chaining
...
# software-design-document-author — SDD generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `software-design-document@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`software-design-document-audit`](../software-design-document-audit/SKILL.md) by default.

`prompt_revision: sdd_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a SDD."
- "Generate v2 of the SDD from the updated source."
- "Draft a SDD for <project>."

If the user asks to *audit an existing SDD*, route to `software-design-document-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  SDD-{NNN}-{slug}.md
...
```json
{
  ...
  "artefacts_written": [
    {"id": "SDD-001", "path": "./sdds/SDD-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "software-design-document-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/software-design-document/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `sdd_author.sdd_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `software-design-document-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `software-design-document-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   software-design-document-author
Input:
