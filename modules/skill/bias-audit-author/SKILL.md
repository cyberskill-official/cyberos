---
# ── Identity ─────────────────────────────────────────────────────────
name: bias-audit-author
description: >-
  Author a quarterly bias audit for each AI feature in production (feature inventory + fairness metrics per feature + pass/fail per metric + remediation plan + next-audit date). Chief Ethics Officer + CAIO co-authored. Use when user asks to "draft a bias audit" or "create the bias audit". Do NOT use for "audit existing bias audit" (use bias-audit-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: bias-audit_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['bias-audit-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        bias-audit
    ...
    pin_path:  cyberos/skill/contracts/bias-audit/
  - id:        nats-subjects
    version:   v1
...
# bias-audit-author — BIAS-AUDIT generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `bias-audit@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`bias-audit-audit`](../bias-audit-audit/SKILL.md) by default.

`prompt_revision: bias-audit_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a BIAS-AUDIT."
- "Generate v2 of the BIAS-AUDIT from the updated source."
- "Draft a BIAS-AUDIT for <project>."

If the user asks to *audit an existing BIAS-AUDIT*, route to `bias-audit-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 bias-audit_author@1.0.0
template_version:                bias-audit@1   (loaded from cyberos/skill/contracts/bias-audit/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  BIAS-AUDIT-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./bias-audits/",
  "manifest_path": "./bias-audits/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "BIAS-AUDIT-001", "path": "./bias-audits/BIAS-AUDIT-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "bias-audit-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/bias-audit/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `bias-audit_author.bias-audit_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
- If chained to `bias-audit-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `bias-audit-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   bias-audit-author
Input:
