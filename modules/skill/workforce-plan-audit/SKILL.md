---
# ── Identity ─────────────────────────────────────────────────────────
name: workforce-plan-audit
description: >-
  Audit one or more existing workforce-plan@1 markdowns against workforce-plan_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after workforce-plan-author. Use when user asks to "audit this workforce plan" or "check the workforce plan". Do NOT use for "draft a new workforce plan" (use workforce-plan-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: workforce-plan@1
  cyberos-rubric-version: workforce-plan_rubric@1.0
...
depends_on_contracts:
  - id:        workforce-plan
    ...
    pin_path:  cyberos/skill/contracts/workforce-plan/
  - id:        nats-subjects
    version:   v1
...
# workforce-plan-audit — WORKFORCE-PLAN auditor

> Standalone trigger that runs `workforce-plan_rubric@1.0` against one or
> more existing `workforce-plan@1` markdowns and writes a sibling
> `workforce-plan.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`workforce-plan-author`](../workforce-plan-author/SKILL.md).

`prompt_revision: workforce-plan_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `WORKFORCE-PLAN`s."
- "Has `WORKFORCE-PLAN-007` changed since the last audit?"
- "Tell me which `WORKFORCE-PLAN`s would fail acceptance today."

Also invoked automatically by the supervisor when `workforce-plan-author`'s output envelope sets `next_skill_recommendation: workforce-plan-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        workforce-plan-audit
skill_version:                   1.0.0
prompt_revision:                 workforce-plan_audit@1.0.0
template_version:                workforce-plan@1   (loaded from cyberos/skill/contracts/workforce-plan/template.md)
audit_rubric_version:            workforce-plan_rubric@1.0
...
```json
{
  "artefact_paths": ["./workforce-plans/WORKFORCE-PLAN-001-foo.md", "./workforce-plans/WORKFORCE-PLAN-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "workforce-plan-author",
    "manifest_path": "./workforce-plans/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./workforce-plans/WORKFORCE-PLAN-001-foo.md", "audit_path": "./workforce-plans/WORKFORCE-PLAN-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./workforce-plans/WORKFORCE-PLAN-002-bar.md", "audit_path": "./workforce-plans/WORKFORCE-PLAN-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   workforce-plan-audit
Input:
  artefact_paths:  [./team-a/WORKFORCE-PLAN-001-something.md, ./team-b/WORKFORCE-PLAN-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
