# `workforce_plan_rubric@1.0` — machine-checkable Workforce Plan rubric
...
| `FM-004` | `template` equals `workforce-plan@1` | error | true |
...
Per the `contracts/workforce-plan/template.md` H2 list. Each H2 in the template MUST be present + non-empty in the authored artefact. See template for the canonical headings.

| rule_id | Check | Severity |
| ------- | ----- | -------- |
| `SEC-001` | Every H2 declared in `contracts/workforce-plan/template.md` is present in the authored artefact | error |
...
- `cyberos/skill/contracts/workforce-plan/CONTRACT.md` + `template.md`
