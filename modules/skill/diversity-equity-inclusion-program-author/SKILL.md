---
# ── Identity ─────────────────────────────────────────────────────────
name: diversity-equity-inclusion-program-author
description: >-
  Author the annual DEI / Inclusion & Belonging program (current representation + promotion parity + inclusion survey + ERG support + inclusive hiring + program goals + budget). CDO-Diversity or CHRO authored. Use when user asks to "draft a diversity equity inclusion program" or "create the diversity equity inclusion program". Do NOT use for "audit existing diversity equity inclusion program" (use diversity-equity-inclusion-program-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: dei-program_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['diversity-equity-inclusion-program-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        dei-program
    ...
    pin_path:  cyberos/skill/contracts/diversity-equity-inclusion-program/
  - id:        nats-subjects
    version:   v1
...
# diversity-equity-inclusion-program-author — DEI-PROGRAM generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `diversity-equity-inclusion-program@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`diversity-equity-inclusion-program-audit`](../diversity-equity-inclusion-program-audit/SKILL.md) by default.

`prompt_revision: dei-program_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a DEI-PROGRAM."
- "Generate v2 of the DEI-PROGRAM from the updated source."
- "Draft a DEI-PROGRAM for <project>."

If the user asks to *audit an existing DEI-PROGRAM*, route to `diversity-equity-inclusion-program-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        diversity-equity-inclusion-program-author
skill_version:                   1.0.0
prompt_revision:                 dei-program_author@1.0.0
template_version:                diversity-equity-inclusion-program@1   (loaded from cyberos/skill/contracts/diversity-equity-inclusion-program/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  DEI-PROGRAM-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./dei-programs/",
  "manifest_path": "./dei-programs/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "DEI-PROGRAM-001", "path": "./dei-programs/DEI-PROGRAM-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "diversity-equity-inclusion-program-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/diversity-equity-inclusion-program/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `dei-program_author.dei-program_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `diversity-equity-inclusion-program-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `diversity-equity-inclusion-program-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   diversity-equity-inclusion-program-author
Input:
