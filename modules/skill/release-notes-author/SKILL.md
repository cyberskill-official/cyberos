...
description: |
  Author customer-facing release notes (release-notes@1) for a release
  candidate, drawn from CHANGELOG entries + merged PRs since the last
  release tag. Splits by Keep-a-Changelog categories (Added, Changed,
  Deprecated, Removed, Fixed, Security) and surfaces breaking changes,
  security advisories, and upgrade guides up-front. Covers Software
  Development Process.md §2(i) release. Chains naturally into
  release-notes-audit.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: i
  cyberos-template: release-notes@1
  cyberos-rubric-target: release_notes_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['release-notes-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        release-notes
    ...
    pin_path:  cyberos/skill/contracts/release-notes/
  - id:        nats-subjects
    version:   v1
...
# release-notes-author — RELEASE-NOTES generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `release-notes@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`release-notes-audit`](../release-notes-audit/SKILL.md) by default.

`prompt_revision: release-notes_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a RELEASE-NOTES."
- "Generate v2 of the RELEASE-NOTES from the updated source."
- "Draft a RELEASE-NOTES for <project>."

If the user asks to *audit an existing RELEASE-NOTES*, route to `release-notes-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 release-notes_author@1.0.0
template_version:                release-notes@1   (loaded from cyberos/skill/contracts/release-notes/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  RELEASE-NOTES-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./release-notess/",
  "manifest_path": "./release-notess/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "RELEASE-NOTES-001", "path": "./release-notess/RELEASE-NOTES-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "release-notes-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/release-notes/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `release-notes_author.release-notes_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `release-notes-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `release-notes-audit` (and other downstream consumers).
