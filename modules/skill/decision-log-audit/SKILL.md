...
description: >-
  Audit one or more existing decision-log@1 markdowns against decision-log_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after decision-log-author. Use when user asks to "audit this decision log" or "check the decision log". Do NOT use for "draft a new decision log" (use decision-log-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: decision-log@1
  cyberos-rubric-version: decision-log_rubric@1.0
...
# decision-log-audit — DECISION-LOG auditor

> Standalone trigger that runs `decision-log_rubric@1.0` against one or
> more existing `decision-log@1` markdowns and writes a sibling
> `decision-log.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`decision-log-author`](../decision-log-author/SKILL.md).

`prompt_revision: decision-log_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `DECISION-LOG`s."
- "Has `DECISION-LOG-007` changed since the last audit?"
- "Tell me which `DECISION-LOG`s would fail acceptance today."

Also invoked automatically by the supervisor when `decision-log-author`'s output envelope sets `next_skill_recommendation: decision-log-audit` (the default chain).
...
audit_rubric_version:            decision-log_rubric@1.0
...
```json
{
  "artefact_paths": ["./decision-logs/DECISION-LOG-001-foo.md", "./decision-logs/DECISION-LOG-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "decision-log-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./decision-logs/DECISION-LOG-001-foo.md", "audit_path": "./decision-logs/DECISION-LOG-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./decision-logs/DECISION-LOG-002-bar.md", "audit_path": "./decision-logs/DECISION-LOG-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
Input:
  artefact_paths:  [./team-a/DECISION-LOG-001-something.md, ./team-b/DECISION-LOG-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
