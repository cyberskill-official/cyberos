# `decision_log_rubric@1.0` — machine-checkable Decision Log rubric
...
| `FM-004` | `template` equals `decision-log@1` | error | true |
...
Per the `contracts/decision-log/template.md` H2 list. Each H2 in the template MUST be present + non-empty in the authored artefact. See template for the canonical headings.

| rule_id | Check | Severity |
| ------- | ----- | -------- |
| `SEC-001` | Every H2 declared in `contracts/decision-log/template.md` is present in the authored artefact | error |
...
- `cyberos/skill/contracts/decision-log/CONTRACT.md` + `template.md`
