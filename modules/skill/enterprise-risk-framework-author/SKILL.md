---
# ── Identity ─────────────────────────────────────────────────────────
name: enterprise-risk-framework-author
description: >-
  Author an annual enterprise-risk framework (risk-appetite vs risk-tolerance + KRI taxonomy + scenario library + stress-test catalog + governance cadence). COSO ERM + ISO 31000 aligned. CRO-Risk authored. Use when user asks to "draft a enterprise risk framework" or "create the enterprise risk framework". Do NOT use for "audit existing enterprise risk framework" (use enterprise-risk-framework-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: enterprise-risk-framework_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['enterprise-risk-framework-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        enterprise-risk-framework
    version:   v1
    purpose:   generation_skeleton
    pin_path:  cyberos/skill/contracts/enterprise-risk-framework/
  - id:        nats-subjects
    version:   v1
...
# enterprise-risk-framework-author — ENTERPRISE-RISK-FRAMEWORK generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `enterprise-risk-framework@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`enterprise-risk-framework-audit`](../enterprise-risk-framework-audit/SKILL.md) by default.

`prompt_revision: enterprise-risk-framework_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a ENTERPRISE-RISK-FRAMEWORK."
- "Generate v2 of the ENTERPRISE-RISK-FRAMEWORK from the updated source."
- "Draft a ENTERPRISE-RISK-FRAMEWORK for <project>."

If the user asks to *audit an existing ENTERPRISE-RISK-FRAMEWORK*, route to `enterprise-risk-framework-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 enterprise-risk-framework_author@1.0.0
template_version:                enterprise-risk-framework@1   (loaded from cyberos/skill/contracts/enterprise-risk-framework/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  ENTERPRISE-RISK-FRAMEWORK-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./enterprise-risk-frameworks/",
  "manifest_path": "./enterprise-risk-frameworks/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "ENTERPRISE-RISK-FRAMEWORK-001", "path": "./enterprise-risk-frameworks/ENTERPRISE-RISK-FRAMEWORK-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "enterprise-risk-framework-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/enterprise-risk-framework/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `enterprise-risk-framework_author.enterprise-risk-framework_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `enterprise-risk-framework-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `enterprise-risk-framework-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   enterprise-risk-framework-author
Input:
