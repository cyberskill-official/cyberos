# `trust_portal_update_rubric@1.0` — machine-checkable Trust Portal Update rubric
...
| `FM-004` | `template` equals `trust-portal-update@1` | error | true |
...
Per the `contracts/trust-portal-update/template.md` H2 list — every H2 in template MUST be present + non-empty in the authored artefact.

| rule_id | Check | Severity |
| ------- | ----- | -------- |
| `SEC-001` | Every H2 declared in `contracts/trust-portal-update/template.md` is present in the authored artefact | error |
...
- `cyberos/skill/contracts/trust-portal-update/CONTRACT.md` + `template.md`
