---
# ── Identity ─────────────────────────────────────────────────────────
name: trust-portal-update-audit
description: >-
  Audit one or more existing trust-portal-update@1 markdowns against trust-portal-update_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after trust-portal-update-author. Use when user asks to "audit this trust portal update" or "check the trust portal update". Do NOT use for "draft a new trust portal update" (use trust-portal-update-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: trust-portal-update@1
  cyberos-rubric-version: trust-portal-update_rubric@1.0
...
# trust-portal-update-audit — TRUST-PORTAL-UPDATE auditor

> Standalone trigger that runs `trust-portal-update_rubric@1.0` against one or
> more existing `trust-portal-update@1` markdowns and writes a sibling
> `trust-portal-update.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`trust-portal-update-author`](../trust-portal-update-author/SKILL.md).

`prompt_revision: trust-portal-update_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `TRUST-PORTAL-UPDATE`s."
- "Has `TRUST-PORTAL-UPDATE-007` changed since the last audit?"
- "Tell me which `TRUST-PORTAL-UPDATE`s would fail acceptance today."

Also invoked automatically by the supervisor when `trust-portal-update-author`'s output envelope sets `next_skill_recommendation: trust-portal-update-audit` (the default chain).
...
audit_rubric_version:            trust-portal-update_rubric@1.0
...
```json
{
  "artefact_paths": ["./trust-portal-updates/TRUST-PORTAL-UPDATE-001-foo.md", "./trust-portal-updates/TRUST-PORTAL-UPDATE-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "trust-portal-update-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./trust-portal-updates/TRUST-PORTAL-UPDATE-001-foo.md", "audit_path": "./trust-portal-updates/TRUST-PORTAL-UPDATE-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./trust-portal-updates/TRUST-PORTAL-UPDATE-002-bar.md", "audit_path": "./trust-portal-updates/TRUST-PORTAL-UPDATE-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   trust-portal-update-audit
Input:
  artefact_paths:  [./team-a/TRUST-PORTAL-UPDATE-001-something.md, ./team-b/TRUST-PORTAL-UPDATE-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
