...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['dsr-runbook-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        dsr-runbook
    ...
    pin_path:  cyberos/skill/contracts/dsr-runbook/
  - id:        nats-subjects
    version:   v1
...
# dsr-runbook-author — DSR-RUNBOOK generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `dsr-runbook@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`dsr-runbook-audit`](../dsr-runbook-audit/SKILL.md) by default.
...
If the user asks to *audit an existing DSR-RUNBOOK*, route to `dsr-runbook-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        dsr-runbook-author
skill_version:                   1.0.0
prompt_revision:                 dsr-runbook_author@1.0.0
template_version:                dsr-runbook@1   (loaded from cyberos/skill/contracts/dsr-runbook/template.md)
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "dsr-runbook-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/dsr-runbook/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `dsr-runbook-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `dsr-runbook-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   dsr-runbook-author
Input:
