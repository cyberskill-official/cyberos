---
# ── Identity ─────────────────────────────────────────────────────────
name: security-strategy-audit
description: >-
  Audits a security-strategy@1 artefact against security_strategy_rubric@1.0. Validates FM + SEC, threat-landscape sourcing (Verizon DBIR / industry intel), NIST CSF 2.0 posture-gap mapping, initiative-prioritization rationale, budget-envelope feasibility. Per NIST CSF + ISO/IEC 27001 conventions. Audit one or more existing security-strategy@1 markdowns against security-strategy_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after security-strategy-author. Use when user asks to "audit this security strategy" or "check the security strategy". Do NOT use for "draft a new security strategy" (use security-strategy-author instead). Audit one or more existing security-strategy@1 markdowns against security-strategy_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigg...
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: security-strategy@1
  cyberos-rubric-version: security-strategy_rubric@1.0
...
depends_on_contracts:
  - id:        security-strategy
    ...
    pin_path:  cyberos/skill/contracts/security-strategy/
  - id:        nats-subjects
    version:   v1
...
# security-strategy-audit — SECURITY_STRATEGY auditor

> Standalone trigger that runs `security-strategy_rubric@1.0` against one or
> more existing `security-strategy@1` markdowns and writes a sibling
> `security-strategy.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`security-strategy-author`](../security-strategy-author/SKILL.md).

`prompt_revision: security-strategy_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `SECURITY_STRATEGY`s."
- "Has `SECURITY_STRATEGY-007` changed since the last audit?"
- "Tell me which `SECURITY_STRATEGY`s would fail acceptance today."

Also invoked automatically by the supervisor when `security-strategy-author`'s output envelope sets `next_skill_recommendation: security-strategy-audit` (the default chain).
...
skill_version:                   1.0.0
prompt_revision:                 security-strategy_audit@1.0.0
template_version:                security-strategy@1   (loaded from cyberos/skill/contracts/security-strategy/template.md)
audit_rubric_version:            security-strategy_rubric@1.0
...
```json
{
  "artefact_paths": ["./security-strategys/SECURITY_STRATEGY-001-foo.md", "./security-strategys/SECURITY_STRATEGY-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "security-strategy-author",
    "manifest_path": "./security-strategys/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./security-strategys/SECURITY_STRATEGY-001-foo.md", "audit_path": "./security-strategys/SECURITY_STRATEGY-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./security-strategys/SECURITY_STRATEGY-002-bar.md", "audit_path": "./security-strategys/SECURITY_STRATEGY-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   security-strategy-audit
Input:
  artefact_paths:  [./team-a/SECURITY_STRATEGY-001-something.md, ./team-b/SECURITY_STRATEGY-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
