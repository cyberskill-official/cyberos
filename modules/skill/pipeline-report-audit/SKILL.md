---
# ── Identity ─────────────────────────────────────────────────────────
name: pipeline-report-audit
description: >-
  Audit one or more existing pipeline-report@1 markdowns against pipeline-report_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after pipeline-report-author. Use when user asks to "audit this pipeline report" or "check the pipeline report". Do NOT use for "draft a new pipeline report" (use pipeline-report-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: pipeline-report@1
  cyberos-rubric-version: pipeline-report_rubric@1.0
...
depends_on_contracts:
  - id:        pipeline-report
    ...
    pin_path:  cyberos/skill/contracts/pipeline-report/
  - id:        nats-subjects
    version:   v1
...
# pipeline-report-audit — PIPELINE-REPORT auditor

> Standalone trigger that runs `pipeline-report_rubric@1.0` against one or
> more existing `pipeline-report@1` markdowns and writes a sibling
> `pipeline-report.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`pipeline-report-author`](../pipeline-report-author/SKILL.md).

`prompt_revision: pipeline-report_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `PIPELINE-REPORT`s."
- "Has `PIPELINE-REPORT-007` changed since the last audit?"
- "Tell me which `PIPELINE-REPORT`s would fail acceptance today."

Also invoked automatically by the supervisor when `pipeline-report-author`'s output envelope sets `next_skill_recommendation: pipeline-report-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        pipeline-report-audit
skill_version:                   1.0.0
prompt_revision:                 pipeline-report_audit@1.0.0
template_version:                pipeline-report@1   (loaded from cyberos/skill/contracts/pipeline-report/template.md)
audit_rubric_version:            pipeline-report_rubric@1.0
...
```json
{
  "artefact_paths": ["./pipeline-reports/PIPELINE-REPORT-001-foo.md", "./pipeline-reports/PIPELINE-REPORT-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "pipeline-report-author",
    "manifest_path": "./pipeline-reports/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./pipeline-reports/PIPELINE-REPORT-001-foo.md", "audit_path": "./pipeline-reports/PIPELINE-REPORT-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./pipeline-reports/PIPELINE-REPORT-002-bar.md", "audit_path": "./pipeline-reports/PIPELINE-REPORT-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   pipeline-report-audit
Input:
  artefact_paths:  [./team-a/PIPELINE-REPORT-001-something.md, ./team-b/PIPELINE-REPORT-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
