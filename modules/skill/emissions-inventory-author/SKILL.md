---
# ── Identity ─────────────────────────────────────────────────────────
name: emissions-inventory-author
description: >-
  Author the quarterly Scope 1/2/3 emissions inventory (per-scope emissions + calculation methodology + data sources + verification status + year-over-year delta). GHG Protocol Corporate Standard. CSO-Sustainability authored. Use when user asks to "draft a emissions inventory" or "create the emissions inventory". Do NOT use for "audit existing emissions inventory" (use emissions-inventory-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: emissions-inventory_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['emissions-inventory-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        emissions-inventory
    version:   v1
    purpose:   generation_skeleton
    pin_path:  cyberos/skill/contracts/emissions-inventory/
  - id:        nats-subjects
    version:   v1
...
# emissions-inventory-author — EMISSIONS-INVENTORY generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `emissions-inventory@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`emissions-inventory-audit`](../emissions-inventory-audit/SKILL.md) by default.

`prompt_revision: emissions-inventory_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a EMISSIONS-INVENTORY."
- "Generate v2 of the EMISSIONS-INVENTORY from the updated source."
- "Draft a EMISSIONS-INVENTORY for <project>."

If the user asks to *audit an existing EMISSIONS-INVENTORY*, route to `emissions-inventory-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 emissions-inventory_author@1.0.0
template_version:                emissions-inventory@1   (loaded from cyberos/skill/contracts/emissions-inventory/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  EMISSIONS-INVENTORY-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./emissions-inventorys/",
  "manifest_path": "./emissions-inventorys/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "EMISSIONS-INVENTORY-001", "path": "./emissions-inventorys/EMISSIONS-INVENTORY-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "emissions-inventory-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/emissions-inventory/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `emissions-inventory_author.emissions-inventory_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `emissions-inventory-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `emissions-inventory-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   emissions-inventory-author
Input:
