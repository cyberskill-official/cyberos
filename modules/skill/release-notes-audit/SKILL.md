...
description: >-
  Audit one or more existing release-notes@1 markdowns against release-notes_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after release-notes-author. Use when user asks to "audit this release notes" or "check the release notes". Do NOT use for "draft a new release notes" (use release-notes-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-version: release_notes_rubric@1.0
...
# release-notes-audit — RELEASE-NOTES auditor

> Standalone trigger that runs `release-notes_rubric@1.0` against one or
> more existing `release-notes@1` markdowns and writes a sibling
> `release-notes.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`release-notes-author`](../release-notes-author/SKILL.md).

`prompt_revision: release-notes_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `RELEASE-NOTES`s."
- "Has `RELEASE-NOTES-007` changed since the last audit?"
- "Tell me which `RELEASE-NOTES`s would fail acceptance today."

Also invoked automatically by the supervisor when `release-notes-author`'s output envelope sets `next_skill_recommendation: release-notes-audit` (the default chain).
...
audit_rubric_version:            release-notes_rubric@1.0
...
```json
{
  "artefact_paths": ["./release-notess/RELEASE-NOTES-001-foo.md", "./release-notess/RELEASE-NOTES-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "release-notes-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./release-notess/RELEASE-NOTES-001-foo.md", "audit_path": "./release-notess/RELEASE-NOTES-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./release-notess/RELEASE-NOTES-002-bar.md", "audit_path": "./release-notess/RELEASE-NOTES-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   release-notes-audit
Input:
  artefact_paths:  [./team-a/RELEASE-NOTES-001-something.md, ./team-b/RELEASE-NOTES-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
