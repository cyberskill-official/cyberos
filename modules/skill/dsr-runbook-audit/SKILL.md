---
# ── Identity ─────────────────────────────────────────────────────────
name: dsr-runbook-audit
description: |
  Audit one or more existing dsr-runbook@1 markdowns against
...
metadata:
  ...
  cyberos-template: dsr-runbook@1
...
# dsr-runbook-audit — DSR-RUNBOOK auditor

> Standalone trigger that runs `dsr-runbook_rubric@1.0` against one or
> more existing `dsr-runbook@1` markdowns and writes a sibling
...
> Chains naturally after [`dsr-runbook-author`](../dsr-runbook-author/SKILL.md).
...
Also invoked automatically by the supervisor when `dsr-runbook-author`'s output envelope sets `next_skill_recommendation: dsr-runbook-audit` (the default chain).
