...
| `FM-004` | `template` equals `dsr-runbook@1` | error | true |
...
Per the `contracts/dsr-runbook/template.md` H2 list. Each H2 in the template MUST be present + non-empty in the authored artefact. See template for the canonical headings.

| rule_id | Check | Severity |
| ------- | ----- | -------- |
| `SEC-001` | Every H2 declared in `contracts/dsr-runbook/template.md` is present in the authored artefact | error |
...
- `cyberos/skill/contracts/dsr-runbook/CONTRACT.md` + `template.md`
