---
# ── Identity ─────────────────────────────────────────────────────────
name: hire-decision-author
description: >-
  Author a C-suite hire decision record for board reporting. Renders role spec + candidate slate + interview-loop verdicts + offer terms + risks + decision rationale. CEO + CHRO co-authored. Use when user asks to "draft a hire decision" or "create the hire decision". Do NOT use for "audit existing hire decision" (use hire-decision-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: hire-decision_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['hire-decision-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        hire-decision
    ...
    pin_path:  cyberos/skill/contracts/hire-decision/
  - id:        nats-subjects
    version:   v1
...
# hire-decision-author — HIRE-DECISION generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `hire-decision@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`hire-decision-audit`](../hire-decision-audit/SKILL.md) by default.

`prompt_revision: hire-decision_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a HIRE-DECISION."
- "Generate v2 of the HIRE-DECISION from the updated source."
- "Draft a HIRE-DECISION for <project>."

If the user asks to *audit an existing HIRE-DECISION*, route to `hire-decision-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        hire-decision-author
skill_version:                   1.0.0
prompt_revision:                 hire-decision_author@1.0.0
template_version:                hire-decision@1   (loaded from cyberos/skill/contracts/hire-decision/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  HIRE-DECISION-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./hire-decisions/",
  "manifest_path": "./hire-decisions/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "HIRE-DECISION-001", "path": "./hire-decisions/HIRE-DECISION-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "hire-decision-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/hire-decision/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `hire-decision_author.hire-decision_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `hire-decision-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `hire-decision-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   hire-decision-author
Input:
