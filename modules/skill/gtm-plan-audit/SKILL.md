...
# ── Identity ─────────────────────────────────────────────────────────
name: gtm-plan-audit
description: Audits a gtm-plan@1 artefact against gtm_plan_rubric@1.0. Validates FM + SEC structure, ICP-segmentation rigor, sales-motion-vs-quota-model coherence, pricing-posture-vs-segment alignment. Per Winning by Design + MEDDIC + Predictable Revenue conventions.
  Audit one or more existing gtm-plan@1 markdowns against
...
depends_on_contracts:
  - id:        gtm-plan
    ...
    pin_path:  cyberos/skill/contracts/gtm-plan/
  - id:        nats-subjects
    version:   v1
...
# gtm-plan-audit — GTM_PLAN auditor

> Standalone trigger that runs `gtm-plan_rubric@1.0` against one or
> more existing `gtm-plan@1` markdowns and writes a sibling
> `gtm-plan.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`gtm-plan-author`](../gtm-plan-author/SKILL.md).

`prompt_revision: gtm-plan_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `GTM_PLAN`s."
- "Has `GTM_PLAN-007` changed since the last audit?"
- "Tell me which `GTM_PLAN`s would fail acceptance today."

Also invoked automatically by the supervisor when `gtm-plan-author`'s output envelope sets `next_skill_recommendation: gtm-plan-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        gtm-plan-audit
skill_version:                   1.0.0
prompt_revision:                 gtm-plan_audit@1.0.0
template_version:                gtm-plan@1   (loaded from cyberos/skill/contracts/gtm-plan/template.md)
audit_rubric_version:            gtm-plan_rubric@1.0
...
```json
{
  "artefact_paths": ["./gtm-plans/GTM_PLAN-001-foo.md", "./gtm-plans/GTM_PLAN-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "gtm-plan-author",
    "manifest_path": "./gtm-plans/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./gtm-plans/GTM_PLAN-001-foo.md", "audit_path": "./gtm-plans/GTM_PLAN-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./gtm-plans/GTM_PLAN-002-bar.md", "audit_path": "./gtm-plans/GTM_PLAN-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   gtm-plan-audit
Input:
  artefact_paths:  [./team-a/GTM_PLAN-001-something.md, ./team-b/GTM_PLAN-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
