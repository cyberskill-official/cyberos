---
# ── Identity ─────────────────────────────────────────────────────────
name: transformation-roadmap-author
description: >-
  Author a multi-year transformation roadmap (current state + future state + wave plan + change-mgmt approach + governance + quick wins + metrics dashboard). McKinsey 7S + Kotter 8-step aligned. Chief Transformation Officer or Chief Digital Officer authored. Use when user asks to "draft a transformation roadmap" or "create the transformation roadmap". Do NOT use for "audit existing transformation roadmap" (use transformation-roadmap-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: transformation-roadmap_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['transformation-roadmap-audit']; empty list disables chaining
...
# transformation-roadmap-author — TRANSFORMATION-ROADMAP generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `transformation-roadmap@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`transformation-roadmap-audit`](../transformation-roadmap-audit/SKILL.md) by default.

`prompt_revision: transformation-roadmap_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a TRANSFORMATION-ROADMAP."
- "Generate v2 of the TRANSFORMATION-ROADMAP from the updated source."
- "Draft a TRANSFORMATION-ROADMAP for <project>."

If the user asks to *audit an existing TRANSFORMATION-ROADMAP*, route to `transformation-roadmap-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  TRANSFORMATION-ROADMAP-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./transformation-roadmaps/",
...
```json
{
  ...
  "artefacts_written": [
    {"id": "TRANSFORMATION-ROADMAP-001", "path": "./transformation-roadmaps/TRANSFORMATION-ROADMAP-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "transformation-roadmap-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/transformation-roadmap/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `transformation-roadmap_author.transformation-roadmap_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `transformation-roadmap-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `transformation-roadmap-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   transformation-roadmap-author
Input:
