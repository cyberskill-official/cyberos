---
# ── Identity ─────────────────────────────────────────────────────────
name: partner-program-audit
description: >-
  Audit one or more existing partner-program@1 markdowns against partner-program_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after partner-program-author. Use when user asks to "audit this partner program" or "check the partner program". Do NOT use for "draft a new partner program" (use partner-program-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: partner-program@1
  cyberos-rubric-version: partner-program_rubric@1.0
...
# partner-program-audit — PARTNER-PROGRAM auditor

> Standalone trigger that runs `partner-program_rubric@1.0` against one or
> more existing `partner-program@1` markdowns and writes a sibling
> `partner-program.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`partner-program-author`](../partner-program-author/SKILL.md).

`prompt_revision: partner-program_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `PARTNER-PROGRAM`s."
- "Has `PARTNER-PROGRAM-007` changed since the last audit?"
- "Tell me which `PARTNER-PROGRAM`s would fail acceptance today."

Also invoked automatically by the supervisor when `partner-program-author`'s output envelope sets `next_skill_recommendation: partner-program-audit` (the default chain).
...
audit_rubric_version:            partner-program_rubric@1.0
...
```json
{
  "artefact_paths": ["./partner-programs/PARTNER-PROGRAM-001-foo.md", "./partner-programs/PARTNER-PROGRAM-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "partner-program-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./partner-programs/PARTNER-PROGRAM-001-foo.md", "audit_path": "./partner-programs/PARTNER-PROGRAM-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./partner-programs/PARTNER-PROGRAM-002-bar.md", "audit_path": "./partner-programs/PARTNER-PROGRAM-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   partner-program-audit
Input:
  artefact_paths:  [./team-a/PARTNER-PROGRAM-001-something.md, ./team-b/PARTNER-PROGRAM-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
