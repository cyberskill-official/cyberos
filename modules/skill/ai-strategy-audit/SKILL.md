...
description: >-
  Audits an ai-strategy@1 artefact against ai_strategy_rubric@1.0. Validates FM + SEC structure, EU AI Act risk-classification per use case, NIST AI RMF coverage, MLOps maturity self-assessment honesty, ethics-posture-vs-use-cases internal consistency. Per NIST AI RMF + EU AI Act + ISO/IEC 42001 conventions. Audit one or more existing ai-strategy@1 markdowns against ai-strategy_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after ai-strategy-author. Use when user asks to "audit this ai strategy" or "check the ai strategy". Do NOT use for "draft a new ai strategy" (use ai-strategy-author instead). Audit one or more existing ai-strategy@1 markdowns against ai-strategy_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after ai-strategy-au...
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: ai-strategy@1
  cyberos-rubric-version: ai-strategy_rubric@1.0
...
# ai-strategy-audit — AI_STRATEGY auditor

> Standalone trigger that runs `ai-strategy_rubric@1.0` against one or
> more existing `ai-strategy@1` markdowns and writes a sibling
> `ai-strategy.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`ai-strategy-author`](../ai-strategy-author/SKILL.md).

`prompt_revision: ai-strategy_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `AI_STRATEGY`s."
- "Has `AI_STRATEGY-007` changed since the last audit?"
- "Tell me which `AI_STRATEGY`s would fail acceptance today."

Also invoked automatically by the supervisor when `ai-strategy-author`'s output envelope sets `next_skill_recommendation: ai-strategy-audit` (the default chain).
...
audit_rubric_version:            ai-strategy_rubric@1.0
...
```json
{
  "artefact_paths": ["./ai-strategys/AI_STRATEGY-001-foo.md", "./ai-strategys/AI_STRATEGY-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "ai-strategy-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./ai-strategys/AI_STRATEGY-001-foo.md", "audit_path": "./ai-strategys/AI_STRATEGY-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./ai-strategys/AI_STRATEGY-002-bar.md", "audit_path": "./ai-strategys/AI_STRATEGY-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   ai-strategy-audit
Input:
  artefact_paths:  [./team-a/AI_STRATEGY-001-something.md, ./team-b/AI_STRATEGY-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
