# `retrospective-author` — pipeline

This document describes how `retrospective-author` chains with upstream and downstream skills.
...
|---|---|---|
| `retrospective-audit` | Default after every `artefact_written` event | `next_skill_recommendation: retrospective-audit` in output envelope. |
| `<next-stage>-author` | After `retrospective-audit` returns PASS | Supervisor reads the audit's output envelope and queues the next stage. |
...
|---|---|---|
| `retro_author.retro_written` | `{artefact_id, artefact_path, artefact_hash, source_hash}` | After every successful W3 WRITE. |
| `retro_author.batch_complete` | `{batch_run_id, artefacts_written, batch_outcome}` | At the end of a WORKER batch. |
| `retro_author.hitl_pause` | `{artefact_id, blocking_issues}` | When the batch halts on HITL. |
| `retro_author.amendment_request` | `{amendment_id, risk_class, change_description}` | When the author proposes a plan amendment. |
...
- `cyberos/skill/contracts/retrospective/` — the artefact template this skill generates.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/retrospective-audit/` — the sibling audit skill that validates this skill's output.
