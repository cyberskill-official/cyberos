---
# ── Identity ─────────────────────────────────────────────────────────
name: software-requirements-specification-author
description: >-
  Author a Software Requirements Specification (srs@1) per IEEE 830-1998 from an approved PRD, SOW, or discovery brief. Enumerates business, functional, and non-functional requirements; the NFR block enforces ISO/IEC 25010:2023's nine quality characteristics (functional suitability, performance efficiency, compatibility, interaction capability, reliability, security, maintainability, flexibility, safety). Covers SDP §2(b) Requirements. Chains naturally into software-requirements-specification-audit. Use when user asks to "draft a software requirements specification" or "create the software requirements specification". Do NOT use for "audit existing software requirements specification" (use software-requirements-specification-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: b
  cyberos-template: software-requirements-specification@1
  cyberos-rubric-target: srs_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['software-requirements-specification-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        srs
    ...
    pin_path:  cyberos/skill/contracts/software-requirements-specification/
  - id:        nats-subjects
    version:   v1
...
# software-requirements-specification-author — SRS generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `software-requirements-specification@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`software-requirements-specification-audit`](../software-requirements-specification-audit/SKILL.md) by default.

`prompt_revision: srs_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a SRS."
- "Generate v2 of the SRS from the updated source."
- "Draft a SRS for <project>."

If the user asks to *audit an existing SRS*, route to `software-requirements-specification-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 srs_author@1.0.0
template_version:                software-requirements-specification@1   (loaded from cyberos/skill/contracts/software-requirements-specification/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  SRS-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./srss/",
  "manifest_path": "./srss/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "SRS-001", "path": "./srss/SRS-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "software-requirements-specification-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/software-requirements-specification/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `srs_author.srs_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `software-requirements-specification-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `software-requirements-specification-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   software-requirements-specification-author
Input:
