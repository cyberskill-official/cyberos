...
# deployment-checklist-audit — DEPLOY-CHECKLIST auditor

> Standalone trigger that runs `deploy-checklist_rubric@1.0` against one or
> more existing `deployment-checklist@1` markdowns and writes a sibling
...
> Chains naturally after [`deployment-checklist-author`](../deployment-checklist-author/SKILL.md).
...
Also invoked automatically by the supervisor when `deployment-checklist-author`'s output envelope sets `next_skill_recommendation: deployment-checklist-audit` (the default chain).
...
```json
{
  ...
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "deployment-checklist-author",
...
```
Persona: cuo-cpo
Skill:   deployment-checklist-audit
Input:
