---
# ── Identity ─────────────────────────────────────────────────────────
name: requirements-traceability-matrix-audit
description: >-
  Audit one or more existing requirements-traceability-matrix@1 markdowns against rtm_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after requirements-traceability-matrix-author. Use when user asks to "audit this requirements traceability matrix" or "check the requirements traceability matrix". Do NOT use for "draft a new requirements traceability matrix" (use requirements-traceability-matrix-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: requirements-traceability-matrix@1
  cyberos-rubric-version: rtm_rubric@1.0
...
depends_on_contracts:
  - id:        rtm
    ...
    pin_path:  cyberos/skill/contracts/requirements-traceability-matrix/
  - id:        nats-subjects
    version:   v1
...
# requirements-traceability-matrix-audit — RTM auditor

> Standalone trigger that runs `rtm_rubric@1.0` against one or
> more existing `requirements-traceability-matrix@1` markdowns and writes a sibling
> `rtm.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`requirements-traceability-matrix-author`](../requirements-traceability-matrix-author/SKILL.md).

`prompt_revision: rtm_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `RTM`s."
- "Has `RTM-007` changed since the last audit?"
- "Tell me which `RTM`s would fail acceptance today."

Also invoked automatically by the supervisor when `requirements-traceability-matrix-author`'s output envelope sets `next_skill_recommendation: requirements-traceability-matrix-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        requirements-traceability-matrix-audit
skill_version:                   1.0.0
prompt_revision:                 rtm_audit@1.0.0
template_version:                requirements-traceability-matrix@1   (loaded from cyberos/skill/contracts/requirements-traceability-matrix/template.md)
audit_rubric_version:            rtm_rubric@1.0
...
```json
{
  "artefact_paths": ["./rtms/RTM-001-foo.md", "./rtms/RTM-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "requirements-traceability-matrix-author",
    "manifest_path": "./rtms/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./rtms/RTM-001-foo.md", "audit_path": "./rtms/RTM-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./rtms/RTM-002-bar.md", "audit_path": "./rtms/RTM-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   requirements-traceability-matrix-audit
Input:
  artefact_paths:  [./team-a/RTM-001-something.md, ./team-b/RTM-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
