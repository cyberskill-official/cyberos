---
# ── Identity ─────────────────────────────────────────────────────────
name: nda-triage-audit
description: Audits an nda-triage@1 artefact against nda_triage_rubric@1.0. Validates FM + SEC structure, screening-checklist completeness (mutuality / term / carveouts / non-compete / non-solicit / residuals / governing-law), and triage-decision consistency with the screening outcomes.
  Audit one or more existing nda-triage@1 markdowns against
...
depends_on_contracts:
  - id:        nda-triage
    ...
    pin_path:  cyberos/skill/contracts/nda-triage/
  - id:        nats-subjects
    version:   v1
...
# nda-triage-audit — NDA_TRIAGE auditor

> Standalone trigger that runs `nda-triage_rubric@1.0` against one or
> more existing `nda-triage@1` markdowns and writes a sibling
> `nda-triage.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`nda-triage-author`](../nda-triage-author/SKILL.md).

`prompt_revision: nda-triage_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `NDA_TRIAGE`s."
- "Has `NDA_TRIAGE-007` changed since the last audit?"
- "Tell me which `NDA_TRIAGE`s would fail acceptance today."

Also invoked automatically by the supervisor when `nda-triage-author`'s output envelope sets `next_skill_recommendation: nda-triage-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        nda-triage-audit
skill_version:                   1.0.0
prompt_revision:                 nda-triage_audit@1.0.0
template_version:                nda-triage@1   (loaded from cyberos/skill/contracts/nda-triage/template.md)
audit_rubric_version:            nda-triage_rubric@1.0
...
```json
{
  "artefact_paths": ["./nda-triages/NDA_TRIAGE-001-foo.md", "./nda-triages/NDA_TRIAGE-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "nda-triage-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./nda-triages/NDA_TRIAGE-001-foo.md", "audit_path": "./nda-triages/NDA_TRIAGE-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./nda-triages/NDA_TRIAGE-002-bar.md", "audit_path": "./nda-triages/NDA_TRIAGE-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   nda-triage-audit
Input:
  artefact_paths:  [./team-a/NDA_TRIAGE-001-something.md, ./team-b/NDA_TRIAGE-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
