---
# ── Identity ─────────────────────────────────────────────────────────
name: capacity-plan-author
description: >-
  Author a quarterly capacity-plan rebase (per-team FTE × utilisation × backlog + new-work intake + utilisation forecast + hire/contract/defer recommendations). COO authored. Target utilisation 70-85% for consulting. Use when user asks to "draft a capacity plan" or "create the capacity plan". Do NOT use for "audit existing capacity plan" (use capacity-plan-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: capacity-plan_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['capacity-plan-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        capacity-plan
    ...
    pin_path:  cyberos/skill/contracts/capacity-plan/
  - id:        nats-subjects
    version:   v1
...
# capacity-plan-author — CAPACITY-PLAN generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `capacity-plan@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`capacity-plan-audit`](../capacity-plan-audit/SKILL.md) by default.

`prompt_revision: capacity-plan_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a CAPACITY-PLAN."
- "Generate v2 of the CAPACITY-PLAN from the updated source."
- "Draft a CAPACITY-PLAN for <project>."

If the user asks to *audit an existing CAPACITY-PLAN*, route to `capacity-plan-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 capacity-plan_author@1.0.0
template_version:                capacity-plan@1   (loaded from cyberos/skill/contracts/capacity-plan/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  CAPACITY-PLAN-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./capacity-plans/",
  "manifest_path": "./capacity-plans/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "CAPACITY-PLAN-001", "path": "./capacity-plans/CAPACITY-PLAN-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "capacity-plan-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/capacity-plan/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `capacity-plan_author.capacity-plan_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
- If chained to `capacity-plan-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `capacity-plan-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   capacity-plan-author
Input:
