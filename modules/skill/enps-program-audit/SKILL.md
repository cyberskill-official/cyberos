---
# ── Identity ─────────────────────────────────────────────────────────
name: enps-program-audit
description: |
  Audit one or more existing enps-program@1 markdowns against
...
metadata:
  ...
  cyberos-template: enps-program@1
...
depends_on_contracts:
  - id:        enps-program
    ...
    pin_path:  cyberos/skill/contracts/enps-program/
  - id:        nats-subjects
    version:   v1
...
# enps-program-audit — ENPS-PROGRAM auditor

> Standalone trigger that runs `enps-program_rubric@1.0` against one or
> more existing `enps-program@1` markdowns and writes a sibling
...
> Chains naturally after [`enps-program-author`](../enps-program-author/SKILL.md).
...
Also invoked automatically by the supervisor when `enps-program-author`'s output envelope sets `next_skill_recommendation: enps-program-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        enps-program-audit
skill_version:                   1.0.0
prompt_revision:                 enps-program_audit@1.0.0
template_version:                enps-program@1   (loaded from cyberos/skill/contracts/enps-program/template.md)
...
```json
{
  ...
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "enps-program-author",
