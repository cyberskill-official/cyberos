...
description: >-
  Audit one or more existing board-deck@1 markdowns against board-deck_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after board-deck-author. Use when user asks to "audit this board deck" or "check the board deck". Do NOT use for "draft a new board deck" (use board-deck-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: board-deck@1
  cyberos-rubric-version: board-deck_rubric@1.0
...
# board-deck-audit — BOARD-DECK auditor

> Standalone trigger that runs `board-deck_rubric@1.0` against one or
> more existing `board-deck@1` markdowns and writes a sibling
> `board-deck.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`board-deck-author`](../board-deck-author/SKILL.md).

`prompt_revision: board-deck_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `BOARD-DECK`s."
- "Has `BOARD-DECK-007` changed since the last audit?"
- "Tell me which `BOARD-DECK`s would fail acceptance today."

Also invoked automatically by the supervisor when `board-deck-author`'s output envelope sets `next_skill_recommendation: board-deck-audit` (the default chain).
...
audit_rubric_version:            board-deck_rubric@1.0
...
```json
{
  "artefact_paths": ["./board-decks/BOARD-DECK-001-foo.md", "./board-decks/BOARD-DECK-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "board-deck-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./board-decks/BOARD-DECK-001-foo.md", "audit_path": "./board-decks/BOARD-DECK-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./board-decks/BOARD-DECK-002-bar.md", "audit_path": "./board-decks/BOARD-DECK-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
Input:
  artefact_paths:  [./team-a/BOARD-DECK-001-something.md, ./team-b/BOARD-DECK-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
