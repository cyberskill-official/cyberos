...
description: >-
  Audit one or more existing implementation-plan@1 markdowns against impl-plan_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after implementation-plan-author. Use when user asks to "audit this implementation plan" or "check the implementation plan". Do NOT use for "draft a new implementation plan" (use implementation-plan-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: f
  cyberos-template: implementation-plan@1
  cyberos-rubric-version: impl_plan_rubric@1.0
...
depends_on_contracts:
  - id:        impl-plan
    ...
    pin_path:  cyberos/skill/contracts/implementation-plan/
  - id:        nats-subjects
    version:   v1
...
# implementation-plan-audit — IMPL-PLAN auditor

> Standalone trigger that runs `impl-plan_rubric@1.0` against one or
> more existing `implementation-plan@1` markdowns and writes a sibling
> `impl-plan.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`implementation-plan-author`](../implementation-plan-author/SKILL.md).

`prompt_revision: impl-plan_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `IMPL-PLAN`s."
- "Has `IMPL-PLAN-007` changed since the last audit?"
- "Tell me which `IMPL-PLAN`s would fail acceptance today."

Also invoked automatically by the supervisor when `implementation-plan-author`'s output envelope sets `next_skill_recommendation: implementation-plan-audit` (the default chain).
...
skill_version:                   1.0.0
prompt_revision:                 impl-plan_audit@1.0.0
template_version:                implementation-plan@1   (loaded from cyberos/skill/contracts/implementation-plan/template.md)
audit_rubric_version:            impl-plan_rubric@1.0
...
```json
{
  "artefact_paths": ["./impl-plans/IMPL-PLAN-001-foo.md", "./impl-plans/IMPL-PLAN-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "implementation-plan-author",
    "manifest_path": "./impl-plans/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./impl-plans/IMPL-PLAN-001-foo.md", "audit_path": "./impl-plans/IMPL-PLAN-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./impl-plans/IMPL-PLAN-002-bar.md", "audit_path": "./impl-plans/IMPL-PLAN-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
Input:
  artefact_paths:  [./team-a/IMPL-PLAN-001-something.md, ./team-b/IMPL-PLAN-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
