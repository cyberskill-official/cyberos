---
# ── Identity ─────────────────────────────────────────────────────────
name: key-risk-indicator-dashboard-author
description: >-
  Author the quarterly KRI dashboard (KRI table per risk category + breach detail + trend charts + control effectiveness + mitigation status). CRO-Risk authored. Use when user asks to "draft a key risk indicator dashboard" or "create the key risk indicator dashboard". Do NOT use for "audit existing key risk indicator dashboard" (use key-risk-indicator-dashboard-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: kri-dashboard_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['key-risk-indicator-dashboard-audit']; empty list disables chaining
...
# key-risk-indicator-dashboard-author — KRI-DASHBOARD generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `key-risk-indicator-dashboard@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`key-risk-indicator-dashboard-audit`](../key-risk-indicator-dashboard-audit/SKILL.md) by default.

`prompt_revision: kri-dashboard_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a KRI-DASHBOARD."
- "Generate v2 of the KRI-DASHBOARD from the updated source."
- "Draft a KRI-DASHBOARD for <project>."

If the user asks to *audit an existing KRI-DASHBOARD*, route to `key-risk-indicator-dashboard-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        key-risk-indicator-dashboard-author
skill_version:                   1.0.0
prompt_revision:                 kri-dashboard_author@1.0.0
...
naming_pattern:                  KRI-DASHBOARD-{NNN}-{slug}.md
...
```json
{
  ...
  "artefacts_written": [
    {"id": "KRI-DASHBOARD-001", "path": "./kri-dashboards/KRI-DASHBOARD-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "key-risk-indicator-dashboard-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/key-risk-indicator-dashboard/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `kri-dashboard_author.kri-dashboard_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
- If chained to `key-risk-indicator-dashboard-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `key-risk-indicator-dashboard-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   key-risk-indicator-dashboard-author
Input:
