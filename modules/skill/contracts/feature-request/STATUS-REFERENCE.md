...
|---|---|---|---|
| 1 | `draft` | Author has started writing the spec; not yet audited. | `feature-request-author` |
| 2 | `ready_to_implement` | Spec passes `feature-request-audit` at 10/10; eligible for the build queue. **Also the status an FR returns to when an in-cycle step (implementing / reviewing / testing) fails or is blocked — see §1.3.** | `feature-request-audit`, `backlog-state-update-author` (rework path) |
...
| # | Status | Meaning |
|---|---|---|
| 9 | `on_hold` | Deliberately deferred — out of scope for the current wave, will revisit later. Stays in BACKLOG.md as a future candidate. Skipped by the default `ship-feature-requests` queue. |
...
```
                       ...
                       ▼                                                          │
                    draft                                                         │
                       │ feature-request-audit (10/10)                            │
                       ▼                                                          │
                ready_to_implement ◄──────── (in-cycle fail/blocked rework) ──────┤
                       │ ship-feature-requests start                              │
                       ▼                                                          │
...
The `re_audit` mode that briefly existed in `implement-backlog-frs` v1.2.0 is also retired — the operator just HITL-flips `done → ready_to_review` instead, and `ship-feature-requests` re-runs the review + test gates naturally.
