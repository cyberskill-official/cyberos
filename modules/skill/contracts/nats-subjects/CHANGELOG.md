...
- `schema.json` — JSON Schema for every subject's payload shape: `fr_written`, `batch_complete`, `hitl_pause`, `audit_written`, `audit_batch_complete`, `refinement_proposed`, `session_lifecycle` (covers both `session_start` and `session_end`).
