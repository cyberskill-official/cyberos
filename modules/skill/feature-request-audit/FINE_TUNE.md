# `feature-request-audit` — fine-tune discipline override

Default discipline at `../docs/FINE_TUNE.md`. This file documents the **feature-request-audit-specific overrides**.

## Why feature-request-audit is different

`feature-request-audit` ships with `audit_rubric@2.0` ported verbatim from the proven legacy `cuo/cpo/feature-request-audit` skill (2026-02 vintage). The rubric was battle-tested against 50+ FRs in `cyberos/docs/feature-requests/` before being ported. Bumping it requires extra scrutiny because:
...
3. The feature-request-author + feature-request-audit pair is the cyberos project's primary feature-tracking workflow. Audit instability shakes confidence in every FR's status.
...
- Changing the cross-skill XCHAIN-001/002 contract with feature-request-author.
