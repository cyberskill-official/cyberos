---
# ── Identity ─────────────────────────────────────────────────────────
name: change-mgmt-plan-audit
description: |
  Audit one or more existing change-mgmt-plan@1 markdowns against
  ...
  change-mgmt-plan-author.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: <SDP §2 stage letter or "cross">
  cyberos-template: change-mgmt-plan@1
...
# change-mgmt-plan-audit — CHANGE-MGMT-PLAN auditor

> Standalone trigger that runs `change-mgmt-plan_rubric@1.0` against one or
> more existing `change-mgmt-plan@1` markdowns and writes a sibling
...
> Chains naturally after [`change-mgmt-plan-author`](../change-mgmt-plan-author/SKILL.md).
...
Also invoked automatically by the supervisor when `change-mgmt-plan-author`'s output envelope sets `next_skill_recommendation: change-mgmt-plan-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        change-mgmt-plan-audit
skill_version:                   1.0.0
prompt_revision:                 change-mgmt-plan_audit@1.0.0
...
```json
{
  ...
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "change-mgmt-plan-author",
