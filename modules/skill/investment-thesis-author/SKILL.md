...
description: >-
  Author a per-position investment thesis (market structure + entry rationale + risk-reward analysis + position sizing + exit conditions + monitoring plan). Soros/Druckenmiller thesis-discipline + CFA Institute mandate framework. CIO-Investment authored. Use when user asks to "draft a investment thesis" or "create the investment thesis". Do NOT use for "audit existing investment thesis" (use investment-thesis-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: investment-thesis_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['investment-thesis-audit']; empty list disables chaining
...
# investment-thesis-author — INVESTMENT-THESIS generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `investment-thesis@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`investment-thesis-audit`](../investment-thesis-audit/SKILL.md) by default.

`prompt_revision: investment-thesis_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a INVESTMENT-THESIS."
- "Generate v2 of the INVESTMENT-THESIS from the updated source."
- "Draft a INVESTMENT-THESIS for <project>."

If the user asks to *audit an existing INVESTMENT-THESIS*, route to `investment-thesis-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  INVESTMENT-THESIS-{NNN}-{slug}.md
...
```json
{
  ...
  "artefacts_written": [
    {"id": "INVESTMENT-THESIS-001", "path": "./investment-thesiss/INVESTMENT-THESIS-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "investment-thesis-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/investment-thesis/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `investment-thesis_author.investment-thesis_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `investment-thesis-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `investment-thesis-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   investment-thesis-author
Input:
