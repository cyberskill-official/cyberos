...
description: Authors the annual (or per-event) IP portfolio strategy — patent filings, trademark portfolio, trade-secret protection plan, defensive vs offensive posture, FTO (freedom-to-operate) opinions. Per WIPO IP strategy framework + USPTO MPEP + Madrid Protocol (trademarks) + UTSA/DTSA (trade secrets) + PCT for international filings.
  ...
  versioned ip-strategy@1 file under output_dir, with per-claim authority
  markers and provenance to the source. Chains naturally into
  ip-strategy-audit by default. Refuses to author when upstream artefact
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['ip-strategy-audit']; empty list disables chaining
...
# ip-strategy-author — IP_STRATEGY generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `ip-strategy@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`ip-strategy-audit`](../ip-strategy-audit/SKILL.md) by default.
...
If the user asks to *audit an existing IP_STRATEGY*, route to `ip-strategy-audit` instead. If both, the supervisor chains the two.
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "ip-strategy-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/ip-strategy/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `ip-strategy-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `ip-strategy-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   ip-strategy-author
Input:
