# `ip-strategy-author` — pipeline

This document describes how `ip-strategy-author` chains with upstream and downstream skills.
...
|---|---|---|
| `ip-strategy-audit` | Default after every `artefact_written` event | `next_skill_recommendation: ip-strategy-audit` in output envelope. |
| `<next-stage>-author` | After `ip-strategy-audit` returns PASS | Supervisor reads the audit's output envelope and queues the next stage. |
...
- `cyberos/skill/contracts/ip-strategy/` — the artefact template this skill generates.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/ip-strategy-audit/` — the sibling audit skill that validates this skill's output.
