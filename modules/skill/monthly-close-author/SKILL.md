---
# ── Identity ─────────────────────────────────────────────────────────
name: monthly-close-author
description: >-
  Author the monthly close package (close-calendar adherence + GL completeness + reconciliation + journal entries + variance commentary + audit trail). Target close cycle <10 business days. CFO / CAO-Accounting authored. Use when user asks to "draft a monthly close" or "create the monthly close". Do NOT use for "audit existing monthly close" (use monthly-close-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: monthly-close_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['monthly-close-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        monthly-close
    ...
    pin_path:  cyberos/skill/contracts/monthly-close/
  - id:        nats-subjects
    version:   v1
...
# monthly-close-author — MONTHLY-CLOSE generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `monthly-close@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`monthly-close-audit`](../monthly-close-audit/SKILL.md) by default.

`prompt_revision: monthly-close_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a MONTHLY-CLOSE."
- "Generate v2 of the MONTHLY-CLOSE from the updated source."
- "Draft a MONTHLY-CLOSE for <project>."

If the user asks to *audit an existing MONTHLY-CLOSE*, route to `monthly-close-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        monthly-close-author
skill_version:                   1.0.0
prompt_revision:                 monthly-close_author@1.0.0
template_version:                monthly-close@1   (loaded from cyberos/skill/contracts/monthly-close/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  MONTHLY-CLOSE-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./monthly-closes/",
  "manifest_path": "./monthly-closes/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "MONTHLY-CLOSE-001", "path": "./monthly-closes/MONTHLY-CLOSE-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "monthly-close-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/monthly-close/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `monthly-close_author.monthly-close_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `monthly-close-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `monthly-close-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   monthly-close-author
Input:
