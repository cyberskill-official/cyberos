---
# ── Identity ─────────────────────────────────────────────────────────
name: product-roadmap-audit
description: >-
  Audit one or more existing product-roadmap@1 markdowns against product-roadmap_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after product-roadmap-author. Use when user asks to "audit this product roadmap" or "check the product roadmap". Do NOT use for "draft a new product roadmap" (use product-roadmap-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: product-roadmap@1
  cyberos-rubric-version: product-roadmap_rubric@1.0
...
depends_on_contracts:
  - id:        product-roadmap
    ...
    pin_path:  cyberos/skill/contracts/product-roadmap/
  - id:        nats-subjects
    version:   v1
...
# product-roadmap-audit — PRODUCT-ROADMAP auditor

> Standalone trigger that runs `product-roadmap_rubric@1.0` against one or
> more existing `product-roadmap@1` markdowns and writes a sibling
> `product-roadmap.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`product-roadmap-author`](../product-roadmap-author/SKILL.md).

`prompt_revision: product-roadmap_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `PRODUCT-ROADMAP`s."
- "Has `PRODUCT-ROADMAP-007` changed since the last audit?"
- "Tell me which `PRODUCT-ROADMAP`s would fail acceptance today."

Also invoked automatically by the supervisor when `product-roadmap-author`'s output envelope sets `next_skill_recommendation: product-roadmap-audit` (the default chain).
...
skill_version:                   1.0.0
prompt_revision:                 product-roadmap_audit@1.0.0
template_version:                product-roadmap@1   (loaded from cyberos/skill/contracts/product-roadmap/template.md)
audit_rubric_version:            product-roadmap_rubric@1.0
...
```json
{
  "artefact_paths": ["./product-roadmaps/PRODUCT-ROADMAP-001-foo.md", "./product-roadmaps/PRODUCT-ROADMAP-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "product-roadmap-author",
    "manifest_path": "./product-roadmaps/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./product-roadmaps/PRODUCT-ROADMAP-001-foo.md", "audit_path": "./product-roadmaps/PRODUCT-ROADMAP-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./product-roadmaps/PRODUCT-ROADMAP-002-bar.md", "audit_path": "./product-roadmaps/PRODUCT-ROADMAP-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   product-roadmap-audit
Input:
  artefact_paths:  [./team-a/PRODUCT-ROADMAP-001-something.md, ./team-b/PRODUCT-ROADMAP-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
