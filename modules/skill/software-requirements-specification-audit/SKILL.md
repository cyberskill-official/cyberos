---
# ── Identity ─────────────────────────────────────────────────────────
name: software-requirements-specification-audit
description: >-
  Audit one or more existing software-requirements-specification@1 markdowns against srs_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after software-requirements-specification-author. Use when user asks to "audit this software requirements specification" or "check the software requirements specification". Do NOT use for "draft a new software requirements specification" (use software-requirements-specification-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-version: srs_rubric@1.0
...
# software-requirements-specification-audit — SRS auditor

> Standalone trigger that runs `srs_rubric@1.0` against one or
> more existing `software-requirements-specification@1` markdowns and writes a sibling
> `srs.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`software-requirements-specification-author`](../software-requirements-specification-author/SKILL.md).

`prompt_revision: srs_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `SRS`s."
- "Has `SRS-007` changed since the last audit?"
- "Tell me which `SRS`s would fail acceptance today."

Also invoked automatically by the supervisor when `software-requirements-specification-author`'s output envelope sets `next_skill_recommendation: software-requirements-specification-audit` (the default chain).
...
audit_rubric_version:            srs_rubric@1.0
...
```json
{
  "artefact_paths": ["./srss/SRS-001-foo.md", "./srss/SRS-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "software-requirements-specification-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./srss/SRS-001-foo.md", "audit_path": "./srss/SRS-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./srss/SRS-002-bar.md", "audit_path": "./srss/SRS-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   software-requirements-specification-audit
Input:
  artefact_paths:  [./team-a/SRS-001-something.md, ./team-b/SRS-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
