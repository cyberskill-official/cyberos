...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['13-week-cash-flow-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        13-week-cash-flow
    ...
    pin_path:  cyberos/skill/contracts/13-week-cash-flow/
  - id:        nats-subjects
    version:   v1
...
# 13-week-cash-flow-author — 13-WEEK-CASH-FLOW generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `13-week-cash-flow@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`13-week-cash-flow-audit`](../13-week-cash-flow-audit/SKILL.md) by default.
...
If the user asks to *audit an existing 13-WEEK-CASH-FLOW*, route to `13-week-cash-flow-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        13-week-cash-flow-author
skill_version:                   1.0.0
prompt_revision:                 13-week-cash-flow_author@1.0.0
template_version:                13-week-cash-flow@1   (loaded from cyberos/skill/contracts/13-week-cash-flow/template.md)
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "13-week-cash-flow-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/13-week-cash-flow/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `13-week-cash-flow-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `13-week-cash-flow-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   13-week-cash-flow-author
Input:
