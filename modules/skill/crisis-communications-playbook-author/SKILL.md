---
# ── Identity ─────────────────────────────────────────────────────────
name: crisis-communications-playbook-author
description: >-
  Author the annual crisis-comms playbook (severity tiers + activation criteria + holding statement library + spokesperson matrix + escalation tree + post-crisis review template). PRSA + Coombs SCCT aligned. CCO-Communications + CSO-Security co-authored. Use when user asks to "draft a crisis communications playbook" or "create the crisis communications playbook". Do NOT use for "audit existing crisis communications playbook" (use crisis-communications-playbook-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: crisis-comms-playbook_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['crisis-communications-playbook-audit']; empty list disables chaining
...
# crisis-communications-playbook-author — CRISIS-COMMS-PLAYBOOK generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `crisis-communications-playbook@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`crisis-communications-playbook-audit`](../crisis-communications-playbook-audit/SKILL.md) by default.

`prompt_revision: crisis-comms-playbook_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a CRISIS-COMMS-PLAYBOOK."
- "Generate v2 of the CRISIS-COMMS-PLAYBOOK from the updated source."
- "Draft a CRISIS-COMMS-PLAYBOOK for <project>."

If the user asks to *audit an existing CRISIS-COMMS-PLAYBOOK*, route to `crisis-communications-playbook-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  CRISIS-COMMS-PLAYBOOK-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./crisis-comms-playbooks/",
...
```json
{
  ...
  "artefacts_written": [
    {"id": "CRISIS-COMMS-PLAYBOOK-001", "path": "./crisis-comms-playbooks/CRISIS-COMMS-PLAYBOOK-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "crisis-communications-playbook-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/crisis-communications-playbook/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- `PIPELINE.md` — how this skill chains to `crisis-communications-playbook-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   crisis-communications-playbook-author
Input:
