# `model-card-author` — human summary format
...
```
MODEL-CARD batch <batch_run_id> complete

Wrote N MODEL-CARD(s):
  - MODEL-CARD-001: <slug> — PASS (audit verdict: pass, 0 open issues)
  - MODEL-CARD-002: <slug> — HITL_PAUSE (2 blocking issues — see HITL_BATCH_REQUEST below)
  - MODEL-CARD-003: <slug> — PASS (audit verdict: pass, 1 warning)

Manifest:    <output_dir>/manifest.json
Total time:  <seconds>s
Next:        model-card-audit will run automatically on PASS items. HITL items wait for your reply.
```
