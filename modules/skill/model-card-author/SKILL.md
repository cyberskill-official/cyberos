---
# ── Identity ─────────────────────────────────────────────────────────
name: model-card-author
description: >-
  Author a per-AI-model model card (identity + intended use + out-of-scope uses + training data + evaluation results + bias analysis + safety considerations + limitations + contact). Hugging Face model-card pattern + Mitchell et al. 2019. CAIO + Chief Ethics Officer co-authored. Use when user asks to "draft a model card" or "create the model card". Do NOT use for "audit existing model card" (use model-card-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: model-card_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['model-card-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        model-card
    ...
    pin_path:  cyberos/skill/contracts/model-card/
  - id:        nats-subjects
    version:   v1
...
# model-card-author — MODEL-CARD generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `model-card@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`model-card-audit`](../model-card-audit/SKILL.md) by default.

`prompt_revision: model-card_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a MODEL-CARD."
- "Generate v2 of the MODEL-CARD from the updated source."
- "Draft a MODEL-CARD for <project>."

If the user asks to *audit an existing MODEL-CARD*, route to `model-card-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 model-card_author@1.0.0
template_version:                model-card@1   (loaded from cyberos/skill/contracts/model-card/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  MODEL-CARD-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./model-cards/",
  "manifest_path": "./model-cards/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "MODEL-CARD-001", "path": "./model-cards/MODEL-CARD-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "model-card-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/model-card/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `model-card_author.model-card_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `model-card-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `model-card-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   model-card-author
Input:
