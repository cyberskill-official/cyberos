...
metadata:
  ...
  cyberos-rubric-version: test_strategy_rubric@1.0
...
# test-strategy-audit — TEST-STRATEGY auditor

> Standalone trigger that runs `test-strategy_rubric@1.0` against one or
> more existing `test-strategy@1` markdowns and writes a sibling
> `test-strategy.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`test-strategy-author`](../test-strategy-author/SKILL.md).

`prompt_revision: test-strategy_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `TEST-STRATEGY`s."
- "Has `TEST-STRATEGY-007` changed since the last audit?"
- "Tell me which `TEST-STRATEGY`s would fail acceptance today."

Also invoked automatically by the supervisor when `test-strategy-author`'s output envelope sets `next_skill_recommendation: test-strategy-audit` (the default chain).
...
audit_rubric_version:            test-strategy_rubric@1.0
...
```json
{
  "artefact_paths": ["./test-strategys/TEST-STRATEGY-001-foo.md", "./test-strategys/TEST-STRATEGY-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "test-strategy-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./test-strategys/TEST-STRATEGY-001-foo.md", "audit_path": "./test-strategys/TEST-STRATEGY-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./test-strategys/TEST-STRATEGY-002-bar.md", "audit_path": "./test-strategys/TEST-STRATEGY-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   test-strategy-audit
Input:
  artefact_paths:  [./team-a/TEST-STRATEGY-001-something.md, ./team-b/TEST-STRATEGY-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
