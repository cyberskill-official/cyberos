---
# ── Identity ─────────────────────────────────────────────────────────
name: customer-advisory-board-audit
description: >-
  Audit one or more existing customer-advisory-board@1 markdowns against customer-advisory-board_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after customer-advisory-board-author. Use when user asks to "audit this customer advisory board" or "check the customer advisory board". Do NOT use for "draft a new customer advisory board" (use customer-advisory-board-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: customer-advisory-board@1
  cyberos-rubric-version: customer-advisory-board_rubric@1.0
...
depends_on_contracts:
  - id:        customer-advisory-board
    ...
    pin_path:  cyberos/skill/contracts/customer-advisory-board/
  - id:        nats-subjects
    version:   v1
...
# customer-advisory-board-audit — CUSTOMER-ADVISORY-BOARD auditor

> Standalone trigger that runs `customer-advisory-board_rubric@1.0` against one or
> more existing `customer-advisory-board@1` markdowns and writes a sibling
> `customer-advisory-board.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`customer-advisory-board-author`](../customer-advisory-board-author/SKILL.md).

`prompt_revision: customer-advisory-board_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `CUSTOMER-ADVISORY-BOARD`s."
- "Has `CUSTOMER-ADVISORY-BOARD-007` changed since the last audit?"
- "Tell me which `CUSTOMER-ADVISORY-BOARD`s would fail acceptance today."

Also invoked automatically by the supervisor when `customer-advisory-board-author`'s output envelope sets `next_skill_recommendation: customer-advisory-board-audit` (the default chain).
...
skill_version:                   1.0.0
prompt_revision:                 customer-advisory-board_audit@1.0.0
template_version:                customer-advisory-board@1   (loaded from cyberos/skill/contracts/customer-advisory-board/template.md)
audit_rubric_version:            customer-advisory-board_rubric@1.0
...
```json
{
  "artefact_paths": ["./customer-advisory-boards/CUSTOMER-ADVISORY-BOARD-001-foo.md", "./customer-advisory-boards/CUSTOMER-ADVISORY-BOARD-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "customer-advisory-board-author",
    "manifest_path": "./customer-advisory-boards/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./customer-advisory-boards/CUSTOMER-ADVISORY-BOARD-001-foo.md", "audit_path": "./customer-advisory-boards/CUSTOMER-ADVISORY-BOARD-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./customer-advisory-boards/CUSTOMER-ADVISORY-BOARD-002-bar.md", "audit_path": "./customer-advisory-boards/CUSTOMER-ADVISORY-BOARD-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   customer-advisory-board-audit
Input:
  artefact_paths:  [./team-a/CUSTOMER-ADVISORY-BOARD-001-something.md, ./team-b/CUSTOMER-ADVISORY-BOARD-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
