# `customer_advisory_board_rubric@1.0` — machine-checkable Customer Advisory Board rubric
...
| `FM-004` | `template` equals `customer-advisory-board@1` | error | true |
...
Per the `contracts/customer-advisory-board/template.md` H2 list — every H2 in template MUST be present + non-empty in the authored artefact.

| rule_id | Check | Severity |
| ------- | ----- | -------- |
| `SEC-001` | Every H2 declared in `contracts/customer-advisory-board/template.md` is present in the authored artefact | error |
...
- `cyberos/skill/contracts/customer-advisory-board/CONTRACT.md` + `template.md`
