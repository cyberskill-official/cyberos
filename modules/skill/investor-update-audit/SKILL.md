---
# ── Identity ─────────────────────────────────────────────────────────
name: investor-update-audit
description: >-
  Audit one or more existing investor-update@1 markdowns against investor-update_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after investor-update-author. Use when user asks to "audit this investor update" or "check the investor update". Do NOT use for "draft a new investor update" (use investor-update-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: investor-update@1
  cyberos-rubric-version: investor-update_rubric@1.0
...
depends_on_contracts:
  - id:        investor-update
    ...
    pin_path:  cyberos/skill/contracts/investor-update/
  - id:        nats-subjects
    version:   v1
...
# investor-update-audit — INVESTOR-UPDATE auditor

> Standalone trigger that runs `investor-update_rubric@1.0` against one or
> more existing `investor-update@1` markdowns and writes a sibling
> `investor-update.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`investor-update-author`](../investor-update-author/SKILL.md).

`prompt_revision: investor-update_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `INVESTOR-UPDATE`s."
- "Has `INVESTOR-UPDATE-007` changed since the last audit?"
- "Tell me which `INVESTOR-UPDATE`s would fail acceptance today."

Also invoked automatically by the supervisor when `investor-update-author`'s output envelope sets `next_skill_recommendation: investor-update-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        investor-update-audit
skill_version:                   1.0.0
prompt_revision:                 investor-update_audit@1.0.0
template_version:                investor-update@1   (loaded from cyberos/skill/contracts/investor-update/template.md)
audit_rubric_version:            investor-update_rubric@1.0
...
```json
{
  "artefact_paths": ["./investor-updates/INVESTOR-UPDATE-001-foo.md", "./investor-updates/INVESTOR-UPDATE-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "investor-update-author",
    "manifest_path": "./investor-updates/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./investor-updates/INVESTOR-UPDATE-001-foo.md", "audit_path": "./investor-updates/INVESTOR-UPDATE-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./investor-updates/INVESTOR-UPDATE-002-bar.md", "audit_path": "./investor-updates/INVESTOR-UPDATE-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   investor-update-audit
Input:
  artefact_paths:  [./team-a/INVESTOR-UPDATE-001-something.md, ./team-b/INVESTOR-UPDATE-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
