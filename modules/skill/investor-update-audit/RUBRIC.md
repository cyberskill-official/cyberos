# `investor_update_rubric@1.0` — machine-checkable Investor Update rubric
...
| `FM-004` | `template` equals `investor-update@1` | error | true |
...
Per the `contracts/investor-update/template.md` H2 list. Each H2 in the template MUST be present + non-empty in the authored artefact. See template for the canonical headings.

| rule_id | Check | Severity |
| ------- | ----- | -------- |
| `SEC-001` | Every H2 declared in `contracts/investor-update/template.md` is present in the authored artefact | error |
...
- `cyberos/skill/contracts/investor-update/CONTRACT.md` + `template.md`
