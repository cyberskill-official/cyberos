---
# ── Identity ─────────────────────────────────────────────────────────
name: capital-allocation-memo-author
description: >-
  Author a capital-allocation memo for board approval (buy vs build vs partner vs M&A). Renders decision + options analysed + financial model + strategic fit + risks + recommendation. CEO + CFO co-authored. Use when user asks to "draft a capital allocation memo" or "create the capital allocation memo". Do NOT use for "audit existing capital allocation memo" (use capital-allocation-memo-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['capital-allocation-memo-audit']; empty list disables chaining
...
# capital-allocation-memo-author — CAP-ALLOC-MEMO generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `capital-allocation-memo@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`capital-allocation-memo-audit`](../capital-allocation-memo-audit/SKILL.md) by default.

`prompt_revision: cap-alloc-memo_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a CAP-ALLOC-MEMO."
- "Generate v2 of the CAP-ALLOC-MEMO from the updated source."
- "Draft a CAP-ALLOC-MEMO for <project>."

If the user asks to *audit an existing CAP-ALLOC-MEMO*, route to `capital-allocation-memo-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  CAP-ALLOC-MEMO-{NNN}-{slug}.md
...
```json
{
  ...
  "artefacts_written": [
    {"id": "CAP-ALLOC-MEMO-001", "path": "./cap-alloc-memos/CAP-ALLOC-MEMO-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "capital-allocation-memo-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/capital-allocation-memo/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `cap-alloc-memo_author.cap-alloc-memo_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `capital-allocation-memo-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `capital-allocation-memo-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   capital-allocation-memo-author
Input:
