# `capital-allocation-memo-author` — standalone interview
...
|---|---|---|
| `source_files` | "Which source file(s) should I read to draft the `CAP-ALLOC-MEMO`?" | One or more existing paths or URLs. The skill confirms each path resolves and is UTF-8 readable. |
| `output_dir` | "Where should I write the `CAP-ALLOC-MEMO` markdown(s)?" | An absolute or project-relative directory path. The skill creates the directory if it does not exist (subject to scope sandbox). |
...
| `batch_size` | "How many `CAP-ALLOC-MEMO`s per batch? (default 3, max 10)" | 3 |
| `caller_persona` | "Who is asking? (cuo-cpo / cuo-cto / cuo-clo / cuo-cseco / cuo-coo / cuo-ceo)" | `cuo-cpo` |
| `chain_to` | "Should I chain to the audit skill afterwards? (default yes)" | `['capital-allocation-memo-audit']` |
