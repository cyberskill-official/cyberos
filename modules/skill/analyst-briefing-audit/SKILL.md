---
# ── Identity ─────────────────────────────────────────────────────────
name: analyst-briefing-audit
description: >-
  Audit one or more existing analyst-briefing@1 markdowns against analyst-briefing_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after analyst-briefing-author. Use when user asks to "audit this analyst briefing" or "check the analyst briefing". Do NOT use for "draft a new analyst briefing" (use analyst-briefing-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: analyst-briefing@1
  cyberos-rubric-version: analyst-briefing_rubric@1.0
...
depends_on_contracts:
  - id:        analyst-briefing
    ...
    pin_path:  cyberos/skill/contracts/analyst-briefing/
  - id:        nats-subjects
    version:   v1
...
# analyst-briefing-audit — ANALYST-BRIEFING auditor

> Standalone trigger that runs `analyst-briefing_rubric@1.0` against one or
> more existing `analyst-briefing@1` markdowns and writes a sibling
> `analyst-briefing.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`analyst-briefing-author`](../analyst-briefing-author/SKILL.md).

`prompt_revision: analyst-briefing_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `ANALYST-BRIEFING`s."
- "Has `ANALYST-BRIEFING-007` changed since the last audit?"
- "Tell me which `ANALYST-BRIEFING`s would fail acceptance today."

Also invoked automatically by the supervisor when `analyst-briefing-author`'s output envelope sets `next_skill_recommendation: analyst-briefing-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        analyst-briefing-audit
skill_version:                   1.0.0
prompt_revision:                 analyst-briefing_audit@1.0.0
template_version:                analyst-briefing@1   (loaded from cyberos/skill/contracts/analyst-briefing/template.md)
audit_rubric_version:            analyst-briefing_rubric@1.0
...
```json
{
  "artefact_paths": ["./analyst-briefings/ANALYST-BRIEFING-001-foo.md", "./analyst-briefings/ANALYST-BRIEFING-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "analyst-briefing-author",
    "manifest_path": "./analyst-briefings/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./analyst-briefings/ANALYST-BRIEFING-001-foo.md", "audit_path": "./analyst-briefings/ANALYST-BRIEFING-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./analyst-briefings/ANALYST-BRIEFING-002-bar.md", "audit_path": "./analyst-briefings/ANALYST-BRIEFING-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   analyst-briefing-audit
Input:
  artefact_paths:  [./team-a/ANALYST-BRIEFING-001-something.md, ./team-b/ANALYST-BRIEFING-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
