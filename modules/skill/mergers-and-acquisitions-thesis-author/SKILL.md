---
# ── Identity ─────────────────────────────────────────────────────────
name: mergers-and-acquisitions-thesis-author
description: >-
  Author a per-target M&A thesis (target overview + strategic fit + valuation + integration plan + antitrust considerations + risks + recommendation). McKinsey + Bain M&A integration playbooks. CSO-Strategy + CFO co-authored. Use when user asks to "draft a mergers and acquisitions thesis" or "create the mergers and acquisitions thesis". Do NOT use for "audit existing mergers and acquisitions thesis" (use mergers-and-acquisitions-thesis-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: mna-thesis_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['mergers-and-acquisitions-thesis-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        mna-thesis
    ...
    pin_path:  cyberos/skill/contracts/mergers-and-acquisitions-thesis/
  - id:        nats-subjects
    version:   v1
...
# mergers-and-acquisitions-thesis-author — MNA-THESIS generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `mergers-and-acquisitions-thesis@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`mergers-and-acquisitions-thesis-audit`](../mergers-and-acquisitions-thesis-audit/SKILL.md) by default.

`prompt_revision: mna-thesis_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a MNA-THESIS."
- "Generate v2 of the MNA-THESIS from the updated source."
- "Draft a MNA-THESIS for <project>."

If the user asks to *audit an existing MNA-THESIS*, route to `mergers-and-acquisitions-thesis-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        mergers-and-acquisitions-thesis-author
skill_version:                   1.0.0
prompt_revision:                 mna-thesis_author@1.0.0
template_version:                mergers-and-acquisitions-thesis@1   (loaded from cyberos/skill/contracts/mergers-and-acquisitions-thesis/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  MNA-THESIS-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./mna-thesiss/",
  "manifest_path": "./mna-thesiss/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "MNA-THESIS-001", "path": "./mna-thesiss/MNA-THESIS-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "mergers-and-acquisitions-thesis-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/mergers-and-acquisitions-thesis/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `mna-thesis_author.mna-thesis_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `mergers-and-acquisitions-thesis-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `mergers-and-acquisitions-thesis-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   mergers-and-acquisitions-thesis-author
Input:
