...
name: decomm-audit
description: |
  Audit one or more existing decomm@1 markdowns against
...
# decomm-audit — DECOMM auditor

> Standalone trigger that runs `decomm_rubric@1.0` against one or
> more existing `decomm@1` markdowns and writes a sibling
...
> Chains naturally after [`decomm-author`](../decomm-author/SKILL.md).
...
Also invoked automatically by the supervisor when `decomm-author`'s output envelope sets `next_skill_recommendation: decomm-audit` (the default chain).
