---
# ── Identity ─────────────────────────────────────────────────────────
name: partner-program-author
description: >-
  Author the annual partner-program design (program purpose + tier structure + partner economics + enablement + co-marketing + governance + cadence). Crossbeam / Reveal + TSIA partner-program maturity model. CCO-Commercial authored. Use when user asks to "draft a partner program" or "create the partner program". Do NOT use for "audit existing partner program" (use partner-program-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: partner-program_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['partner-program-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        partner-program
    ...
    pin_path:  cyberos/skill/contracts/partner-program/
  - id:        nats-subjects
    version:   v1
...
# partner-program-author — PARTNER-PROGRAM generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `partner-program@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`partner-program-audit`](../partner-program-audit/SKILL.md) by default.

`prompt_revision: partner-program_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a PARTNER-PROGRAM."
- "Generate v2 of the PARTNER-PROGRAM from the updated source."
- "Draft a PARTNER-PROGRAM for <project>."

If the user asks to *audit an existing PARTNER-PROGRAM*, route to `partner-program-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        partner-program-author
skill_version:                   1.0.0
prompt_revision:                 partner-program_author@1.0.0
template_version:                partner-program@1   (loaded from cyberos/skill/contracts/partner-program/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  PARTNER-PROGRAM-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./partner-programs/",
  "manifest_path": "./partner-programs/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "PARTNER-PROGRAM-001", "path": "./partner-programs/PARTNER-PROGRAM-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "partner-program-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/partner-program/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `partner-program_author.partner-program_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `partner-program-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `partner-program-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   partner-program-author
Input:
