...
metadata:
  ...
  module: skill
  stage: cross
  cyberos-template: intellectual-property-strategy@1
...
# intellectual-property-strategy-audit — IP_STRATEGY auditor

> Standalone trigger that runs `ip-strategy_rubric@1.0` against one or
> more existing `intellectual-property-strategy@1` markdowns and writes a sibling
...
> Chains naturally after [`intellectual-property-strategy-author`](../intellectual-property-strategy-author/SKILL.md).
...
Also invoked automatically by the supervisor when `intellectual-property-strategy-author`'s output envelope sets `next_skill_recommendation: intellectual-property-strategy-audit` (the default chain).
...
```json
{
  ...
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "intellectual-property-strategy-author",
...
```
Persona: cuo-cpo
Skill:   intellectual-property-strategy-audit
Input:
