...
name: budget-audit
description: >-
  Audit one or more existing budget@1 markdowns against budget_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after budget-author. Use when user asks to "audit this budget" or "check the budget". Do NOT use for "draft a new budget" (use budget-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: budget@1
  cyberos-rubric-version: budget_rubric@1.0
...
# budget-audit — BUDGET auditor

> Standalone trigger that runs `budget_rubric@1.0` against one or
> more existing `budget@1` markdowns and writes a sibling
> `budget.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`budget-author`](../budget-author/SKILL.md).

`prompt_revision: budget_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `BUDGET`s."
- "Has `BUDGET-007` changed since the last audit?"
- "Tell me which `BUDGET`s would fail acceptance today."

Also invoked automatically by the supervisor when `budget-author`'s output envelope sets `next_skill_recommendation: budget-audit` (the default chain).
...
audit_rubric_version:            budget_rubric@1.0
...
```json
{
  "artefact_paths": ["./budgets/BUDGET-001-foo.md", "./budgets/BUDGET-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "budget-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./budgets/BUDGET-001-foo.md", "audit_path": "./budgets/BUDGET-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./budgets/BUDGET-002-bar.md", "audit_path": "./budgets/BUDGET-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
Skill:   budget-audit
Input:
  artefact_paths:  [./team-a/BUDGET-001-something.md, ./team-b/BUDGET-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
