...
This document describes how `fr-author` chains with upstream and downstream skills.
...
|---|---|---|
| `fr-audit` | Default after every `artefact_written` event | `next_skill_recommendation: fr-audit` in output envelope. |
| `<next-stage>-author` | After `fr-audit` returns PASS | Supervisor reads the audit's output envelope and queues the next stage. |
...
- `cyberos/skill/contracts/fr/` — the artefact template this skill generates.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/fr-audit/` — the sibling audit skill that validates this skill's output.
