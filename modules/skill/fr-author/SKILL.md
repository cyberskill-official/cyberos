...
description: |
  ...
  SDP §2(b) Requirements — backlog. Chains naturally into fr-audit.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['fr-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        fr
    ...
    pin_path:  cyberos/skill/contracts/fr/
  - id:        nats-subjects
    version:   v1
...
If the user asks to *audit an existing FR*, route to `fr-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 fr_author@1.0.0
template_version:                fr@1   (loaded from cyberos/skill/contracts/fr/template.md)
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "fr-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/fr/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `fr-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `fr-audit` (and other downstream consumers).
