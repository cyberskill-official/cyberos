# `press-release-audit` — pipeline

This document describes how `press-release-audit` chains with upstream and downstream skills.

## Upstream

| Upstream skill | Trigger | Hand-off |
|---|---|---|
| `press-release-author` (per `artefact_written` event) | Default chain | Passes the just-written artefact path + manifest path via input envelope's `upstream_context`. |
...
|---|---|---|
| `press-release-author` (RESUME phase) | When verdict is `needs_human` and operator replies; or when STALE-001 fires with REVERT_TO_MANIFEST | Sets `requires_regen: true` in output envelope; supervisor invokes author. |
...
|---|---|---|
| `press-release_audit.audit_written` | `{artefact_path, audit_path, audited_file_sha256, verdict}` | After every successful Step 8 WRITE. |
| `press-release_audit.audit_batch_complete` | `{batch_run_id, per_artefact, hitl_required}` | At the end of a batch. |
| `press-release_audit.hitl_pause` | `{artefact_path, blocking_issues}` | When the batch halts on HITL. |
...
- `cyberos/skill/contracts/press-release/` — the artefact template this skill audits.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/press-release-author/` — the sibling author skill whose output this skill validates.
