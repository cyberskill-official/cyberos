...
description: >-
  Audit one or more existing press-release@1 markdowns against press-release_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after press-release-author. Use when user asks to "audit this press release" or "check the press release". Do NOT use for "draft a new press release" (use press-release-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: press-release@1
  cyberos-rubric-version: press-release_rubric@1.0
...
# press-release-audit — PRESS-RELEASE auditor

> Standalone trigger that runs `press-release_rubric@1.0` against one or
> more existing `press-release@1` markdowns and writes a sibling
> `press-release.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`press-release-author`](../press-release-author/SKILL.md).

`prompt_revision: press-release_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `PRESS-RELEASE`s."
- "Has `PRESS-RELEASE-007` changed since the last audit?"
- "Tell me which `PRESS-RELEASE`s would fail acceptance today."

Also invoked automatically by the supervisor when `press-release-author`'s output envelope sets `next_skill_recommendation: press-release-audit` (the default chain).
...
```json
{
  "artefact_paths": ["./press-releases/PRESS-RELEASE-001-foo.md", "./press-releases/PRESS-RELEASE-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "press-release-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./press-releases/PRESS-RELEASE-001-foo.md", "audit_path": "./press-releases/PRESS-RELEASE-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./press-releases/PRESS-RELEASE-002-bar.md", "audit_path": "./press-releases/PRESS-RELEASE-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   press-release-audit
Input:
  artefact_paths:  [./team-a/PRESS-RELEASE-001-something.md, ./team-b/PRESS-RELEASE-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
