...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['knowledge-pipeline-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        knowledge-pipeline
    ...
    pin_path:  cyberos/skill/contracts/knowledge-pipeline/
  - id:        nats-subjects
    version:   v1
...
# knowledge-pipeline-author — KNOWLEDGE_PIPELINE generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `knowledge-pipeline@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`knowledge-pipeline-audit`](../knowledge-pipeline-audit/SKILL.md) by default.

`prompt_revision: knowledge-pipeline_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a KNOWLEDGE_PIPELINE."
- "Generate v2 of the KNOWLEDGE_PIPELINE from the updated source."
- "Draft a KNOWLEDGE_PIPELINE for <project>."

If the user asks to *audit an existing KNOWLEDGE_PIPELINE*, route to `knowledge-pipeline-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        knowledge-pipeline-author
skill_version:                   1.0.0
prompt_revision:                 knowledge-pipeline_author@1.0.0
template_version:                knowledge-pipeline@1   (loaded from cyberos/skill/contracts/knowledge-pipeline/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  KNOWLEDGE_PIPELINE-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./knowledge-pipelines/",
  "manifest_path": "./knowledge-pipelines/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "KNOWLEDGE_PIPELINE-001", "path": "./knowledge-pipelines/KNOWLEDGE_PIPELINE-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "knowledge-pipeline-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/knowledge-pipeline/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `knowledge-pipeline_author.knowledge-pipeline_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
- If chained to `knowledge-pipeline-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `knowledge-pipeline-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   knowledge-pipeline-author
Input:
