---
# ── Identity ─────────────────────────────────────────────────────────
name: privacy-impact-assessment-author
description: >-
  Author a per-feature Privacy Impact Assessment (feature description + data flow + data classes + legal basis + DPIA risk assessment + mitigations + DPO sign-off). Mandatory per GDPR Article 35 + Vietnam Decree 13/2023. CPO-Privacy authored. Use when user asks to "draft a privacy impact assessment" or "create the privacy impact assessment". Do NOT use for "audit existing privacy impact assessment" (use privacy-impact-assessment-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: pia_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['privacy-impact-assessment-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        pia
    ...
    pin_path:  cyberos/skill/contracts/privacy-impact-assessment/
  - id:        nats-subjects
    version:   v1
...
# privacy-impact-assessment-author — PIA generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `privacy-impact-assessment@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`privacy-impact-assessment-audit`](../privacy-impact-assessment-audit/SKILL.md) by default.

`prompt_revision: pia_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a PIA."
- "Generate v2 of the PIA from the updated source."
- "Draft a PIA for <project>."

If the user asks to *audit an existing PIA*, route to `privacy-impact-assessment-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 pia_author@1.0.0
template_version:                privacy-impact-assessment@1   (loaded from cyberos/skill/contracts/privacy-impact-assessment/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  PIA-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./pias/",
  "manifest_path": "./pias/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "PIA-001", "path": "./pias/PIA-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "privacy-impact-assessment-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/privacy-impact-assessment/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `pia_author.pia_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `privacy-impact-assessment-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `privacy-impact-assessment-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   privacy-impact-assessment-author
Input:
