...
metadata:
  ...
  module: skill
  stage: i
  cyberos-template: deployment-checklist@1
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['deployment-checklist-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        deploy-checklist
    ...
    pin_path:  cyberos/skill/contracts/deployment-checklist/
  - id:        nats-subjects
    version:   v1
...
# deployment-checklist-author — DEPLOY-CHECKLIST generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `deployment-checklist@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`deployment-checklist-audit`](../deployment-checklist-audit/SKILL.md) by default.
...
If the user asks to *audit an existing DEPLOY-CHECKLIST*, route to `deployment-checklist-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 deploy-checklist_author@1.0.0
template_version:                deployment-checklist@1   (loaded from cyberos/skill/contracts/deployment-checklist/template.md)
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "deployment-checklist-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/deployment-checklist/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `deployment-checklist-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `deployment-checklist-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   deployment-checklist-author
Input:
