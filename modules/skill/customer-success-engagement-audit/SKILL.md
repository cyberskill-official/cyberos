---
# ── Identity ─────────────────────────────────────────────────────────
name: customer-success-engagement-audit
description: >-
  Audit one or more existing customer-success-engagement@1 markdowns against cs-engagement_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after customer-success-engagement-author. Use when user asks to "audit this customer success engagement" or "check the customer success engagement". Do NOT use for "draft a new customer success engagement" (use customer-success-engagement-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: customer-success-engagement@1
  cyberos-rubric-version: cs-engagement_rubric@1.0
...
# customer-success-engagement-audit — CS-ENGAGEMENT auditor

> Standalone trigger that runs `cs-engagement_rubric@1.0` against one or
> more existing `customer-success-engagement@1` markdowns and writes a sibling
> `cs-engagement.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`customer-success-engagement-author`](../customer-success-engagement-author/SKILL.md).

`prompt_revision: cs-engagement_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `CS-ENGAGEMENT`s."
- "Has `CS-ENGAGEMENT-007` changed since the last audit?"
- "Tell me which `CS-ENGAGEMENT`s would fail acceptance today."

Also invoked automatically by the supervisor when `customer-success-engagement-author`'s output envelope sets `next_skill_recommendation: customer-success-engagement-audit` (the default chain).
...
audit_rubric_version:            cs-engagement_rubric@1.0
...
```json
{
  "artefact_paths": ["./cs-engagements/CS-ENGAGEMENT-001-foo.md", "./cs-engagements/CS-ENGAGEMENT-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "customer-success-engagement-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./cs-engagements/CS-ENGAGEMENT-001-foo.md", "audit_path": "./cs-engagements/CS-ENGAGEMENT-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./cs-engagements/CS-ENGAGEMENT-002-bar.md", "audit_path": "./cs-engagements/CS-ENGAGEMENT-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   customer-success-engagement-audit
Input:
  artefact_paths:  [./team-a/CS-ENGAGEMENT-001-something.md, ./team-b/CS-ENGAGEMENT-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
