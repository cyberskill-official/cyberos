# `feature-request-author` — pipeline

This document describes how `feature-request-author` chains with upstream and downstream skills.
...
|---|---|---|
| `feature-request-audit` | Default after every `artefact_written` event | `next_skill_recommendation: feature-request-audit` in output envelope. |
| `<next-stage>-author` | After `feature-request-audit` returns PASS | Supervisor reads the audit's output envelope and queues the next stage. |
...
- `cyberos/skill/contracts/feature-request/` — the artefact template this skill generates.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/feature-request-audit/` — the sibling audit skill that validates this skill's output.
