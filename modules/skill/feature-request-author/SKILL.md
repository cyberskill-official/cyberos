...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['feature-request-audit']; empty list disables chaining
...
# feature-request-author — FR generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `feature-request@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`feature-request-audit`](../feature-request-audit/SKILL.md) by default.
...
- "Turn this <input> into a FR."
- "Generate v2 of the FR from the updated source."
- "Draft a FR for <project>."

If the user asks to *audit an existing FR*, route to `feature-request-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 fr_author@1.0.0
template_version:                feature-request@1   (loaded from cyberos/skill/contracts/feature-request/template.md)
...
```json
{
  ...
  "artefacts_written": [
    {"id": "FR-001", "path": "./frs/FR-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "feature-request-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/feature-request/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `feature-request-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `feature-request-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   feature-request-author
Input:
