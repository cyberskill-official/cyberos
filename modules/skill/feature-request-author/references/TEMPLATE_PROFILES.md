# FR template profiles (FR-CUO-208) - normative, both templates side by side

<!-- verification preamble (executable, FR-SKILL-117 convention):
  grep -c '^## engineering-spec@1' THIS_FILE            -> 1
  grep -c '^## feature_request@1' THIS_FILE             -> 1
  grep -c 'rule families' THIS_FILE                     -> >= 2 (one list per profile)
  newest exemplars validate: any docs/feature-requests/**/FR-*.md with '## §1 - Description'
  parses per the engineering-spec grammar below; any with 'template: feature_request@1'
  parses per the feature_request grammar below.
-->

Resolution chain (who decides which template a NEW FR uses):

1. explicit per-invocation operator override, else
2. `.cyberos/config.yaml` `fr_template` (FR-CUO-207), else
3. default `engineering-spec@1`.

The resolved template MUST be echoed in /create-feature-requests' PLAN (value + source) so the
...
## Detection (audit side; from the file itself, FR-CUO-208 §1 #4)

- frontmatter `template: feature_request@1` present -> feature_request@1 (FM-004 binds it)
...
- Frontmatter: `id, title, module, priority (MUST|SHOULD|COULD), status (10-value enum), class
  (product|improvement), verify, phase, owner, created, shipped, memory_chain_hash, related_frs,
...
- Authoring rules: feature-request-author SKILL.md §12 (the single normative home; not duplicated here).
...
- Frontmatter: `template: feature_request@1` (FM-004) plus the FM-101..111 field set (title, author,
...
adding a new family requires touching both this file and RUBRIC.md (FR-CUO-208 §10 #3).
