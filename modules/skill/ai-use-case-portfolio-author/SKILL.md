---
# ── Identity ─────────────────────────────────────────────────────────
name: ai-use-case-portfolio-author
description: >-
  Author the quarterly AI use-case portfolio review (executive summary + use-case inventory by stage + EU AI Act risk-class distribution + production-deployment count + ROI tracking + kill-list candidates + pipeline coverage). CAIO authored. Use when user asks to "draft a ai use case portfolio" or "create the ai use case portfolio". Do NOT use for "audit existing ai use case portfolio" (use ai-use-case-portfolio-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: ai-use-case-portfolio_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['ai-use-case-portfolio-audit']; empty list disables chaining
...
# ai-use-case-portfolio-author — AI-USE-CASE-PORTFOLIO generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `ai-use-case-portfolio@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`ai-use-case-portfolio-audit`](../ai-use-case-portfolio-audit/SKILL.md) by default.

`prompt_revision: ai-use-case-portfolio_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a AI-USE-CASE-PORTFOLIO."
- "Generate v2 of the AI-USE-CASE-PORTFOLIO from the updated source."
- "Draft a AI-USE-CASE-PORTFOLIO for <project>."

If the user asks to *audit an existing AI-USE-CASE-PORTFOLIO*, route to `ai-use-case-portfolio-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        ai-use-case-portfolio-author
skill_version:                   1.0.0
prompt_revision:                 ai-use-case-portfolio_author@1.0.0
...
naming_pattern:                  AI-USE-CASE-PORTFOLIO-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./ai-use-case-portfolios/",
...
```json
{
  ...
  "artefacts_written": [
    {"id": "AI-USE-CASE-PORTFOLIO-001", "path": "./ai-use-case-portfolios/AI-USE-CASE-PORTFOLIO-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "ai-use-case-portfolio-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/ai-use-case-portfolio/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `ai-use-case-portfolio_author.ai-use-case-portfolio_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
- If chained to `ai-use-case-portfolio-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `ai-use-case-portfolio-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   ai-use-case-portfolio-author
Input:
