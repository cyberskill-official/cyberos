---
# ── Identity ─────────────────────────────────────────────────────────
name: internal-newsletter-author
description: >-
  Author the monthly internal newsletter (CEO note + wins + announcements + team spotlights + upcoming dates + prior-month Q&A + one-click reply). CCO-Communications + Chief of Staff co-authored. Use when user asks to "draft a internal newsletter" or "create the internal newsletter". Do NOT use for "audit existing internal newsletter" (use internal-newsletter-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: internal-newsletter_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['internal-newsletter-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        internal-newsletter
    ...
    pin_path:  cyberos/skill/contracts/internal-newsletter/
  - id:        nats-subjects
    version:   v1
...
# internal-newsletter-author — INTERNAL-NEWSLETTER generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `internal-newsletter@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`internal-newsletter-audit`](../internal-newsletter-audit/SKILL.md) by default.

`prompt_revision: internal-newsletter_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a INTERNAL-NEWSLETTER."
- "Generate v2 of the INTERNAL-NEWSLETTER from the updated source."
- "Draft a INTERNAL-NEWSLETTER for <project>."

If the user asks to *audit an existing INTERNAL-NEWSLETTER*, route to `internal-newsletter-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 internal-newsletter_author@1.0.0
template_version:                internal-newsletter@1   (loaded from cyberos/skill/contracts/internal-newsletter/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  INTERNAL-NEWSLETTER-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./internal-newsletters/",
  "manifest_path": "./internal-newsletters/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "INTERNAL-NEWSLETTER-001", "path": "./internal-newsletters/INTERNAL-NEWSLETTER-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "internal-newsletter-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/internal-newsletter/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `internal-newsletter_author.internal-newsletter_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `internal-newsletter-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `internal-newsletter-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   internal-newsletter-author
Input:
