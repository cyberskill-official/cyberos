...
metadata:
  ...
  module: skill
  stage: cross
  cyberos-template: customer-360@1
...
# customer-360-audit — CUSTOMER_360 auditor

> Standalone trigger that runs `customer-360_rubric@1.0` against one or
> more existing `customer-360@1` markdowns and writes a sibling
> `customer-360.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`customer-360-author`](../customer-360-author/SKILL.md).

`prompt_revision: customer-360_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `CUSTOMER_360`s."
- "Has `CUSTOMER_360-007` changed since the last audit?"
- "Tell me which `CUSTOMER_360`s would fail acceptance today."

Also invoked automatically by the supervisor when `customer-360-author`'s output envelope sets `next_skill_recommendation: customer-360-audit` (the default chain).
...
audit_rubric_version:            customer-360_rubric@1.0
...
```json
{
  "artefact_paths": ["./customer-360s/CUSTOMER_360-001-foo.md", "./customer-360s/CUSTOMER_360-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "customer-360-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./customer-360s/CUSTOMER_360-001-foo.md", "audit_path": "./customer-360s/CUSTOMER_360-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./customer-360s/CUSTOMER_360-002-bar.md", "audit_path": "./customer-360s/CUSTOMER_360-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   customer-360-audit
Input:
  artefact_paths:  [./team-a/CUSTOMER_360-001-something.md, ./team-b/CUSTOMER_360-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
