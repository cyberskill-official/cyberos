---
# ── Identity ─────────────────────────────────────────────────────────
name: innovation-portfolio-author
description: >-
  Author the quarterly innovation portfolio review (portfolio summary + ventures by horizon H1/H2/H3 + per-venture status + go/kill/scale recommendations + portfolio metrics + pipeline coverage). McKinsey Three Horizons + Govindarajan Ten Types. Chief Innovation Officer authored. Use when user asks to "draft a innovation portfolio" or "create the innovation portfolio". Do NOT use for "audit existing innovation portfolio" (use innovation-portfolio-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: innovation-portfolio_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['innovation-portfolio-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        innovation-portfolio
    ...
    pin_path:  cyberos/skill/contracts/innovation-portfolio/
  - id:        nats-subjects
    version:   v1
...
# innovation-portfolio-author — INNOVATION-PORTFOLIO generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `innovation-portfolio@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`innovation-portfolio-audit`](../innovation-portfolio-audit/SKILL.md) by default.

`prompt_revision: innovation-portfolio_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a INNOVATION-PORTFOLIO."
- "Generate v2 of the INNOVATION-PORTFOLIO from the updated source."
- "Draft a INNOVATION-PORTFOLIO for <project>."

If the user asks to *audit an existing INNOVATION-PORTFOLIO*, route to `innovation-portfolio-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 innovation-portfolio_author@1.0.0
template_version:                innovation-portfolio@1   (loaded from cyberos/skill/contracts/innovation-portfolio/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  INNOVATION-PORTFOLIO-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./innovation-portfolios/",
  "manifest_path": "./innovation-portfolios/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "INNOVATION-PORTFOLIO-001", "path": "./innovation-portfolios/INNOVATION-PORTFOLIO-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "innovation-portfolio-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/innovation-portfolio/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `innovation-portfolio_author.innovation-portfolio_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `innovation-portfolio-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `innovation-portfolio-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   innovation-portfolio-author
Input:
