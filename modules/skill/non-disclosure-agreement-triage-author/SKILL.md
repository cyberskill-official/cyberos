...
description: >-
  Rapidly triages an incoming NDA — classifies GREEN (standard, sign), YELLOW (counsel review), RED (full legal review). Screens for embedded non-compete, non-solicit, missing mutuality, missing carveouts, residuals clause issues. Per ACC NDA triage standard + WorldCC NDA best practices + ABA Model Mutual NDA. Author a NDA_TRIAGE markdown from source artefact(s). Generates a versioned non-disclosure-agreement-triage@1 file under output_dir, with per-claim authority markers and provenance to the source. Chains naturally into non-disclosure-agreement-triage-audit by default. Refuses to author when upstream artefact is in non-pass state. Use when user asks to "draft a non disclosure agreement triage" or "create the non disclosure agreement triage". Do NOT use for "audit existing non disclosure agreement triage" (use non-disclosure-agreement-triage-audit instead). Author a NDA_TRIAGE markdown from source artefact(s). Generates a versioned non-disclosure-agreement-triage@1 file under output_dir, with per-claim a...
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: nda-triage_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['non-disclosure-agreement-triage-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        nda-triage
    ...
    pin_path:  cyberos/skill/contracts/non-disclosure-agreement-triage/
  - id:        nats-subjects
    version:   v1
...
# non-disclosure-agreement-triage-author — NDA_TRIAGE generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `non-disclosure-agreement-triage@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`non-disclosure-agreement-triage-audit`](../non-disclosure-agreement-triage-audit/SKILL.md) by default.

`prompt_revision: nda-triage_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a NDA_TRIAGE."
- "Generate v2 of the NDA_TRIAGE from the updated source."
- "Draft a NDA_TRIAGE for <project>."

If the user asks to *audit an existing NDA_TRIAGE*, route to `non-disclosure-agreement-triage-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 nda-triage_author@1.0.0
template_version:                non-disclosure-agreement-triage@1   (loaded from cyberos/skill/contracts/non-disclosure-agreement-triage/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  NDA_TRIAGE-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./nda-triages/",
  "manifest_path": "./nda-triages/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "NDA_TRIAGE-001", "path": "./nda-triages/NDA_TRIAGE-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "non-disclosure-agreement-triage-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/non-disclosure-agreement-triage/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `nda-triage_author.nda-triage_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
- If chained to `non-disclosure-agreement-triage-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `non-disclosure-agreement-triage-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   non-disclosure-agreement-triage-author
Input:
