...
description: |
  Author a decommissioning / retirement package (decomm@1) for a
  system being retired. Captures: retirement decision + rationale,
  data-retention policy, data export plan, data-destruction
  certificate, DNS retirement plan, license cancellation list,
  source-archive manifest, customer-comms timeline. Covers Software
  Development Process.md §2(m). Chains naturally into decommissioning-audit.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: m
  cyberos-template: decommissioning@1
  cyberos-rubric-target: decomm_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['decommissioning-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        decomm
    ...
    pin_path:  cyberos/skill/contracts/decommissioning/
  - id:        nats-subjects
    version:   v1
...
# decommissioning-author — DECOMM generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `decommissioning@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`decommissioning-audit`](../decommissioning-audit/SKILL.md) by default.

`prompt_revision: decomm_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a DECOMM."
- "Generate v2 of the DECOMM from the updated source."
- "Draft a DECOMM for <project>."

If the user asks to *audit an existing DECOMM*, route to `decommissioning-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        decommissioning-author
skill_version:                   1.0.0
prompt_revision:                 decomm_author@1.0.0
template_version:                decommissioning@1   (loaded from cyberos/skill/contracts/decommissioning/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  DECOMM-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./decomms/",
  "manifest_path": "./decomms/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "DECOMM-001", "path": "./decomms/DECOMM-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "decommissioning-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/decommissioning/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `decomm_author.decomm_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `decommissioning-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `decommissioning-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   decommissioning-author
Input:
