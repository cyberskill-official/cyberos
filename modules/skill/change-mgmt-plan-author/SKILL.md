...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['change-mgmt-plan-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        change-mgmt-plan
    ...
    pin_path:  cyberos/skill/contracts/change-mgmt-plan/
  - id:        nats-subjects
    version:   v1
...
# change-mgmt-plan-author — CHANGE-MGMT-PLAN generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `change-mgmt-plan@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`change-mgmt-plan-audit`](../change-mgmt-plan-audit/SKILL.md) by default.
...
If the user asks to *audit an existing CHANGE-MGMT-PLAN*, route to `change-mgmt-plan-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        change-mgmt-plan-author
skill_version:                   1.0.0
prompt_revision:                 change-mgmt-plan_author@1.0.0
template_version:                change-mgmt-plan@1   (loaded from cyberos/skill/contracts/change-mgmt-plan/template.md)
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "change-mgmt-plan-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/change-mgmt-plan/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `change-mgmt-plan-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `change-mgmt-plan-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   change-mgmt-plan-author
Input:
