---
# ── Identity ─────────────────────────────────────────────────────────
name: comp-plan-audit
description: |
  Audit one or more existing comp-plan@1 markdowns against
...
# comp-plan-audit — COMP-PLAN auditor

> Standalone trigger that runs `comp-plan_rubric@1.0` against one or
> more existing `comp-plan@1` markdowns and writes a sibling
...
> Chains naturally after [`comp-plan-author`](../comp-plan-author/SKILL.md).
...
Also invoked automatically by the supervisor when `comp-plan-author`'s output envelope sets `next_skill_recommendation: comp-plan-audit` (the default chain).
