...
description: |
  ...
  Development Process.md §2(m). Chains naturally into decomm-audit.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: m
  cyberos-template: decomm@1
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['decomm-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        decomm
    ...
    pin_path:  cyberos/skill/contracts/decomm/
  - id:        nats-subjects
    version:   v1
...
# decomm-author — DECOMM generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `decomm@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`decomm-audit`](../decomm-audit/SKILL.md) by default.
...
If the user asks to *audit an existing DECOMM*, route to `decomm-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        decomm-author
skill_version:                   1.0.0
prompt_revision:                 decomm_author@1.0.0
template_version:                decomm@1   (loaded from cyberos/skill/contracts/decomm/template.md)
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "decomm-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/decomm/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `decomm-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `decomm-audit` (and other downstream consumers).
