# `decomm-author` — pipeline

This document describes how `decomm-author` chains with upstream and downstream skills.
...
|---|---|---|
| `decomm-audit` | Default after every `artefact_written` event | `next_skill_recommendation: decomm-audit` in output envelope. |
| `<next-stage>-author` | After `decomm-audit` returns PASS | Supervisor reads the audit's output envelope and queues the next stage. |
...
- `cyberos/skill/contracts/decomm/` — the artefact template this skill generates.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/decomm-audit/` — the sibling audit skill that validates this skill's output.
