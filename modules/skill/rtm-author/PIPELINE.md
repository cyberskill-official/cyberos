...
This document describes how `rtm-author` chains with upstream and downstream skills.
...
|---|---|---|
| `rtm-audit` | Default after every `artefact_written` event | `next_skill_recommendation: rtm-audit` in output envelope. |
| `<next-stage>-author` | After `rtm-audit` returns PASS | Supervisor reads the audit's output envelope and queues the next stage. |
...
- `cyberos/skill/contracts/rtm/` — the artefact template this skill generates.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/rtm-audit/` — the sibling audit skill that validates this skill's output.
