...
| `FM-117` | `module` | optional; if present, MUST be lowercase AND MUST equal the containing `docs/tasks/<module>/` folder name (TASK-IMP-139). List values are refused. Absent stays legal — the rule governs values, not presence. Outside the `docs/tasks/<module>/<task>/spec.md` shape (or under `_`/`.` trees), only the lowercase half is judged. Note: other templates reuse the id `FM-117` in their own per-template namespaces; within the task@1 family this is the next free id after FM-114. | error | false |
