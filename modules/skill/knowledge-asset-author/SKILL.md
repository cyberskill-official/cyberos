---
# ── Identity ─────────────────────────────────────────────────────────
name: knowledge-asset-author
description: >-
  Author a per-asset knowledge codification (summary + problem context + approach narrative + reusable artefact + adaptation guide + reuse metrics + contact author). Nonaka SECI + Wenger CoP aligned. Chief Knowledge Officer or asset author authored. Use when user asks to "draft a knowledge asset" or "create the knowledge asset". Do NOT use for "audit existing knowledge asset" (use knowledge-asset-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: knowledge-asset_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['knowledge-asset-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        knowledge-asset
    ...
    pin_path:  cyberos/skill/contracts/knowledge-asset/
  - id:        nats-subjects
    version:   v1
...
# knowledge-asset-author — KNOWLEDGE-ASSET generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `knowledge-asset@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`knowledge-asset-audit`](../knowledge-asset-audit/SKILL.md) by default.

`prompt_revision: knowledge-asset_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a KNOWLEDGE-ASSET."
- "Generate v2 of the KNOWLEDGE-ASSET from the updated source."
- "Draft a KNOWLEDGE-ASSET for <project>."

If the user asks to *audit an existing KNOWLEDGE-ASSET*, route to `knowledge-asset-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        knowledge-asset-author
skill_version:                   1.0.0
prompt_revision:                 knowledge-asset_author@1.0.0
template_version:                knowledge-asset@1   (loaded from cyberos/skill/contracts/knowledge-asset/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  KNOWLEDGE-ASSET-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./knowledge-assets/",
  "manifest_path": "./knowledge-assets/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "KNOWLEDGE-ASSET-001", "path": "./knowledge-assets/KNOWLEDGE-ASSET-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "knowledge-asset-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/knowledge-asset/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `knowledge-asset_author.knowledge-asset_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `knowledge-asset-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `knowledge-asset-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   knowledge-asset-author
Input:
