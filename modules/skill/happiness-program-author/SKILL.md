---
# ── Identity ─────────────────────────────────────────────────────────
name: happiness-program-author
description: >-
  Author the quarterly happiness program (culture rituals + perks + community engagement + survey results + action plan + budget). Officevibe/TINYpulse/Culture Amp survey patterns + Shawn Achor 'The Happiness Advantage'. Chief Happiness Officer authored. Use when user asks to "draft a happiness program" or "create the happiness program". Do NOT use for "audit existing happiness program" (use happiness-program-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: happiness-program_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['happiness-program-audit']; empty list disables chaining
...
# happiness-program-author — HAPPINESS-PROGRAM generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `happiness-program@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`happiness-program-audit`](../happiness-program-audit/SKILL.md) by default.

`prompt_revision: happiness-program_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a HAPPINESS-PROGRAM."
- "Generate v2 of the HAPPINESS-PROGRAM from the updated source."
- "Draft a HAPPINESS-PROGRAM for <project>."

If the user asks to *audit an existing HAPPINESS-PROGRAM*, route to `happiness-program-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  HAPPINESS-PROGRAM-{NNN}-{slug}.md
...
```json
{
  ...
  "artefacts_written": [
    {"id": "HAPPINESS-PROGRAM-001", "path": "./happiness-programs/HAPPINESS-PROGRAM-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "happiness-program-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/happiness-program/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `happiness-program_author.happiness-program_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
- If chained to `happiness-program-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `happiness-program-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   happiness-program-author
Input:
