---
# ── Identity ─────────────────────────────────────────────────────────
name: compensation-plan-author
description: >-
  Author an annual or refresh comp plan (philosophy + band structure + quota mechanics for sales + accelerators + SPIFFs + approval matrix). CHRO authored; CRO-Revenue for sales-specific. CEO + CFO sign-off. Use when user asks to "draft a compensation plan" or "create the compensation plan". Do NOT use for "audit existing compensation plan" (use compensation-plan-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: comp-plan_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['compensation-plan-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        comp-plan
    ...
    pin_path:  cyberos/skill/contracts/compensation-plan/
  - id:        nats-subjects
    version:   v1
...
# compensation-plan-author — COMP-PLAN generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `compensation-plan@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`compensation-plan-audit`](../compensation-plan-audit/SKILL.md) by default.

`prompt_revision: comp-plan_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a COMP-PLAN."
- "Generate v2 of the COMP-PLAN from the updated source."
- "Draft a COMP-PLAN for <project>."

If the user asks to *audit an existing COMP-PLAN*, route to `compensation-plan-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        compensation-plan-author
skill_version:                   1.0.0
prompt_revision:                 comp-plan_author@1.0.0
template_version:                compensation-plan@1   (loaded from cyberos/skill/contracts/compensation-plan/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  COMP-PLAN-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./comp-plans/",
  "manifest_path": "./comp-plans/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "COMP-PLAN-001", "path": "./comp-plans/COMP-PLAN-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "compensation-plan-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/compensation-plan/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `comp-plan_author.comp-plan_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
- If chained to `compensation-plan-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `compensation-plan-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   compensation-plan-author
Input:
