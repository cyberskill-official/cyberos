...
description: Rapidly triages an incoming NDA — classifies GREEN (standard, sign), YELLOW (counsel review), RED (full legal review). Screens for embedded non-compete, non-solicit, missing mutuality, missing carveouts, residuals clause issues. Per ACC NDA triage standard + WorldCC NDA best practices + ABA Model Mutual NDA.
  ...
  versioned nda-triage@1 file under output_dir, with per-claim authority
  markers and provenance to the source. Chains naturally into
  nda-triage-audit by default. Refuses to author when upstream artefact
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['nda-triage-audit']; empty list disables chaining
...
# nda-triage-author — NDA_TRIAGE generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `nda-triage@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`nda-triage-audit`](../nda-triage-audit/SKILL.md) by default.
...
If the user asks to *audit an existing NDA_TRIAGE*, route to `nda-triage-audit` instead. If both, the supervisor chains the two.
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "nda-triage-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/nda-triage/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `nda-triage-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `nda-triage-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   nda-triage-author
Input:
