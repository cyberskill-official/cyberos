...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['litigation-management-update-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        litigation-mgmt-update
    ...
    pin_path:  cyberos/skill/contracts/litigation-management-update/
  - id:        nats-subjects
    version:   v1
...
# litigation-management-update-author — LITIGATION_MGMT_UPDATE generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `litigation-management-update@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`litigation-management-update-audit`](../litigation-management-update-audit/SKILL.md) by default.
...
If the user asks to *audit an existing LITIGATION_MGMT_UPDATE*, route to `litigation-management-update-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        litigation-management-update-author
skill_version:                   1.0.0
prompt_revision:                 litigation-mgmt-update_author@1.0.0
template_version:                litigation-management-update@1   (loaded from cyberos/skill/contracts/litigation-management-update/template.md)
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "litigation-management-update-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/litigation-management-update/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `litigation-management-update-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `litigation-management-update-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   litigation-management-update-author
Input:
