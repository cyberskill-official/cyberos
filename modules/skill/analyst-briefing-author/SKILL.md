---
# ── Identity ─────────────────────────────────────────────────────────
name: analyst-briefing-author
description: >-
  Author a per-analyst-firm briefing deck (company overview + positioning + customer traction + product roadmap + competitive landscape + customer references + Q&A prep). CCO-Communications + CEO + CPO-Product co-authored. Use when user asks to "draft a analyst briefing" or "create the analyst briefing". Do NOT use for "audit existing analyst briefing" (use analyst-briefing-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: analyst-briefing_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['analyst-briefing-audit']; empty list disables chaining
...
# analyst-briefing-author — ANALYST-BRIEFING generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `analyst-briefing@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`analyst-briefing-audit`](../analyst-briefing-audit/SKILL.md) by default.

`prompt_revision: analyst-briefing_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a ANALYST-BRIEFING."
- "Generate v2 of the ANALYST-BRIEFING from the updated source."
- "Draft a ANALYST-BRIEFING for <project>."

If the user asks to *audit an existing ANALYST-BRIEFING*, route to `analyst-briefing-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  ANALYST-BRIEFING-{NNN}-{slug}.md
...
```json
{
  ...
  "artefacts_written": [
    {"id": "ANALYST-BRIEFING-001", "path": "./analyst-briefings/ANALYST-BRIEFING-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "analyst-briefing-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/analyst-briefing/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `analyst-briefing_author.analyst-briefing_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `analyst-briefing-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `analyst-briefing-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   analyst-briefing-author
Input:
