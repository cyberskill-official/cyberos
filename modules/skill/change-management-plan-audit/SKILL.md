---
# ── Identity ─────────────────────────────────────────────────────────
name: change-management-plan-audit
description: >-
  Audit one or more existing change-management-plan@1 markdowns against change-mgmt-plan_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after change-management-plan-author. Use when user asks to "audit this change management plan" or "check the change management plan". Do NOT use for "draft a new change management plan" (use change-management-plan-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: change-management-plan@1
  cyberos-rubric-version: change-mgmt-plan_rubric@1.0
...
# change-management-plan-audit — CHANGE-MGMT-PLAN auditor

> Standalone trigger that runs `change-mgmt-plan_rubric@1.0` against one or
> more existing `change-management-plan@1` markdowns and writes a sibling
> `change-mgmt-plan.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`change-management-plan-author`](../change-management-plan-author/SKILL.md).

`prompt_revision: change-mgmt-plan_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `CHANGE-MGMT-PLAN`s."
- "Has `CHANGE-MGMT-PLAN-007` changed since the last audit?"
- "Tell me which `CHANGE-MGMT-PLAN`s would fail acceptance today."

Also invoked automatically by the supervisor when `change-management-plan-author`'s output envelope sets `next_skill_recommendation: change-management-plan-audit` (the default chain).
...
audit_rubric_version:            change-mgmt-plan_rubric@1.0
...
```json
{
  "artefact_paths": ["./change-mgmt-plans/CHANGE-MGMT-PLAN-001-foo.md", "./change-mgmt-plans/CHANGE-MGMT-PLAN-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "change-management-plan-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./change-mgmt-plans/CHANGE-MGMT-PLAN-001-foo.md", "audit_path": "./change-mgmt-plans/CHANGE-MGMT-PLAN-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./change-mgmt-plans/CHANGE-MGMT-PLAN-002-bar.md", "audit_path": "./change-mgmt-plans/CHANGE-MGMT-PLAN-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   change-management-plan-audit
Input:
  artefact_paths:  [./team-a/CHANGE-MGMT-PLAN-001-something.md, ./team-b/CHANGE-MGMT-PLAN-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
