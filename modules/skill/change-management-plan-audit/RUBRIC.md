# `change_mgmt_plan_rubric@1.0` — machine-checkable Change Management Plan rubric
...
| `FM-004` | `template` equals `change-management-plan@1` | error | true |
...
Per the `contracts/change-management-plan/template.md` H2 list — every H2 in template MUST be present + non-empty in the authored artefact.

| rule_id | Check | Severity |
| ------- | ----- | -------- |
| `SEC-001` | Every H2 declared in `contracts/change-management-plan/template.md` is present in the authored artefact | error |
...
- `cyberos/skill/contracts/change-management-plan/CONTRACT.md` + `template.md`
