...
This document describes how `adr-author` chains with upstream and downstream skills.
...
|---|---|---|
| `adr-audit` | Default after every `artefact_written` event | `next_skill_recommendation: adr-audit` in output envelope. |
| `<next-stage>-author` | After `adr-audit` returns PASS | Supervisor reads the audit's output envelope and queues the next stage. |
...
- `cyberos/skill/contracts/adr/` — the artefact template this skill generates.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/adr-audit/` — the sibling audit skill that validates this skill's output.
