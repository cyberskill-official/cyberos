...
description: |
  ...
  into adr-audit, threat-model-author, and sdd-author.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: d
  cyberos-template: adr@1
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['adr-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        adr
    ...
    pin_path:  cyberos/skill/contracts/adr/
  - id:        nats-subjects
    version:   v1
...
# adr-author — ADR generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `adr@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`adr-audit`](../adr-audit/SKILL.md) by default.
...
If the user asks to *audit an existing ADR*, route to `adr-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 adr_author@1.0.0
template_version:                adr@1   (loaded from cyberos/skill/contracts/adr/template.md)
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "adr-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/adr/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `adr-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `adr-audit` (and other downstream consumers).
