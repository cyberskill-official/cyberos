# `decommissioning-audit` — pipeline

This document describes how `decommissioning-audit` chains with upstream and downstream skills.

## Upstream

| Upstream skill | Trigger | Hand-off |
|---|---|---|
| `decommissioning-author` (per `artefact_written` event) | Default chain | Passes the just-written artefact path + manifest path via input envelope's `upstream_context`. |
...
|---|---|---|
| `decommissioning-author` (RESUME phase) | When verdict is `needs_human` and operator replies; or when STALE-001 fires with REVERT_TO_MANIFEST | Sets `requires_regen: true` in output envelope; supervisor invokes author. |
...
|---|---|---|
| `decomm_audit.audit_written` | `{artefact_path, audit_path, audited_file_sha256, verdict}` | After every successful Step 8 WRITE. |
| `decomm_audit.audit_batch_complete` | `{batch_run_id, per_artefact, hitl_required}` | At the end of a batch. |
| `decomm_audit.hitl_pause` | `{artefact_path, blocking_issues}` | When the batch halts on HITL. |
...
- `cyberos/skill/contracts/decommissioning/` — the artefact template this skill audits.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/decommissioning-author/` — the sibling author skill whose output this skill validates.
