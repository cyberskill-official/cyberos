...
description: >-
  Audit one or more existing decommissioning@1 markdowns against decomm_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after decommissioning-author. Use when user asks to "audit this decommissioning" or "check the decommissioning". Do NOT use for "draft a new decommissioning" (use decommissioning-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-version: decomm_rubric@1.0
...
# decommissioning-audit — DECOMM auditor

> Standalone trigger that runs `decomm_rubric@1.0` against one or
> more existing `decommissioning@1` markdowns and writes a sibling
> `decomm.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`decommissioning-author`](../decommissioning-author/SKILL.md).

`prompt_revision: decomm_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `DECOMM`s."
- "Has `DECOMM-007` changed since the last audit?"
- "Tell me which `DECOMM`s would fail acceptance today."

Also invoked automatically by the supervisor when `decommissioning-author`'s output envelope sets `next_skill_recommendation: decommissioning-audit` (the default chain).
...
```json
{
  "artefact_paths": ["./decomms/DECOMM-001-foo.md", "./decomms/DECOMM-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "decommissioning-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./decomms/DECOMM-001-foo.md", "audit_path": "./decomms/DECOMM-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./decomms/DECOMM-002-bar.md", "audit_path": "./decomms/DECOMM-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   decommissioning-audit
Input:
  artefact_paths:  [./team-a/DECOMM-001-something.md, ./team-b/DECOMM-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
