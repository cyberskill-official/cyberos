...
description: >-
  Author a one-page stage-gate sign-off (stage-gate@1) per modules/cuo/docs/appendices.md (§13 Software Development Process) Template §4.3. Captures: stage name, entry criteria met (Y/N + evidence link), exit criteria met (Y/N + evidence link), risks/issues, decision (Go / Go-with-conditions / No-Go), signatures (EM, TL, Client Sponsor). Run at every stage boundary in fixed-price engagements; optional in T&M. Chains naturally into stage-gate-audit. Use when user asks to "draft a stage gate" or "create the stage gate". Do NOT use for "audit existing stage gate" (use stage-gate-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: c
  cyberos-template: stage-gate@1
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['stage-gate-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        stage-gate
    ...
    pin_path:  cyberos/skill/contracts/stage-gate/
  - id:        nats-subjects
    version:   v1
...
# stage-gate-author — STAGE-GATE generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `stage-gate@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`stage-gate-audit`](../stage-gate-audit/SKILL.md) by default.

`prompt_revision: stage-gate_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a STAGE-GATE."
- "Generate v2 of the STAGE-GATE from the updated source."
- "Draft a STAGE-GATE for <project>."

If the user asks to *audit an existing STAGE-GATE*, route to `stage-gate-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        stage-gate-author
skill_version:                   1.0.0
prompt_revision:                 stage-gate_author@1.0.0
template_version:                stage-gate@1   (loaded from cyberos/skill/contracts/stage-gate/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  STAGE-GATE-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./stage-gates/",
  "manifest_path": "./stage-gates/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "STAGE-GATE-001", "path": "./stage-gates/STAGE-GATE-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "stage-gate-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/stage-gate/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `stage-gate_author.stage-gate_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
- If chained to `stage-gate-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `stage-gate-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   stage-gate-author
Input:
