# `comp_plan_rubric@1.0` — machine-checkable Comp Plan rubric
...
| `FM-004` | `template` equals `compensation-plan@1` | error | true |
...
Per the `contracts/compensation-plan/template.md` H2 list. Each H2 in the template MUST be present + non-empty in the authored artefact. See template for the canonical headings.

| rule_id | Check | Severity |
| ------- | ----- | -------- |
| `SEC-001` | Every H2 declared in `contracts/compensation-plan/template.md` is present in the authored artefact | error |
...
- `cyberos/skill/contracts/compensation-plan/CONTRACT.md` + `template.md`
