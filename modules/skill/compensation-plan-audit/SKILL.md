---
# ── Identity ─────────────────────────────────────────────────────────
name: compensation-plan-audit
description: >-
  Audit one or more existing compensation-plan@1 markdowns against comp-plan_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after compensation-plan-author. Use when user asks to "audit this compensation plan" or "check the compensation plan". Do NOT use for "draft a new compensation plan" (use compensation-plan-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: compensation-plan@1
  cyberos-rubric-version: comp-plan_rubric@1.0
...
# compensation-plan-audit — COMP-PLAN auditor

> Standalone trigger that runs `comp-plan_rubric@1.0` against one or
> more existing `compensation-plan@1` markdowns and writes a sibling
> `comp-plan.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`compensation-plan-author`](../compensation-plan-author/SKILL.md).

`prompt_revision: comp-plan_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `COMP-PLAN`s."
- "Has `COMP-PLAN-007` changed since the last audit?"
- "Tell me which `COMP-PLAN`s would fail acceptance today."

Also invoked automatically by the supervisor when `compensation-plan-author`'s output envelope sets `next_skill_recommendation: compensation-plan-audit` (the default chain).
...
audit_rubric_version:            comp-plan_rubric@1.0
...
```json
{
  "artefact_paths": ["./comp-plans/COMP-PLAN-001-foo.md", "./comp-plans/COMP-PLAN-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "compensation-plan-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./comp-plans/COMP-PLAN-001-foo.md", "audit_path": "./comp-plans/COMP-PLAN-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./comp-plans/COMP-PLAN-002-bar.md", "audit_path": "./comp-plans/COMP-PLAN-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   compensation-plan-audit
Input:
  artefact_paths:  [./team-a/COMP-PLAN-001-something.md, ./team-b/COMP-PLAN-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
