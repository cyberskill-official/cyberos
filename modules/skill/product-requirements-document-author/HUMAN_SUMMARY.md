# `product-requirements-document-author` — human summary format
...
```
PRD batch <batch_run_id> complete

Wrote N PRD(s):
  - PRD-001: <slug> — PASS (audit verdict: pass, 0 open issues)
  - PRD-002: <slug> — HITL_PAUSE (2 blocking issues — see HITL_BATCH_REQUEST below)
  - PRD-003: <slug> — PASS (audit verdict: pass, 1 warning)

Manifest:    <output_dir>/manifest.json
Total time:  <seconds>s
Next:        product-requirements-document-audit will run automatically on PASS items. HITL items wait for your reply.
```
