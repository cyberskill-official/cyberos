...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['impl-plan-audit']; empty list disables chaining
...
# impl-plan-author — IMPL-PLAN generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `impl-plan@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`impl-plan-audit`](../impl-plan-audit/SKILL.md) by default.
...
If the user asks to *audit an existing IMPL-PLAN*, route to `impl-plan-audit` instead. If both, the supervisor chains the two.
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "impl-plan-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/impl-plan/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `impl-plan-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `impl-plan-audit` (and other downstream consumers).
