# `impl-plan-author` — pipeline

This document describes how `impl-plan-author` chains with upstream and downstream skills.
...
|---|---|---|
| `impl-plan-audit` | Default after every `artefact_written` event | `next_skill_recommendation: impl-plan-audit` in output envelope. |
| `<next-stage>-author` | After `impl-plan-audit` returns PASS | Supervisor reads the audit's output envelope and queues the next stage. |
...
- `cyberos/skill/contracts/impl-plan/` — the artefact template this skill generates.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/impl-plan-audit/` — the sibling audit skill that validates this skill's output.
