---
# ── Identity ─────────────────────────────────────────────────────────
name: cap-alloc-memo-audit
description: |
  Audit one or more existing cap-alloc-memo@1 markdowns against
  ...
  cap-alloc-memo-author.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: <SDP §2 stage letter or "cross">
  cyberos-template: cap-alloc-memo@1
...
# cap-alloc-memo-audit — CAP-ALLOC-MEMO auditor

> Standalone trigger that runs `cap-alloc-memo_rubric@1.0` against one or
> more existing `cap-alloc-memo@1` markdowns and writes a sibling
...
> Chains naturally after [`cap-alloc-memo-author`](../cap-alloc-memo-author/SKILL.md).
...
Also invoked automatically by the supervisor when `cap-alloc-memo-author`'s output envelope sets `next_skill_recommendation: cap-alloc-memo-audit` (the default chain).
...
```json
{
  ...
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "cap-alloc-memo-author",
