...
metadata:
  ...
  cyberos-rubric-version: runbook_rubric@1.0
...
# runbook-audit — RUNBOOK auditor

> Standalone trigger that runs `runbook_rubric@1.0` against one or
> more existing `runbook@1` markdowns and writes a sibling
> `runbook.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`runbook-author`](../runbook-author/SKILL.md).

`prompt_revision: runbook_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `RUNBOOK`s."
- "Has `RUNBOOK-007` changed since the last audit?"
- "Tell me which `RUNBOOK`s would fail acceptance today."

Also invoked automatically by the supervisor when `runbook-author`'s output envelope sets `next_skill_recommendation: runbook-audit` (the default chain).
...
audit_rubric_version:            runbook_rubric@1.0
...
```json
{
  "artefact_paths": ["./runbooks/RUNBOOK-001-foo.md", "./runbooks/RUNBOOK-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "runbook-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./runbooks/RUNBOOK-001-foo.md", "audit_path": "./runbooks/RUNBOOK-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./runbooks/RUNBOOK-002-bar.md", "audit_path": "./runbooks/RUNBOOK-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
Skill:   runbook-audit
Input:
  artefact_paths:  [./team-a/RUNBOOK-001-something.md, ./team-b/RUNBOOK-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
