...
4. Verify state — flavor-aware checks: FR DAG coherence + audit-score grep (cyberos) OR broken-link + orphan-ref check (generic)

Helper scripts:
- `find_fragments.py` — phase 1 scanner
- `propose_absorbs.py` — phase 2 merge-plan generator
- `coherence_check.py` — phase 4 cyberos FR DAG checker
