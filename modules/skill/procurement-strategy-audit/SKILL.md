---
# ── Identity ─────────────────────────────────────────────────────────
name: procurement-strategy-audit
description: >-
  Audit one or more existing procurement-strategy@1 markdowns against procurement-strategy_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after procurement-strategy-author. Use when user asks to "audit this procurement strategy" or "check the procurement strategy". Do NOT use for "draft a new procurement strategy" (use procurement-strategy-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: procurement-strategy@1
  cyberos-rubric-version: procurement-strategy_rubric@1.0
...
depends_on_contracts:
  - id:        procurement-strategy
    ...
    pin_path:  cyberos/skill/contracts/procurement-strategy/
  - id:        nats-subjects
    version:   v1
...
# procurement-strategy-audit — PROCUREMENT-STRATEGY auditor

> Standalone trigger that runs `procurement-strategy_rubric@1.0` against one or
> more existing `procurement-strategy@1` markdowns and writes a sibling
> `procurement-strategy.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`procurement-strategy-author`](../procurement-strategy-author/SKILL.md).

`prompt_revision: procurement-strategy_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `PROCUREMENT-STRATEGY`s."
- "Has `PROCUREMENT-STRATEGY-007` changed since the last audit?"
- "Tell me which `PROCUREMENT-STRATEGY`s would fail acceptance today."

Also invoked automatically by the supervisor when `procurement-strategy-author`'s output envelope sets `next_skill_recommendation: procurement-strategy-audit` (the default chain).
...
skill_version:                   1.0.0
prompt_revision:                 procurement-strategy_audit@1.0.0
template_version:                procurement-strategy@1   (loaded from cyberos/skill/contracts/procurement-strategy/template.md)
audit_rubric_version:            procurement-strategy_rubric@1.0
...
```json
{
  "artefact_paths": ["./procurement-strategys/PROCUREMENT-STRATEGY-001-foo.md", "./procurement-strategys/PROCUREMENT-STRATEGY-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "procurement-strategy-author",
    "manifest_path": "./procurement-strategys/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./procurement-strategys/PROCUREMENT-STRATEGY-001-foo.md", "audit_path": "./procurement-strategys/PROCUREMENT-STRATEGY-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./procurement-strategys/PROCUREMENT-STRATEGY-002-bar.md", "audit_path": "./procurement-strategys/PROCUREMENT-STRATEGY-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   procurement-strategy-audit
Input:
  artefact_paths:  [./team-a/PROCUREMENT-STRATEGY-001-something.md, ./team-b/PROCUREMENT-STRATEGY-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
