---
# ── Identity ─────────────────────────────────────────────────────────
name: campaign-plan-author
description: >-
  Author a per-campaign plan (objective + audience + channel mix + creative concept + launch + sustain calendar + measurement + kill criteria). CMO authored. Use when user asks to "draft a campaign plan" or "create the campaign plan". Do NOT use for "audit existing campaign plan" (use campaign-plan-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: campaign-plan_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['campaign-plan-audit']; empty list disables chaining
...
# campaign-plan-author — CAMPAIGN-PLAN generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `campaign-plan@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`campaign-plan-audit`](../campaign-plan-audit/SKILL.md) by default.

`prompt_revision: campaign-plan_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a CAMPAIGN-PLAN."
- "Generate v2 of the CAMPAIGN-PLAN from the updated source."
- "Draft a CAMPAIGN-PLAN for <project>."

If the user asks to *audit an existing CAMPAIGN-PLAN*, route to `campaign-plan-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  CAMPAIGN-PLAN-{NNN}-{slug}.md
...
```json
{
  ...
  "artefacts_written": [
    {"id": "CAMPAIGN-PLAN-001", "path": "./campaign-plans/CAMPAIGN-PLAN-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "campaign-plan-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/campaign-plan/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `campaign-plan_author.campaign-plan_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
- If chained to `campaign-plan-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `campaign-plan-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   campaign-plan-author
Input:
