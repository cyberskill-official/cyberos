---
# ── Identity ─────────────────────────────────────────────────────────
name: rhythm-of-business-author
description: >-
  Author the annual + quarterly rhythm-of-business calendar (annual cadence map + quarterly drumbeat + weekly rhythm + per-function review + blackout windows). Chief of Staff authored. Use when user asks to "draft a rhythm of business" or "create the rhythm of business". Do NOT use for "audit existing rhythm of business" (use rhythm-of-business-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: rhythm-of-business_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['rhythm-of-business-audit']; empty list disables chaining
...
# rhythm-of-business-author — RHYTHM-OF-BUSINESS generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `rhythm-of-business@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`rhythm-of-business-audit`](../rhythm-of-business-audit/SKILL.md) by default.

`prompt_revision: rhythm-of-business_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a RHYTHM-OF-BUSINESS."
- "Generate v2 of the RHYTHM-OF-BUSINESS from the updated source."
- "Draft a RHYTHM-OF-BUSINESS for <project>."

If the user asks to *audit an existing RHYTHM-OF-BUSINESS*, route to `rhythm-of-business-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  RHYTHM-OF-BUSINESS-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./rhythm-of-businesss/",
...
```json
{
  ...
  "artefacts_written": [
    {"id": "RHYTHM-OF-BUSINESS-001", "path": "./rhythm-of-businesss/RHYTHM-OF-BUSINESS-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "rhythm-of-business-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/rhythm-of-business/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `rhythm-of-business_author.rhythm-of-business_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `rhythm-of-business-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `rhythm-of-business-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   rhythm-of-business-author
Input:
