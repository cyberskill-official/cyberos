---
# ── Identity ─────────────────────────────────────────────────────────
name: procurement-strategy-author
description: >-
  Author the annual procurement strategy (category strategy + supplier consolidation thesis + sustainable procurement targets + risk management + digital procurement initiatives + savings target). CIPS standards + Kraljic matrix. CPO-Procurement authored. Use when user asks to "draft a procurement strategy" or "create the procurement strategy". Do NOT use for "audit existing procurement strategy" (use procurement-strategy-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: procurement-strategy_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['procurement-strategy-audit']; empty list disables chaining
...
# procurement-strategy-author — PROCUREMENT-STRATEGY generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `procurement-strategy@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`procurement-strategy-audit`](../procurement-strategy-audit/SKILL.md) by default.

`prompt_revision: procurement-strategy_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a PROCUREMENT-STRATEGY."
- "Generate v2 of the PROCUREMENT-STRATEGY from the updated source."
- "Draft a PROCUREMENT-STRATEGY for <project>."

If the user asks to *audit an existing PROCUREMENT-STRATEGY*, route to `procurement-strategy-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  PROCUREMENT-STRATEGY-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./procurement-strategys/",
...
```json
{
  ...
  "artefacts_written": [
    {"id": "PROCUREMENT-STRATEGY-001", "path": "./procurement-strategys/PROCUREMENT-STRATEGY-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "procurement-strategy-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/procurement-strategy/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `procurement-strategy_author.procurement-strategy_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `procurement-strategy-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `procurement-strategy-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   procurement-strategy-author
Input:
