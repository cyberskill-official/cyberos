...
description: |
  ...
  feeds stages d/e/f). Chains naturally into prd-audit.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: prd@1
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['prd-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        prd
    ...
    pin_path:  cyberos/skill/contracts/prd/
  - id:        nats-subjects
    version:   v1
...
# prd-author — PRD generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `prd@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`prd-audit`](../prd-audit/SKILL.md) by default.
...
- "Turn this <input> into a PRD."
- "Generate v2 of the PRD from the updated source."
- "Draft a PRD for <project>."

If the user asks to *audit an existing PRD*, route to `prd-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 prd_author@1.0.0
template_version:                prd@1   (loaded from cyberos/skill/contracts/prd/template.md)
...
```json
{
  ...
  "artefacts_written": [
    {"id": "PRD-001", "path": "./prds/PRD-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "prd-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/prd/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `prd_author.prd_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `prd-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `prd-audit` (and other downstream consumers).
