...
This document describes how `prd-author` chains with upstream and downstream skills.
...
|---|---|---|
| `prd-audit` | Default after every `artefact_written` event | `next_skill_recommendation: prd-audit` in output envelope. |
| `<next-stage>-author` | After `prd-audit` returns PASS | Supervisor reads the audit's output envelope and queues the next stage. |
...
- `cyberos/skill/contracts/prd/` — the artefact template this skill generates.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/prd-audit/` — the sibling audit skill that validates this skill's output.
