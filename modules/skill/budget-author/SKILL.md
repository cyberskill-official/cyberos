...
name: budget-author
description: >-
  Author the annual budget (revenue + opex + capex + hiring + per-function envelope + scenario sensitivities). CFO authored; CEO + board approved. Use when user asks to "draft a budget" or "create the budget". Do NOT use for "audit existing budget" (use budget-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: budget_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['budget-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        budget
    ...
    pin_path:  cyberos/skill/contracts/budget/
  - id:        nats-subjects
    version:   v1
...
# budget-author — BUDGET generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `budget@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`budget-audit`](../budget-audit/SKILL.md) by default.

`prompt_revision: budget_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a BUDGET."
- "Generate v2 of the BUDGET from the updated source."
- "Draft a BUDGET for <project>."

If the user asks to *audit an existing BUDGET*, route to `budget-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        budget-author
skill_version:                   1.0.0
prompt_revision:                 budget_author@1.0.0
template_version:                budget@1   (loaded from cyberos/skill/contracts/budget/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  BUDGET-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./budgets/",
  "manifest_path": "./budgets/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "BUDGET-001", "path": "./budgets/BUDGET-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "budget-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/budget/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `budget_author.budget_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `budget-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `budget-audit` (and other downstream consumers).
