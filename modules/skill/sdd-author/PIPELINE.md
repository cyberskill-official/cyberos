...
This document describes how `sdd-author` chains with upstream and downstream skills.
...
|---|---|---|
| `sdd-audit` | Default after every `artefact_written` event | `next_skill_recommendation: sdd-audit` in output envelope. |
| `<next-stage>-author` | After `sdd-audit` returns PASS | Supervisor reads the audit's output envelope and queues the next stage. |
...
- `cyberos/skill/contracts/sdd/` — the artefact template this skill generates.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/sdd-audit/` — the sibling audit skill that validates this skill's output.
