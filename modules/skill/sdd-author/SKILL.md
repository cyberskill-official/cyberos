...
description: |
  ...
  sdd-audit and impl-plan-author.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: e
  cyberos-template: sdd@1
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['sdd-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        sdd
    ...
    pin_path:  cyberos/skill/contracts/sdd/
  - id:        nats-subjects
    version:   v1
...
# sdd-author — SDD generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `sdd@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`sdd-audit`](../sdd-audit/SKILL.md) by default.
...
If the user asks to *audit an existing SDD*, route to `sdd-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 sdd_author@1.0.0
template_version:                sdd@1   (loaded from cyberos/skill/contracts/sdd/template.md)
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "sdd-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/sdd/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `sdd-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `sdd-audit` (and other downstream consumers).
