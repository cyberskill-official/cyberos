---
# ── Identity ─────────────────────────────────────────────────────────
name: campaign-plan-audit
description: >-
  Audit one or more existing campaign-plan@1 markdowns against campaign-plan_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after campaign-plan-author. Use when user asks to "audit this campaign plan" or "check the campaign plan". Do NOT use for "draft a new campaign plan" (use campaign-plan-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: campaign-plan@1
  cyberos-rubric-version: campaign-plan_rubric@1.0
...
depends_on_contracts:
  - id:        campaign-plan
    ...
    pin_path:  cyberos/skill/contracts/campaign-plan/
  - id:        nats-subjects
    version:   v1
...
# campaign-plan-audit — CAMPAIGN-PLAN auditor

> Standalone trigger that runs `campaign-plan_rubric@1.0` against one or
> more existing `campaign-plan@1` markdowns and writes a sibling
> `campaign-plan.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`campaign-plan-author`](../campaign-plan-author/SKILL.md).

`prompt_revision: campaign-plan_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `CAMPAIGN-PLAN`s."
- "Has `CAMPAIGN-PLAN-007` changed since the last audit?"
- "Tell me which `CAMPAIGN-PLAN`s would fail acceptance today."

Also invoked automatically by the supervisor when `campaign-plan-author`'s output envelope sets `next_skill_recommendation: campaign-plan-audit` (the default chain).
...
skill_version:                   1.0.0
prompt_revision:                 campaign-plan_audit@1.0.0
template_version:                campaign-plan@1   (loaded from cyberos/skill/contracts/campaign-plan/template.md)
audit_rubric_version:            campaign-plan_rubric@1.0
...
```json
{
  "artefact_paths": ["./campaign-plans/CAMPAIGN-PLAN-001-foo.md", "./campaign-plans/CAMPAIGN-PLAN-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "campaign-plan-author",
    "manifest_path": "./campaign-plans/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./campaign-plans/CAMPAIGN-PLAN-001-foo.md", "audit_path": "./campaign-plans/CAMPAIGN-PLAN-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./campaign-plans/CAMPAIGN-PLAN-002-bar.md", "audit_path": "./campaign-plans/CAMPAIGN-PLAN-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   campaign-plan-audit
Input:
  artefact_paths:  [./team-a/CAMPAIGN-PLAN-001-something.md, ./team-b/CAMPAIGN-PLAN-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
