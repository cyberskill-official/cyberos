...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['postmortem-audit']; empty list disables chaining
...
# postmortem-author — POSTMORTEM generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `postmortem@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`postmortem-audit`](../postmortem-audit/SKILL.md) by default.

`prompt_revision: postmortem_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a POSTMORTEM."
- "Generate v2 of the POSTMORTEM from the updated source."
- "Draft a POSTMORTEM for <project>."

If the user asks to *audit an existing POSTMORTEM*, route to `postmortem-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  POSTMORTEM-{NNN}-{slug}.md
...
```json
{
  ...
  "artefacts_written": [
    {"id": "POSTMORTEM-001", "path": "./postmortems/POSTMORTEM-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "postmortem-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/postmortem/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `postmortem_author.postmortem_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `postmortem-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `postmortem-audit` (and other downstream consumers).
