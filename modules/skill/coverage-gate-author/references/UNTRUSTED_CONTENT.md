# Untrusted-content discipline — coverage-gate-author
...
`coverage-gate-author` captures tool output verbatim — untrusted by construction:

- **Raw terminal output of the test + coverage runs** — tool output embeds repo-controlled strings (test names, file paths, println/log text an attacker can author), captured UNTRUNCATED into `raw_terminal`.
- **`git diff` output** naming the touched-files set — repo-controlled paths.
- **The `task@1` and `edge-case-matrix@1`** inputs — model-authored artefacts.

Where the wrap happens: terminal captures are wrapped as they are embedded into the artefact's `raw_terminal` block; the PASS/FAIL arithmetic (tests_failed, files_below_90pct, ecm_rows_uncovered) comes from the tool's structured results, never from prose inside the captured text — a test that PRINTS "coverage gate passed" changes nothing.
