---
# ── Identity ─────────────────────────────────────────────────────────
name: sustainability-report-author
description: >-
  Author the annual sustainability report (executive summary + governance + strategy + risk-opportunity + Scope 1/2/3 metrics + targets + progress + assurance statement). CSRD/ESRS or ISSB IFRS S1/S2 framework. CSO-Sustainability or Chief ESG Officer authored. Use when user asks to "draft a sustainability report" or "create the sustainability report". Do NOT use for "audit existing sustainability report" (use sustainability-report-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['sustainability-report-audit']; empty list disables chaining
...
# sustainability-report-author — SUSTAINABILITY-REPORT generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `sustainability-report@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`sustainability-report-audit`](../sustainability-report-audit/SKILL.md) by default.

`prompt_revision: sustainability-report_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a SUSTAINABILITY-REPORT."
- "Generate v2 of the SUSTAINABILITY-REPORT from the updated source."
- "Draft a SUSTAINABILITY-REPORT for <project>."

If the user asks to *audit an existing SUSTAINABILITY-REPORT*, route to `sustainability-report-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        sustainability-report-author
skill_version:                   1.0.0
prompt_revision:                 sustainability-report_author@1.0.0
...
naming_pattern:                  SUSTAINABILITY-REPORT-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./sustainability-reports/",
...
```json
{
  ...
  "artefacts_written": [
    {"id": "SUSTAINABILITY-REPORT-001", "path": "./sustainability-reports/SUSTAINABILITY-REPORT-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "sustainability-report-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/sustainability-report/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `sustainability-report_author.sustainability-report_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `sustainability-report-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `sustainability-report-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   sustainability-report-author
Input:
