# `nps_program_rubric@1.0` — machine-checkable NPS Program rubric
...
| `FM-004` | `template` equals `net-promoter-score-program@1` | error | true |
...
Per the `contracts/net-promoter-score-program/template.md` H2 list. Each H2 in the template MUST be present + non-empty in the authored artefact. See template for the canonical headings.

| rule_id | Check | Severity |
| ------- | ----- | -------- |
| `SEC-001` | Every H2 declared in `contracts/net-promoter-score-program/template.md` is present in the authored artefact | error |
...
- `cyberos/skill/contracts/net-promoter-score-program/CONTRACT.md` + `template.md`
