---
# ── Identity ─────────────────────────────────────────────────────────
name: net-promoter-score-program-audit
description: >-
  Audit one or more existing net-promoter-score-program@1 markdowns against nps-program_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after net-promoter-score-program-author. Use when user asks to "audit this net promoter score program" or "check the net promoter score program". Do NOT use for "draft a new net promoter score program" (use net-promoter-score-program-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: net-promoter-score-program@1
  cyberos-rubric-version: nps-program_rubric@1.0
...
# net-promoter-score-program-audit — NPS-PROGRAM auditor

> Standalone trigger that runs `nps-program_rubric@1.0` against one or
> more existing `net-promoter-score-program@1` markdowns and writes a sibling
> `nps-program.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`net-promoter-score-program-author`](../net-promoter-score-program-author/SKILL.md).

`prompt_revision: nps-program_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `NPS-PROGRAM`s."
- "Has `NPS-PROGRAM-007` changed since the last audit?"
- "Tell me which `NPS-PROGRAM`s would fail acceptance today."

Also invoked automatically by the supervisor when `net-promoter-score-program-author`'s output envelope sets `next_skill_recommendation: net-promoter-score-program-audit` (the default chain).
...
audit_rubric_version:            nps-program_rubric@1.0
...
```json
{
  "artefact_paths": ["./nps-programs/NPS-PROGRAM-001-foo.md", "./nps-programs/NPS-PROGRAM-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "net-promoter-score-program-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./nps-programs/NPS-PROGRAM-001-foo.md", "audit_path": "./nps-programs/NPS-PROGRAM-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./nps-programs/NPS-PROGRAM-002-bar.md", "audit_path": "./nps-programs/NPS-PROGRAM-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   net-promoter-score-program-audit
Input:
  artefact_paths:  [./team-a/NPS-PROGRAM-001-something.md, ./team-b/NPS-PROGRAM-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
