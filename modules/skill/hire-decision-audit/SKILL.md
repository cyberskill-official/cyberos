---
# ── Identity ─────────────────────────────────────────────────────────
name: hire-decision-audit
description: >-
  Audit one or more existing hire-decision@1 markdowns against hire-decision_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after hire-decision-author. Use when user asks to "audit this hire decision" or "check the hire decision". Do NOT use for "draft a new hire decision" (use hire-decision-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: hire-decision@1
  cyberos-rubric-version: hire-decision_rubric@1.0
...
# hire-decision-audit — HIRE-DECISION auditor

> Standalone trigger that runs `hire-decision_rubric@1.0` against one or
> more existing `hire-decision@1` markdowns and writes a sibling
> `hire-decision.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`hire-decision-author`](../hire-decision-author/SKILL.md).

`prompt_revision: hire-decision_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `HIRE-DECISION`s."
- "Has `HIRE-DECISION-007` changed since the last audit?"
- "Tell me which `HIRE-DECISION`s would fail acceptance today."

Also invoked automatically by the supervisor when `hire-decision-author`'s output envelope sets `next_skill_recommendation: hire-decision-audit` (the default chain).
...
audit_rubric_version:            hire-decision_rubric@1.0
...
```json
{
  "artefact_paths": ["./hire-decisions/HIRE-DECISION-001-foo.md", "./hire-decisions/HIRE-DECISION-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "hire-decision-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./hire-decisions/HIRE-DECISION-001-foo.md", "audit_path": "./hire-decisions/HIRE-DECISION-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./hire-decisions/HIRE-DECISION-002-bar.md", "audit_path": "./hire-decisions/HIRE-DECISION-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   hire-decision-audit
Input:
  artefact_paths:  [./team-a/HIRE-DECISION-001-something.md, ./team-b/HIRE-DECISION-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
