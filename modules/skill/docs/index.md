...
`cyberos-init` vendors the author/audit skills into every initialised project under `.cyberos/cuo/skills/`, and the Claude plugin (payload marketplace) exposes `/init`, `/update`, `/changelog`, `/help`, and the `ship-feature-requests` skill. The same skill bodies drive the workflow everywhere - there is no repo-specific fork.
