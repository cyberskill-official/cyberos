---
# ── Identity ─────────────────────────────────────────────────────────
name: software-design-document-audit
description: >-
  Audit one or more existing software-design-document@1 markdowns against sdd_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after software-design-document-author. Use when user asks to "audit this software design document" or "check the software design document". Do NOT use for "draft a new software design document" (use software-design-document-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: e
  cyberos-template: software-design-document@1
  cyberos-rubric-version: sdd_rubric@1.0
...
depends_on_contracts:
  - id:        sdd
    ...
    pin_path:  cyberos/skill/contracts/software-design-document/
  - id:        nats-subjects
    version:   v1
...
# software-design-document-audit — SDD auditor

> Standalone trigger that runs `sdd_rubric@1.0` against one or
> more existing `software-design-document@1` markdowns and writes a sibling
> `sdd.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`software-design-document-author`](../software-design-document-author/SKILL.md).

`prompt_revision: sdd_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `SDD`s."
- "Has `SDD-007` changed since the last audit?"
- "Tell me which `SDD`s would fail acceptance today."

Also invoked automatically by the supervisor when `software-design-document-author`'s output envelope sets `next_skill_recommendation: software-design-document-audit` (the default chain).
...
skill_version:                   1.0.0
prompt_revision:                 sdd_audit@1.0.0
template_version:                software-design-document@1   (loaded from cyberos/skill/contracts/software-design-document/template.md)
audit_rubric_version:            sdd_rubric@1.0
...
```json
{
  "artefact_paths": ["./sdds/SDD-001-foo.md", "./sdds/SDD-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "software-design-document-author",
    "manifest_path": "./sdds/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./sdds/SDD-001-foo.md", "audit_path": "./sdds/SDD-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./sdds/SDD-002-bar.md", "audit_path": "./sdds/SDD-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   software-design-document-audit
Input:
  artefact_paths:  [./team-a/SDD-001-something.md, ./team-b/SDD-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
