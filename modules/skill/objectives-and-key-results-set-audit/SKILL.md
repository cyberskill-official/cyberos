---
# ── Identity ─────────────────────────────────────────────────────────
name: objectives-and-key-results-set-audit
description: >-
  Audit one or more existing objectives-and-key-results-set@1 markdowns against okr-set_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after objectives-and-key-results-set-author. Use when user asks to "audit this objectives and key results set" or "check the objectives and key results set". Do NOT use for "draft a new objectives and key results set" (use objectives-and-key-results-set-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: objectives-and-key-results-set@1
  cyberos-rubric-version: okr-set_rubric@1.0
...
depends_on_contracts:
  - id:        okr-set
    ...
    pin_path:  cyberos/skill/contracts/objectives-and-key-results-set/
  - id:        nats-subjects
    version:   v1
...
# objectives-and-key-results-set-audit — OKR-SET auditor

> Standalone trigger that runs `okr-set_rubric@1.0` against one or
> more existing `objectives-and-key-results-set@1` markdowns and writes a sibling
> `okr-set.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`objectives-and-key-results-set-author`](../objectives-and-key-results-set-author/SKILL.md).

`prompt_revision: okr-set_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `OKR-SET`s."
- "Has `OKR-SET-007` changed since the last audit?"
- "Tell me which `OKR-SET`s would fail acceptance today."

Also invoked automatically by the supervisor when `objectives-and-key-results-set-author`'s output envelope sets `next_skill_recommendation: objectives-and-key-results-set-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        objectives-and-key-results-set-audit
skill_version:                   1.0.0
prompt_revision:                 okr-set_audit@1.0.0
template_version:                objectives-and-key-results-set@1   (loaded from cyberos/skill/contracts/objectives-and-key-results-set/template.md)
audit_rubric_version:            okr-set_rubric@1.0
...
```json
{
  "artefact_paths": ["./okr-sets/OKR-SET-001-foo.md", "./okr-sets/OKR-SET-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "objectives-and-key-results-set-author",
    "manifest_path": "./okr-sets/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./okr-sets/OKR-SET-001-foo.md", "audit_path": "./okr-sets/OKR-SET-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./okr-sets/OKR-SET-002-bar.md", "audit_path": "./okr-sets/OKR-SET-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   objectives-and-key-results-set-audit
Input:
  artefact_paths:  [./team-a/OKR-SET-001-something.md, ./team-b/OKR-SET-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
