# `okr_set_rubric@1.0` — machine-checkable OKR Set rubric
...
| `FM-004` | `template` equals `objectives-and-key-results-set@1` | error | true |
...
Per the `contracts/objectives-and-key-results-set/template.md` H2 list. Each H2 in the template MUST be present + non-empty in the authored artefact. See template for the canonical headings.

| rule_id | Check | Severity |
| ------- | ----- | -------- |
| `SEC-001` | Every H2 declared in `contracts/objectives-and-key-results-set/template.md` is present in the authored artefact | error |
...
- `cyberos/skill/contracts/objectives-and-key-results-set/CONTRACT.md` + `template.md`
