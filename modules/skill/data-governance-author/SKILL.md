---
# ── Identity ─────────────────────────────────────────────────────────
name: data-governance-author
description: >-
  Author the annual data-governance framework (classification + stewardship + access control + retention + lineage + privacy controls + compliance mapping). CDO-Data + CCO-Compliance + CPO-Privacy co-authored. Use when user asks to "draft a data governance" or "create the data governance". Do NOT use for "audit existing data governance" (use data-governance-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: data-governance_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['data-governance-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        data-governance
    ...
    pin_path:  cyberos/skill/contracts/data-governance/
  - id:        nats-subjects
    version:   v1
...
# data-governance-author — DATA-GOVERNANCE generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `data-governance@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`data-governance-audit`](../data-governance-audit/SKILL.md) by default.

`prompt_revision: data-governance_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a DATA-GOVERNANCE."
- "Generate v2 of the DATA-GOVERNANCE from the updated source."
- "Draft a DATA-GOVERNANCE for <project>."

If the user asks to *audit an existing DATA-GOVERNANCE*, route to `data-governance-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        data-governance-author
skill_version:                   1.0.0
prompt_revision:                 data-governance_author@1.0.0
template_version:                data-governance@1   (loaded from cyberos/skill/contracts/data-governance/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  DATA-GOVERNANCE-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./data-governances/",
  "manifest_path": "./data-governances/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "DATA-GOVERNANCE-001", "path": "./data-governances/DATA-GOVERNANCE-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "data-governance-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/data-governance/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `data-governance_author.data-governance_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `data-governance-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `data-governance-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   data-governance-author
Input:
