---
# ── Identity ─────────────────────────────────────────────────────────
name: remote-policy-author
description: >-
  Author the annual remote/hybrid work policy (eligibility + arrangement types + tooling standards + meeting norms + tax/legal implications + culture rituals + performance reviews). GitLab Remote Manifesto + Buffer/Doist/Automattic patterns. Chief Remote Officer or CHRO authored. Use when user asks to "draft a remote policy" or "create the remote policy". Do NOT use for "audit existing remote policy" (use remote-policy-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: remote-policy_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['remote-policy-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        remote-policy
    ...
    pin_path:  cyberos/skill/contracts/remote-policy/
  - id:        nats-subjects
    version:   v1
...
# remote-policy-author — REMOTE-POLICY generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `remote-policy@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`remote-policy-audit`](../remote-policy-audit/SKILL.md) by default.

`prompt_revision: remote-policy_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a REMOTE-POLICY."
- "Generate v2 of the REMOTE-POLICY from the updated source."
- "Draft a REMOTE-POLICY for <project>."

If the user asks to *audit an existing REMOTE-POLICY*, route to `remote-policy-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 remote-policy_author@1.0.0
template_version:                remote-policy@1   (loaded from cyberos/skill/contracts/remote-policy/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  REMOTE-POLICY-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./remote-policys/",
  "manifest_path": "./remote-policys/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "REMOTE-POLICY-001", "path": "./remote-policys/REMOTE-POLICY-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "remote-policy-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/remote-policy/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `remote-policy_author.remote-policy_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `remote-policy-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `remote-policy-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   remote-policy-author
Input:
