# `remote-policy-author` — human summary format
...
```
REMOTE-POLICY batch <batch_run_id> complete

Wrote N REMOTE-POLICY(s):
  - REMOTE-POLICY-001: <slug> — PASS (audit verdict: pass, 0 open issues)
  - REMOTE-POLICY-002: <slug> — HITL_PAUSE (2 blocking issues — see HITL_BATCH_REQUEST below)
  - REMOTE-POLICY-003: <slug> — PASS (audit verdict: pass, 1 warning)

Manifest:    <output_dir>/manifest.json
Total time:  <seconds>s
Next:        remote-policy-audit will run automatically on PASS items. HITL items wait for your reply.
```
