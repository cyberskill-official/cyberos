...
name: okr-set-author
description: |
  Author a quarterly OKR set (company / function / team cascade) for the CEO or Chief of Staff. Renders objectives + key results with owner, cadence, measurement source, and parent/child cascade links. Validated by okr-set-audit.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['okr-set-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        okr-set
    ...
    pin_path:  cyberos/skill/contracts/okr-set/
  - id:        nats-subjects
    version:   v1
...
# okr-set-author — OKR-SET generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `okr-set@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`okr-set-audit`](../okr-set-audit/SKILL.md) by default.
...
If the user asks to *audit an existing OKR-SET*, route to `okr-set-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        okr-set-author
skill_version:                   1.0.0
prompt_revision:                 okr-set_author@1.0.0
template_version:                okr-set@1   (loaded from cyberos/skill/contracts/okr-set/template.md)
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "okr-set-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/okr-set/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `okr-set-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `okr-set-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   okr-set-author
Input:
