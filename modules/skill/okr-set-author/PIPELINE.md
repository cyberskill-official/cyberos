# `okr-set-author` — pipeline

This document describes how `okr-set-author` chains with upstream and downstream skills.
...
|---|---|---|
| `okr-set-audit` | Default after every `artefact_written` event | `next_skill_recommendation: okr-set-audit` in output envelope. |
| `<next-stage>-author` | After `okr-set-audit` returns PASS | Supervisor reads the audit's output envelope and queues the next stage. |
...
- `cyberos/skill/contracts/okr-set/` — the artefact template this skill generates.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/okr-set-audit/` — the sibling audit skill that validates this skill's output.
