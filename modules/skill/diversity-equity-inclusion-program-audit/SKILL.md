---
# ── Identity ─────────────────────────────────────────────────────────
name: diversity-equity-inclusion-program-audit
description: >-
  Audit one or more existing diversity-equity-inclusion-program@1 markdowns against dei-program_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after diversity-equity-inclusion-program-author. Use when user asks to "audit this diversity equity inclusion program" or "check the diversity equity inclusion program". Do NOT use for "draft a new diversity equity inclusion program" (use diversity-equity-inclusion-program-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: diversity-equity-inclusion-program@1
  cyberos-rubric-version: dei-program_rubric@1.0
...
# diversity-equity-inclusion-program-audit — DEI-PROGRAM auditor

> Standalone trigger that runs `dei-program_rubric@1.0` against one or
> more existing `diversity-equity-inclusion-program@1` markdowns and writes a sibling
> `dei-program.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`diversity-equity-inclusion-program-author`](../diversity-equity-inclusion-program-author/SKILL.md).

`prompt_revision: dei-program_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `DEI-PROGRAM`s."
- "Has `DEI-PROGRAM-007` changed since the last audit?"
- "Tell me which `DEI-PROGRAM`s would fail acceptance today."

Also invoked automatically by the supervisor when `diversity-equity-inclusion-program-author`'s output envelope sets `next_skill_recommendation: diversity-equity-inclusion-program-audit` (the default chain).
...
audit_rubric_version:            dei-program_rubric@1.0
...
```json
{
  "artefact_paths": ["./dei-programs/DEI-PROGRAM-001-foo.md", "./dei-programs/DEI-PROGRAM-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "diversity-equity-inclusion-program-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./dei-programs/DEI-PROGRAM-001-foo.md", "audit_path": "./dei-programs/DEI-PROGRAM-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./dei-programs/DEI-PROGRAM-002-bar.md", "audit_path": "./dei-programs/DEI-PROGRAM-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   diversity-equity-inclusion-program-audit
Input:
  artefact_paths:  [./team-a/DEI-PROGRAM-001-something.md, ./team-b/DEI-PROGRAM-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
