---
# ── Identity ─────────────────────────────────────────────────────────
name: program-charter-author
description: >-
  Author a per-program charter (purpose + scope-in + scope-out + governance + success criteria + funding envelope + risks + timeline + executive sponsor). PMI Program Management Standard. Chief Transformation Officer or Chief Innovation Officer authored. Use when user asks to "draft a program charter" or "create the program charter". Do NOT use for "audit existing program charter" (use program-charter-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: program-charter_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['program-charter-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        program-charter
    ...
    pin_path:  cyberos/skill/contracts/program-charter/
  - id:        nats-subjects
    version:   v1
...
# program-charter-author — PROGRAM-CHARTER generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `program-charter@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`program-charter-audit`](../program-charter-audit/SKILL.md) by default.

`prompt_revision: program-charter_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a PROGRAM-CHARTER."
- "Generate v2 of the PROGRAM-CHARTER from the updated source."
- "Draft a PROGRAM-CHARTER for <project>."

If the user asks to *audit an existing PROGRAM-CHARTER*, route to `program-charter-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        program-charter-author
skill_version:                   1.0.0
prompt_revision:                 program-charter_author@1.0.0
template_version:                program-charter@1   (loaded from cyberos/skill/contracts/program-charter/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  PROGRAM-CHARTER-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./program-charters/",
  "manifest_path": "./program-charters/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "PROGRAM-CHARTER-001", "path": "./program-charters/PROGRAM-CHARTER-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "program-charter-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/program-charter/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `program-charter_author.program-charter_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `program-charter-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `program-charter-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   program-charter-author
Input:
