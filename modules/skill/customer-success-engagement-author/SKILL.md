---
# ── Identity ─────────────────────────────────────────────────────────
name: customer-success-engagement-author
description: >-
  Author a per-customer engagement plan (context + health-score + QBR cadence + expansion theses + churn-risk indicators + executive coverage). CCO-Customer authored. Use when user asks to "draft a customer success engagement" or "create the customer success engagement". Do NOT use for "audit existing customer success engagement" (use customer-success-engagement-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: cs-engagement_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['customer-success-engagement-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        cs-engagement
    ...
    pin_path:  cyberos/skill/contracts/customer-success-engagement/
  - id:        nats-subjects
    version:   v1
...
# customer-success-engagement-author — CS-ENGAGEMENT generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `customer-success-engagement@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`customer-success-engagement-audit`](../customer-success-engagement-audit/SKILL.md) by default.

`prompt_revision: cs-engagement_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a CS-ENGAGEMENT."
- "Generate v2 of the CS-ENGAGEMENT from the updated source."
- "Draft a CS-ENGAGEMENT for <project>."

If the user asks to *audit an existing CS-ENGAGEMENT*, route to `customer-success-engagement-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        customer-success-engagement-author
skill_version:                   1.0.0
prompt_revision:                 cs-engagement_author@1.0.0
template_version:                customer-success-engagement@1   (loaded from cyberos/skill/contracts/customer-success-engagement/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  CS-ENGAGEMENT-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./cs-engagements/",
  "manifest_path": "./cs-engagements/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "CS-ENGAGEMENT-001", "path": "./cs-engagements/CS-ENGAGEMENT-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "customer-success-engagement-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/customer-success-engagement/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `cs-engagement_author.cs-engagement_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `customer-success-engagement-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `customer-success-engagement-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   customer-success-engagement-author
Input:
