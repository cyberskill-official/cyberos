...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['pia-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        pia
    ...
    pin_path:  cyberos/skill/contracts/pia/
  - id:        nats-subjects
    version:   v1
...
# pia-author — PIA generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `pia@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`pia-audit`](../pia-audit/SKILL.md) by default.
...
If the user asks to *audit an existing PIA*, route to `pia-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 pia_author@1.0.0
template_version:                pia@1   (loaded from cyberos/skill/contracts/pia/template.md)
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "pia-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/pia/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `pia-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `pia-audit` (and other downstream consumers).
