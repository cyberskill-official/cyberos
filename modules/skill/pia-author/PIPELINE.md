...
This document describes how `pia-author` chains with upstream and downstream skills.
...
|---|---|---|
| `pia-audit` | Default after every `artefact_written` event | `next_skill_recommendation: pia-audit` in output envelope. |
| `<next-stage>-author` | After `pia-audit` returns PASS | Supervisor reads the audit's output envelope and queues the next stage. |
...
- `cyberos/skill/contracts/pia/` — the artefact template this skill generates.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/pia-audit/` — the sibling audit skill that validates this skill's output.
