---
# ── Identity ─────────────────────────────────────────────────────────
name: trust-portal-update-author
description: >-
  Author the monthly trust-portal content update (certifications + audit results + sub-processor list + incident-status timeline + FAQ updates + customer-trust briefing log). Vanta / Drata / SafeBase trust-portal patterns. Chief Trust Officer + CPO-Privacy co-authored. Use when user asks to "draft a trust portal update" or "create the trust portal update". Do NOT use for "audit existing trust portal update" (use trust-portal-update-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: trust-portal-update_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['trust-portal-update-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        trust-portal-update
    version:   v1
    purpose:   generation_skeleton
    pin_path:  cyberos/skill/contracts/trust-portal-update/
  - id:        nats-subjects
    version:   v1
...
# trust-portal-update-author — TRUST-PORTAL-UPDATE generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `trust-portal-update@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`trust-portal-update-audit`](../trust-portal-update-audit/SKILL.md) by default.

`prompt_revision: trust-portal-update_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a TRUST-PORTAL-UPDATE."
- "Generate v2 of the TRUST-PORTAL-UPDATE from the updated source."
- "Draft a TRUST-PORTAL-UPDATE for <project>."

If the user asks to *audit an existing TRUST-PORTAL-UPDATE*, route to `trust-portal-update-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        trust-portal-update-author
skill_version:                   1.0.0
prompt_revision:                 trust-portal-update_author@1.0.0
template_version:                trust-portal-update@1   (loaded from cyberos/skill/contracts/trust-portal-update/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  TRUST-PORTAL-UPDATE-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./trust-portal-updates/",
  "manifest_path": "./trust-portal-updates/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "TRUST-PORTAL-UPDATE-001", "path": "./trust-portal-updates/TRUST-PORTAL-UPDATE-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "trust-portal-update-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/trust-portal-update/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `trust-portal-update_author.trust-portal-update_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `trust-portal-update-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `trust-portal-update-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   trust-portal-update-author
Input:
