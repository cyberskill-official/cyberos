---
# ── Identity ─────────────────────────────────────────────────────────
name: data-strategy-author
description: >-
  Author the annual data strategy (vision + data-as-product principles + governance framework + data quality charter + ML platform roadmap + investment portfolio + measurable goals). DAMA-DMBOK + Data Mesh aligned. CDO-Data authored. Use when user asks to "draft a data strategy" or "create the data strategy". Do NOT use for "audit existing data strategy" (use data-strategy-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: data-strategy_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['data-strategy-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        data-strategy
    ...
    pin_path:  cyberos/skill/contracts/data-strategy/
  - id:        nats-subjects
    version:   v1
...
# data-strategy-author — DATA-STRATEGY generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `data-strategy@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`data-strategy-audit`](../data-strategy-audit/SKILL.md) by default.

`prompt_revision: data-strategy_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a DATA-STRATEGY."
- "Generate v2 of the DATA-STRATEGY from the updated source."
- "Draft a DATA-STRATEGY for <project>."

If the user asks to *audit an existing DATA-STRATEGY*, route to `data-strategy-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        data-strategy-author
skill_version:                   1.0.0
prompt_revision:                 data-strategy_author@1.0.0
template_version:                data-strategy@1   (loaded from cyberos/skill/contracts/data-strategy/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  DATA-STRATEGY-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./data-strategys/",
  "manifest_path": "./data-strategys/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "DATA-STRATEGY-001", "path": "./data-strategys/DATA-STRATEGY-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "data-strategy-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/data-strategy/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `data-strategy_author.data-strategy_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
- If chained to `data-strategy-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `data-strategy-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   data-strategy-author
Input:
