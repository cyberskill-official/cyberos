---
# ── Identity ─────────────────────────────────────────────────────────
name: ethics-review-author
description: >-
  Author a per-feature ethics review (feature summary + intended uses + anti-uses + harm vectors per stakeholder + mitigations + residual-harm acceptance + decision + sign-off). IEEE Ethically Aligned Design + Markkula Center applied-ethics framework. Chief Ethics Officer authored. Use when user asks to "draft a ethics review" or "create the ethics review". Do NOT use for "audit existing ethics review" (use ethics-review-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: ethics-review_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['ethics-review-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        ethics-review
    ...
    pin_path:  cyberos/skill/contracts/ethics-review/
  - id:        nats-subjects
    version:   v1
...
# ethics-review-author — ETHICS-REVIEW generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `ethics-review@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`ethics-review-audit`](../ethics-review-audit/SKILL.md) by default.

`prompt_revision: ethics-review_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a ETHICS-REVIEW."
- "Generate v2 of the ETHICS-REVIEW from the updated source."
- "Draft a ETHICS-REVIEW for <project>."

If the user asks to *audit an existing ETHICS-REVIEW*, route to `ethics-review-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        ethics-review-author
skill_version:                   1.0.0
prompt_revision:                 ethics-review_author@1.0.0
template_version:                ethics-review@1   (loaded from cyberos/skill/contracts/ethics-review/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  ETHICS-REVIEW-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./ethics-reviews/",
  "manifest_path": "./ethics-reviews/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "ETHICS-REVIEW-001", "path": "./ethics-reviews/ETHICS-REVIEW-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "ethics-review-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/ethics-review/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `ethics-review_author.ethics-review_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
- If chained to `ethics-review-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `ethics-review-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   ethics-review-author
Input:
