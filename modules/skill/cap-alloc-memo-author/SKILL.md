...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['cap-alloc-memo-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        cap-alloc-memo
    ...
    pin_path:  cyberos/skill/contracts/cap-alloc-memo/
  - id:        nats-subjects
    version:   v1
...
# cap-alloc-memo-author — CAP-ALLOC-MEMO generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `cap-alloc-memo@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`cap-alloc-memo-audit`](../cap-alloc-memo-audit/SKILL.md) by default.
...
If the user asks to *audit an existing CAP-ALLOC-MEMO*, route to `cap-alloc-memo-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        cap-alloc-memo-author
skill_version:                   1.0.0
prompt_revision:                 cap-alloc-memo_author@1.0.0
template_version:                cap-alloc-memo@1   (loaded from cyberos/skill/contracts/cap-alloc-memo/template.md)
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "cap-alloc-memo-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/cap-alloc-memo/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `cap-alloc-memo-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `cap-alloc-memo-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   cap-alloc-memo-author
Input:
