...
metadata:
  ...
  module: skill
  stage: cross
  cyberos-template: customer-health-review@1
...
# customer-health-review-audit — CUSTOMER_HEALTH_REVIEW auditor

> Standalone trigger that runs `customer-health-review_rubric@1.0` against one or
> more existing `customer-health-review@1` markdowns and writes a sibling
> `customer-health-review.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`customer-health-review-author`](../customer-health-review-author/SKILL.md).

`prompt_revision: customer-health-review_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `CUSTOMER_HEALTH_REVIEW`s."
- "Has `CUSTOMER_HEALTH_REVIEW-007` changed since the last audit?"
- "Tell me which `CUSTOMER_HEALTH_REVIEW`s would fail acceptance today."

Also invoked automatically by the supervisor when `customer-health-review-author`'s output envelope sets `next_skill_recommendation: customer-health-review-audit` (the default chain).
...
audit_rubric_version:            customer-health-review_rubric@1.0
...
```json
{
  "artefact_paths": ["./customer-health-reviews/CUSTOMER_HEALTH_REVIEW-001-foo.md", "./customer-health-reviews/CUSTOMER_HEALTH_REVIEW-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "customer-health-review-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./customer-health-reviews/CUSTOMER_HEALTH_REVIEW-001-foo.md", "audit_path": "./customer-health-reviews/CUSTOMER_HEALTH_REVIEW-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./customer-health-reviews/CUSTOMER_HEALTH_REVIEW-002-bar.md", "audit_path": "./customer-health-reviews/CUSTOMER_HEALTH_REVIEW-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   customer-health-review-audit
Input:
  artefact_paths:  [./team-a/CUSTOMER_HEALTH_REVIEW-001-something.md, ./team-b/CUSTOMER_HEALTH_REVIEW-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
