...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['employee-net-promoter-score-program-audit']; empty list disables chaining
...
# employee-net-promoter-score-program-author — ENPS-PROGRAM generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `employee-net-promoter-score-program@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`employee-net-promoter-score-program-audit`](../employee-net-promoter-score-program-audit/SKILL.md) by default.

`prompt_revision: enps-program_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a ENPS-PROGRAM."
- "Generate v2 of the ENPS-PROGRAM from the updated source."
- "Draft a ENPS-PROGRAM for <project>."

If the user asks to *audit an existing ENPS-PROGRAM*, route to `employee-net-promoter-score-program-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  ENPS-PROGRAM-{NNN}-{slug}.md
...
```json
{
  ...
  "artefacts_written": [
    {"id": "ENPS-PROGRAM-001", "path": "./enps-programs/ENPS-PROGRAM-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "employee-net-promoter-score-program-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/employee-net-promoter-score-program/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `enps-program_author.enps-program_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `employee-net-promoter-score-program-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `employee-net-promoter-score-program-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   employee-net-promoter-score-program-author
Input:
