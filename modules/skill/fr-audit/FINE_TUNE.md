# `fr-audit` — fine-tune discipline override

Default discipline at `../docs/FINE_TUNE.md`. This file documents the **fr-audit-specific overrides**.

## Why fr-audit is different

`fr-audit` ships with `audit_rubric@2.0` ported verbatim from the proven legacy `cuo/cpo/fr-audit` skill (2026-02 vintage). The rubric was battle-tested against 50+ FRs in `cyberos/docs/feature-requests/` before being ported. Bumping it requires extra scrutiny because:
...
3. The fr-author + fr-audit pair is the cyberos project's primary feature-tracking workflow. Audit instability shakes confidence in every FR's status.
...
- Changing the cross-skill XCHAIN-001/002 contract with fr-author.
...
- The cyberos project's `docs/feature-requests/AUTHORING.md` — the operator-side authoring discipline this rubric audits.
