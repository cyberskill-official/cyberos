# `comp-plan-author` — pipeline

This document describes how `comp-plan-author` chains with upstream and downstream skills.
...
|---|---|---|
| `comp-plan-audit` | Default after every `artefact_written` event | `next_skill_recommendation: comp-plan-audit` in output envelope. |
| `<next-stage>-author` | After `comp-plan-audit` returns PASS | Supervisor reads the audit's output envelope and queues the next stage. |
...
- `cyberos/skill/contracts/comp-plan/` — the artefact template this skill generates.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/comp-plan-audit/` — the sibling audit skill that validates this skill's output.
