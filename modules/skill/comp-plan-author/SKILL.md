...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['comp-plan-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        comp-plan
    ...
    pin_path:  cyberos/skill/contracts/comp-plan/
  - id:        nats-subjects
    version:   v1
...
# comp-plan-author — COMP-PLAN generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `comp-plan@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`comp-plan-audit`](../comp-plan-audit/SKILL.md) by default.
...
If the user asks to *audit an existing COMP-PLAN*, route to `comp-plan-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        comp-plan-author
skill_version:                   1.0.0
prompt_revision:                 comp-plan_author@1.0.0
template_version:                comp-plan@1   (loaded from cyberos/skill/contracts/comp-plan/template.md)
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "comp-plan-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/comp-plan/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `comp-plan-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `comp-plan-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   comp-plan-author
Input:
