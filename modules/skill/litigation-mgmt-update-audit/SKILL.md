---
# ── Identity ─────────────────────────────────────────────────────────
name: litigation-mgmt-update-audit
description: Audits a litigation-mgmt-update@1 artefact against litigation_mgmt_update_rubric@1.0. Validates FM + SEC structure, per-matter exposure quantification, settlement-posture freshness, outside-counsel-cost benchmarking vs ACC Value Challenge, and reserve-recommendation consistency with accounting reserve.
  Audit one or more existing litigation-mgmt-update@1 markdowns against
  ...
  litigation-mgmt-update-author.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: litigation-mgmt-update@1
...
depends_on_contracts:
  - id:        litigation-mgmt-update
    version:   v1
    purpose:   validation_target
    pin_path:  cyberos/skill/contracts/litigation-mgmt-update/
  - id:        nats-subjects
    version:   v1
...
# litigation-mgmt-update-audit — LITIGATION_MGMT_UPDATE auditor

> Standalone trigger that runs `litigation-mgmt-update_rubric@1.0` against one or
> more existing `litigation-mgmt-update@1` markdowns and writes a sibling
> `litigation-mgmt-update.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`litigation-mgmt-update-author`](../litigation-mgmt-update-author/SKILL.md).

`prompt_revision: litigation-mgmt-update_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `LITIGATION_MGMT_UPDATE`s."
- "Has `LITIGATION_MGMT_UPDATE-007` changed since the last audit?"
- "Tell me which `LITIGATION_MGMT_UPDATE`s would fail acceptance today."

Also invoked automatically by the supervisor when `litigation-mgmt-update-author`'s output envelope sets `next_skill_recommendation: litigation-mgmt-update-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        litigation-mgmt-update-audit
skill_version:                   1.0.0
prompt_revision:                 litigation-mgmt-update_audit@1.0.0
template_version:                litigation-mgmt-update@1   (loaded from cyberos/skill/contracts/litigation-mgmt-update/template.md)
audit_rubric_version:            litigation-mgmt-update_rubric@1.0
...
```json
{
  "artefact_paths": ["./litigation-mgmt-updates/LITIGATION_MGMT_UPDATE-001-foo.md", "./litigation-mgmt-updates/LITIGATION_MGMT_UPDATE-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "litigation-mgmt-update-author",
    "manifest_path": "./litigation-mgmt-updates/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./litigation-mgmt-updates/LITIGATION_MGMT_UPDATE-001-foo.md", "audit_path": "./litigation-mgmt-updates/LITIGATION_MGMT_UPDATE-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./litigation-mgmt-updates/LITIGATION_MGMT_UPDATE-002-bar.md", "audit_path": "./litigation-mgmt-updates/LITIGATION_MGMT_UPDATE-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   litigation-mgmt-update-audit
Input:
  artefact_paths:  [./team-a/LITIGATION_MGMT_UPDATE-001-something.md, ./team-b/LITIGATION_MGMT_UPDATE-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
