---
# ── Identity ─────────────────────────────────────────────────────────
name: cs-engagement-audit
description: |
  Audit one or more existing cs-engagement@1 markdowns against
  ...
  cs-engagement-author.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: <SDP §2 stage letter or "cross">
  cyberos-template: cs-engagement@1
...
# cs-engagement-audit — CS-ENGAGEMENT auditor

> Standalone trigger that runs `cs-engagement_rubric@1.0` against one or
> more existing `cs-engagement@1` markdowns and writes a sibling
...
> Chains naturally after [`cs-engagement-author`](../cs-engagement-author/SKILL.md).
...
Also invoked automatically by the supervisor when `cs-engagement-author`'s output envelope sets `next_skill_recommendation: cs-engagement-audit` (the default chain).
...
```json
{
  ...
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "cs-engagement-author",
