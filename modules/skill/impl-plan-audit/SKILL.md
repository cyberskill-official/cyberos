---
# ── Identity ─────────────────────────────────────────────────────────
name: impl-plan-audit
description: |
  Audit one or more existing impl-plan@1 markdowns against
...
depends_on_contracts:
  - id:        impl-plan
    ...
    pin_path:  cyberos/skill/contracts/impl-plan/
  - id:        nats-subjects
    version:   v1
...
# impl-plan-audit — IMPL-PLAN auditor

> Standalone trigger that runs `impl-plan_rubric@1.0` against one or
> more existing `impl-plan@1` markdowns and writes a sibling
...
> Chains naturally after [`impl-plan-author`](../impl-plan-author/SKILL.md).
...
Also invoked automatically by the supervisor when `impl-plan-author`'s output envelope sets `next_skill_recommendation: impl-plan-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        impl-plan-audit
skill_version:                   1.0.0
prompt_revision:                 impl-plan_audit@1.0.0
template_version:                impl-plan@1   (loaded from cyberos/skill/contracts/impl-plan/template.md)
