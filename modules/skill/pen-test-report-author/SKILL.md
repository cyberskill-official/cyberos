...
description: Authors a penetration-test engagement report (internal red-team or external third-party). Executive summary, scope + rules of engagement, methodology, findings ranked by CVSS with PoC + remediation, retest plan. Per OWASP Web Security Testing Guide v4.2 + PTES + NIST SP 800-115 + OWASP ASVS v5.0 + MITRE ATT&CK.
  ...
  versioned pen-test-report@1 file under output_dir, with per-claim authority
  markers and provenance to the source. Chains naturally into
  pen-test-report-audit by default. Refuses to author when upstream artefact
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['pen-test-report-audit']; empty list disables chaining
...
# pen-test-report-author — PEN_TEST_REPORT generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `pen-test-report@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`pen-test-report-audit`](../pen-test-report-audit/SKILL.md) by default.
...
- "Turn this <input> into a PEN_TEST_REPORT."
- "Generate v2 of the PEN_TEST_REPORT from the updated source."
- "Draft a PEN_TEST_REPORT for <project>."

If the user asks to *audit an existing PEN_TEST_REPORT*, route to `pen-test-report-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        pen-test-report-author
skill_version:                   1.0.0
prompt_revision:                 pen-test-report_author@1.0.0
...
```json
{
  ...
  "artefacts_written": [
    {"id": "PEN_TEST_REPORT-001", "path": "./pen-test-reports/PEN_TEST_REPORT-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "pen-test-report-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/pen-test-report/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `pen-test-report_author.pen-test-report_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `pen-test-report-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `pen-test-report-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   pen-test-report-author
Input:
