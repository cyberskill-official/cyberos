...
description: |
  ...
  due dates. Chains naturally into retro-audit.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: l
  cyberos-template: retro@1
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['retro-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        retro
    ...
    pin_path:  cyberos/skill/contracts/retro/
  - id:        nats-subjects
    version:   v1
...
# retro-author — RETRO generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `retro@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`retro-audit`](../retro-audit/SKILL.md) by default.
...
If the user asks to *audit an existing RETRO*, route to `retro-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        retro-author
skill_version:                   1.0.0
prompt_revision:                 retro_author@1.0.0
template_version:                retro@1   (loaded from cyberos/skill/contracts/retro/template.md)
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "retro-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/retro/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `retro-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `retro-audit` (and other downstream consumers).
