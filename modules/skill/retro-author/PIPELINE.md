# `retro-author` — pipeline

This document describes how `retro-author` chains with upstream and downstream skills.
...
|---|---|---|
| `retro-audit` | Default after every `artefact_written` event | `next_skill_recommendation: retro-audit` in output envelope. |
| `<next-stage>-author` | After `retro-audit` returns PASS | Supervisor reads the audit's output envelope and queues the next stage. |
...
- `cyberos/skill/contracts/retro/` — the artefact template this skill generates.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/retro-audit/` — the sibling audit skill that validates this skill's output.
