# `task-author` — pipeline

This document describes how `task-author` chains with upstream and downstream skills.
...
|---|---|---|
| `task-audit` | Default after every `artefact_written` event | `next_skill_recommendation: task-audit` in output envelope. |
| `<next-stage>-author` | After `task-audit` returns PASS | Supervisor reads the audit's output envelope and queues the next stage. |
...
- `cyberos/skill/contracts/task/` — the artefact template this skill generates.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/task-audit/` — the sibling audit skill that validates this skill's output.
