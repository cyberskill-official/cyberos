...
description: >-
  ...
  task-audit. Do NOT use for "audit existing FRs" (use
  task-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
...
If the user asks to *audit an existing FR*, route to `task-audit` instead. If both, the supervisor chains the two.
...
>
> Authored FRs live at `cyberos/docs/tasks/{module}/FR-{MOD}-{NNN}-{slug}/spec.md` with the audit at `{same-folder}/audit.md` and media in `{same-folder}/assets/` (TASK-DOCS-004 folder-per-FR; assets/ created on first asset). This file is the operator-side companion to the skill-side `RUBRIC.md`.
