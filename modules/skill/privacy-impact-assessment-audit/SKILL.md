---
# ── Identity ─────────────────────────────────────────────────────────
name: privacy-impact-assessment-audit
description: >-
  Audit one or more existing privacy-impact-assessment@1 markdowns against pia_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after privacy-impact-assessment-author. Use when user asks to "audit this privacy impact assessment" or "check the privacy impact assessment". Do NOT use for "draft a new privacy impact assessment" (use privacy-impact-assessment-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: privacy-impact-assessment@1
  cyberos-rubric-version: pia_rubric@1.0
...
# privacy-impact-assessment-audit — PIA auditor

> Standalone trigger that runs `pia_rubric@1.0` against one or
> more existing `privacy-impact-assessment@1` markdowns and writes a sibling
> `pia.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`privacy-impact-assessment-author`](../privacy-impact-assessment-author/SKILL.md).

`prompt_revision: pia_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `PIA`s."
- "Has `PIA-007` changed since the last audit?"
- "Tell me which `PIA`s would fail acceptance today."

Also invoked automatically by the supervisor when `privacy-impact-assessment-author`'s output envelope sets `next_skill_recommendation: privacy-impact-assessment-audit` (the default chain).
...
```json
{
  "artefact_paths": ["./pias/PIA-001-foo.md", "./pias/PIA-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "privacy-impact-assessment-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./pias/PIA-001-foo.md", "audit_path": "./pias/PIA-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./pias/PIA-002-bar.md", "audit_path": "./pias/PIA-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   privacy-impact-assessment-audit
Input:
  artefact_paths:  [./team-a/PIA-001-something.md, ./team-b/PIA-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
