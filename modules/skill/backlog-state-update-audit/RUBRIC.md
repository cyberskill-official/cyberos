...
constants: TOTAL_ROWS_MIN=8 (MUST tasks) | BRANCH_COVERAGE_MIN=80 | COVERAGE_THRESHOLD=90 (config-overridable, TASK-CUO-207) families: BSU | BSU-INS verdict: pass requires 10/10; any family failure -> fail; ambiguity -> needs_human
...
| `BSU-002` | line_number resolves to a real task row in the pre-image | SKILL.md line_number clause |
...
| `BSU-INS-003` | insert-row: placed in the correct module section, task-STEM sort order kept | TASK-CUO-205 §1 #4 + INSERT_ROW_CASES CASE-08 |
| `BSU-INS-004` | insert-row: no other line changed except that section's header counts | TASK-CUO-205 §1 #4 |
| `BSU-INS-005` | insert-row: status valid enum AND equals task frontmatter status at write time | TASK-CUO-205 §1 #4 |
