---
# ── Identity ─────────────────────────────────────────────────────────
name: internal-newsletter-audit
description: >-
  Audit one or more existing internal-newsletter@1 markdowns against internal-newsletter_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after internal-newsletter-author. Use when user asks to "audit this internal newsletter" or "check the internal newsletter". Do NOT use for "draft a new internal newsletter" (use internal-newsletter-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
...
# internal-newsletter-audit — INTERNAL-NEWSLETTER auditor

> Standalone trigger that runs `internal-newsletter_rubric@1.0` against one or
> more existing `internal-newsletter@1` markdowns and writes a sibling
> `internal-newsletter.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`internal-newsletter-author`](../internal-newsletter-author/SKILL.md).

`prompt_revision: internal-newsletter_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `INTERNAL-NEWSLETTER`s."
- "Has `INTERNAL-NEWSLETTER-007` changed since the last audit?"
- "Tell me which `INTERNAL-NEWSLETTER`s would fail acceptance today."

Also invoked automatically by the supervisor when `internal-newsletter-author`'s output envelope sets `next_skill_recommendation: internal-newsletter-audit` (the default chain).
...
audit_rubric_version:            internal-newsletter_rubric@1.0
...
```json
{
  "artefact_paths": ["./internal-newsletters/INTERNAL-NEWSLETTER-001-foo.md", "./internal-newsletters/INTERNAL-NEWSLETTER-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "internal-newsletter-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./internal-newsletters/INTERNAL-NEWSLETTER-001-foo.md", "audit_path": "./internal-newsletters/INTERNAL-NEWSLETTER-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./internal-newsletters/INTERNAL-NEWSLETTER-002-bar.md", "audit_path": "./internal-newsletters/INTERNAL-NEWSLETTER-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   internal-newsletter-audit
Input:
  artefact_paths:  [./team-a/INTERNAL-NEWSLETTER-001-something.md, ./team-b/INTERNAL-NEWSLETTER-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
