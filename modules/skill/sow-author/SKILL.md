...
description: |
  ...
  approval and HITL gates. Chains naturally into sow-audit by default.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
...
# sow-author — Statement of Work generator

> Authors a Statement of Work (SOW) / Project Charter from a discovery brief, lead form, or kick-off interview. Implements the 12-section skeleton from Software Development Process.md §4.9. Chains naturally into [`sow-audit`](../sow-audit/SKILL.md).
...
If the user wants to *audit an existing SOW* instead of authoring, route to `sow-audit`.
...
skill_version:                   1.0.0
prompt_revision:                 sow_author@1.0.0
template_version:                sow@1   (loaded from cyberos/skill/contracts/project-brief/template.md plus §4.9 skeleton)
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "sow-audit"
}
```
...
- **W5 ROUTE** — invoke `sow-audit` with the SOW path; forward verdict into manifest.
...
- `PIPELINE.md` — chain to sow-audit and downstream PRD/SRS skills.
...
The skill produces `./engagements/acme-2026/SOW-acme-20260517.md` and queues `sow-audit` to validate it against `sow_rubric@1.0`. Audit returns PASS / HITL_PAUSE / FAIL — operator iterates until 10/10.
