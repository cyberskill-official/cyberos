...
This document describes how `sow-author` chains with upstream and downstream skills.
...
|---|---|---|
| `sow-audit` | Default after every `artefact_written` event | `next_skill_recommendation: sow-audit` in output envelope. |
| `<next-stage>-author` | After `sow-audit` returns PASS | Supervisor reads the audit's output envelope and queues the next stage. |
...
- `cyberos/skill/contracts/sow/` — the artefact template this skill generates.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/sow-audit/` — the sibling audit skill that validates this skill's output.
