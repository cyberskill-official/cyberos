...
description: >-
  Authors the annual (or per-event) IP portfolio strategy — patent filings, trademark portfolio, trade-secret protection plan, defensive vs offensive posture, FTO (freedom-to-operate) opinions. Per WIPO IP strategy framework + USPTO MPEP + Madrid Protocol (trademarks) + UTSA/DTSA (trade secrets) + PCT for international filings. Author a IP_STRATEGY markdown from source artefact(s). Generates a versioned intellectual-property-strategy@1 file under output_dir, with per-claim authority markers and provenance to the source. Chains naturally into intellectual-property-strategy-audit by default. Refuses to author when upstream artefact is in non-pass state. Use when user asks to "draft a intellectual property strategy" or "create the intellectual property strategy". Do NOT use for "audit existing intellectual property strategy" (use intellectual-property-strategy-audit instead). Author a IP_STRATEGY markdown from source artefact(s). Generates a versioned intellectual-property-strategy@1 file under output_dir, wit...
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: ip-strategy_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['intellectual-property-strategy-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        ip-strategy
    ...
    pin_path:  cyberos/skill/contracts/intellectual-property-strategy/
  - id:        nats-subjects
    version:   v1
...
# intellectual-property-strategy-author — IP_STRATEGY generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `intellectual-property-strategy@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`intellectual-property-strategy-audit`](../intellectual-property-strategy-audit/SKILL.md) by default.

`prompt_revision: ip-strategy_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a IP_STRATEGY."
- "Generate v2 of the IP_STRATEGY from the updated source."
- "Draft a IP_STRATEGY for <project>."

If the user asks to *audit an existing IP_STRATEGY*, route to `intellectual-property-strategy-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 ip-strategy_author@1.0.0
template_version:                intellectual-property-strategy@1   (loaded from cyberos/skill/contracts/intellectual-property-strategy/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  IP_STRATEGY-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./ip-strategys/",
  "manifest_path": "./ip-strategys/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "IP_STRATEGY-001", "path": "./ip-strategys/IP_STRATEGY-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "intellectual-property-strategy-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/intellectual-property-strategy/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `ip-strategy_author.ip-strategy_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `intellectual-property-strategy-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `intellectual-property-strategy-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   intellectual-property-strategy-author
Input:
