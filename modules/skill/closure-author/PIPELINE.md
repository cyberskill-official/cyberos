# `closure-author` — pipeline

This document describes how `closure-author` chains with upstream and downstream skills.
...
|---|---|---|
| `closure-audit` | Default after every `artefact_written` event | `next_skill_recommendation: closure-audit` in output envelope. |
| `<next-stage>-author` | After `closure-audit` returns PASS | Supervisor reads the audit's output envelope and queues the next stage. |
...
|---|---|---|
| `closure_author.closure_written` | `{artefact_id, artefact_path, artefact_hash, source_hash}` | After every successful W3 WRITE. |
| `closure_author.batch_complete` | `{batch_run_id, artefacts_written, batch_outcome}` | At the end of a WORKER batch. |
| `closure_author.hitl_pause` | `{artefact_id, blocking_issues}` | When the batch halts on HITL. |
| `closure_author.amendment_request` | `{amendment_id, risk_class, change_description}` | When the author proposes a plan amendment. |
...
- `cyberos/skill/contracts/closure/` — the artefact template this skill generates.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/closure-audit/` — the sibling audit skill that validates this skill's output.
