...
name: closure-author
description: >-
  Author a project closure package (closure@1) at the end of an engagement. Captures: sign-off certificate (client + EM + TL), lessons-learned compilation (from per-iteration retros), knowledge- transfer pack (source-code handover, runbooks ready, credentials rotated, runbook ownership transferred), asset handover record, client NPS, on-time/on-budget closure metrics. Covers modules/cuo/docs/appendices.md (§13 Software Development Process) §2(l). Chains naturally into closure-audit. Use when user asks to "draft a closure" or "create the closure". Do NOT use for "audit existing closure" (use closure-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: l
  cyberos-template: closure@1
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['closure-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        closure
    ...
    pin_path:  cyberos/skill/contracts/closure/
  - id:        nats-subjects
    version:   v1
...
# closure-author — CLOSURE generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `closure@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`closure-audit`](../closure-audit/SKILL.md) by default.

`prompt_revision: closure_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a CLOSURE."
- "Generate v2 of the CLOSURE from the updated source."
- "Draft a CLOSURE for <project>."

If the user asks to *audit an existing CLOSURE*, route to `closure-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 closure_author@1.0.0
template_version:                closure@1   (loaded from cyberos/skill/contracts/closure/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  CLOSURE-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./closures/",
  "manifest_path": "./closures/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "CLOSURE-001", "path": "./closures/CLOSURE-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "closure-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/closure/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `closure_author.closure_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `closure-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `closure-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   closure-author
Input:
