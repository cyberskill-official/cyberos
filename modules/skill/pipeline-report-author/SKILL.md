---
# ── Identity ─────────────────────────────────────────────────────────
name: pipeline-report-author
description: >-
  Author the weekly pipeline report (summary + pipeline by stage + coverage vs quota + win-rate + deal velocity + at-risk deals + forecast roll-up). CRO-Revenue authored. Target coverage 3-4×. Use when user asks to "draft a pipeline report" or "create the pipeline report". Do NOT use for "audit existing pipeline report" (use pipeline-report-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: pipeline-report_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['pipeline-report-audit']; empty list disables chaining
...
# pipeline-report-author — PIPELINE-REPORT generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `pipeline-report@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`pipeline-report-audit`](../pipeline-report-audit/SKILL.md) by default.

`prompt_revision: pipeline-report_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a PIPELINE-REPORT."
- "Generate v2 of the PIPELINE-REPORT from the updated source."
- "Draft a PIPELINE-REPORT for <project>."

If the user asks to *audit an existing PIPELINE-REPORT*, route to `pipeline-report-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  PIPELINE-REPORT-{NNN}-{slug}.md
...
```json
{
  ...
  "artefacts_written": [
    {"id": "PIPELINE-REPORT-001", "path": "./pipeline-reports/PIPELINE-REPORT-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "pipeline-report-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/pipeline-report/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `pipeline-report_author.pipeline-report_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `pipeline-report-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `pipeline-report-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   pipeline-report-author
Input:
