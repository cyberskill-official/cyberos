...
This document describes how `srs-author` chains with upstream and downstream skills.
...
|---|---|---|
| `srs-audit` | Default after every `artefact_written` event | `next_skill_recommendation: srs-audit` in output envelope. |
| `<next-stage>-author` | After `srs-audit` returns PASS | Supervisor reads the audit's output envelope and queues the next stage. |
...
- `cyberos/skill/contracts/srs/` — the artefact template this skill generates.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/srs-audit/` — the sibling audit skill that validates this skill's output.
