...
description: |
  ...
  naturally into srs-audit.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: b
  cyberos-template: srs@1
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['srs-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        srs
    ...
    pin_path:  cyberos/skill/contracts/srs/
  - id:        nats-subjects
    version:   v1
...
# srs-author — SRS generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `srs@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`srs-audit`](../srs-audit/SKILL.md) by default.
...
If the user asks to *audit an existing SRS*, route to `srs-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 srs_author@1.0.0
template_version:                srs@1   (loaded from cyberos/skill/contracts/srs/template.md)
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "srs-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/srs/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `srs-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `srs-audit` (and other downstream consumers).
