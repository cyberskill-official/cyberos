---
# ── Identity ─────────────────────────────────────────────────────────
name: breach-notification-audit
description: >-
  Audit one or more existing breach-notification@1 markdowns against breach-notification_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after breach-notification-author. Use when user asks to "audit this breach notification" or "check the breach notification". Do NOT use for "draft a new breach notification" (use breach-notification-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: breach-notification@1
  cyberos-rubric-version: breach-notification_rubric@1.0
...
depends_on_contracts:
  - id:        breach-notification
    ...
    pin_path:  cyberos/skill/contracts/breach-notification/
  - id:        nats-subjects
    version:   v1
...
# breach-notification-audit — BREACH-NOTIFICATION auditor

> Standalone trigger that runs `breach-notification_rubric@1.0` against one or
> more existing `breach-notification@1` markdowns and writes a sibling
> `breach-notification.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`breach-notification-author`](../breach-notification-author/SKILL.md).

`prompt_revision: breach-notification_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `BREACH-NOTIFICATION`s."
- "Has `BREACH-NOTIFICATION-007` changed since the last audit?"
- "Tell me which `BREACH-NOTIFICATION`s would fail acceptance today."

Also invoked automatically by the supervisor when `breach-notification-author`'s output envelope sets `next_skill_recommendation: breach-notification-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        breach-notification-audit
skill_version:                   1.0.0
prompt_revision:                 breach-notification_audit@1.0.0
template_version:                breach-notification@1   (loaded from cyberos/skill/contracts/breach-notification/template.md)
audit_rubric_version:            breach-notification_rubric@1.0
...
```json
{
  "artefact_paths": ["./breach-notifications/BREACH-NOTIFICATION-001-foo.md", "./breach-notifications/BREACH-NOTIFICATION-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "breach-notification-author",
    "manifest_path": "./breach-notifications/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./breach-notifications/BREACH-NOTIFICATION-001-foo.md", "audit_path": "./breach-notifications/BREACH-NOTIFICATION-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./breach-notifications/BREACH-NOTIFICATION-002-bar.md", "audit_path": "./breach-notifications/BREACH-NOTIFICATION-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   breach-notification-audit
Input:
  artefact_paths:  [./team-a/BREACH-NOTIFICATION-001-something.md, ./team-b/BREACH-NOTIFICATION-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
