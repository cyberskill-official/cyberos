# `breach_notification_rubric@1.0` — machine-checkable Breach Notification rubric
...
| `FM-004` | `template` equals `breach-notification@1` | error | true |
...
Per the `contracts/breach-notification/template.md` H2 list. Each H2 in the template MUST be present + non-empty in the authored artefact. See template for the canonical headings.

| rule_id | Check | Severity |
| ------- | ----- | -------- |
| `SEC-001` | Every H2 declared in `contracts/breach-notification/template.md` is present in the authored artefact | error |
...
- `cyberos/skill/contracts/breach-notification/CONTRACT.md` + `template.md`
