---
# ── Identity ─────────────────────────────────────────────────────────
name: safety-report-author
description: >-
  Author the quarterly clinical-safety report (executive summary + AE aggregation + signal detection + SAE inventory + risk-benefit assessment + regulatory submissions). ICH E2D PSUR + FDA MedWatch + EMA EudraVigilance compliant. Chief Medical Officer authored. Use when user asks to "draft a safety report" or "create the safety report". Do NOT use for "audit existing safety report" (use safety-report-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: safety-report_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['safety-report-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        safety-report
    ...
    pin_path:  cyberos/skill/contracts/safety-report/
  - id:        nats-subjects
    version:   v1
...
# safety-report-author — SAFETY-REPORT generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `safety-report@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`safety-report-audit`](../safety-report-audit/SKILL.md) by default.

`prompt_revision: safety-report_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a SAFETY-REPORT."
- "Generate v2 of the SAFETY-REPORT from the updated source."
- "Draft a SAFETY-REPORT for <project>."

If the user asks to *audit an existing SAFETY-REPORT*, route to `safety-report-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        safety-report-author
skill_version:                   1.0.0
prompt_revision:                 safety-report_author@1.0.0
template_version:                safety-report@1   (loaded from cyberos/skill/contracts/safety-report/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  SAFETY-REPORT-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./safety-reports/",
  "manifest_path": "./safety-reports/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "SAFETY-REPORT-001", "path": "./safety-reports/SAFETY-REPORT-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "safety-report-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/safety-report/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `safety-report_author.safety-report_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `safety-report-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `safety-report-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   safety-report-author
Input:
