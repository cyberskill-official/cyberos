...
metadata:
  ...
  module: skill
  stage: cross
  cyberos-template: penetration-test-report@1
...
depends_on_contracts:
  - id:        pen-test-report
    ...
    pin_path:  cyberos/skill/contracts/penetration-test-report/
  - id:        nats-subjects
    version:   v1
...
# penetration-test-report-audit — PEN_TEST_REPORT auditor

> Standalone trigger that runs `pen-test-report_rubric@1.0` against one or
> more existing `penetration-test-report@1` markdowns and writes a sibling
...
> Chains naturally after [`penetration-test-report-author`](../penetration-test-report-author/SKILL.md).
...
Also invoked automatically by the supervisor when `penetration-test-report-author`'s output envelope sets `next_skill_recommendation: penetration-test-report-audit` (the default chain).
...
```json
{
  ...
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "penetration-test-report-author",
...
```
Persona: cuo-cpo
Skill:   penetration-test-report-audit
Input:
