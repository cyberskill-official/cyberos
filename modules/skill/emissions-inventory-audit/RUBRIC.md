# `emissions_inventory_rubric@1.0` — machine-checkable Emissions Inventory rubric
...
| `FM-004` | `template` equals `emissions-inventory@1` | error | true |
...
Per the `contracts/emissions-inventory/template.md` H2 list — every H2 in template MUST be present + non-empty in the authored artefact.

| rule_id | Check | Severity |
| ------- | ----- | -------- |
| `SEC-001` | Every H2 declared in `contracts/emissions-inventory/template.md` is present in the authored artefact | error |
...
- `cyberos/skill/contracts/emissions-inventory/CONTRACT.md` + `template.md`
