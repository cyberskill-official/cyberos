---
# ── Identity ─────────────────────────────────────────────────────────
name: emissions-inventory-audit
description: >-
  Audit one or more existing emissions-inventory@1 markdowns against emissions-inventory_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after emissions-inventory-author. Use when user asks to "audit this emissions inventory" or "check the emissions inventory". Do NOT use for "draft a new emissions inventory" (use emissions-inventory-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: emissions-inventory@1
  cyberos-rubric-version: emissions-inventory_rubric@1.0
...
# emissions-inventory-audit — EMISSIONS-INVENTORY auditor

> Standalone trigger that runs `emissions-inventory_rubric@1.0` against one or
> more existing `emissions-inventory@1` markdowns and writes a sibling
> `emissions-inventory.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`emissions-inventory-author`](../emissions-inventory-author/SKILL.md).

`prompt_revision: emissions-inventory_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `EMISSIONS-INVENTORY`s."
- "Has `EMISSIONS-INVENTORY-007` changed since the last audit?"
- "Tell me which `EMISSIONS-INVENTORY`s would fail acceptance today."

Also invoked automatically by the supervisor when `emissions-inventory-author`'s output envelope sets `next_skill_recommendation: emissions-inventory-audit` (the default chain).
...
```json
{
  "artefact_paths": ["./emissions-inventorys/EMISSIONS-INVENTORY-001-foo.md", "./emissions-inventorys/EMISSIONS-INVENTORY-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "emissions-inventory-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./emissions-inventorys/EMISSIONS-INVENTORY-001-foo.md", "audit_path": "./emissions-inventorys/EMISSIONS-INVENTORY-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./emissions-inventorys/EMISSIONS-INVENTORY-002-bar.md", "audit_path": "./emissions-inventorys/EMISSIONS-INVENTORY-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   emissions-inventory-audit
Input:
  artefact_paths:  [./team-a/EMISSIONS-INVENTORY-001-something.md, ./team-b/EMISSIONS-INVENTORY-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
