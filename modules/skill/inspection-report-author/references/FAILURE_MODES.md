...
## §6  Skill-specific codes (inspection-report-author)

Derived from measured failures in the inspect-harden calibration (GUIDELINE §6).

| Code | Reason | Recovery |
|---|---|---|
| `IRA-FM-001` | Absence claim rested on one search shape (INS-EVD-7 / INS-EVD-9). | Re-run with two differently shaped searches + search space + sensitivity. |
| `IRA-FM-002` | Conclusion drawn from a truncated listing (INS-DISC-13). | Enumerate the whole tree before searching any part; never conclude from truncated output. |
| `IRA-FM-003` | Confidence conflated truth with significance (INS-EVD-10). | Move significance to severity; keep confidence about whether the finding is true. |
| `IRA-FM-004` | Compensating layer not read before defect asserted (INS-XLAYER-2). | Read the compensating layer; record reversal if the defect is already mitigated. |
| `IRA-FM-005` | Ledger row count ≠ taxonomy for declared INSPECT-SPEC (69 vs 75). | Regenerate ledger from findings; match taxonomy for the declared spec version. |
