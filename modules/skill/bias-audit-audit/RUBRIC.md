# `bias_audit_rubric@1.0` — machine-checkable Bias Audit rubric
...
| `FM-004` | `template` equals `bias-audit@1` | error | true |
...
Per the `contracts/bias-audit/template.md` H2 list. Each H2 in the template MUST be present + non-empty in the authored artefact. See template for the canonical headings.

| rule_id | Check | Severity |
| ------- | ----- | -------- |
| `SEC-001` | Every H2 declared in `contracts/bias-audit/template.md` is present in the authored artefact | error |
