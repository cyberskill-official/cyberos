---
# ── Identity ─────────────────────────────────────────────────────────
name: bias-audit-audit
description: >-
  Audit one or more existing bias-audit@1 markdowns against bias-audit_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after bias-audit-author. Use when user asks to "audit this bias audit" or "check the bias audit". Do NOT use for "draft a new bias audit" (use bias-audit-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: bias-audit@1
  cyberos-rubric-version: bias-audit_rubric@1.0
...
# bias-audit-audit — BIAS-AUDIT auditor

> Standalone trigger that runs `bias-audit_rubric@1.0` against one or
> more existing `bias-audit@1` markdowns and writes a sibling
> `bias-audit.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`bias-audit-author`](../bias-audit-author/SKILL.md).

`prompt_revision: bias-audit_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `BIAS-AUDIT`s."
- "Has `BIAS-AUDIT-007` changed since the last audit?"
- "Tell me which `BIAS-AUDIT`s would fail acceptance today."

Also invoked automatically by the supervisor when `bias-audit-author`'s output envelope sets `next_skill_recommendation: bias-audit-audit` (the default chain).
...
```json
{
  "artefact_paths": ["./bias-audits/BIAS-AUDIT-001-foo.md", "./bias-audits/BIAS-AUDIT-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "bias-audit-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./bias-audits/BIAS-AUDIT-001-foo.md", "audit_path": "./bias-audits/BIAS-AUDIT-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./bias-audits/BIAS-AUDIT-002-bar.md", "audit_path": "./bias-audits/BIAS-AUDIT-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   bias-audit-audit
Input:
  artefact_paths:  [./team-a/BIAS-AUDIT-001-something.md, ./team-b/BIAS-AUDIT-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
