---
# ── Identity ─────────────────────────────────────────────────────────
name: kri-dashboard-audit
description: |
  Audit one or more existing kri-dashboard@1 markdowns against
  ...
  kri-dashboard-author.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: <SDP §2 stage letter or "cross">
  cyberos-template: kri-dashboard@1
...
# kri-dashboard-audit — KRI-DASHBOARD auditor

> Standalone trigger that runs `kri-dashboard_rubric@1.0` against one or
> more existing `kri-dashboard@1` markdowns and writes a sibling
...
> Chains naturally after [`kri-dashboard-author`](../kri-dashboard-author/SKILL.md).
...
Also invoked automatically by the supervisor when `kri-dashboard-author`'s output envelope sets `next_skill_recommendation: kri-dashboard-audit` (the default chain).
...
```json
{
  ...
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "kri-dashboard-author",
