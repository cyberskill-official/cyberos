---
# ── Identity ─────────────────────────────────────────────────────────
name: strategy-document-audit
description: >-
  Audit one or more existing strategy-document@1 markdowns against strategy-doc_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after strategy-document-author. Use when user asks to "audit this strategy document" or "check the strategy document". Do NOT use for "draft a new strategy document" (use strategy-document-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: strategy-document@1
  cyberos-rubric-version: strategy-doc_rubric@1.0
...
# strategy-document-audit — STRATEGY-DOC auditor

> Standalone trigger that runs `strategy-doc_rubric@1.0` against one or
> more existing `strategy-document@1` markdowns and writes a sibling
> `strategy-doc.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`strategy-document-author`](../strategy-document-author/SKILL.md).

`prompt_revision: strategy-doc_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `STRATEGY-DOC`s."
- "Has `STRATEGY-DOC-007` changed since the last audit?"
- "Tell me which `STRATEGY-DOC`s would fail acceptance today."

Also invoked automatically by the supervisor when `strategy-document-author`'s output envelope sets `next_skill_recommendation: strategy-document-audit` (the default chain).
...
audit_rubric_version:            strategy-doc_rubric@1.0
...
```json
{
  "artefact_paths": ["./strategy-docs/STRATEGY-DOC-001-foo.md", "./strategy-docs/STRATEGY-DOC-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "strategy-document-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./strategy-docs/STRATEGY-DOC-001-foo.md", "audit_path": "./strategy-docs/STRATEGY-DOC-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./strategy-docs/STRATEGY-DOC-002-bar.md", "audit_path": "./strategy-docs/STRATEGY-DOC-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   strategy-document-audit
Input:
  artefact_paths:  [./team-a/STRATEGY-DOC-001-something.md, ./team-b/STRATEGY-DOC-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
