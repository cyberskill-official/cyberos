# `strategy_doc_rubric@1.0` — machine-checkable Strategy Document rubric
...
| `FM-004` | `template` equals `strategy-document@1` | error | true |
...
Per the `contracts/strategy-document/template.md` H2 list — every H2 in template MUST be present + non-empty in the authored artefact.

| rule_id | Check | Severity |
| ------- | ----- | -------- |
| `SEC-001` | Every H2 declared in `contracts/strategy-document/template.md` is present in the authored artefact | error |
...
- `cyberos/skill/contracts/strategy-document/CONTRACT.md` + `template.md`
