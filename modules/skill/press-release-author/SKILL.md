...
description: >-
  Author a per-announcement press release (headline + dateline + lede + supporting paragraphs + spokesperson quotes + boilerplate + media contact + embargo terms). AP Stylebook conventions. CCO-Communications authored. Use when user asks to "draft a press release" or "create the press release". Do NOT use for "audit existing press release" (use press-release-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: press-release_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['press-release-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        press-release
    ...
    pin_path:  cyberos/skill/contracts/press-release/
  - id:        nats-subjects
    version:   v1
...
# press-release-author — PRESS-RELEASE generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `press-release@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`press-release-audit`](../press-release-audit/SKILL.md) by default.

`prompt_revision: press-release_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a PRESS-RELEASE."
- "Generate v2 of the PRESS-RELEASE from the updated source."
- "Draft a PRESS-RELEASE for <project>."

If the user asks to *audit an existing PRESS-RELEASE*, route to `press-release-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 press-release_author@1.0.0
template_version:                press-release@1   (loaded from cyberos/skill/contracts/press-release/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  PRESS-RELEASE-{NNN}-{slug}.md
...
```json
{
  ...
  "manifest_path": "./press-releases/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "PRESS-RELEASE-001", "path": "./press-releases/PRESS-RELEASE-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "press-release-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/press-release/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `press-release_author.press-release_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `press-release-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `press-release-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   press-release-author
Input:
