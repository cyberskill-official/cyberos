...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['penetration-test-report-audit']; empty list disables chaining
...
# penetration-test-report-author — PEN_TEST_REPORT generator
...
> into [`penetration-test-report-audit`](../penetration-test-report-audit/SKILL.md) by default.
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "penetration-test-report-audit"
}
```
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `penetration-test-report-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `penetration-test-report-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   penetration-test-report-author
Input:
