...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['soc2-evidence-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        soc2-evidence
    ...
    pin_path:  cyberos/skill/contracts/soc2-evidence/
  - id:        nats-subjects
    version:   v1
...
# soc2-evidence-author — SOC2_EVIDENCE generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `soc2-evidence@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`soc2-evidence-audit`](../soc2-evidence-audit/SKILL.md) by default.

`prompt_revision: soc2-evidence_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a SOC2_EVIDENCE."
- "Generate v2 of the SOC2_EVIDENCE from the updated source."
- "Draft a SOC2_EVIDENCE for <project>."

If the user asks to *audit an existing SOC2_EVIDENCE*, route to `soc2-evidence-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  SOC2_EVIDENCE-{NNN}-{slug}.md
...
```json
{
  ...
  "artefacts_written": [
    {"id": "SOC2_EVIDENCE-001", "path": "./soc2-evidences/SOC2_EVIDENCE-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "soc2-evidence-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/soc2-evidence/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `soc2-evidence_author.soc2-evidence_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `soc2-evidence-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `soc2-evidence-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   soc2-evidence-author
Input:
