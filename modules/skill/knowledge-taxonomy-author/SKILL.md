---
# ── Identity ─────────────────────────────────────────────────────────
name: knowledge-taxonomy-author
description: >-
  Author the quarterly knowledge-taxonomy refresh (tag hierarchy + categorisation rules + metadata schema + search-evolution roadmap + freshness audit policy + coverage gap analysis). ANSI Z39.19 thesaurus standard. Chief Knowledge Officer authored. Use when user asks to "draft a knowledge taxonomy" or "create the knowledge taxonomy". Do NOT use for "audit existing knowledge taxonomy" (use knowledge-taxonomy-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: knowledge-taxonomy_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['knowledge-taxonomy-audit']; empty list disables chaining
...
# knowledge-taxonomy-author — KNOWLEDGE-TAXONOMY generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `knowledge-taxonomy@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`knowledge-taxonomy-audit`](../knowledge-taxonomy-audit/SKILL.md) by default.

`prompt_revision: knowledge-taxonomy_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a KNOWLEDGE-TAXONOMY."
- "Generate v2 of the KNOWLEDGE-TAXONOMY from the updated source."
- "Draft a KNOWLEDGE-TAXONOMY for <project>."

If the user asks to *audit an existing KNOWLEDGE-TAXONOMY*, route to `knowledge-taxonomy-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  KNOWLEDGE-TAXONOMY-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./knowledge-taxonomys/",
...
```json
{
  ...
  "artefacts_written": [
    {"id": "KNOWLEDGE-TAXONOMY-001", "path": "./knowledge-taxonomys/KNOWLEDGE-TAXONOMY-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "knowledge-taxonomy-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/knowledge-taxonomy/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `knowledge-taxonomy_author.knowledge-taxonomy_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `knowledge-taxonomy-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `knowledge-taxonomy-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   knowledge-taxonomy-author
Input:
