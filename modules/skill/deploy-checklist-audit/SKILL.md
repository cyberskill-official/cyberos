---
# ── Identity ─────────────────────────────────────────────────────────
name: deploy-checklist-audit
description: |
  Audit one or more existing deploy-checklist@1 markdowns against
...
metadata:
  ...
  cyberos-rubric-version: deploy_checklist_rubric@1.0
...
# deploy-checklist-audit — DEPLOY-CHECKLIST auditor

> Standalone trigger that runs `deploy-checklist_rubric@1.0` against one or
> more existing `deploy-checklist@1` markdowns and writes a sibling
> `deploy-checklist.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`deploy-checklist-author`](../deploy-checklist-author/SKILL.md).

`prompt_revision: deploy-checklist_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `DEPLOY-CHECKLIST`s."
- "Has `DEPLOY-CHECKLIST-007` changed since the last audit?"
- "Tell me which `DEPLOY-CHECKLIST`s would fail acceptance today."

Also invoked automatically by the supervisor when `deploy-checklist-author`'s output envelope sets `next_skill_recommendation: deploy-checklist-audit` (the default chain).
...
```json
{
  "artefact_paths": ["./deploy-checklists/DEPLOY-CHECKLIST-001-foo.md", "./deploy-checklists/DEPLOY-CHECKLIST-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "deploy-checklist-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./deploy-checklists/DEPLOY-CHECKLIST-001-foo.md", "audit_path": "./deploy-checklists/DEPLOY-CHECKLIST-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./deploy-checklists/DEPLOY-CHECKLIST-002-bar.md", "audit_path": "./deploy-checklists/DEPLOY-CHECKLIST-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   deploy-checklist-audit
Input:
  artefact_paths:  [./team-a/DEPLOY-CHECKLIST-001-something.md, ./team-b/DEPLOY-CHECKLIST-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
