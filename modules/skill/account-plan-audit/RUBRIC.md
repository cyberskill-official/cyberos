# `account_plan_rubric@1.0` — machine-checkable Account Plan rubric
...
| `FM-004` | `template` equals `account-plan@1` | error | true |
...
Per the `contracts/account-plan/template.md` H2 list. Each H2 in the template MUST be present + non-empty in the authored artefact. See template for the canonical headings.

| rule_id | Check | Severity |
| ------- | ----- | -------- |
| `SEC-001` | Every H2 declared in `contracts/account-plan/template.md` is present in the authored artefact | error |
...
- `cyberos/skill/contracts/account-plan/CONTRACT.md` + `template.md`
