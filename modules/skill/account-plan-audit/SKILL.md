---
# ── Identity ─────────────────────────────────────────────────────────
name: account-plan-audit
description: >-
  Audit one or more existing account-plan@1 markdowns against account-plan_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after account-plan-author. Use when user asks to "audit this account plan" or "check the account plan". Do NOT use for "draft a new account plan" (use account-plan-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: account-plan@1
  cyberos-rubric-version: account-plan_rubric@1.0
...
depends_on_contracts:
  - id:        account-plan
    ...
    pin_path:  cyberos/skill/contracts/account-plan/
  - id:        nats-subjects
    version:   v1
...
# account-plan-audit — ACCOUNT-PLAN auditor

> Standalone trigger that runs `account-plan_rubric@1.0` against one or
> more existing `account-plan@1` markdowns and writes a sibling
> `account-plan.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`account-plan-author`](../account-plan-author/SKILL.md).

`prompt_revision: account-plan_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `ACCOUNT-PLAN`s."
- "Has `ACCOUNT-PLAN-007` changed since the last audit?"
- "Tell me which `ACCOUNT-PLAN`s would fail acceptance today."

Also invoked automatically by the supervisor when `account-plan-author`'s output envelope sets `next_skill_recommendation: account-plan-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        account-plan-audit
skill_version:                   1.0.0
prompt_revision:                 account-plan_audit@1.0.0
template_version:                account-plan@1   (loaded from cyberos/skill/contracts/account-plan/template.md)
audit_rubric_version:            account-plan_rubric@1.0
...
```json
{
  "artefact_paths": ["./account-plans/ACCOUNT-PLAN-001-foo.md", "./account-plans/ACCOUNT-PLAN-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "account-plan-author",
    "manifest_path": "./account-plans/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./account-plans/ACCOUNT-PLAN-001-foo.md", "audit_path": "./account-plans/ACCOUNT-PLAN-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./account-plans/ACCOUNT-PLAN-002-bar.md", "audit_path": "./account-plans/ACCOUNT-PLAN-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   account-plan-audit
Input:
  artefact_paths:  [./team-a/ACCOUNT-PLAN-001-something.md, ./team-b/ACCOUNT-PLAN-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
