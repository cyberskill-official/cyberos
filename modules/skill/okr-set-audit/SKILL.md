...
name: okr-set-audit
description: |
  Audit one or more existing okr-set@1 markdowns against
...
# okr-set-audit — OKR-SET auditor

> Standalone trigger that runs `okr-set_rubric@1.0` against one or
> more existing `okr-set@1` markdowns and writes a sibling
...
> Chains naturally after [`okr-set-author`](../okr-set-author/SKILL.md).
...
Also invoked automatically by the supervisor when `okr-set-author`'s output envelope sets `next_skill_recommendation: okr-set-audit` (the default chain).
