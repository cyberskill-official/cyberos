...
description: >-
  Audit one or more existing threat-model@1 markdowns against threat-model_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after threat-model-author. Use when user asks to "audit this threat model" or "check the threat model". Do NOT use for "draft a new threat model" (use threat-model-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-version: threat_model_rubric@1.0
...
# threat-model-audit — THREAT-MODEL auditor

> Standalone trigger that runs `threat-model_rubric@1.0` against one or
> more existing `threat-model@1` markdowns and writes a sibling
> `threat-model.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`threat-model-author`](../threat-model-author/SKILL.md).

`prompt_revision: threat-model_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `THREAT-MODEL`s."
- "Has `THREAT-MODEL-007` changed since the last audit?"
- "Tell me which `THREAT-MODEL`s would fail acceptance today."

Also invoked automatically by the supervisor when `threat-model-author`'s output envelope sets `next_skill_recommendation: threat-model-audit` (the default chain).
...
audit_rubric_version:            threat-model_rubric@1.0
...
```json
{
  "artefact_paths": ["./threat-models/THREAT-MODEL-001-foo.md", "./threat-models/THREAT-MODEL-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "threat-model-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./threat-models/THREAT-MODEL-001-foo.md", "audit_path": "./threat-models/THREAT-MODEL-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./threat-models/THREAT-MODEL-002-bar.md", "audit_path": "./threat-models/THREAT-MODEL-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   threat-model-audit
Input:
  artefact_paths:  [./team-a/THREAT-MODEL-001-something.md, ./team-b/THREAT-MODEL-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
