---
# ── Identity ─────────────────────────────────────────────────────────
name: transparency-report-author
description: >-
  Author the quarterly transparency report (data requests received + AI-system disclosures + content-moderation actions + government info-requests + methodology + contact). Google + Microsoft + Meta transparency-report exemplars. Chief Trust Officer authored. Use when user asks to "draft a transparency report" or "create the transparency report". Do NOT use for "audit existing transparency report" (use transparency-report-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: transparency-report_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['transparency-report-audit']; empty list disables chaining
...
# transparency-report-author — TRANSPARENCY-REPORT generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `transparency-report@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`transparency-report-audit`](../transparency-report-audit/SKILL.md) by default.

`prompt_revision: transparency-report_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a TRANSPARENCY-REPORT."
- "Generate v2 of the TRANSPARENCY-REPORT from the updated source."
- "Draft a TRANSPARENCY-REPORT for <project>."

If the user asks to *audit an existing TRANSPARENCY-REPORT*, route to `transparency-report-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        transparency-report-author
skill_version:                   1.0.0
prompt_revision:                 transparency-report_author@1.0.0
...
naming_pattern:                  TRANSPARENCY-REPORT-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./transparency-reports/",
...
```json
{
  ...
  "artefacts_written": [
    {"id": "TRANSPARENCY-REPORT-001", "path": "./transparency-reports/TRANSPARENCY-REPORT-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "transparency-report-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/transparency-report/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `transparency-report_author.transparency-report_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
- If chained to `transparency-report-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `transparency-report-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   transparency-report-author
Input:
