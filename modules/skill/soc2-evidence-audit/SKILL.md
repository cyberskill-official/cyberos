...
description: >-
  ...
  soc2-evidence-author.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: soc2-evidence@1
...
> Standalone trigger that runs `soc2-evidence_rubric@1.0` against one or
> more existing `soc2-evidence@1` markdowns and writes a sibling
> `soc2-evidence.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`soc2-evidence-author`](../soc2-evidence-author/SKILL.md).
...
- "Audit these existing `SOC2_EVIDENCE`s."
- "Has `SOC2_EVIDENCE-007` changed since the last audit?"
- "Tell me which `SOC2_EVIDENCE`s would fail acceptance today."
