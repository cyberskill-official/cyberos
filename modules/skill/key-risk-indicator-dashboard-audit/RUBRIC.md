# `kri_dashboard_rubric@1.0` — machine-checkable KRI Dashboard rubric
...
| `FM-004` | `template` equals `key-risk-indicator-dashboard@1` | error | true |
...
Per the `contracts/key-risk-indicator-dashboard/template.md` H2 list — every H2 in template MUST be present + non-empty in the authored artefact.

| rule_id | Check | Severity |
| ------- | ----- | -------- |
| `SEC-001` | Every H2 declared in `contracts/key-risk-indicator-dashboard/template.md` is present in the authored artefact | error |
...
- `cyberos/skill/contracts/key-risk-indicator-dashboard/CONTRACT.md` + `template.md`
