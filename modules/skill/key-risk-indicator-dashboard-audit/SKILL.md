---
# ── Identity ─────────────────────────────────────────────────────────
name: key-risk-indicator-dashboard-audit
description: >-
  Audit one or more existing key-risk-indicator-dashboard@1 markdowns against kri-dashboard_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after key-risk-indicator-dashboard-author. Use when user asks to "audit this key risk indicator dashboard" or "check the key risk indicator dashboard". Do NOT use for "draft a new key risk indicator dashboard" (use key-risk-indicator-dashboard-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: key-risk-indicator-dashboard@1
  cyberos-rubric-version: kri-dashboard_rubric@1.0
...
depends_on_contracts:
  - id:        kri-dashboard
    ...
    pin_path:  cyberos/skill/contracts/key-risk-indicator-dashboard/
  - id:        nats-subjects
    version:   v1
...
# key-risk-indicator-dashboard-audit — KRI-DASHBOARD auditor

> Standalone trigger that runs `kri-dashboard_rubric@1.0` against one or
> more existing `key-risk-indicator-dashboard@1` markdowns and writes a sibling
> `kri-dashboard.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`key-risk-indicator-dashboard-author`](../key-risk-indicator-dashboard-author/SKILL.md).

`prompt_revision: kri-dashboard_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `KRI-DASHBOARD`s."
- "Has `KRI-DASHBOARD-007` changed since the last audit?"
- "Tell me which `KRI-DASHBOARD`s would fail acceptance today."

Also invoked automatically by the supervisor when `key-risk-indicator-dashboard-author`'s output envelope sets `next_skill_recommendation: key-risk-indicator-dashboard-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        key-risk-indicator-dashboard-audit
skill_version:                   1.0.0
prompt_revision:                 kri-dashboard_audit@1.0.0
template_version:                key-risk-indicator-dashboard@1   (loaded from cyberos/skill/contracts/key-risk-indicator-dashboard/template.md)
audit_rubric_version:            kri-dashboard_rubric@1.0
...
```json
{
  "artefact_paths": ["./kri-dashboards/KRI-DASHBOARD-001-foo.md", "./kri-dashboards/KRI-DASHBOARD-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "key-risk-indicator-dashboard-author",
    "manifest_path": "./kri-dashboards/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./kri-dashboards/KRI-DASHBOARD-001-foo.md", "audit_path": "./kri-dashboards/KRI-DASHBOARD-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./kri-dashboards/KRI-DASHBOARD-002-bar.md", "audit_path": "./kri-dashboards/KRI-DASHBOARD-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   key-risk-indicator-dashboard-audit
Input:
  artefact_paths:  [./team-a/KRI-DASHBOARD-001-something.md, ./team-b/KRI-DASHBOARD-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
