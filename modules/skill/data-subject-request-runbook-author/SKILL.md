---
# ── Identity ─────────────────────────────────────────────────────────
name: data-subject-request-runbook-author
description: >-
  Author the DSR runbook (intake workflow + identity verification + data-search procedure + response-template library + regulatory-deadline tracking + DPO escalation). CPO-Privacy authored. Covers GDPR Articles 15-22 + Vietnam Decree 13/2023 DSR rights. Use when user asks to "draft a data subject request runbook" or "create the data subject request runbook". Do NOT use for "audit existing data subject request runbook" (use data-subject-request-runbook-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: dsr-runbook_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['data-subject-request-runbook-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        dsr-runbook
    ...
    pin_path:  cyberos/skill/contracts/data-subject-request-runbook/
  - id:        nats-subjects
    version:   v1
...
# data-subject-request-runbook-author — DSR-RUNBOOK generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `data-subject-request-runbook@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`data-subject-request-runbook-audit`](../data-subject-request-runbook-audit/SKILL.md) by default.

`prompt_revision: dsr-runbook_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a DSR-RUNBOOK."
- "Generate v2 of the DSR-RUNBOOK from the updated source."
- "Draft a DSR-RUNBOOK for <project>."

If the user asks to *audit an existing DSR-RUNBOOK*, route to `data-subject-request-runbook-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 dsr-runbook_author@1.0.0
template_version:                data-subject-request-runbook@1   (loaded from cyberos/skill/contracts/data-subject-request-runbook/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  DSR-RUNBOOK-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./dsr-runbooks/",
  "manifest_path": "./dsr-runbooks/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "DSR-RUNBOOK-001", "path": "./dsr-runbooks/DSR-RUNBOOK-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "data-subject-request-runbook-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/data-subject-request-runbook/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `dsr-runbook_author.dsr-runbook_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `data-subject-request-runbook-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `data-subject-request-runbook-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   data-subject-request-runbook-author
Input:
