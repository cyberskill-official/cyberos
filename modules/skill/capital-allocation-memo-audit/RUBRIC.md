# `cap_alloc_memo_rubric@1.0` — machine-checkable Capital Allocation Memo rubric
...
| `FM-004` | `template` equals `capital-allocation-memo@1` | error | true |
...
Per the `contracts/capital-allocation-memo/template.md` H2 list. Each H2 in the template MUST be present + non-empty in the authored artefact. See template for the canonical headings.

| rule_id | Check | Severity |
| ------- | ----- | -------- |
| `SEC-001` | Every H2 declared in `contracts/capital-allocation-memo/template.md` is present in the authored artefact | error |
...
- `cyberos/skill/contracts/capital-allocation-memo/CONTRACT.md` + `template.md`
