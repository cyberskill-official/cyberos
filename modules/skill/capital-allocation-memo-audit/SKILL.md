---
# ── Identity ─────────────────────────────────────────────────────────
name: capital-allocation-memo-audit
description: >-
  Audit one or more existing capital-allocation-memo@1 markdowns against cap-alloc-memo_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after capital-allocation-memo-author. Use when user asks to "audit this capital allocation memo" or "check the capital allocation memo". Do NOT use for "draft a new capital allocation memo" (use capital-allocation-memo-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: capital-allocation-memo@1
  cyberos-rubric-version: cap-alloc-memo_rubric@1.0
...
depends_on_contracts:
  - id:        cap-alloc-memo
    ...
    pin_path:  cyberos/skill/contracts/capital-allocation-memo/
  - id:        nats-subjects
    version:   v1
...
# capital-allocation-memo-audit — CAP-ALLOC-MEMO auditor

> Standalone trigger that runs `cap-alloc-memo_rubric@1.0` against one or
> more existing `capital-allocation-memo@1` markdowns and writes a sibling
> `cap-alloc-memo.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`capital-allocation-memo-author`](../capital-allocation-memo-author/SKILL.md).

`prompt_revision: cap-alloc-memo_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `CAP-ALLOC-MEMO`s."
- "Has `CAP-ALLOC-MEMO-007` changed since the last audit?"
- "Tell me which `CAP-ALLOC-MEMO`s would fail acceptance today."

Also invoked automatically by the supervisor when `capital-allocation-memo-author`'s output envelope sets `next_skill_recommendation: capital-allocation-memo-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        capital-allocation-memo-audit
skill_version:                   1.0.0
prompt_revision:                 cap-alloc-memo_audit@1.0.0
template_version:                capital-allocation-memo@1   (loaded from cyberos/skill/contracts/capital-allocation-memo/template.md)
audit_rubric_version:            cap-alloc-memo_rubric@1.0
...
```json
{
  "artefact_paths": ["./cap-alloc-memos/CAP-ALLOC-MEMO-001-foo.md", "./cap-alloc-memos/CAP-ALLOC-MEMO-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "capital-allocation-memo-author",
    "manifest_path": "./cap-alloc-memos/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./cap-alloc-memos/CAP-ALLOC-MEMO-001-foo.md", "audit_path": "./cap-alloc-memos/CAP-ALLOC-MEMO-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./cap-alloc-memos/CAP-ALLOC-MEMO-002-bar.md", "audit_path": "./cap-alloc-memos/CAP-ALLOC-MEMO-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   capital-allocation-memo-audit
Input:
  artefact_paths:  [./team-a/CAP-ALLOC-MEMO-001-something.md, ./team-b/CAP-ALLOC-MEMO-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
