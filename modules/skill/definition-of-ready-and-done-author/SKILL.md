...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['definition-of-ready-and-done-audit']; empty list disables chaining
...
# definition-of-ready-and-done-author — DOR-DOD generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `definition-of-ready-and-done@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`definition-of-ready-and-done-audit`](../definition-of-ready-and-done-audit/SKILL.md) by default.

`prompt_revision: dor-dod_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a DOR-DOD."
- "Generate v2 of the DOR-DOD from the updated source."
- "Draft a DOR-DOD for <project>."

If the user asks to *audit an existing DOR-DOD*, route to `definition-of-ready-and-done-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  DOR-DOD-{NNN}-{slug}.md
...
```json
{
  ...
  "artefacts_written": [
    {"id": "DOR-DOD-001", "path": "./dor-dods/DOR-DOD-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "definition-of-ready-and-done-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/definition-of-ready-and-done/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `dor-dod_author.dor-dod_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `definition-of-ready-and-done-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `definition-of-ready-and-done-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   definition-of-ready-and-done-author
Input:
