...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['objectives-and-key-results-set-audit']; empty list disables chaining
...
# objectives-and-key-results-set-author — OKR-SET generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `objectives-and-key-results-set@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`objectives-and-key-results-set-audit`](../objectives-and-key-results-set-audit/SKILL.md) by default.

`prompt_revision: okr-set_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a OKR-SET."
- "Generate v2 of the OKR-SET from the updated source."
- "Draft a OKR-SET for <project>."

If the user asks to *audit an existing OKR-SET*, route to `objectives-and-key-results-set-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  OKR-SET-{NNN}-{slug}.md
...
```json
{
  ...
  "artefacts_written": [
    {"id": "OKR-SET-001", "path": "./okr-sets/OKR-SET-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "objectives-and-key-results-set-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/objectives-and-key-results-set/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `okr-set_author.okr-set_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `objectives-and-key-results-set-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `objectives-and-key-results-set-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   objectives-and-key-results-set-author
Input:
