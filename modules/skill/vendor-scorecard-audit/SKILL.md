---
# ── Identity ─────────────────────────────────────────────────────────
name: vendor-scorecard-audit
description: >-
  Audit one or more existing vendor-scorecard@1 markdowns against vendor-scorecard_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after vendor-scorecard-author. Use when user asks to "audit this vendor scorecard" or "check the vendor scorecard". Do NOT use for "draft a new vendor scorecard" (use vendor-scorecard-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: vendor-scorecard@1
  cyberos-rubric-version: vendor-scorecard_rubric@1.0
...
depends_on_contracts:
  - id:        vendor-scorecard
    ...
    pin_path:  cyberos/skill/contracts/vendor-scorecard/
  - id:        nats-subjects
    version:   v1
...
# vendor-scorecard-audit — VENDOR-SCORECARD auditor

> Standalone trigger that runs `vendor-scorecard_rubric@1.0` against one or
> more existing `vendor-scorecard@1` markdowns and writes a sibling
> `vendor-scorecard.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`vendor-scorecard-author`](../vendor-scorecard-author/SKILL.md).

`prompt_revision: vendor-scorecard_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `VENDOR-SCORECARD`s."
- "Has `VENDOR-SCORECARD-007` changed since the last audit?"
- "Tell me which `VENDOR-SCORECARD`s would fail acceptance today."

Also invoked automatically by the supervisor when `vendor-scorecard-author`'s output envelope sets `next_skill_recommendation: vendor-scorecard-audit` (the default chain).
...
skill_version:                   1.0.0
prompt_revision:                 vendor-scorecard_audit@1.0.0
template_version:                vendor-scorecard@1   (loaded from cyberos/skill/contracts/vendor-scorecard/template.md)
audit_rubric_version:            vendor-scorecard_rubric@1.0
...
```json
{
  "artefact_paths": ["./vendor-scorecards/VENDOR-SCORECARD-001-foo.md", "./vendor-scorecards/VENDOR-SCORECARD-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "vendor-scorecard-author",
    "manifest_path": "./vendor-scorecards/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./vendor-scorecards/VENDOR-SCORECARD-001-foo.md", "audit_path": "./vendor-scorecards/VENDOR-SCORECARD-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./vendor-scorecards/VENDOR-SCORECARD-002-bar.md", "audit_path": "./vendor-scorecards/VENDOR-SCORECARD-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   vendor-scorecard-audit
Input:
  artefact_paths:  [./team-a/VENDOR-SCORECARD-001-something.md, ./team-b/VENDOR-SCORECARD-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
