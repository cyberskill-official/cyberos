# `vendor_scorecard_rubric@1.0` — machine-checkable Vendor Scorecard rubric
...
| `FM-004` | `template` equals `vendor-scorecard@1` | error | true |
...
Per the `contracts/vendor-scorecard/template.md` H2 list. Each H2 in the template MUST be present + non-empty in the authored artefact. See template for the canonical headings.

| rule_id | Check | Severity |
| ------- | ----- | -------- |
| `SEC-001` | Every H2 declared in `contracts/vendor-scorecard/template.md` is present in the authored artefact | error |
...
- `cyberos/skill/contracts/vendor-scorecard/CONTRACT.md` + `template.md`
