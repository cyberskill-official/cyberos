---
# ── Identity ─────────────────────────────────────────────────────────
name: pen-test-report-audit
description: Audits a pen-test-report@1 artefact against pen_test_report_rubric@1.0. Validates FM + SEC, per-finding CVSS justification, PoC reproducibility, remediation-guidance specificity, retest-plan feasibility. Per OWASP WSTG + PTES + NIST SP 800-115 conventions.
  Audit one or more existing pen-test-report@1 markdowns against
  ...
  pen-test-report-author.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: pen-test-report@1
...
depends_on_contracts:
  - id:        pen-test-report
    ...
    pin_path:  cyberos/skill/contracts/pen-test-report/
  - id:        nats-subjects
    version:   v1
...
# pen-test-report-audit — PEN_TEST_REPORT auditor

> Standalone trigger that runs `pen-test-report_rubric@1.0` against one or
> more existing `pen-test-report@1` markdowns and writes a sibling
> `pen-test-report.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`pen-test-report-author`](../pen-test-report-author/SKILL.md).

`prompt_revision: pen-test-report_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `PEN_TEST_REPORT`s."
- "Has `PEN_TEST_REPORT-007` changed since the last audit?"
- "Tell me which `PEN_TEST_REPORT`s would fail acceptance today."

Also invoked automatically by the supervisor when `pen-test-report-author`'s output envelope sets `next_skill_recommendation: pen-test-report-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        pen-test-report-audit
skill_version:                   1.0.0
prompt_revision:                 pen-test-report_audit@1.0.0
template_version:                pen-test-report@1   (loaded from cyberos/skill/contracts/pen-test-report/template.md)
audit_rubric_version:            pen-test-report_rubric@1.0
...
```json
{
  "artefact_paths": ["./pen-test-reports/PEN_TEST_REPORT-001-foo.md", "./pen-test-reports/PEN_TEST_REPORT-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "pen-test-report-author",
    "manifest_path": "./pen-test-reports/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./pen-test-reports/PEN_TEST_REPORT-001-foo.md", "audit_path": "./pen-test-reports/PEN_TEST_REPORT-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./pen-test-reports/PEN_TEST_REPORT-002-bar.md", "audit_path": "./pen-test-reports/PEN_TEST_REPORT-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   pen-test-report-audit
Input:
  artefact_paths:  [./team-a/PEN_TEST_REPORT-001-something.md, ./team-b/PEN_TEST_REPORT-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
