---
# ── Identity ─────────────────────────────────────────────────────────
name: stage-gate-audit
description: >-
  Audit one or more existing stage-gate@1 markdowns against stage-gate_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after stage-gate-author. Use when user asks to "audit this stage gate" or "check the stage gate". Do NOT use for "draft a new stage gate" (use stage-gate-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-version: stage_gate_rubric@1.0
...
# stage-gate-audit — STAGE-GATE auditor

> Standalone trigger that runs `stage-gate_rubric@1.0` against one or
> more existing `stage-gate@1` markdowns and writes a sibling
> `stage-gate.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`stage-gate-author`](../stage-gate-author/SKILL.md).

`prompt_revision: stage-gate_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `STAGE-GATE`s."
- "Has `STAGE-GATE-007` changed since the last audit?"
- "Tell me which `STAGE-GATE`s would fail acceptance today."

Also invoked automatically by the supervisor when `stage-gate-author`'s output envelope sets `next_skill_recommendation: stage-gate-audit` (the default chain).
...
audit_rubric_version:            stage-gate_rubric@1.0
...
```json
{
  "artefact_paths": ["./stage-gates/STAGE-GATE-001-foo.md", "./stage-gates/STAGE-GATE-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "stage-gate-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./stage-gates/STAGE-GATE-001-foo.md", "audit_path": "./stage-gates/STAGE-GATE-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./stage-gates/STAGE-GATE-002-bar.md", "audit_path": "./stage-gates/STAGE-GATE-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   stage-gate-audit
Input:
  artefact_paths:  [./team-a/STAGE-GATE-001-something.md, ./team-b/STAGE-GATE-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
