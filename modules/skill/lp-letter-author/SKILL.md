---
# ── Identity ─────────────────────────────────────────────────────────
name: lp-letter-author
description: |
  Author the quarterly LP letter (portfolio performance + attribution + market commentary + new positions + outlook + asks + appendix). ILPA Reporting Template + Howard Marks Oaktree memo style. CIO-Investment authored.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: lp-letter_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['lp-letter-audit']; empty list disables chaining
...
# lp-letter-author — LP-LETTER generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `lp-letter@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`lp-letter-audit`](../lp-letter-audit/SKILL.md) by default.
...
If the user asks to *audit an existing LP-LETTER*, route to `lp-letter-audit` instead. If both, the supervisor chains the two.
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "lp-letter-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/lp-letter/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `lp-letter-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `lp-letter-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   lp-letter-author
Input:
