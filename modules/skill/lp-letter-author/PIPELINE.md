# `lp-letter-author` — pipeline

This document describes how `lp-letter-author` chains with upstream and downstream skills.
...
|---|---|---|
| `lp-letter-audit` | Default after every `artefact_written` event | `next_skill_recommendation: lp-letter-audit` in output envelope. |
| `<next-stage>-author` | After `lp-letter-audit` returns PASS | Supervisor reads the audit's output envelope and queues the next stage. |
...
- `cyberos/skill/contracts/lp-letter/` — the artefact template this skill generates.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/lp-letter-audit/` — the sibling audit skill that validates this skill's output.
