---
# ── Identity ─────────────────────────────────────────────────────────
name: crisis-communications-playbook-audit
description: >-
  Audit one or more existing crisis-communications-playbook@1 markdowns against crisis-comms-playbook_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after crisis-communications-playbook-author. Use when user asks to "audit this crisis communications playbook" or "check the crisis communications playbook". Do NOT use for "draft a new crisis communications playbook" (use crisis-communications-playbook-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: crisis-communications-playbook@1
  cyberos-rubric-version: crisis-comms-playbook_rubric@1.0
...
depends_on_contracts:
  - id:        crisis-comms-playbook
    version:   v1
    purpose:   validation_target
    pin_path:  cyberos/skill/contracts/crisis-communications-playbook/
  - id:        nats-subjects
    version:   v1
...
# crisis-communications-playbook-audit — CRISIS-COMMS-PLAYBOOK auditor

> Standalone trigger that runs `crisis-comms-playbook_rubric@1.0` against one or
> more existing `crisis-communications-playbook@1` markdowns and writes a sibling
> `crisis-comms-playbook.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`crisis-communications-playbook-author`](../crisis-communications-playbook-author/SKILL.md).

`prompt_revision: crisis-comms-playbook_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `CRISIS-COMMS-PLAYBOOK`s."
- "Has `CRISIS-COMMS-PLAYBOOK-007` changed since the last audit?"
- "Tell me which `CRISIS-COMMS-PLAYBOOK`s would fail acceptance today."

Also invoked automatically by the supervisor when `crisis-communications-playbook-author`'s output envelope sets `next_skill_recommendation: crisis-communications-playbook-audit` (the default chain).
...
skill_version:                   1.0.0
prompt_revision:                 crisis-comms-playbook_audit@1.0.0
template_version:                crisis-communications-playbook@1   (loaded from cyberos/skill/contracts/crisis-communications-playbook/template.md)
audit_rubric_version:            crisis-comms-playbook_rubric@1.0
...
```json
{
  "artefact_paths": ["./crisis-comms-playbooks/CRISIS-COMMS-PLAYBOOK-001-foo.md", "./crisis-comms-playbooks/CRISIS-COMMS-PLAYBOOK-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "crisis-communications-playbook-author",
    "manifest_path": "./crisis-comms-playbooks/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./crisis-comms-playbooks/CRISIS-COMMS-PLAYBOOK-001-foo.md", "audit_path": "./crisis-comms-playbooks/CRISIS-COMMS-PLAYBOOK-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./crisis-comms-playbooks/CRISIS-COMMS-PLAYBOOK-002-bar.md", "audit_path": "./crisis-comms-playbooks/CRISIS-COMMS-PLAYBOOK-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   crisis-communications-playbook-audit
Input:
  artefact_paths:  [./team-a/CRISIS-COMMS-PLAYBOOK-001-something.md, ./team-b/CRISIS-COMMS-PLAYBOOK-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
