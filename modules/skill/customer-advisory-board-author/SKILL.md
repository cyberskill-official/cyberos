---
# ── Identity ─────────────────────────────────────────────────────────
name: customer-advisory-board-author
description: >-
  Author the annual CAB program (charter + member selection + meeting agendas + insight capture + action mapping back to roadmap + member NPS). CCO-Customer + CPO-Product co-authored. Use when user asks to "draft a customer advisory board" or "create the customer advisory board". Do NOT use for "audit existing customer advisory board" (use customer-advisory-board-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: customer-advisory-board_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['customer-advisory-board-audit']; empty list disables chaining
...
# customer-advisory-board-author — CUSTOMER-ADVISORY-BOARD generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `customer-advisory-board@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`customer-advisory-board-audit`](../customer-advisory-board-audit/SKILL.md) by default.

`prompt_revision: customer-advisory-board_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a CUSTOMER-ADVISORY-BOARD."
- "Generate v2 of the CUSTOMER-ADVISORY-BOARD from the updated source."
- "Draft a CUSTOMER-ADVISORY-BOARD for <project>."

If the user asks to *audit an existing CUSTOMER-ADVISORY-BOARD*, route to `customer-advisory-board-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  CUSTOMER-ADVISORY-BOARD-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./customer-advisory-boards/",
...
```json
{
  ...
  "artefacts_written": [
    {"id": "CUSTOMER-ADVISORY-BOARD-001", "path": "./customer-advisory-boards/CUSTOMER-ADVISORY-BOARD-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "customer-advisory-board-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/customer-advisory-board/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `customer-advisory-board_author.customer-advisory-board_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `customer-advisory-board-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `customer-advisory-board-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   customer-advisory-board-author
Input:
