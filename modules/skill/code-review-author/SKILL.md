...
description: >-
  Author a structured code-review write-up (code-review@1) per IEEE 1028 review standard from a PR diff + linked impl-plan + linked task. Use when user asks to "review this PR", "draft a code review for the auth changes", or "audit the implementation against the task". Captures correctness vs ticket, readability, test coverage, secret-scan, injection paths, input validation, error handling, logging without PII, performance, backwards compatibility, and AI-specific checks per SDP §5 (hallucinated APIs, oversized diffs). Chains naturally into code-review-audit. Do NOT use for "audit an existing code-review.md" (use code-review-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: g
  cyberos-template: code-review@1
  cyberos-rubric-target: code_review_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['code-review-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        code-review
    ...
    pin_path:  cyberos/skill/contracts/code-review/
  - id:        nats-subjects
    version:   v1
...
# code-review-author — CODE-REVIEW generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `code-review@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`code-review-audit`](../code-review-audit/SKILL.md) by default.

`prompt_revision: code-review_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a CODE-REVIEW."
- "Generate v2 of the CODE-REVIEW from the updated source."
- "Draft a CODE-REVIEW for <project>."

If the user asks to *audit an existing CODE-REVIEW*, route to `code-review-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 code-review_author@1.0.0
template_version:                code-review@1   (loaded from cyberos/skill/contracts/code-review/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  CODE-REVIEW-{NNN}-{slug}.md
...
```json
{
  ...
  "manifest_path": "./code-reviews/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "CODE-REVIEW-001", "path": "./code-reviews/CODE-REVIEW-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "code-review-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/code-review/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- Emit a sibling `review-findings.json` beside every `code-review.md`, valid against `review-findings@1` (`envelopes/review-findings.schema.json`): one record per finding, `clause_ref` a real §1 clause or `null` (never fabricated), `[]` for a clean review. Adding it MUST NOT change `code-review.md`'s shape.
...
- `PIPELINE.md` — how this skill chains to `code-review-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   code-review-author
Input:
