---
# ── Identity ─────────────────────────────────────────────────────────
name: employee-net-promoter-score-program-audit
description: >-
  Audit one or more existing employee-net-promoter-score-program@1 markdowns against enps-program_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after employee-net-promoter-score-program-author. Use when user asks to "audit this employee net promoter score program" or "check the employee net promoter score program". Do NOT use for "draft a new employee net promoter score program" (use employee-net-promoter-score-program-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: employee-net-promoter-score-program@1
  cyberos-rubric-version: enps-program_rubric@1.0
...
depends_on_contracts:
  - id:        enps-program
    ...
    pin_path:  cyberos/skill/contracts/employee-net-promoter-score-program/
  - id:        nats-subjects
    version:   v1
...
# employee-net-promoter-score-program-audit — ENPS-PROGRAM auditor

> Standalone trigger that runs `enps-program_rubric@1.0` against one or
> more existing `employee-net-promoter-score-program@1` markdowns and writes a sibling
> `enps-program.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`employee-net-promoter-score-program-author`](../employee-net-promoter-score-program-author/SKILL.md).

`prompt_revision: enps-program_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `ENPS-PROGRAM`s."
- "Has `ENPS-PROGRAM-007` changed since the last audit?"
- "Tell me which `ENPS-PROGRAM`s would fail acceptance today."

Also invoked automatically by the supervisor when `employee-net-promoter-score-program-author`'s output envelope sets `next_skill_recommendation: employee-net-promoter-score-program-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        employee-net-promoter-score-program-audit
skill_version:                   1.0.0
prompt_revision:                 enps-program_audit@1.0.0
template_version:                employee-net-promoter-score-program@1   (loaded from cyberos/skill/contracts/employee-net-promoter-score-program/template.md)
audit_rubric_version:            enps-program_rubric@1.0
...
```json
{
  "artefact_paths": ["./enps-programs/ENPS-PROGRAM-001-foo.md", "./enps-programs/ENPS-PROGRAM-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "employee-net-promoter-score-program-author",
    "manifest_path": "./enps-programs/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./enps-programs/ENPS-PROGRAM-001-foo.md", "audit_path": "./enps-programs/ENPS-PROGRAM-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./enps-programs/ENPS-PROGRAM-002-bar.md", "audit_path": "./enps-programs/ENPS-PROGRAM-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   employee-net-promoter-score-program-audit
Input:
  artefact_paths:  [./team-a/ENPS-PROGRAM-001-something.md, ./team-b/ENPS-PROGRAM-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
