---
# ── Identity ─────────────────────────────────────────────────────────
name: postmortem-audit
description: >-
  Audit one or more existing postmortem@1 markdowns against postmortem_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after postmortem-author. Use when user asks to "audit this postmortem" or "check the postmortem". Do NOT use for "draft a new postmortem" (use postmortem-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: j
  cyberos-template: postmortem@1
  cyberos-rubric-version: postmortem_rubric@1.0
...
depends_on_contracts:
  - id:        postmortem
    ...
    pin_path:  cyberos/skill/contracts/postmortem/
  - id:        nats-subjects
    version:   v1
...
# postmortem-audit — POSTMORTEM auditor

> Standalone trigger that runs `postmortem_rubric@1.0` against one or
> more existing `postmortem@1` markdowns and writes a sibling
> `postmortem.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`postmortem-author`](../postmortem-author/SKILL.md).

`prompt_revision: postmortem_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `POSTMORTEM`s."
- "Has `POSTMORTEM-007` changed since the last audit?"
- "Tell me which `POSTMORTEM`s would fail acceptance today."

Also invoked automatically by the supervisor when `postmortem-author`'s output envelope sets `next_skill_recommendation: postmortem-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        postmortem-audit
skill_version:                   1.0.0
prompt_revision:                 postmortem_audit@1.0.0
template_version:                postmortem@1   (loaded from cyberos/skill/contracts/postmortem/template.md)
audit_rubric_version:            postmortem_rubric@1.0
...
```json
{
  "artefact_paths": ["./postmortems/POSTMORTEM-001-foo.md", "./postmortems/POSTMORTEM-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "postmortem-author",
    "manifest_path": "./postmortems/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./postmortems/POSTMORTEM-001-foo.md", "audit_path": "./postmortems/POSTMORTEM-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./postmortems/POSTMORTEM-002-bar.md", "audit_path": "./postmortems/POSTMORTEM-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   postmortem-audit
Input:
  artefact_paths:  [./team-a/POSTMORTEM-001-something.md, ./team-b/POSTMORTEM-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
