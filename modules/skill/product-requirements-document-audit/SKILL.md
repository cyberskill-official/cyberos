---
# ── Identity ─────────────────────────────────────────────────────────
name: product-requirements-document-audit
description: >-
  Audit one or more existing product-requirements-document@1 markdowns against prd_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after product-requirements-document-author. Use when user asks to "audit this product requirements document" or "check the product requirements document". Do NOT use for "draft a new product requirements document" (use product-requirements-document-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: product-requirements-document@1
  cyberos-rubric-version: prd_rubric@1.0
...
depends_on_contracts:
  - id:        prd
    ...
    pin_path:  cyberos/skill/contracts/product-requirements-document/
  - id:        nats-subjects
    version:   v1
...
# product-requirements-document-audit — PRD auditor

> Standalone trigger that runs `prd_rubric@1.0` against one or
> more existing `product-requirements-document@1` markdowns and writes a sibling
> `prd.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`product-requirements-document-author`](../product-requirements-document-author/SKILL.md).

`prompt_revision: prd_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `PRD`s."
- "Has `PRD-007` changed since the last audit?"
- "Tell me which `PRD`s would fail acceptance today."

Also invoked automatically by the supervisor when `product-requirements-document-author`'s output envelope sets `next_skill_recommendation: product-requirements-document-audit` (the default chain).
...
```json
{
  "artefact_paths": ["./prds/PRD-001-foo.md", "./prds/PRD-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "product-requirements-document-author",
    "manifest_path": "./prds/manifest.json"
  }
}
```
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./prds/PRD-001-foo.md", "audit_path": "./prds/PRD-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./prds/PRD-002-bar.md", "audit_path": "./prds/PRD-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   product-requirements-document-audit
Input:
  artefact_paths:  [./team-a/PRD-001-something.md, ./team-b/PRD-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
