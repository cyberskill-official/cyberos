---
# ── Identity ─────────────────────────────────────────────────────────
name: limited-partner-letter-author
description: >-
  Author the quarterly LP letter (portfolio performance + attribution + market commentary + new positions + outlook + asks + appendix). ILPA Reporting Template + Howard Marks Oaktree memo style. CIO-Investment authored. Use when user asks to "draft a limited partner letter" or "create the limited partner letter". Do NOT use for "audit existing limited partner letter" (use limited-partner-letter-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['limited-partner-letter-audit']; empty list disables chaining
...
> into [`limited-partner-letter-audit`](../limited-partner-letter-audit/SKILL.md) by default.
