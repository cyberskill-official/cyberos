...
name: closure-audit
description: >-
  Audit one or more existing closure@1 markdowns against closure_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after closure-author. Use when user asks to "audit this closure" or "check the closure". Do NOT use for "draft a new closure" (use closure-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-version: closure_rubric@1.0
...
# closure-audit — CLOSURE auditor

> Standalone trigger that runs `closure_rubric@1.0` against one or
> more existing `closure@1` markdowns and writes a sibling
> `closure.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`closure-author`](../closure-author/SKILL.md).

`prompt_revision: closure_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `CLOSURE`s."
- "Has `CLOSURE-007` changed since the last audit?"
- "Tell me which `CLOSURE`s would fail acceptance today."

Also invoked automatically by the supervisor when `closure-author`'s output envelope sets `next_skill_recommendation: closure-audit` (the default chain).
...
audit_rubric_version:            closure_rubric@1.0
...
```json
{
  "artefact_paths": ["./closures/CLOSURE-001-foo.md", "./closures/CLOSURE-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "closure-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./closures/CLOSURE-001-foo.md", "audit_path": "./closures/CLOSURE-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./closures/CLOSURE-002-bar.md", "audit_path": "./closures/CLOSURE-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
Skill:   closure-audit
Input:
  artefact_paths:  [./team-a/CLOSURE-001-something.md, ./team-b/CLOSURE-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
