---
# ── Identity ─────────────────────────────────────────────────────────
name: program-charter-audit
description: >-
  Audit one or more existing program-charter@1 markdowns against program-charter_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after program-charter-author. Use when user asks to "audit this program charter" or "check the program charter". Do NOT use for "draft a new program charter" (use program-charter-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: program-charter@1
  cyberos-rubric-version: program-charter_rubric@1.0
...
# program-charter-audit — PROGRAM-CHARTER auditor

> Standalone trigger that runs `program-charter_rubric@1.0` against one or
> more existing `program-charter@1` markdowns and writes a sibling
> `program-charter.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`program-charter-author`](../program-charter-author/SKILL.md).

`prompt_revision: program-charter_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `PROGRAM-CHARTER`s."
- "Has `PROGRAM-CHARTER-007` changed since the last audit?"
- "Tell me which `PROGRAM-CHARTER`s would fail acceptance today."

Also invoked automatically by the supervisor when `program-charter-author`'s output envelope sets `next_skill_recommendation: program-charter-audit` (the default chain).
...
audit_rubric_version:            program-charter_rubric@1.0
...
```json
{
  "artefact_paths": ["./program-charters/PROGRAM-CHARTER-001-foo.md", "./program-charters/PROGRAM-CHARTER-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "program-charter-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./program-charters/PROGRAM-CHARTER-001-foo.md", "audit_path": "./program-charters/PROGRAM-CHARTER-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./program-charters/PROGRAM-CHARTER-002-bar.md", "audit_path": "./program-charters/PROGRAM-CHARTER-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   program-charter-audit
Input:
  artefact_paths:  [./team-a/PROGRAM-CHARTER-001-something.md, ./team-b/PROGRAM-CHARTER-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
