---
# ── Identity ─────────────────────────────────────────────────────────
name: onboarding-pack-audit
description: >-
  Audit one or more existing onboarding-pack@1 markdowns against onboarding-pack_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after onboarding-pack-author. Use when user asks to "audit this onboarding pack" or "check the onboarding pack". Do NOT use for "draft a new onboarding pack" (use onboarding-pack-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: onboarding-pack@1
  cyberos-rubric-version: onboarding-pack_rubric@1.0
...
# onboarding-pack-audit — ONBOARDING-PACK auditor

> Standalone trigger that runs `onboarding-pack_rubric@1.0` against one or
> more existing `onboarding-pack@1` markdowns and writes a sibling
> `onboarding-pack.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`onboarding-pack-author`](../onboarding-pack-author/SKILL.md).

`prompt_revision: onboarding-pack_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `ONBOARDING-PACK`s."
- "Has `ONBOARDING-PACK-007` changed since the last audit?"
- "Tell me which `ONBOARDING-PACK`s would fail acceptance today."

Also invoked automatically by the supervisor when `onboarding-pack-author`'s output envelope sets `next_skill_recommendation: onboarding-pack-audit` (the default chain).
...
```json
{
  "artefact_paths": ["./onboarding-packs/ONBOARDING-PACK-001-foo.md", "./onboarding-packs/ONBOARDING-PACK-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "onboarding-pack-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./onboarding-packs/ONBOARDING-PACK-001-foo.md", "audit_path": "./onboarding-packs/ONBOARDING-PACK-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./onboarding-packs/ONBOARDING-PACK-002-bar.md", "audit_path": "./onboarding-packs/ONBOARDING-PACK-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   onboarding-pack-audit
Input:
  artefact_paths:  [./team-a/ONBOARDING-PACK-001-something.md, ./team-b/ONBOARDING-PACK-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
