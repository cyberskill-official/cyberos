# `strategy-document-author` — human summary format
...
```
STRATEGY-DOC batch <batch_run_id> complete

Wrote N STRATEGY-DOC(s):
  - STRATEGY-DOC-001: <slug> — PASS (audit verdict: pass, 0 open issues)
  - STRATEGY-DOC-002: <slug> — HITL_PAUSE (2 blocking issues — see HITL_BATCH_REQUEST below)
  - STRATEGY-DOC-003: <slug> — PASS (audit verdict: pass, 1 warning)

Manifest:    <output_dir>/manifest.json
Total time:  <seconds>s
Next:        strategy-document-audit will run automatically on PASS items. HITL items wait for your reply.
```
