---
# ── Identity ─────────────────────────────────────────────────────────
name: strategy-document-author
description: >-
  Author the annual 3-5yr strategy document (vision + strategic diagnosis-the-kernel + guiding policy + coherent actions + OKR cascade tie-in + capability investments + financial framework + strategic risks + measurement plan). Rumelt Good-Strategy kernel + Roger Martin Playing-to-Win choice cascade. CSO-Strategy or CEO authored. Use when user asks to "draft a strategy document" or "create the strategy document". Do NOT use for "audit existing strategy document" (use strategy-document-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: strategy-doc_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['strategy-document-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        strategy-doc
    ...
    pin_path:  cyberos/skill/contracts/strategy-document/
  - id:        nats-subjects
    version:   v1
...
# strategy-document-author — STRATEGY-DOC generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `strategy-document@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`strategy-document-audit`](../strategy-document-audit/SKILL.md) by default.

`prompt_revision: strategy-doc_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a STRATEGY-DOC."
- "Generate v2 of the STRATEGY-DOC from the updated source."
- "Draft a STRATEGY-DOC for <project>."

If the user asks to *audit an existing STRATEGY-DOC*, route to `strategy-document-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        strategy-document-author
skill_version:                   1.0.0
prompt_revision:                 strategy-doc_author@1.0.0
template_version:                strategy-document@1   (loaded from cyberos/skill/contracts/strategy-document/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  STRATEGY-DOC-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./strategy-docs/",
  "manifest_path": "./strategy-docs/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "STRATEGY-DOC-001", "path": "./strategy-docs/STRATEGY-DOC-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "strategy-document-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/strategy-document/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `strategy-doc_author.strategy-doc_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `strategy-document-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `strategy-document-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   strategy-document-author
Input:
