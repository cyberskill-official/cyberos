---
# ── Identity ─────────────────────────────────────────────────────────
name: board-deck-author
description: >-
  Author a quarterly board deck (10-15 slides) for the CEO or CFO. Renders executive summary + KPIs vs plan + strategic narrative + financial detail + risks + board asks. Validated by board-deck-audit. Use when user asks to "draft a board deck" or "create the board deck". Do NOT use for "audit existing board deck" (use board-deck-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: board-deck_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['board-deck-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        board-deck
    ...
    pin_path:  cyberos/skill/contracts/board-deck/
  - id:        nats-subjects
    version:   v1
...
# board-deck-author — BOARD-DECK generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `board-deck@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`board-deck-audit`](../board-deck-audit/SKILL.md) by default.

`prompt_revision: board-deck_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a BOARD-DECK."
- "Generate v2 of the BOARD-DECK from the updated source."
- "Draft a BOARD-DECK for <project>."

If the user asks to *audit an existing BOARD-DECK*, route to `board-deck-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 board-deck_author@1.0.0
template_version:                board-deck@1   (loaded from cyberos/skill/contracts/board-deck/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  BOARD-DECK-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./board-decks/",
  "manifest_path": "./board-decks/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "BOARD-DECK-001", "path": "./board-decks/BOARD-DECK-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "board-deck-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/board-deck/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `board-deck_author.board-deck_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
- If chained to `board-deck-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `board-deck-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   board-deck-author
Input:
