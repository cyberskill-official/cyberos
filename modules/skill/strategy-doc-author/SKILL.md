...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['strategy-doc-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        strategy-doc
    ...
    pin_path:  cyberos/skill/contracts/strategy-doc/
  - id:        nats-subjects
    version:   v1
...
# strategy-doc-author — STRATEGY-DOC generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `strategy-doc@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`strategy-doc-audit`](../strategy-doc-audit/SKILL.md) by default.
...
If the user asks to *audit an existing STRATEGY-DOC*, route to `strategy-doc-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        strategy-doc-author
skill_version:                   1.0.0
prompt_revision:                 strategy-doc_author@1.0.0
template_version:                strategy-doc@1   (loaded from cyberos/skill/contracts/strategy-doc/template.md)
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "strategy-doc-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/strategy-doc/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `strategy-doc-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `strategy-doc-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   strategy-doc-author
Input:
