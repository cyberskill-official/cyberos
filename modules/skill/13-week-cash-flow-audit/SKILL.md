---
# ── Identity ─────────────────────────────────────────────────────────
name: 13-week-cash-flow-audit
description: |
  Audit one or more existing 13-week-cash-flow@1 markdowns against
  ...
  13-week-cash-flow-author.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: <SDP §2 stage letter or "cross">
  cyberos-template: 13-week-cash-flow@1
...
# 13-week-cash-flow-audit — 13-WEEK-CASH-FLOW auditor

> Standalone trigger that runs `13-week-cash-flow_rubric@1.0` against one or
> more existing `13-week-cash-flow@1` markdowns and writes a sibling
...
> Chains naturally after [`13-week-cash-flow-author`](../13-week-cash-flow-author/SKILL.md).
...
Also invoked automatically by the supervisor when `13-week-cash-flow-author`'s output envelope sets `next_skill_recommendation: 13-week-cash-flow-audit` (the default chain).
...
```
CONTRACT_ECHO
skill_id:                        13-week-cash-flow-audit
skill_version:                   1.0.0
prompt_revision:                 13-week-cash-flow_audit@1.0.0
...
```json
{
  ...
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "13-week-cash-flow-author",
