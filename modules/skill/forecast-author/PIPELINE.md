# `forecast-author` — pipeline

This document describes how `forecast-author` chains with upstream and downstream skills.
...
|---|---|---|
| `forecast-audit` | Default after every `artefact_written` event | `next_skill_recommendation: forecast-audit` in output envelope. |
| `<next-stage>-author` | After `forecast-audit` returns PASS | Supervisor reads the audit's output envelope and queues the next stage. |
...
|---|---|---|
| `forecast_author.forecast_written` | `{artefact_id, artefact_path, artefact_hash, source_hash}` | After every successful W3 WRITE. |
| `forecast_author.batch_complete` | `{batch_run_id, artefacts_written, batch_outcome}` | At the end of a WORKER batch. |
| `forecast_author.hitl_pause` | `{artefact_id, blocking_issues}` | When the batch halts on HITL. |
| `forecast_author.amendment_request` | `{amendment_id, risk_class, change_description}` | When the author proposes a plan amendment. |
...
- `cyberos/skill/contracts/forecast/` — the artefact template this skill generates.
- `cyberos/skill/contracts/nats-subjects/` — the NATS subject naming contract.
- `cyberos/skill/forecast-audit/` — the sibling audit skill that validates this skill's output.
