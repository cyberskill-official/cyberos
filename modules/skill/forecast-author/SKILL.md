---
# ── Identity ─────────────────────────────────────────────────────────
name: forecast-author
description: >-
  Author a monthly / quarterly financial forecast (revenue + cost + cash). Renders forecast vs prior + assumption log + forecast-accuracy reflection. CFO authored. Target accuracy ±5%. Use when user asks to "draft a forecast" or "create the forecast". Do NOT use for "audit existing forecast" (use forecast-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: forecast_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['forecast-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        forecast
    ...
    pin_path:  cyberos/skill/contracts/forecast/
  - id:        nats-subjects
    version:   v1
...
# forecast-author — FORECAST generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `forecast@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`forecast-audit`](../forecast-audit/SKILL.md) by default.

`prompt_revision: forecast_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a FORECAST."
- "Generate v2 of the FORECAST from the updated source."
- "Draft a FORECAST for <project>."

If the user asks to *audit an existing FORECAST*, route to `forecast-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        forecast-author
skill_version:                   1.0.0
prompt_revision:                 forecast_author@1.0.0
template_version:                forecast@1   (loaded from cyberos/skill/contracts/forecast/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  FORECAST-{NNN}-{slug}.md
...
```json
{
  ...
  "manifest_path": "./forecasts/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "FORECAST-001", "path": "./forecasts/FORECAST-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "forecast-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/forecast/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `forecast_author.forecast_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `forecast-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `forecast-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   forecast-author
Input:
