...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['regulatory-filing-audit']; empty list disables chaining
...
# regulatory-filing-author — REGULATORY_FILING generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `regulatory-filing@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`regulatory-filing-audit`](../regulatory-filing-audit/SKILL.md) by default.

`prompt_revision: regulatory-filing_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a REGULATORY_FILING."
- "Generate v2 of the REGULATORY_FILING from the updated source."
- "Draft a REGULATORY_FILING for <project>."

If the user asks to *audit an existing REGULATORY_FILING*, route to `regulatory-filing-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        regulatory-filing-author
skill_version:                   1.0.0
prompt_revision:                 regulatory-filing_author@1.0.0
...
naming_pattern:                  REGULATORY_FILING-{NNN}-{slug}.md
...
```json
{
  ...
  "artefacts_written": [
    {"id": "REGULATORY_FILING-001", "path": "./regulatory-filings/REGULATORY_FILING-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "regulatory-filing-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/regulatory-filing/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `regulatory-filing_author.regulatory-filing_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `regulatory-filing-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `regulatory-filing-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   regulatory-filing-author
Input:
