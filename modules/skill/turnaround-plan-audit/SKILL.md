---
# ── Identity ─────────────────────────────────────────────────────────
name: turnaround-plan-audit
description: >-
  Audit one or more existing turnaround-plan@1 markdowns against turnaround-plan_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after turnaround-plan-author. Use when user asks to "audit this turnaround plan" or "check the turnaround plan". Do NOT use for "draft a new turnaround plan" (use turnaround-plan-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: turnaround-plan@1
  cyberos-rubric-version: turnaround-plan_rubric@1.0
...
# turnaround-plan-audit — TURNAROUND-PLAN auditor

> Standalone trigger that runs `turnaround-plan_rubric@1.0` against one or
> more existing `turnaround-plan@1` markdowns and writes a sibling
> `turnaround-plan.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`turnaround-plan-author`](../turnaround-plan-author/SKILL.md).

`prompt_revision: turnaround-plan_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `TURNAROUND-PLAN`s."
- "Has `TURNAROUND-PLAN-007` changed since the last audit?"
- "Tell me which `TURNAROUND-PLAN`s would fail acceptance today."

Also invoked automatically by the supervisor when `turnaround-plan-author`'s output envelope sets `next_skill_recommendation: turnaround-plan-audit` (the default chain).
...
audit_rubric_version:            turnaround-plan_rubric@1.0
...
```json
{
  "artefact_paths": ["./turnaround-plans/TURNAROUND-PLAN-001-foo.md", "./turnaround-plans/TURNAROUND-PLAN-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "turnaround-plan-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./turnaround-plans/TURNAROUND-PLAN-001-foo.md", "audit_path": "./turnaround-plans/TURNAROUND-PLAN-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./turnaround-plans/TURNAROUND-PLAN-002-bar.md", "audit_path": "./turnaround-plans/TURNAROUND-PLAN-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   turnaround-plan-audit
Input:
  artefact_paths:  [./team-a/TURNAROUND-PLAN-001-something.md, ./team-b/TURNAROUND-PLAN-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
