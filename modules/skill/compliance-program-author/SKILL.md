---
# ── Identity ─────────────────────────────────────────────────────────
name: compliance-program-author
description: >-
  Author the compliance management system (applicable regulations + regulatory-mapping matrix + control catalog with owner + training plan + incident-response procedure + audit cadence). CCO-Compliance authored. Use when user asks to "draft a compliance program" or "create the compliance program". Do NOT use for "audit existing compliance program" (use compliance-program-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: compliance-program_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['compliance-program-audit']; empty list disables chaining
...
# compliance-program-author — COMPLIANCE-PROGRAM generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `compliance-program@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`compliance-program-audit`](../compliance-program-audit/SKILL.md) by default.

`prompt_revision: compliance-program_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a COMPLIANCE-PROGRAM."
- "Generate v2 of the COMPLIANCE-PROGRAM from the updated source."
- "Draft a COMPLIANCE-PROGRAM for <project>."

If the user asks to *audit an existing COMPLIANCE-PROGRAM*, route to `compliance-program-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        compliance-program-author
skill_version:                   1.0.0
prompt_revision:                 compliance-program_author@1.0.0
...
naming_pattern:                  COMPLIANCE-PROGRAM-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./compliance-programs/",
...
```json
{
  ...
  "artefacts_written": [
    {"id": "COMPLIANCE-PROGRAM-001", "path": "./compliance-programs/COMPLIANCE-PROGRAM-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "compliance-program-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/compliance-program/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `compliance-program_author.compliance-program_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `compliance-program-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `compliance-program-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   compliance-program-author
Input:
