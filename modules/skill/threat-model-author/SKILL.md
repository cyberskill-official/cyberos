...
description: |
  Author a STRIDE-based threat model (threat-model@1) from an SRS plus
  accepted ADRs. Enumerates trust boundaries, data flows, threats per
  STRIDE category (Spoofing, Tampering, Repudiation, Information
  Disclosure, Denial of Service, Elevation of Privilege), mitigations
  mapped to OWASP Top 10:2025 (A01-A10) and OWASP ASVS controls.
  Covers Software Development Process.md §2(d) Architecture security
  review. Chains naturally into threat-model-audit.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: d
  cyberos-template: threat-model@1
  cyberos-rubric-target: threat_model_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['threat-model-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        threat-model
    ...
    pin_path:  cyberos/skill/contracts/threat-model/
  - id:        nats-subjects
    version:   v1
...
# threat-model-author — THREAT-MODEL generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `threat-model@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`threat-model-audit`](../threat-model-audit/SKILL.md) by default.

`prompt_revision: threat-model_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a THREAT-MODEL."
- "Generate v2 of the THREAT-MODEL from the updated source."
- "Draft a THREAT-MODEL for <project>."

If the user asks to *audit an existing THREAT-MODEL*, route to `threat-model-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 threat-model_author@1.0.0
template_version:                threat-model@1   (loaded from cyberos/skill/contracts/threat-model/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  THREAT-MODEL-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./threat-models/",
  "manifest_path": "./threat-models/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "THREAT-MODEL-001", "path": "./threat-models/THREAT-MODEL-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "threat-model-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/threat-model/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `threat-model_author.threat-model_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `threat-model-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `threat-model-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   threat-model-author
Input:
