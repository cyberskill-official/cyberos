# `threat-model-author` — human summary format
...
```
THREAT-MODEL batch <batch_run_id> complete

Wrote N THREAT-MODEL(s):
  - THREAT-MODEL-001: <slug> — PASS (audit verdict: pass, 0 open issues)
  - THREAT-MODEL-002: <slug> — HITL_PAUSE (2 blocking issues — see HITL_BATCH_REQUEST below)
  - THREAT-MODEL-003: <slug> — PASS (audit verdict: pass, 1 warning)

Manifest:    <output_dir>/manifest.json
Total time:  <seconds>s
Next:        threat-model-audit will run automatically on PASS items. HITL items wait for your reply.
```
