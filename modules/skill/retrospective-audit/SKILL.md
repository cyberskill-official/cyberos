...
description: >-
  Audit one or more existing retrospective@1 markdowns against retro_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after retrospective-author. Use when user asks to "audit this retrospective" or "check the retrospective". Do NOT use for "draft a new retrospective" (use retrospective-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-version: retro_rubric@1.0
...
> Chains naturally after [`retrospective-author`](../retrospective-author/SKILL.md).
...
- "Audit these existing `RETRO`s."
- "Has `RETRO-007` changed since the last audit?"
- "Tell me which `RETRO`s would fail acceptance today."
