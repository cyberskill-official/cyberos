...
description: >-
  Audit one or more existing project-plan@1 markdowns against project-plan_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after project-plan-author. Use when user asks to "audit this project plan" or "check the project plan". Do NOT use for "draft a new project plan" (use project-plan-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-version: project_plan_rubric@1.0
...
# project-plan-audit — PROJECT-PLAN auditor

> Standalone trigger that runs `project-plan_rubric@1.0` against one or
> more existing `project-plan@1` markdowns and writes a sibling
> `project-plan.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`project-plan-author`](../project-plan-author/SKILL.md).

`prompt_revision: project-plan_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `PROJECT-PLAN`s."
- "Has `PROJECT-PLAN-007` changed since the last audit?"
- "Tell me which `PROJECT-PLAN`s would fail acceptance today."

Also invoked automatically by the supervisor when `project-plan-author`'s output envelope sets `next_skill_recommendation: project-plan-audit` (the default chain).
...
audit_rubric_version:            project-plan_rubric@1.0
...
```json
{
  "artefact_paths": ["./project-plans/PROJECT-PLAN-001-foo.md", "./project-plans/PROJECT-PLAN-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "project-plan-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./project-plans/PROJECT-PLAN-001-foo.md", "audit_path": "./project-plans/PROJECT-PLAN-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./project-plans/PROJECT-PLAN-002-bar.md", "audit_path": "./project-plans/PROJECT-PLAN-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   project-plan-audit
Input:
  artefact_paths:  [./team-a/PROJECT-PLAN-001-something.md, ./team-b/PROJECT-PLAN-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
