---
# ── Identity ─────────────────────────────────────────────────────────
name: change-management-plan-author
description: >-
  Author a per-program change-mgmt plan (program context + stakeholder analysis + comms plan + training plan + adoption tracking + resistance mitigation). Prosci ADKAR + Kotter 8-step aligned. Chief Transformation Officer authored. Use when user asks to "draft a change management plan" or "create the change management plan". Do NOT use for "audit existing change management plan" (use change-management-plan-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: change-mgmt-plan_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['change-management-plan-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        change-mgmt-plan
    ...
    pin_path:  cyberos/skill/contracts/change-management-plan/
  - id:        nats-subjects
    version:   v1
...
# change-management-plan-author — CHANGE-MGMT-PLAN generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `change-management-plan@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`change-management-plan-audit`](../change-management-plan-audit/SKILL.md) by default.

`prompt_revision: change-mgmt-plan_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a CHANGE-MGMT-PLAN."
- "Generate v2 of the CHANGE-MGMT-PLAN from the updated source."
- "Draft a CHANGE-MGMT-PLAN for <project>."

If the user asks to *audit an existing CHANGE-MGMT-PLAN*, route to `change-management-plan-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 change-mgmt-plan_author@1.0.0
template_version:                change-management-plan@1   (loaded from cyberos/skill/contracts/change-management-plan/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  CHANGE-MGMT-PLAN-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./change-mgmt-plans/",
  "manifest_path": "./change-mgmt-plans/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "CHANGE-MGMT-PLAN-001", "path": "./change-mgmt-plans/CHANGE-MGMT-PLAN-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "change-management-plan-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/change-management-plan/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `change-mgmt-plan_author.change-mgmt-plan_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `change-management-plan-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `change-management-plan-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   change-management-plan-author
Input:
