...
metadata:
  ...
  module: skill
  stage: h
  cyberos-template: test-strategy@1
  cyberos-rubric-target: test_strategy_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['test-strategy-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        test-strategy
    ...
    pin_path:  cyberos/skill/contracts/test-strategy/
  - id:        nats-subjects
    version:   v1
...
# test-strategy-author — TEST-STRATEGY generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `test-strategy@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`test-strategy-audit`](../test-strategy-audit/SKILL.md) by default.

`prompt_revision: test-strategy_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a TEST-STRATEGY."
- "Generate v2 of the TEST-STRATEGY from the updated source."
- "Draft a TEST-STRATEGY for <project>."

If the user asks to *audit an existing TEST-STRATEGY*, route to `test-strategy-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 test-strategy_author@1.0.0
template_version:                test-strategy@1   (loaded from cyberos/skill/contracts/test-strategy/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  TEST-STRATEGY-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./test-strategys/",
  "manifest_path": "./test-strategys/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "TEST-STRATEGY-001", "path": "./test-strategys/TEST-STRATEGY-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "test-strategy-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/test-strategy/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `test-strategy_author.test-strategy_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `test-strategy-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `test-strategy-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   test-strategy-author
Input:
