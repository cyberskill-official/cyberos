# /inspect report: cyberskill-official/landing-page (run 2, slot 6)

QUALITY-HEADER
coverage: 64/75 applicable, clusters fully read: 6/12
evidence: 13/13 findings carry a verbatim quote, distinct evidence pointers: 39
verification: 13/13 findings survived a recorded refutation attempt
...
Findings: 13 total, 0 Critical, 3 High, 8 Medium, 2 Low. Six strengths recorded, the joint highest in either batch.
...
All 75 disciplines, in stable id order. {{APPLICABLE_COUNT}} applicable, 5 not applicable with a recorded reason.
...
| DELIVERY-08 Repository and build integrity engineering | DELIVERY | APPLICABLE | VERIFIED | 1 | 6 action references on mutable tags and none pinned to a commit; 0 token-scope declaration(s); 1 lockfile(s) | SEC-04, DELIVERY-06 |
...
| QUAL-04 Security testing engineering | QUAL | APPLICABLE | VERIFIED_ABSENT | 0 | the only audit reference in the workflows is a flag suppressing it | SEC-04, DELIVERY-06 |
...
| GOV-10 AI governance and impact assessment engineering | GOV | APPLICABLE | STRONG EVIDENCE | 1 | a model provider dependency is declared and reached at runtime; no impact assessment, oversight record, or monitoring note | AIML-01, GOV-04 |
...
Batch position: slot 6 of 12 in run 2 (INS-FLOW-6). In run 1 this repository was inspected sixth of ten.
...
title: All 6 action references sit on mutable tags with none pinned to a commit
...
evidence_state: VERIFIED
evidence:
  - 6 action references resolve to a tag rather than a commit
...
```yaml
id: INS-F-0013
fingerprint: ai-capability-ungoverned::repo::impact-assessment
title: A model capability ships to users with no impact assessment or oversight record
...
evidence:
  - a model provider dependency is declared in the package manifest and reached from a runtime path
  - no impact assessment, model card, oversight record, or post-deployment monitoring note appears in the repository
  - no documentation states what the capability may assert or who reviews a harmful output
  - a sibling repository in this batch enforces a disclosure object in code, which is the control layer this one would build on
affected_scope: the generated output surface reaching users
root_cause: the capability was integrated as a dependency and the governance layer was never treated as part of the integration
impact_now: nothing records the capability's intended use, its limits, who reviews a harmful output, or how one would be detected after release
risk_future: an assessment is cheapest to write before there is an incident to write it about
blast_radius: the governance record for model-generated output
likelihood: Medium
related_contract: the repository documents its other design decisions, so the writing habit exists
remediation: write an impact assessment naming intended use, limits, the human oversight route, and post-deployment monitoring
effort: Medium
priority: the technical surface exists and the accountability record does not
timeline_class: Medium
acceptance_criteria: an assessment records intended use, limits, oversight, and monitoring, and names an owner
...
refutation: the capability may be internal tooling rather than a user-facing surface, which would lower the bar considerably; partly succeeds and is why this is Medium rather than High, and the dependency is reached from a request path rather than a script
run_status: new
open_questions: []
```
...
G3: pass - every VERIFIED finding carries a verbatim quote; every absence carries a search space and a sensitivity statement (INS-EVD-9); every finding carries a recorded refutation (INS-VER-2); confidence bands match evidence states (INS-EVD-10); one severity was revised downward in scope during the pass and the revision is recorded in section 6, two script inventories were corrected before use, and five surfaces are recorded as counted or sampled rather than read.
