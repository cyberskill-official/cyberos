# /inspect report: cyberskill-official/practice
...
Check the row-level security setting in the project dashboard before anything else in this report.

practice is an exam-practice platform whose item bank is generated by a model pipeline. It has the most thoughtful intellectual-property controls of any repository across both batches: a provenance record bound to every bank item, a coverage gate that fails when any item lacks one or any record dangles, and a separate similarity check against reproduction. For a product whose central legal risk is reproducing copyrighted exam material, that is the right control built by someone who understood the problem.

Two things undercut it. The provenance gate runs only in a local pre-commit hook, which any contributor skips with a single flag, and never in continuous integration. And no committed SQL enables row-level security or declares a policy, while the anon key, which is public by construction and is additionally hardcoded as a literal fallback in source, is used to write to tables from the browser. Tables in scope include subscriber email addresses and exam results.

I could not determine from the repository whether row-level security is enabled in the live project through the dashboard rather than in SQL. That question decides whether the first finding is a schema-as-code gap or an open door, and it takes minutes to answer.

Findings: 9 total, 1 Critical, 2 High, 4 Medium, 2 Low. Six strengths recorded.
...
All 69 disciplines, in stable id order. 59 applicable, 10 not applicable with a recorded reason.
...
| DATA-02 Database engineering | DATA | APPLICABLE | VERIFIED | 0 | supabase/ five SQL files defining tables and capture routines | SEC-03, DATA-01 |
...
| SEC-03 Identity and access engineering | SEC | APPLICABLE | VERIFIED | 2 | src/lib/supabase.ts:4-11, no policy statement in any committed SQL | SEC-01, DATA-02 |
...
```yaml
id: INS-F-0001
fingerprint: no-rls-with-published-anon-key::supabase::policies
title: No row-level security appears in any committed SQL while a published anon key writes to tables
...
evidence_state: VERIFIED
evidence:
  - no CREATE POLICY or ROW LEVEL SECURITY statement appears anywhere under supabase/ or src/
  - the five committed SQL files define tables including subscriber records and exam results
  - src/lib/supabase.ts:11 constructs a client from the anon key and src/components/BugReporter.tsx:42 inserts through it from the browser
  - a table created by raw SQL rather than through the dashboard has row-level security disabled by default
  - quote: "      const { error: dbError } = await supabase.from('bug_reports').insert({"
affected_scope: every table the anon role holds a grant on, which on this evidence is all of them
root_cause: the schema was authored as raw SQL files, which do not enable row-level security by default, and no policy was added afterwards
impact_now: the anon key is public by construction, since it ships to the browser, and without row-level security it is a read and write credential rather than a scoped one; the tables in scope include subscriber email addresses and exam results, which are personal data
risk_future: the same key is additionally hardcoded as a literal fallback in source, so it survives any change to how the environment is configured
blast_radius: every table in the project, including personal data
likelihood: High
related_contract: src/lib/supabaseAdmin.ts keeps the service role path structurally separate, which is the right shape and is what makes the missing policies on the anon path the whole question
remediation: confirm in the project dashboard whether row-level security is enabled per table; where it is not, enable it and add a policy per table scoped to the authenticated identity, keeping the bug report insert as the one deliberate anonymous write
effort: Medium
priority: first (Critical; verification is minutes and it decides whether this is a defect or an incident)
timeline_class: Immediate
acceptance_criteria: a client holding only the anon key can perform no operation on any table beyond the one anonymous insert the product intends, and every table's policy is committed as SQL
validation_method: connect with the anon key and attempt select, insert, update, and delete against each table, confirming each is refused except the intended insert
regression_gate: a test that runs those attempts with an anon-key client and asserts refusal
rollback: none applicable while the exposure stands; enabling policies is the safe direction
owner_discipline: SEC-03
review_required: security
approval_required: yes
run_status: new
open_questions: [is row-level security already enabled in the live project through the dashboard rather than in committed SQL, which would make this a schema-as-code gap rather than an exposure]
```
...
Next action for /harden: this one begins with the operator rather than the agent. Open the project dashboard and record, per table, whether row-level security is enabled. That answer decides whether INS-F-0001 is a schema-as-code gap to close at leisure or an open door to shut today, and no code change should be made before it is known. Acceptance proves the finding closed when a client holding only the anon key is refused every operation except the single anonymous insert the product intends, and when every table's policy is committed as SQL rather than living only in the dashboard.

NEXT-ACTION: INS-F-0001 no-rls-with-published-anon-key::supabase::policies
