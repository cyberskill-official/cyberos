# /inspect report: cyberskill-official/shopass
...
shopass is the largest and most disciplined repository across both batches: fifteen services in Go, TypeScript, and Python behind one gateway, four shared Go modules, a browser extension, a web client, and a six-job gate covering all three languages plus compose validation. Seven strengths are recorded, the most of any repository inspected.
...
All 69 disciplines, in stable id order. 61 applicable, 8 not applicable with a recorded reason. This is the broadest applicable surface across both batches, and the first repository where the internal platform row applies.
...
| SEC-02 Privacy engineering | SEC | APPLICABLE | SUSPECTED | 0 | services/comply exists; retention policy was not located in the read set | GOV-04, SEC-03 |
...
| EXP-02 Accessibility engineering | EXP | APPLICABLE | NOT FOUND | 0 | no accessibility test or dependency was located in web or extension | EXP-01 |
...
| GOV-08 Evolution, sustainability, maintenance, and disposal engineering | GOV | APPLICABLE | VERIFIED_ABSENT | 1 | no dependency automation across seventeen Go modules and four Node manifests | SEC-04 |
...
Three hypotheses were formed during this pass and all three were wrong. They are recorded because reversing them is most of what this inspection produced.
...
title: No dependency automation across seventeen Go modules and four Node manifests
...
evidence:
  ...
  - seventeen module files and four package manifests declare dependencies independently
  - the workflow comment at line 44 records a toolchain patch applied by hand for standard library advisories
affected_scope: twenty-one dependency manifests across three languages
root_cause: automation was not configured and the surface has grown to twenty-one manifests
impact_now: the vulnerability scan reports advisories and nothing proposes the fixes, so every raise is manual across twenty-one files; the workflow comment showing a hand-applied toolchain patch is the evidence of what that costs
risk_future: the manifest count grows with each service, and the scan that finds problems has no counterpart that resolves them
blast_radius: patch latency across the whole tree
likelihood: Medium
related_contract: the scan already runs on every push, so proposals would arrive with evidence attached
remediation: configure dependency automation covering all three ecosystems against a maintained preset
effort: Small
priority: eighth (Low; it compounds slowly but across twenty-one manifests)
timeline_class: Medium
acceptance_criteria: dependency updates are proposed automatically for Go, Node, and Python manifests
validation_method: confirm a proposal appears for an outdated dependency in each ecosystem
...
INS-F-0007, consult and honour the target's crawl directives per adapter. INS-F-0008, configure dependency automation across all three ecosystems.
...
Medium term: INS-F-0007, INS-F-0008.
...
G3: pass - every VERIFIED finding carries a verbatim quote; three hypotheses were falsified during the pass and all three corrections are recorded in section 6, two findings are held at Medium confidence with the reason stated, and five surfaces are recorded as counted rather than read.
