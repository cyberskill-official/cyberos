# /inspect report: zintaen/kristen-calendar (run 2, slot 8)

QUALITY-HEADER
coverage: 65/75 applicable, clusters fully read: 6/12
...
Findings: 13 total, 1 Critical, 3 High, 7 Medium, 2 Low.
...
All 75 disciplines, in stable id order. {{APPLICABLE_COUNT}} applicable, 4 not applicable with a recorded reason. This is the broadest applicable surface in the batch, and the first repository where localisation, native client, and internal package publication all apply at once.
...
| DELIVERY-08 Repository and build integrity engineering | DELIVERY | APPLICABLE | VERIFIED | 1 | 10 action references on mutable tags and none pinned to a commit; 0 token-scope declaration(s); 1 lockfile(s) | SEC-04, DELIVERY-06 |
...
Batch position: slot 8 of 12 in run 2 (INS-FLOW-6). In run 1 this repository was inspected fifth of ten.
...
title: All 10 action references sit on mutable tags with none pinned to a commit
...
evidence_state: VERIFIED
evidence:
  - 10 action references resolve to a tag rather than a commit
...
G3: pass - every VERIFIED finding carries a verbatim quote; every absence carries a search space and a sensitivity statement (INS-EVD-9); every finding carries a recorded refutation (INS-VER-2); confidence bands match evidence states (INS-EVD-10); credential values were redacted to prefix and format only, INS-F-0006 is recorded as latent with the reason stated, and section 6 records five limitations including three surfaces inventoried but not read.
