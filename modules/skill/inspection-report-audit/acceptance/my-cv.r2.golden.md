# /inspect report: zintaen/my-cv (run 2, slot 12)

QUALITY-HEADER
coverage: 51/75 applicable, clusters fully read: 6/12
evidence: 17/17 findings carry a verbatim quote, distinct evidence pointers: 51
verification: 17/17 findings survived a recorded refutation attempt
...
Findings: 17 total, 0 Critical, 3 High, 10 Medium, 4 Low.
...
All 75 disciplines, in stable id order. {{APPLICABLE_COUNT}} applicable, 18 not applicable with a recorded reason.
...
| DELIVERY-08 Repository and build integrity engineering | DELIVERY | APPLICABLE | VERIFIED | 1 | 7 action references on mutable tags and none pinned to a commit; 1 token-scope declaration(s); 1 lockfile(s) | SEC-04, DELIVERY-06 |
...
Batch position: slot 12 of 12 in run 2 (INS-FLOW-6). In run 1 this repository was inspected first of ten.
...
title: All 7 action references sit on mutable tags with none pinned to a commit
...
evidence_state: VERIFIED
evidence:
  - 7 action references resolve to a tag rather than a commit
  - no reference in the workflows carries a forty-character commit
  - 1 workflow declares a token scope, so the default scope applies elsewhere
