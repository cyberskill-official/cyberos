---
# ── Identity ─────────────────────────────────────────────────────────
name: limited-partner-letter-audit
description: >-
  Audit one or more existing limited-partner-letter@1 markdowns against lp-letter_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after limited-partner-letter-author. Use when user asks to "audit this limited partner letter" or "check the limited partner letter". Do NOT use for "draft a new limited partner letter" (use limited-partner-letter-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: limited-partner-letter@1
  cyberos-rubric-version: lp-letter_rubric@1.0
...
depends_on_contracts:
  - id:        lp-letter
    ...
    pin_path:  cyberos/skill/contracts/limited-partner-letter/
  - id:        nats-subjects
    version:   v1
...
# limited-partner-letter-audit — LP-LETTER auditor

> Standalone trigger that runs `lp-letter_rubric@1.0` against one or
> more existing `limited-partner-letter@1` markdowns and writes a sibling
> `lp-letter.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`limited-partner-letter-author`](../limited-partner-letter-author/SKILL.md).

`prompt_revision: lp-letter_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `LP-LETTER`s."
- "Has `LP-LETTER-007` changed since the last audit?"
- "Tell me which `LP-LETTER`s would fail acceptance today."

Also invoked automatically by the supervisor when `limited-partner-letter-author`'s output envelope sets `next_skill_recommendation: limited-partner-letter-audit` (the default chain).
...
audit_rubric_version:            lp-letter_rubric@1.0
...
```json
{
  "artefact_paths": ["./lp-letters/LP-LETTER-001-foo.md", "./lp-letters/LP-LETTER-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "limited-partner-letter-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./lp-letters/LP-LETTER-001-foo.md", "audit_path": "./lp-letters/LP-LETTER-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./lp-letters/LP-LETTER-002-bar.md", "audit_path": "./lp-letters/LP-LETTER-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   limited-partner-letter-audit
Input:
  artefact_paths:  [./team-a/LP-LETTER-001-something.md, ./team-b/LP-LETTER-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
