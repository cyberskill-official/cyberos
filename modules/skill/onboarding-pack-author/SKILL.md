---
# ── Identity ─────────────────────────────────────────────────────────
name: onboarding-pack-author
description: >-
  Author a per-new-hire (or per-executive) onboarding pack (pre-start checklist + day-1 + week-1 milestones + 30/60/90 + buddy + first-quarter review template). CHRO authored; Chief of Staff for exec onboarding. Use when user asks to "draft a onboarding pack" or "create the onboarding pack". Do NOT use for "audit existing onboarding pack" (use onboarding-pack-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: onboarding-pack_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['onboarding-pack-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        onboarding-pack
    ...
    pin_path:  cyberos/skill/contracts/onboarding-pack/
  - id:        nats-subjects
    version:   v1
...
# onboarding-pack-author — ONBOARDING-PACK generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `onboarding-pack@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`onboarding-pack-audit`](../onboarding-pack-audit/SKILL.md) by default.

`prompt_revision: onboarding-pack_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a ONBOARDING-PACK."
- "Generate v2 of the ONBOARDING-PACK from the updated source."
- "Draft a ONBOARDING-PACK for <project>."

If the user asks to *audit an existing ONBOARDING-PACK*, route to `onboarding-pack-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 onboarding-pack_author@1.0.0
template_version:                onboarding-pack@1   (loaded from cyberos/skill/contracts/onboarding-pack/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  ONBOARDING-PACK-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./onboarding-packs/",
  "manifest_path": "./onboarding-packs/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "ONBOARDING-PACK-001", "path": "./onboarding-packs/ONBOARDING-PACK-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "onboarding-pack-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/onboarding-pack/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `onboarding-pack_author.onboarding-pack_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
- If chained to `onboarding-pack-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `onboarding-pack-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   onboarding-pack-author
Input:
