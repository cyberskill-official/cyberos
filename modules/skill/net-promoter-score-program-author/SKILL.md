---
# ── Identity ─────────────────────────────────────────────────────────
name: net-promoter-score-program-author
description: >-
  Author the quarterly NPS program (survey instrument + cohort coverage + response-rate target + detractor outreach + promoter activation + action plan). CCO-Customer or CPO-Product authored. Use when user asks to "draft a net promoter score program" or "create the net promoter score program". Do NOT use for "audit existing net promoter score program" (use net-promoter-score-program-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: nps-program_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['net-promoter-score-program-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        nps-program
    ...
    pin_path:  cyberos/skill/contracts/net-promoter-score-program/
  - id:        nats-subjects
    version:   v1
...
# net-promoter-score-program-author — NPS-PROGRAM generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `net-promoter-score-program@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`net-promoter-score-program-audit`](../net-promoter-score-program-audit/SKILL.md) by default.

`prompt_revision: nps-program_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a NPS-PROGRAM."
- "Generate v2 of the NPS-PROGRAM from the updated source."
- "Draft a NPS-PROGRAM for <project>."

If the user asks to *audit an existing NPS-PROGRAM*, route to `net-promoter-score-program-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        net-promoter-score-program-author
skill_version:                   1.0.0
prompt_revision:                 nps-program_author@1.0.0
template_version:                net-promoter-score-program@1   (loaded from cyberos/skill/contracts/net-promoter-score-program/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  NPS-PROGRAM-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./nps-programs/",
  "manifest_path": "./nps-programs/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "NPS-PROGRAM-001", "path": "./nps-programs/NPS-PROGRAM-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "net-promoter-score-program-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/net-promoter-score-program/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `nps-program_author.nps-program_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `net-promoter-score-program-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `net-promoter-score-program-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   net-promoter-score-program-author
Input:
