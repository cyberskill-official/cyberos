...
description: >-
  Authors the annual information-security strategy. Threat landscape, current posture vs NIST CSF 2.0, CIS Controls IG1/IG2/IG3 maturity, prioritized initiatives, budget envelope, security OKRs, board narrative. Per NIST CSF 2.0 + ISO/IEC 27001:2022 + CIS Controls v8 + Verizon DBIR + MITRE ATT&CK. Author a SECURITY_STRATEGY markdown from source artefact(s). Generates a versioned security-strategy@1 file under output_dir, with per-claim authority markers and provenance to the source. Chains naturally into security-strategy-audit by default. Refuses to author when upstream artefact is in non-pass state. Use when user asks to "draft a security strategy" or "create the security strategy". Do NOT use for "audit existing security strategy" (use security-strategy-audit instead). Author a SECURITY_STRATEGY markdown from source artefact(s). Generates a versioned security-strategy@1 file under output_dir, with per-claim authority markers and provenance to the source. Chains naturally into security-strategy-audit by d...
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: security-strategy_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['security-strategy-audit']; empty list disables chaining
...
# security-strategy-author — SECURITY_STRATEGY generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `security-strategy@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`security-strategy-audit`](../security-strategy-audit/SKILL.md) by default.

`prompt_revision: security-strategy_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a SECURITY_STRATEGY."
- "Generate v2 of the SECURITY_STRATEGY from the updated source."
- "Draft a SECURITY_STRATEGY for <project>."

If the user asks to *audit an existing SECURITY_STRATEGY*, route to `security-strategy-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  SECURITY_STRATEGY-{NNN}-{slug}.md
...
```json
{
  ...
  "artefacts_written": [
    {"id": "SECURITY_STRATEGY-001", "path": "./security-strategys/SECURITY_STRATEGY-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "security-strategy-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/security-strategy/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `security-strategy_author.security-strategy_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
- If chained to `security-strategy-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `security-strategy-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   security-strategy-author
Input:
