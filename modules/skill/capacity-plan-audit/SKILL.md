---
# ── Identity ─────────────────────────────────────────────────────────
name: capacity-plan-audit
description: >-
  Audit one or more existing capacity-plan@1 markdowns against capacity-plan_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after capacity-plan-author. Use when user asks to "audit this capacity plan" or "check the capacity plan". Do NOT use for "draft a new capacity plan" (use capacity-plan-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: capacity-plan@1
  cyberos-rubric-version: capacity-plan_rubric@1.0
...
# capacity-plan-audit — CAPACITY-PLAN auditor

> Standalone trigger that runs `capacity-plan_rubric@1.0` against one or
> more existing `capacity-plan@1` markdowns and writes a sibling
> `capacity-plan.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`capacity-plan-author`](../capacity-plan-author/SKILL.md).

`prompt_revision: capacity-plan_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `CAPACITY-PLAN`s."
- "Has `CAPACITY-PLAN-007` changed since the last audit?"
- "Tell me which `CAPACITY-PLAN`s would fail acceptance today."

Also invoked automatically by the supervisor when `capacity-plan-author`'s output envelope sets `next_skill_recommendation: capacity-plan-audit` (the default chain).
...
audit_rubric_version:            capacity-plan_rubric@1.0
...
```json
{
  "artefact_paths": ["./capacity-plans/CAPACITY-PLAN-001-foo.md", "./capacity-plans/CAPACITY-PLAN-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "capacity-plan-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./capacity-plans/CAPACITY-PLAN-001-foo.md", "audit_path": "./capacity-plans/CAPACITY-PLAN-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./capacity-plans/CAPACITY-PLAN-002-bar.md", "audit_path": "./capacity-plans/CAPACITY-PLAN-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   capacity-plan-audit
Input:
  artefact_paths:  [./team-a/CAPACITY-PLAN-001-something.md, ./team-b/CAPACITY-PLAN-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
