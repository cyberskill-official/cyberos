---
# ── Identity ─────────────────────────────────────────────────────────
name: contract-review-audit
description: >-
  Audits a contract-review@1 artefact against contract_review_rubric@1.0. Validates FM + SEC structure, deviation log completeness vs the playbook, redline severity rationale, GREEN/YELLOW/RED risk classification consistency. Per ABA Model Contract Clauses + ACC Contract Playbook conventions. Audit one or more existing contract-review@1 markdowns against contract-review_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after contract-review-author. Use when user asks to "audit this contract review" or "check the contract review". Do NOT use for "draft a new contract review" (use contract-review-author instead). Audit one or more existing contract-review@1 markdowns against contract-review_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally...
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: contract-review@1
  cyberos-rubric-version: contract-review_rubric@1.0
...
# contract-review-audit — CONTRACT_REVIEW auditor

> Standalone trigger that runs `contract-review_rubric@1.0` against one or
> more existing `contract-review@1` markdowns and writes a sibling
> `contract-review.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`contract-review-author`](../contract-review-author/SKILL.md).

`prompt_revision: contract-review_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `CONTRACT_REVIEW`s."
- "Has `CONTRACT_REVIEW-007` changed since the last audit?"
- "Tell me which `CONTRACT_REVIEW`s would fail acceptance today."

Also invoked automatically by the supervisor when `contract-review-author`'s output envelope sets `next_skill_recommendation: contract-review-audit` (the default chain).
...
audit_rubric_version:            contract-review_rubric@1.0
...
```json
{
  "artefact_paths": ["./contract-reviews/CONTRACT_REVIEW-001-foo.md", "./contract-reviews/CONTRACT_REVIEW-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "contract-review-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./contract-reviews/CONTRACT_REVIEW-001-foo.md", "audit_path": "./contract-reviews/CONTRACT_REVIEW-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./contract-reviews/CONTRACT_REVIEW-002-bar.md", "audit_path": "./contract-reviews/CONTRACT_REVIEW-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   contract-review-audit
Input:
  artefact_paths:  [./team-a/CONTRACT_REVIEW-001-something.md, ./team-b/CONTRACT_REVIEW-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
