---
# ── Identity ─────────────────────────────────────────────────────────
name: data-product-audit
description: >-
  Audit one or more existing data-product@1 markdowns against data-product_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after data-product-author. Use when user asks to "audit this data product" or "check the data product". Do NOT use for "draft a new data product" (use data-product-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: data-product@1
  cyberos-rubric-version: data-product_rubric@1.0
...
depends_on_contracts:
  - id:        data-product
    ...
    pin_path:  cyberos/skill/contracts/data-product/
  - id:        nats-subjects
    version:   v1
...
# data-product-audit — DATA-PRODUCT auditor

> Standalone trigger that runs `data-product_rubric@1.0` against one or
> more existing `data-product@1` markdowns and writes a sibling
> `data-product.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`data-product-author`](../data-product-author/SKILL.md).

`prompt_revision: data-product_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `DATA-PRODUCT`s."
- "Has `DATA-PRODUCT-007` changed since the last audit?"
- "Tell me which `DATA-PRODUCT`s would fail acceptance today."

Also invoked automatically by the supervisor when `data-product-author`'s output envelope sets `next_skill_recommendation: data-product-audit` (the default chain).
...
```json
{
  "artefact_paths": ["./data-products/DATA-PRODUCT-001-foo.md", "./data-products/DATA-PRODUCT-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "data-product-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./data-products/DATA-PRODUCT-001-foo.md", "audit_path": "./data-products/DATA-PRODUCT-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./data-products/DATA-PRODUCT-002-bar.md", "audit_path": "./data-products/DATA-PRODUCT-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   data-product-audit
Input:
  artefact_paths:  [./team-a/DATA-PRODUCT-001-something.md, ./team-b/DATA-PRODUCT-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
