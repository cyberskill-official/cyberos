# `architecture-decision-record-audit` — human summary format
...
```
ADR audit batch complete

Audited N ADR(s):
  - ADR-001-foo.md → audit verdict: pass (0 open, 1 warning, 3 iterations)
  - ADR-002-bar.md → audit verdict: needs_human (2 blocking issues — see HITL_BATCH_REQUEST below)
  - ADR-003-baz.md → audit verdict: pass (0 open, 0 warnings, 1 iteration)

Rubric:        adr_rubric@1.0
