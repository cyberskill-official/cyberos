...
metadata:
  ...
  cyberos-rubric-version: adr_rubric@1.0
...
# architecture-decision-record-audit — ADR auditor

> Standalone trigger that runs `adr_rubric@1.0` against one or
> more existing `architecture-decision-record@1` markdowns and writes a sibling
> `adr.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`architecture-decision-record-author`](../architecture-decision-record-author/SKILL.md).

`prompt_revision: adr_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `ADR`s."
- "Has `ADR-007` changed since the last audit?"
- "Tell me which `ADR`s would fail acceptance today."

Also invoked automatically by the supervisor when `architecture-decision-record-author`'s output envelope sets `next_skill_recommendation: architecture-decision-record-audit` (the default chain).
...
audit_rubric_version:            adr_rubric@1.0
...
```json
{
  "artefact_paths": ["./adrs/ADR-001-foo.md", "./adrs/ADR-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "architecture-decision-record-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./adrs/ADR-001-foo.md", "audit_path": "./adrs/ADR-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./adrs/ADR-002-bar.md", "audit_path": "./adrs/ADR-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   architecture-decision-record-audit
Input:
  artefact_paths:  [./team-a/ADR-001-something.md, ./team-b/ADR-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
