...
## §6  Skill-specific codes (harden-record-author)

| Code | Reason | Recovery |
|---|---|---|
| `HRA-FM-001` | Finding with non-none `operator_prerequisites` classified as agent. | Re-run harden-plan.mjs ≥1.0.1; treat as split/operator. |
| `HRA-FM-002` | Scope breach — changed path outside declared finding scope. | Revert out-of-scope change; record collateral or halt. |
| `HRA-FM-003` | Finding closed without recorded human verdict. | Re-open; obtain verbatim review_verdict (HRD-HITL-3). |
| `HRA-FM-004` | Input report failed inspect-lint. | Route back to inspection-report-author; do not work findings. |
