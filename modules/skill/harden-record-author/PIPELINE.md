...
|---|---|---|
| `harden-record-audit` | Default after every successful write | `next_skill_recommendation: harden-record-audit` in output envelope. |
...
| subject | when |
|---|---|
| `harden_record_author.artefact_written` | After every successful write of `hardening-record@1`. |
| `harden_record_author.hitl_pause` | When a human gate blocks. |
