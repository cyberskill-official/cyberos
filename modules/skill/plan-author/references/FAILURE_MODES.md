...
## §4  Skill-specific codes — `PLAN-AUTHOR-*`

These are not failures of the machinery; they are the skill's own designed halts surfacing as codes so a supervisor can distinguish them from BOOT errors:

| Code | Reason | Recovery |
|---|---|---|
| `PLAN-AUTHOR-001` | Mode detection resolved `ambiguous` (no `.cyberos/`, no git HEAD, substantive uncommitted source). | HALT and ASK — the operator names `greenfield` or `brownfield`; never guessed (SKILL.md §2, INV-001). |
| `PLAN-AUTHOR-002` | Brownfield, but the repo-wide scan is unavailable (`repo-context-map-author` unreachable or failed). | Blocks the decision (INV-002). Operator repairs the scan path; the run resumes at the scan, not at the interview. |
| `PLAN-AUTHOR-003` | The §7 decision gate has no recorded verdict. | Emit NOTHING (INV-007). Resume when the operator answers `APPROVE / REVISE / ABORT`. |
| `PLAN-AUTHOR-004` | `memory-append verify` failed after the BRAIN append. | Do not report success (INV-009). Operator runs `cyberos doctor` on the store; the plan re-emits its rows once the chain verifies. |

## §5  How a failure mode surfaces
...
2. The skill emits the code in the response as a single fenced block:
   ...
   recovery: <suggested next step from §1-§4 above>
   ```
...
4. The operator's next invocation re-enters cleanly — an `ABORT`ed or halted plan run left no partial artefact to reconcile (INV-007).

## §6  Cross-references

- `INVARIANTS.md` (bundle root) — self-audit invariant catalog.
- `references/UNTRUSTED_CONTENT.md` (sibling file) — the wrapping discipline whose marker hits surface through SAFE-003, not through BOOT codes.
