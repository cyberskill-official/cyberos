---
# ── Identity ─────────────────────────────────────────────────────────
name: investor-update-author
description: >-
  Author a monthly investor update for VCs / LPs. Renders TL;DR + metrics dashboard + wins + losses + cash position + asks. Authored by CEO with CFO financial input. Use when user asks to "draft a investor update" or "create the investor update". Do NOT use for "audit existing investor update" (use investor-update-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: investor-update_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['investor-update-audit']; empty list disables chaining
...
# investor-update-author — INVESTOR-UPDATE generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `investor-update@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`investor-update-audit`](../investor-update-audit/SKILL.md) by default.

`prompt_revision: investor-update_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a INVESTOR-UPDATE."
- "Generate v2 of the INVESTOR-UPDATE from the updated source."
- "Draft a INVESTOR-UPDATE for <project>."

If the user asks to *audit an existing INVESTOR-UPDATE*, route to `investor-update-audit` instead. If both, the supervisor chains the two.
...
naming_pattern:                  INVESTOR-UPDATE-{NNN}-{slug}.md
...
```json
{
  ...
  "artefacts_written": [
    {"id": "INVESTOR-UPDATE-001", "path": "./investor-updates/INVESTOR-UPDATE-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "investor-update-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/investor-update/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `investor-update_author.investor-update_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `investor-update-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `investor-update-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   investor-update-author
Input:
