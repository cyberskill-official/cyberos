---
# ── Identity ─────────────────────────────────────────────────────────
name: turnaround-plan-author
description: >-
  Author the post-distress turnaround plan (diagnostic + cash conservation + cost-out + non-core divestiture + creditor renegotiation + new-financing path + KERP/KEIP + milestone sign-off). AlixPartners / FTI / A&M / Alvarez turnaround playbooks. CRO-Restructuring authored. Use when user asks to "draft a turnaround plan" or "create the turnaround plan". Do NOT use for "audit existing turnaround plan" (use turnaround-plan-audit instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: turnaround-plan_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['turnaround-plan-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        turnaround-plan
    ...
    pin_path:  cyberos/skill/contracts/turnaround-plan/
  - id:        nats-subjects
    version:   v1
...
# turnaround-plan-author — TURNAROUND-PLAN generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `turnaround-plan@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`turnaround-plan-audit`](../turnaround-plan-audit/SKILL.md) by default.

`prompt_revision: turnaround-plan_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a TURNAROUND-PLAN."
- "Generate v2 of the TURNAROUND-PLAN from the updated source."
- "Draft a TURNAROUND-PLAN for <project>."

If the user asks to *audit an existing TURNAROUND-PLAN*, route to `turnaround-plan-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        turnaround-plan-author
skill_version:                   1.0.0
prompt_revision:                 turnaround-plan_author@1.0.0
template_version:                turnaround-plan@1   (loaded from cyberos/skill/contracts/turnaround-plan/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  TURNAROUND-PLAN-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./turnaround-plans/",
  "manifest_path": "./turnaround-plans/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "TURNAROUND-PLAN-001", "path": "./turnaround-plans/TURNAROUND-PLAN-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "turnaround-plan-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/turnaround-plan/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `turnaround-plan_author.turnaround-plan_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `turnaround-plan-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `turnaround-plan-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   turnaround-plan-author
Input:
