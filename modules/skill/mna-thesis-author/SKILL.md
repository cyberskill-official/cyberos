...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['mna-thesis-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        mna-thesis
    ...
    pin_path:  cyberos/skill/contracts/mna-thesis/
  - id:        nats-subjects
    version:   v1
...
# mna-thesis-author — MNA-THESIS generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `mna-thesis@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`mna-thesis-audit`](../mna-thesis-audit/SKILL.md) by default.
...
If the user asks to *audit an existing MNA-THESIS*, route to `mna-thesis-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        mna-thesis-author
skill_version:                   1.0.0
prompt_revision:                 mna-thesis_author@1.0.0
template_version:                mna-thesis@1   (loaded from cyberos/skill/contracts/mna-thesis/template.md)
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "mna-thesis-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/mna-thesis/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `mna-thesis-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `mna-thesis-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   mna-thesis-author
Input:
