---
# ── Identity ─────────────────────────────────────────────────────────
name: crisis-comms-playbook-author
description: |
  Author the annual crisis-comms playbook (severity tiers + activation criteria + holding statement library + spokesperson matrix + escalation tree + post-crisis review template). PRSA + Coombs SCCT aligned. CCO-Communications + CSO-Security co-authored.
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['crisis-comms-playbook-audit']; empty list disables chaining
...
# crisis-comms-playbook-author — CRISIS-COMMS-PLAYBOOK generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `crisis-comms-playbook@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`crisis-comms-playbook-audit`](../crisis-comms-playbook-audit/SKILL.md) by default.
...
If the user asks to *audit an existing CRISIS-COMMS-PLAYBOOK*, route to `crisis-comms-playbook-audit` instead. If both, the supervisor chains the two.
...
```
CONTRACT_ECHO
skill_id:                        crisis-comms-playbook-author
skill_version:                   1.0.0
prompt_revision:                 crisis-comms-playbook_author@1.0.0
...
```json
{
  ...
  "hitl_pending": false,
  "next_skill_recommendation": "crisis-comms-playbook-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/crisis-comms-playbook/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
...
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `crisis-comms-playbook-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `crisis-comms-playbook-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   crisis-comms-playbook-author
Input:
