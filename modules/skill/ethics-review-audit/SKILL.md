---
# ── Identity ─────────────────────────────────────────────────────────
name: ethics-review-audit
description: >-
  Audit one or more existing ethics-review@1 markdowns against ethics-review_rubric@1.0. Produces a sibling .audit.md per artefact plus an AUDIT_BATCH_SUMMARY. Halts on needs_human verdicts; resumable on audited_file_sha256. Standalone trigger or chains naturally after ethics-review-author. Use when user asks to "audit this ethics review" or "check the ethics review". Do NOT use for "draft a new ethics review" (use ethics-review-author instead).
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  stage: cross
  cyberos-template: ethics-review@1
  cyberos-rubric-version: ethics-review_rubric@1.0
...
# ethics-review-audit — ETHICS-REVIEW auditor

> Standalone trigger that runs `ethics-review_rubric@1.0` against one or
> more existing `ethics-review@1` markdowns and writes a sibling
> `ethics-review.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`ethics-review-author`](../ethics-review-author/SKILL.md).

`prompt_revision: ethics-review_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `ETHICS-REVIEW`s."
- "Has `ETHICS-REVIEW-007` changed since the last audit?"
- "Tell me which `ETHICS-REVIEW`s would fail acceptance today."

Also invoked automatically by the supervisor when `ethics-review-author`'s output envelope sets `next_skill_recommendation: ethics-review-audit` (the default chain).
...
audit_rubric_version:            ethics-review_rubric@1.0
...
```json
{
  "artefact_paths": ["./ethics-reviews/ETHICS-REVIEW-001-foo.md", "./ethics-reviews/ETHICS-REVIEW-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "ethics-review-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./ethics-reviews/ETHICS-REVIEW-001-foo.md", "audit_path": "./ethics-reviews/ETHICS-REVIEW-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./ethics-reviews/ETHICS-REVIEW-002-bar.md", "audit_path": "./ethics-reviews/ETHICS-REVIEW-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   ethics-review-audit
Input:
  artefact_paths:  [./team-a/ETHICS-REVIEW-001-something.md, ./team-b/ETHICS-REVIEW-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
