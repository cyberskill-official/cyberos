...
description: >-
  Authors a quarterly customer-health review across the ACTIVE customer base — health-score distribution (green/yellow/red by segment+tier), at-risk-account intervention list, expansion-pipeline candidates, advocacy-pipeline (referrals + case studies), CS team utilization, cohort trend analysis. Distinct from churn-analysis (backward on churners). Per Gainsight Customer Success Operating Model + Catalyst CS Ops + TSIA Customer Success benchmarks + Bessemer Cloud Index CS benchmarks. Author a CUSTOMER_HEALTH_REVIEW markdown from source artefact(s). Generates a versioned customer-health-review@1 file under output_dir, with per-claim authority markers and provenance to the source. Use when user asks to "draft a customer health review" or "create the customer health review". Do NOT use for "audit existing customer health review" (use customer-health-review-audit instead). Author a CUSTOMER_HEALTH_REVIEW markdown from source artefact(s). Generates a versioned customer-health-review@1 file under output_dir, with...
license: Apache-2.0
metadata:
  version: 1.0.0
  module: skill
  ...
  cyberos-rubric-target: customer-health-review_rubric@1.0
...
expects:
  ...
  optional_fields:
    ...
    - caller_persona
    - trace_id
    - chain_to               # default ['customer-health-review-audit']; empty list disables chaining
...
depends_on_contracts:
  - id:        customer-health-review
    version:   v1
    purpose:   generation_skeleton
    pin_path:  cyberos/skill/contracts/customer-health-review/
  - id:        nats-subjects
    version:   v1
...
# customer-health-review-author — CUSTOMER_HEALTH_REVIEW generator

> Standalone trigger that turns one or more <input> documents into a
> versioned, audited `customer-health-review@1` markdown. Halts at PLAN approval and
> HITL gates; resumable from `manifest.json` state. Chains naturally
> into [`customer-health-review-audit`](../customer-health-review-audit/SKILL.md) by default.

`prompt_revision: customer-health-review_author@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Turn this <input> into a CUSTOMER_HEALTH_REVIEW."
- "Generate v2 of the CUSTOMER_HEALTH_REVIEW from the updated source."
- "Draft a CUSTOMER_HEALTH_REVIEW for <project>."

If the user asks to *audit an existing CUSTOMER_HEALTH_REVIEW*, route to `customer-health-review-audit` instead. If both, the supervisor chains the two.
...
skill_version:                   1.0.0
prompt_revision:                 customer-health-review_author@1.0.0
template_version:                customer-health-review@1   (loaded from cyberos/skill/contracts/customer-health-review/template.md)
output_dir:                      <from caller>
manifest_path:                   <from caller; default: <output_dir>/manifest.json>
naming_pattern:                  CUSTOMER_HEALTH_REVIEW-{NNN}-{slug}.md
...
```json
{
  ...
  "output_dir": "./customer-health-reviews/",
  "manifest_path": "./customer-health-reviews/manifest.json",
  "batch_size": 3,
...
```json
{
  ...
  "artefacts_written": [
    {"id": "CUSTOMER_HEALTH_REVIEW-001", "path": "./customer-health-reviews/CUSTOMER_HEALTH_REVIEW-001-foo.md", "artefact_hash": "<sha256>", "status": "PASS|HITL_PAUSE|EXHAUSTED"}
  ],
  "amendments_pending": ["AMD-NNN", "..."],
  "hitl_pending": false,
  "next_skill_recommendation": "customer-health-review-audit"
}
```
...
- **W2 GENERATE** — render the artefact by adapting the template loaded from `cyberos/skill/contracts/customer-health-review/template.md` (declared via `depends_on_contracts:`) to this artefact's source_refs, applying anti-fabrication rules (`references/ANTI_FABRICATION.md`).
- **W3 WRITE** — `write_file(artefact.file_path, body)`. Compute `artefact_hash`. Append one `artefact_write` row to `genie.action_log`.
- **W4 EMIT EVENT** — publish a NATS subject `customer-health-review_author.customer-health-review_written` carrying `(artefact_id, artefact_path, artefact_hash)`.
- **W5 ROUTE** — depending on whether the chained audit is wired:
  - If chained to `customer-health-review-audit`: invoke it with the just-written artefact's path. Forward its `overall_status` into `artefacts[X].status`.
...
- `PIPELINE.md` — how this skill chains to `customer-health-review-audit` (and other downstream consumers).

## §10  How to use this skill — direct invocation example

```
Persona: cuo-cpo
Skill:   customer-health-review-author
Input:
