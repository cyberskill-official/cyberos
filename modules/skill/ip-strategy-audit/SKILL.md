---
# ── Identity ─────────────────────────────────────────────────────────
name: ip-strategy-audit
description: Audits an ip-strategy@1 artefact against ip_strategy_rubric@1.0. Validates FM + SEC structure, portfolio-inventory completeness, posture-rationale internal consistency, FTO assessment coverage, and enforcement-budget feasibility. Per WIPO IP framework + USPTO MPEP.
  Audit one or more existing ip-strategy@1 markdowns against
...
metadata:
  ...
  module: skill
  stage: cross
  cyberos-template: ip-strategy@1
...
depends_on_contracts:
  - id:        ip-strategy
    ...
    pin_path:  cyberos/skill/contracts/ip-strategy/
  - id:        nats-subjects
    version:   v1
...
# ip-strategy-audit — IP_STRATEGY auditor

> Standalone trigger that runs `ip-strategy_rubric@1.0` against one or
> more existing `ip-strategy@1` markdowns and writes a sibling
> `ip-strategy.audit.md` per item. Halts on `needs_human` verdicts via
> the standard Question primitive; resumable on `audited_file_sha256`.
> Chains naturally after [`ip-strategy-author`](../ip-strategy-author/SKILL.md).

`prompt_revision: ip-strategy_audit@1.0.0`

## When to invoke this skill

CUO routes a request here when the user wants to:

- "Audit these existing `IP_STRATEGY`s."
- "Has `IP_STRATEGY-007` changed since the last audit?"
- "Tell me which `IP_STRATEGY`s would fail acceptance today."

Also invoked automatically by the supervisor when `ip-strategy-author`'s output envelope sets `next_skill_recommendation: ip-strategy-audit` (the default chain).
...
skill_version:                   1.0.0
prompt_revision:                 ip-strategy_audit@1.0.0
template_version:                ip-strategy@1   (loaded from cyberos/skill/contracts/ip-strategy/template.md)
audit_rubric_version:            ip-strategy_rubric@1.0
...
```json
{
  "artefact_paths": ["./ip-strategys/IP_STRATEGY-001-foo.md", "./ip-strategys/IP_STRATEGY-002-bar.md"],
  "caller_persona": "cuo-cpo",
  "trace_id": "<uuid>",
  "upstream_context": {
    "from_skill": "ip-strategy-author",
...
```json
{
  ...
  "exit_code": 1,
  "per_artefact": [
    {"artefact_path": "./ip-strategys/IP_STRATEGY-001-foo.md", "audit_path": "./ip-strategys/IP_STRATEGY-001-foo.audit.md", "status": "pass", "iterations": 1, "audited_file_sha256": "<hex>"},
    {"artefact_path": "./ip-strategys/IP_STRATEGY-002-bar.md", "audit_path": "./ip-strategys/IP_STRATEGY-002-bar.audit.md", "status": "needs_human", "iterations": 3, "audited_file_sha256": "<hex>"}
  ],
  "hitl_required": true,
...
```
Persona: cuo-cpo
Skill:   ip-strategy-audit
Input:
  artefact_paths:  [./team-a/IP_STRATEGY-001-something.md, ./team-b/IP_STRATEGY-018-other.md]
  caller_persona:  cuo-cpo
  trace_id:        <uuid>
