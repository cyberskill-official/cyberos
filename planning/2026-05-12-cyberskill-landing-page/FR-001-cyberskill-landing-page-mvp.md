...
provenance:
  source: chat
  source_ref: 2026-05-12 cyberos chain run --pitch "CyberSkill landing-page…"
  confidence: 0.85
acceptance_criteria:
...
tasks:
  - id: FR-001-T-01
    ...
    acceptance_test:
      shell: "cd landing-page && pnpm install && pnpm dev --port 4321 & sleep 5 && curl -s http://localhost:4321/ | grep -q 'Hello CyberSkill' && kill %1"
    sizing: S
    dependencies: []
    parallelisable: true
    assignable_to: [human]
  ...
  - id: FR-001-T-02
    ...
    acceptance_test:
      shell: "find src/content/copy/en -name '*.md' | xargs -I {} grep -L '—\\|leverage\\|robust\\|ensure\\|comprehensive\\|seamless\\|delve' {} | wc -l | grep -E '^[5-9]|[1-9][0-9]+$'"
    sizing: M
    dependencies: [FR-001-T-01]
    parallelisable: true
    assignable_to: [human]
    estimated_hours: 4.0
    status: draft
  ...
  - id: FR-001-T-03
    ...
    acceptance_test:
      shell: "[ $(find src/content/copy/vi -name '*.md' | wc -l) -eq $(find src/content/copy/en -name '*.md' | wc -l) ]"
    sizing: M
  ...
  - id: FR-001-T-04
    ...
    acceptance_test:
      shell: "pnpm build && grep -q 'Start a conversation' dist/index.html"
    sizing: M
    dependencies: [FR-001-T-01]
  ...
  - id: FR-001-T-05
    ...
    acceptance_test:
      shell: "pnpm test tests/contact-form.spec.ts"
    sizing: M
  ...
  - id: FR-001-T-06
    ...
    acceptance_test:
      shell: "pnpm build && grep -q 'application/ld+json' dist/index.html && grep -q 'plausible' dist/index.html && [ -f dist/sitemap-index.xml ]"
    sizing: M
    dependencies: [FR-001-T-04]
    parallelisable: true
    assignable_to: [ai-agent]
  ...
  - id: FR-001-T-07
    ...
    acceptance_test:
      shell: "pnpm dlx @lhci/cli autorun --collect.numberOfRuns=1 --assert.assertions.categories:performance=0.9 --assert.assertions.categories:accessibility=0.9"
    sizing: M
    dependencies: [FR-001-T-04, FR-001-T-05, FR-001-T-06]
    parallelisable: false
    assignable_to: [human]
    estimated_hours: 3.0
    status: draft
  ...
  - id: FR-001-T-08
    ...
    acceptance_test:
      shell: "curl -s -o /dev/null -w '%{http_code}' https://cyberskill.world/ | grep -q 200 && curl -s -o /dev/null -w '%{http_code}' https://cyberskill.world/vi/ | grep -q 200"
    sizing: S
    dependencies: [FR-001-T-07]
    parallelisable: false
    assignable_to: [human]
    estimated_hours: 2.0
    status: draft
...
This FR was drafted from the operator's chat pitch ("CyberSkill landing-page MVP: marketing site for our Vietnam-based software consultancy" + the 10 acceptance criteria provided). Authority marker: `llm-explicit` on the technical task breakdown; `human-confirmed` on the company-facts paragraphs (slogan, founding year, address, registered name) which trace back to `memories/people/PERSON-001-stephen-cheng.md` and `memories/facts/FACT-001-cyberskill-company.md`.

`source_ref`: `2026-05-12 cyberos chain run --pitch "CyberSkill landing-page MVP..." --profile solo`.

Untrusted-content discipline (AGENTS.md §4.2): the pitch text was treated as operator-supplied input; no external untrusted content fed into this draft.

## Tasks

See the `tasks:` frontmatter list above. 8 tasks total: 2 S + 6 M. 3 AI-assignable, 5 human-assignable, 1 either. Dependency DAG: T-01 → {T-02, T-04, T-06} → T-03 (parallel of T-02), T-04 → T-05; {T-04, T-05, T-06} → T-07 → T-08. Parallelisable on T-02 / T-03 / T-04 / T-06 in the early phase.
