...
## FR-001-T-01 — Set up Slack app + OAuth scopes
...
```shell
curl -s -X POST https://slack.com/api/auth.test -H 'Authorization: Bearer $SLACK_BOT_TOKEN' | jq -e '.ok == true'
```
...
## FR-001-T-02 — Vector-index HR policy docs
...
```shell
python3 scripts/reindex.py && python3 -c 'import chromadb; c=chromadb.PersistentClient(path="data/hr-policies.chromadb"); print(c.list_collections())' | grep hr-policies
```
...
## FR-001-T-03 — Implement Q&A handler
...
## FR-001-T-04 — Add audit logger + rate limiter
...
```shell
pytest tests/test_audit.py tests/test_ratelimit.py -v
```
...
## FR-001-T-05 — Build test corpus + run accuracy eval
...
```shell
python3 scripts/eval_accuracy.py | grep -E 'PASS|24/30|25/30|26/30|27/30|28/30|29/30|30/30'
```
...
## FR-001-T-06 — Deploy + production smoke test
